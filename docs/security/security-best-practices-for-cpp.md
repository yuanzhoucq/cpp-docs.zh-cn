...
此编译器选项将激活报告潜在安全问题（比如缓冲区溢出、未初始化的内存、null 指针取消引用和内存泄漏）的代码分析。 此选项默认已关闭。 有关详细信息，请参阅[C/c + + 代码分析概述](/cpp/code-quality/code-analysis-for-c-cpp-overview)。
