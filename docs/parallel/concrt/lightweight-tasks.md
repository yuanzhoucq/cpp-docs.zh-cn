...
ms.locfileid: "77141854"
---
# <a name="lightweight-tasks"></a>轻量级任务

本文档介绍并发运行时中轻量级任务的角色。 *轻量级任务*是直接从 `concurrency::Scheduler` 或 `concurrency::ScheduleGroup` 对象进行计划的任务。 轻量级任务类似于你向 Windows API [CreateThread](/windows/win32/api/processthreadsapi/nf-processthreadsapi-createthread)函数提供的功能。 因此，当您改编现有代码以使用并发运行时的计划功能时，轻量级任务非常有用。 并发运行时本身使用轻量级任务来计划异步代理并在异步消息块之间发送消息。
