...
使用[concurrency:: CurrentScheduler:: create](reference/currentscheduler-class.md#create)、 [Concurrency:: 调度器:: create](reference/scheduler-class.md#create)或[concurrency:: SetDefaultSchedulerPolicy](reference/scheduler-class.md#setdefaultschedulerpolicy)方法创建计划程序实例时, 提供[并发::SchedulerPolicy](../../parallel/concrt/reference/schedulerpolicy-class.md)对象, 其中包含指定计划程序行为的键值对的集合。 `SchedulerPolicy`构造函数采用可变数量的参数。 第一个参数是要指定的策略元素的数目。 其余的参数是每个策略元素的键值对。 下面的示例创建一个`SchedulerPolicy`对象, 该对象指定三个策略元素。 运行时对未指定的策略键使用默认值。
...
每个计划程序在计划任务时使用其自己的策略。 与一个计划程序关联的策略不影响任何其他计划程序的行为。 此外, 在创建`Scheduler`对象后, 将无法更改计划程序策略。
...
若要检查使用特定计划程序策略来控制计划程序行为的示例, 请[参阅如何:指定特定的计划](../../parallel/concrt/how-to-specify-specific-scheduler-policies.md)程序[策略以及如何:创建使用特定计划程序策略](../../parallel/concrt/how-to-create-agents-that-use-specific-scheduler-policies.md)的代理。
