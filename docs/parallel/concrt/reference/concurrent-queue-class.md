...
|名称|说明|  
|----------|-----------------|  
|[clear](#clear)|清除并发队列，销毁所有当前排队元素。 此方法不是并发安全的。|  
|[empty](#empty)|测试是否并发队列为空时刻调用此方法。 此方法是并发安全的。|  
|[get_allocator](#get_allocator)|返回用于构造并发队列的分配器的副本。 此方法是并发安全的。|  
|[push](#push)|已重载。 排入队列的并发队列结尾处的项。 此方法是并发安全的。|  
|[try_pop](#try_pop)|如果找到一个对象中取出某项从队列。 此方法是并发安全的。|  
|[unsafe_begin](#unsafe_begin)|已重载。 返回一个迭代器类型的`iterator`或`const_iterator`到并发队列开头。 此方法不是并发安全的。|  
|[unsafe_end](#unsafe_end)|已重载。 返回一个迭代器类型的`iterator`或`const_iterator`并发队列的末尾。 此方法不是并发安全的。|  
...
### <a name="remarks"></a>备注  
 如果项目已成功取消排队，该参数`_Dest`接收取消排队的值保留在队列中的原始值将被销毁，并且此函数将返回`true`。 如果没有要取消排队的项，则此函数将返回`false`不会阻塞，并且的内容的情况下`_Dest`是未定义的参数。  
