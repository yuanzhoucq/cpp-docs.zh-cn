...
|名称|描述|  
|----------|-----------------|  
|[concurrent_unordered_multimap 构造函数](#ctor)|已重载。 构造并发无序多重映射。|  
...
|[insert 方法](#insert)|已重载。 添加到元素`concurrent_unordered_multimap`对象。|  
|[key_eq 方法](#key_eq)|返回存储的相等性比较函数对象。|  
|[swap 方法](#swap)|交换两个内容`concurrent_unordered_multimap`对象。 此方法不是并发安全的。|  
|[unsafe_erase 方法](#unsafe_erase)|已重载。 从其中移除元素`concurrent_unordered_multimap`指定位置处。 此方法不是并发安全的。|  
  
### <a name="public-operators"></a>公共运算符  
  
|名称|描述|  
|----------|-----------------|  
|[运算符 = 运算符](#operator_eq)|已重载。 另一种内容分配`concurrent_unordered_multimap`对象传递给它。 此方法不是并发安全的。|  
