...
ms.locfileid: "43678505"
---
# <a name="concurrentunorderedmultiset-class"></a>concurrent_unordered_multiset 类
`concurrent_unordered_multiset`类是控制元素的类型 K.变长序列的并发安全容器序列表示方式支持并发安全追加、 元素访问、 迭代器访问和迭代器遍历操作。  
...
#### <a name="parameters"></a>参数  
 ...
 `_Allocator_type`  
 表示存储的分配器对象封装有关分配和解除分配的并发向量内存的详细信息的类型。 此参数为可选参数，默认值为 `std::allocator<K>`。  
...
|[insert](#insert)|已重载。 将元素添加到`concurrent_unordered_multiset`对象。|  
|[key_eq](#key_eq)|存储的相等比较函数对象。|  
|[swap](#swap)|交换两个内容`concurrent_unordered_multiset`对象。 此方法不是并发安全的。|  
|[unsafe_erase](#unsafe_erase)|已重载。 从其中移除元素`concurrent_unordered_multiset`指定位置处。 此方法不是并发安全的。|  
  
### <a name="public-operators"></a>公共运算符  
  
|名称|描述|  
|----------|-----------------|  
|[operator=](#operator_eq)|已重载。 另一种内容分配`concurrent_unordered_multiset`到此对象。 此方法不是并发安全的。|  
...
##  <a name="operator_eq"></a> 运算符 = 
 ...
 另一种内容分配`concurrent_unordered_multiset`到此对象。 此方法不是并发安全的。  
