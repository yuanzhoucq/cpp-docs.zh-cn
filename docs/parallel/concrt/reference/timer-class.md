...
|名称|描述|  
|----------|-----------------|  
|[timer](#ctor)|已重载。 构造`timer`将在指定时间间隔后激发给定的消息的消息块。|  
...
|名称|描述|  
|----------|-----------------|  
|[pause](#pause)|停止`timer`消息块。 如果它是一个重复`timer`消息块，它可以重新启动通过后续`start()`调用。 对于非重复计时器，这具有相同的效果`stop`调用。|  
