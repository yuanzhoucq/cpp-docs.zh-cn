...
|Id|返回当前上下文的唯一标识符，该标识符在当前上下文所属的计划程序中是唯一的。|
...
|[产生](#yield)|让出执行，这样另一个上下文可进行执行。 如果没有其他上下文可接受执行，则计划程序可将执行让出给另一个操作系统线程。|
...
## <a name="block"></a><a name="block"></a>模块
...
`Block`调用或将调用方法后，必须将其与另一个执行上下文中的[解除阻止](#unblock)方法的调用配对，以便它再次运行。 请注意，在代码为另一个线程发布其上下文以便能够调用方法的点 `Unblock` 和执行实际方法调用的点之间存在关键时间间隔 `Block` 。 在此时间段内，您不能调用任何可能会反过来阻止或取消阻止其自身原因（例如获取一个锁）的方法。 对 `Block` 和方法的调用 `Unblock` 不会跟踪阻止和取消阻止的原因。 只有一个对象应具有对的所有权 `Block` -  `Unblock` 。

此方法可能会引发各种异常，包括[scheduler_resource_allocation_error](scheduler-resource-allocation-error-class.md)。

## <a name="context"></a><a name="dtor"></a>~ 上下文
...
如果上下文当前正在虚拟处理器上执行，则为当前正在其上执行上下文的虚拟处理器的标识符;否则为值 `-1` 。
...
如果当前上下文附加到计划程序，则当前上下文在当前上下文所属的计划程序中是唯一的，它是当前上下文的标识符;否则为值 `-1` 。
...
如果将计划程序附加到调用上下文，并且任务组在该上下文上执行内联任务，则指示该任务组是否正在活动取消（或将很快）;否则为值 **`false`** 。
...
## <a name="oversubscribe"></a><a name="oversubscribe"></a>过度
...
如果 **`true`** 为，则指示应在超额订阅的持续时间内添加额外的虚拟处理器。 如果为 **`false`** ，则表示超额订阅应结束，并且应删除以前添加的虚拟处理器。
...
如果当前上下文附加到计划程序并处理计划组，则为当前上下文正在处理的计划程序组的标识符;否则为值 `-1` 。

## <a name="unblock"></a><a name="unblock"></a>阻挡
...
在对 `Unblock` [Block](#block)方法的相应调用之前调用方法是完全合法的。 只要对和方法的调用 `Block` `Unblock` 都是正确配对的，运行时就会正确地处理任意排序的自然争用。 在 `Unblock` 调用前发出的调用会 `Block` 否定调用的效果 `Block` 。

此方法可能会引发多个异常。 如果上下文尝试 `Unblock` 自行调用方法，将引发[context_self_unblock](context-self-unblock-class.md)异常。 如果对 `Block` 和 `Unblock` 的调用没有正确配对（例如，对 `Unblock` 当前正在运行的上下文进行了两次调用），则会引发[context_unblock_unbalanced](context-unblock-unbalanced-class.md)异常。

请注意，在代码为另一个线程发布其上下文以便能够调用方法的点 `Unblock` 和执行实际方法调用的点之间存在关键时间间隔 `Block` 。 在此时间段内，您不能调用任何可能会反过来阻止或取消阻止其自身原因（例如获取一个锁）的方法。 对 `Block` 和方法的调用 `Unblock` 不会跟踪阻止和取消阻止的原因。 只有一个对象应具有 `Block` 和对的所有权 `Unblock` 。
...
如果当前上下文附加到计划程序，则为当前上下文正在其上执行的虚拟处理器的标识符;否则为值 `-1` 。
