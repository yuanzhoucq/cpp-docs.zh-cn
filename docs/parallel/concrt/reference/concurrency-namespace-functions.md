...
|[EnableTracing](#enabletracing)|[忙](#free)|[GetExecutionContextId](#getexecutioncontextid)|
...
如果其中一个任务被取消或引发异常，则返回的任务将在 "已取消" 状态下提前完成，如果遇到，则会引发异常（如果有），如果对该任务调用[task：： get](task-class.md#get)或 `task::wait`，将引发该异常。
