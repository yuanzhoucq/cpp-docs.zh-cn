...
|名称|说明|  
|----------|-----------------|  
|[operator=](#operator_eq)|已重载。 另一种内容分配`concurrent_unordered_set`对象传递给它。 此方法不是并发安全的。|  
...
### <a name="return-value"></a>返回值  
 前两个成员函数返回一个迭代器，指定已移除，任何元素之外保留的第一个元素或[结束](#end)（); 如果不存在此元素。 第三个成员函数返回它会移除的元素的数目。  
  
### <a name="remarks"></a>备注  
 第一个成员函数删除指向的元素`_Where`。 第二个成员函数范围内移除的元素 [ `_Begin`， `_End`)。  
  
 第三个成员函数中由分隔的范围内移除的元素[equal_range](#equal_range)(KVal)。  
