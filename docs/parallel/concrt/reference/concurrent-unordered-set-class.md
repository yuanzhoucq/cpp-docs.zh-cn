---
title: "concurrent_unordered_set 类 |Microsoft 文档"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
`concurrent_unordered_set`类是并发安全的容器，用于控制的不同长序列的元素类型 k。支持并发安全的方式表示该序列追加、 元素访问、 迭代器访问和迭代器遍历操作。  
...
|名称|描述|  
|----------|-----------------|  
|[concurrent_unordered_set 构造函数](#ctor)|已重载。 构造并发无序的集。|  
...
|[insert 方法](#insert)|已重载。 添加到元素`concurrent_unordered_set`对象。|  
|[key_eq 方法](#key_eq)|返回存储的相等性比较函数对象。|  
|[swap 方法](#swap)|交换两个内容`concurrent_unordered_set`对象。 此方法不是并发安全的。|  
|[unsafe_erase 方法](#unsafe_erase)|已重载。 从其中移除元素`concurrent_unordered_set`指定位置处。 此方法不是并发安全的。|  
  
### <a name="public-operators"></a>公共运算符  
  
|名称|说明|  
|----------|-----------------|  
|[运算符 = 运算符](#operator_eq)|已重载。 另一种内容分配`concurrent_unordered_set`对象传递给它。 此方法不是并发安全的。|  
...
##  <a name="a-nameinserta-insert"></a><a name="insert"></a>插入 
 ...
 添加到元素`concurrent_unordered_set`对象。  
...
##  <a name="a-nameoperatoreqa-operator"></a><a name="operator_eq"></a>运算符 = 
 ...
 另一种内容分配`concurrent_unordered_set`对象传递给它。 此方法不是并发安全的。  
...
### <a name="return-value"></a>返回值  
 参考这`concurrent_unordered_set`对象。  
