...
> [!NOTE]
>  C\+\+ 语言标准将以下划线 \(`_`\) 字符开始的标识符保留给实现（如库）使用。  不要在代码中使用这些名称。  不保证其名称遵循此约定的代码元素的行为，并且这类行为在将来发布的版本中可能会有所更改。  基于上述原因，并发运行时文档中将省略这类代码元素。  
  
## 本节内容  
 [concurrency 命名空间](../../../parallel/concrt/reference/concurrency-namespace.md)  
 `concurrency` 命名空间提供了一些类和函数，可以利用它们访问并发运行时（针对 C\+\+ 的并发编程框架）。  有关更多信息，请参见[并发运行时](../../../parallel/concrt/concurrency-runtime.md)。  
