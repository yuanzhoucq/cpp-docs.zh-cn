...
运行时使用的默认计划程序，如果未指定`_PScheduler`或`_PGroup`参数。
...
`true` 如果代理已被取消，`false`否则为。 如果它已启动运行或已完成，代理不能取消。
...
`true` 如果代理移至`agent_done`状态，`false`否则为。 已取消的代理不能移动到`agent_done`状态。
