...
- 将 lambda 表达式传递给任务时，不要捕获通过引用在堆栈上分配的变量。

- 对在 lambda 表达式中捕获的变量是显式的，以便可以通过值与引用来确定要捕获的内容。 因此我们建议不要将 `[=]` 或 `[&]` 选项用于 lambda 表达式。

一种常用模式是将延续链中的一个任务分配给变量，而另一个任务读取该变量。 不能按值捕获，因为每个延续任务都将包含变量的不同副本。 对于堆栈分配的变量，也无法通过引用捕获，因为该变量可能不再有效。
...
## <a name="the-task-class"></a><a name="task-class"></a>Task 类
...
`task` 是模板类。 类型参数 `T` 是由任务生成的结果的类型。 如果任务不返回值，则此类型可以为 **`void`** 。 `T`不能使用 **`const`** 修饰符。

当你创建任务时，你将提供执行任务正文的*工作函数*。 此工作函数采用 lambda 函数、函数指针或函数对象的形式。 若要等待任务完成而不获取结果，请调用[concurrency：： task：： wait](reference/task-class.md#wait)方法。 `task::wait`方法返回[concurrency：： task_status](reference/concurrency-namespace-enums.md#task_group_status)值，该值描述任务是否已完成或已取消。 若要获取任务的结果，请调用[concurrency：： task：： get](reference/task-class.md#get)方法。 此方法调用 `task::wait` 以等待任务完成，因此会在结果可用之前阻止当前线程的执行。
...
> [!TIP]
> 若要了解特定于 UWP 应用中的任务的详细信息，请参阅[c + + 中的异步编程](/windows/uwp/threading-async/asynchronous-programming-in-cpp-universal-windows-platform-apps)和[在适用于 UWP 应用的 c + + 中创建异步操作](../../parallel/concrt/creating-asynchronous-operations-in-cpp-for-windows-store-apps.md)。
...
- 提供有关应如何计划延续的提示。 （这仅适用于通用 Windows 平台（UWP）应用。 有关详细信息，请参阅[在适用于 UWP 应用的 c + + 中创建异步操作](../../parallel/concrt/creating-asynchronous-operations-in-cpp-for-windows-store-apps.md)。
...
## <a name="value-based-versus-task-based-continuations"></a><a name="value-versus-task"></a>基于值与基于任务的继续

对于其返回类型是 `T` 的 `task` 对象，可以向其延续任务提供 `T` 或 `task<T>` 类型的值。 采用类型的延续 `T` 称为*基于值的延续*。 基于值的延续计划在先行任务完成而未出现错误并且未取消时执行。 采用类型 `task<T>` 作为参数的继续称为*基于任务的继续符*。 基于任务的延续始终计划为在先行任务完成时执行，甚至是在先行任务取消或引发异常时执行。 随后然后调用 `task::get` 以获取先行任务的结果。 如果前面的任务已取消， `task::get` 则会引发[concurrency：： task_canceled](../../parallel/concrt/reference/task-canceled-class.md)。 如果先行任务引发了异常，则 `task::get` 会再次引发该异常。 基于任务的延续在先行任务取消时不会标记为已取消。
...
### <a name="the-when_all-function"></a><a name="when-all"></a>When_all 函数
...
可将延续与 `when_all` 结合使用以在任务集完成之后执行操作，这十分常见。 下面的示例修改了上一个示例，以便打印每个产生结果的三个任务的总和 **`int`** 。
...
下面是一个实用函数，可用于确保程序观察到所有异常。 对于处于提供的范围内的每个任务，`observe_all_exceptions` 会触发再次引发的任何异常，然后会吞并该异常。

[!code-cpp[concrt-eh-when_all#1](../../parallel/concrt/codesnippet/cpp/task-parallelism-concurrency-runtime_10.cpp)]

请考虑使用 c + + 和 XAML 的 UWP 应用，并将一组文件写入磁盘。 下面的示例演示如何使用 `when_all` 和 `observe_all_exceptions` 确保该程序观察到所有异常。
...
1. 在 MainPage 中，将这些前向声明添加到 **`private`** `MainPage` 类声明的部分。
...
### <a name="the-when_any-function"></a><a name="when-any"></a>When_any 函数
...
> [!TIP]
> 与一样 `when_all` ，是非 `when_any` 阻塞的，可以安全地在 ASTA 线程上的 UWP 应用中调用。
...
实现此目的的两种方法是使用延续，或启动任务并等待任务的工作函数内的事件。 但是在某些情况下，无法使用以上方法之一。 例如，若要创建延续，必须具有先行任务。 但是，如果你没有前面的任务，则可以创建*任务完成事件*，稍后将完成事件链接到前面的任务（如果任务可用）。 此外，因为正在等待的任务也会阻止线程，所以可以使用任务完成事件在异步操作完成时执行工作，从而释放线程。

[Concurrency：： task_completion_event](../../parallel/concrt/reference/task-completion-event-class.md)类有助于简化这类任务。 与 `task` 类一样，类型参数 `T` 是由任务生成的结果的类型。 如果任务不返回值，则此类型可以为 **`void`** 。 `T`不能使用 **`const`** 修饰符。 通常会将 `task_completion_event` 对象提供给在它的值可用时向它发信号的线程或任务。 同时，一个或多个任务设置为该事件的侦听器。 设置事件时，侦听器任务完成，其延续会计划运行。

有关使用 `task_completion_event` 来实现在延迟后完成的任务的示例，请参阅[如何：创建在延迟后完成的任务](../../parallel/concrt/how-to-create-a-task-that-completes-after-a-delay.md)。
...
> [!IMPORTANT]
> PPL 还定义[concurrency：:p arallel_invoke](reference/concurrency-namespace-functions.md#parallel_invoke)算法，该算法使用 `structured_task_group` 类并行执行一组任务。 因为 `parallel_invoke` 算法具有更简洁的语法，所以我们建议尽可能使用它而不是 `structured_task_group` 类。 主题[并行算法](../../parallel/concrt/parallel-algorithms.md) `parallel_invoke` 更详细地介绍。
...
## <a name="comparing-task_group-to-structured_task_group"></a><a name="comparing-groups"></a>比较 task_group 与 structured_task_group
...
`task_group`调用[concurrency：： task_group：： wait](reference/task-group-class.md#wait)或[concurrency：： task_group：： run_and_wait](reference/task-group-class.md#run_and_wait)方法之后，可以在对象上运行其他任务。 相反，如果在 `structured_task_group` 调用[concurrency：： structured_task_group：： wait](reference/structured-task-group-class.md#wait)或[concurrency：： structured_task_group：： run_and_wait](reference/structured-task-group-class.md#run_and_wait)方法之后对对象运行其他任务，则该行为是不确定的。
...
有关演示如何使用算法的完整示例 `parallel_invoke` ，请参阅[如何：使用 Parallel_invoke 编写并行排序例程](../../parallel/concrt/how-to-use-parallel-invoke-to-write-a-parallel-sort-routine.md)和[如何：使用 parallel_invoke 执行并行操作](../../parallel/concrt/how-to-use-parallel-invoke-to-execute-parallel-operations.md)。 有关使用 `task_group` 类实现异步先期实现的完整示例，请参阅[演练：实现先期](../../parallel/concrt/walkthrough-implementing-futures.md)情况。

## <a name="robust-programming"></a><a name="robust"></a>可靠的编程
...
|[如何：创建在延迟一段时间后完成的任务](../../parallel/concrt/how-to-create-a-task-that-completes-after-a-delay.md)|演示如何使用 `task` 、 `cancellation_token_source` 、 `cancellation_token` 和 `task_completion_event` 类来创建一个在延迟后完成的任务。|
|[演练：实现先期备货](../../parallel/concrt/walkthrough-implementing-futures.md)|演示如何在并发运行时中将现有功能合并为执行更多操作的功能。|
