...
- [不要对小型循环体进行并行化](#small-loops)

- [表示最高可能级别的并行](#highest)

- [使用 parallel_invoke 来解决分治问题](#divide-and-conquer)

- [使用取消或异常处理来中断并行循环](#breaking-loops)

- [了解取消和异常处理如何影响对象销毁](#object-destruction)

- [不要在并行循环中反复阻止](#repeated-blocking)

- [当取消并行工作时，不要执行停滞操作](#blocking)
...
## <a name="do-not-parallelize-small-loop-bodies"></a><a name="small-loops"></a>不要并行化小型循环主体
...
## <a name="express-parallelism-at-the-highest-possible-level"></a><a name="highest"></a>最高可能级别的快速并行度
...
## <a name="use-parallel_invoke-to-solve-divide-and-conquer-problems"></a><a name="divide-and-conquer"></a>使用 parallel_invoke 解决除法和治问题
...
有关此示例的完整版本，请参阅[如何：使用 Parallel_invoke 编写并行排序例程](../../parallel/concrt/how-to-use-parallel-invoke-to-write-a-parallel-sort-routine.md)。 有关该算法的详细信息 `parallel_invoke` ，请参阅[并行算法](../../parallel/concrt/parallel-algorithms.md)。

[[顶部](#top)]

## <a name="use-cancellation-or-exception-handling-to-break-from-a-parallel-loop"></a><a name="breaking-loops"></a>使用取消或异常处理来中断并行循环
...
尽管异常处理是一种比取消机制效率更低的取消并行工作的方法，但在某些情况中异常处理也非常适用。 例如，下面的方法，`for_all`，以递归方式在 `tree` 结构的每个节点上执行工作函数。 在此示例中， `_children` 数据成员是包含对象的[std：： list](../../standard-library/list-class.md) `tree` 。
...
## <a name="understand-how-cancellation-and-exception-handling-affect-object-destruction"></a><a name="object-destruction"></a>了解取消和异常处理如何影响对象析构
...
尽管这种模式在其自身上没有任何问题，但建议使用下面并行运行两个任务的代码。 第一个任务创建 `Container` 对象，第二个任务取消整个任务。 为了举例说明，该示例使用两个[concurrency：： event](../../parallel/concrt/reference/event-class.md)对象，以确保在 `Container` 创建对象之后取消操作，并确保在 `Container` 取消操作发生之后销毁对象。
...
## <a name="do-not-block-repeatedly-in-a-parallel-loop"></a><a name="repeated-blocking"></a>不要在并行循环中重复阻塞
...
请考虑以下示例，该示例在循环的每次迭代中调用[concurrency：： send](reference/concurrency-namespace-functions.md#send)函数 `parallel_for` 。 由于 `send` 以协作方式停滞，所以每次调用 `send` 时运行时都会创建一个新线程来运行其他工作。

[!code-cpp[concrt-repeated-blocking#1](../../parallel/concrt/codesnippet/cpp/best-practices-in-the-parallel-patterns-library_12.cpp)]

我们建议你重构代码来避免这种模式。 在此示例中，可以通过 `send` 在序列循环中调用来避免创建其他线程 **`for`** 。

[[顶部](#top)]

## <a name="do-not-perform-blocking-operations-when-you-cancel-parallel-work"></a><a name="blocking"></a>取消并行工作时不要执行阻止操作
...
请看下面定义 `parallel_find_answer` 函数的示例，该函数搜索所提供的满足提供的谓词函数的数组的元素。 当谓词函数返回时 **`true`** ，并行工作函数将创建一个 `Answer` 对象并取消整个任务。
...
## <a name="do-not-write-to-shared-data-in-a-parallel-loop"></a><a name="shared-writes"></a>不要在并行循环中写入共享数据
...
有关此示例的完整版本，请参阅[如何：使用可组合来提高性能](../../parallel/concrt/how-to-use-combinable-to-improve-performance.md)。 有关类的详细信息 `combinable` ，请参阅[并行容器和对象](../../parallel/concrt/parallel-containers-and-objects.md)。

[[顶部](#top)]

## <a name="when-possible-avoid-false-sharing"></a><a name="false-sharing"></a>如果可能，请避免错误共享
...
如果必须在任务之间共享数据，建议使用[concurrency：：可组合](../../parallel/concrt/reference/combinable-class.md)类。 `combinable` 类以不太可能发生错误共享的方式创建线程本地变量。 有关类的详细信息 `combinable` ，请参阅[并行容器和对象](../../parallel/concrt/parallel-containers-and-objects.md)。

[[顶部](#top)]

## <a name="make-sure-that-variables-are-valid-throughout-the-lifetime-of-a-task"></a><a name="lifetime"></a>确保变量在任务的整个生存期内有效
...
[如何：使用 parallel_invoke 编写并行排序例程](../../parallel/concrt/how-to-use-parallel-invoke-to-write-a-parallel-sort-routine.md)<br/>
[如何：使用取消中断并行循环](../../parallel/concrt/how-to-use-cancellation-to-break-from-a-parallel-loop.md)<br/>
