...
## <a name="event-tracing"></a>事件跟踪  
 ...
 这样的工具[并发可视化工具](/visualstudio/profiling/concurrency-visualizer)利用此功能; 因此，你通常不必直接使用这些事件。 但是，这些事件非常有用的当你正在开发自定义的探查器，或者在使用事件的跟踪工具如[Xperf](http://go.microsoft.com/fwlink/linkid=160628)。  
