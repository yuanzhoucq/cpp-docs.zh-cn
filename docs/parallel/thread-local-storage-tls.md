...
线程本地存储 (TLS) 是一种方法，给定的多线程进程中的每个线程可以使用这种方法分配用以存储线程特定的数据的位置。 TLS API （[TlsAlloc](/windows/win32/api/processthreadsapi/nf-processthreadsapi-tlsalloc)）支持动态绑定（运行时）线程特定数据。 除了现有的 API 实现外，Win32 和 Microsoft c + + 编译器现在还支持静态绑定（加载时）每线程数据。
