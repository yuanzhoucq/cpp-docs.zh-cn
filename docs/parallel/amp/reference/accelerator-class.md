...
获取对象的字符串常量， `accelerator` 可用于在使用流式处理 Simd 扩展（SSE）的多核 cpu 上执行 C++ AMP 代码。
...
返回一个布尔值，该值指示快捷键是否支持双精度算术运算，包括在和之间的带外乘法加法（FMA）、除法、倒数和强制转换 **`int`****`double`**
...
返回一个布尔值，指示快捷键是否限制了对双精度算术的支持。 如果加速器仅支持有限限制，则不支持在和之间进行带外乘法加法（FMA）、除法、倒数和强制转换 **`int`** **`double`** 。
...
获取一个布尔值，该值指示快捷键是否对双精度算术的支持有限。 如果加速器仅支持有限限制，则不支持在和之间进行带外乘法加法（FMA）、除法、倒数和强制转换 **`int`** **`double`** 。
