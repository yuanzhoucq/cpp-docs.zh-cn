...
|[ordered](#ordered-openmp-clauses)|在并行所需[有关](openmp-directives.md#for-openmp)语句如果[排序](openmp-directives.md#ordered-openmp-directives)指令是在循环中使用。|
...
*size*<br/>
（可选）指定的迭代的大小。 *大小*必须是整数。 时未有效*类型*是`runtime`。
