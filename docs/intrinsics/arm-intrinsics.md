...
ms.locfileid: "74163505"
---
# <a name="arm-intrinsics"></a>ARM 内部函数

Microsoft C++编译器（MSVC）使以下内部函数在 ARM 体系结构上可用。 有关 ARM 的详细信息，请参阅[Arm 开发人员文档](https://developer.arm.com/docs)网站上的体系结构和软件开发工具部分。
...
MSVC 与 ARM 编译器之间的主要区别在于，MSVC 添加了 `vldX` 的 `_ex` 变体，并 `vstX` 向量负载和存储说明。 `_ex` 变量采用附加参数，该参数指定指针参数的对齐情况但在其他方面与其非 `_ex` 对应项相同。
...
|_arm_umlal|UMLAL|无符号 __int64 _arm_umlal （无符号 \__int64 _RdHiLo，无符号 int _Rn，无符号 int _Rm）|
...
|__emit||void __emit （无符号 \__int32 操作码）<br /><br /> 将指定指令插入由编译器输出的指令流中。<br /><br /> `opcode` 的值必须是在编译时已知的常量表达式。 指令字大小为 16 位且忽略 `opcode` 最重要的 16 位。<br /><br /> 编译器不会尝试解释 `opcode` 的内容，并且不会在执行插入的指令之前确保 CPU 或内存状态。<br /><br /> 编译器假定 CPU 和内存状态在执行插入的指令后保持不变。 因此，更改状态的指令可能会对编译器生成的普通代码产生不利影响。<br /><br /> 出于此原因，请使用 `emit` 仅插入影响编译器不正常处理的 CPU 状态的指令（例如，协处理器状态）或实现使用 `declspec(naked)`声明的函数。|
...
|__rdpmccntr64||未签名 __int64 \__rdpmccntr64 （void）|
...
|_CountOneBits64||无符号整数 _CountOneBits64 （无符号 \__int64）|
...
|_MoveToCoprocessor64|MCRR|void _MoveToCoprocessor64 （无符号 \__int64，无符号整数，无符号整数，无符号整数）<br /><br /> 使用协处理器数据传输指令，读取 ARM 协处理器中的数据。 有关详细信息，请参阅[_MoveToCoprocessor64](#MoveToCo64)。|
...
内部函数的 `coproc`、`opcode1`、`crn`、`crm`和 `opcode2` 参数的值必须是在编译时已知的常量表达式。
...
|__nop|void __nop （void）**注意：** 在 ARM 平台上，如果在目标体系结构中实现了一个指令，则此函数将生成 nop 指令;否则，将生成不会更改程序或 CPU 状态的替代指令，例如 `MOV r8, r8`。 它在功能上等效于其他硬件体系结构的 \__nop 内部函数。 由于目标体系结构作为优化，对程序或 CPU 的状态不起作用的指令可能会被忽略，因此指令不一定会消耗 CPU 周期。 因此，请不要使用 \__nop 内部操作代码序列的执行时间，除非你确定 CPU 的行为方式。 相反，可以使用 \__nop 内部，将下一条指令与特定32位边界地址对齐。|
|__yield|void __yield （void）**注意：** 在 ARM 平台上，此函数生成 yield 指令，这表示线程正在执行可从执行暂时挂起的任务（例如旋转锁），而不会对程序产生负面影响。 它使 CPU 可以在执行周期中执行其他任务，否则将会浪费这些任务。|
...
|_enable|void __cdecl _enable （void）**注意：** 在 ARM 平台上，此函数生成 CPSIE 指令;它仅可用作内部函数。|
...
下表总结了非 bittest 互锁内部函数的 ARM 支持。 表中的每个单元格都对应一个名称，这些名称的派生方式是将该行的最左侧单元格中的操作名和该列的最上面单元格中的类型名附加到 `_Interlocked`。 例如，`Xor` 行与 `8` 列相交处的单元对应于 `_InterlockedXor8`，并且完全受支持。 大部分受支持的函数提供以下可选后缀：`_acq`、`_rel` 和 `_nf`。 `_acq` 后缀表示“获取”语义，而 `_rel` 后缀表示“发布”语义。 `_nf` 或 "无防护" 后缀对于 ARM 是唯一的，将在下一节中讨论。
...
普通互锁位测试内部函数对于所有平台都是通用的。 ARM 添加了 `_acq`、`_rel`和 `_nf` 变体，只需修改操作的关卡语义，如本文前面的[_nf （无防护）后缀](#nf_suffix)中所述。
