...
ms.locfileid: "70216099"
---
# <a name="arm-intrinsics"></a>ARM 内部函数

Microsoft C++编译器 (MSVC) 使以下内部函数在 ARM 体系结构上可用。 有关 ARM 的详细信息, 请参阅 ARM 信息中心网站上的[Arm 体系结构参考手册](https://go.microsoft.com/fwlink/p/?LinkId=522049)和[Arm 汇编程序工具指南](https://go.microsoft.com/fwlink/p/?LinkId=246102)。

##  <a name="top"></a>NEON

适用于 ARM 的霓虹灯矢量指令集扩展提供了 Single Instruction Multiple Data (SIMD) 功能, 这些功能与在 x86 和 x64 体系结构处理器通用的 MMX 和 SSE 矢量指令集中的功能类似。

根据头文件 `arm_neon.h`，霓虹灯内部函数受到支持。 对霓虹灯内部函数的 MSVC 支持类似于 ARM 编译器的支持, 如 arm 信息中心网站上的[Arm 编译器工具链版本4.1 编译器参考](https://go.microsoft.com/fwlink/p/?LinkId=251083)的附录 G 中所述。

MSVC 与 ARM 编译器之间的主要区别在于, MSVC 添加`_ex`了`vldX`和`vstX`矢量负载和存储说明的变体。 `_ex` 变量采用附加参数，该参数指定指针参数的对齐情况但在其他方面与其非 `_ex` 对应项相同。

##  <a name="A"></a>ARM 特定的内部函数列表

|函数名|指令|函数原型|
|-------------------|-----------------|------------------------|
|_arm_smlal|SMLAL|__int64 _arm_smlal (\__int64 _RdHiLo, int _Rn, int _Rm)|
|_arm_umlal|UMLAL|未签名的 __int64 _arm_umlal \_(无符号 _int64 _RdHiLo, 无符号 int _Rn, 无符号 int _Rm)|
...
|__emit||void __emit (无\_符号 _int32 操作码)<br /><br /> 将指定指令插入由编译器输出的指令流中。<br /><br /> `opcode` 的值必须是在编译时已知的常量表达式。 指令字大小为 16 位且忽略 `opcode` 最重要的 16 位。<br /><br /> 编译器不会尝试解释的内容`opcode` , 并且不会在执行插入的指令之前确保 CPU 或内存状态。<br /><br /> 编译器假定 CPU 和内存状态在执行插入的指令后保持不变。 因此, 更改状态的指令可能会对编译器生成的普通代码产生不利影响。<br /><br /> 出于此原因, 请`emit`仅使用来插入影响编译器不正常处理的 CPU 状态的指令 (例如, 协处理器状态) 或实现使用`declspec(naked)`声明的函数。|
...
|__rdpmccntr64||未签名\_的 __int64 _rdpmccntr64 (void)|
...
|_CountLeadingOnes64||无符号 int _CountLeadingOnes64 ( \_无符号 _int64)|
|_CountLeadingSigns||unsigned int _CountLeadingSigns(long)|
|_CountLeadingSigns64||无符号 int _CountLeadingSigns64\_(_int64)|
|_CountLeadingZeros||unsigned int _CountLeadingZeros(unsigned long)|
|_CountLeadingZeros64||无符号 int _CountLeadingZeros64 ( \_无符号 _int64)|
|_CountOneBits||unsigned int _CountOneBits(unsigned long)|
|_CountOneBits64||无符号 int _CountOneBits64 ( \_无符号 _int64)|
...
|_MoveToCoprocessor64|MCRR|void _MoveToCoprocessor64 (无\_符号 _int64, 无符号整数, 无符号整数, 无符号整数)<br /><br /> 使用协处理器数据传输指令，读取 ARM 协处理器中的数据。 有关详细信息, 请参阅[_MoveToCoprocessor64](#MoveToCo64)。|
...
内部函数`__dmb` (数据内存障碍`__dsb` )、(数据同步屏障) 和`__isb` (指令同步屏障) 使用以下预定义的值来指定内存关卡限制共享域和受此操作影响的访问类型。
...
您可以使用`__iso_volatile_load8/16/32/64`和`__iso_volatile_store8/16/32/64`内部函数显式执行不受编译器优化限制的内存访问。 编译器无法删除、同步或更改这些操作的相对顺序, 但它不会产生隐式硬件内存障碍。 因此，硬件仍可能对跨多个线程的可观察内存访问进行重新排序。 更准确地说, 这些内部函数等效于在 **/volatile: iso**下编译的以下表达式。
...
`coproc`内部函数`opcode1` 、`crm`、、和`opcode2`参数的值必须是在编译时已知的常量表达式。 `crn`
...
|__nop|void __nop (void)**注意:** 如果在目标体系结构中实现了一个指令，则此函数在 ARM 平台上生成一个 NOP 指令；否则，生成一个不会更改项目或 CPU 状态的替代指令，例如 `MOV r8, r8`。 它在功能上等效于\_其他硬件体系结构的 _nop 内部函数。 由于目标体系结构作为优化, 对程序或 CPU 的状态不起作用的指令可能会被忽略, 因此指令不一定会消耗 CPU 周期。 因此, 请不要使用\__nop 内部函数来处理代码序列的执行时间, 除非您一定要了解 CPU 的行为方式。 相反, 可以使用\__nop 内部函数将下一条指令与特定32位边界地址对齐。|
|__yield|void __yield (void)**注意:** 在 ARM 平台上，此函数生成 YIELD 指令，这表示线程正在执行可从执行暂时挂起的任务（如旋转锁）而不会对计划产生负面影响。 它使 CPU 可以在执行周期中执行其他任务, 否则将会浪费这些任务。|
|_AddressOfReturnAddress|void \* _AddressOfReturnAddress (void)|
|_BitScanForward|无符号 char _BitScanForward (无\*符号长 _Index, 无符号长 _Mask)|
|_BitScanReverse|无符号 char _BitScanReverse (无\*符号长 _Index, 无符号长 _Mask)|
...
|_disable|void __cdecl _disable (void)**注意:** 在 ARM 平台上, 此函数生成 CPSID 指令;它仅可用作内部函数。|
|_enable|void __cdecl _ _ (void)**说明:** 在 ARM 平台上, 此函数生成 CPSIE 指令;它仅可用作内部函数。|
...
|_rotl64|未签名\_的 __int64 _cdecl _rotl64 \_(无符号 _int64 _Value, int _Shift)|
|_rotl8|unsigned char _rotl8(unsigned char _Value, unsigned char _Shift)|
|_rotr|unsigned int __cdecl _rotr(unsigned int _Value, int _Shift)|
|_rotr16|unsigned short _rotr16(unsigned short _Value, unsigned char _Shift)|
|_rotr64|未签名\_的 __int64 _cdecl _rotr64 \_(无符号 _int64 _Value, int _Shift)|
...
互锁内部函数是用于执行原子读取-修改-写入操作的一组内部函数。 其中一些互锁内部函数通用于所有平台。 它们是单独列出的, 因为它们有大量的定义, 但由于它们的定义主要是冗余的, 因此更容易考虑它们。 它们的名称可用于派生确切行为。

下表总结了非 bittest 互锁内部函数的 ARM 支持。 表中的每个单元格都对应一个名称，这些名称的派生方式是将该行的最左侧单元格中的操作名和该列的最上面单元格中的类型名附加到 `_Interlocked`。 例如, `Xor`行`8`与列`_InterlockedXor8`相交处的单元格与和完全支持。 大部分受支持的函数提供以下可选后缀：`_acq`、`_rel` 和 `_nf`。 `_acq` 后缀表示“获取”语义，而 `_rel` 后缀表示“发布”语义。 或`_nf` "无防护" 后缀对于 ARM 是唯一的, 将在下一节中讨论。
...
- **Partial**: 支持无格式`_acq`、和`_nf`窗体。

- “无”：不支持

###  <a name="nf_suffix"></a>_nf (无围栏) 后缀

或 "无防护" 后缀指示操作不会像其他三个窗体 (无格式、 `_acq`和`_rel`) 那样表现为任何类型的内存屏障, 这三种形式均表现为某种屏障。 `_nf` `_nf`窗体的一种可能用途是维护一个统计信息计数器, 该计数器由多个线程同时更新, 但其值在多个线程执行时不使用。
...
普通互锁位测试内部函数对于所有平台都是通用的。 ARM 添加`_acq`了`_rel`、和`_nf`变体, 只需修改操作的关卡语义, 如本文前面的[_nf (无防护) 后缀](#nf_suffix)中所述。

|函数名|函数原型|
|-------------------|------------------------|
|_interlockedbittestandreset|无符号 char _interlockedbittestandreset (长\*可变, 长)|
|_interlockedbittestandreset_acq|无符号 char _interlockedbittestandreset_acq (长\*可变, 长)|
|_interlockedbittestandreset_nf|无符号 char _interlockedbittestandreset_nf (长\*可变, 长)|
|_interlockedbittestandreset_rel|无符号 char _interlockedbittestandreset_rel (长\*可变, 长)|
|_interlockedbittestandset|无符号 char _interlockedbittestandset (长\*可变, 长)|
|_interlockedbittestandset_acq|无符号 char _interlockedbittestandset_acq (长\*可变, 长)|
|_interlockedbittestandset_nf|无符号 char _interlockedbittestandset_nf (长\*可变, 长)|
|_interlockedbittestandset_rel|无符号 char _interlockedbittestandset_rel (长\*可变, 长)|
