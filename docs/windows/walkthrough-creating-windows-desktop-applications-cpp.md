...
1. 若要处理的消息，我们首先添加一个消息循环来侦听 Windows 发送的消息。 当应用程序收到一条消息时，此循环将该消息调度到你`WndProc`函数以进行处理。 该消息循环类似于以下代码。
   ...
   结构和消息循环中的函数的详细信息，请参阅[MSG](https://msdn.microsoft.com/library/windows/desktop/ms644958)， [GetMessage](https://msdn.microsoft.com/library/windows/desktop/ms644936)， [TranslateMessage](https://msdn.microsoft.com/library/windows/desktop/ms644955)，和[DispatchMessage](https://msdn.microsoft.com/library/windows/desktop/ms644934).
...
1. 在 **“生成”** 菜单上，选择 **“生成解决方案”**。 编译的结果应出现在**输出**Visual Studio 窗口中的。
