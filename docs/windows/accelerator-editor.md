...
ms.locfileid: "84623485"
---
# <a name="accelerator-editor-c"></a>快捷键编辑器（c + +）

快捷键对应表是一个 c + + Windows 资源，其中包含一个快捷键列表（称为快捷键）和与它们关联的命令标识符。 一个程序可以拥有多个快捷键对应表。
...
可以使用 [类视图](/visualstudio/ide/viewing-the-structure-of-code) 将快捷键命令与代码挂接。 有关预定义快捷键的列表，请参阅[加速键](predefined-accelerator-keys.md)。
...
- 如果**类型**属性是**ASCII**，则**修饰符**属性可能只是 `None` 或 `Alt` ，或者它可以有一个使用**Ctrl**键的快捷键，该快捷键通过在键前面使用来指定 `^` 。
...
在 c + + 项目中，可以直接使用**快捷键编辑器**中的就地编辑来编辑快捷键对应表。
...
1. 键入要用作快捷键的*键*，或右键单击并选择 "**下一键类型**" 以设置键组合，或单击 "前往菜单" "**编辑" "编辑**  >  **下一个键**"。

1. 如有必要，请更改**修饰符**和**类型**，然后按**enter**。

> [!NOTE]
> 确保定义的所有快捷键都是唯一的。 可以将多个键组合分配给相同的 ID，但不会产生任何影响，例如，可将**Ctrl** + **P**和**F8**分配给 ID_PRINT。 但是，分配给多个 ID 的组合键不能正常运行，例如，将**Ctrl** + **Z**同时分配给 ID_SPELL_CHECK 和 ID_THESAURUS。
...
[资源编辑器](resource-editors.md)<br/>
[加速键](predefined-accelerator-keys.md)<br/>
