---
title: 类型参数的泛型约束 (C + + CLI) |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-windows
...
约束子句包括**其中**跟类型参数、 冒号 (**:**)，和约束，类型参数指定的限制性质。 **其中**是上下文相关关键字; 请参阅[上下文相关的关键字](../windows/context-sensitive-keywords-cpp-component-extensions.md)有关详细信息。 分隔多个**其中**子句有空格。
...
此约束要求类型参数用于`T`实现`IComparable<T>`在编译时。 它还允许接口方法，如`CompareTo`调用。 调用接口方法的类型参数实例不需要进行强制转换。
