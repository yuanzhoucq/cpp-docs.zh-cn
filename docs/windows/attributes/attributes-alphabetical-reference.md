...
|[值](defaultvalue.md)|允许为类型化可选参数指定默认值。|
...
|[限制](restricted.md)|指定不能任意调用某个库或模块、接口或调度接口的成员。|
...
|[string](string-cpp.md)|指示 **`char`** 必须将一维、、 **`wchar_t`** `byte` 或等效数组或指向此类数组的指针视为字符串。|
