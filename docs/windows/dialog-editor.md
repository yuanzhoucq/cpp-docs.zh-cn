...
- 若要打开该编辑器，请在 "**资源视图**" 窗口中双击对话框的 .rc 文件，或 > **其他 Windows** > **资源视图**中的 "中转到" 菜单**视图**。
...
当C++您在项目中打开**对话框编辑器**时，**对话框编辑器**工具栏会自动显示在您的解决方案的顶部，但是，如果您显式关闭工具栏，则下次打开**对话框编辑器**时需要调用它。 您可以通过在可用工具栏和窗口列表中选择它来切换其显示。
...
|格式.顶部对齐|**Ctrl** + **Shift** + **上箭头**|将所选控件的上边缘与主导控件对齐。|
...
|Format.SizeToContent|**轮班** + **F7**|调整所选控件的大小，使其适合标题文本。|
