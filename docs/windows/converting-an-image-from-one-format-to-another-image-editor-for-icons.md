...
可以在图像编辑器中打开 GIF 或 JPEG 格式的图像并将其保存为位图。  同样，可以打开位图文件并将其保存为 GIF 或 JPEG 格式。  所处理的图像并非必须是项目的一部分才能在开发环境中进行编辑（请参见[独立运行图像编辑](../mfc/editing-an-image-outside-of-a-project-image-editor-for-icons.md)）。  
  
### 将图像从一种格式转换为另一种格式  
  
1.  在图像编辑器中打开图像。  
  
2.  从**“文件”**菜单中，选择**“*filename* 另存为”**。  
  
3.  在**“另存文件为”**对话框中，在**“文件名”**框中键入文件名和表示所需格式的扩展名。  
