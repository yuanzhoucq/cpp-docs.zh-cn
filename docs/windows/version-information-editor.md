...
在`FILEFLAGS` **版本信息编辑器**中编辑密钥时, 请注意, 不能在 .rc 文件的 "**属性**" 窗口中设置 "**调试**"、"**专用生成**" 或 "**特殊生成**" 属性:

   - **版本信息编辑器**将根据`_DEBUG`生成标志, `#ifdef`在资源脚本中设置 Debug 属性。

  - `FILEFLAGS`如果键在版本信息表中设置了值, 则键的 "属性" 窗口中的相应 "私有生成" 属性将为 True。 `Private Build` 如果**值**为空, 则属性将为**False**。 同样,**版本信息**表中的**特殊生成**键将绑定到`FILEFLAGS`密钥的**特殊生成**属性。
