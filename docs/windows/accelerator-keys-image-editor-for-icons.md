...
ms.locfileid: "84622003"
---
# <a name="accelerator-keys-c-image-editor-for-icons"></a>快捷键（图标的 c + + 图像编辑器）
...
|命令|键|描述|
|-------------|----------|-----------------|
|AirBrushTool|**Ctrl**  + **一个**|使用具有选定大小和颜色的喷枪进行绘制。|
|图像.画笔工具|**Ctrl**  + **B**|使用所选形状、大小和颜色绘制画笔。|
|CopyAndOutlineSelection|**Ctrl**  + **移位**  + **U**|创建当前选定内容的副本并绘制其轮廓。 如果当前选定内容中包含背景颜色，则在选择[透明](../windows/choosing-a-transparent-or-opaque-background-image-editor-for-icons.md)效果时，将排除该背景色。|
...
|EraserTool|**Ctrl**  + **移位**  + **I**|清除图像的一部分（使用当前背景色）。|
...
|图像.新建图像类型|**插入**|启动 "[新建 \<Device> 图像类型" 对话框](../windows/new-device-image-type-dialog-box-image-editor-for-icons.md)，使用该对话框可以为不同图像类型创建图像。|
...
|图像.旋转 90 度|**Ctrl**  + **移位**  + **H**|将图像或选定内容旋转 90 度。|
...
|图像.显示平铺网格|**Ctrl**  + **移位**  + **Alt**  + **S**|切换磁贴网格（选择或清除 "[网格设置" 对话框](../windows/grid-settings-dialog-box-image-editor-for-icons.md)中的 "**磁贴网格**" 选项）。|
|图像.小画笔|**Ctrl**  + **.** （句点）|将**画笔**大小缩小为一个像素。 （请参见本表中的 Image.LargerBrush 和 Image.SmallerBrush。）|
|图像.较小画笔|**Ctrl**  +  Ctrl **-** 差|减小画笔大小（每个方向上减小一个像素）。 若要再次扩展画笔大小，请参见本表中的 Image.LargerBrush。|
