...
1.  在“图像”菜单上，单击“打开设备图像”并从当前图像列表中选择一个图像。  例如，选择图标的 32x32 的 16 色版本。  
  
2.  复制当前显示的图标图像 \(**Ctrl\+C**\)。  
  
3.  在另一个“图像编辑器”窗口中打开该图标的另一个图像。  例如，打开该图标的 16x16 的 16 色版本。  
  
4.  将图标图像从一个“图像编辑器”窗口粘贴 \(**Ctrl\+V**\) 到另一个窗口。  如果要将一个较大的图标图像粘贴进一个较小的图标图像，可以使用图标句柄调整图像的大小。  
