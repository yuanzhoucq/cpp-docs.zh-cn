...
ms.locfileid: "39605638"
---
# <a name="resizing-an-image-image-editor-for-icons"></a>调整图像大小（图标的图像编辑器）
调整图像大小图像编辑器的行为取决于是否已经[选定](../windows/selecting-an-area-of-an-image-image-editor-for-icons.md)整个图像或只是它的一部分。  
  
 如果所选内容包含仅映像的一部分，图像编辑器通过删除行来缩小选定内容或像素和用当前背景色，或它填充空出的区域的列通过复制行或列的像素为单位拉伸所选内容。  
  
 当所选内容包括整个图像时，图像编辑器是收缩和拉伸图像，或裁剪和对其进行扩展。  
  
 有两种方式调整图像大小： 调整大小控点和[属性窗口](/visualstudio/ide/reference/properties-window)。 您可以拖动调整大小控点，若要更改的所有大小或映像的一部分。 可以拖动调整大小控点都是可靠的。 您不能拖动空心的句柄。 可以使用属性窗口来调整整个图像，大小不是所选的一部分。  
  
 ![大小调整控点上位图](../mfc/media/vcimageeditorsizinghandles.gif "vcImageEditorSizingHandles")  
大小调整控点  
  
> [!NOTE]
>  如果您有中选择的平铺网格选项[网格设置对话框](../windows/grid-settings-dialog-box-image-editor-for-icons.md)，然后调整大小将下一步的平铺网格线对齐。 如果仅像素网格选项是选择 （默认设置），调整大小将对齐到下一个可用的像素。  
