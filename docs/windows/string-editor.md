...
**字符串编辑器**将确保不使用已在使用中的 ID。 如果选择一个已在使用的 ID，**字符串编辑器**会通知你，然后分配一个通用唯一 ID，例如 `IDS_STRING58113`。
