...
2.  在[“属性”窗口](../Topic/Properties%20Window.md)中的**“宽度”**和**“高度”**框中，键入想要的尺寸。  
  
     如果是增加图像的大小，则图像编辑器向右、向下或者同时向右向下扩展图像，并用当前背景色填充新的区域。  并不拉伸图像。  
  
     如果是减小图像的大小，则图像编辑器在图像的右边缘或下边缘或者同时在这两个边缘进行裁剪。  
  
    > [!NOTE]
    >  使用“宽度”和“高度”属性只能调整整个图像的大小，而不能调整部分选定内容的大小。  
