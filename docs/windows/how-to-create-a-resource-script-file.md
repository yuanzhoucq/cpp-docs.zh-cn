...
>
> 此信息不适用于 Windows 桌面应用程序或通用 Windows 平台应用中的资源。 .NET 语言中的项目不使用资源脚本文件。 将资源添加到托管项目的信息，请参阅[资源文件](../windows/resource-files-visual-studio.md)，[应用程序资源和资源管理系统](/windows/uwp/app-resources/)，并[桌面应用中的资源](/dotnet/framework/resources/index)中 *.NET Framework 开发人员指南*。

## <a name="create-a-resource-script"></a>创建资源脚本
...
1. 为资源脚本中提供的名称**名称**文本，然后选择**打开**。

### <a name="to-open-a-resource-script"></a>若要打开资源脚本

可以在资源脚本中查看资源，而无需打开一个项目。 脚本文件将在而不是在中打开的文档窗口中打开**资源视图**窗口 （如它在项目内打开该文件时）。

> [!NOTE]
> 只有当该文件是 （在外部项目） 独立打开时，某些命令可用。 打开文件独立意味着不先加载项目的情况下打开它。
>
...
1. 在中**打开的文件**对话框中，导航到资源脚本文件，突出显示该文件，然后选择**打开**。
...
1. 这两个.rc 文件时打开，请在**窗口**菜单 （或右键单击一个.rc 文件中的） 选择**新建水平制表符组**或**新建垂直制表符组**。
...
> [!TIP]
> 一种快捷方式是右键单击.rc 文件中的**解决方案资源管理器**，选择**打开方式...** ，然后选择**源代码 （文本） 编辑器**。

### <a name="to-add-mfc-support"></a>添加 MFC 支持

通常情况下，如果您构建 Windows 使用的 Microsoft 基础类 (MFC) 应用程序[MFC 应用程序向导](../mfc/reference/mfc-application-wizard.md)，向导将生成一组基本的文件 （包括资源脚本 (.rc) 文件），其中包含的核心功能MFC。 但是，在编辑不基于 MFC 的 Windows 应用程序的.rc 文件时，以下的 MFC 功能不可用：

特定于 MFC 的一些功能包括 MFC 代码向导、 菜单提示字符串、 组合框控件和 ActiveX 控件承载的列表内容。
...
>
> 通过右键单击**资源视图**窗口来启动命令的快捷菜单，或双击以停靠或取消停靠该窗口的标题栏上。 右键单击标题栏提供了可用于控制窗口的行为的其他命令。 有关详细信息，请参阅[Windows Management](/visualstudio/ide/customizing-window-layouts-in-visual-studio)。

使用**添加资源**对话框将资源添加到具有以下属性的 c + + Windows 桌面应用程序项目：
...
使用**新的自定义资源**对话框具有以下属性的 c + + 项目中创建新的自定义资源：
...
- 在中**资源视图**，选择你的.rc 文件，然后选择**编辑**菜单，然后选择**添加资源**。 在中**添加资源**对话框框中，选择你想要添加到项目的资源的类型。

   > [!TIP]
   > 此外可以右键单击.rc 文件中的**资源视图**，然后选择**添加资源**从快捷菜单。

- 在中**解决方案资源管理器**，右键单击项目文件夹，然后选择**添加**，然后选择**添加资源**快捷菜单上。 在中**添加资源**对话框框中，选择你想要添加到项目的资源的类型。

   > [!NOTE]
   > 如果还没有在项目中的.rc 文件，此步骤将创建一个。 然后可以重复此步骤将特定资源类型添加到新的 .rc 文件。

- 在中[类视图](/visualstudio/ide/viewing-the-structure-of-code)，右键单击您的类，然后选择**添加**。 选择**添加资源**从快捷菜单，然后选择你想要添加到项目的资源类型。

- 在中**项目**菜单中，选择**添加资源**。

## <a name="create-a-resource-template"></a>创建资源模板
...
1. 在中**添加新项**对话框中**模板：** 窗格中，选择**资源模板文件 (.rct)**。
...
1. 在中**添加资源**对话框框中，选择加号 (**+**) 旁边的展开资源节点并查看所有模板可用于该资源的资源。
