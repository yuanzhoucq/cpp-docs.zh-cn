...
`__inline` 关键字指示编译器用函数定义中的代码替换函数调用的每个实例。 但是，替换操作完全由编译器自行决定。 例如，如果某个函数的地址被采用或者由于过大而无法内联，则编译器不会内联该函数。
...
不应该将使用 `__inline` 的函数与内联汇编程序代码混淆。 有关详细信息，请参阅[内联汇编程序](../c-language/inline-assembler-c.md)。
