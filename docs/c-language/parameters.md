...
/\**属性 seq*特定于 Microsoft\*/

*声明者*：<br/>
&nbsp;&nbsp;&nbsp;&nbsp;pointer**<sub>opt direct-declarator</sub> **

*直接声明者*： /\*函数声明人\*/<br/>
&nbsp;&nbsp;&nbsp;&nbsp;*直接声明器*  **（**  *参数类型列表*  **）** /\*新式声明器\*/<br/>
&nbsp;&nbsp;&nbsp;&nbsp;*直接声明者*  **（**  *标识符列表*<sub>选择</sub> **）** /\*过时式声明人\*/
...
如果参数列表中至少有一个参数，则列表可以以逗号结尾，后跟三个句点 **（...）。** 此构造称为"省略表示法"，表示函数的参数数可变。 （有关详细信息[，请参阅具有可变参数数的调用](../c-language/calls-with-a-variable-number-of-arguments.md)。但是，对函数的调用必须具有至少与最后一个逗号之前的参数相同的参数数。

如果实参不会传递到函数，则形参的列表将替换为关键字 `void`。 对 `void` 的这种用法不同于将其用作类型说明符。

参数的顺序和类型（包括省略号表示法的任何用法）在所有函数声明（如果有）和函数定义中都必须相同。 进行常用算术转换后，自变量的类型与对应参数的类型必须是赋值兼容的。 （有关算术转换的信息，请参阅[常用算术转换](../c-language/usual-arithmetic-conversions.md)。不会检查省略号之后的参数。 参数可以具有任何基础、结构、联合、指针或数组类型。
