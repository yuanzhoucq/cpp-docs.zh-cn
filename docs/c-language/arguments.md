...
```  
 ...
 `x` 的地址存储在 `num1` 中，而 `y` 的地址存储在 `num2` 中。 现在，这两个名称或“别名”存在于同一个位置。 对 `*num1` 中的 `*num2` 和 `swap` 的引用是对 `x` 中的 `y` 和 `main` 的有效引用。 `swap` 中的赋值实际交换了 `x` 和 `y` 的内容。 因此，`return` 语句不是必需的。  
