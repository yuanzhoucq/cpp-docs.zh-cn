...
ms.locfileid: "87229657"
---
# <a name="identifiers-in-primary-expressions"></a>主要表达式中的标识符

标识符可以具有整型、`float`、`enum`、`struct`、`union`、数组、指针或函数类型。 如果已将标识符声明为指定对象（此时为左值）或声明为函数（此时为函数指示符），则它是主函数。 有关左值的定义，请参阅[左值和右值表达式](../c-language/l-value-and-r-value-expressions.md)。
