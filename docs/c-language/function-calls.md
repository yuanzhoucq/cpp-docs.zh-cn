...
> [!NOTE]
> 函数参数列表中的表达式可以任何顺序进行计算，因此其值可能受其他参数的副作用而更改的参数具有未定义的值。 函数调用运算符定义的序列点仅保证在将控制权传递给所调用函数之前计算参数列表中的所有副作用。 （请注意，在堆栈上推送参数的顺序是单独的问题。有关详细信息，请参阅[序列点](../c-language/c-sequence-points.md)。
