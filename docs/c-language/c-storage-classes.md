 ...
 **\_\_declspec** \( *extended\-decl\-modifier\-seq* \) \/\* Microsoft 专用 \*\/  
 ...
 除 `__declspec` 之外，您只能在声明中的 *declaration\-specifier* 中使用一个 *storage\-class\-specifier*。  如果没有制定存储类规范，块中的声明将创建自动对象。  
  
 使用 **auto** 或 **register** 说明符声明的项具有本地生存期。  使用 **static** 或 `extern` 说明符声明的项具有全局生存期。  
  
 由于 `typedef` 和 `__declspec` 与其他四个 *storage\-class\-specifier* 终端的语义不同，因此将分开讨论它们。  有关 `typedef` 的特定信息，请参阅 [Typedef 声明](../c-language/typedef-declarations.md)。  有关 `__declspec` 的特定信息，请参阅[扩展存储类特性](../c-language/c-extended-storage-class-attributes.md)。  
