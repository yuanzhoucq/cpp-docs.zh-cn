...
```  
 ...
 以 `#define` 开始的行是预处理指令。  这些指令告知预处理器通过 FILE1.C.用数字 `1`、`2` 和 `3` 分别替换标识符 `ONE`、`TWO` 和 `THREE`。  但是，此类指令不适用于 FILE2.C，将单独对其进行编译，然后将其与 FILE1.C 链接。  以 `#include` 开始的行告知编译器包含文件 STDIO.H，该文件包含 `printf` 函数的原型。  [预处理器指令](../preprocessor/preprocessor-directives.md)在《预处理器参考》中进行了解释。  
