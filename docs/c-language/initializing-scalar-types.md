...
-   外部变量声明和所有 **static** 变量（无论外部还是内部）的初始值必须是常量表达式。（有关详细信息，请参阅[常量表达式](../c-language/c-constant-expressions.md)。）由于任何外部声明的变量或静态变量的地址为常量，因此可将该地址用于初始化内部声明的 **static** 指针变量。  但是，**auto** 变量的地址不能用作静态初始值设定项，因为它可能在每次执行块时都不同。  您可使用常量或变量值初始化 **auto** 和 **register** 变量。  
...
```  
 ...
 此语句使用另一变量 `x` 的地址初始化指针 `b`。  
  
```  
int *const a = &z;  
```  
  
 指针 `a` 是使用名为 `z` 的变量的地址初始化的。  但是，由于该指针被指定为 **const**，因此变量 `a` 只能被初始化，而从不能被修改。  该指针始终指向同一位置。  
...
```  
 ...
 全局变量 `GLOBAL` 是在外部级别声明的，因此它具有全局生存期。  局部变量 `LOCAL` 具有 **auto** 存储类，并且在执行从中声明它的函数时只具有地址。  因此，不允许尝试使用 `LOCAL` 的地址初始化 **static** 指针变量 `lp`。  **static** 指针变量 `gp` 可初始化为 `GLOBAL` 的地址，因为该地址始终相同。  同样，`*rp` 可初始化，因为 `rp` 是局部变量，并且可具有非常量初始值设定项。  每次输入块时，`LOCAL` 都具有新地址，该地址之后将赋给 `rp`。  
