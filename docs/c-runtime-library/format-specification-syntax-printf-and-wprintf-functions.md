...
> [!NOTE]
> 在 Visual Studio 2015 中，`printf`函数和`scanf`函数系列声明为**内联**，并移动到`<stdio.h>`和`<conio.h>`标头。 如果要迁移较旧的代码，您可能会看到*LNK2019*与这些函数有关。 有关详细信息，请参阅[Visual C++更改历史记录 2003-2015](../porting/visual-cpp-change-history-2003-2015.md#stdio_and_conio)。

## <a name="type-conversion-specifier"></a>类型转换说明符

类型** 转换说明符字符指定是否要将相应的参数解释为字符、字符串、指针、整数或浮点数。 类型** 字符是唯一必填的转换规范字段，它出现在任何可选字段之后。

将根据相应的类型** 字符和可选的[大小](#size)前缀对紧跟格式字符串的参数进行解释。 将通过使用 **c** 或 **C**指定字符类型 `char` 和 `wchar_t` 的转换，将通过使用 **s** 或 **S** 指定单字节和多字节或宽字符字符串，具体取决于正在使用的格式设置函数。 通过使用**c**和**s**指定的字符和字符串参数`char`解释为`char*``printf`和族函数，`wchar_t`或`wchar_t*`由族函数解释。 `wprintf` 通过使用 **C** 和 **S** 指定的字符和字符串参数将被 `printf` 系列函数解释为 `wchar_t` 和 `wchar_t*`，或被 `wprintf` 系列函数解释为 `char` 和 `char*`。 此行为特定于 Microsoft。

整数`short`类型，如`int`， `long`， `long long`，`unsigned`及其变体， 通过使用**d，** **i，** **o，** **u，** **x**和**x**指定。`float`浮点类型，如 ，`double`和`long double`， 通过使用**a**、 **e、** **E、** **f**、 **f**、 **g**和**g**进行指定。 **A**默认情况下，除非它们由*大小*前缀修改，否则将强制`int`键入整数参数，并将浮点参数强制为`double`。 在 64 位系统上，`int` 是 32 位的值；因此，确定 64 位整数的输出格式时，将把它截断，除非使用 **ll** 或 **I64** 的大小** 前缀。 由 **p** 指定的指针类型使用平台的默认指针大小。

> [!NOTE]
> **特定于微软：****Z**类型字符以及`printf``wprintf`**c**c、C、s**C**和**s****S**类型字符与 和 函数一起使用时的行为是 Microsoft 扩展。 在所有的格式设置函数中，ISO C 标准始终对窄字符和字符串使用 **c** 和 **s**，而对宽字符和字符串使用 **C** 和 **S**。
...
|**F**|浮点|具有窗体 _-_*ddd*的已签名值 __。__*ddd*，其中*ddd*是一个或多个十进制数字。 小数点前的数字位数取决于数字的度量值，小数点后的数字位数取决于请求的精度，或为默认的六位数。|
...
在转换规范中，可选宽度规范字段出现在任何标志** 字符之后。 宽度** 参数是控制输出的最小字符数量的非负十进制整数。 如果输出值中的字符数小于指定的宽度，则将空白添加到值的左侧或右侧，具体取决于是否指定了左对齐标志 （），**-** 直到达到最小宽度。 如果 0 作为宽度** 的前缀，则将向整数或浮点转换添加前导零，直到达到最小宽度为止，但转换到 infinity 或 NaN 时除外。
...
|类型|含义|默认|
|----------|-------------|-------------|
|**a**、**A**|精度指定此点后的数字位数。|默认精度为 13。 如果精度为 0，则除非使用标志，**#** 否则不会打印小数点。|
|**c**， **C**|精度不产生任何影响。|打印字符。|
...
> [!NOTE]
> **特定于微软：****I（** 大写**i）、I32、I64**和**w**参数大小修改器前缀是微软扩展，不兼容ISO C。 **I64** h 前缀（在与 `char` 类型的数据一起使用时）和 l（小写 L）前缀（在与 `double` 类型的数据一起使用时）是 Microsoft 扩展********。
