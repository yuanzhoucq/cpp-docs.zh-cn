...
将根据相应的类型字符和可选的[大小](#size)前缀对紧跟格式字符串的参数进行解释。 将通过使用 `char`c`wchar_t` 或 **C**指定字符类型 **和** 的转换，将通过使用 **s** 或 **S** 指定单字节和多字节或宽字符字符串，具体取决于正在使用的格式设置函数。 通过使用 **c** 和 **s** 指定的字符和字符串参数将被 `char` 系列函数解释为 `char*` 和 `printf`，或被 `wchar_t` 系列函数解释为 `wchar_t*` 和 `wprintf`。 通过使用 **C** 和 **S** 指定的字符和字符串参数将被 `wchar_t` 系列函数解释为 `wchar_t*` 和 `printf`，或被 `char` 系列函数解释为 `char*` 和 `wprintf`。 此行为是 Microsoft 特定的。
...
|**指向**|浮点|有符号的十六进制双精度浮点值，形式为 [-]0X*h.hhhh*__P±__*dd*，其中 *h.hhhh* 是尾数的十六进制数（使用大写字母），*dd* 是一位或多位指数。 精度指定此点后的数字位数。|
...
**hc** 或 **hC** 类型说明符与 **函数中的**c`printf` 以及 **函数中的**C`wprintf` 是同义的。 **lc**、**lC**、**wc** 或 **wC** 类型说明符与 **函数中的**C`printf` 以及 **函数中的**c`wprintf` 是同义的。 **hs** 或 **hS** 类型说明符与 **函数中的**s`printf` 以及 **函数中的**S`wprintf` 是同义的。 **ls**、**lS**、**ws** 或 **wS** 类型说明符与 **函数中的**S`printf` 以及 **函数中的**s`wprintf` 是同义的。

> [!NOTE]
> **特定于 Microsoft 的：** **I** （大写 I）、 **I32**、 **I64**和**w**参数大小修饰符前缀是 Microsoft 扩展，与 ISO C 兼容。 h 前缀（在与 **类型的数据一起使用时）和 l（小写 L）前缀（在与** 类型的数据一起使用时）是 Microsoft 扩展`char``double`。
