...
ms.locfileid: "75301036"
---
# <a name="unlink"></a>unlink

Microsoft 实现的 POSIX 函数名称 `unlink` 是[_unlink](unlink-wunlink.md)函数的不推荐使用的别名。 默认情况下，它会生成[编译器警告（等级3） C4996](../../error-messages/compiler-warnings/compiler-warning-level-3-c4996.md)。 名称已弃用，因为它不遵循特定于实现的名称的标准 C 规则。 但是，该函数仍受支持。
