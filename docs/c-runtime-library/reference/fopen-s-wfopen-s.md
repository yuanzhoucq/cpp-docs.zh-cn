...
此外，在文本模式下回车换行符组合将转换为单一的换行输入，并换行字符将转换为输出回车返回换行组合。 当 Unicode 流 I/O 函数在文本模式（默认设置）下运行时，源或目标流将假定为一系列多字节字符。 因此，Unicode 流输入函数将多字节字符转换为宽字符 (就像调用**mbtowc**函数)。 出于相同原因，Unicode 流输出函数将宽字符转换为多字节字符 (就像调用**wctomb**函数)。
