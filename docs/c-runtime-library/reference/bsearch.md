...
执行排序数组的二进制搜索。 此函数有一个更安全的版本；请参阅 [bsearch\_s](../../c-runtime-library/reference/bsearch-s.md)。  
...
|\> 0|键大于数组元素。|  
 ...
 此函数验证其参数。 如果 `compare`、`key` 或 `num` 为 `NULL`，或者如果 `base` 为 `NULL` 且 \*`num` 不为零，或者如果 `width` 为零，则将调用无效的参数处理程序（如[参数验证](../../c-runtime-library/parameter-validation.md)中所述）。 如果允许继续执行，则将 `errno` 设置为 `EINVAL` 并且该函数将返回 `NULL`。  
