...
|目标缓冲区太小，无法包含转换后的字符串（除非*计数*为 **_TRUNCATE**; 有关详细信息，请参阅 "备注"）|**ERANGE**|

如果发生这些情况中的任何一个，都会调用无效的参数异常，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。 如果允许执行继续，则该函数将返回错误代码并按表中所示设置**errno** 。

## <a name="remarks"></a>备注

**Mbsrtowcs_s**函数通过使用*mbstate*中包含的转换状态，将*mbstr*间接指向的多字节字符的字符串转换为存储在*wcstr*所指向的缓冲区中的宽字符。 在满足以下条件之一前，该转换将一直对每个字符执行：
...
如果*count*是特殊值[_TRUNCATE](../../c-runtime-library/truncate.md)，则**mbsrtowcs_s**会将尽可能多的字符串转换为目标缓冲区，同时仍然为 null 终止符留下空间。
...
如果**mbsrtowcs_s**遇到在当前区域设置中无效的多字节字符，它将在 *&#42;pReturnValue*中设置-1，将目标缓冲区*wcstr*设置为空字符串，将**errno**设置为**eilseq 且**，然后返回**Eilseq 且**。

如果由*mbstr*和*wcstr*指向的序列重叠，则**mbsrtowcs_s**的行为不确定。 **mbsrtowcs_s**受当前区域设置的 LC_TYPE 类别影响。

> [!IMPORTANT]
> 确保*wcstr*和*mbstr*不重叠，并且该*计数*正确反映了要转换的多字节字符的数量。

**Mbsrtowcs_s**函数的可重启性不同于[mbstowcs_s、_mbstowcs_s_l](mbstowcs-s-mbstowcs-s-l.md) 。 转换状态存储在*mbstate*中，以便后续调用相同的或其他可重启的函数。 混合使用可重启函数和不可重启函数时，结果不确定。 例如，如果使用了对**mbsrtowcs_s**的后续调用而不是**mbstowcs_s**，则应用程序应使用**mbsrlen**而不是**mbslen**。
...
如果当前线程中的函数都不调用**setlocale** ，只要此函数正在执行且*mbstate*参数不是 null 指针， **mbsrtowcs_s**函数就是多线程安全的。
