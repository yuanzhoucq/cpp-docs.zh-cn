...
**vprintf**和**vwprintf**返回写入的字符数，不包括终止 null 字符或为负值，如果发生输出错误。 如果*格式*是 null 指针，或如果格式字符串包含无效格式字符，则调用无效参数处理程序，如中所述[参数验证](../../c-runtime-library/parameter-validation.md)。 如果允许执行继续，函数将返回-1 并设置**errno**到**EINVAL**。
...
**vwprintf**是宽字符版本的**vprintf**; 如果在 ANSI 模式下打开流，则两个函数的行为完全相同。 **vprintf**当前不支持输出到 UNICODE 流。
