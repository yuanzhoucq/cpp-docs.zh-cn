...
**vprintf**并**vwprintf**返回写入的字符，不包括终止 null 字符，则为负值，如果发生输出错误数。 如果*格式*是空指针，或如果格式字符串包含无效格式字符，将调用无效参数处理程序，如中所述[参数验证](../../c-runtime-library/parameter-validation.md)。 如果允许执行继续，函数将返回-1 并设置**errno**到**EINVAL**。
...
**vwprintf**是宽字符版本**vprintf**; 如果在 ANSI 模式下打开流，则两个函数行为相同。 **vprintf**当前不到 UNICODE 流支持输出。

使用这些函数的版本 **_l**后缀完全相同，只不过它们使用传递中而不是当前线程区域设置的区域设置参数。

> [!IMPORTANT]
> 确保 format 不是用户定义的字符串。 有关详细信息，请参阅 [避免缓冲区溢出](/windows/desktop/SecBP/avoiding-buffer-overruns)。 请注意，已检测到无效格式字符串并发生错误。
