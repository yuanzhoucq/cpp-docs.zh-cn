...
*arg0*， *arg1*，...*argn*<br/>
指向参数的指针的列表。 *Arg0*自变量通常是一个指向*cmdname*。 自变量*arg1*通过*argn*是指向构成新参数列表的字符串的指针。 以下*argn*，必须有**NULL**指针，用以标记自变量列表的末尾。
...
返回值从同步 **_spawnlpe**或 **_wspawnlpe** (**_P_WAIT**指定的*模式*) 是新的退出状态过程。 返回值从异步 **_spawnlpe**或 **_wspawnlpe** (**_P_NOWAIT**或 **_P_NOWAITO**指定的*模式*) 是进程句柄。 如果进程正常终止，则退出状态为 0。 则可以将退出状态设置为非零值，如果生成的进程专门使用非零自变量调用**退出**例程。 如果更新过程未显式设置正退出状态，则正退出状态指示因中止或中断而造成的异常退出。 返回值-1 指示的错误 （不启动新过程）。 在这种情况下， **errno**设置为以下值之一。
