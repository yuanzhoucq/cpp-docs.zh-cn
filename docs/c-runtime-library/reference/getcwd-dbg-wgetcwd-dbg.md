...
## <a name="remarks"></a>备注  
 `_getcwd_dbg` 和 `_wgetcwd_dbg` 函数与 `_getcwd` 和 `_wgetcwd` 完全相同，只是当定义 _`DEBUG` 时，如果将 `malloc` 作为第一个参数进行传递，则这些函数将使用 `_malloc_dbg` 的调试版本和 `NULL` 来分配内存。 有关详细信息，请参阅 [_malloc_dbg](../../c-runtime-library/reference/malloc-dbg.md)。  
