...
使用 **_set_FMA3_enable**函数可以启用或禁用 CRT 库的超越数学浮点函数中的 FMA3 指令。 返回值反映更改后使用的实现。 如果 CPU 不支持 FMA3 指令，则此函数将无法在库中启用它们，并且返回值为零。 使用 **_get_FMA3_enable**获取库的当前状态。 默认情况下，在 X64 平台上，CRT 启动代码检测 CPU 是否支持 FMA3 指令，并启用或禁用库中的 FMA3 实现。
...
**_Set_FMA3_enable**和 **_get_FMA3_enable**函数仅在 X64 版本的 CRT 中可用。
