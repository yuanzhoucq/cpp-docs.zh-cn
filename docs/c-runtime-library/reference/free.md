...
*memblock*以前分配给要释放的内存块。

## <a name="remarks"></a>备注

**免费**函数释放的内存块 (*memblock*)，为以前分配通过调用**calloc**， **malloc**，或**realloc**。 已释放的字节数等于请求时已分配块的字节数 (或重新分配的情况下**realloc**)。 如果*memblock*是**NULL**，忽略指针和**免费**立即返回。 尝试释放了无效的指针 (指向由未分配的内存块的指针**calloc**， **malloc**，或**realloc**) 可能会影响后续分配请求和会导致错误。
...
**免费**标记`__declspec(noalias)`，这意味着，保证函数不能修改全局变量。 有关详细信息，请参阅 [noalias](../../cpp/noalias.md)。
