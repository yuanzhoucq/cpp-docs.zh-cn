...
|`_topen`|`_open`|`_open`|`_wopen`|  
 ...
 `oflag` 是一个整数表达式，由在 \<fcntl.h> 中定义的以下一个或多个清单常量或常量组合构成。  
  
 `_O_APPEND`  
 在执行每个写入操作之前，将文件指针移动到文件末尾。  
  
 `_O_BINARY`  
 在二进制（未转换）模式下打开该文件。 （有关二进制模式的说明，请参阅 [fopen](../../c-runtime-library/reference/fopen-wfopen.md)。）  
...
>  
 ...
 当使用两个或多个清单常量来形成 `oflag` 自变量时，这些常量将与按位 OR 运算符合并 (`|`)。 有关二进制模式和文本模式的讨论，请参阅[文本和二进制模式文件 I/O](../../c-runtime-library/text-and-binary-mode-file-i-o.md)。  
 ...
 当给定这两个常量时，将使用按位 OR 运算符 (`|`) 连接它们。 在 Windows 中，所有文件均可读；不会提供只写权限。 因此，模式 `_S_IWRITE` 和 `_S_IREAD` `|` `_S_IWRITE` 是等效的。  
