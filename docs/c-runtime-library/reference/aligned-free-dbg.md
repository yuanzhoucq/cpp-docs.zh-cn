...
## <a name="remarks"></a>备注  
 `_aligned_free_dbg` 函数是 [_aligned_free](../../c-runtime-library/reference/aligned-free.md) 函数的调试版本。 未定义 [_DEBUG](../../c-runtime-library/debug.md) 时，每个对 `_aligned_free_dbg` 的调用都会减少到对 _`aligned_free` 的调用。 \_`aligned_free` 和 `_aligned_free_dbg` 都可释放基堆中的内存块，但是 `_aligned_free_dbg` 还包含一种调试功能：能够在堆链接列表中保留已释放的块，以便模拟内存不足的情况。  
