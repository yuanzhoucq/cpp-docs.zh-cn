...
## 备注  
 这些函数中的每个创建并执行新进程，通过一组指向命令行参数的指针。  
  
 `_execv` 函数验证其参数.  如果 `cmdname` 是 null 指针，或者，如果 `argv` 是 null 指针，为空数组的指针，或者，如果数组包含一个空字符串作为第一个参数，`_execv` 函数调用的参数无效处理程序如 [参数验证](../../c-runtime-library/parameter-validation.md)所述。  如果允许执行继续，则这些功能将 `EINVAL` 设置为 `errno` 并返回 \-1。  不启动任何进程。  
