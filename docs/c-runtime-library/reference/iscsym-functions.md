...
*c*<br/>
要测试的整数。 *c*应在该函数的窄字符版本 0-255 范围内。
...
这两 **__iscsym**并 **__iswcsym**返回非零值，如果*c*是字母、 下划线或数字。 这两 **__iscsymf**并 **__iswcsymf**返回非零值，如果*c*是一个字母或下划线。 每个例程将返回 0，如果*c*不满足测试条件。 使用这些函数的版本 **_l**后缀完全相同，只不过它们使用*区域设置*而不是当前区域设置传入用于其区域设置相关的行为。 有关详细信息，请参阅 [Locale](../../c-runtime-library/locale.md)。

## <a name="remarks"></a>备注

除非定义了预处理器宏 _CTYPE_DISABLE_MACROS，否则这些例程将被定义为宏。 在使用这些例程的宏版本时，可多次计算参数。 使用参数列表中带副作用的表达式时要小心。

为了向后兼容**iscsym**并**iscsymf**定义为宏时，才[ &#95; &#95;STDC&#95; &#95; ](../../preprocessor/predefined-macros.md)未定义或定义为 0;否则不进行定义。
...
**Iscsym**， **iscsymf**， **__iscsym**， **__iswcsym**， **__iscsymf**， **__iswcsymf**， **_iscsym_l**， **_iswcsym_l**， **_iscsymf_l**，并且 **_iswcsymf_l**例程Microsoft 专用。 有关其他兼容性信息，请参阅 [兼容性](../../c-runtime-library/compatibility.md)。
