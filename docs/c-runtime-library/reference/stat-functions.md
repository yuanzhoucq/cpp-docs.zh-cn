...
## <a name="remarks"></a>备注  
 ...
 `_stat` 与 `_stat64i32`等效， `struct``_stat` 包含 64 位时间。 This is true unless `_USE_32BIT_TIME_T` is defined, in which case the old behavior is in effect; `_stat` uses a 32-bit time, and `struct``_stat` contains a 32-bit time. 这一点也适用于 `_stati64`。  
