...
ms.locfileid: "75300672"
---
# <a name="spawnvp"></a>spawnvp

特定于 Microsoft 的函数名称 `spawnvp` 是[_spawnvp](spawnvp-wspawnvp.md)函数的不推荐使用的别名。 默认情况下，它会生成[编译器警告（等级3） C4996](../../error-messages/compiler-warnings/compiler-warning-level-3-c4996.md)。 名称已弃用，因为它不遵循特定于实现的名称的标准 C 规则。 但是，该函数仍受支持。
