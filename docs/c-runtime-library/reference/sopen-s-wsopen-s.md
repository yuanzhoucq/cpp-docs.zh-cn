---
title: _sopen_s、_wsopen_s | Microsoft 文档
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
...
**EINVAL**| 无效*oflag*， *shflag*，或*pmode*自变量，或*pfh*或*filename*是空指针。
**EMFILE**|没有更多可用的文件描述符。
**ENOENT**|未找到文件或路径。

如果将无效参数传递到该函数，则调用无效参数处理程序，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。 如果允许执行继续，则**errno**设置为**EINVAL**和**EINVAL**返回。
...
**_Sopen_s**函数将打开指定的文件*filename*和使文件共享的读取或写入，做好准备，由定义*oflag*和*shflag*. **_wsopen_s**是宽字符版本的 **_sopen_s**; *filename*参数 **_wsopen_s**是宽字符字符串。 **_wsopen_s**和 **_sopen_s**否则具有相同行为。
...
整数表达式*oflag*通过合并一个或多个清单常量而中定义形成\<fcntl.h >。 当两个或多个常量构成参数*oflag*，使用按位 OR 运算符合并它们: ( **&#124;** )。
...
自变量*shflag*是包含在中定义的以下清单常量之一的常量表达式\<share.h >。
...
*Pmode*自变量始终是必需的与不同在 **_sopen**。 当指定 **_O_CREAT**，如果文件不存在， *pmode*指定第一次关闭新文件时设置的文件的权限设置。 否则为*pmode*将被忽略。 *pmode*是一个包含一个或两个清单常量的整数表达式 **_S_IWRITE**和 **_S_IREAD**，其定义中\<sys\stat.h >。 当给定这两个常量时，将使用按位“或”运算符合并它们。 含义*pmode*如下。
