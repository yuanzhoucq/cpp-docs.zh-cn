...
## <a name="remarks"></a>备注  
 `_sopen_s` 函数将打开由 `filename` 指定的文件并使该文件做好共享的读写准备，如 `oflag` 和 `shflag` 所定义。 `_wsopen_s` 是 `_sopen_s` 的宽字符版本；`filename` 的 `_wsopen_s` 参数是宽字符字符串。 除此以外，`_wsopen_s` 和 `_sopen_s` 的行为完全相同。  
...
|`_tsopen_s`|`_sopen_s`|`_sopen_s`|`_wsopen_s`|  
 ...
 整数表达式 `oflag` 是通过合并在 \<fcntl.h> 中定义的一个或多个清单常量而形成的。 当两个或多个常量构成 `oflag` 参数时，这些常量将与按位 OR 运算符合并 ( `|` )。  
...
>  `_O_TRUNC` 标志会损坏指定文件的内容。  
 ...
 如果使用 `_sopen_s`（追加模式）和 `_O_WRONLY | _O_APPEND`、`_O_WTEXT` 或 `_O_U16TEXT` 调用 `_O_U8TEXT`，则它首先会尝试打开该文件以供读取和写入、读取 BOM，然后重新打开它以供只写。 如果无法打开该文件以供读取和写入，则它将打开该文件以供只写，并使用 Unicode 模式设置的默认值。  
