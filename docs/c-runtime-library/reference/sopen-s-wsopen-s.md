...
如果出现错误，则通过*pfh*返回-1 （除非*pfh*为空指针）。
...
整数表达式*oflag*是通过组合 \<fcntl.h> > 中定义的一个或多个清单常量而形成的。 当两个或多个常量构成参数*oflag*时，它们将与按位 "或" **&#124;** 运算符（）组合在一起。
...
如果通过 **_O_WRONLY** |  **_O_APPEND** （追加模式）和 **_O_WTEXT**、 **_O_U16TEXT**或 **_O_U8TEXT**调用 **_sopen_s** ，它首先会尝试打开文件进行读取和写入、读取 BOM，然后将其重新打开以进行写入。 如果无法打开该文件以供读取和写入，则它将打开该文件以供只写，并使用 Unicode 模式设置的默认值。
...
*Pmode*参数始终是必需的，与 **_sopen**中的不同。 指定 **_O_CREAT**时，如果该文件不存在， *pmode*将指定文件的权限设置，该设置将在首次关闭新文件时设置。 否则，将忽略*pmode* 。 *pmode*是一个整数表达式，其中包含一个或两个清单常量 **_S_IWRITE**和 **_S_IREAD**，它们是在 \<sys\stat.h > 中定义的。 当给定这两个常量时，将使用按位“或”运算符合并它们。 *Pmode*的含义如下所示。
