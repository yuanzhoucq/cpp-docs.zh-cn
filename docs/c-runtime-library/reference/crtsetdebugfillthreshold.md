...
description: 使用 _CrtSetDebugFillThreshold 函数可设置要填充安全 CRT 函数的最大缓冲区量。
ms.date: 10/31/2019
api_name:
...
某些安全增强型 CRT 函数的调试版本将使用特殊字符（0xFE）来填充传递给它们的缓冲区。 此填充字符有助于查找向函数传递错误大小的情况。 遗憾的是，这样也会降低性能。 若要提高性能，请使用 **_CrtSetDebugFillThreshold**对大于*newThreshold*阈值的缓冲区禁用缓冲区填充。 如果*newThreshold*的值为0，则将对所有缓冲区禁用该值。
...
此函数是 Microsoft 特定函数。 有关兼容性的详细信息，请参阅 [兼容性](../../c-runtime-library/compatibility.md)。
