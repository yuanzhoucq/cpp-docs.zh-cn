...
**_realloc_dbg**是[realloc](realloc.md)函数的调试版本。 未定义[_debug](../../c-runtime-library/debug.md)时，对 **_realloc_dbg**的每次调用都会减少到对**realloc**的调用。 **Realloc**和 **_realloc_dbg**都重新分配基堆中的内存块，但 **_realloc_dbg**提供了若干调试功能：要测试泄漏的块的用户部分两侧的缓冲区，用于测试泄漏的块类型参数跟踪特定分配类型和*文件名*/*linenumber*信息，以确定分配请求的源。
...
如果内存分配失败或所需的内存量（包括前面提到的开销）超过 **_HEAP_MAXREQ**， **_realloc_dbg**会将**errno**设置为**ENOMEM** 。 有关此代码及其他错误代码的信息，请参阅 [errno、_doserrno、_sys_errlist 和 _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。
