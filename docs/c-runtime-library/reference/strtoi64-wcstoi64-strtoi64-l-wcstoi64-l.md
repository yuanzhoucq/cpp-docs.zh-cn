...
**_strtoi64**返回表示字符串中的值*strSource*，只是时表示形式将导致溢出，在此情况下它返回 **_I64_MAX**或 **_I64_MIN**。 如果无法执行任何转换，函数将返回 0。 **_wcstoi64**类似地为返回值**strtoi64**。

**_I64_MAX**和 **_I64_MIN**限制中定义。H。
...
**_Strtoi64**函数将转换*strSource*到 **__int64**。 这两个函数停止读取字符串*strSource*它们不能将其识别为一个数字部分的第一个字符。 这可能是终止 null 字符，也可能是第一个数字的字符大于或等于*基*。 **_wcstoi64**是宽字符版本的 **_strtoi64**; 其*strSource*参数是宽字符字符串。 否则这些函数具有相同行为。
...
区域设置的**LC_NUMERIC**类别设置确定基数字符在识别*strSource * *;* 详细信息，请参阅[setlocale](setlocale-wsetlocale.md)。 不带 _l 后缀的函数使用当前区域设置;**_strtoi64_l**和 **_wcstoi64_l**相同相应函数而无需 **_l**后缀只不过它们使用传递的区域设置。 有关详细信息，请参阅 [Locale](../../c-runtime-library/locale.md)。

如果*endptr*不**NULL**，停止扫描的字符的指针指向的位置存储*endptr*。 如果可以不执行任何转换 （未找到有效的数字或指定了无效的基本） 的值*strSource*指向的位置存储*endptr*。

**_strtoi64**需要*strSource*以指向以下形式的字符串：
