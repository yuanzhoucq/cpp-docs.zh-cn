...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "_aligned_realloc_dbg 函数"
  - "aligned_realloc_dbg 函数"
...
## 备注  
 ...
 如果内存分配失败，或者如果所需的内存量（包括之前提到过的开销）超过 `_HEAP_MAXREQ`，则 `_aligned_realloc_dbg` 将 `errno` 设置为 `ENOMEM`。  有关此错误代码和其他错误代码的信息，请参阅 [errno、\_doserrno、\_sys\_errlist 和 \_sys\_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。  
