...
## 备注  
 ...
 `mbrtowc` 函数的可重启性不同于 [mbtowc、\_mbtowc\_l](../../c-runtime-library/reference/mbtowc-mbtowc-l.md)。  转换状态存储在 `mbstate` 中，以便后续调用相同的或其他可重启函数。  混合使用可重启函数和不可重启函数时，结果不确定。  例如，如果使用 `wcsrlen`（而非 `wcslen`）的后续调用，则应用程序应使用 `wcsrtombs`，而不是 `wcstombs`。  
