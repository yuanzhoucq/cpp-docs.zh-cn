...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "_aligned_offset_recalloc_dbg 函数"
  - "aligned_offset_recalloc_dbg 函数"
...
## 备注  
 `_aligned_offset_realloc_dbg` 是 [\_aligned\_offset\_recalloc](../../c-runtime-library/reference/aligned-offset-recalloc.md) 函数的调试版本。  未定义 [\_DEBUG](../../c-runtime-library/debug.md) 时，每个对 `_aligned_offset_recalloc_dbg` 的调用都会减少到对 \_`aligned_offset_recalloc` 的调用。  \_`aligned_offset_recalloc` 和 `_aligned_offset_recalloc_dbg` 都可重新分配基堆中的内存块，但是 `_aligned_offset_recalloc_dbg` 还包含几种调试功能：用于测试泄漏的块的用户部分两侧的缓冲区、用于跟踪特定分配类型的块类型参数，以及用于确定分配请求的源的 `filename`\/`linenumber` 信息。  
 ...
 如果内存分配失败或请求的大小 \(`num` \* `size`\) 大于 `_HEAP_MAXREQ`，则此函数会将 `errno` 设置为 `ENOMEM`。  有关 `errno`的更多信息，请参见[errno、\_doserrno、\_sys\_errlist 和 \_sys\_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。  此外，`_aligned_offset_recalloc_dbg` 将验证其参数。  如果 `alignment` 不是 2 的幂或 `offset` 大于或等于请求的大小且非零，则此函数调用的参数处理程序无效，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。  如果允许执行继续，则此函数将返回 `NULL` 并将 `errno` 设置为 `EINVAL`。  
