...
**_Mbcjistojms**函数将日本行业标准 (JIS) 字符转换为 Microsoft 日文汉字 (Shift JIS) 字符。 仅当主管和结尾字节在范围内 0x21-0x7E，转换的字符。 如果超出此范围是主管或试用字节**errno**设置为**EILSEQ**。 有关此代码及其他错误代码的详细信息，请参阅 [errno、_doserrno、_sys_errlist 和 _sys_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。
...
值*c*应为 16 位值，其上限的 8 位表示要转换的字符的前导字节，其较低的 8 位表示结尾字节。
