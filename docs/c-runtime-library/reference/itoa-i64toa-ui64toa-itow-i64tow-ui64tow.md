---
title: "_itoa、_i64toa、_ui64toa、_itow、_i64tow、_ui64tow | Microsoft 文档"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
## <a name="remarks"></a>备注  
 `_itoa`、`_i64toa` 和 `_ui64toa` 函数将将给定的 `value` 参数数字转换为以 null 结尾的字符串并将结果（`_itoa` 最多 33 个字符，`_i64toa` 和 `_ui64toa` 最多 65 个字符）存储在 `str` 中。 如果 `radix` 等于 10 且 `value` 为负，则已存储字符串的第一个字符为减号 ( `-` )。 `_itow`、`_i64tow` 和 `_ui64tow` 分别是 `_itoa`、`_i64toa` 和 `_ui64toa` 的宽字符版本。  
