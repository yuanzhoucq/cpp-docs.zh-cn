...
**freopen_s**函数关闭当前与*流*关联的文件，并将*流*重新分配给*路径*指定的文件。 **_wfreopen_s**是 **_freopen_s**的宽字符版本;**_wfreopen_s的***路径*和*模式*参数是宽字符字符串。 **_wfreopen_s**和 **_freopen_s**行为相同。

如果任何*pFile、**路径*、*模式*或*流*为**NULL，** 或者如果*路径*是空字符串，则这些函数将调用无效的参数处理程序，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。 如果允许执行继续，这些函数将**errno**设置为**EINVAL**并返回**EINVAL**。
...
**freopen_s**通常用于将预先打开的文件**stdin、std****和** **stder**重定向到用户指定的文件。 与*流*关联的新文件将打开 mode ，该*模式*是指定文件请求的访问类型的字符串，如下所示：

|*模式*|访问|
|-|-|
| **"r"** | 打开以便读取。 如果文件不存在或找不到，**则freopen_s**调用失败。 |
