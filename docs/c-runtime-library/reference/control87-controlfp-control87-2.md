...
**_Control87**函数获取并设置浮点控制字。 浮点控制字允许程序在浮点数学包中更改精度、舍入和无穷模式，具体取决于平台。 此外可以使用 **_control87**屏蔽或取消屏蔽浮点异常。 如果为值*掩码*等于 0， **_control87**获取浮点控制字。 如果*掩码*是为非零，将设置为控制字为新值： 对于位于任何位 （即等于 1） 中*掩码*中的对应位*新*用于更新管理word。 换而言之， **fpcntrl** = ((**fpcntrl** & ~*掩码*) &#124; (*新* & *掩码*))其中**fpcntrl**是浮点控制字。
