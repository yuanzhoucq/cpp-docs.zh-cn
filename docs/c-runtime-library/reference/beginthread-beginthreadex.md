...
如果成功，则这些函数将返回新创建的线程的句柄;但是，如果新创建的线程退出速度太快， **_beginthread**可能不会返回有效句柄。 （请参见“备注”节中的讨论。）出现错误时，如果线程过多， **_beginthread**将返回-1L， **Errno**设置为**EAGAIN** ，如果参数无效或堆栈大小不正确，则为**EINVAL** ; 如果资源不足，则设置为**EACCES**如内存）。 出现错误时， **_beginthreadex**将返回0，并设置**errno**和 **_doserrno** 。
...
调用 **_beginthread**或 **_beginthreadex**时，操作系统将处理堆栈的分配;无需将线程堆栈的地址传递给这些函数中的任何一个。 此外， *stack_size*参数可以为0，在这种情况下，操作系统使用的值与为主线程指定的堆栈相同。
