...
**_Sopen**函数将打开*文件名*指定的文件，并准备文件以供*oflag*和*shflag*定义的共享读取或写入。 **_wsopen**是 **_sopen**的宽字符版本; **_wsopen**的*filename*参数是宽字符字符串。 否则 **_wsopen**和 **_sopen**的行为相同。
...
整数表达式*oflag*是通过组合以下一个或多个 \<fcntl.h> > 中定义的清单常量构成的。 当两个或多个常量构成参数*oflag*时，它们将与按位 "或" **&#124;** 运算符（）组合在一起。
...
如果通过 **_O_WRONLY** |  **_O_APPEND** （追加模式）和 **_O_WTEXT**、 **_O_U16TEXT**或 **_O_U8TEXT**调用 **_sopen** ，它首先会尝试打开文件进行读取和写入、读取 BOM，然后将其重新打开以进行写入。 如果无法打开该文件以供读取和写入，则它将打开该文件以供只写，并使用 Unicode 模式设置的默认值。
...
仅当指定 **_O_CREAT**时， *pmode*参数是必需的。 如果文件不存在，则*pmode*指定文件的权限设置，该设置是在首次关闭新文件时设置的。 否则，将忽略*pmode* 。 *pmode*是一个整数表达式，其中包含一个或两个清单常量 **_S_IWRITE**和 **_S_IREAD**，它们是在 \<sys\stat.h > 中定义的。 当给定这两个常量时，将使用按位“或”运算符合并它们。 *Pmode*的含义如下所示。
...
如果未授予写入权限，则该文件为只读。 在 Windows 操作系统中，所有文件均可读；不可能提供只写权限。 因此， **_S_IWRITE**和 **_S_IREAD** | 的模式 **_S_IWRITE**是等效的。
