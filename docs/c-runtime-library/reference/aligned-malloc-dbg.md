...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "_aligned_malloc_dbg 函数"
  - "aligned_malloc_dbg 函数"
...
## 备注  
 ...
 如果内存分配失败，或者如果所需的内存量（包括之前提到过的开销）超过 `_HEAP_MAXREQ`，则 `_aligned_malloc_dbg` 将 `errno` 设置为 `ENOMEM`。  有关此错误代码和其他错误代码的信息，请参阅 [errno、\_doserrno、\_sys\_errlist 和 \_sys\_nerr](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md)。  此外，`_aligned_malloc_dbg` 将验证其参数。  如果 `alignment` 不是 2 的幂或 `size` 是零，则此函数调用的参数处理程序无效，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。  如果允许执行继续，则此函数将返回 `NULL` 并将 `errno` 设置为 `EINVAL`。  
