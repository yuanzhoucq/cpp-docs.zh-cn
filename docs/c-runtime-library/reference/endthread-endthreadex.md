...
**_endthread**会自动关闭线程句柄。 （此行为与 Win32 **ExitThread** API 不同。）因此，当你使用 **_beginthread**和 **_endthread**时，不要通过调用 Win32 [CloseHandle](/windows/win32/api/handleapi/nf-handleapi-closehandle) API 来显式关闭线程句柄。
