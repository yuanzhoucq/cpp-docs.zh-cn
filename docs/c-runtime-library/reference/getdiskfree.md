...
```  
 ...
 此函数验证其参数。 如果 `driveinfo` 指针为 `NULL` 或者 `drive` 指定无效的驱动器，则此函数将调用无效参数处理程序，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。 如果允许继续执行，则函数将返回 `EINVAL`，并且将 `errno` 设置为 `EINVAL`。 有效驱动器值范围为 0 到 26。 `drive` 值 0 指定当前的驱动器；之后，数字映射到英语字母表中的字母，例如 1 表示驱动器 A，3 表示驱动器 C，依此类推。  
