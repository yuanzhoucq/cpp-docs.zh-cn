...
## 返回值  
 除非表示形式导致溢出（在这种情况下，它将返回 `LLONG_MAX` 或 `LLONG_MIN`），否则 `strtoll` 返回该字符串 `nptr` 中表示的值。  如果不能执行转换，则该函数会返回 0。  `wcstoll` 返回值类似于 `strtoll`。  
  
 `LLONG_MAX` 和 `LLONG_MIN` 在 LIMITS.H. 中进行定义。  
...
## 备注  
 `strtoll` 函数将 `nptr` 转换为 `long long`。  当他们不能识别的第一个数字字符出现时，函数都停止读入字符串 `nptr`。  这可能是终止 null 字符，也可能是大于或等于 `base` 的第一个数字字符。  `wcstoll` 是 `strtoll` 的宽字符版本；其 `nptr` 参数是宽字符字符串。  否则这三个函数否则具有相同行为。  
