...
将整数转换为字符串。 这些是[_itoa的版本，_itow功能](itoa-itow.md)具有安全增强功能，如[CRT 中的安全功能中](../../c-runtime-library/security-features-in-the-crt.md)所述。
...
|any|**空**|any|any|**埃因瓦尔**|
|any|any|<=0|any|**埃因瓦尔**|
|any|any|<= 所需结果字符串的长度|any|**埃因瓦尔**|
|any|any|any|*半径*< 2 或*半径*> 36|**埃因瓦尔**|

### <a name="security-issues"></a>安全问题

如果*缓冲区*不指向有效内存且不是**NULL，** 或者缓冲区的长度不足以容纳结果字符串，则这些函数可能会生成访问冲突。

## <a name="remarks"></a>备注

除参数和返回值外 **，_itoa_s**和 **_itow_s**函数子具有与相应的不太安全 **_itoa**和 **_itow**版本相同的行为。

在 C++ 中，使用这些函数由模板重载简化；重载可以自动推导出缓冲区长度 (不再需要指定大小自变量)，并且它们可以自动用以更新、更安全的对应物替换旧的、不安全的函数。 有关详细信息，请参阅[安全模板重载](../../c-runtime-library/secure-template-overloads.md)。

这些函数的调试库版本首先用 0xFE 填充缓冲区。 若要禁用此行为，请使用 [_CrtSetDebugFillThreshold](crtsetdebugfillthreshold.md)。

CRT 包括方便的宏，用于定义转换多个通用基的每个整数类型（包括空终止符和符号字符）的最长可能值所需的缓冲区大小。 有关详细信息，请参阅[最大转化计数宏](itoa-itow.md#maximum-conversion-count-macros)。
...
这些功能特定于 Microsoft。 有关兼容性的详细信息，请参阅[兼容性](../../c-runtime-library/compatibility.md)。

## <a name="example"></a>示例

此示例演示了一些整数转换函数的使用。 请注意[，_countof](countof-macro.md)宏仅适用于在数组声明对编译器可见时确定缓冲区大小，而不是用于已衰减到指针的参数。
