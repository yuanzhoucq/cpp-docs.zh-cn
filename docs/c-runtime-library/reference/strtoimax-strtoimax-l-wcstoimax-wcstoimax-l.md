...
**strtoimax**返回在字符串*strSource*中表示的值，但当表示形式会导致溢出时，它将返回**INTMAX_MAX**或**INTMAX_MIN**， **errno**设置为**ERANGE**. 如果无法执行任何转换，则函数返回 0。 **wcstoimax**将类似值返回到**strtoimax**。
...
区域设置的**LC_NUMERIC**类别设置决定了*strSource*中的基数字符的识别;有关详细信息，请参阅[setlocale、_wsetlocale](setlocale-wsetlocale.md)。 没有 **_l**后缀的函数使用当前区域设置; **_strtoimax_l**和 **_wcstoimax_l**与没有 **_l**后缀的相应函数相同，只不过它们改用传入的区域设置。 有关详细信息，请参阅 [Locale](../../c-runtime-library/locale.md)。
