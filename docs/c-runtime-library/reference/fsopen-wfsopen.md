...
|术语|定义|
|----------|----------------|
|**“r”**|打开以便读取。 如果文件不存在或找不到，则 **_fsopen**调用失败。|
...
|术语|定义|
|----------|----------------|
|**t**|在文本（转换）模式下打开文件。 在此模式下，在输出时，回车换行符（CR-LF）组合转换为单行源（LF），换行符将转换为 CR-LF 组合。 CTRL+Z 也将在输入时解释为文件尾字符。 在打开以进行读取或读取/写入的文件中， **_fsopen**将检查文件末尾的 CTRL + Z 并将其删除（如果可能）。 这样做的原因是，使用[fseek](fseek-fseeki64.md)和[FTELL](ftell-ftelli64.md)在以 CTRL + Z 结尾的文件中移动时，可能会导致[fseek](fseek-fseeki64.md)在文件结尾附近的行为不正确。|
