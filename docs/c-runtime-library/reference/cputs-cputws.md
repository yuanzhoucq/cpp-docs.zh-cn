...
**_cputs**函数*通过直接*str 到控制台来写入指向的 null 端接字符串。 回车换行符 (CR-LF) 组合不会自动追加到字符串。

此函数验证其参数。 如果*str*为**NULL，** 则调用无效参数处理程序，如[参数验证](../../c-runtime-library/parameter-validation.md)中所述。 如果允许继续执行，则**errno**设置为**EINVAL，** 并返回 -1。
