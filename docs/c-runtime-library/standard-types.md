...
|`intptr_t`（长整型或 **`__int64`** ，具体取决于目标平台）|在 Win32 和 Win64 平台上存储指针（或 HANDLE）。|STDDEF.H 和其他包含文件|
...
|`ptrdiff_t`（长整型或 **`__int64`** ，具体取决于目标平台）|两个指针相减的结果。|CRTDEFS.H|
...
|`size_t`（unsigned __int64 或 unsigned integer，取决于目标平台）|运算符的结果 **`sizeof`** 。|CRTDEFS.H 和其他包含文件|
...
|`uintptr_t`（长整型或 **`__int64`** ，具体取决于目标平台）|`intptr_t` 的 unsigned integer 或 unsigned __int64 版本|STDDEF.H 和其他包含文件|
