...
- 区域设置代码页。 大量运行时例程的行为取决于当前的区域设置（包括区域设置代码页）。 （有关详细信息，请参阅[与区域设置相关的例程](../c-runtime-library/locale.md)。）默认情况下，Microsoft 运行时库中所有与区域设置相关的例程均使用对应于 "C" 区域设置的代码页。 运行时可使用对 [setlocale](../c-runtime-library/reference/setlocale-wsetlocale.md) 的调用来更改或查询所使用的区域设置代码页。
