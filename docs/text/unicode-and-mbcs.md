...
ms.locfileid: "62410520"
---
# <a name="unicode-and-mbcs"></a>Unicode 和 MBCS

Microsoft 基础类 (MFC) 库、 Visual C 运行时库C++，和视觉对象C++开发环境可以帮助在国际化编程。 它们提供：
...
- 提供的简化为国际市场编写应用程序的 MBCS 编程的工具支持。
   ...
   Mbcs 的版本的 Windows 操作系统的视觉对象上运行时C++开发系统，包括集成的源代码代码编辑器、 调试器和命令行工具，是完全支持 MBCS 的。 有关详细信息，请参阅[视觉对象中的 MBCS 支持C++ ](../text/mbcs-support-in-visual-cpp.md)。

> [!NOTE]
>  在本文档中，MBCS 用于描述对多字节字符的所有非 Unicode 支持。 视觉对象中C++，MBCS 始终意味着 DBCS。 不支持 2 个字节宽的字符集。
