...
-   通过将合并你的代码中的以下预处理器语句使用直接映射。  
    ...
    ```  
     ...
     采用此方法时，您必须小心以确保使用适当的数据类型的字符串自变量和字符串返回值。 可以使用类型强制转换来确保类型匹配正确，也可以使用 _TXCHAR 一般文本数据类型。 **_TXCHAR**图来键入`char`在 SBCS 代码，但图来键入`unsigned` `char` MBCS 代码中。 有关一般文本宏的详细信息，请参阅[一般文本映射](../c-runtime-library/generic-text-mappings.md)中*运行时库参考*。  
