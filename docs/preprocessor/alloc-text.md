...
**Alloc_text**杂注不处理C++成员函数或重载函数。 它仅适用于用 C 链接声明的函数 (即, 用**extern "C"** 链接规范声明的函数)。 如果尝试对带 C++ 链接的函数使用此杂注，则会生成编译器错误。

由于不支持使用`__based`的函数寻址, 因此, 指定节位置需要使用**alloc_text**杂注。 *Textsection*指定的名称应用双引号引起来。
