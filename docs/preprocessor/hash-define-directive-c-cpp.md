...
`#define` *标识符* `(` *标识符*优化`,`*...* `,`*标识符*优化`)`*令牌字符串*选择  
...
第二种语法形式定义一个带有参数的类似函数的宏。 此形式接受必须出现在括号内的参数的可选列表。 该宏的定义，每个后续匹配项后*标识符*(*标识符*选择，...，*标识符*opt) 使用的版本替换*令牌字符串*替换形参的实参的参数。  
