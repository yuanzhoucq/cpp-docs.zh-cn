---
title: no_search_namespace |Microsoft 文档
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-tools
ms.topic: reference
f1_keywords:
- no_search_namespace
dev_langs:
- C++
helpviewer_keywords:
- no_search_namespace attribute
ms.assetid: 482dc4a7-5acf-456a-a995-ef4ddd5f6018
author: corob-msft
ms.author: corob
ms.workload:
- cplusplus
ms.openlocfilehash: 63cfb62bc6c047d0d3970e8f7fe51a716e50cb3c
ms.sourcegitcommit: d55ac596ba8f908f5d91d228dc070dad31cb8360
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/07/2018
ms.locfileid: "33850237"
---
# <a name="nosearchnamespace"></a>no_search_namespace
**C + + 专用**  
  
 具有相同的功能[no_namespace](../preprocessor/no-namespace.md)属性，但在使用与 #import 指令的类型库上使用[auto_search](../preprocessor/auto-search.md)属性。  
  
## <a name="syntax"></a>语法  
  
```  
no_search_namespace  
```  
  
## <a name="remarks"></a>备注  
 **结束 c + + 专用**  
  
## <a name="see-also"></a>请参阅  
 [#import 属性](../preprocessor/hash-import-attributes-cpp.md)   
 [#import 指令](../preprocessor/hash-import-directive-cpp.md)