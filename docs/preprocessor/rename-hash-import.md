...
> **#import** *类型库***重命名（** "*OldName*" **，** "*NewName*" **）**
...
例如，假设类型库中存在名为 `MyParent` 的属性，并且在标头文件中定义了宏 `GetMyParent`，在 `#import` 前面使用了该宏。 由于 `GetMyParent` 是错误处理 `get` 属性的包装函数的默认名称，因此将发生名称冲突。 若要解决此问题，请在 `#import` 语句中使用以下特性：
...
这是因为名称 `GetMyParent` 仅出现在生成的类型库标头文件中。
