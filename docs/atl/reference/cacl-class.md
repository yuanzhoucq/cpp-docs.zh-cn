...
|[CAcl::GetAclEntries](#getaclentries)|检索 `CAcl` 对象中的访问控制列表（ACL）项。|
...
|[CAcl：： IsEmpty](#isempty)|测试项的 `CAcl` 对象。|
|[CAcl：： IsNull](#isnull)|返回 `CAcl` 对象的状态。|
|[CAcl::RemoveAce](#removeace)|从 `CAcl` 对象中移除特定的 ACE （访问控制项）。|
|[CAcl::RemoveAces](#removeaces)|从应用于给定 `CSid`的 `CAcl` 中移除所有 Ace （访问控制项）。|
...
|名称|说明|
|----------|-----------------|
|[CAcl：： operator const ACL *](#operator_const_acl__star)|将 `CAcl` 对象强制转换为 `ACL` 结构。|
...
此 typedef 指定用于定义访问控制项（ACE）对象的性质（如 ACCESS_ALLOWED_ACE_TYPE 或 ACCESS_DENIED_ACE_TYPE）的数组类型。 有关可能的类型的完整列表，请参阅[ACE_HEADER](/windows/win32/api/winnt/ns-winnt-ace_header)定义。
...
此方法将用 `CAcl` 对象中包含的每个 ACE 对象的详细信息填充数组参数。 如果不需要特定数组的详细信息，请使用 NULL。
...
如果 `CAcl` 对象不为 NULL，并且不包含任何项，则返回 TRUE。 如果 `CAcl` 对象为 NULL 或包含至少一个条目，则返回 FALSE。
...
从应用于给定 `CSid`的 `CAcl` 中移除 alls Ace （访问控制项）。
...
`CAcl` 可以设置为空，也可以设置为 NULL：这两个状态是不同的。
