...
> [!NOTE]
> `IDispEventImpl`和`IDispEventSimpleImpl`提供其自己实现**iunknown:: Queryinterface**启用每个`IDispEventImpl`或`IDispEventSimpleImpl`基类，以充当一个单独的 COM 标识，同时仍然允许在主要的 COM 对象的直接访问类成员。  
...
> [!NOTE]
>  如果你的类派生自多个`IDispEventSimpleImpl`类，你将需要消除调用此方法的范围调用具有特定基类你感兴趣的歧义。  
