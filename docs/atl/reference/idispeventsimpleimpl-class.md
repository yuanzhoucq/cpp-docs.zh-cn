...
|[IDispEventSimpleImpl::DispEventUnadvise](#dispeventunadvise)|中断与事件源的连接。|
...
`IDispEventSimpleImpl`提供实现事件调度接口的方法, 无需为该接口上的每个方法/事件提供实现代码。 `IDispEventSimpleImpl`提供`IDispatch`方法的实现。 你只需为你感兴趣处理的事件提供实现。
...
- 提供每个事件的类型信息, 方法是将指向[_ATL_FUNC_INFO](../../atl/reference/atl-func-info-structure.md)结构的指针作为参数传递给每个项。 在 x86 平台上, `_ATL_FUNC_INFO.cc`值必须与调用方法的回调函数 CC_CDECL。

- 调用[DispEventAdvise](#dispeventadvise) , 以在源对象与基类之间建立连接。

- 调用[DispEventUnadvise](#dispeventunadvise)断开连接。

对于需要处理事件`IDispEventSimpleImpl`的每个对象, 必须从 (为*nID*使用唯一值) 派生。 您可以通过 unadvising 对一个源对象重复使用基类, 然后针对不同的源对象进行通知, 但是单个对象可以同时处理的最大源对象数受`IDispEventSimpleImpl`基类的数目限制。

`IDispEventSimplImpl`提供与[IDispEventImpl](../../atl/reference/idispeventimpl-class.md)相同的功能, 只不过它不会从类型库获取有关接口的类型信息。 向导仅`IDispEventImpl`基于生成代码, 但您可以通过手动添加`IDispEventSimpleImpl`代码来使用。 当`IDispEventSimpleImpl`你没有描述事件接口的类型库, 或者想要避免使用类型库关联的开销时, 请使用。

> [!NOTE]
> `IDispEventImpl`和`IDispEventSimpleImpl`提供自己的`IUnknown::QueryInterface`实现来使每`IDispEventImpl`个`IDispEventSimpleImpl`或基类充当单独的 com 标识, 同时仍然允许直接访问主 com 对象中的类成员。
...
`DispEventAdvise`与中`pdiid`指定的事件源建立连接。
...
`DispEventAdvise`断开使用中`pdiid`指定的事件源建立的连接。

##  <a name="getidsofnames"></a>  IDispEventSimpleImpl::GetIDsOfNames

的这一`IDispatch::GetIDsOfNames`实现将返回 E_NOTIMPL。
...
的这一`IDispatch::GetTypeInfo`实现将返回 E_NOTIMPL。
...
的这一`IDispatch::GetTypeInfoCount`实现将返回 E_NOTIMPL。
...
`Unadvise`断开使用中`pdiid`指定的默认事件源建立的连接。
