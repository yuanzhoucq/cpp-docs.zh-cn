...
每个跟踪的第一部分将始终为 `ATL: QIThunk` 。 接下来是一个值，该值标识正在使用的特定*接口 thunk* 。 接口 thunk 是一个对象，用于维护引用计数，并提供此处使用的跟踪功能。 对 `QueryInterface` 接口的请求 `IUnknown` （在这种情况下，每次都将返回相同的 thunk 以符合 COM 的标识规则）的每次调用时都会创建一个新的接口 thunk。
