...
从 ATL 7.0，`CComModule` 被弃用：有关更多详细信息，请参阅[ATL Module 类](../../atl/atl-module-classes.md)。
...
`CComModule` 实例使用对象映射来维护一组类对象定义。 此对象映射实现为 `_ATL_OBJMAP_ENTRY` 结构的数组，并包含以下内容的信息：
...
运行 ATL COM 运行向导时，向导会自动生成 `_Module`、`CComModule` 的全局实例或从其派生的类。 有关 ATL 项目向导的详细信息，请参阅文章[创建 Atl 项目](../../atl/reference/creating-an-atl-project.md)。

除了 `CComModule`之外，ATL 还提供[CComAutoThreadModule](../../atl/reference/ccomautothreadmodule-class.md)，它为 Exe 和 Windows 服务实现了单元模型模块。 如果要在多个单元中创建对象，请从 `CComAutoThreadModule` 派生模块。
...
从 ATL 7.0，`CComModule` 已过时：有关更多详细信息，请参阅[ATL Module 类](../../atl/atl-module-classes.md)。
...
返回[m_hInstResource](#m_hinstresource)的数据成员。
...
返回[m_hInstTypeLib](#m_hinsttypelib)的数据成员。
...
[Init](#init)方法将 `m_hInst` 设置为传递到 `DLLMain` 或 `WinMain`的句柄。
...
[Init](#init)方法将 `m_hInstResource` 设置为传递到 `DLLMain` 或 `WinMain`的句柄。 可以显式将 `m_hInstResource` 设置为资源的句柄。

[GetResourceInstance](#getresourceinstance)方法返回存储在 `m_hInstResource`中的句柄。
...
[Init](#init)方法将 `m_hInstTypeLib` 设置为传递到 `DLLMain` 或 `WinMain`的句柄。 可以显式将 `m_hInstTypeLib` 设置为类型库的句柄。

[GetTypeLibInstance](#gettypelibinstance)方法返回存储在 `m_hInstTypeLib`中的句柄。
...
dwFlags<br/>
中指定要在注册表中输入的线程模型。 可能的值为 THREADFLAGS_APARTMENT、THREADFLAGS_BOTH 或 AUTPRXFLAG。
...
dwFlags<br/>
中确定类对象的连接类型。 可能的值为 REGCLS_SINGLEUSE、REGCLS_MULTIPLEUSE 或 REGCLS_MULTI_SEPARATE。 有关这些值的说明，请参阅 Windows SDK 中的[REGCLS](/windows/win32/api/combaseapi/ne-combaseapi-regcls) 。
...
请参阅[OBJECT_ENTRY_AUTO](object-map-macros.md#object_entry_auto) ，了解有关如何将条目添加到对象映射的信息。

`RegisterServer` 将 `DLLRegisterServer` DLL 自动调用，或通过 `/RegServer` 命令行选项 `WinMain` 为 EXE 运行。
...
`UnregisterServer` 将 `DLLUnregisterServer` DLL 自动调用，或通过 `/UnregServer` 命令行选项 `WinMain` 为 EXE 运行。
...
`UpdateRegistryClass` 调用[RegisterClassHelper](#registerclasshelper)或[UnregisterClassHelper](#unregisterclasshelper)，具体取决于*bRegister*的值。

通过指定[DECLARE_REGISTRY](registry-macros.md#declare_registry)宏，处理对象映射时将自动调用 `UpdateRegistryClass`。
...
通过指定[DECLARE_REGISTRY_RESOURCE](registry-macros.md#declare_registry_resource)或[DECLARE_REGISTRY_RESOURCEID](registry-macros.md#declare_registry_resourceid)宏，处理对象映射时将自动调用 `UpdateRegistryFromResourceD`。

> [!NOTE]
>  若要在运行时替换替换值，请不要指定 DECLARE_REGISTRY_RESOURCE 或 DECLARE_REGISTRY_RESOURCEID 宏。 而是创建 `_ATL_REGMAP_ENTRIES` 结构的数组，其中每个条目都包含一个与值配对的变量占位符，以便在运行时替换占位符。 然后调用 `UpdateRegistryFromResourceD`，同时传递*pMapEntries*参数的数组。 这会将 `_ATL_REGMAP_ENTRIES` 结构中的所有替换值添加到注册机构的替换地图。
...
类似于[UpdateRegistryFromResourceD](#updateregistryfromresourced) ，但 `UpdateRegistryFromResourceS` 创建指向 ATL 注册表组件（注册机构）的静态链接。

当处理对象映射时，如果将 `#define _ATL_STATIC_REGISTRY` 添加到*pch* （Visual Studio 2017 及更早版本中的*stdafx.h* ），将自动调用 `UpdateRegistryFromResourceS`。

> [!NOTE]
>  若要在运行时替换替换值，请不要指定[DECLARE_REGISTRY_RESOURCE](registry-macros.md#declare_registry_resource)或[DECLARE_REGISTRY_RESOURCEID](registry-macros.md#declare_registry_resourceid)宏。 而是创建 `_ATL_REGMAP_ENTRIES` 结构的数组，其中每个条目都包含一个与值配对的变量占位符，以便在运行时替换占位符。 然后调用 `UpdateRegistryFromResourceS`，同时传递*pMapEntries*参数的数组。 这会将 `_ATL_REGMAP_ENTRIES` 结构中的所有替换值添加到注册机构的替换地图。
