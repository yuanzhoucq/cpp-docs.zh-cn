...
|名称|描述|
|----------|-----------------|
|[IOleControlImpl::FreezeEvents](#freezeevents)|指示容器是否忽略或接受来自控件的事件。|
...
类`IOleControlImpl`提供[IOleControl](/windows/win32/api/ocidl/nn-ocidl-iolecontrol)接口的默认实现, 并通过在`IUnknown`调试版本中将信息发送到转储设备来实现。
...
在 ATL 的实现中`FreezeEvents` , 如果`bFreeze`为 TRUE, `m_nFreezeEvents`则递增控件类的数据成员; `m_nFreezeEvents`如果`bFreeze`为 FALSE, 则递减。
