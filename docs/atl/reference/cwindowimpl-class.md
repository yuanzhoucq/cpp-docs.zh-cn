...
除了 `CWindowImpl`之外，ATL 还提供[CContainedWindow](../../atl/reference/ccontainedwindowt-class.md)以创建包含在另一个对象中的窗口。

基类析构函数（~ `CWindowImplRoot`）可确保在销毁对象之前窗口已消失。

`CWindowImpl` 派生自 `CWindowImplBaseT`，后者派生自 `CWindowImplRoot`，后者派生自 `TBase` 和[CMessageMap](../../atl/reference/cmessagemap-class.md)。
...
*rect*<br/>
中指定窗口位置的[矩形](/previous-versions/dd162897\(v=vs.85\))结构。 可以通过指针或引用来传递 `RECT`。
...
`Create` 首先注册窗口类（如果尚未注册）。 新创建的窗口将自动附加到 `CWindowImpl` 对象上。

> [!NOTE]
>  如果已调用[SubclassWindow](#subclasswindow)，请不要调用 `Create`。

若要使用基于现有窗口类的窗口类，请从 `CWindowImpl` 派生类，并包含[DECLARE_WND_SUPERCLASS](window-class-macros.md#declare_wnd_superclass)宏。 现有窗口类的窗口过程保存在[m_pfnSuperWindowProc](#m_pfnsuperwindowproc)中。 有关详细信息，请参阅[CWindowImpl](../../atl/reference/cwindowimpl-class.md)概述。

> [!NOTE]
>  如果将0用作*MenuOrID*参数的值，则必须将其指定为0u （默认值），以避免出现编译器错误。
...
若要将现有窗口类作为超类，请从 `CWindowImpl` 派生类，并包含[DECLARE_WND_SUPERCLASS](window-class-macros.md#declare_wnd_superclass)宏来重写 `GetWndClassInfo`。 有关详细信息，请参阅[CWindowImpl](../../atl/reference/cwindowimpl-class.md)概述。
...
[CWindowImpl：:D efwindowproc](#defwindowproc)将消息信息发送到保存在 `m_pfnSuperWindowProc`中的窗口过程。

##  <a name="onfinalmessage"></a>CWindowImpl：： OnFinalMessage

接收最后一条消息后调用（通常为 WM_NCDESTROY）。
...
`OnFinalMessage` 的默认实现不执行任何操作，但你可以重写此函数以在销毁窗口之前处理清理。 如果要在销毁窗口后自动删除对象，则可以在此函数中调用**delete this** 。

##  <a name="subclasswindow"></a>CWindowImpl：： SubclassWindow

子类由*hWnd*标识的窗口，并将其附加到 `CWindowImpl` 对象上。
...
> [!NOTE]
>  如果已调用[Create](#create)，请不要调用 `SubclassWindow`。
...
`WindowProc` 使用默认消息映射（使用[BEGIN_MSG_MAP](message-map-macros-atl.md#begin_msg_map)声明）将消息定向到适当的处理程序。 如有必要，`WindowProc` 调用[DefWindowProc](#defwindowproc)进行其他消息处理。 如果未处理最后一条消息，`WindowProc` 执行以下操作：
