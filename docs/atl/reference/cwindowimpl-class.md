 ...
 *dwStyle*  
 [in]窗口的样式。 此值被合并在一起对窗口特征类提供的样式。 默认值为特征提供了类样式的完全控制。 有关可能的值的列表，请参阅[CreateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632679) Windows SDK 中。  
  
 *dwExStyle*  
 [in]扩展的窗口样式。 此值被合并在一起对窗口特征类提供的样式。 默认值为特征提供了类样式的完全控制。 有关可能的值的列表，请参阅[CreateWindowEx](http://msdn.microsoft.com/library/windows/desktop/ms632680) Windows SDK 中。  
...
|类型的窗口|窗口过程|  
|--------------------|----------------------|  
|一个窗口，基于一个新的窗口类，通过指定[{2&gt;declare_wnd_class&lt;2](window-class-macros.md#declare_wnd_class)宏。|[DefWindowProc](http://msdn.microsoft.com/library/windows/desktop/ms633572) Win32 函数。|  
