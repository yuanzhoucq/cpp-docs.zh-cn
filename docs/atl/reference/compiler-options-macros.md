...
|[_ATL_ENABLE_PTM_WARNING](#_atl_enable_ptm_warning)|定义此宏，以便使用C++标准兼容的语法，非标准语法用来初始化指向成员函数的指针时，将生成 C4867 编译器错误。|
...
ATL 和 MFC 库已更改，以匹配该视觉对象C++编译器的改进了的标准C++符合性。 根据 ANSIC++标准，应该是指向类成员函数的指针的语法`&CMyClass::MyFunc`。
