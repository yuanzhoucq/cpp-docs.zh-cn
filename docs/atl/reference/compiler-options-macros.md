...
|[_ATL_ENABLE_PTM_WARNING](#_atl_enable_ptm_warning)|定义此宏以便使用C++符合标准的语法, 在使用非标准语法初始化指向成员函数的指针时, 将生成 C4867 编译器错误。|
...
在 Visual C++ .net 2002 之前, ATL 禁用了许多警告, 并将其保留为禁用状态, 以便它们永远不会显示在用户代码中。 尤其是在下列情况下：
...
如果添加`#define`了此操作, ATL 标头将小心保留这些警告的状态, 以使它们不会被全局禁用 (或者, 如果用户显式禁用各个警告, 而不是启用它们)。

默认情况下, `#define`新项目在*pch* (Visual Studio 2017 及更早版本) 中具有此设置。
...
如果定义了此构造函数, 则采用单个参数的所有 CString 构造函数都将使用 explicit 关键字进行编译, 这会阻止输入参数的隐式转换。 这意味着, 在定义 _UNICODE 时, 如果尝试使用 char * 字符串作为 CString 构造函数参数, 则会产生编译器错误。 如果需要防止在窄到宽字符串类型之间进行隐式转换, 请使用此宏。

通过对所有构造函数字符串参数使用 _T 宏, 无论是否定义 _UNICODE, 都可以定义 _ATL_CSTRING_EXPLICIT_CONSTRUCTORS 并避免编译错误。

##  <a name="_atl_enable_ptm_warning"></a>  _ATL_ENABLE_PTM_WARNING

定义此宏, 以强制将 ANSI C++标准兼容语法用于指向成员函数的指针。 如果使用非标准语法初始化指向成员函数的指针, 则使用此宏将导致生成 C4867 编译器错误。
...
ATL 和 MFC 库已更改, 以匹配 Microsoft C++编译器改进的标准C++符合性。 根据 ANSI C++标准, 指向类成员函数的指针的语法应该是`&CMyClass::MyFunc`。

如果未定义[_ATL_ENABLE_PTM_WARNING](#_atl_enable_ptm_warning) (默认情况), ATL/MFC 将禁用宏映射中的 C4867 错误 (特别是消息映射), 以便在早期版本中创建的代码可以像以前一样继续生成。 如果定义 **_ATL_ENABLE_PTM_WARNING**, 则代码应该符合C++标准。

但是, 非标准窗体已弃用。 需要将现有代码移到C++符合标准的标准语法。 例如，以下代码：
...
如果无法在类的构造函数和析构函数中初始化 vtable 指针, 则链接器可以消除 vtable 以及它指向的所有函数。 扩展到 **__declspec (novtable)** 。
...
如果要确保编译器不会内联函数, 请使用此符号, 即使它必须声明为内联, 以便可以将其放置在标头文件中。 扩展到 **__declspec (noinline)** 。
