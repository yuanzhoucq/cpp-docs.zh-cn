...
`CComSafeArray` 为 [SAFEARRAY Data Type](/windows/desktop/api/oaidl/ns-oaidl-tagsafearray) 类提供包装器，从而可以轻松地创建并管理几乎所有支持 VARIANT 的类型的一维数组和多维数组。
...
一个`CComSafeArray`对象可以创建从现有`SAFEARRAYBOUND`结构和数字的维度，或通过数组和更低绑定中指定的元素数。 如果数组为从访问C++，下限应为 0。 其他语言可能会允许其他值下限 （例如，-10 到 10 的范围的元素与 Visual 基本支持数组）。
...
[SAFEARRAY 数据类型](/windows/desktop/api/oaidl/ns-oaidl-tagsafearray)<br/>
