...
|[CWindow：:D estroyWindow](#destroywindow)|销毁与对象关联的窗口 `CWindow` 。|
|[CWindow：:D etach](#detach)|将窗口与对象分离 `CWindow` 。|
...
|[CWindow：： operator =](#operator_eq)|为对象分配 HWND `CWindow` 。|

### <a name="public-data-members"></a>公共数据成员

|名称|描述|
|----------|-----------------|
|[CWindow：： m_hWnd](#m_hwnd)|与对象关联的窗口的句柄 `CWindow` 。|
|[CWindow：： rcDefault](#rcdefault)|包含默认窗口尺寸。|

## <a name="remarks"></a>备注

`CWindow`提供用于在 ATL 中操作窗口的基本功能。 许多 `CWindow` 方法只是包装 Win32 API 函数之一。 例如，比较和的原型 `CWindow::ShowWindow` `ShowWindow` ：

|CWindow 方法|Win32 函数|
|--------------------|--------------------|
|**BOOL ShowWindow （int** `nCmdShow` **）;**|**BOOL ShowWindow （HWND** `hWnd` **，int** `nCmdShow` **）;**|

`CWindow::ShowWindow``ShowWindow`通过将 `CWindow::m_hWnd` 作为第一个参数传递来调用 Win32 函数。 `CWindow`直接包装 Win32 函数的每个方法都传递 `m_hWnd` 成员; 因此，大部分 `CWindow` 文档将向你介绍 Windows SDK。

> [!NOTE]
> 并非每个与窗口相关的 Win32 函数都由包装 `CWindow` ，并且并非每个 `CWindow` 方法都包装 Win32 函数。
...
- 使用以下派生自的类之一创建窗口或为窗口创建子类 `CWindow` ：
...
*rect*<br/>
中指定窗口位置[_U_RECT](../../atl/reference/u-rect-class.md)类型的变量。 默认值为 NULL。 当此参数为 NULL 时，使用的值 `CWindow::rcDefault` 。
...
> [!NOTE]
> `CWindow::CWindow`不创建窗口。 类[CWindowImpl](../../atl/reference/cwindowimpl-class.md)、 [CContainedWindow](../../atl/reference/ccontainedwindowt-class.md)和[CDialogImpl](../../atl/reference/cdialogimpl-class.md) （派生自的类 `CWindow` ）提供了一个创建窗口或对话框的方法，然后将该对话框分配给 `CWindow::m_hWnd` 。 你还可以使用[CreateWindow](/windows/win32/api/winuser/nf-winuser-createwindoww) Win32 函数。
...
将[m_hWnd](#m_hwnd)与 `CWindow` 对象分离，并将设置 `m_hWnd` 为 NULL。
...
## <a name="cwindowgetfont"></a><a name="getfont"></a>CWindow：： Ivsfontandcolorstorage.getfont 错误
...
## <a name="cwindowinvalidate"></a><a name="invalidate"></a>CWindow：：无效
...
将参数的 NULL 传递 `RECT` 到 `InvalidateRect` Win32 函数。
...
此方法的第二个版本使用[矩形](/windows/win32/api/windef/ns-windef-rect)结构来确定窗口的新位置、宽度和高度。
...
`CWindow`通过将[m_hWnd](#m_hwnd)成员设置为，为对象分配 HWND `hWnd` 。
...
此方法的第二个版本使用[矩形](/windows/win32/api/windef/ns-windef-rect)结构设置窗口的新位置、宽度和高度。
