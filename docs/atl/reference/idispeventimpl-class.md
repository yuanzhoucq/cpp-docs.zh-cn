...
> [!NOTE]
> `IDispEventImpl`和`IDispEventSimpleImpl`提供其自己实现**iunknown:: Queryinterface**启用每个`IDispEventImpl`和`IDispEventSimpleImpl`基类，以充当一个单独的 COM 标识，同时仍然允许直接访问类主要的 COM 对象中的成员。  
