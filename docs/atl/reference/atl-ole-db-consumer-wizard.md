...
- **数据源**
   ...
   借助“数据源”**** 按钮，可以使用指定 OLE DB 提供程序来创建指定数据源。 在你单击此按钮后，“数据链接属性”**** 对话框随即出现。 若要详细了解如何生成连接字符串和“数据链接属性”**** 对话框，请参阅 Windows SDK 文档中的[数据链接 API 概述](/previous-versions/windows/desktop/ms718102(v=vs.85))。
  ...
  - **连接**选项卡
      ...
      此选项卡的内容具体视你选择哪个提供程序而定。 尽管有许多类型的提供程序，本节涵盖两个最常见的连接：SQL 和 ODBC 数据。 其他类型数据就是在下面所述字段的基础上进行类似变体。

      对于 SQL 数据：

      1. **选择或输入服务器名称：** 单击下拉列表菜单以显示网络上的所有已注册数据服务器，然后选择一个。

      1. **输入要登录到服务器的信息：** 输入要登录到数据服务器的用户名和密码。
      ...
      1. **选择服务器上的数据库：** 单击下拉列表菜单以显示数据服务器上的所有已注册数据库，然后选择一个数据库。

         \- 或 -

         **将数据库文件附加到数据库名称：** 指定要用作数据库的文件;输入显式路径名称。
      ...
      1. **指定数据源：** 可以使用数据源名称或连接字符串。
         ...
         **使用连接字符串：** 输入已获取的连接字符串，或单击 **"生成**"按钮;将显示 **"选择数据源**"对话框。 选择文件或计算机数据源，并单击“确定”****。
  ...
  - **高级**选项卡
      ...
      **网络设置：** 指定**模拟级别**（服务器在模拟客户端时允许使用的模拟级别;直接对应于 RPC 模拟级别）**和保护级别**（客户端和服务器之间发送的数据保护级别;直接对应于 RPC 保护级别）。

      **其他：** 在**Connect 超时中**，指定超时发生前允许的空闲时间秒数。 在“访问权限”**** 中，指定数据连接的访问权限。
  ...
  - “所有”**** 选项卡
      ...
      单击“确定”**** 以完成操作。 此时，“选择数据库对象”**** 对话框显示。 在此对话框中，选择使用者将使用的表、视图或存储过程。
...
- **支持**
   ...
   选中复选框可以指定使用者要支持的更新种类（默认值为“无”）。 以下各项会在属性集映射中设置 [DBPROP_IRowsetChange](/previous-versions/windows/desktop/ms715892(v=vs.85))，以及 [DBPROP_UPDATABILITY](/previous-versions/windows/desktop/ms722676(v=vs.85)) 的相应条目。
