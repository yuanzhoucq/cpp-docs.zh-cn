 ...
 **连接**  
  ...
  对于 SQL 数据：  
    ...
    **使用数据源名称：** 此下拉列表显示在计算机中注册的数据源。 您可以设置提前使用 ODBC 数据源管理器数据源-或-**使用连接字符串：** 是输入连接字符串已获取，或单击**生成**按钮;**选择数据源**对话框随即出现。 选择文件或计算机的数据源，然后单击**确定**。  
 ...
 **特性化**  
 ...
 -   如果选择使用者**类型**的表，则向导将使用`db_source`并`db_table`属性来创建表和表访问器类声明中，并使用`db_column`以便创建列映射，例如：  
...
```
   ...
   而不是使用`CTable`模板类来声明表和表访问器类和 BEGIN_COLUMN_MAP 和 END_COLUMN_MAP 宏，以创建列映射，例如：  
...
-   如果选择使用者**类型**的命令，则向导将使用`db_source`并`db_command`属性，并使用`db_column`以便创建列映射，例如：  
