...
- **数据源**
   ...
   **数据源**按钮的使你可以设置使用指定的 OLE DB 访问接口的指定的数据源。 单击此按钮时**数据链接属性**对话框随即出现。 有关详细信息生成连接字符串和**数据链接属性**对话框中，请参阅[数据链接 API 概述](/previous-versions/windows/desktop/ms718102)Windows SDK 文档中。
   ...
   - **连接**选项卡
      ...
      1. **选择或输入服务器名称：** 单击要显示所有已注册的数据服务器在网络上的下拉列表菜单并选择一个。
      ...
      1. **选择在服务器上的数据库：** 单击要显示所有已注册的数据库数据服务器上的下拉列表菜单并选择一个。

         \- 或 -

         **将数据库文件附加为数据库名称：** 指定要用作数据库; 的文件输入的显式路径名。
      ...
      1. **指定数据的源：** 可以使用数据源名称或连接字符串。
         ...
         **使用连接字符串：** 请输入连接字符串已获取，或单击**构建**按钮;**选择数据源**对话框随即出现。 选择文件或计算机的数据源，然后单击**确定**。
   ...
   - **高级**选项卡
      ...
      **其他：** 在中**连接超时**，指定发生超时之前允许的空闲时间的秒数。 在中**访问权限**，在数据连接上指定的访问权限。
