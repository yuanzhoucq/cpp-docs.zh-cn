...
[IDataObject](/windows/win32/api/objidl/nn-objidl-idataobject)接口提供支持统一数据传输的方法。 `IDataObject`使用标准格式结构[FORMATETC](/windows/win32/api/objidl/ns-objidl-formatetc)和[STGMEDIUM](/windows/win32/api/objidl/ns-objidl-stgmedium)检索和存储数据。
