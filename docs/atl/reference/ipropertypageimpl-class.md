...
|[IPropertyPageImpl::Deactivate](#deactivate)|销毁用`Activate`创建的窗口。|
...
|[IPropertyPageImpl::SetObjects](#setobjects)|提供与属性页`IUnknown`关联的对象的指针数组。 这些对象通过调用来`Apply`接收当前属性页值。|
...
|[IPropertyPageImpl::m_pPageSite](#m_ppagesite)|指向属性页`IPropertyPageSite`与属性框通信时所使用的接口。|
|[IPropertyPageImpl::m_ppUnk](#m_ppunk)|指向与属性页关联`IUnknown`的对象的指针数组。|
|[IPropertyPageImpl::m_size](#m_size)|存储属性页对话框的高度和宽度 (以像素为单位)。|

## <a name="remarks"></a>备注

[IPropertyPage](/windows/win32/api/ocidl/nn-ocidl-ipropertypage)接口允许对象管理属性表中的特定属性页。 类`IPropertyPageImpl`提供此接口的默认实现, 并通过`IUnknown`在调试版本中将信息发送到转储设备来实现。
