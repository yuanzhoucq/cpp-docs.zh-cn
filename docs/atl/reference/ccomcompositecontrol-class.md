...
派生自类`CComCompositeControl`的类继承 ActiveX 复合控件的功能。 派生自`CComCompositeControl`的 ActiveX 控件由标准对话框承载。 这些类型的控件称为复合控件, 因为它们能够承载其他控件 (本机 Windows 控件和 ActiveX 控件)。

`CComCompositeControl`通过查找子类中的枚举数据成员, 标识用于创建复合控件的对话框资源。 此子类的成员 IDD 设置为将用作控件窗口的对话框资源的资源 ID。 下面是派生自`CComCompositeControl`的类应包含的数据成员的一个示例, 用于标识要用于控件窗口的对话框资源:
...
> [!NOTE]
>  为了使加速器能够正常`CComCompositeControl`运行, 必须在创建控件时加载快捷键对应表, 将句柄和加速器数传递回[IOleControlImpl:: GetControlInfo](../../atl/reference/iolecontrolimpl-class.md#getcontrolinfo), 最后销毁表控件的释放。
...
|E_POINTER  |此错误通常表示控件的事件接收器映射中的条目出现问题, 或在`IDispEventImpl`或`IDispEventSimpleImpl`基类中使用的模板自变量出现问题。|
|CONNECT_E_ADVISELIMIT  |连接点已达到其连接限制, 无法再接受。|
|CONNECT_E_CANNOTCONNECT  |接收器不支持此连接点所需的接口。|
|CONNECT_E_NOCONNECTION  |Cookie 值不表示有效的连接。 此错误通常表示控件的事件接收器映射中的条目出现问题, 或在`IDispEventImpl`或`IDispEventSimpleImpl`基类中使用的模板自变量出现问题。|

### <a name="remarks"></a>备注

此方法的基实现在事件接收器映射中搜索各个条目。 然后, 它将建议或 unadvises 连接到事件接收器映射的接收器条目所描述的 COM 对象。 此成员方法还依赖于这样一个事实: 派生类从的一个实例`IDispEventImpl`继承, 以对接收器映射中要通知或 unadvised 的每个控件继承。
