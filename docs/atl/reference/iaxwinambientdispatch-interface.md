---
title: IAxWinAmbientDispatch 接口 |Microsoft 文档
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-atl
...
|[get_AllowShowUI](#get_allowshowui)|**AllowShowUI**属性指定是否允许所承载的控件以显示其自身用户界面。|  
...
|[get_DisplayAsDefault](#get_displayasdefault)|**DisplayAsDefault**是允许控件查看它是否默认控件的环境属性。|  
...
|[get_MessageReflect](#get_messagereflect)|**MessageReflect**环境属性指定容器是否将反映到托管控件的消息。|  
|[get_OptionKeyPath](#get_optionkeypath)|**OptionKeyPath**属性指定对用户设置的注册表项路径。|  
|[get_ShowGrabHandles](#get_showgrabhandles)|**ShowGrabHandles**环境属性允许要查找是否它应绘制本身使用抓取句柄的控件。|  
|[get_ShowHatching](#get_showhatching)|**ShowHatching**环境属性使控件可以了解是否它应绘制自身阴影线。|  
...
|[put_AllowShowUI](#put_allowshowui)|**AllowShowUI**属性指定是否允许所承载的控件以显示其自身用户界面。|  
...
|[put_DisplayAsDefault](#put_displayasdefault)|**DisplayAsDefault**是允许控件查看它是否默认控件的环境属性。|  
...
|[put_MessageReflect](#put_messagereflect)|**MessageReflect**环境属性指定容器是否将反映到托管控件的消息。|  
|[put_OptionKeyPath](#put_optionkeypath)|**OptionKeyPath**属性指定对用户设置的注册表项路径。|  
|[put_UserMode](#put_usermode)|**UserMode**属性指定的容器的环境的用户模式。|  
  
## <a name="remarks"></a>备注  
 此接口公开的 ATL 的 ActiveX 控件承载对象。 将可用的环境属性设置为托管的控件，或指定容器的行为的其他方面此接口上调用的方法。 来补充由所提供的属性`IAxWinAmbientDispatch`，使用[IAxWinAmbientDispatchEx](../../atl/reference/iaxwinambientdispatchex-interface.md)。  
