...
| 宏 | 描述 |
|--|--|
| [DECLARE_AGGREGATABLE](#declare_aggregatable) | 声明可以 (默认) 聚合您的对象。 |
...
以上类定义指定将用作 `CMyClassFactory` 对象的默认类工厂。 `CMyClassFactory` 必须从派生 `CComClassFactory` 并重写 `CreateInstance` 。
...
`CMyLicense`，的模板参数 `CComClassFactory2` 必须实现静态函数 `VerifyLicenseKey` 、 `GetLicenseKey` 和 `IsLicenseValid` 。 下面是一个简单许可证类的示例：
...
如果在进程外服务器) 中创建多个单元中的对象 (，请将 DECLARE_CLASSFACTORY_AUTO_THREAD 添加到类中。
...
ATL 对象通过从 [CComCoClass](../../atl/reference/ccomcoclass-class.md)派生来通常获取类工厂。 此类包含宏 [DECLARE_CLASSFACTORY](#declare_classfactory)，该宏声明 `CComClassFactory` 为默认类工厂。 若要使用 `CComClassFactorySingleton` ，请在对象的类定义中指定 [DECLARE_CLASSFACTORY_SINGLETON](#declare_classfactory_singleton) 宏。 例如：
...
如果收到未定义的编译器错误消息，请将此宏添加到对象 `GetControllingUnknown` 中 (例如，在 `CComAggregateCreator`) 中。
...
`CreateInstance`如果尝试聚合到你的对象，DECLARE_NOT_AGGREGATABLE 会导致 CLASS_E_NOAGGREGATION) 返回错误 (。
...
如果尝试将 `CoCreate` 非聚合对象作为对象，DECLARE_ONLY_AGGREGATABLE 将导致 E_FAIL)  (错误。
...
使用 DECLARE_POLY_AGGREGATABLE 的优点在于，您可以避免 `CComAggObject` `CComObject` 在模块中同时使用和来处理聚合和非聚合情况。 单个 `CComPolyObject` 对象处理两种情况。 这意味着在您的模块中只存在一个 vtable 副本和一个函数副本。 如果 vtable 很大，这可能会显著降低模块大小。 但是，如果你的 vtable 很小，则使用 `CComPolyObject` 可能会导致模块大小稍微大一些，因为它未针对聚合或非聚合对象进行优化，如 `CComAggObject` 和 `CComObject` 。
...
如果在 [FinalConstruct](ccomobjectrootex-class.md#finalconstruct) 过程中 () 内部聚合对象递增引用计数，然后将计数递减为0，则保护对象不被删除。
