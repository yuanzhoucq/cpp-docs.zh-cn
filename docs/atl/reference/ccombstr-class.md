...
|[CComBSTR::operator >](#operator_gt)|比较`CComBSTR`使用字符串。|  
...
> [!NOTE]
>  `CComBSTR`类提供了许多采用 ANSI 或 Unicode 字符串作为自变量的成员 （构造函数、 赋值运算符和比较运算符）。 这些函数的 ANSI 版本是效率低于 Unicode 的对应的因为通常内部创建临时的 Unicode 字符串。 为提高效率，请尽可能使用 Unicode 版本。  
...
### <a name="return-value"></a>返回值  
 一份[m_str](#m_str)成员。 如果`m_str`是**NULL**，返回**NULL**。  
...
### <a name="parameters"></a>参数  
 请参阅[LoadString](http://msdn.microsoft.com/library/windows/desktop/ms647486) Windows SDK 中。  
