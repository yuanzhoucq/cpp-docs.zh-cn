...
- 你可创建具有内部或受保护的私有构造函数的公共未密封基类，前提是该基类从现有未密封基类（如 `Windows::UI::Xaml::DependencyObject`）直接或间接派生。 不支持跨 .winmd 文件继承用户定义的 ref 类；但是，ref 类可从其他 .winmd 文件中定义的接口继承。 只能在同一 Windows 运行时组件或通用 Windows 平台应用中从用户定义的 ref 类创建派生类。
