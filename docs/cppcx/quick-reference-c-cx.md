...
|概念|标准 C++|C++/CX|备注|
|-------------|--------------------|------------------------------------------------------------------|-------------|
|基本类型|C++ 基础类型。|C + +/CX 实现 Windows 运行时中定义的基础类型的基本类型。|`default`命名空间包含 c + +/cx 内置的基本类型。 编译器将 c + +/CX 基础类型隐式映射到标准 c + + 类型。<br /><br /> `Platform`命名空间系列包含实现基础 Windows 运行时类型的类型。|
||**`bool`**|**`bool`**|8 位布尔值。|
||**`wchar_t`**, **`char16_t`**|`char16`|表示 Unicode (UTF-16) 码位的 16 位非数字值。|
...
|对象生存期管理|`delete`*标识符*<br /><br /> `delete[]`  *标志*|（调用析构函数。）|生存期由引用计数确定。 调用删除会调用析构函数，但它本身不释放内存。|
|数组声明|*T 标识符* `[]`<br /><br /> `std::array`*标识符*|`Array<` *T* `^>^` *identifier* `(` *size* `)`<br /><br /> -或-<br /><br /> `WriteOnlyArray<` *T* `^>`  *identifier* `(` *size* `)`|声明类型 T^ 的一维可修改或只写数组。 该数组本身也是必须用句柄到对象修饰符声明的引用计数对象。<br /><br /> （数组声明使用位于 `Platform` 命名空间中的模板标头类。）|
|类声明|**`class`***标识符*`{}`<br /><br /> **`struct`***标识符*`{}`|**`ref class`***标识符*`{}`<br /><br /> **`ref struct`***标识符*`{}`|声明具有默认私有可访问性的运行时类。<br /><br /> 声明具有默认公共可访问性的运行时类。|
|结构声明|**`struct`***标识符*`{}`<br /><br /> （即，纯旧数据结构 (POD)）|**`value class`***标识符*`{}`<br /><br /> **`value struct`***标识符*`{}`|声明具有默认私有可访问性的 POD 结构。<br /><br /> 值类可在 Windows 元数据中表示，但标准 C++ 类不能。<br /><br /> 声明具有默认公共可访问性的 POD 结构。<br /><br /> 值结构可在 Windows 元数据中表示，但标准 C++ 结构不能。|
|接口声明|只包含纯虚函数的抽象类。|**`interface class`***标识符*`{}`<br /><br /> **`interface struct`***标识符*`{}`|声明具有默认私有可访问性的接口。<br /><br /> 声明具有默认公共可访问性的接口。|
|委托|`std::function`|`public delegate` *返回类型* *delegate-type-identifier* `(` *[ parameters ]* `);`|声明一个可以象函数调用一样被调用的对象。|
|事件|（不适用）|**`event`***委托类型标识符**事件标识符*`;`<br /><br /> *委托类型标识符*委托*标识符*  =  `ref new` *委托类型标识符* `( this` *[，parameters]*`);`<br /><br /> *event-identifier* `+=` *delegate-identifier* `;`<br /><br /> -或-<br /><br /> `EventRegistrationToken`*标记标识符*  = *obj* `.`*事件标识符* `+=`*委托标识符*`;`<br /><br /> -或-<br /><br /> **`auto`***标记标识符*  = *obj*。*事件标识符* `::add(`*委托标识符*`);`<br /><br /> *obj* `.` *event-identifier* `-=` *token-identifier* `;`<br /><br /> -或-<br /><br /> *obj* `.` *event-identifier* `::remove(` *token-identifier* `);`|声明一个事件对象，它存储在事件发生时被调用的事件处理程序（委托）的集合。<br /><br /> 创建事件处理程序。<br /><br /> 添加事件处理程序。<br /><br /> 添加事件处理程序会返回事件标记 (*token-identifier*)。 如果要显式移除事件处理程序，则必须保存事件标记供以后使用。<br /><br /> 移除事件处理程序。<br /><br /> 若要移除事件处理程序，必须指定在添加事件处理程序时保存的事件标记。|
|属性|（不适用）|**`property`***T* *标识符*;<br /><br /> **`property`***T* *标识符* `[` *索引*`];`<br /><br /> **`property`***T* `default[` *索引*`];`|声明类或对象成员函数可通过用于访问数据成员或索引数组元素的同一语法来访问。<br /><br /> 在类或对象成员函数上声明属性。<br /><br /> 在对象成员函数上声明索引属性。<br /><br /> 在类成员函数上声明索引属性。|
