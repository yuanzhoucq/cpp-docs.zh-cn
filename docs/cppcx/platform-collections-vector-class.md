...
如果你尝试使用**向量**类型在公共返回值或参数，编译器将引发的错误 C3986。 你可以通过更改参数来纠正错误或返回值类型到[Windows::Foundation::Collections::IVector](/uwp/api/Windows.Foundation.Collections.IVector_T_)。 有关更多信息，请参见 [集合 (C++/CX)](../cppcx/collections-c-cx.md)。
...
*dest*  
调用方分配的开始，以指定的元素的项数组*startIndex*向量中的最后一个元素结尾。
...
*index*  
项的从零开始索引如果参数*值*找到; 否则为 0。

*索引*参数为 0，如果项是向量的第一个元素，或者未找到该项目。 如果返回值为 `true`，则表明找到该项目，并且它是第一个元素；否则，表示未找到该项目。
