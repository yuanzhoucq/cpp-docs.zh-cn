...
ms.locfileid: "87218398"
---
# <a name="platformcollectionsvector-class"></a>Platform::Collections::Vector 类

表示可按照索引单独访问的对象的有序集合。 实现[Windows：： Foundation：：集合：： IObservableVector](/uwp/api/windows.foundation.collections.iobservablevector-1)以帮助 XAML[数据绑定](/windows/uwp/data-binding/data-binding-in-depth)。
...
保存第一个（）返回的迭代器的一种简便方法是将返回值分配给使用 **`auto`** 类型推导关键字声明的变量。 例如，`auto x = myVector->First();`。 此迭代器知道该集合的长度。
...
## <a name="vectorreplaceall-method"></a><a name="replaceall"></a>Vector：： ReplaceAll 方法
...
## <a name="vectorvector-constructor"></a><a name="ctor"></a>Vector：： Vector 构造函数
