...
6. 使用以下代码替换 `dllmain.cpp`中的代码。 此文件定义 DLL 导出函数。 这些函数使用[Microsoft：： WRL：： module](module-class.md)类来管理模块的类工厂。
...
1. 向 Windows 注册表注册 COM 组件。 为此，请创建一个注册项文件，将其命名为 `RegScript.reg`，然后添加以下文本。 将 *\<dll 路径 >* 替换为 dll 的路径（例如，`C:\temp\WRLClassicCOM\Debug\CalculatorComponent.dll`。
