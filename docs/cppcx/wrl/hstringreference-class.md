...
第一个构造函数初始化一个与参数*str*大小相同的新 `HStringReference` 对象。

第二个构造函数将初始化大小由参数*长度*的新 `HStringReference` 对象。
...
现有的*其他*对象的值将复制到当前 `HStringReference` 对象，然后销毁*另*一个对象。
