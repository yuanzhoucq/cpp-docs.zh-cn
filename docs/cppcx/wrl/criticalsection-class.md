...
[CriticalSection：： ~ CriticalSection](#tilde-criticalsection) | 取消初始化并销毁当前 `CriticalSection` 对象。
...
[CriticalSection：： TryLock](#trylock) | 尝试输入临界区而不进行阻止。 如果调用成功，则调用线程将获取关键部分的所有权。
...
如果成功输入了临界区，或者当前线程已拥有临界区，则为非零值。 如果另一个线程已拥有临界区，则为零。
