...
description: 通用 Windows 平台应用中不支持 CRT 功能的参考指南。
ms.date: 04/16/2020
...
ms.locfileid: "81480793"
---
# <a name="crt-functions-not-supported-in-universal-windows-platform-apps"></a>通用 Windows 平台应用中不支持的 CRT 函数

构建通用 Windows 平台 （UWP） 应用时，许多 C 运行时 （CRT） 功能不可用。 有时可以使用解决方法， 例如，您可以使用 Windows 运行时或 Win32 API。 在其他情况下，CRT 功能已被禁用，因为相应的功能或支持的 API 不适用于 UWP 应用。 要查找 Windows 运行时支持的替代方法，请参阅[UWP 应用中 Windows API 的替代方法](/uwp/win32-and-com/alternatives-to-windows-apis-uwp)。

下表列出了生成 UWP 应用时不可用的 CRT 功能。 它指示应用的任何解决方法。
...
|_getpid | 这些函数已过时。|使用 Win32 API `GetCurrentProcessId`。|
...
|_loaddll _getdllprocaddr _unloaddll|这些是以前 CRT 版本中的已过时函数。 此外，用户无法加载除同一应用程序包中的 DLL 以外的 DLL。|使用 Win32 API `LoadPackagedLibrary`、 `GetProcAddress`和 `FreeLibrary` 加载和使用打包的 DLL。|
...
|_heapwalk _heapadd _heapchk _heapset _heapused|这些函数通常用于处理堆。 但是，UWP 应用中不支持对应 Win32 API。 而且，应用无法再创建或使用专用堆。|无解决方法。 但是， `_heapwalk` 在 DEBUG CRT 中可用（仅用于进行调试）。 这些功能不能在上载到 Microsoft 应用商店的应用中使用。|

以下功能在通用应用的 CRT 中可用。 但是，仅当无法使用相应的 Win32 或 Windows 运行时 API 时（例如移植大型代码库时）才使用它们：

|||
|-|-|
|单字节字符串函数（例如， `strcat`、 `strcpy`、 `strlwr`等）。|使 UWP 应用严格使用 Unicode，因为所有公开的 Win32 API 和 Windows 运行时 API 都仅使用 Unicode 字符集。  单字节函数保留用于移植大型代码库，但应避免以其他方式。 如果可能，应使用相应的宽字符函数。|
|流 IO 和低级文件 IO 函数（例如， `fopen`、 `open`等）。|这些功能是同步的，不建议 UWP 应用使用。 在 UWP 应用中，使用异步 API 打开、读取和写入文件，以防止锁定 UI 线程。 这类 API 的示例是在 `Windows::Storage::FileIO` 类中的一个。|

## <a name="windows-8x-store-apps-and-windows-phone-8x-apps"></a>Windows 8.x 应用商店应用和 Windows Phone 8.x 应用

前面提到的 API 和以下 API 在 Windows 8.x 应用商店应用和 Windows Phone 8.x 应用中都不可用。
