...
---
# 委托 (C++-CX)
`delegate` 关键字用于声明一个参考类型，后者是标准 C\+\+ 中的函数对象在 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]中的等效项。 委托声明类似于函数声明；它指定被包装的函数必须具有的返回类型和参数类型。 以下是用户定义的委托声明：  
...
```  
 ...
 当声明要跨 Windows 运行时应用程序二进制接口向客户端公开的委托时，请使用 [Windows::Foundation::TypedEventHandler\<TSender, TResult\>](http://msdn.microsoft.com/%20library/windows/apps/br225997.aspx)。 此委托已预定义代理和存根二进制文件，使其可以由 Javascript 客户端使用。  
  
## 使用委托  
 在创建 [!INCLUDE[win8_appname_long](../cppcx/includes/win8-appname-long-md.md)]应用程序时，经常将委托作为 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]类公开的事件类型使用。 若要订阅事件，请通过指定匹配委托签名的函数或 lambda 创建其委托类型的一个实例。 然后使用 `+=` 运算符将委托对象传递到类上的事件成员。 这称为“订阅事件”。 当类实例“触发”事件时，你的函数随同由你的对象或其他对象添加的任何其他处理程序一起被调用。  
...
> [!WARNING]
>  通常，对于事件处理程序，除非要格外注意以避免循环引用，否则更好的做法是使用命名函数而不是 lambda。 命名函数会通过弱引用捕获“this”指针，而 lambda 通过强引用捕获该指针并创建一个循环引用。 有关更多信息，请参见[弱引用和中断循环](../cppcx/weak-references-and-breaking-cycles-c-cx.md)。  
  
 按照约定，由 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]定义的事件处理程序委托名称的形式为 \*EventHandler，例如，RoutedEventHandler、SizeChangedEventHandler 或 SuspendingEventHandler。 同样按照约定，事件处理程序委托具有两个参数并返回 void。 在没有类型参数的委托中，第一个参数的类型为 [Platform::Object^](../cppcx/platform-object-class.md)，它保持对发送方即触发事件的对象的引用。 在事件处理程序方法中使用该参数前，必须转换回原始类型。 在具有类型参数的事件处理程序委托中，第一个类型参数指定发送方的类型，第二个参数是保持事件相关信息的 ref 类的句柄。 按照约定，此类命名为 \*EventArgs。 例如，RoutedEventHandler 委托具有类型为 RoutedEventArgs^ 的第二个参数，DragEventHander 具有类型为 DragEventArgs^ 的第二个参数。  
...
## 创建自定义委托  
 你可以定义自己的委托，以便定义事件处理程序，或允许客户将自定义功能传递到你的 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]组件。 与任何其他 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]类型一样，公共委托不能声明为泛型委托。  
...
> [!NOTE]
>  你可以像对待任何 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]引用类型那样，在引用委托类型时使用“^”符号。  
  
 事件声明始终有一个委托类型。 此示例在 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]中显示一个典型的委托类型签名：  
  
 [!code-cpp[cx_delegates#122](../snippets/cpp/VS_Snippets_Misc/cx_delegates/cpp/delegatesevents/class1.h#122)]  
  
 `Click` 类中的 `Windows:: UI::Xaml::Controls::Primitives::ButtonBase` 事件属于 `RoutedEventHandler` 类型。 有关更多信息，请参见[事件](../cppcx/events-c-cx.md)。  
 ...
 在下一个示例中，客户端应用程序将自定义委托传递给 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]组件中的一个公共方法，该方法针对 `Vector` 中的每一项目执行委托：  
...
## 委托和线程  
 ...
 如果客户端应用程序加载在一个线程单元运行的 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]组件，并将委托提供给该组件，则默认情况下，在 STA 线程上直接调用该委托。 大多数 [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)]组件既可在 STA 中也可在 MTA 中运行。  
