 ...
 **文本**  
 ...
 一个*文本字符* 就是用单引号括起来的一个字符，一个*文本字符串* 就是用双引号括起来的一个字符序列。 如果使用文本初始化 String^ 变量，则编译器假定文本由 `char16` 字符组成。 也就是说，在 **\_T\(\)** 或 **TEXT\(\)** 宏中，不必在文本前加上字符串修饰符“L”或用引号将文本括起来。 有关 Unicode 的 C\+\+ 支持的更多信息，请参见 [Unicode 编程摘要](../text/unicode-programming-summary.md)。  
...
## 字符串长度和嵌入的 NULL 值  
 [String::Length 方法](../cppcx/string-length-method.md)返回字符串中字符的数量，而非字节的数量。 当使用堆栈语义构造字符串时，一般不计算终止 NULL 字符，除非您明确指定。  
