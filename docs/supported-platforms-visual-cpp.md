...
\*\*\*可以安装 Visual Studio 安装程序中的“使用 C++ 的 Linux 开发”工作负荷，面向 Linux 平台。 有关说明，请参阅[下载、安装和设置 Linux 工作负荷](linux/download-install-and-setup-the-linux-development-workload.md)。 此工具集在目标计算机上编译可执行文件，使用户能够为支持的任何体系结构执行生成操作。
