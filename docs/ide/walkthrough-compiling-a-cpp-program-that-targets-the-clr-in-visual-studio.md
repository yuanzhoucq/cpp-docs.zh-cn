---
title: 编译面向 CLR 的 C++ 程序 | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-ide
...
ms.locfileid: "33336090"
---
# <a name="walkthrough-compiling-a-c-program-that-targets-the-clr-in-visual-studio"></a>演练：在 Visual Studio 中编译面向 CLR 的 C++ 程序
可以创建使用 .NET 类的 Visual C++ 程序，并通过使用 Visual Studio 开发环境来对其进行编译。  
  
 在此过程中，你可以键入自己的 Visual C++ 程序或采用示例程序之一。 我们在此过程中使用的示例程序创建名为 textfile.txt 的文本文件，并将其保存到项目目录。  
...
5.  向项目添加新的源文件：  
    ...
    -   单击“C++ 文件 (.cpp)”并键入文件名，然后单击“添加”。  
     ...
     .cpp 文件会出现在解决方案资源管理器中的“源文件”文件夹中，并且当你按需要在该文件中键入代码时，会出现一个选项卡式窗口。  
  
6.  单击 Visual Studio 中新创建的选项卡并键入有效的 Visual C++ 程序或从示例程序中复制粘贴一个。  
  
     例如，可以使用[如何：编写文本文件 (C++/CLI)](../dotnet/how-to-write-a-text-file-cpp-cli.md) 示例程序（位于编程指南中的“文件处理和 I/O”节点）。  
...
9. 如果使用了示例程序，在运行程序时会显示一个命令窗口，该窗口指示已创建文本文件。 按任意键关闭命令窗口。  
 ...
 **上一篇：**[演练：在命令行上编译本机 C++ 程序](../build/walkthrough-compiling-a-native-cpp-program-on-the-command-line.md) | **下一篇：**[演练：在命令行上编译 C 程序](../build/walkthrough-compile-a-c-program-on-the-command-line.md)  
