...
- **通知函数**
  ...
  仅 MFC 调度接口。 仅当在“实现类型”下指定了“成员变量”时才可用********。 如果属性更改，请设置调用的通知函数的名称。 默认情况下，通知函数的名称设置为`On`*属性名称*`Changed`。 可以编辑此名称。
...
- **实现类型**
  ...
  |**成员变量**|指定属性添加为成员变量。 可以将自定义属性或大多数常用属性添加为成员变量。 不能为`Caption`、`hWnd`和`Text`属性指定**成员变量**。<br /><br /> 在变量名称和通知函数下提供默认名称********。 可以编辑此名称。|
...
- **参数属性**
  ...
  仅 ATL 接口。 设置**参数名称**指定的参数是`in`、`out`都为 或 无。
