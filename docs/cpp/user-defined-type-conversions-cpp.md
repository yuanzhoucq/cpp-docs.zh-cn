...
ms.locfileid: "80187929"
---
# <a name="user-defined-type-conversions-c"></a>用户定义的类型转换 (C++)

*转换*从不同类型的值生成某种类型的新值。 *标准转换*内置于C++语言中并支持其内置类型，并且你可以创建*用户定义的转换*，以便在用户定义的类型之间执行转换。
...
可能导致问题的隐式转换的一个常见示例是转换为**bool**。 你可能想要创建一个可在布尔上下文中使用的类类型的原因，例如，使其可用于控制**if**语句或循环，但当编译器执行到内置类型的用户定义的转换时，允许编译器在以后应用其他标准转换。 这一额外的标准转换目的是允许从**简短**到**int**的升级，但它还为不太明显的转换（例如，从**bool**到**int**）提供了门，这允许你的类类型在你从未预期的整数上下文中使用。 这个特定问题称为*安全 Bool 问题*。 此类问题是**explicit**关键字可以帮助的地方。

**Explicit**关键字告诉编译器指定的转换不能用于执行隐式转换。 如果希望在引入**explicit**关键字之前隐式转换的语法方便，您必须接受隐式转换有时会创建或使用不太方便的命名转换函数作为解决方法的意外后果。 现在，通过使用**explicit**关键字，可以创建仅用于执行显式强制转换或直接初始化的方便转换，而不会导致安全 Bool 问题举例说明的问题类型。

自 c + + 98 后，可以将该**显式**关键字应用于转换构造函数，并应用于 c + + 11 之后的转换函数。 以下部分包含有关如何使用**explicit**关键字的详细信息。

## <a name="conversion-constructors"></a><a name="ConvCTOR"></a>转换构造函数

转换构造函数定义了从用户定义的类型或内置类型到用户定义的类型的转换。 下面的示例演示了一个转换构造函数，该构造函数将内置类型**double**转换为用户定义的类型 `Money`。
...
请注意，对函数 `display_balance` 的首次调用（它将采用类型 `Money` 的自变量）不需要转换，因为它的自变量类型正确。 但是，在第二次调用 `display_balance`时，需要进行转换，因为自变量的类型（值为 `49.95`的**双精度**值）不是函数期望的值。 该函数不能直接使用此值，但是由于存在从参数类型（**双**）到匹配参数的类型的转换（`Money`），因此，将从自变量构造 `Money` 类型的临时值并用于完成函数调用。 在对 `display_balance`的第三次调用中，请注意，参数不是**double**，而是值为 `9.99`的**float** ，但仍可以完成函数调用，因为编译器可以执行标准转换（在这种情况下，从**float**到**double**），然后执行从**double**到 `Money` 的用户定义的转换，以完成必要的转换。
...
通过将转换构造函数声明为**显式**，它只能用于对对象执行直接初始化或执行显式强制转换。 这将阻止接受类类型的自变量的函数同样隐式接受转换构造函数的源类型的自变量，并且将阻止从该源类型的值复制初始化类的类型。 以下示例演示了如何定义显式转换构造函数，以及它对哪个代码格式正确所产生的影响。
...
在此示例中，请注意，你可以继续使用显示转换构造函数来执行 `payable` 的直接初始化。 如果你要改为复制初始化 `Money payable = 79.99;`，这将是一个错误操作。 对 `display_balance` 的首次调用不受影响，因为自变量的类型正确。 对 `display_balance` 的第二次调用是一个错误，因为转换构造函数无法用于执行隐式转换。 对 `display_balance` 的第三次调用是合法的，因为显式强制转换为 `Money`，但请注意，编译器仍会通过插入从**float**到**double**的隐式强制转换来完成强制转换。
...
转换函数定义了从用户定义的类型到其他类型的转换。 这些函数有时称为“强制转换运算符”，因为在值强制转换为其他类型时将随转换构造函数一起调用它们。 下面的示例演示了一个转换函数，该函数将用户定义类型（`Money`）转换为内置类型**double**：
...
请注意，将成员变量 `amount` 设为 private，并将公共转换函数转换为**double**类型，只是为了返回 `amount`的值。 在函数 `display_balance` 中，当 `balance` 的值通过使用流插入运算符 `<<` 流入标准输出时，将执行隐式转换。 由于没有为用户定义的类型 `Money`定义流插入运算符，但对于内置类型**double**而言，编译器可以使用从 `Money` 到**double**的转换函数来满足流插入运算符。

转换函数通过派生类继承。 派生类中的转换函数仅在其转换为完全相同的类型时覆盖继承的转换函数。 例如，派生类**operator int**的用户定义的转换函数不会重写（甚至还会影响）基本类**运算符**的用户定义的转换函数，即使标准转换定义**int**和**short**之间的转换关系也是如此。
...
在这里，转换函数**运算符 double**是显式的，并在函数 `display_balance` 中引入了显式强制转换到类型**double** 。 若已省略此强制转换，则编译器将无法定位适用于类型 `<<` 的相应流插入运算符 `Money`，并且将发生错误。
