...
表达式根据其运算符的优先级和分组来计算。  （[词法约定](../cpp/cpp-built-in-operators-precedence-and-associativity.md)中的[运算符优先级和关联性](../cpp/lexical-conventions.md)显示了 C\+\+ 运算符对表达式实施的关系。）  
...
```  
 ...
 ![表达式中的计算顺序](../Image/vc38ZV1.gif "vc38ZV1")  
表达式计算顺序  
...
3.  左移 \(\<\<\) 在此表达式中具有最低的优先级，但有两个匹配项。  由于左移运算符从左到右分组，因此先计算左子表达式，再计算右子表达式。  
 ...
 ![带括号表达式的计算顺序](../cpp/media/vc38zv2.png "vc38ZV2")  
带括号的表达式计算顺序  
