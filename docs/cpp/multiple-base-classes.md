...
如[多重继承](http://msdn.microsoft.com/zh-cn/3b74185e-2beb-4e29-8684-441e51d2a2ca)中所述，类可以从多个基类派生。  在多重继承模型中（其中，类派生自多个基类），使用 *base\-list* 语法元素指定基类（请参阅[概述](../misc/overview-of-derived-classes.md)中的“语法”一节）。  例如，可以指定派生自 `CollectionOfBook` 和 `Collection` 的 `Book` 的类声明：  
...
## 虚拟基类  
 ...
 ![模拟的午餐排队图](../cpp/media/vc38xp1.png "vc38XP1")  
模拟午餐排队图  
 ...
 ![模拟的午餐排队对象](../cpp/media/vc38xp2.png "vc38XP2")  
模拟午餐排队对象  
...
```  
 ...
 ![模拟午餐排队对象和虚拟基类](../cpp/media/vc38xp3.png "vc38XP3")  
使用虚拟基类模拟午餐排队对象  
  
 一个类可以同时具有一个给定类型的虚拟组件和非虚拟组件。  下图演示了这种情况。  
  
 ![类的虚拟组件与非虚拟组件](../cpp/media/vc38xp4.png "vc38XP4")  
同一个类的虚拟组件与非虚拟组件  
 ...
 ![对象布局中的虚拟继承与非虚拟继承](../cpp/media/vc38xp5.png "vc38XP5")  
带虚拟和非虚拟继承的对象布局  
...
2.  如果重载函数是明确的，则将解析它们。  （有关函数重载多义性的详细信息，请参阅[参数匹配](../misc/argument-matching.md)。）  
  
3.  如果对名称的访问违背了成员访问权限，则会生成错误消息。  （有关详细信息，请参阅[成员访问控制](../cpp/member-access-control-cpp.md)。）  
...
-   将使用 address\-of 运算符获取的指针显式转换为基类类型 `A` 的效果。  请注意，将该对象的地址强制转换为 `A*` 类型并不总是为编译器提供足够的信息，以供 `A` 类型的子对象进行选择；在这种情况下，将存在两个子对象。  
 ...
 ![指针到基类的不明确转换](../cpp/media/vc38xt1.png "vc38XT1")  
指针到基类的不明确转换  
...
### 多义性和虚拟基类  
 ...
 ![虚拟派生与非虚拟派生](../cpp/media/vc38xr1.png "vc38XR1")  
虚拟和非虚拟派生  
