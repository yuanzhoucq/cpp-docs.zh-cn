...
应用后缀增量运算符（）的效果 **++** 是操作数的值增加一个适当类型的单位。 同样，应用后缀减量运算符（）的效果 **--** 是操作数的值减小了一个适当类型的单位。
...
后缀递增和后缀减量运算符的操作数必须是可修改的（而不是 **`const`** ）算术或指针类型的左值。 结果的类型与*后缀表达式*的类型相同，但它不再是左值。

**Visual Studio 2017 版本15.3 及更高版本**（可用于[/std： c + + 17](../build/reference/std-specify-language-standard-version.md)）：后缀递增或递减运算符的操作数的类型不能为 **`bool`** 。
