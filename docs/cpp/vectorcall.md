...
ms.locfileid: "62393865"
---
# <a name="vectorcall"></a>__vectorcall

**Microsoft 专用**

**__Vectorcall**调用约定指定函数的参数是在寄存器中，在可能的情况中传递。 **__vectorcall**的参数，而使用更多寄存器[__fastcall](../cpp/fastcall.md)值，则默认[x64 调用约定](../build/x64-calling-convention.md)使用。 **__Vectorcall**及更高版本包括流式处理 SIMD 扩展 2 (SSE2) 的 x86 和 x64 处理器上的本机代码中仅支持调用约定。 使用 **__vectorcall**函数传递多个浮点或 SIMD 矢量自变量的速度和执行操作，以便利用参数的寄存器中加载。 以下列表显示了所共有的 x86 和 x64 实现的功能 **__vectorcall**。 本文的后面部分解释了这些差异。
...
**__Vectorcall** x64 调用约定扩展标准 x64 调用约定以利用其他寄存器。 根据参数列表中的位置将整数类型参数和矢量类型参数映射到寄存器。 将 HVA 自变量分配给未使用的矢量寄存器。
...
当前六个参数（按从左到右的顺序）都是矢量类型参数时，将根据参数位置通过 0 到 5 的 SSE 矢量寄存器中的值传递它们。 浮点和 **__m128**传递类型在 XMM 寄存器中，并 **__m256**类型在 YMM 传递注册。 这与标准 x64 调用约定不同，因为矢量类型是通过值而不是引用传递的，并且还使用了其他寄存器。 为矢量类型参数分配影子堆栈空间固定为 8 个字节，并[/homeparams](../build/reference/homeparams-copy-register-parameters-to-stack.md)选项不适用于。 在第七个参数位置和后面的参数位置的矢量类型参数在堆栈上通过对由调用方分配的内存的引用进行传递。

为矢量自变量分配寄存器后，分配的 HVA 自变量的数据成员，按升序对未使用的矢量寄存器 XMM0 至 XMM5 (或 YMM0 至 YMM5，对于 **__m256**类型)，只要有足够的寄存器适用于整个 HVA。 如果没有足够多的可用寄存器，HVA 自变量将通过对由调用方分配的内存的引用进行传递。 HVA 自变量的影子堆栈空间固定为 8 个字节，且带有未定义的内容。 在参数列表中按从左到右的顺序将 HVA 参数分配给寄存器，并且这些参数可处于任意位置。 位于未分配给矢量寄存器的前四个参数位置中的任一位置的 HVA 参数将通过由对应于该位置的整数寄存器中的引用进行传递。 在第四个参数位置后通过引用传递的 HVA 自变量将被推入堆栈。
...
由调用方在 x64 维护堆栈的实现 **__vectorcall**。 调用方 prolog 和 epilog 代码分配并清除被调用函数的堆栈。 参数从右向左推入堆栈；并且为传递到寄存器中的参数分配影子堆栈空间。
...
将在参数列表中从左至右发现的前两个整数类型参数分别放入 ECX 和 EDX 中。 隐藏**这**指针被视为第一个整数类型参数，并且在 ECX 中传递。 前六个矢量类型参数通过 XMM 或 YMM 寄存器中的 SSE 矢量寄存器 0 到 5（具体取决于参数大小）中的值传递。
...
为矢量自变量分配寄存器后，的数据成员的 HVA 自变量分配以升序未使用的矢量寄存器 XMM0 至 XMM5 (或 YMM0 至 YMM5，对于 **__m256**类型)，只要有足够的寄存器适用于整个 HVA。 如果没有足量的可用寄存器，则 HVA 自变量将在堆栈上通过对由调用方分配的内存的引用进行传递。 不会为 HVA 参数分配堆栈影子空间。 在参数列表中按从左到右的顺序将 HVA 参数分配给寄存器，并且这些参数可处于任意位置。
