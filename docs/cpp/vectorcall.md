...
|大小写转换约定|不执行任何大小写转换。|  
 ...
 使用[/Gv](../build/reference/gd-gr-gv-gz-calling-convention.md)编译器选项将导致每个函数中要作为编译的模块`__vectorcall`的函数是成员函数，除非使用冲突的调用约定特性声明，将使用`vararg`变量参数列表中，或具有名称`main`。  
...
```  
 ...
 成员函数可以通过使用 `__vectorcall` 说明符进行声明。 隐藏**这**通过作为第一个整数类型参数的寄存器传递指针。  
...
```  
 ...
 必须在创建指向 `__vectorcall` 函数的指针时，指定`__vectorcall` 调用约定修饰符。 下一个示例创建**typedef**到指针`__vectorcall`采用四个函数**double**自变量并返回 **__m256**值：  
