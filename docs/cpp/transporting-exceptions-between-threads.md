...
函数返回一个`exception_ptr`对象, 该对象引用由 e 参数指定的异常。 `make_exception_ptr`
...
你的应用程序的异常处理模型决定了它是否可以捕获和传输异常。 Visual C++ 支持三种模型，这些模型可以处理 C++ 异常、结构化异常处理 (SEH) 异常和公共语言运行时 (CLR) 异常。 使用[/EH](../build/reference/eh-exception-handling-model.md)和[/clr](../build/reference/clr-common-language-runtime-compilation.md)编译器选项来指定应用程序的异常处理模型。
...
- **/Clr**编译器选项和**catch**语句可以传输C++异常。 **/Clr**编译器选项隐含了 **/eha**选项的规范。 请注意，编译器不支持传输托管异常。 这是因为派生自 system.exception[类](../standard-library/exception-class.md)的托管异常已经是可通过使用 common 语言运行时的功能在线程之间移动的对象。
   ...
   > [!IMPORTANT]
   > 我们建议您指定 **/EHsc**编译器选项和 catch 仅 C++ 异常。 如果使用 **/eha**或 **/clr**编译器选项, 并使用带有省略号*异常声明*(`catch(...)`) 的**catch**语句, 就会给您带来安全威胁。 您可能打算使用**catch**语句来捕获几个特定的异常。 但是，`catch(...)` 语句将捕获所有的 C++ 和 SEH 异常，包括致命的意外异常。 如果忽略意外异常或处理不当，恶意代码就可以趁此机会破坏你程序的安全性。

## <a name="usage"></a>用法

以下各节`exception_ptr`介绍如何使用类型、 `current_exception`、 `rethrow_exception`和`make_exception_ptr`函数传输异常。

## <a name="exception_ptr-type"></a>exception_ptr 类型

使用 `exception_ptr` 对象可引用当前异常或用户指定异常的实例。 在 Microsoft 实现中，异常由 [EXCEPTION_RECORD](/windows/desktop/api/winnt/ns-winnt-exception_record) 结构表示。 每个 `exception_ptr` 对象包含一个异常引用字段，该字段指向表示异常的 `EXCEPTION_RECORD` 结构的副本。

当你声明 `exception_ptr` 变量时，该变量不与任何异常相关联。 也就是说，其异常引用字段为 NULL。 此类 `exception_ptr` 对象称为 *null exception_ptr*。

使用 `current_exception` 或 `make_exception_ptr` 函数可将异常指派给 `exception_ptr` 对象。 将异常指派给 `exception_ptr` 变量时，该变量的异常引用字段将指向该异常的副本。 如果没有足够的内存来复制异常，异常引用字段将指向 [std::bad_alloc](../standard-library/bad-alloc-class.md) 异常的副本。 `current_exception`如果或`make_exception_ptr`函数因任何其他原因不能复制异常, 该函数将调用[terminate](../c-runtime-library/reference/terminate-crt.md)函数以退出当前进程。
...
在**catch**块中调用函数。`current_exception` 如果异常处于飞行中并且**catch**块可以捕获异常, 则函数将`current_exception`返回`exception_ptr`引用该异常的对象。 否则，该函数将返回 null `exception_ptr` 对象。

### <a name="details"></a>详细信息

函数捕获处于飞行中的异常, 而不考虑**catch**语句是否指定[异常声明](../cpp/try-throw-and-catch-statements-cpp.md)语句。 `current_exception`
...
如果使用 **/eha**编译器选项, 您可以在C++ **catch**块中捕获 SEH 异常。 `current_exception` 函数返回引用 SEH 异常的 `exception_ptr` 对象。 如果调用 thetransported `exception_ptr`对象作为其参数, 则函数将引发SEH异常。`rethrow_exception`

如果`current_exception`在 SEH **__finally**终止`exception_ptr`处理程序、 **__except**异常处理程序或 **__except**筛选器表达式中调用该函数, 则该函数将返回 null。
...
你可以将结构化异常 (SE) 转换器函数与传输异常功能结合使用。 如果 SEH 异常转换为 C++ 异常，`current_exception` 函数返回的 `exception_ptr` 将引用转换后的异常，而不是原始 SEH 异常。 `rethrow_exception` 函数随后引发转换后的异常，而不是原始异常。 有关 SE 转换器功能的详细信息, 请参阅[_set_se_translator](../c-runtime-library/reference/set-se-translator.md)。
...
`exception_ptr` `current_exception` C++调用函数等效于引发异常, 在 catch 块中捕获它, 然后调用函数以返回引用异常的对象。 `make_exception_ptr` Microsoft 实现的 `make_exception_ptr` 函数比调用并捕获异常更高效。
