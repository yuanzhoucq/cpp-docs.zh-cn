...
4.  `noexcept`，指定函数是否可以引发异常。  在下面的示例中，函数在 `is_pod` 表达式计算结果为 `true` 时不引发异常。  
...
8.  （仅限非静态成员函数）ref 限定符，向编译器指定隐式对象参数 \(\*this\) 是右值引用与左值引用时要选择的  函数重载。  
  
 下图显示了函数定义的各个部分。  灰色区域是函数体。  
  
 ![函数定义部分](../cpp/media/vc38ru1.png "vc38RU1")  
函数定义部分  
...
```  
 ...
 有关详细信息，请参阅[默认参数](../cpp/default-arguments.md)和[类模板的默认参数](../Topic/Default%20Arguments%20for%20Class%20Templates.md)。  
...
```  
 ...
 下面的示例（基于来自 [N3493](http://www.open-std.org/JTC1/SC22/WG21/docs/papers/2013/n3493.html) 的代码）演示的 `decltype(auto)` 用于采用在模板实例化之前未知的返回类型实现函数参数的完美转发。  
