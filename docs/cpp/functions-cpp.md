...
函数是执行某种操作的代码块。 函数可以选择性地定义使调用方可以将实参传递到函数中的输入形参。 函数可以选择性地返回值作为输出。 函数可用于在单个可重用块中封装常用操作（理想情况是使用可清晰地描述函数行为的名称）。 以下函数接受来自调用方的两个整数并返回它们的总和;*a*和*b*是**int**类型的*参数*。
...
1. 一个 `noexcept` 表达式，指定函数是否会引发异常。 在下面的示例中，如果 `is_pod` 表达式的计算结果为**true**，则函数不引发异常。
...
1. （仅限成员函数）Cv 限定符，用于指定函数是**const**函数还是**volatile**函数。

1. （仅限成员函数）**虚拟**、`override`或 `final`。 **virtual**指定可以在派生类中重写函数。 `override` 表示派生类中的函数在重写虚函数。 `final` 表示函数不能在任何进一步的派生类中进行重写。 有关详细信息，请参阅[虚拟函数](../cpp/virtual-functions.md)。

1. （仅限成员函数）应用于成员函数的**静态**意味着该函数不与类的任何对象实例相关联。

1. （仅限非静态成员函数）引用限定符，用于指定编译器在隐式对象参数（\*此）是右值引用与左值引用时，要选择的函数的重载。 有关详细信息，请参阅[函数重载](function-overloading.md#ref-qualifiers)。

下图显示了函数定义的各个部分。 灰色区域是函数体。

![函数定义的组成部分](../cpp/media/vc38ru1.gif "函数定义部分") <br/>
函数定义部分
...
可以将成员函数声明为**const** ，以指定不允许函数更改类中任何数据成员的值。 通过将成员函数声明为**const**，可帮助编译器强制实施*const 正确性*。 如果有人错误地尝试使用声明为**const**的函数修改对象，则会引发编译器错误。 有关详细信息，请参阅[const](const-cpp.md)。

当函数生成的值可能在编译时确定时，将该函数声明为 `constexpr`。 Constexpr 函数的执行速度通常比常规函数快。 有关详细信息，请参阅[constexpr](constexpr-cpp.md)。
...
使用参数声明列表中的单个关键字**void**声明的函数不采用任何参数，前提是关键字**void**是参数声明列表中的第一个和唯一一个成员。 列表中其他位置的**void**类型参数会产生错误。 例如：
...
请注意，尽管指定**void**参数是非法的，但此处所述的除外，从类型**void**派生的类型（如指向**void**的指针和**void**的数组）可以出现在参数声明列表的任何位置。
...
“普通”返回类型位于函数签名左侧。 *尾随返回类型*位于签名的最右侧，并以-> 运算符开头。 当返回值的类型取决于模板参数时，结尾返回类型在函数模板中尤其有用。
...
当**自动**与尾随返回类型一起使用时，它只用作 decltype 表达式生成的任何一个占位符，而不是自身执行类型推导。
