...
ms.locfileid: "32418424"
---
# <a name="c-built-in-operators-precedence-and-associativity"></a>C++ 内置运算符、 优先级和关联性

C++ 语言包括所有 C 运算符并添加多个新的运算符。 运算符指定对一个或多个操作数执行的计算。

运算符*优先*包含多个运算符的表达式中指定的运算顺序。 运算符*关联性*指定是否在表达式中包含具有相同优先级的多个运算符，操作数组合与其左侧还是右侧的一个。 下表显示 C++ 运算符的优先级和关联性（从最高优先级到最低优先级）。 优先级别编号相同的运算符具有等同的优先级别，除非由括号显式施加另一种关系。
...
|[重新解释后的类型转换](../cpp/reinterpret-cast-operator.md)|[reinterpret_cast](../cpp/reinterpret-cast-operator.md)|
...
|**组 12 优先级，从左到右的关联性**|
|[按位与或](../cpp/bitwise-inclusive-or-operator-pipe.md)|[&#124;](../cpp/bitwise-inclusive-or-operator-pipe.md)|
|**组 13 优先级，从左到右的关联性**|
...
|[按位与或赋值](../cpp/assignment-operators.md)|[&#124;=](../cpp/assignment-operators.md)|
