...
ms.locfileid: "81365117"
---
# <a name="using-dllimport-and-dllexport-in-c-classes"></a>在 C++ 类中使用 dllimport 和 dllexport

**微软特定**

可以使用**dllimport**或**dllexport**属性声明C++类。 这些形式表示已导入或导出整个类。 以这种方式导出的类称为可导出类。
...
当您声明类**dllexport**时，将导出其所有成员函数和静态数据成员。 您必须在同一程序中提供所有此类成员的定义。 否则，将生成链接器错误。 此规则有一个例外情况，即对于纯虚函数，您无需为其提供显式定义。 但是，由于基类的析构函数始终在调用抽象类的析构函数，因此纯虚拟析构函数必须始终提供定义。 请注意，这些规则对不可导出的类是相同的。

如果导出类类型的数据或返回类的函数，请务必导出类。

## <a name="dllimport-classes"></a><a name="_pluslang_dllexport_classesdllexportclasses"></a>dllimport 类

当您声明类**dllimport**时，将导入其所有成员函数和静态数据成员。 与非类类型上的**dllimport**和**dllexport**行为不同，静态数据成员无法在同一程序中指定定义**dllimport**类的定义。
...
可导出类的所有基类都必须是可导出的。 否则，会生成编译器警告。 此外，同样是类的所有可访问成员必须是可导出的。 此规则允许**dllexport**类从**dllimport**类继承 **，dllimport**类从**dllexport**类继承（尽管不建议继承后者）。 通常来说，对 DLL 客户端可访问的所有内容（根据 C++ 访问规则）都应该是可导出接口的一部分。 这包括在内联函数中引用的私有数据成员。
...
由于类中的成员函数和静态数据具有外部链接，因此可以使用**dllimport**或**dllexport**属性声明它们，除非导出整个类。 如果导入或导出整个类，则禁止成员函数和数据明确声明为**dllimport**或**dllexport。** 如果将类定义中的静态数据成员声明为**dllexport，** 则定义必须发生在同一程序中的某处（与非类外部链接一样）。

同样，您可以使用**dllimport**或**dllexport**属性声明成员函数。 在这种情况下，您必须在同一程序中的某个位置提供**dllexport**定义。
...
- 如果将成员定义为**dllexport，** 但不包括在类定义中，则生成编译器错误。 必须在类头中定义成员。

- 尽管允许将类成员定义为**dllimport**或**dllexport，** 但不能覆盖类定义中指定的接口。

- 如果在声明该函数的类定义正文之外的其他位置定义成员函数，则如果该函数定义为**dllexport**或**dllimport（** 如果此定义与类声明中指定的定义不同），则将发出警告。
