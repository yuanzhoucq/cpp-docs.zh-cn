...
ms.locfileid: "81369866"
---
# <a name="member-access-control-c"></a>成员访问控制 (C++)

访问控件使您能够将类[的公共](../cpp/public-cpp.md)接口与[专用](../cpp/private-cpp.md)实现详细信息和仅供派生类使用的[受保护](../cpp/protected-cpp.md)成员分开。 访问说明符应用于在它之后声明的所有成员，直到遇到下一个访问说明符。
...
默认访问在类中是**私有**的，并且在结构或联合中**是公共**的。 类中的访问说明符可按任何顺序使用任意次数。 类类型的对象的存储分配是取决于实现的，但成员一定能分配到访问说明符之间的依次升高的内存地址。

## <a name="member-access-control"></a>成员访问控制

|访问类型|含义|
|--------------------|-------------|
|[private](../cpp/private-cpp.md)|声明为**私有**的类成员只能由类的成员函数和好友（类或函数）使用。|
|[protected](../cpp/protected-cpp.md)|声明为**受保护**的类成员可以由类的成员函数和好友（类或函数）使用。 此外，它们还可由派生自该类的类使用。|
|[public](../cpp/public-cpp.md)|声明为**公共**的类成员可以由任何函数使用。|
...
您可以声明派生类而不使用基类访问说明符。 在这种情况下，如果派生类声明使用**类**关键字，则派生被视为私有。 如果派生类声明使用**结构**关键字，则派生被视为公共。 例如，以下代码：
...
请注意，声明为具有私有访问权限的成员不能访问函数或派生类，除非这些函数或类使用基类中的**frienda**声明声明。

**联合**类型不能具有基类。

> [!NOTE]
> 指定私有基类时，建议显式使用**私有**关键字，以便派生类的用户了解成员访问权限。

## <a name="access-control-and-static-members"></a>访问控制和静态成员

将基类指定为**私有**类时，它只影响非静态成员。 在派生类中，公共静态成员仍是可访问的。 但是，使用指针、引用或对象访问基类的成员需要转换，此时将再次应用访问控制。 请考虑以下示例：
...
在前面的代码中，访问控制禁止从指向 `Derived2` 的指针转换为指向 `Base` 的指针。 **此**指针是隐式类型`Derived2 *`。 要选择函数`CountOf`，必须**将其**转换为类型`Base *`。 不允许执行此类转换，因为 `Base` 是 `Derived2` 的私有间接基类。 到私有基类类型的转换仅对于指向立即派生类的指针是可接受的。 因此，可以将 `Derived1 *` 类型的指针转换为 `Base *` 类型。
...
应用于[虚拟](../cpp/virtual-cpp.md)函数的访问控制由用于进行函数调用的类型确定。 重写函数的声明不会影响给定类型的访问控制。 例如：
