 ...
 *expression1* **[***expression2***] [***expression3***]**...  
 ...
 下标表达式从左至右关联。 首先计算最左侧的下标表达式 expression1[expression2]。 通过添加 expression1 和 expression2 得到的地址构成一个指针表达式；然后 expression3 将添加到此指针表达式，从而构成一个新的指针表达式，依此类推，直到添加最后一个下标表达式。 间接寻址运算符 (**\***) 计算之后应用最后一个下标的表达式，除非最终指针值寻址数组类型。  
...
```  
 ...
 下标运算符是可交换的。 因此，表达式*数组*[*索引*] 和*数组*[*数组*] 一定是等效的长达下标未重载运算符 (请参阅[重载运算符](../cpp/operator-overloading.md))。 第一种形式是最常见的编码做法，但它们都有效。  
