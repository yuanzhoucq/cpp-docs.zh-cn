...
使用 **`__sptr`** **`__uptr`** 32 位指针声明中的或修饰符来指定编译器如何将32位指针转换为64位指针。 例如，在将 32 位指针分配给 64 位指针变量或在 64 位平台上取消引用 64 位指针时，将转换该指针。
...
下面的示例声明使用和修饰符的32位指针 **`__sptr`** **`__uptr`** ，将每个32位指针分配给一个64位指针变量，然后显示每个64位指针的十六进制值。 该示例利用本机 64 位编译器进行编译并在 64 位平台上执行。
