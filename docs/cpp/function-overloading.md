...
|Variant 2（可使用标准转换将 `int` 转换为 `long`）|Variant 1（可使用标准转换将 `int` 转换为 `long`）|  
 ...
 对于自变量匹配，具有函数 *n* 默认自变量将被视为 *n* + 1 个单独函数，每个都有不同数量的参数。  
...
## <a name="argument-type-differences"></a>自变量类型差异  
 ...
 出于相同原因，函数通过修改的类型的自变量**const**或`volatile`处理没有不同的基类型出于重载的目的。  
...
```  
 ...
 在前面的示例中，用户定义的转换，**运算符长**，调用来执行转换`udc`类型**长**。 如果没有用户定义的转换类型**长**已定义，转换将执行，如下所示： 类型`UDC`将转换为类型`int`使用用户定义的转换。 从类型的标准转换`int`类型**长**将应用以匹配声明中的自变量。  
...
## <a name="argument-matching-and-the-this-pointer"></a>参数匹配和 this 指针  
 ...
 当`->`成员选择运算符用于访问类的成员函数`class_name`、`this`指针自变量具有一种`class_name * const`。 如果成员声明为`const`或`volatile`的类型是`const class_name * const`和`volatile class_name * const`分别。  
