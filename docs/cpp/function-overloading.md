...
ms.locfileid: "80179778"
---
# <a name="function-overloading"></a>函数重载

C++ 允许同一范围内具有相同名称的多个函数的规范。 这些函数称为*重载*函数。 重载函数使你能够为函数提供不同的语义，具体取决于参数的类型和数量。

例如，采用 `std::string` 参数的 `print` 函数可能会执行的任务与采用类型为**double**的参数的任务不同。 重载使您不必使用诸如 `print_string` 或 `print_double`这样的名称。 在编译时，编译器根据调用方传入的参数类型选择要使用的重载。  如果调用 `print(42.0)`，则将调用 `void print(double d)` 函数。 如果调用 `print("hello world")`，则将调用 `void print(std::string)` 重载。
...
|集 1：其第一个自变量的类型为 Fraction 的候选函数|设置2：可将第二个参数转换为**Int**类型的候选函数|
|--------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
|Variant 1|变体1（使用标准转换可将**int**转换为**long** ）|
|Variant 3||

集2中的函数是指具有从实参类型到形参类型的隐式转换的函数，在此类函数中，有一个函数，将实参类型转换为其形参类型的 "开销" 为小.
...
|集1：具有**Int**类型的第一个参数的候选函数|集2：具有**Int**类型的第二个参数的候选函数|
|---------------------------------------------------------------------|----------------------------------------------------------------------|
|变体2（使用标准转换可将**int**转换为**long** ）|变体1（使用标准转换可将**int**转换为**long** ）|
...
由于同样的原因，通过**const**或**volatile**修改的类型的函数参数的处理方式与用于重载的基类型的处理方式不同。

但是，函数重载机制可以区分由**const**和**volatile**限定的引用和对基类型的引用。 它可以实现如下所示的代码：
...
指向**const**和**volatile**对象的指针也被视为不同于用于重载的基类型的指针。
...
最终的转换序列（如果有）称为最佳匹配序列。 有多种方法可以使用标准转换将**int**类型的对象转换为**无符号长**的类型（如[标准转换](../cpp/standard-conversions.md)中所述）：

- 从**int**转换为**long** ，然后从**long**转换为**无符号长**。
...
|*类型名称*|**const** *类型名称*|
|*类型名称*|**可变***类型名称*|
|*类型名称* __\*__|**const** *type-name* __\*__|
...
1. 完全匹配。 用于调用函数的类型与函数原型中声明的类型之间的完全匹配始终是最佳匹配。 常用转换的序列将归类为完全匹配。 但是，不进行任何转换的序列被认为比转换的序列更好：
   ...
   - 从指针，到指向**const**的指针（`type` <strong>\*</strong>到**const** `type` <strong>\*</strong>）。

   - 从指针到指向**易变**的指针（`type` <strong>\*</strong>到**volatile** `type` <strong>\*</strong>）。

   - 从引用，到对**const** （`type` **&** 到**const** `type` **&** ）的引用。

   - 从引用，到对**volatile** （`type` **&** 到**volatile** `type` **&** ）的引用。

1. 使用提升的匹配。 未归类为仅包含整数提升的任何序列、从**float**到**double**的转换和普通转换归类为使用提升的匹配。 尽管比不上完全匹配，但使用提升的匹配仍优于使用标准转换的匹配。

1. 使用标准转换的匹配。 未归类为完全匹配或仅包含标准转换和常用转换的使用提升的匹配的序列将归类为使用标准转换的匹配。 在此类别中，以下规则将适用：

   - 从指向派生类的指针到指向直接或间接基类的指针的转换优于转换为 `void *` 或 `const void *`。
...
![显示&#45;首选转换的多个继承](../cpp/media/vc391t2.gif "显示&#45;首选转换的多个继承") <br/>
显示首选转换的多继承图

从 `C*` 类型到 `B*` 类型的转换优于从 `C*` 类型到 `A*` 类型的转换。 原因是它们位于同一个路径上，且 `B*` 更为接近。 但是，从类型 `C*` 转换到类型 `D*` 并不优于转换为类型 `A*`;由于转换遵循不同的路径，因此没有任何首选项。
...
类 `UDC` 的用户定义的转换类型为**int**和**long**类型。 因此，编译器会考虑针对将匹配的对象类型的转换：`UDC`。 存在到**int**的转换，并且已选择它。
...
在前面的示例中，调用了用户定义的转换**运算符 long**，以将 `udc` 转换为**long**类型。 如果未定义用户定义的类型为**long**的转换，则将继续执行以下操作：类型 `UDC` 应使用用户定义的转换转换为**int**类型。 然后，将应用从**int**类型到**long**类型的标准转换以匹配声明中的自变量。
...
这两个版本的 `Func` 都需要用户定义的转换才能将类型**int**转换为类类型参数。 可能的转换包括：

- 从类型**int**转换为类型 `UDC1` （用户定义的转换）。

- 从类型**int**转换为**long**类型;然后转换为类型 `UDC2` （两步转换）。
...
类成员函数的处理方式不同，具体取决于它们是否声明为**静态**。 由于非静态函数具有提供**this**指针的隐式参数，因此将非静态函数视为具有一个比静态函数更多的参数;否则，这些声明是相同的。

这些非静态成员函数要求隐含**this**指针与调用函数的对象类型匹配，或者对于重载运算符，它们要求第一个参数与要对其应用运算符的对象匹配。 （有关重载运算符的详细信息，请参阅[重载运算符](../cpp/operator-overloading.md)。）

与重载函数中的其他参数不同，在尝试匹配**此**指针参数时，不会引入临时对象，也不会尝试转换。

当 `->` 成员选择运算符用于访问 `class_name`类的成员函数时，**此**指针参数具有 `class_name * const`类型。 如果将成员声明为**const**或**volatile**，则这些类型分别 `const class_name * const` 和 `volatile class_name * const`。
...
通过引用限定符，可以根据**由此**指向的对象是右值还是左值，来重载成员函数。  当你选择不提供对数据的指针访问权限时，可以使用此功能来避免不必要的复制操作。 例如，假定类 `C` 在其构造函数中初始化某些数据，并在成员函数 `get_data()`中返回该数据的副本。 如果类型 `C` 的对象是要销毁的右值，则编译器将选择 `get_data() &&` 重载，这会移动数据而不是复制数据。
...
前面的代码显示函数 `func` 中的两个定义。 由于**extern**语句的原因，采用类型 `char *` 的参数的定义是本地 `main`。 因此，采用**int**类型的参数的定义是隐藏的，对 `func` 的第一次调用是错误的。
...
对 `Account::Deposit` 中 `Deposit` 的调用将调用私有成员函数。 此调用是正确的，因为 `Account::Deposit` 是成员函数，并且有权访问类的私有成员。
