...
ms.locfileid: "81367916"
---
# <a name="namespaces-c"></a>命名空间 (C++)

命名空间是一个声明性区域，为其内部的标识符（类型、函数和变量等的名称）提供一个范围。 命名空间用于将代码组织到逻辑组中，还可用于避免名称冲突，尤其是在基本代码包括多个库时。 命名空间范围内的所有标识符彼此可见，而没有任何限制。 命名空间外部的标识符可以通过为每个标识符使用完全限定的名称来访问`std::vector<std::string> vec;`成员，或者通过使用[声明](../cpp/using-declaration.md)获取单个标识符 （），`using std::string`或者使用[指令](../cpp/namespaces-cpp.md#using_directives)访问命名空间中的所有标识符 （）。`using namespace std;` 头文件中的代码应始终使用完全限定的命名空间名称。
...
## <a name="using-directives"></a><a id="using_directives"></a>使用指令

**using**指令允许使用**命名空间**中的所有名称，而不将*命名空间名称*作为显式限定符。 如果在命名空间中使用多个不同的标识符，则在实现文件（即 *.cpp）中使用 using 指令;如果只是使用一个或两个标识符，则考虑使用声明，以仅将这些标识符引入作用域，而不是命名空间中的所有标识符。 如果本地变量的名称与命名空间变量的名称相同，则隐藏命名空间变量。 使命名空间变量具有与全局变量相同的名称是错误的。
...
contosodata.cpp 中的函数实现应使用完全限定的名称，即使您在文件顶部放置**了 using**指令：
...
如果未在显式命名空间中声明某个标识符，则该标识符属于隐式全局命名空间的一部分。 通常，尽量避免在全局范围内进行声明，但入口点[主函数](../c-language/main-function-and-program-execution.md)除外，该函数必须位于全局命名空间中。 若要显式限定全局标识符，请使用没有名称的范围解析运算符，如 `::SomeFunction(x);` 中所示。 这将使标识符与任何其他命名空间中具有相同名称的任何内容区分开来，并且还有助于使其他人更轻松地了解你的代码。

## <a name="the-std-namespace"></a>Std 命名空间

所有C++标准库类型和函数都在嵌套在`std`的命名空间或命名空间中`std`声明。

## <a name="nested-namespaces"></a>嵌套命名空间

可以嵌套命名空间。 普通嵌套命名空间对其父成员具有非限定访问权限，但父成员对嵌套命名空间没有非限定访问权限（除非声明为内联），如下例所示：
...
命名空间名称必须是唯一的，这意味着通常它们不应太短。 如果名称的长度使代码难以读取，或者键入无法使用指令的标头文件中是繁琐的，则可以创建一个命名空间别名，作为实际名称的缩写。 例如：
...
这称为未命名或匿名命名空间，当您希望使变量声明对其他文件中的代码不可见（即为它们提供内部链接）而不必创建命名命名空间时，它非常有用。 同一文件中的所有代码都可以看到未命名的命名空间中的标识符，但这些标识符以及命名空间本身在该文件外部（或更准确地说，在翻译单元外部）不可见。
