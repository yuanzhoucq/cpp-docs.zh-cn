...
在 C++ 中，如果某个类型未声明它本身，则编译器将自动为该类型生成默认构造函数、复制构造函数、复制赋值运算符和析构函数。 这些函数称为*特殊成员函数*，它们使简单的用户定义类型C++像结构在 C 中一样。也就是说，您可以创建、复制和销毁它们，而无需执行任何其他编码工作。 C++11 会将移动语义引入语言中，并将移动构造函数和移动赋值运算符添加到编译器可自动生成的特殊成员函数的列表中。
...
这些规则的结果也可能泄漏到对象层次结构中。 例如，如果由于任何原因，基类未能具有可以从派生类（即不需要任何参数**的公共**或**受保护**构造函数）调用的默认构造函数，则派生自它的类无法自动生成自己的默认构造函数。
...
- 必须私下声明复制构造函数来隐藏它，但由于它声明了它，因此阻止自动生成默认构造函数。 如果你需要默认构造函数，则必须显式定义一个（即使它不执行任何操作）。
...
请注意，只要类的正文是不可琳的，就可以默认为类正文之外的特殊成员函数。
...
可以删除特殊成员函数以及普通成员函数和非成员函数，以阻止定义或调用它们。 删除特殊成员函数提供了一种更简洁的方法，可以防止编译器生成不需要的特殊成员函数。 必须在声明函数时将其删除；不能在这之后通过声明一个函数然后不再使用的方式来将其删除。
...
请注意，在前面的示例中，使用`call_with_true_double_only` **float**参数调用会导致编译器错误，但使用 int 参数调用不会;使用`call_with_true_double_only`**int**参数进行调用不会。在**int**案例中，参数将从**int**提升到**双精度**，并成功调用函数**的双**版本，即使这可能不是预期。 为了确保使用非 double 参数对此函数的任何调用都会导致编译器错误，可以声明已删除的函数的模板版本。
