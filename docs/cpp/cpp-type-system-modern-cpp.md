...
已纠正的代码示例使用本地堆栈内存创建 `pNumber` 指向的后备存储。 我们使用基本类型，以求简单。 在实践中, 指针的后备存储最常是用户定义的类型, 这些类型是在使用**新**的关键字表达式 (在 C 样式编程中, 是较早的 `malloc()`使用了 C 运行时库函数)。 分配后, 这些变量通常称为对象, 尤其是在它们基于类定义时。 使用**new**分配的内存必须由相应的**delete**语句删除 (或者, 如果使用`malloc()`函数来分配它, 则为 C 运行时函数`free()`)。
