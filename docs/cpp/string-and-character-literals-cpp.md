...
-   类型的 utf-32 字符文本`char32_t`，例如 `U'a'`  
 ...
 用于字符文本的字符可以是除保留的字符反斜杠之外的任何字符 ('\\)，单引号 （'） 或新行。 可以使用转义序列指定保留字符。 可以通过使用通用字符名称指定字符，只要类型的大小足以保留字符。  
  
### <a name="encoding"></a>编码  
 字符文本进行编码，以不同的方式基于其前缀。  
  
-   字符文本不带前缀的是普通字符文本。 普通字符文本的值包含单个字符转义序列，或者可以在执行字符集中表示的通用字符名称的值等于其执行字符集中的编码数字值。 普通字符文本包含多个字符、 转义序列或通用字符名称是*multicharacter 文本*。 多字符的文本或普通字符文本中不能表示执行字符集中是有条件地支持具有类型为 int 的并且其值是实现定义。  
  
-   字符文本开头 L 前缀是宽字符文本。 宽字符文本包含单个字符、 转义序列或通用字符名称的值具有的值等于其中设置，除非字符文本没有任何表示形式执行宽字符的编码数字值执行宽字符集，在这种情况下的值是实现定义。 宽字符文本包含多个字符、 转义序列或通用字符名称的值是实现定义的。  
  
-   字符文本 u8 前缀开头是字符文本的 utf-8。 Utf-8 字符文本的值包含单个字符转义序列，或如果它可以由单个 utf-8 代码单位 （对应于 C0 控件和基本拉丁，通用字符名称具有的值等于其 ISO 10646 码位值Unicode 块）。 如果值不能由单个 utf-8 代码单元，则程序将格式不正确。 Utf-8 字符文本包含多个字符、 转义序列或通用字符名称是格式不正确。  
  
-   U 前缀开头的字符文本为的 utf-16 字符文本。 Utf-16 字符文本的值包含单个字符转义序列，或如果它可以由单个 utf-16 代码单位 （对应于基本多语言平面表示，通用字符名称具有的值等于其 ISO 10646 码位值). 如果值不能由单个的 utf-16 代码单位表示，则程序将格式不正确。 Utf-16 字符文本包含多个字符、 转义序列或通用字符名称是格式不正确。  
  
-   字符文本 U 前缀开头是 utf-32 字符文本。 Utf-32 字符文本的值包含单个字符转义序列，或者通用字符名称的值等于其 ISO 10646 码位值。 Utf-8 字符文本包含多个字符、 转义序列或通用字符名称是格式不正确。  
...
|警报（响铃）|\a|十六进制|\xhhh|  
 ...
 下面的代码演示使用普通字符文本的转义字符的一些示例。 相同的转义序列语法是对其他字符文本类型有效。  
 ...
 **Microsoft 专用**  
 ...
 若要创建一个从普通字符文本的值 （不带前缀的那些），编译器将转换的字符或为一个 32 位整数内的 8 位值的单引号之间的字符序列。 文本中的多个字符根据需要从高序位到低序位填充相应字节。 为了创建 `char` 值，编译器采用低序位字节。 为了创建 `wchar_t` 或 `char16_t` 值，编译器采用低序位字。 如果在分配的字节或字上设置了任何位，则编译器会警告结果被截断。  
...
```  
 ...
 十六进制转义序列包含一个反斜杠，后跟字符 `x`和一个十六进制数字序列。 不包含十六进制数字的转义序列将导致编译器错误 C2153：“十六进制文本必须至少有一个十六进制数字”。 将忽略前导零。 对于将具有十六进制和非十六进制字符的转义序列，其求值结果为全部由十六进制字符组成的十六进制转义序列，后跟非十六进制字符。   在普通或使用 u8 前缀字符文本，最大的十六进制值为 0xFF。 在使用 L 或 u 前缀的宽字符文本中，最大的十六进制值为 0xFFFF。 在使用 U 前缀的宽字符文本中，最大的十六进制值为 0xFFFFFFFF。  
 ...
 **结束 Microsoft 专用**  
 ...
 反斜杠字符 (\\) 放置在行末尾时将作为行继续符。 如果你希望反斜杠字符显示为字符文本，则必须在一行中键入两个反斜杠 (`\\`)。 有关行继续符的详细信息，请参阅 [Phases of Translation](../preprocessor/phases-of-translation.md)。  
...
### <a name="narrow-string-literals"></a>窄字符串文本  
 窄字符串文本是非前缀且以双引号分隔、 以 null 结尾的类型的数组`const char[n]`，其中 n 是以字节为单位数组的长度。 窄字符串文本可包含除双引号 (`"`)、反斜杠 (`\`) 或换行符以外的所有图形字符。 窄字符串文本还可包含上面列出的转义序列和装入一个字节中的通用字符名称。  
...
#### <a name="utf-8-encoded-strings"></a>UTF-8 编码的字符串  
 ...
 Utf-8 编码字符串是 u8 为前缀且以双引号分隔、 以 null 结尾的类型的数组`const char[n]`，其中 n 是以字节为单位的已编码数组的长度。 以 u8 为前缀的字符串文本可包含除双引号 (`"`)、反斜杠 (`\`) 或换行符以外的所有图形字符。 以 u8 为前缀的字符串文本还可包含上面列出的转义序列和任何通用字符名称。  
...
### <a name="wide-string-literals"></a>宽字符串文本  
 宽字符串是常量的以 null 结尾数组`wchar_t`且具有前缀`L`和包含除双引号 （"）、 反斜杠任何图形 (\\)，或换行字符。 宽字符串文本可包含上面列出的转义序列和任何通用字符名称。  
...
```  
 ...
 请注意，`strlen()`和`wcslen()`不包括终止 null 字符，其大小等于字符串类型的元素大小的大小： char * 字符串中的一个字节，wchar_t 两个字节\*或 char16_t\*字符串，四个字节 char32_t\*字符串。  
...
```  
 ...
 你可能会导致编译器发出错误设置时，将字符串转换为 non_const 字符指针时[/zc: strictstrings （禁用字符串文本类型转换）](../build/reference/zc-strictstrings-disable-string-literal-type-conversion.md)编译器选项。 我们建议将其用于符合标准的可移植代码。 使用 `auto` 关键字声明经过字符串文本初始化的指针也是一个很好的做法，因为它可以解析为正确（常数）的类型。 例如，此代码示例捕捉到一次在编译时写入字符串文本的尝试：  
