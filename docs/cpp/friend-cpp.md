...
友元声明中声明的函数被视为已使用**extern**关键字声明的函数。 有关详细信息，请参阅[extern](extern-cpp.md)。
...
前面的示例将类名 `ForwardDeclared` 输入到范围中，但是完整的声明（具体而言，声明函数 `IsAFriend` 的部分）是未知的。 因此，`HasFriends` 类中的**友元**声明会生成错误。
...
如果尚未声明引用的类型，请使用 `class friend F`：
...
在下面的示例中，`friend F` 引用在 NS 范围之外声明的 `F` 类。
...
**Friend**函数是一个函数，它不是类的成员，但可以访问类的私有和受保护成员。 友元函数不被视为类成员；它们是获得了特殊访问权限的普通外部函数。 友元不在类的范围内，并且不使用成员选择运算符（ **.** 和- **>** ），除非它们是另一个类的成员。 **友元**函数由授予访问权限的类声明。 **友元**声明可放置在类声明中的任何位置。 它不受访问控制关键字的影响。

以下示例显示 `Point` 类和友元函数 `ChangePrivate`。 **Friend**函数有权访问作为参数收到的 `Point` 对象的私有数据成员。
...
在前面的示例中，仅为函数 `A::Func1( B& )` 授予对类 `B` 的友元访问权限。 因此，对私有成员 `_b` 的访问在类 `A` 的 `Func1` 中是正确的，而不是在 `Func2`中。
...
托管类型（在/Cli C++中）不能有任何友元函数、友元类或友元接口。
