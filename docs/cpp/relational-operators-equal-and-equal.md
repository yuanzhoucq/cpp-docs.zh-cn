...
## <a name="comparing-pointers"></a>比较指针  
 在比较两个指向同一类型的对象的指针时，结果由程序的地址空间中所指向的对象的位置决定。 也可以将指针与计算结果为 0 的常量表达式或 void * 类型的指针进行比较。 如果将指针进行比较的类型的指针 void \*，另一个指针将隐式转换为类型 void \*。 然后进行比较。  
  
 不能比较两个类型不同的指针，除非：  
  
-   一个类型是派生自另一个类型的类类型。  
  
-   至少有一个指针显式转换（强制转换）为类型 void *。 (另一个指针将隐式转换为类型 void\*转换。)  
  
 两个指向同一对象的相同类型的指针一定是相等的。 如果比较两个指向对象的非静态成员的指针，则以下规则将适用：  
  
-   如果类类型不是联合，并且不由分隔的两个成员*访问说明符*，例如，公共、 受保护的或私有，指向声明的成员的上一次将比较大于声明的成员的指针前面。  
