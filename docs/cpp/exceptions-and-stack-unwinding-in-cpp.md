...
在 C++ 异常机制中，控制从 throw 语句移至可处理引发类型的第一个 catch 语句。 当达到 catch 语句时，位于范围内之间 throw 语句和 catch 语句的自动变量的所有过程中被销毁称为*堆栈展开*。 在堆栈展开中，执行将继续，如下所示：  
  
1.  控制通过正常顺序执行到达 `try` 语句。 执行 `try` 块内的受保护部分。  
  
2.  如果执行受保护的部分的过程中未引发异常，将不会执行 `catch` 块后面的 `try` 子句。 执行将在关联的 `catch` 块后的最后一个 `try` 子句后面的语句上继续。  
...
5.  如果找到匹配的 `catch` 处理程序，并且它通过值进行捕获，则通过复制异常对象来初始化其形参。 如果它通过引用进行捕获，则初始化参数以引用异常对象。 在初始化形参后，堆栈的展开过程将开始。 这包括对与 `try` 处理程序关联的 `catch` 块的开头和异常的引发站点之间完全构造（但尚未析构）的所有自动对象的析构。 析构按照与构造相反的顺序发生。 执行 `catch` 处理程序且程序会在最后一个处理程序之后（即，在不是 `catch` 处理程序的第一个语句或构造处）恢复执行。 控制只能通过引发的异常进入 `catch` 处理程序，而绝不会通过 `goto` 语句或 `case` 语句中的 `switch` 标签进入。  
