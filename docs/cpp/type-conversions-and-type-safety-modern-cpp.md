...
在很多表达式中，C 样式数组将隐式转换为指向该数组中的第一个元素的指针，并且可能在没有提示的情况下进行常量转换。 尽管此方法很方便，但它也可能容易出错。 例如，下面设计错误的代码示例似乎过程，但它会编译并生成 "p" 的结果。 首先，将 "帮助" 字符串常量文本转换为 **`char*`** 指向数组的第一个元素的; 然后，该指针将递增三个元素，以使其现在指向最后一个元素 "p"。
...
C 样式强制转换运算符与调用运算符 () 相同，因此在代码中不显眼，容易被忽略。 这两种方法都是错误的，因为它们难以一目了然地识别或搜索，它们完全不同于调用、和的任意组合 **`static`** **`const`** **`reinterpret_cast`** 。 确定旧式强制转换的实际作用可能很困难并且容易出错。 基于所有这些原因，当需要进行强制转换时，我们建议使用以下 C++ 强制转换运算符之一。在某些情况下，它们更加类型安全，并且可以更加明确地表达编程目的：

- **`static_cast`**，用于仅在编译时检查的强制转换。 **`static_cast`** 如果编译器检测到您尝试在完全不兼容的类型之间进行转换，将返回错误。 您还可以使用它在指向基对象的指针和指向派生对象的指针之间强制转换，但编译器无法总是判断出此类转换在运行时是否安全。
...
- **`dynamic_cast`**，用于以安全的运行时检查的指针转换为指向派生的指针。 **`dynamic_cast`** 比 **`static_cast`** 用于 type.2，但运行时检查会产生一些开销。
...
- **`const_cast`**，用于转换掉变量的特性 **`const`** ，或 **`const`** 将非变量转换为 **`const`** 。 **`const`** 使用此运算符的 "强制转换为"，就像使用 C 样式强制转换一样容易出错，只不过使用时不 **`const_cast`** 太可能意外地执行强制转换。 有时，您必须强制转换掉 **`const`** 变量的特性，例如，将 **`const`** 变量传递给采用非参数的函数 **`const`** 。 以下示例演示如何执行此操作。
...
- **`reinterpret_cast`**，用于不相关类型（如指针类型和）之间的转换 **`int`** 。
   ...
   下面的示例演示如何 **`reinterpret_cast`** 与不同 **`static_cast`** 。
