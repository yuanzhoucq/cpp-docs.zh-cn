...
类或结构由其成员组成。  类的工作由其成员函数执行。  它所维持的状态存储在其数据成员中。  成员的初始化由构造函数完成，释放内存和释放资源等清理工作也由构造函数完成。  在 C\+\+ 11 和更高版本中，数据成员可以（并且通常应该）在声明时初始化。  
...
-   [数据成员](../cpp/static-members-cpp.md)包括内置类型和其他用户定义的类型。  
  
-   运算符  
  
-   [嵌套类声明](../cpp/nested-class-declarations.md)和  
...
## 成员可访问性  
 在成员列表中声明类的成员。  可使用称为访问说明符的关键字将类的成员列表中分为任意数量的 `private`、`protected` 和 **public** 部分。  冒号 **:** 必须跟在访问说明符后面。  这些部分不需要是连续的，也就是说，这些关键字中的任何一个都可能在成员列表中多次出现。  关键字指定所有成员直到下一个访问说明符或右大括号的访问。  有关详细信息，请参阅[成员访问控制 \(C\+\+\)](../cpp/member-access-control-cpp.md)。  
  
## 静态成员  
 可将数据成员声明为静态，这表示类的所有对象都有权访问它的同一副本。  可将成员函数声明为静态，在这种情况下它只能访问类的静态数据成员（且不具有 *this* 指针）。  有关详细信息，请参阅[静态数据成员](../cpp/static-members-cpp.md)。  
