...
|`long double`|相同于`double`|无|与 `double` 相同|
|`wchar_t`|2|`__wchar_t`|0 到 65,535|

根据使用方式， `__wchar_t` 的变量指定宽字符类型或多字节字符类型。 在 `L` 字符或字符串常量前使用前缀来选择宽字符类型常量。

`signed` 和 `unsigned` 是可用于任何整型（ `bool`除外）的修饰符。 请注意，对于重载和模板等机制而言， `char`、 `signed char`和 `unsigned char` 是三种不同的类型。

`int`和 `unsigned int` 类型的大小为4字节。 但是，可移植代码不应依赖于的大小， `int` 因为语言标准允许实现特定的。

Visual Studio 中的 C/C++ 还支持按大小分类的整型。 有关详细信息，请参阅[__int8、 \_ _int16、 \_ _int32、 \_ _int64](../cpp/int8-int16-int32-int64.md)和[整数限制](../cpp/integer-limits.md)。
