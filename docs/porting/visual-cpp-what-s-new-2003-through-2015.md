...
- [可视化工作室 2015 更新 2 的一致性改进](#VS_Update2)
...
- **placement new 和 placement delete**
   ...
   函数 `void operator delete(void *, size_t)` 是与 C++11 中的 placement new 函数 `void * operator new(size_t, size_t)` 对应的 placement delete 运算符********。 使用 C++14 调整了大小的释放，此 delete 函数现在是常用释放函数 （全局 delete 运算符）**********。 该标准要求，如果使用**放置新**查找相应的**删除**函数，并查找通常的处理函数，则程序格式不正确。
   ...
   出现此问题的原因是已定义的**放置删除**运算符和新的全局大小**删除**运算符之间的函数签名匹配。 考虑是否可使用任何 placement new 和 placement delete 运算符的其他类型（`size_t` 除外********）。  请注意， typedef 的类型取决于编译器；在 Visual C++ 中，它是一个无符号整型的 typedef`size_t` ************。 较好的解决办法就是使用如下的枚举类型：

   ```cpp
    enum class my_type : size_t {};
   ```

   然后，更改对 placement new 和 placement delete 的定义，以使用此类型作为第二个参数（而不是 `size_t`）********。 您还需要更新对**放置新类型的**调用以传递新类型（例如，通过使用`static_cast<my_type>`从整数值转换），并更新**新**定义和**删除**以强制转换回整数类型。 您无需为此使用**枚举**;具有成员的`size_t`类类型也工作。
   ...
   如果你不想立即更新代码，可通过编译器选项 `/Zc:sizedDealloc-` 还原到之前的行为。 如果使用此选项，则双参数**删除**函数不存在，不会导致与**放置删除**运算符发生冲突。
...
### <a name="conformance-improvements-in-visual-studio-2015-update-1"></a><a name="VS_Update1"></a>可视化工作室中的一致性改进 2015 更新 1
...
- **可能会因对表达式 SFINAE 的部分支持而发出其他警告和错误**
   ...
   在这个新行为分析 decltype 表达式时（将依赖名称指定为类型时必须使用 关键字 typename，而此表达式未使用），编译器将发出编译器警告 C4346 和编译器错误 C2923********。
...
-  成员变量将防止出现隐式定义的构造函数和赋值运算符：在早期版本的编译器，具有 volatile 成员变量的类可自动生成默认的复制/移动构造函数和默认的复制/移动赋值运算符`volatile` ********。 这种旧行为不正确，也不符合 C++ 标准。 编译器现在认为拥有可变成员变量的类具有非常用构造函数和赋值运算符，这将防止自动生成这些运算符的默认实现。 当此类为某一联合（或类中的匿名联合）的成员时，会将联合（或包含匿名联合的类）的复制/移动构造函数和复制/移动赋值运算符的隐式定义为已删除。 如果尝试构造或复制联合（或包含匿名联合的类）而不显示定义它们是错误的，将导致编译器发出编译器错误 C2280。
...
### <a name="conformance-improvements-in-visual-studio-2015-update-3"></a><a name="VS_Update3"></a>可视化工作室 2015 更新 3 的一致性改进
...
- make_unique\<T>（args...）和make_unique<T_>（n）
...
- **支持值结构中的装箱类型。**
   ...
   现在，可以使用可以为 null 的字段（例如，`IBox<int>^`而不是**int）** 定义值类型。这意味着字段可以具有值，也可以等于**nullptr**。
...
**基于上下文的成员列表筛选。** 无法访问的成员已从 IntelliSense 成员列表中筛选出来。 例如，私有成员不会在成员列表中显示，除非您修改了实现此类型的代码。 当成员列表处于打开状态时，您可以按**Ctrl**+**J**删除一个级别的筛选（仅适用于当前成员列表窗口）。 您可以再次按**Ctrl**+**J**以删除文本筛选并显示每个成员。
...
- **自动矢量器**。 编译器会分析代码中的循环，如果可能，还会发出使用向量寄存器的指令和显示在所有新式处理器上的指令。 这使循环运行得更快。 （处理器指令被称为 SSE，即流式处理 SIMD 扩展）。 此项优化自动应用，无需启用或请求。
- **自动并行器**。 编译器可以分析编码中的循环并发出指令，向多个核心或处理器分散计算。 这能让循环运行得更快。 此项优化不会自动启用，必须进行请求。 在许多情况下，这都有助于在想并行化的循环之前将 `#pragma loop(hint_parallel(N))` 循环 (hint_parallel(N)) 包含在你的代码中。
...
除了**并行任务**窗口和**并行堆栈**窗口外，Visual Studio 2012 还提供了新的**并行监视**窗口，以便您可以检查所有线程和进程中的表达式值，并执行对结果的排序和筛选。 也可使用可视化工具来扩展窗口，并可利用跨所有工具窗口的全新多进程支持。
...
**列表成员的增强功能。** 当您在代码编辑器中键入代码时，"**列表成员**"下拉列表会自动显示。 结果是筛选过的，所以只会显示与你的类型相关的成员。 可控制成员列表采用的筛选逻辑。为此，请在“文本编辑器” > “C/C++” > “高级”下的“选项”对话框中操作****************。

**语义着色。** 现在默认对类型、枚举、宏和其他 C++ 标记进行着色。

**参考突出显示。** 选择一个符号现在将在当前文件中突出显示该符号的所有实例。 按 **"Ctrl**+**向上移位**+**箭头**"或 **"Ctrl**+**向下移位**+**箭头**"以在突出显示的参照之间移动。 可在“文本编辑器” > “C/C++” > “高级”下的“选项”对话框中关闭此功能****************。
...
- 在 **"代码分析"** 窗口中，可以将禁止选择的警告的杂注插入到源代码中。
...
为了更好地理解代码，可以对二进制、类、命名空间和解决方案中的包含文件生成依赖项关系图。 在菜单栏上，选择**体系结构** > **生成依赖项图**，然后**为解决方案**或 **"包括文件"** 生成依赖项图。 图形生成完成后，可以通过展开每个节点来浏览它，通过节点之间移动来学习依赖关系，并通过选择节点的快捷菜单上的 **"查看内容**"来浏览源代码。 要生成包含文件的依赖项图，请在\*.cpp 源代码文件或\*.h 标头文件的快捷菜单上，选择 **"包含文件的生成图形**"。

#### <a name="architecture-explorer"></a>体系结构资源管理器

通过使用**体系结构资源管理器**，可以浏览C++解决方案、项目或文件中的资产。 在菜单栏上，选择**体系结构** > **Windows** > **体系结构资源管理器**。 您可以选择您感兴趣的节点，例如 **，类视图**。 这时工具窗口的右侧会展开一个命名空间列表。 如果选择一个命名空间，会出现一个新列，显示此命名空间中的类、结构和枚举列表。 你可以继续浏览这些资产，也可以返回最左侧的那一列开始另一项查询。 请参阅**使用体系结构资源管理器查找代码**。

#### <a name="code-coverage"></a>代码覆盖率

已更新代码覆盖率，在运行时动态检测二进制。 这降低了配置开销，提升了性能。 你还可以从 C++ 应用的单元测试收集代码覆盖率数据。 创建C++单元测试后，可以使用**单元测试资源管理器**在解决方案中发现测试。 要运行单元测试并为其收集代码覆盖率数据，在**单元测试资源管理器**中，请选择 **"分析代码覆盖率**"。 可在“代码覆盖率结果”窗口（在菜单栏上选择“测试” > “窗口” > “代码覆盖率结果”）中查看代码覆盖率结果****************。

## <a name="whats-new-for-c-in-visual-studio-2010"></a>Visual Studio 2010 中 C++ 的新增功能

### <a name="c-compiler-and-linker"></a>C++ 编译器和链接器

**自动关键字。** **自动**关键字具有新用途。 使用**auto**关键字的默认含义声明其类型从变量声明中的初始化表达式推断出的变量。 通过 `/Zc:auto` 编译器选项，可调用 auto 关键字的新旧含义****。

**decltype 类型说明符。** **deltype 类型**指定器返回指定表达式的类型。 将**decltype 类型**指定器与**auto**关键字结合使用，声明一种类型很复杂或仅对编译器所知。 例如，将这个组合用于声明返回类型取决于其模板自变量类型的模板函数。 或者声明调用其他函数且返回被调用函数类型的模板函数。

**兰姆达表达式。** Lambda 函数有函数体但没有名称。 lambda 函数兼具函数指针和函数对象的最佳特性。 将 lambda 函数本身用作模板函数参数而不是函数对象，或与**auto**关键字一起声明其类型为 lambda 的变量。

**Rvalue 引用。** Rvalue 引用声明符 (&&) 声明对 rvalue 的引用。 通过 rvalue 引用，可以使用更多语义和完美转移编写出更有效的构造函数、函数和模板。

**static_assert 声明。** **static_assert**声明在编译时测试软件断言，这与在运行时测试的其他断言机制不同。 如果断言失败，则编译将失败，并且会发出特定的错误消息。
...
特定于 Microsoft**的__nullptr**关键字与**nullptr**具有相同的含义，但它仅适用于本机代码。 如果使用 `/clr` 编译器选项编译本机 C/C++ 代码，则编译器无法确定 nullptr 关键字是本机项还是托管项****。 要向编译器明确您的意图，请使用 nullptr 关键字指定托管**术语，__nullptr**指定本机术语。
...
**VC++ 目录。** VC++ 目录设置现在位于两个位置。 使用项目属性页可为 VC++ 目录设置每个项目的值。 使用**属性管理器**和属性表为 VC++ 目录设置全局的按配置值。
...
**重启管理器。** 重新启动管理器会在应用程序意外关闭或重新启动时，自动保存文档并重新启动该应用程序。 例如，在自动更新关闭了某个应用程序后，可以使用重新启动管理器来启动该应用程序。 有关如何将应用程序配置为使用重新启动管理器的详细信息，请参阅**如何：添加重新启动管理器支持**。
...
MFC 现在支持动画和 Direct2D 图形。 MFC 库中新增了一些 MFC 类和函数来支持此功能。 此外还新增了两个演练来分别演示如何将 D2D 对象和动画对象添加到项目中。 这些演练是**演练：将 D2D 对象添加到 MFC 项目和****演练：向 MFC 项目添加动画**。
...
由于这种职责分离，类**视图**和新的**导航到**对话框等浏览功能由基于新的 SQL Server 桌面数据库 （.sdf） 文件的系统处理，该文件取代了旧的无编译浏览 （.ncb） 文件。 IntelliSense 功能（例如，快速信息、自动完成和参数帮助）仅在需要时才会分析转换单元。 混合功能（如新的**呼叫层次结构**窗口）使用浏览和 IntelliSense 功能的组合。
由于 IntelliSense 仅处理你目前需要的信息，因此 IDE 的响应速度更快。 另外，由于信息更新，IDE 视图和窗口更准确了。 最后，由于 IDE 基础结构的组织更合理、处理能力更强并且可伸缩性更大，因此可以处理更大的项目。

**改进的 IntelliSense 错误。** IDE 可更好地检测可能会损失 IntelliSense 的错误，并在错误下显示红色波浪下划线。 此外，IDE 将 IntelliSense 错误报告到**错误列表窗口**。 要显示导致问题的代码，请双击**错误列表窗口中**的错误。
...
**导航到。** **"导航到"** 对话框允许您搜索项目中与指定字符串匹配的所有符号和文件。 随着你在搜索字符串中键入其他字符，搜索结果会即时进行修改。 "**结果**反馈"字段告诉您找到的项目数，并帮助您决定是否限制搜索。 **"类型/范围**、**位置**"和 **"预览"** 反馈字段可帮助您消除具有类似名称的项目的歧义。 此外，你可以扩展此功能以支持其他编程语言。

**并行调试和分析。** Visual Studio 调试器可识别并发运行时，帮助排除并行处理应用程序故障。 可使用新的并发探查器工具直观地显示应用程序的总体行为。 此外，可使用新的工具窗口直观地显示任务及其调用堆栈的状态。

**功能区设计器。** **功能区设计器**是一个图形编辑器，允许您创建和修改 MFC 功能区 UI。 最终的功能区 UI 由基于 XML 的资源文件 (.mfcribbon-ms) 表示。 对于现有应用程序，可以通过临时添加几行代码，然后调用**功能区设计器**来捕获当前功能区 UI。 创建功能区资源文件之后，可以使用一些用于加载功能区资源的语句来替换手写的功能区 UI 代码。

**调用层次结构。** "**调用层次结构"** 窗口允许您导航到由特定函数调用的所有函数，或导航到调用特定函数的所有函数。

### <a name="tools"></a>工具

**MFC 类向导。** Visual C++ 2010 恢复了备受好评的 MFC 类向导工具。 通过使用 MFC 类向导，可以很方便地向项目中添加类、消息和变量，而不必手动修改源文件集。

**ATL 控制向导。** ATL 控件向导不再自动填充 `ProgID` 字段。 如果某个 ATL 控件没有 `ProgID`，则其他工具可能无法使用该控件。 例如，“插入活动控件”对话框需要控件具有 `ProgID`****。 有关对话框的详细信息，请参阅**插入 ActiveX 控件对话框**。
...
- 在 ATL、MFC 和 Win32 应用程序中创建的对话框现在符合 Windows Vista 样式指南。 使用 Visual Studio 2008 创建新项目时，插入应用程序中的所有对话框将符合 Windows Vista 样式指南。 如果重新编译使用早期版本的 Visual Studio 创建的项目，所有现有对话框将保持以前的外观。 有关如何将对话框插入到应用程序中的详细信息，请参阅**对话框编辑器**。

- **ATL 项目**向导现在可以选择为所有用户注册组件。 从 Visual Studio 2008 开始，**由 ATL 项目**向导创建的 COM 组件和类型库在注册表的HKEY_CURRENT_USER节点中注册，除非您**为所有用户选择"注册"组件**。
- **ATL 项目**向导不再提供创建属性化 ATL 项目的选项。 从 Visual Studio 2008 开始 **，ATL 项目**向导没有更改新项目的属性状态的选项。 该向导创建的所有新 ATL 项目现在都未经属性化。
...
- **类设计器**现在对本机C++代码的支持有限。 在早期版本的 Visual Studio 中，**类设计器**仅使用 Visual C# 和可视化基本版。 C++用户现在可以使用**类设计器**，但只能在只读模式下使用 。 有关如何将**类设计器**与C++一起使用的详细信息，请参阅**在类设计器中使用可视C++代码**。
...
- C++ 引入了封送处理库。 封送处理库为在本机和托管环境之间封送数据提供了一个易于使用的优化方法。 封送处理库可以替代其他更复杂、效率更低的方法（例如使用 PInvoke）。 有关详细信息 **，请参阅C++封送的概述**。

#### <a name="atl-server"></a>ATL 服务器

- ATL 服务器作为共享源项目发布。
- 大多数 ATL 服务器基本代码已作为共享源项目发布在 CodePlex 上，而不是作为 Visual Studio 2008 的一部分进行安装。 与 ATL 服务器相关联的多个文件不再属于 Visual Studio。 有关已删除文件的列表，请参阅**删除的 ATL 服务器文件**。
...
- 内部函数支持更新的 AMD 和 Intel 处理器。 多条新的内部函数指令在新型 AMD 和 Intel 处理器中提供更强大的功能。 有关新内部函数的详细信息，请参阅**补充流式 SIMD 扩展 3 指令**、**流式 SIMD 扩展 4 指令****、SSE4A 和高级位操作固有****、AES 内部函数****、_mm_clmulepi64_si128**和 **__rdtscp**。
...
- 弃用的编译器选项：此版本中已弃用许多编译器选项;有关详细信息 **，请参阅弃用编译器选项**。
...
- 内联函数启发式已更新;有关详细信息，请参阅**内联****、__inline、__forceinline**和 **__forceinline****inline_depth**
...
- 依赖于区域设置的函数现在有了新的版本，这些版本将区域设置作为参数，而不是使用当前的区域设置。 这些新功能具有 **_l**后缀。 添加了一些新函数，用于区域设置对象。 新函数包括 `_get_current_locale`、`_create_locale` 和 `_free_locale`。
- 添加了新函数，以支持锁定和解锁文件句柄。
...
- 几个字符串函数现在可以选择截断字符串，而不是在输出缓冲区太小时失败;参见 **_TRUNCATE**。
- `_set_se_translator` 现需要使用`/EHa` 编译器选项。
- 现在，在 `/Za` 下（对于 C 代码）和手动设置 STDC（对于 C++ 代码）时，`fpos_t` 会变为 __int64****____。 它曾经是一个**结构**。
...
- 删除了 `/noBool` 编译器选项。 编译器现在允许**bool**仅在C++源代码文件中显示为关键字（而不是标识符）。
- **长长**类型现在作为 **__int64****的 typedef**提供，请注意，CRT**中尚未**长时间支持。
