...
```  
 ...
 当前，项目正使用 MBCS（多字节字符集）进行生成，因此 char 是合适的字符数据类型。  但是，为了实现更轻松地将代码更新为 UTF\-16 Unicode，我们将其更新为 TCHAR，TCHAR 解析为 char 或 wchar\_t，具体取决于项目设置中的**“字符集”**属性是否设置为 MBCS 或 Unicode。  
