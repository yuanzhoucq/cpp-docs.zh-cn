...
此代码在 Visual C++ 支持内置的 bool 类型前编写。 在此类代码中，BOOL 是**int**的**typedef** 。**Int**类型是**带符号**类型，而**带符号 int**的位表示形式是将第一位用作符号位，因此 int 类型的位域可以解释为表示0或-1，这可能不是预期的。
