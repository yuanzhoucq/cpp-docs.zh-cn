...
问题在于 `i` 声明为 `UINT` 并 `lCount` 声明为 **`long`** ，因此有符号/无符号不匹配。 将的类型更改为很不方便 `lCount` `UINT` ，因为它从使用类型的中获取值， `IMtsEventInfo::get_Count` **`long`** 而不是用户代码。 因此，我们向代码添加强制转换。 C 样式转换将对数字转换（如此类）执行此操作，但 **`static_cast`** 建议使用样式。
