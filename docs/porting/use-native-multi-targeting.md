...
ms.locfileid: "81353264"
---
# <a name="use-native-multi-targeting-in-visual-studio-to-build-old-projects"></a>使用 Visual Studio 中的本机多目标来生成旧项目

通常，建议在安装最新版本的 Visual Studio 时更新项目。 更新项目和代码所要花费的成本通常会大过新 IDE、编译器、库和工具所带来的好处。 但是，我们知道你可能无法更新某些项目。 您可能有绑定到旧库或平台的二进制文件，出于维护原因无法升级。 你的代码可能使用非标准的语言构造，如果移动到更新的编译器，构造会发生中断。 代码可能依赖于为特定 Visual C++ 版本编译的第三方库。 或者可能需要为某些组织生成库，而这些组织必须以某个 Visual C++ 的特定较旧版本为目标。
...
还可以面向 Visual Studio 2008 项目，但不能一成不变地使用这些项目。 有关详细信息，请参阅**Visual Studio 2008**说明部分。
...
使用较旧的工具集时，可以利用许多最新的 IDE 功能，但不能利用 C++ 编译器、库和生成工具中的最新改进。 例如，将无法使用新的语言一致性改进、新调试和代码分析功能，或实现更快的最新工具集生成吞吐量。 也有一些 IDE 功能与旧的工具集不兼容。 例如，内存探查器中可能缺少类型信息，重构操作 **"转换为原始字符串文本"** 将生成C++11兼容的代码，在使用 Visual Studio 2012 或较旧的工具集时，这些代码不会编译。
...
Visual Studio 2008 有它自己的专用构建系统C++称为**VCBuild**。 从 Visual Studio 2010 开始，Visual Studio C++ 项目改为使用 **MSBuild**。 这意味着，无论是永久升级还是多目标级，都必须执行更新步骤，才能在最新版本的 Visual Studio 中构建 Visual Studio 2008 项目。 更新的项目仍然会生成与使用 Visual Studio 2008 IDE 创建的二进制文件完全兼容的二进制文件。

首先，除了当前版本的 Visual Studio 之外，还必须将 Visual Studio 2010 安装在安装 Visual Studio 2008 的计算机上。 只有 Visual Studio 2010 安装面向 Visual Studio 2008 项目所需的**MSBuild**脚本。
...
> [!NOTE]
> 以下步骤仅适用于多目标方案。 如果打算将项目永久升级到以后的工具集，则下一步是保存项目，在 Visual Studio 2019 中打开该项目，并解决那里出现的生成问题。

升级完成后，如果日志报告中有关于任何项目的错误或警告，请仔细查看。 从**VCBuild**转换为**MSBuild**可能会导致问题。 请确保已了解并执行报告中列出的所有操作项。 有关升级日志报告以及将**VCBuild**转换为**MSBuild**时可能出现的问题的详细信息，请参阅此[C++本机多目标](https://blogs.msdn.microsoft.com/vcblog/2009/12/08/c-native-multi-targeting/)博客帖子。
