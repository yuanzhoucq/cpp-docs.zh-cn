...
ms.locfileid: "62314021"
---
# <a name="walkthrough-compiling-a-ccx-program-on-the-command-line"></a>演练：编译C++命令行上的 /CX 程序

可以创建面向 Windows 运行时的 Visual C++ 程序，并在命令行上生成这些程序。 Visual C++ 支持 Visual C++ 组件扩展 (C++/CX)，其中具有面向 Windows 运行时编程模型的其他类型和运算符。 可以使用C++/CX 来构建通用 Windows 平台 (UWP)、 Windows Phone 8.1 和 Windows 桌面应用程序。 有关详细信息，请参阅[的 c + 教程 + /cli CX](https://msdn.microsoft.com/magazine/dn166929.aspx)并[运行时平台的组件扩展](../extensions/component-extensions-for-runtime-platforms.md)。
...
1. 在菜单栏上依次选择**文件** > **保存**。
   ...
   你已创建视觉对象C++使用 Windows 运行时的源文件[平台命名空间](../cppcx/platform-namespace-c-cx.md)命名空间。
