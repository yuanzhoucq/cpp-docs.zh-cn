...
> [!NOTE]
>  **/Ze**选项已弃用，因为其行为是在默认情况下。 我们建议你使用[/Zc （一致性）](../../build/reference/zc-conformance.md)编译器选项来控制特定的语言扩展功能。 有关不推荐使用的编译器选项的列表，请参阅**已弃用并删除的编译器选项**主题中[按类别列出的编译器选项](../../build/reference/compiler-options-listed-by-category.md)。

Visual c + + 编译器提供了数之外写于 ANSI C89、 ISO C99 或 ISO c + + 标准中指定的功能。 这些功能统称为 Microsoft 扩展 C 和 c + +。 这些扩展时，将按默认情况下，提供的不可用于 **/Za**指定选项。 有关特定扩展的详细信息，请参阅[Microsoft C 和 c + + 扩展](../../build/reference/microsoft-extensions-to-c-and-cpp.md)。
