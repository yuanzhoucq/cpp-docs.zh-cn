 ...
 **\/guard:cf** 选项使编译器在编译时为间接调用目标分析控制流，然后插入代码以在运行时验证目标。 默认情况下，**\/guard:cf** 处于关闭状态，必须显式启用它。 若要显式禁用此选项，请使用 **\/guard:cf\-**。  
 ...
 必须将 **\/guard:cf** 选项传递给编译器和链接器以生成使用 CFG 利用缓解技术的代码。 如果你的二进制文件是使用单个 `cl` 命令生成的，则编译器会将该选项传递到链接器。 如果你分别编译和链接，则必须同时在编译器和链接器命令上设置该选项。 \/DYNAMICBASE 链接器选项也是必需的。 若要验证你的二进制文件具有 CFG 数据，请使用 `dumpbin /headers /loadconfig` 命令。 支持 CFG 的二进制文件在 EXE 或 DLL 的特征列表中具有 `Guard`，并且 Guard 标志包括 `CF 检测`和 `FID 表存在`。  
