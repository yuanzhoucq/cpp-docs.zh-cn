---
title: "DUMPBIN 选项 | Microsoft Docs"
ms.custom: ""
ms.date: "11/04/2016"
ms.reviewer: ""
ms.suite: ""
...
选项由选项说明符（短划线 \( – \) 或者正斜杠 \(\/\)）后跟选项的名称组成。  选项名不能缩写。  某些选项带参数，参数在冒号 \(:\) 后指定。  在选项规范内不允许有空格或制表符。  使用一个或多个空格或制表符来分隔命令行中的选项规范。  选项名及其关键字或文件名参数不区分大小写。  大多数选项适用于所有二进制文件；少数选项仅适用于某些类型的文件。  默认情况下，DUMPBIN 将信息发送到标准输出。  使用 [\/OUT](../../build/reference/out-dumpbin.md) 选项将输出发送到文件。  
