...
ms.locfileid: "77257749"
---
# <a name="cc-property-pages"></a>C/C++属性页

以下属性页位于 "**项目** > **属性**" 下 > **配置属性** > **C/C++** ：
...
将编译器警告视为错误。 对于新项目，最好在每次编译时使用[/wx](wx-treat-linker-warnings-as-errors.md) 。 解决所有警告，尽量减少难以发现的代码缺陷。

### <a name="warning-version"></a>警告版本

隐藏编译器的特定版本后引入的警告。 将[/Wv： xx\[\[.zzzzz\]\]](wx-treat-linker-warnings-as-errors.md)。
...
- **仅 __inline** -只展开标记为 "**内联**"、"`__inline`"、"`__forceinline`" 或 "`__inline`" 的函数。 或在类声明C++中定义的成员函数中。
- 标记为**内联**或 `__inline` 的**任何合适**的扩展函数以及编译器选择的任何其他函数。 （扩展发生于编译器的判断，通常称为*自动内联*。）
...
通过将代码生成延迟到链接时间来启用跨模块优化。 需要链接器选项 "链接时间代码生成"。 设置[/gl](gl-whole-program-optimization.md)。
...
预处理 C 和C++源文件，并将预处理的输出写入文件。 此选项取消编译，不生成 *`.obj`* 文件。
...
编译器仅在程序映像中创建相同字符串的一个只读副本。 这会生成较小的程序，这种优化称为*字符串池*。 [/O1、/O2](o-options-optimize-code.md)和[/zi](z7-zi-zi-debug-information-format.md)会自动设置[/gf](gf-eliminate-duplicate-strings.md)选项。

### <a name="enable-minimal-rebuild"></a>启用最小重新生成

启用最小重新生成，它确定是否C++重新编译包含已更改C++的类定义、存储在标头中 *`.h`* 文件的源文件。
...
- **多线程调试**-定义 _DEBUG 和 _MT。 此选项还会使编译器将库名*libcmtd.lib*放入 *`.obj`* 文件中，以便链接器使用*libcmtd.lib*解析外部符号。
...
允许使用在支持增强指令集的处理器上找到的指令。 例如，SSE、SSE2、AVX 和 AVX2 增强到 IA-32。 而且，AVX 和 AVX2 对 x64 的增强功能。 目前 **`/arch:SSE`** 和 **`/arch:SSE2`** 仅在为 x86 体系结构生成时可用。 如果未指定任何选项，则编译器将使用在支持 SSE2 的处理器上找到的指令。 可以通过 **`/arch:IA32`** 禁用增强说明的使用。 有关详细信息，请参阅[/arch （x86）](arch-x86.md)、 [/arch （x64）](arch-x64.md)和[/arch （ARM）](arch-arm.md)。
...
- **严格**-最严格的浮点模型。 **`/fp:strict`** 会导致 **`fp_contract`** 关闭并 **`fenv_access`** 。 **`/fp:except`** 隐含，可以通过显式指定 **`/fp:except-`** 来禁用。 与 **`/fp:except-`** 一起使用时， **`/fp:strict`** 强制实施严格的浮点语义，但不考虑异常事件。
- **Fast** -在大多数情况下创建最快的代码。
...
如果指定，则类型**wchar_t**将成为映射到 `__wchar_t` 的本机类型，其方式与将**短**映射到 `__int16`的方式相同。 [/Zc：](zc-wchar-t-wchar-t-is-native-type.md)默认情况下 wchar_t 启用。
...
确定编译器C++启用的语言标准。 尽可能使用最新版本。 设置[/std： c + + 14，/std： c + + 17，/std： c + + 最新版本](std-specify-language-standard-version.md)。
...
指定生成的预编译头文件的路径或名称。 设置[/fp](fp-name-dot-pch-file.md)。
...
指定 ASM 列表文件的相对路径或名称;可以是文件名或目录名。 设置[/Fa](fa-fa-listing-file.md)。
...
指定生成的 XML 文档文件的名称;可以是文件名或目录名。 设置[/doc：\<名称 >](doc-process-documentation-comments-c-cpp.md)。

## <a name="cc-browse-information-properties"></a>C/C++浏览信息属性

### <a name="enable-browse-information"></a>启用浏览信息

指定 *`.bsc`* 文件中的浏览信息的级别。 设置[/fr](fr-fr-create-dot-sbr-file.md)。

### <a name="browse-information-file"></a>浏览信息文件

指定浏览器信息文件的可选名称。 将[/fr\<名称 >](fr-fr-create-dot-sbr-file.md)。
...
为 *`.c`* 和 *`.cpp`* 文件选择 "编译语言" 选项。 设置[/tc、/tp](tc-tp-tc-tp-specify-source-file-type.md)。
...
禁用指定的警告编号。 将警告编号放在分号分隔的列表中。 将[/wd\<num >](compiler-option-warning-level.md)设置。

### <a name="forced-include-file"></a>强制包含文件

一个或多个强制包含文件。 将[/fi\<名称 >](fi-name-forced-include-file.md)。

### <a name="forced-using-file"></a>强制 #using 文件

指定一个或多个强制的 #using 文件。 将[/FU\<名称 >](fu-name-forced-hash-using-file.md)。
...
不会在 *`.obj`* 文件中包含默认库名称。 设置[/zl](zl-omit-default-library-name.md)。
