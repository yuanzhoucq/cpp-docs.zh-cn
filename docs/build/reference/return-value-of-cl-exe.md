...
还可使用 Visual Studio 中的错误查找实用工具来了解编译器错误消息的意思。 在 Visual Studio 命令 shell 中，输入**errlook.exe**以启动该实用程序; 或在 Visual Studio IDE 中，在菜单栏上，选择**工具**，**错误查找**。 输入错误值以查找与错误相关的描述性文本。 有关详细信息请参阅[ERRLOOK 参考](errlook-reference.md)。
