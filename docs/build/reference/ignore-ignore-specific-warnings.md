...
默认情况下，链接会报告所有警告。 指定**忽略：** `warning`告知链接器禁止显示特定的警告编号。 若要忽略多个警告，请用逗号分隔警告编号。
...
|LNK4207|`filename`已编译 /Yc /Yu /Z7; 无法创建 PDB; 用 /Zi 重新编译; 正在链接对象如同没有调试信息一样|
