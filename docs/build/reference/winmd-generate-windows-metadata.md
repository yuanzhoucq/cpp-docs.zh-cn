...
**/WINMD**链接器选项使用适用于 UWP 应用和 Windows 运行时组件，以控制创建 Windows 运行时元数据 (.winmd) 文件。 .Winmd 文件是一种类型的 DLL，其中包含元数据，Windows 运行时类型，并在运行时组件，这些类型的实现的情况下。 元数据遵循[ECMA-335](https://www.ecma-international.org/publications/standards/Ecma-335.htm)标准。
