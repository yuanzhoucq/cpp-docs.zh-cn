...
若要将函数作为内联展开的候选项排除，可以使用[__declspec （noinline）](../../cpp/noinline.md)或标记的区域[#pragma auto_inline （off）](../../preprocessor/auto-inline.md) ，并[#pragma auto_inline （on）](../../preprocessor/auto-inline.md)指令。 有关为编译器提供内联提示的另一种方式的信息，请参阅[#pragma 内部](../../preprocessor/intrinsic.md)指令。
