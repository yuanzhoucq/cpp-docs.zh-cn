...
> [!NOTE]
>  大多数情况下，您需要从创建 .net 模块的编译中向链接器传递 .obj 文件。  一个例外是.netmodule 专门创建了[/clr: pure](../../build/reference/clr-common-language-runtime-compilation.md)。  将 .dll 或 .netmodule MSIL 模块文件传递到链接器可能导致 LNK1107。  
...
-   如果你模块将用于构建的托管的库，并且将生成链接器的所有模块输入**/clr: pure**或**/clr: safe**、 用作链接器的模块输入的.obj 文件，并生成.dll （程序集） 或.netmodule （模块），如果你只想要公开库中的托管的类型。 **/clr:pure** 和 **/clr:safe** 编译器选项在 Visual Studio 2015 中已弃用。 如果你想要公开库中的托管的类型，并且如果还想 c + + 应用程序使用库中的本机类型，你的库将包含 （你将还需要每个模块的.h 文件的库组件模块的.obj 文件以便它们可引用使用 #include 源代码中的代码)。  
