...
仅在指定的 NMAKE 生成*目标*或如果未指定，第一个目标中生成文件。 第一个生成文件目标可以是[伪目标](pseudotargets.md)生成其他目标。 NMAKE 使用 /F; 使用指定的生成文件如果未指定 /F，它使用当前目录中的生成文件文件。 如果指定了不到生成文件，它使用推理规则来生成命令行*目标*。
