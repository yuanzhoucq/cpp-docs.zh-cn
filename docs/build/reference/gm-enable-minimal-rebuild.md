...
**/Gm**已弃用。 它可能不会触发生成某些类型的标头文件更改。 从项目中，可以安全地删除此选项。 若要提高生成时间，我们建议使用预编译标头和增量和并行改为生成选项。 有关不推荐使用的编译器选项的列表，请参阅**已弃用并删除的编译器选项**主题中[按类别列出的编译器选项](compiler-options-listed-by-category.md)。
...
由于增量链接器不支持通过使用包含在.obj 文件中的 Windows 元数据[/ZW （Windows 运行时编译）](zw-windows-runtime-compilation.md)选项， **/Gm**选项与不兼容 **/ZW**。
