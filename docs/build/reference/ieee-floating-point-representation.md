...
ms.locfileid: "50661716"
---
# <a name="ieee-floating-point-representation"></a>IEEE 浮点表示形式

Microsoft Visual c + + 是与 IEEE 数值标准一致。 IEEE 754 标准介绍了浮点格式，表示在硬件中的实数的方式。 MSVC 编译器的目标硬件中可表示的浮点数字的至少五个内部格式，但编译器仅使用其中两个。 *单精度*（4 字节） 和*双精度*在 Visual c + + 中使用 （8 字节） 格式。 使用关键字声明单精度**float**。 使用关键字声明双精度**double**。 此外指定了 IEEE 标准*半精度*（2 个字节） 和*四元组精度*（16 字节） 格式，以及*双扩展精度*（10 字节）格式，为实现某些 C 和 c + + 编译器**长双精度型**数据类型。 在 Visual c + + 编译器**长双精度**数据类型会被视为不同的类型，但存储类型映射到**double**。 不存在，但是，内部函数和程序集语言支持的计算使用其他格式，包括双扩展精度 （10 字节） 格式，在支持的硬件。
...
|双精度|登录位、 11 位指数、 52 位有效数|
|双扩展精度|登录位，15 位指数，64 位有效数|
...
|双扩展精度|`SXXXXXXX`|`XXXXXXXX`|`1MMMMMMM`|`MMMMMMMM`|...|`MMMMMMMM`|
