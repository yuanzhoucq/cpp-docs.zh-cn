...
-   #### <a name="to-use-a-listing-to-view-decorated-names"></a>若要使用列表查看修饰名  
    ...
    1.  生成列表，从而编译包含数据、 对象或函数定义或原型的源文件[列出文件类型](../../build/reference/fa-fa-listing-file.md)编译器选项设置为与源代码的程序集 (**/FAs**)。  
  
         例如，输入`cl /c /FAs example.cpp`在开发人员命令提示符下，生成列表文件 example.asm。  
...
-   #### <a name="to-use-dumpbin-to-view-decorated-names"></a>若要使用 DUMPBIN 查看修饰名  
    ...
    1.  若要查看在.obj 或.lib 文件中的导出的符号，输入`dumpbin /symbols``objfile`在开发人员命令提示符。  
  
    2.  若要查找符号的修饰形式，请查找括号中的未修饰名。 修饰的名位于同一行中，管道后 (&#124;) 字符和未修饰名称的前面。  
