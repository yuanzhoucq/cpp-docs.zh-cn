...
利用 **/Zi**不会影响优化。 但是， **/Zi**意味着 **/debug**; 请参阅[/DEBUG （生成调试信息）](debug-generate-debug-info.md)有关详细信息。
...
**/ZI**选项将强制同时[/Gy （启用函数级链接）](gy-enable-function-level-linking.md)并[/FC （完整的源代码文件路径中诊断）](fc-full-path-of-source-code-file-in-diagnostics.md)在编译中使用的选项。

**/ZI**与不兼容[/clr （公共语言运行时编译）](clr-common-language-runtime-compilation.md)。
