...
指定 **/kernel**选项告知编译器和链接器应将裁定哪些语言功能是允许在内核模式下，还要确保有足够的表达力来避免运行时不稳定，这是唯一的内核模式C++。 此目标通过以下方法实现：禁止使用在内核模式中有破坏性的 C++ 语言功能，并针对可能有破坏性但无法禁用的 C++ 语言功能提供警告。
...
1. 在中**其他选项**框中，添加`/kernel`或`/kernel-`。
