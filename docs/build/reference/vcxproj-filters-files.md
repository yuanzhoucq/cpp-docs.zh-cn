...
*筛选器*文件（@no__t .vcxproj）是一个位于根项目文件夹中的 MSBuild 格式的 XML 文件。 它指定哪些文件类型将进入**解决方案资源管理器**的逻辑文件夹。 在下图中， *.cpp*文件位于 "**源文件**" 节点下。 *.h*文件位于 "**头文件**" 节点下， *.ico*和 *.rc*文件位于 "**资源文件**" 下。 此位置由筛选器文件控制。
...
第一次启动项目时，将创建包含 @no__t 1 节点的 `ItemGroup`。 如果要生成自己的 .vcxproj 文件，请确保所有项目项在筛选器文件中也有一个条目。 @No__t-0 节点中的值将基于文件扩展名覆盖默认筛选。 使用 Visual Studio 向项目添加新项时，IDE 将在筛选器文件中添加单个文件项。 如果更改文件扩展名，则不会自动重新分配筛选器。 
...
若要创建嵌套逻辑文件夹，请在筛选器中声明所有节点 `ItemGroup`，如下所示。 每个子节点必须声明最顶层父节点的完整逻辑路径。 在下面的示例中，必须声明空的 `ParentFilter`，因为在以后的节点中引用了该值。
