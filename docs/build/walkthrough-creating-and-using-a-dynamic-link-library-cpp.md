...
![在 Visual Studio 中生成的解决方案](media/mathlibrary-solution-explorer-162.png "在 Visual Studio 中生成的解决方案")
...
1. 在 "**新建项目**" 对话框的左窗格中，选择 **"已安装** > 的 > **Visual C++**  **Windows 桌面**"。 在中心窗格中，选择 "**动态链接库（DLL）** "。 在 "**名称**" 框中输入*MathLibrary*以指定项目的名称。 保留默认**位置**和**解决方案名称**值。 设置用于**创建新解决方案**的**解决方案**。 如果未选中，请选中 "**创建解决方案的目录**"。
...
![在 Visual Studio 中生成的解决方案](media/mathlibrary-solution-explorer-159.png "在 Visual Studio 中生成的解决方案")
...
请注意文件顶部的预处理器语句。 DLL 项目的新项目模板将项目**名称**&#95;导出添加到定义的预处理器宏。 在此示例中，Visual Studio 在生成 MathLibrary DLL 项目时定义 MATHLIBRARY&#95;EXPORTS。
...
若要验证到目前为止是否一切正常，请编译动态链接库。 若要编译，请在菜单栏上选择“生成” > “生成解决方案”。 DLL 和相关的编译器输出放置在一个名为*Debug*的文件夹中。 如果创建发布版本，则会将输出放入名为*Release*的文件夹中。 输出应如下所示：
...
1. 在菜单栏上，选择 "**文件** > " "**新建** > **项目**" 以打开 "新建**项目**" 对话框。
...
此代码可以编译，但不会链接。 如果现在生成客户端应用，"错误列表" 将显示几个 LNK2019 错误。 这是因为您的项目缺少某些信息：尚未指定你的项目尚未依赖于*MathLibrary*库。 而且，您还没有告诉链接器如何查找*MathLibrary*文件。
...
1. 在“附加库目录”对话框的顶部窗格中双击以启用编辑控件。 在编辑控件中，指定指向 MathLibrary.lib 文件位置的路径。 默认情况下，它位于 DLL 解决方案文件夹下名为 "*调试*" 的文件夹中。 如果创建发布版本，则会将该文件放置在一个名为*release*的文件夹中。 您可以使用`$(IntDir)`宏，使链接器可以找到您的 DLL，无论您创建的是哪种版本。 如果已按照指示将客户端项目置于 DLL 项目的单独解决方案中，则相对路径应如下所示：
   ...
   如果 DLL 和客户端项目位于同一解决方案中，则相对路径应如下所示：
