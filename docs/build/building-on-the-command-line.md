...
在 Visual Studio 安装程序中选择一个 C++ 工作负载时，它会安装 Visual Studio 平台工具集。 平台工具集具有适用于特定 Visual C++ Studio 版本的所有 C 和工具。 这些工具包括 C/C++编译器、链接器、组装器以及其他生成工具和匹配库。 您可以在命令行中使用所有这些工具。 它们还由 Visual Studio IDE 在内部使用。 有单独的 x86 托管和 x64 托管的编译器和工具可为 x86、x64、ARM 和 ARM64 目标生成代码。 用于特定主机和目标生成体系结构的每组工具都存储在其自己的目录中。

需要设置几个特定的环境变量，这些工具才能正常运行。 这些变量用于将工具添加到路径，以及设置包括文件、库文件和 SDK 位置。 为了便于设置这些环境变量，安装程序会在安装期间创建自定义的命令文件或批处理文件。 您可以运行以下命令文件之一，设置特定的主机和目标生成体系结构、Windows SDK 版本和平台工具集。 为方便起见，安装程序还会在 "开始" 菜单中创建快捷方式。 快捷方式使用这些命令文件为主机和目标的特定组合启动 "开发人员命令提示" 窗口。 这些快捷键确保所有必需的环境变量已设置好并可供使用。

所需的环境变量特定于你的安装和所选的生成体系结构。 它们还可以通过产品更新或升级来更改。 这就是我们建议你使用已安装的命令提示符快捷方式或命令文件的原因，而不是自行设置环境变量。 有关详细信息，请参阅[为命令行生成设置路径和环境变量](setting-the-path-and-environment-variables-for-command-line-builds.md)。

安装的工具集、命令文件和快捷方式取决于您的计算机处理器以及您在安装过程中选择的选项。 通常会安装 x86 托管工具和构建 x86 和 x64 代码的跨工具。 如果你有64位 Windows，则还会安装用于构建 x86 和 x64 代码的 x64 托管工具和跨工具。 如果选择可选C++的通用 Windows 平台工具，则还将安装用于生成 ARM 和 ARM64 代码的 x86 和 x64 工具。 其他工作负载可能需安装其他工具。
...
"开始" 菜单文件夹和快捷方式名称因 Visual Studio 的已安装版本而异。 如果设置了此项，则它们也依赖于安装**别名**。 例如，假设您安装了 Visual Studio 2019，并为其提供了*最新*的别名。 在名为 " **Visual Studio 2019**" 的文件夹中，"开发人员命令提示" 快捷方式命名为 "**开发人员命令提示" VS 2019 （最新）** "。

::: moniker-end
::: moniker range="= vs-2017"

"开始" 菜单文件夹和快捷方式名称因 Visual Studio 的已安装版本而异。 如果设置了此项，则它们也依赖于安装**别名**。 例如，假设您安装了 Visual Studio 2017，并为其提供了*最新*的别名。 在名为 " **Visual Studio 2017**" 的文件夹中，"开发人员命令提示" 快捷方式命名为 "**开发人员命令提示" VS 2017 （最新）** "。

::: moniker-end
::: moniker range="< vs-2017"

"开始" 菜单文件夹和快捷方式名称因 Visual Studio 的已安装版本而异。 例如，假设您安装了 Visual Studio 2015。 **对于 VS 2015**，开发人员命令提示快捷方式的名称为开发人员命令提示。
...
若要以更快的速度打开开发人员命令提示符，请在桌面搜索框中输入 "*开发人员命令提示*"。 然后选择所需的结果。

## <a name="developer_command_file_locations"></a> 开发人员命令文件位置

如果希望在现有的命令提示符窗口中设置生成环境，可以使用安装程序创建的命令文件之一。 建议你在新的 "命令提示符" 窗口中设置环境。 建议你稍后在同一命令窗口中切换环境。

::: moniker range=">= vs-2019"

命令文件的位置取决于你安装的 Visual Studio 的版本以及在安装过程中所做的选择。 对于 Visual Studio 2019，64位系统上的典型安装位置位于 \\Program Files （x86）\\Microsoft Visual Studio\\2019\\*版*。 *版本*可以是社区、Professional、Enterprise、BuildTools 或你提供的其他昵称。

::: moniker-end
::: moniker range="= vs-2017"

命令文件的位置取决于你安装的 Visual Studio 的版本以及在安装过程中所做的选择。 对于 Visual Studio 2017，64位系统上的典型安装位置位于 \\Program Files （x86）\\Microsoft Visual Studio\\2017\\*版*。 *版本*可以是社区、Professional、Enterprise、BuildTools 或你提供的其他昵称。

::: moniker-end
::: moniker range="< vs-2017"

命令文件位置取决于 Visual Studio 版本和安装目录。 对于 Visual Studio 2015，典型安装位置位于 \\Program Files （x86）\\Microsoft Visual Studio 14.0。

::: moniker-end

主开发人员命令提示命令文件 Vsdevcmd.bat 位于 Common7\\工具子目录中。 如果未指定任何参数，则它会将环境设置为使用 x86 本机工具来生成32位 x86 代码。

::: moniker range=">= vs-2017"

更多命令文件可用于设置特定的生成体系结构。 可用的命令文件取决于已安装的 Visual Studio 工作负荷和选项。 在 Visual Studio 2017 和 Visual Studio 2019 中，你将在 VC\\辅助\\Build 子目录中找到它们。

::: moniker-end
::: moniker range="< vs-2017"

更多命令文件可用于设置特定的生成体系结构。 可用的命令文件取决于已安装的 Visual Studio 工作负荷和选项。 在 Visual Studio 2015 中，它们位于 VC、VC\\bin 或 VC\\bin\\*体系结构*子目录，其中*体系结构*是本机或跨编译器选项之一。
...
在现有命令窗口中指定特定生成体系结构的最简单方法是使用 vcvarsall.bat 文件。 使用 vcvarsall.bat 设置环境变量以配置本机32位或64位编译的命令行。 参数可用于指定 x86、x64、ARM 或 ARM64 处理器的交叉编译。 你可以以 Microsoft Store、通用 Windows 平台或 Windows 桌面平台为目标。 甚至可以指定要使用的 Windows SDK，并选择平台工具集版本。

当与没有参数一起使用时，vcvarsall.bat 将环境变量配置为使用32位 Windows 桌面目标的当前 x86 本机编译器。 可以添加自变量来配置环境，以使用任意本机或跨平台编译器工具。 如果指定的配置在计算机上未安装或不可用，则 vcvarsall.bat 将显示错误消息。
...
architecture<br/>
此可选参数指定要使用的主机和目标体系结构。 如果未指定*体系结构*，则使用默认的生成环境。 支持以下参数：
...
使用 **-vcvars_ver = 14.0**来指定 Visual Studio 2015 编译器工具集。

#### <a name="vcvarsall"></a>在现有的 "命令提示符" 窗口中设置生成环境

1. 在命令提示符处，使用 CD 命令更改为 Visual Studio 安装目录。 然后，再次使用 CD 更改为包含特定于配置的命令文件的子目录。 对于 Visual Studio 2019 和 Visual Studio 2017，请使用*VC\\辅助\\生成*子目录。 对于 Visual Studio 2015，请使用*VC*子目录。

1. 输入用于首选开发人员环境的命令。 例如，若要使用最新的 Windows SDK 和 Visual Studio 编译器工具集在64位平台上构建 UWP 的 ARM 代码，请使用以下命令行：
...
打开 "开发人员命令提示" 快捷方式的 "属性" 对话框，查看使用的命令目标。 例如，“VS 2019 的 x64 本机工具命令提示符”快捷方式的目标如以下所示：
...
打开 "开发人员命令提示" 快捷方式的 "属性" 对话框，查看使用的命令目标。 例如， **VS 2017**快捷方式的 x64 本机工具命令提示目标类似于以下内容：
...
打开 "开发人员命令提示" 快捷方式的 "属性" 对话框，查看使用的命令目标。 例如， **VS2015 x64 本机工具命令提示**快捷方式的目标类似于以下内容：

`%comspec% /k "C:\Program Files (x86)\Microsoft Visual Studio 14.0\VC\vcvarsall.bat" amd64`

::: moniker-end

特定于体系结构的批处理文件设置“architecture”参数并调用 vcvarsall.bat。 您可以将相同的选项传递给这些批处理文件，就像传递到 vcvarsall.bat 一样，也可以直接调用 vcvarsall.bat。 若要为自己的命令快捷方式指定参数，请用双引号将其添加到命令末尾。 例如，下面是使用最新 Windows SDK 在64位平台上构建 UWP 代码的快捷方式。 若要使用较早的编译器工具集，请指定版本号。 在快捷方式中使用类似此命令目标的内容：
...
若要在命令提示符C++下生成 C/项目，Visual Studio 提供了以下命令行工具：
...
使用 MSBuild (msbuild.exe) 和项目文件 (.vcxproj) 配置生成并间接调用该工具集。 它相当于在 Visual Studio IDE 中运行 "**生成**项目" 或 "**生成解决方案**" 命令。 从命令行运行 MSBuild 是一种高级方案，不常建议使用。

[DEVENV](/visualstudio/ide/reference/devenv-command-line-switches)<br/>
将 DEVENV （devenv）与命令行开关（如 **/Build**或 **/Clean** ）结合使用，以在不显示 Visual Studio IDE 的情况下执行某些生成命令。 通常，DEVENV 优先于直接使用 MSBuild，因为可以让 Visual Studio 处理 MSBuild 的复杂性。
...
[演练：在命令行C++上编译本机程序](walkthrough-compiling-a-native-cpp-program-on-the-command-line.md)<br/>
提供一个示例，演示如何在命令行上创建C++和编译程序。

[演练：在命令行上编译 C 程序](walkthrough-compile-a-c-program-on-the-command-line.md)<br/>
介绍了如何编译采用 C 编程语言编写的程序。

[演练：在命令C++行上编译/cli 程序](walkthrough-compiling-a-cpp-cli-program-on-the-command-line.md)<br/>
介绍了如何创建和编译使用 .NET Framework 的 C++/CLI 程序。

[演练：在命令C++行上编译/cx 程序](walkthrough-compiling-a-cpp-cx-program-on-the-command-line.md)<br/>
...
如何将环境变量设置为使用32位或64位工具集来面向 x86、x64、ARM 和 ARM64 平台。
