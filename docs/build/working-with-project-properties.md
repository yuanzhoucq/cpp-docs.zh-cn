...
在 IDE 中，生成项目的需要的全部信息都公开为属性。 此信息包括应用程序名称、扩展名（如 DLL、LIB、EXE）、编译器选项、链接器选项、调试器设置、自定义生成步骤和许多其他操作。 通常，使用*属性页*可以查看和修改这些属性。 若要访问属性页，请从主菜单中选择 "**项目** > 项目 **_名称_** " "属性"，或者右键单击**解决方案资源管理器**中的项目节点，然后选择 "**属性**"。
...
![Visual C&#43; &#43;项目默认值](media/visual-c---project-defaults.png "视觉C++对象默认值")
...
![显示活动&#43; &#43;配置的 Visual C 属性页](media/visual-c---property-pages-showing-active-configuration.png "显示C++活动配置的视觉对象属性页")

下图显示相同的项目属性页，但该配置已更改为发布。 请注意“优化”属性的不同值。 另请注意，活动配置仍是调试。 可以在此处设置任何配置的属性；它不必处于活动状态。

![显示 release&#43; &#43; config 的 Visual C 属性页](media/visual-c---property-pages-showing-release-config.png "显示C++ release config 的视觉对象属性页")
...
下图显示了 Visual Studio C++项目的属性页。 在左窗格中，选择 " **VC + + 目录**"*规则*，右窗格会列出与该规则相关联的属性。 `$(...)` 值称为*宏*。 宏是编译时常量，是指由 Visual Studio 或 MSBuild 系统定义的值，或是由用户定义的值。 通过使用宏（而不是硬编码值，例如目录路径），可以更轻松地在计算机之间以及 Visual Studio 的版本之间共享属性设置，并且可更好地确保项目设置正确地参与[属性继承](project-property-inheritance.md)。
...
项宏<br/>
具有语法 `%(name)`。 对于文件来说，仅适用于该文件的项宏，例如可以使用 `%(AdditionalIncludeDirectories)` 来指定仅适用于特定文件的包含目录。 这种项宏与 MSBuild 中的 `ItemGroup` 元数据相对应。 在项目配置中使用时，项宏适用于特定类型的所有文件。 例如，C/C++“预处理器定义”配置属性可以采用适用于项目中所有 .cpp 文件的  **项宏**`%(PreprocessorDefinitions)`。 这种项宏与 MSBuild 中的 `ItemDefinitionGroup` 元数据相对应。 有关详细信息，请参阅[项定义](/visualstudio/msbuild/item-definitions)。
...
1. 打开 "**属性管理器**" 窗口。 （在菜单栏上，选择 "**查看** > 属性管理器**或** > **其他 Windows** > **属性管理器**"。） **Property Manager**打开属性表的快捷菜单（名称以 "用户" 结尾），然后选择 "**属性**"。 此时将打开该属性表的“属性页”对话框。
...
![属性&#95;编辑器&#95;下拉列表](media/property_editor_dropdown.png "属性_编辑器_下拉列表")
