...
1. 如果清单嵌入在二进制文件中，请确保 RT_MANIFEST 的 ID 对于此类型的二进制文件是正确的。 若要使用的资源 id 的详细信息，请参阅[作为资源 (Windows) 使用的并行程序集](/windows/desktop/SbsCs/using-side-by-side-assemblies-as-a-resource)。 如果清单位于单独文件中，请在 XML 编辑器或文本编辑器中打开它。 有关清单和部署规则的详细信息，请参阅[清单](https://msdn.microsoft.com/library/aa375365)。
