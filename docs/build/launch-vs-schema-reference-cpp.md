...
|`projectTarget`|string|指定在生成`project`时调用的可选目标。 该`projectTarget`名称必须已经存在, 并与**调试目标**下拉列表中的名称匹配。|
|`debugType`|string|根据代码类型 (本机、托管或混合) 指定调试模式。 如果未设置此参数, 则会自动检测到此参数。 允许的值: "本机"、"托管"、"mixed"。|
|`inheritEnvironments`|array|指定从多个源继承的一组环境变量。 可以在文件中定义一些变量 (如 CMakeSettings 或 Cppproperties.json), 并使它们可用于调试上下文|
|`args`|array|指定传递给已启动程序的命令行参数。|
|`currentDir`|string|指定生成目标的完整目录路径。 如果未设置此参数, 则会自动检测到此参数。|
|`noDebug`|boolean|指定是否调试已启动的程序。 如果未指定此参数, 则`false`此参数的默认值为。|
...
|`buildConfigurations`|array| 指定应用配置的生成模式的名称的键值对。 例如, `Debug`或`Release`和要根据所选生成模式使用的配置。
...
|-|-|-|
|**Property**|类型|**说明**|
|`program`|string|远程计算机上的程序可执行文件的完整路径。 当使用 CMake 时, 可以`${debugInfo.fullTargetPath}`将宏用作此字段的值。|
|`processId`|integer|要将调试器附加到的可选进程 ID。|
|`sourceFileMap`|对象 (object)|传递给调试引擎的可选源文件映射。 格式: `{ "\<Compiler source location>": "\<Editor source location>" }`或`{ "\<Compiler source location>": { "editorPath": "\<Editor source location>", "useForBreakpoints": true } }`。 例如: `{ "/home/user/foo": "C:\\foo" }`或`{ "/home/user/foo": { "editorPath": "c:\\foo", "useForBreakpoints": true } }`。 请参阅[源文件映射选项](#source_file_map_options)。|
...
|`remoteMachineName`|string|托管 gdb 的远程 Linux 计算机和要调试的程序。 使用连接管理器添加新的 Linux 计算机。 当使用 CMake 时, 可以`${debugInfo.remoteMachineName}`将宏用作此字段的值。|
|`cwd`|string|远程计算机上的目标的工作目录。 当使用 CMake 时, 可以`${debugInfo.defaultWorkingDirectory}`将宏用作此字段的值。 除非在*cmakelists.txt*文件中重写, 否则默认值为远程工作区根目录。|
...
externalConsole|boolean|如果为 true, 则为调试对象启动控制台。 如果`false`为, 则不会启动任何控制台。 默认为 `false`。 注意：出于技术原因, 在某些情况下将忽略此选项。|
...
|`quoteArgs`|boolean|如果各个参数包含字符 (如空格或制表符), 是否应将其括起来？ 如果`false`为, 则调试器命令将不再自动加引号。 默认值为 `true`。|
...
|`useForBreakpoints`|boolean|设置断点时, 应使用此源映射。 如果`false`为, 则仅使用文件名和行号来设置断点。 如果`true`为, 则在使用此源映射时, 将使用文件和行号的完整路径来设置断点。 否则, 在设置断点时, 只会使用文件名和行号。 默认值为 `true`。|
