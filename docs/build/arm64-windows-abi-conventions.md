...
1. 如果参数为复合类型则会将参数复制到内存中的调整后的 nsaa 处。 NSAA 按参数的大小递增。 现在已分配参数。
...
Windows 中的代码编译的启用帧指针 ([/Oy-](reference/oy-frame-pointer-omission.md)) 以实现快速审核堆栈。 此结论就是 x29 (fp) 通常指向链是 {fp，lr} 中的下一个链接对，该值指示在前一个帧上堆栈和寄信人地址的指针。 鼓励第三方代码，以便改进分析和跟踪也启用帧指针。
