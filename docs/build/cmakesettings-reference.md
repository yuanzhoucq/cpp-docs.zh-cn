...
- `cacheGenerationCommand`：指定命令行工具和参数，例如*gencache 调试*以生成缓存。 当用户显式请求重新生成或修改 Cmakelists.txt 或 CMakeSettings 文件时，将从指定环境中的 shell 运行该命令，以进行配置。
- `cacheRoot`：指定 CMake 缓存的路径。 此目录应包含现有的*cmakecache.txt*文件。
- `clangTidyChecks`：以逗号分隔的警告列表，将传递到 clang;允许使用通配符，并且 "-" 前缀将删除检查。
...
- `remoteCopyAdditionalIncludeDirectories`：要从远程计算机复制的其他包含目录以支持 IntelliSense。 格式为 "/path1;/path2..."。
- `remoteCopyExcludeDirectories`：包括不从远程计算机复制的目录。 格式为 "/path1;/path2..."。
- `remoteCopyUseCompilerDefaults`：指定是否使用编译器的默认值定义和包含 IntelliSense 的路径。 仅当使用的编译器不支持 gcc 样式的参数时，才应为 false。
...
- `variables`：包含 CMake 变量的名称-值对，这些变量将作为 **-D** *_名称_传递=_值_* 为 CMake。 如果你的 CMake 项目生成说明将任何变量直接添加到*cmakecache.txt*文件，则建议你将其添加到此处。 下面的示例演示如何为 14.14.26428 MSVC 工具集指定名称/值对：
