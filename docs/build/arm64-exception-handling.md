...
1. Prolog 和 epilogs 趋向于彼此镜像。 利用这种常见的特性，描述展开所需的元数据大小会大幅降低。 在函数体中，无论是否撤消序言操作，都不重要，或者是否以向前的方式完成 epilog 的操作。 这两种操作应产生完全相同的结果。
...
![包含打包的展开数据的 pdata 记录](media/arm64-exception-handling-packed-unwind-data.png "包含打包的展开数据的 pdata 记录")
