...
|字偏移量|位|用途|  
|-----------------|----------|-------------|  
|0|0-31|*正常启动 RVA*是函数的开始的 32 位 RVA。 如果该函数包含 Thumb 代码，则必须设置该地址的低位。|  
|1|0-1|*标志*是 2 位字段，该值指示如何解释第二个.pdata 字的剩余 30 位。 如果*标志*为 0，则剩余的位将形成*异常信息 RVA* (具有低两位隐式 0)。 如果*标志*为非零，则剩余的位将形成*已打包展开数据*结构。|  
|1|2-31|*异常信息 RVA*或*已打包展开数据*。<br /><br /> *异常信息 RVA*是存储在.xdata 部分中的长度可变的异常信息结构的地址。 此数据必须是对齐的 4 字节。<br /><br /> *已打包展开数据*是从函数，采用规范格式展开所需的操作的概括的说明。 在这种情况下，不需要任何 .xdata 记录。|  
...
|-----------------|----------|-------------|  
|0|0-31|*正常启动 RVA*是函数的开始的 32 位 RVA。 如果该函数包含 Thumb 代码，则必须设置该地址的低位。|  
|1|0-1|*标志*是具有以下含义的一个 2 位字段：<br /><br /> -00 = 已打包展开数据未使用;剩余的位指向.xdata 记录。<br />-01 = 已打包展开数据。<br />-10 = 已打包展开其中假定函数没有序言的数据。 这对于描述未与函数开始位置进行连接的函数片段很有用。<br />-11 = 保留。|  
|1|2-12|*函数长度*是一个 11 位字段，提供以字节为单位除以 2 的整个函数的长度。 如果函数的大小大于 4K 字节，则必须改用一个完整的 .xdata 记录。|  
|1|13-14|*Ret*是 2 位字段，该值指示函数如何返回：<br /><br /> -00 = 通过 pop {pc} 返回 ( *L*标志位必须在此情况下设置为 1)。<br />-01 = 通过使用 16 位分支返回。<br />-10 = 通过使用 32 位分支返回。<br />-11 根本 = 没有任何尾声。 这对于描述一个不连续的函数片段很有用，该片段只能包含一个序言，然而其尾声位于其他位置。|  
|1|15|*H*是一个 1 位标志，指示是否函数"寻址"整数参数按将其推送在函数中，开始注册 (r0 r3)，并在返回之前释放 16 字节的堆栈。 （0 = 不会对寄存器进行寻址，1 = 对寄存器进行寻址。）|  
|1|16-18|*Reg* 3 位字段，它指示的最后一个索引保存非易失寄存器。 如果*R*位为 0，则仅整数寄存器正在保存，并且假定 r4 rN，其中 N 等于 4 的范围 + *Reg*。如果*R*位为 1，则仅浮点寄存器正在保存，并且假定范围内 d8 dN，其中 N 等于 8 + *Reg*。特殊组合*R* = 1 和*Reg* = 7 指示未保存任何寄存器。|  
|1|19|*R*是 1 位标志，它指示是否已保存的非易失寄存器是整数寄存器 (0) 还是浮点寄存器 (1)。 如果*R*设置为 1 和*Reg*字段设置为 7，则已推送任何非易失性寄存器。|  
|1|20|*L*是 1 位标志，它指示函数是否保存/还原 LR，以及通过指示的其他寄存器*Reg*字段。 （0 = 不保存/还原，1 = 保存/还原。）|  
|1|21|*C*是 1 位标志，它指示函数是否包含额外的指令，若要设置帧链以加快堆栈审核 (1)，或不 (0)。 如果设置了此位，则会将 r11 隐式地添加到已保存的整数非易失寄存器列表中。 (请参阅以下如果限制*C*使用标志。)|  
|1|22-31|*堆栈调整*是 10 位字段，该值指示的堆栈的分配针对此函数，除以 4 的字节数。 但是，只能直接对 0x000-0x3F3 之间的值进行编码。 分配超过 4044 字节的堆栈的函数必须使用完整的 .xdata 记录。 如果*堆栈调整*字段大于 0x3F4 或更大，则低 4 位具有特殊含义：<br /><br /> -位 0-1 指示堆栈调整 (1-4) 减 1 的单词数。<br />如果序言此调整合并到其推送操作，位 2 设置为 1。<br />如果尾声此调整合并到其弹出操作位 3 设置为 1。|  
...
2.  异常数据之后 (如果*E*标头中的位设置为 0) 是一份有关尾声范围都将被打包为一个字，并递增起始偏移量的顺序存储的信息。 每个范围都将包含以下字段：  
...
|FF||||-|end|  
 ...
 这显示了展开代码中的每个字节的十六进制值的范围*代码*，以及操作码大小*Opsize*和相应的原始指令解释。 空单元格指示较短的展开代码。 在具有涵盖多个字节的较大值的指令中，将首先存储最高有效位。 *Opsize*字段显示与每个 thumb-2 操作关联的隐式操作码大小。 具有不同编码的表格中明显重复的项将用来区分不同的操作码大小。  
