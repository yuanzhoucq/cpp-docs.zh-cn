...
- 在将移动 /clr 时： 纯编译为 /clr，删除[/ENTRY](../../build/reference/entry-entry-point-symbol.md)从链接器行的选项。 这提供 CRT 初始化，并允许应用程序启动时执行的静态初始值设定项。 **/Clr: pure**编译器选项是在 Visual Studio 2015 中已过时，并在 Visual Studio 2017 中不支持。
