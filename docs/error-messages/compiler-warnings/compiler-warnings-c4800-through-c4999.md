...
|[编译器警告（等级4） C4820](compiler-warning-level-4-c4820.md)|在构造 "*member_name*" 后添加了 "*bytes*" 字节填充|
...
|[编译器警告（等级1） C4838](compiler-warning-level-1-c4838.md)|从 "*type_1*" 转换到 "*type_2*" 需要收缩转换|
...
| 编译器警告（等级4） C4845 | 如果未在命令行上指定 "/d1initall\[0\|1\|2\|3]"，则忽略 "\_\_declspec （不\_初始化\_全部）" |
| 编译器警告（等级4） C4846 | "*value*" 不是 "/d1initall" 的有效参数：忽略了命令行标志 |
| 编译器警告（等级4） C4847 | "\_\_declspec （不\_全部初始化\_）" 只能应用于函数、类类型或局部变量：已忽略 |
| 编译器警告（等级1） C4848 | c + + 17 及更早版本中的标准特性 "no\_唯一\_地址" 是供应商扩展 |
|[编译器警告（等级4） C4866](c4866.md)| 对于调用*operator_name* ，编译器可能不会强制执行从左到右的计算顺序|
...
|编译器警告（等级2） C4872|编译以下位置的 concurrency： arallel_for_each:p 的调用关系图时，检测到浮点数被零除： "*location*"|
...
|编译器警告（等级1） C4882|已弃用将函子与非常量调用运算符一起传递给 concurrency：:p arallel_for_each|
...
|[编译器警告（等级1） C4920](compiler-warning-level-1-c4920.md)|枚举枚举成员 member_1 = value_1 在枚举枚举中已被视为 member_2 = value_2|
...
|[编译器警告（等级4） C4932](compiler-warning-level-4-c4932.md)|不区分 __identifier （*标识符*）和 \__identifier （*标识符*）|
|编译器警告（等级1） C4934|"__delegate （多播）" 已弃用，请改用 "\__delegate"|
...
|[编译器警告（等级4） C4938](compiler-warning-level-4-c4938.md)|"*var*"：浮点缩减变量可能会导致 "/fp： strict" 或 "#pragma" 下的结果不一致 fenv_access|
...
|编译器警告（等级1） C4966|"*function*" 具有不受支持的段名称 __code_seg 批注，已忽略批注|
|编译器警告 C4970|委托构造函数：由于 "*type*" 是静态的，因此忽略了目标对象|
|编译器警告（等级1） C4971|参数顺序： \<目标对象 >，已不推荐使用委托构造函数的 \<目标函数 >，\<目标 object = "" >\<|
...
|[编译器警告（等级4） C4985](compiler-warning-level-4-c4985.md)|"*symbol_name*"：以前的声明上不存在特性。|
...
| [编译器警告（等级2） C5046](c5046.md) | "*function*"：未定义包含带有内部链接的类型的符号 |
| 编译器警告（等级1） C5047 | 如果不支持模块存在\_，则使用非标准 \_\_ |
...
| 编译器警告（等级1） C5100 | \_\_VA\_参数 \_\_保留用于可变参数宏 |
...
| 编译器警告（等级1） C5104 | 在宏替换列表中找到 "*string1*#*string2*"，是否表示 "*string1*" "#*string2*"？ |
