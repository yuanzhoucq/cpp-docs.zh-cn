...
没有重要的用例时可以针对发出此警告C++。 如果要声明相同的类型具有相同名称在两个不同的文件，如果这些声明包含虚函数，并声明不相同，编译器可以发出警告 C4744 虚函数表。 警告是因为有两个不同大小的虚拟函数的表相同的类型，并且链接器必须选择其中一个将合并到可执行文件。  有可能，这可能会导致错误的虚拟函数调用程序。
