...
ms.locfileid: "50516514"
---
# <a name="compiler-warning-level-4-c4233"></a>编译器警告（等级 4）C4233

> 使用了非标准扩展:*关键字*关键字仅在 c + +，C 中不受支持

编译器将源代码编译为 C，而不是 c + +，而你使用才有效 c + + 中的关键字。 编译器会将源文件编译为 C 如果源代码文件的扩展名为.c 或使用[/Tc](../../build/reference/tc-tp-tc-tp-specify-source-file-type.md)。
