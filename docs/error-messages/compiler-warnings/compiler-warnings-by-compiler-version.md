...
|||
|-|-|
C4843|*type1*： 对数组或函数类型的引用异常处理程序无法访问，请使用*type2*改为
C4844|导出模块*模块名*; 现在是声明模块接口的首选的语法
C5039|*函数*： 指针或对可能引发函数引用传递给下-EHc extern C 函数。 如果此函数将引发异常，则可能出现未定义的行为。
C5040|动态异常规范是有效的仅在 C + + 14 及更早版本;将视为 noexcept （false）
C5041|*定义*: constexpr 静态数据成员的超行定义，则不需要而且在 C + + 17 中已弃用
...
C4474|*函数*： 为格式字符串中传递的参数太多
C4475|*函数*： 长度修饰符*修饰符*不能用于类型字段字符*字符*格式说明符中
C4476|*函数*': 未知的类型字段字符*字符*格式说明符中
C4477|*函数*： 格式字符串*字符串*要求类型自变量*类型*，但可变参数自变量*数*具有类型*类型*
C4478|*函数*： 位置和非位置占位符不能混合在同一格式字符串
...
C4371|*声明*： 类的布局可能已更改从早期版本的编译器由于更好地封装成员*成员*
