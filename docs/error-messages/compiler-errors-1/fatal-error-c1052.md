...
若要在命令行上指定的编译器生成的 PDB 文件名称，请使用[/Fd](../../build/reference/fd-program-database-file-name.md)编译器选项。 若要在 IDE 中指定的编译器生成的 PDB 文件名称，打开**属性页**用于你的项目，并在对话框**配置属性**， **C /C++**，**输出文件**页上，将**程序数据库文件名**属性。 默认情况下，此属性是`$(IntDir)vc$(PlatformToolsetVersion).pdb`。
