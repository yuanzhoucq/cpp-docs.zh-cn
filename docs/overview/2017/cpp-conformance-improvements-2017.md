...
在 Visual Studio 2017 版本 15.9 中，代码会引发此错误：错误 C3615: constexpr 函数 "S<int>::f" 不会生成常数表达式。注意: 对未定义的函数或为未声明为 "constexpr" 的函数的调用导致了故障。注意: 请参阅 "g" 的用法。 要避免该错误，请从函数 f() 的显式实例化中删除 `constexpr` 限定符。 
