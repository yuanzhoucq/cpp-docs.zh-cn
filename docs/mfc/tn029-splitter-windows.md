...
## <a name="terminology-used-by-implementation"></a>实现使用的术语  
 ...
 `CSplitterWnd`：  
 提供拆分窗格的控件和所有窗格对行或列之间共享的滚动条的窗口。 从零开始的数字与指定行和列 (在第一个窗格中，行 = 0 且列 = 0)。  
  
 窗格中：  
 特定于应用程序窗口的`CSplitterWnd`管理。 一个窗格，通常是一个对象，派生自[CView 类](../mfc/reference/cview-class.md)，但可以是任何[CWnd](../mfc/reference/cwnd-class.md)对象都有相应的子窗口 id。  
 ...
 拆分器中：  
 动态中的控件`CSplitterWnd`可用来创建新行或列的窗格。 它位于顶部或左侧的水平滚动条的垂直滚动条。  
