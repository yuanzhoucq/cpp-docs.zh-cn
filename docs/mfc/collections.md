...
本主题的后面部分简要分别介绍了所提供的三种集合形式。 要比较形状的特征，以帮助您确定哪个最适合您的程序，请参阅[选择集合类的建议](../mfc/recommendations-for-choosing-a-collection-class.md)。
...
若要实现包含任何类型的对象的类型安全的集合，最简单的方法是使用其中一个基于 MFC 模板的类。 有关这些类的示例，请参阅 MFC 示例[COLLECT](../overview/visual-cpp-samples.md)。
...
["选择集合类建议"](../mfc/recommendations-for-choosing-a-collection-class.md)中的 MFC 集合类表的特征根据这些特征（形状以外的）描述 MFC 集合类：
...
- [选择集合类的建议](../mfc/recommendations-for-choosing-a-collection-class.md)

- [如何：创建类型安全集合](../mfc/how-to-make-a-type-safe-collection.md)

- [创建堆栈和队列集合](../mfc/creating-stack-and-queue-collections.md)

- [CArray：：添加](../mfc/reference/carray-class.md#add)
