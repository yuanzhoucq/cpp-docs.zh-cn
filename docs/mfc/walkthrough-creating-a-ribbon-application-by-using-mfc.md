...
2.  首先，向功能区添加自定义的类别，通过双击**类别**中**工具箱**。  
     ...
     右键单击**Category1** ，然后单击**属性**。 在**属性**窗口中，更改**标题**到*自定义*。  
...
3.  接下来，自定义面板。 面板用于对逻辑上相关的项进行分组。 例如，在**主页**选项卡的此应用程序，**剪切**，**复制**，和**粘贴**命令都位于**剪贴板**面板。 要自定义面板，请右键单击**Panel1** ，然后单击**属性**。 在**属性**窗口中，更改**标题**到*收藏夹*。  
...
2.  首先，添加**打印**按钮。 **打印**按钮都将具有包含子菜单**快速打印**命令通过使用默认打印机进行打印。 已为此应用程序定义所有这些命令。 它们位于应用程序菜单上。  
     ...
     在**属性**窗口中，更改**ID**属性**ID_FILE_PRINT**，应已定义的。 更改**标题**到*打印*。 更改**图像索引**到*4*。  
  
     若要创建**快速打印**菜单上，单击属性值列旁边**菜单项**，然后单击省略号 (**...**).在**项编辑器**，单击未标记**添加**按钮创建菜单项。 在**属性**窗口中，更改**标题**到*快速打印*， **ID**到*ID_FILE_PRINT_DIRECT*，和**映像**到*5*。 图像属性指定 IDB_FILESMALL 位图资源中的“快速打印”图标。  
