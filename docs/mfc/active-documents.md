...
活动文档可以创建一种或多种类型的数据[视图](#requirements_for_view_objects)（例如，普通、大纲、页面布局等）。 视图的行为像通过其可查看数据的筛选器。 即使文档只有一种类型的视图，你仍可以支持多个视图，作为支持新窗口功能的方式（例如，Office 应用程序中 "**窗口**" 菜单上的 "**新建窗口**" 项）。
...
- 支持一个或多个视图，每个视图均能就地激活。 也就是说，视图必须支持接口 `IOleDocumentView` `IOleInPlaceObject` 和接口以及 `IOleInPlaceActiveObject` （使用容器 `IOleInPlaceSite` 和 `IOleInPlaceFrame` 接口）。
...
视图对象也可以是命令目标。 通过实现 `IOleCommandTarget` 视图可接收源自容器的用户界面的命令（如 "**新建**"、"**打开**"、 **"另存为**"、"**文件**" 菜单上的 "**打印**" 和 "**编辑**" 菜单上的 "**复制**"、"**粘贴**"、"**撤消**"）。 有关详细信息，请参阅[消息处理和命令目标](message-handling-and-command-targets.md)。
