...
若要为标题项提供拖放支持，请指定 HDS_DRAGDROP 样式。 标题项的拖放支持使用户能够对标题控件的标题项进行重新排序。 默认行为将提供要拖动的标题项的半透明化拖动图像和新位置的可视指示器（如果已拖动标题项）。  
 ...
 为常见的拖放功能，在处理 HDN_BEGINDRAG 和 HDN_ENDDRAG 通知，可以将扩展默认的拖放行为。 此外可以自拖动图像的外观定义通过重写[cheaderctrl:: Createdragimage](../mfc/reference/cheaderctrl-class.md#createdragimage)成员函数。  
