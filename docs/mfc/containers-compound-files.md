...
- `LockBytes`对象表示存储对象与物理硬件之间的接口。 它们确定如何将实际的字节写入到该对象访问的`LockBytes`任何存储设备, 例如硬盘或全局内存区域。 有关`LockBytes` 对象`ILockBytes`和接口的详细信息, 请参阅*OLE 程序员参考*。
...
###  <a name="_core_incremental_access_to_files"></a>增量访问文件
...
###  <a name="_core_standardization"></a>实现
...
##  <a name="_core_using_compound_files_format_for_your_data"></a>对数据使用复合文件格式
