...
如果控件使用共享数据（如静态成员变量），则应使用临界区保护对该数据的访问，以防止多个线程同时修改数据。 若要为此目的设置关键部分，请在控件的类中声明类 `CCriticalSection` 的静态成员变量。 无论代码访问共享数据，都应使用此临界区对象的 `Lock` 和 `Unlock` 成员函数。
...
然后，可以通过临界区保护对 `_strShared` 静态成员的访问：
...
支持单元模型线程的控件应通过将命名值 "ThreadingModel" 中的值指定为 "单元"，并将其类 id 注册表项中的值指定为 "单元" *，\\* **InprocServer32**键。 若要使此密钥自动注册到你的控件，请将第六个参数中的*afxregapartmentthreading 改*标志传递给 `AfxOleRegisterControlClass`：
