...
|了解即使在不添加您自己的任何一行代码的情况下，框架和 MFC 应用程序向导也能提供的内容。|生成主干应用程序并在 Visual C++ 中运行它。|正在运行的主干应用程序派生了很多标准**文件**，**编辑**，**视图**，和**帮助**框架中的菜单命令。 对于 MDI 应用程序，您还将获得一个完整的功能性 Windows 菜单，而框架将管理 MDI 子窗口的创建、排列和析构。|  
