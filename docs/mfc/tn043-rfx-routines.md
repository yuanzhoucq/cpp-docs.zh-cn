...
- `MarkForAddNew` -如果值已更改，因为 AddNew 调用，脏页标记

- `MarkForUpdate` -如果值已更改，因为编辑调用，脏页标记

- `Name` -追加标记为已更新的字段的字段名称

- `NameValue` -追加"\<列名称 > ="标记为已更新的字段
...
`RFX_Long` 和`RFX_Int`： 这些是最简单的 RFX 函数。 数据值不需要任何特殊的解释，并且为固定的数据大小。

`RFX_Single` 和`RFX_Double`： 如 RFX_Long RFX_Int 上述，这些函数会简单和可以进行广泛使用的默认实现。 它们存储在 dbflt.cpp 而不是 dbrfx.cpp，但是，若要启用加载运行时仅当在显式引用时浮点库。

`RFX_Text` 和`RFX_Binary`： 这两个函数预分配静态缓冲区以保存字符串/二进制文件的信息，并必须使用 ODBC SQLBindCol 注册这些缓冲区而不是注册 （&） 值。 因此，这两个函数具有大量的特殊情况代码。

`RFX_Date`: ODBC 返回自己 TIMESTAMP_STRUCT 数据结构中的日期和时间信息。 此函数动态分配 TIMESTAMP_STRUCT 作为代理用于发送和接收日期时间数据。 各种操作必须使用 c + + 之间传输的日期和时间信息`CTime`对象和 TIMESTAMP_STRUCT 代理。 这将增加复杂性此函数有相当大，但它是如何使用代理服务器进行数据转换的一个很好示例。

`RFX_LongBinary`： 这是唯一的类库不使用列绑定来接收和发送数据的 RFX 函数。 此函数将忽略 BindFieldToColumn 操作而是在修正操作时，分配的存储空间来容纳传入的 SQL_LONGVARCHAR 或 SQL_LONGVARBINARY 数据，然后执行 SQLGetData 呼叫将检索到的已分配存储的值。 在准备将发送回数据源 （如名称和值的操作） 的数据值时，此函数将使用 ODBC 的 DATA_AT_EXEC 功能。 请参阅[技术说明 45](../mfc/tn045-mfc-database-support-for-long-varchar-varbinary.md)有关使用 SQL_LONGVARBINARY 和 SQL_LONGVARCHARs 的详细信息。
