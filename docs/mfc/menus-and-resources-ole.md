...
|IDR_\<项目 > 类型|如果已打开的文件在 MDI 应用程序中使用。 在应用程序以独立模式运行时使用。 这是在非 OLE 应用程序中使用的标准菜单。|
|IDR_\<项目 > TYPE_SRVR_IP|在位置中打开对象时使用的服务器或容器。|
|IDR_\<项目 > TYPE_SRVR_EMB|如果不使用就地激活的情况下打开对象服务器应用程序使用。|
