...
3.  在 `CFindComboBox` 类中，重写 `PreTranslateMessage` 虚方法。 此方法使组合框，以处理[WM_KEYDOWN](/windows/desktop/inputdev/wm-keydown)消息。 如果用户点击 Esc 键 (`VK_ESCAPE`)，则将焦点返回到主框架窗口。 如果用户点击 Enter 键 (`VK_ENTER`)，发布到主框架窗口包含的 WM_COMMAND 消息`ID_EDIT_FIND_COMBO`命令 id。  
