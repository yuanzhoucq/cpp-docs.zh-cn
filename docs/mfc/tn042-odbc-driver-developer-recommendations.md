...
## <a name="odbcs-cursor-library"></a>ODBC 的游标库  
 ...
 例如，大多数 1.0 驱动程序不支持向后滚动。 游标库可以检测到这种，和将缓存的驱动程序中的行并将它们呈现根据请求中的 FETCH_PREV 调用`SQLExtendedFetch`。  
...
## <a name="begintrans-committrans-rollback"></a>BeginTrans，CommitTrans，回滚  
 `SQLSetConnectOption SQL_AUTOCOMMIT` 和`SQLTransact SQL_COMMIT`， **SQL_ROLLBACK**和**SQL_AUTOCOMMIT**事务请求都将会调用。  
  
## <a name="crecordsets"></a>CRecordsets  
 `SQLAllocStmt``SQLPrepare`， `SQLExecute` (有关`Open`和`Requery`)， `SQLExecDirect` （对于更新操作），`SQLFreeStmt`必须支持。 `SQLNumResultCols` 和`SQLDescribeCol`将对结果集在不同时间调用。  
...
## <a name="updating"></a>Updating  
 ...
 `SQLRowCount` 执行验证只有 1 记录已由更新后，将调用`SQLExecDirect`。  
  
## <a name="forwardonly-cursors"></a>ForwardOnly 游标  
 仅`SQLFetch`需要`Move`操作。 请注意， **forwardOnly**游标不支持更新。  
