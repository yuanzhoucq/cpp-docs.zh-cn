...
>  对于给定的控件，你，如果您愿意，可以用值的属性和最多的控件属性的一个成员变量的多个成员变量。 你可以映射到控件，因为多个对象附加到控件或任何其他窗口时，会导致多义性的消息映射中只有一个 MFC 对象。  
 ...
 此处的成员变量`m_Checkbox`提供成员函数相同的目的`GetMyCheckbox`中所示[控件没有代码向导对类型安全的访问](../mfc/type-safe-access-to-controls-without-code-wizards.md)。 如果复选框不是自动复选框，你仍需要处理程序的对话框类中**BN_CLICKED**时单击该按钮的控件通知消息。  
