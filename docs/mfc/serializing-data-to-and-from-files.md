...
##  <a name="_core_the_document.92.s_role_in_serialization"></a> 序列化中的文档的角色  
 框架自动响应文件菜单打开、 保存，并将另存为命令通过调用文档的`Serialize`成员函数，如果它实现的。 ID_FILE_OPEN 命令，例如，在应用程序对象调用处理程序函数。 在此过程中，用户将看到并响应文件打开的对话框中，并 framework 获取用户选择的文件名。 框架创建`CArchive`对象设置为将数据加载到文档并将传递到存档`Serialize`。 框架已打开该文件。 在文档中的代码`Serialize`成员函数将读取通过存档，根据需要重新构造数据对象中的数据。 有关序列化的详细信息，请参阅文章[序列化](../mfc/serialization-in-mfc.md)。  
