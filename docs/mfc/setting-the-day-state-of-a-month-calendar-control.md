...
1.  使用属性窗口中，添加到 MCN_GETDAYSTATE 消息通知处理程序*m_monthcal*对象 (请参阅[消息映射到函数](../mfc/reference/mapping-messages-to-functions.md))。  
  
2.  在处理程序主体中，添加以下代码：  
  
     [!code-cpp[NVC_MFCControlLadenDialog#26](../mfc/codesnippet/cpp/setting-the-day-state-of-a-month-calendar-control_1.cpp)]  
  
     该示例将转换*pNMHDR*指向正确的类型，然后确定请求多少个月的信息 (`pDayState->cDayState`)。 对于每个月，当前位域 (`pDayState->prgDayState[i]`) 都会初始化为零，然后设置所需的日期（在本例中为每月的第 15 天）。  
