...
> [!NOTE]
>  由于就地激活仅适用于嵌入的（而非链接的）项，缩放也仅适用于嵌入对象。 您将在用于缩放的 `COleServerDoc` 和 `COleServerItem` 中看到 API。 此分歧的原因是工作项链接和嵌入项有效的函数位于`COleServerItem`（这可以具有常规实现） 和仅对嵌入对象有效的函数位于`COleServerDoc`类 (从服务器的角度来看，它是**文档**在嵌入)。  
...
## <a name="mfc-support-for-in-place-resizing"></a>MFC 对就地调整大小的支持  
 ...
 您可以实施自定义调整大小并仍利用提供的用户界面`COleResizeBar`通过重写中的 WM_SIZECHILD 消息你`COleIPFrameWnd`类。 有关特定平台 WM_SIZECHILD 的详细信息，请参阅[技术说明 24](../mfc/tn024-mfc-defined-messages-and-resources.md)。  
