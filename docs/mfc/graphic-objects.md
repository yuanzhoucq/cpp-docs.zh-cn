...
> [!NOTE]
> 类[CImage](../atl-mfc-shared/reference/cimage-class.md)提供了增强的位图支持。

类库中的每个图形对象类都具有一个构造函数，使你可以创建该类的图形对象，随后必须使用适当的创建函数（如 `CreatePen`）初始化这些对象。

类库中的每个图形对象类都具有一个强制转换运算符，可将 MFC 对象强制转换为关联的 Windows 句柄。 生成的句柄在关联对象将它分离之前都有效。 使用对象`Detach`的成员函数分离句柄。
...
1. 在堆栈帧上定义图形对象。 使用特定于类型的创建函数（如 `CreatePen`）初始化对象。 或者，在构造函数中初始化对象。 请参阅有关[单阶段和两阶段创建](../mfc/one-stage-and-two-stage-construction-of-objects.md)的讨论，该讨论提供示例代码。

1. [将对象选择到当前设备上下文中](../mfc/selecting-a-graphic-object-into-a-device-context.md)，保存之前选择的旧图形对象。
...
- [在设备上下文中选择图形对象](../mfc/selecting-a-graphic-object-into-a-device-context.md)
