...
此代码声明了两个称为 `GetArray` 的函数 `SetArray`，并在访问属性时允许用户请求特定的行和列。

此外，"添加属性向导" 将以下行添加到控件调度映射，位于控件类实现（。CPP）文件：

[!code-cpp[NVC_MFC_AxUI#36](../mfc/codesnippet/cpp/mfc-activex-controls-advanced-topics_3.cpp)]

最后，`GetArray` 和 `SetArray` 函数的实现添加到的末尾。CPP 文件。 在大多数情况下，你将修改 Get 函数以返回属性的值。 Set 函数通常包含应在属性更改之前或之后执行的代码。
...
如果 ActiveX 控件的基类是 `COleControl`的，则可以重写[CWnd：:P retranslatemessage](../mfc/reference/cwnd-class.md#pretranslatemessage)来处理消息，然后由容器处理。 使用此方法时，如果你在 `PreTranslateMessage`的重写中处理消息，则始终返回**TRUE** 。
