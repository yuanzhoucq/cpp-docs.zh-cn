# [MFC 和 ATL](mfc-and-atl.md)
# [MFC 桌面应用程序](mfc-desktop-applications.md)
# [MFC 概念](mfc-concepts.md)
## [常规 MFC 主题](general-mfc-topics.md)
### [使用 MFC 源文件](using-the-mfc-source-files.md)
#### [注释示例](an-example-of-the-comments.md)
#### [- Implementation 注释](decrement-implementation-comment.md)
#### [-- Constructors 注释](decrement-constructors-comment.md)
#### [-- Attributes 注释](decrement-attributes-comment.md)
#### [-- Operations 注释](decrement-operations-comment.md)
#### [-- Overridables 注释](decrement-overridables-comment.md)
### [MFC 库版本](mfc-library-versions.md)
#### [MFC MBCS DLL 加载项](mfc-mbcs-dll-add-on.md)
### [使用类编写 Windows 应用程序](using-the-classes-to-write-applications-for-windows.md)
#### [框架 (MFC)](framework-mfc.md)
#### [SDI 和 MDI](sdi-and-mdi.md)
#### [文档、视图和框架](documents-views-and-the-framework.md)
#### [向导和资源编辑器](wizards-and-the-resource-editors.md)
### [基于框架生成](building-on-the-framework.md)
#### [MFC 应用程序的构建操作顺序](sequence-of-operations-for-building-mfc-applications.md)
#### [OLE 应用程序的创建操作顺序](sequence-of-operations-for-creating-ole-applications.md)
#### [ActiveX 控件的创建操作顺序](sequence-of-operations-for-creating-activex-controls.md)
#### [数据库应用程序的创建操作顺序](sequence-of-operations-for-creating-database-applications.md)
#### [框架如何调用你的代码](how-the-framework-calls-your-code.md)
### [CWinApp：应用程序类](cwinapp-the-application-class.md)
#### [CWinApp 和 MFC 应用程序向导](cwinapp-and-the-mfc-application-wizard.md)
#### [可重写的 CWinApp 成员函数](overridable-cwinapp-member-functions.md)
#### [InitInstance 成员函数](initinstance-member-function.md)
#### [Run 成员函数](run-member-function.md)
#### [ExitInstance 成员函数](exitinstance-member-function.md)
#### [OnIdle 成员函数](onidle-member-function.md)
#### [特殊的 CWinApp 服务](special-cwinapp-services.md)
### [文档模板和文档-视图创建过程](document-templates-and-the-document-view-creation-process.md)
#### [文档模板创建](document-template-creation.md)
#### [文档-视图创建](document-view-creation.md)
#### [MFC 对象之间的关系](relationships-among-mfc-objects.md)
#### [创建新文档、窗口和视图](creating-new-documents-windows-and-views.md)
### [管理 MFC 模块的状态数据](managing-the-state-data-of-mfc-modules.md)
#### [导出的 DLL 函数入口点](exported-dll-function-entry-points.md)
#### [COM 接口入口点](com-interface-entry-points.md)
#### [窗口程序入口点](window-procedure-entry-points.md)
### [空闲环路处理](idle-loop-processing.md)
### [支持 MFC 模块状态中的激活上下文](support-for-activation-contexts-in-the-mfc-module-state.md)
### [隔离 MFC 公用控件库](isolation-of-the-mfc-common-controls-library.md)
### [Windows Vista 公用控件的生成要求](build-requirements-for-windows-vista-common-controls.md)
#### [弃用的 ANSI API](deprecated-ansi-apis.md)
### [如何：添加重启管理器支持](how-to-add-restart-manager-support.md)
### [动态布局](dynamic-layout.md)
## [使用 CObject](using-cobject.md)
### [从 CObject 派生类](deriving-a-class-from-cobject.md)
#### [指定功能级别](specifying-levels-of-functionality.md)
### [访问运行时类信息](accessing-run-time-class-information.md)
### [动态对象创建](dynamic-object-creation.md)
### [CObject 类：常见问题解答](cobject-class-frequently-asked-questions.md)
#### [是否必须从 CObject 中派生新类？](do-i-have-to-derive-new-classes-from-cobject-q.md)
#### [从 CObject 派生类所带来的开销](what-does-it-cost-me-to-derive-a-class-from-cobject-q.md)
## [集合](collections.md)
### [关于选择集合类的建议](recommendations-for-choosing-a-collection-class.md)
### [基于模板的类](template-based-classes.md)
### [如何：创建类型安全集合](how-to-make-a-type-safe-collection.md)
### [访问集合的所有成员](accessing-all-members-of-a-collection.md)
#### [删除 CObject 集合中的所有对象](deleting-all-objects-in-a-cobject-collection.md)
### [创建堆栈和队列集合](creating-stack-and-queue-collections.md)
## [MFC 中的异常处理](exception-handling-in-mfc.md)
### [异常：版本 3.0 中对异常宏的更改](exceptions-changes-to-exception-macros-in-version-3-0.md)
### [异常：捕获和删除异常](exceptions-catching-and-deleting-exceptions.md)
### [异常：从 MFC 异常宏转换](exceptions-converting-from-mfc-exception-macros.md)
### [异常：使用 MFC 宏和 C++ 异常](exceptions-using-mfc-macros-and-cpp-exceptions.md)
### [异常：检查异常内容](exceptions-examining-exception-contents.md)
### [异常：释放异常中的对象](exceptions-freeing-objects-in-exceptions.md)
### [异常：从你自己的函数中抛出异常](exceptions-throwing-exceptions-from-your-own-functions.md)
### [异常：构造函数中的异常](exceptions-exceptions-in-constructors.md)
### [异常：数据库异常](exceptions-database-exceptions.md)
### [异常：OLE 异常](exceptions-ole-exceptions.md)
## [MFC 中的文件](files-in-mfc.md)
### [打开文件](opening-files.md)
### [读取和写入文件](reading-and-writing-files.md)
### [关闭文件](closing-files.md)
### [访问文件状态](accessing-file-status.md)
## [界面元素](interface-elements.md)
## [MAPI](mapi.md)
### [MFC 中的 MAPI 支持](mapi-support-in-mfc.md)
### [MAPI 示例](mapi-samples.md)
## [内存管理](memory-management.md)
### [内存管理：帧分配](memory-management-frame-allocation.md)
### [内存管理：堆分配](memory-management-heap-allocation.md)
#### [内存管理： 示例](memory-management-examples.md)
#### [内存管理：可调整大小的内存块](memory-management-resizable-memory-blocks.md)
## [消息处理和映射](message-handling-and-mapping.md)
### [框架中的消息和命令](messages-and-commands-in-the-framework.md)
#### [消息](messages.md)
#### [消息处理程序](message-handlers.md)
#### [消息类别](message-categories.md)
#### [映射消息](mapping-messages.md)
#### [用户界面对象和命令 ID](user-interface-objects-and-command-ids.md)
##### [命令 ID](command-ids.md)
##### [标准命令](standard-commands.md)
#### [命令目标](command-targets.md)
### [框架如何调用处理程序](how-the-framework-calls-a-handler.md)
#### [消息发送和接收](message-sending-and-receiving.md)
#### [非命令消息如何到达其处理程序](how-noncommand-messages-reach-their-handlers.md)
#### [命令传送](command-routing.md)
#### [命令传送示例](command-routing-illustration.md)
#### [OnCmdMsg 处理程序](oncmdmsg-handler.md)
#### [重写标准命令传送](overriding-the-standard-command-routing.md)
### [框架如何搜索消息映射](how-the-framework-searches-message-maps.md)
#### [消息映射的位置](where-to-find-message-maps.md)
#### [派生消息映射](derived-message-maps.md)
### [声明消息处理程序函数](declaring-message-handler-functions.md)
#### [标准 Windows 消息的处理程序](handlers-for-standard-windows-messages.md)
#### [命令和控件通知的处理程序](handlers-for-commands-and-control-notifications.md)
#### [消息映射范围的处理程序](handlers-for-message-map-ranges.md)
#### [处理反射消息](handling-reflected-messages.md)
### [如何：在状态栏中显示命令信息](how-to-display-command-information-in-the-status-bar.md)
### [如何：创建模板类的消息映射](how-to-create-a-message-map-for-a-template-class.md)
## [MFC COM](mfc-com.md)
### [活动文档包容](active-document-containment.md)
#### [活动文档包含示例：Office 活页夹](example-of-active-document-containment-office-binder.md)
#### [创建活动文档容器应用程序](creating-an-active-document-container-application.md)
#### [活动文档容器](active-document-containers.md)
##### [帮助菜单合并](help-menu-merging.md)
##### [以编程方式打印](programmatic-printing.md)
##### [消息处理和命令目标](message-handling-and-command-targets.md)
#### [活动文档服务器](active-document-servers.md)
#### [活动文档](active-documents.md)
### [自动化](automation.md)
#### [自动化客户端](automation-clients.md)
##### [自动化客户端：使用类型库](automation-clients-using-type-libraries.md)
#### [自动化服务器](automation-servers.md)
##### [自动化服务器：对象生存期问题](automation-servers-object-lifetime-issues.md)
### [连接点](connection-points.md)
## [MFC Internet 编程基础知识](mfc-internet-programming-basics.md)
### [与 Internet 相关的 MFC 类](internet-related-mfc-classes.md)
### [按主题划分的 Internet 信息](internet-information-by-topic.md)
### [按任务划分的 Internet 信息](internet-information-by-task.md)
### [Internet 上的 Active 技术](active-technology-on-the-internet.md)
### [WinInet 基础知识](wininet-basics.md)
### [HTML 基础知识](html-basics.md)
## [MFC Internet 编程任务](mfc-internet-programming-tasks.md)
### [应用程序设计选项](application-design-choices.md)
### [编写 MFC 应用程序](writing-mfc-applications.md)
### [Internet 上的 ActiveX 控件](activex-controls-on-the-internet.md)
### [升级现有的 ActiveX 控件](upgrading-an-existing-activex-control.md)
### [Internet 上的异步名字对象](asynchronous-monikers-on-the-internet.md)
### [测试 Internet 应用程序](testing-internet-applications.md)
### [Internet 安全性 (C++)](internet-security-cpp.md)
## [MFC 中的 OLE](ole-in-mfc.md)
### [OLE 后台](ole-background.md)
#### [OLE 后台：链接和嵌入](ole-background-linking-and-embedding.md)
#### [OLE 后台：容器和服务器](ole-background-containers-and-servers.md)
#### [OLE 后台：实现策略](ole-background-implementation-strategies.md)
#### [OLE 后台：MFC 实现](ole-background-mfc-implementation.md)
### [激活 (C++)](activation-cpp.md)
#### [激活：谓词](activation-verbs.md)
### [容器](containers.md)
#### [容器：实现容器](containers-implementing-a-container.md)
#### [容器：客户端项](containers-client-items.md)
#### [容器：客户端项通知](containers-client-item-notifications.md)
#### [容器：客户端项状态](containers-client-item-states.md)
#### [容器：复合文件](containers-compound-files.md)
#### [容器：用户界面问题](containers-user-interface-issues.md)
#### [容器：高级功能](containers-advanced-features.md)
### [数据对象和数据源 (OLE)](data-objects-and-data-sources-ole.md)
#### [数据对象和数据源：创建和破坏](data-objects-and-data-sources-creation-and-destruction.md)
#### [数据对象和数据源：操作](data-objects-and-data-sources-manipulation.md)
### [拖放 (OLE)](drag-and-drop-ole.md)
#### [拖放：实现放置源](drag-and-drop-implementing-a-drop-source.md)
#### [拖放：实现放置目标](drag-and-drop-implementing-a-drop-target.md)
#### [拖放：自定义](drag-and-drop-customizing.md)
### [菜单和资源 (OLE)](menus-and-resources-ole.md)
#### [菜单和资源：容器添加项](menus-and-resources-container-additions.md)
#### [菜单和资源：服务器添加项](menus-and-resources-server-additions.md)
#### [菜单和资源：菜单合并](menus-and-resources-menu-merging.md)
### [注册](registration.md)
### [服务器](servers.md)
#### [服务器：实现服务器](servers-implementing-a-server.md)
#### [服务器：实现服务器文档](servers-implementing-server-documents.md)
#### [服务器：实现就地框架窗口](servers-implementing-in-place-frame-windows.md)
#### [服务器：服务器项](servers-server-items.md)
#### [服务器：用户界面问题](servers-user-interface-issues.md)
### [跟踪器](trackers.md)
#### [跟踪器：在 OLE 应用程序中实现跟踪器](trackers-implementing-trackers-in-your-ole-application.md)
##### [如何：在代码中实现跟踪](how-to-implement-tracking-in-your-code.md)
##### [橡皮筋法和跟踪器](rubber-banding-and-trackers.md)
## [MFC 中的序列化](serialization-in-mfc.md)
### [序列化：创建可序列化类](serialization-making-a-serializable-class.md)
### [序列化：序列化对象](serialization-serializing-an-object.md)
#### [什么是 CArchive 对象](what-is-a-carchive-object.md)
#### [CArchive 对象的两种创建方法](two-ways-to-create-a-carchive-object.md)
#### [使用 CArchive << 和 >> 运算符](using-the-carchive-output-and-input-operators.md)
#### [通过存档存储和加载 CObject](storing-and-loading-cobjects-via-an-archive.md)
### [序列化：序列化与数据库输入-输出](serialization-serialization-vs-database-input-output.md)
#### [关于处理输入-输出的建议](recommendations-for-handling-input-output.md)
#### [MFC 数据库应用程序中的文件菜单](file-menu-in-an-mfc-database-application.md)
## [用户界面元素 (MFC)](user-interface-elements-mfc.md)
### [ActiveX 控件](activex-controls.md)
#### [MFC ActiveX 控件](mfc-activex-controls.md)
##### [MFC ActiveX 控件：优化](mfc-activex-controls-optimization.md)
###### [优化持久性和初始化](optimizing-persistence-and-initialization.md)
###### [提供无窗口激活](providing-windowless-activation.md)
###### [关闭“可见时激活”选项](turning-off-the-activate-when-visible-option.md)
###### [不活动时提供鼠标交互](providing-mouse-interaction-while-inactive.md)
###### [提供无闪烁激活](providing-flicker-free-activation.md)
###### [使用未剪辑的设备上下文](using-an-unclipped-device-context.md)
###### [优化控件绘制](optimizing-control-drawing.md)
##### [MFC ActiveX 控件：绘制 ActiveX 控件](mfc-activex-controls-painting-an-activex-control.md)
##### [MFC ActiveX 控件：事件](mfc-activex-controls-events.md)
##### [MFC ActiveX 控件：向 ActiveX 控件添加内置事件](mfc-activex-controls-adding-stock-events-to-an-activex-control.md)
##### [MFC ActiveX 控件：添加自定义事件](mfc-activex-controls-adding-custom-events.md)
##### [MFC ActiveX 控件：方法](mfc-activex-controls-methods.md)
##### [MFC ActiveX 控件：添加内置方法](mfc-activex-controls-adding-stock-methods.md)
##### [MFC ActiveX 控件：添加自定义方法](mfc-activex-controls-adding-custom-methods.md)
##### [MFC ActiveX 控件：从方法返回错误代码](mfc-activex-controls-returning-error-codes-from-a-method.md)
##### [MFC ActiveX 控件：属性](mfc-activex-controls-properties.md)
##### [MFC ActiveX 控件：添加内置属性](mfc-activex-controls-adding-stock-properties.md)
##### [MFC ActiveX 控件：添加自定义属性](mfc-activex-controls-adding-custom-properties.md)
##### [MFC ActiveX 控件：高级属性实现](mfc-activex-controls-advanced-property-implementation.md)
##### [MFC ActiveX 控件：访问环境属性](mfc-activex-controls-accessing-ambient-properties.md)
##### [MFC ActiveX 控件：属性页](mfc-activex-controls-property-pages.md)
##### [MFC ActiveX 控件：添加另一个自定义属性页](mfc-activex-controls-adding-another-custom-property-page.md)
##### [MFC ActiveX 控件：使用内置属性页](mfc-activex-controls-using-stock-property-pages.md)
##### [MFC ActiveX 控件：创建自动化服务器](mfc-activex-controls-creating-an-automation-server.md)
##### [MFC ActiveX 控件：使用字体](mfc-activex-controls-using-fonts.md)
##### [MFC ActiveX 控件：在 ActiveX 控件中使用图片](mfc-activex-controls-using-pictures-in-an-activex-control.md)
##### [MFC ActiveX 控件：高级主题](mfc-activex-controls-advanced-topics.md)
##### [MFC ActiveX 控件：分发 ActiveX 控件](mfc-activex-controls-distributing-activex-controls.md)
##### [MFC ActiveX 控件：许可 ActiveX 控件](mfc-activex-controls-licensing-an-activex-control.md)
##### [MFC ActiveX 控件：本地化 ActiveX 控件](mfc-activex-controls-localizing-an-activex-control.md)
##### [MFC ActiveX 控件：序列化](mfc-activex-controls-serializing.md)
##### [MFC ActiveX 控件：创建 Windows 控件的子类](mfc-activex-controls-subclassing-a-windows-control.md)
###### [反射窗口消息 ID](reflected-window-message-ids.md)
##### [MFC ActiveX 控件：在 ActiveX 控件中使用数据绑定](mfc-activex-controls-using-data-binding-in-an-activex-control.md)
#### [ActiveX 控件容器](activex-control-containers.md)
##### [ActiveX 控件的容器](containers-for-activex-controls.md)
##### [ActiveX 控件容器：手动启用 ActiveX 控件包含](activex-control-containers-manually-enabling-activex-control-containment.md)
##### [ActiveX 控件容器：将控件插入控件容器应用程序](inserting-a-control-into-a-control-container-application.md)
##### [ActiveX 控件容器：将 ActiveX 控件连接到成员变量](activex-control-containers-connecting-an-activex-control-to-a-member-variable.md)
##### [ActiveX 控件容器：从 ActiveX 控件处理事件](activex-control-containers-handling-events-from-an-activex-control.md)
##### [ActiveX 控件容器：查看和修改控件属性](activex-control-containers-viewing-and-modifying-control-properties.md)
##### [ActiveX 控件容器：对 ActiveX 控件容器中的 ActiveX 控件编程](programming-activex-controls-in-a-activex-control-container.md)
##### [ActiveX 控件容器：在非对话框容器中使用控件](activex-control-containers-using-controls-in-a-non-dialog-container.md)
#### [使用测试容器测试属性和事件](testing-properties-and-events-with-test-container.md)
### [剪贴板](clipboard.md)
#### [剪贴板：何时使用各个剪贴板机制](clipboard-when-to-use-each-clipboard-mechanism.md)
#### [剪贴板：使用 Windows 剪贴板](clipboard-using-the-windows-clipboard.md)
#### [剪贴板：使用 OLE 剪贴板机制](clipboard-using-the-ole-clipboard-mechanism.md)
##### [剪贴板：复制和粘贴数据](clipboard-copying-and-pasting-data.md)
##### [剪贴板：添加其他格式](clipboard-adding-other-formats.md)
### [控件 (MFC)](controls-mfc.md)
#### [公用控件示例列表](common-control-sample-list.md)
#### [创建和使用控件](making-and-using-controls.md)
##### [使用对话框中的公用控件](using-common-controls-in-a-dialog-box.md)
##### [使用对话框编辑器添加控件](using-the-dialog-editor-to-add-controls.md)
##### [手动添加控件](adding-controls-by-hand.md)
##### [从标准控件中派生控件](deriving-controls-from-a-standard-control.md)
##### [将公用控件用作子窗口](using-a-common-control-as-a-child-window.md)
##### [从公用控件中接收通知](receiving-notification-from-common-controls.md)
#### [使用 CAnimateCtrl](using-canimatectrl.md)
##### [使用动画控件](using-an-animation-control.md)
##### [动画控件发送的通知](notifications-sent-by-animation-controls.md)
#### [使用 CDateTimeCtrl](using-cdatetimectrl.md)
##### [创建日期和时间选取器控件](creating-the-date-and-time-picker-control.md)
##### [日期和时间选取器控件示例](date-and-time-picker-control-examples.md)
##### [访问嵌入的月历控件](accessing-the-embedded-month-calendar-control.md)
##### [在日期和时间选取器控件中使用自定义格式字符串](using-custom-format-strings-in-a-date-and-time-picker-control.md)
##### [在日期和时间选取器控件中使用回调字段](using-callback-fields-in-a-date-and-time-picker-control.md)
##### [处理日期和时间选取器控件中的通知消息](processing-notification-messages-in-date-and-time-picker-controls.md)
#### [使用 CComboBoxEx](using-ccomboboxex.md)
##### [创建扩展组合框控件](creating-an-extended-combo-box-control.md)
##### [在扩展组合框控件中使用图像列表](using-image-lists-in-an-extended-combo-box-control.md)
##### [设置单个项的图像](setting-the-images-for-an-individual-item.md)
##### [处理扩展组合框控件中的通知消息](processing-notification-messages-in-extended-combo-box-controls.md)
#### [使用 CHeaderCtrl](using-cheaderctrl.md)
##### [标题控件和列表控件](header-control-and-list-control.md)
##### [标题控件示例](header-control-examples.md)
##### [标题控件中的标题项](header-items-in-a-header-control.md)
##### [自定义标题项的外观](customizing-the-header-item-s-appearance.md)
##### [为标题项提供拖放支持](providing-drag-and-drop-support-for-header-items.md)
##### [对标题控件使用图像列表](using-image-lists-with-header-controls.md)
##### [定义所有者描述的标题控件](making-owner-drawn-header-controls.md)
##### [使用标题控件](working-with-a-header-control.md)
##### [创建标题控件](creating-the-header-control.md)
##### [向标题控件添加项](adding-items-to-the-header-control.md)
##### [对标题控件中的项排序](ordering-items-in-the-header-control.md)
##### [处理标题控件通知](processing-header-control-notifications.md)
#### [使用 CHotKeyCtrl](using-chotkeyctrl.md)
##### [使用热键控件](using-a-hot-key-control.md)
##### [设置热键](setting-a-hot-key.md)
##### [全局热键](global-hot-keys.md)
##### [线程特定的热键](thread-specific-hot-keys.md)
#### [使用 CImageList](using-cimagelist.md)
##### [图像列表类型](types-of-image-lists.md)
##### [使用图像列表](using-an-image-list.md)
##### [操作图像列表](manipulating-image-lists.md)
##### [通过图像列表绘制图像](drawing-images-from-an-image-list.md)
##### [图像列表中的图像覆盖](image-overlays-in-image-lists.md)
##### [从图像列表中拖动图像](dragging-images-from-an-image-list.md)
##### [图像列表中的图像信息](image-information-in-image-lists.md)
#### [使用 CListCtrl](using-clistctrl.md)
##### [列表控件和列表视图](list-control-and-list-view.md)
##### [列表项和图像列表](list-items-and-image-lists.md)
##### [回调项和回调掩码](callback-items-and-the-callback-mask.md)
##### [创建列表控件](creating-the-list-control.md)
##### [创建图像列表](creating-the-image-lists.md)
##### [向控件添加列（报表视图）](adding-columns-to-the-control-report-view.md)
##### [向控件添加项](adding-items-to-the-control.md)
##### [在列表控件中滚动、排列、排序和查找](scrolling-arranging-sorting-and-finding-in-list-controls.md)
##### [实现列表控件中的工作区域](implementing-working-areas-in-list-controls.md)
##### [处理列表控件中的通知消息](processing-notification-messages-in-list-controls.md)
##### [更改列表控件样式](changing-list-control-styles.md)
##### [虚拟列表控件](virtual-list-controls.md)
##### [销毁列表控件](destroying-the-list-control.md)
#### [使用 CMonthCalCtrl](using-cmonthcalctrl.md)
##### [创建月历控件](creating-the-month-calendar-control.md)
##### [月历控件示例](month-calendar-control-examples.md)
##### [处理月历控件中的通知消息](processing-notification-messages-in-month-calendar-controls.md)
##### [设置月历控件的日状态](setting-the-day-state-of-a-month-calendar-control.md)
#### [使用 CProgressCtrl](using-cprogressctrl.md)
##### [进度控件的样式](styles-for-the-progress-control.md)
##### [进度控件的设置](settings-for-the-progress-control.md)
##### [操作进度控件](manipulating-the-progress-control.md)
#### [使用 CReBarCtrl](using-crebarctrl.md)
##### [CReBar 与CReBarCtrl](crebar-vs-crebarctrl.md)
##### [创建 Rebar 控件](creating-a-rebar-control.md)
##### [Rebar 控件和带区](rebar-controls-and-bands.md)
##### [对 Rebar 控件使用图像列表](using-an-image-list-with-a-rebar-control.md)
##### [对 Rebar 控件使用对话栏](using-a-dialog-bar-with-a-rebar-control.md)
##### [处理 Rebar 控件中的通知消息](processing-notification-messages-in-a-rebar-control.md)
#### [使用 CRichEditCtrl](using-cricheditctrl.md)
##### [Rich Edit 控件概述](overview-of-the-rich-edit-control.md)
##### [与 Rich Edit 控件相关的类](classes-related-to-rich-edit-controls.md)
##### [Rich Edit 控件示例](rich-edit-control-examples.md)
##### [Rich Edit 控件中的字符格式设置](character-formatting-in-rich-edit-controls.md)
##### [Rich Edit 控件中的段落格式设置](paragraph-formatting-in-rich-edit-controls.md)
##### [Rich Edit 控件中的当前选定内容](current-selection-in-a-rich-edit-control.md)
##### [Rich Edit 控件中的断字](word-breaks-in-rich-edit-controls.md)
##### [Rich Edit 控件中的剪贴板操作](clipboard-operations-in-rich-edit-controls.md)
##### [Rich Edit 控件中的流操作](stream-operations-in-rich-edit-controls.md)
##### [在 Rich Edit 控件中打印](printing-in-rich-edit-controls.md)
##### [无界限 Rich Edit 控件](bottomless-rich-edit-controls.md)
##### [来自 Rich Edit 控件的通知](notifications-from-a-rich-edit-control.md)
#### [使用 CSliderCtrl](using-csliderctrl.md)
##### [使用滑块控件](using-slider-controls.md)
##### [滑块控件样式](slider-control-styles.md)
##### [滑块控件成员函数](slider-control-member-functions.md)
##### [滑块通知消息](slider-notification-messages.md)
#### [使用 CSpinButtonCtrl](using-cspinbuttonctrl.md)
##### [调节按钮样式](spin-button-styles.md)
##### [调节按钮成员函数](spin-button-member-functions.md)
#### [使用 CStatusBarCtrl](using-cstatusbarctrl.md)
##### [创建状态栏的方法](methods-of-creating-a-status-bar.md)
##### [CStatusBarCtrl 的设置](settings-for-the-cstatusbarctrl.md)
##### [使用 CStatusBarCtrl 创建 CStatusBarCtrl 对象](using-cstatusbarctrl-to-create-a-cstatusbarctrl-object.md)
##### [设置 CStatusBarCtrl 对象的模式](setting-the-mode-of-a-cstatusbarctrl-object.md)
##### [初始化 CStatusBarCtrl 对象的组成部分](initializing-the-parts-of-a-cstatusbarctrl-object.md)
##### [在 CStatusBarCtrl 对象中使用工具提示](using-tooltips-in-a-cstatusbarctrl-object.md)
#### [使用 CTabCtrl](using-ctabctrl.md)
##### [选项卡控件和属性表](tab-controls-and-property-sheets.md)
##### [选项卡和选项卡控件特性](tabs-and-tab-control-attributes.md)
##### [定义所有者描述的选项卡](making-owner-drawn-tabs.md)
##### [使用选项卡控件](working-with-a-tab-control.md)
##### [创建选项卡控件](creating-the-tab-control.md)
##### [向选项卡控件添加选项卡](adding-tabs-to-a-tab-control.md)
##### [处理选项卡控件通知消息](processing-tab-control-notification-messages.md)
#### [使用 CToolBarCtrl](using-ctoolbarctrl.md)
##### [创建工具栏的方法](methods-of-creating-a-toolbar.md)
##### [工具栏控件的设置](settings-for-the-toolbar-control.md)
##### [创建 CToolBarCtrl 对象](creating-a-ctoolbarctrl-object.md)
##### [在工具栏控件中使用图像列表](using-image-lists-in-a-toolbar-control.md)
##### [使用工具栏控件中的下拉按钮](using-drop-down-buttons-in-a-toolbar-control.md)
##### [自定义工具栏控件的外观](customizing-the-appearance-of-a-toolbar-control.md)
##### [处理工具提示通知](handling-tool-tip-notifications.md)
##### [处理自定义通知](handling-customization-notifications.md)
#### [使用 CToolTipCtrl](using-ctooltipctrl.md)
##### [创建工具提示的方法](methods-of-creating-tool-tips.md)
##### [工具提示控件的设置](settings-for-the-tool-tip-control.md)
##### [使用 CToolTipCtrl 创建和操作 CToolTipCtrl 对象](using-ctooltipctrl-to-create-and-manipulate-a-ctooltipctrl-object.md)
##### [操作工具提示控件](manipulating-the-tool-tip-control.md)
#### [使用 CTreeCtrl](using-ctreectrl.md)
##### [CTreeCtrl 与CTreeView](ctreectrl-vs-ctreeview.md)
##### [使用树控件](using-tree-controls.md)
##### [与树控件通信](communicating-with-a-tree-control.md)
##### [树控件样式](tree-control-styles.md)
##### [树控件父项和子项](tree-control-parent-and-child-items.md)
##### [树控件项位置](tree-control-item-position.md)
##### [树控件项标签](tree-control-item-labels.md)
##### [树控件标签编辑](tree-control-label-editing.md)
##### [树控件项状态概述](tree-control-item-states-overview.md)
##### [树控件图像列表](tree-control-image-lists.md)
##### [树控件项选择](tree-control-item-selection.md)
##### [树控件拖放操作](tree-control-drag-and-drop-operations.md)
##### [树控件项信息](tree-control-item-information.md)
##### [树控件通知消息](tree-control-notification-messages.md)
### [控件条](control-bars.md)
### [对话栏](dialog-bars.md)
### [对话框](dialog-boxes.md)
#### [示例：通过菜单命令显示对话框](example-displaying-a-dialog-box-via-a-menu-command.md)
#### [对话框示例列表](dialog-sample-list.md)
#### [框架中的对话框组件](dialog-box-components-in-the-framework.md)
#### [模式和无模式对话框](modal-and-modeless-dialog-boxes.md)
#### [属性表和属性页 (MFC)](property-sheets-and-property-pages-mfc.md)
#### [创建对话框资源](creating-the-dialog-resource.md)
#### [使用代码向导创建对话框类](creating-a-dialog-class-with-code-wizards.md)
##### [创建自己的对话框类](creating-your-dialog-class.md)
#### [对话框的生命周期](life-cycle-of-a-dialog-box.md)
##### [创建并显示对话框](creating-and-displaying-dialog-boxes.md)
##### [创建模式对话框](creating-modal-dialog-boxes.md)
##### [创建无模式对话框](creating-modeless-dialog-boxes.md)
##### [在内存中使用对话框模板](using-a-dialog-template-in-memory.md)
##### [设置对话框的背景色](setting-the-dialog-boxs-background-color.md)
##### [初始化对话框](initializing-the-dialog-box.md)
##### [处理对话框中的 Windows 消息](handling-windows-messages-in-your-dialog-box.md)
##### [从对话框对象检索数据](retrieving-data-from-the-dialog-object.md)
##### [关闭对话框](closing-the-dialog-box.md)
##### [销毁对话框](destroying-the-dialog-box.md)
#### [对话框数据交换和验证](dialog-data-exchange-and-validation.md)
##### [对话框数据交换](dialog-data-exchange.md)
##### [对话框数据验证](dialog-data-validation.md)
#### [对对话框中的控件进行类型安全的访问](type-safe-access-to-controls-in-a-dialog-box.md)
##### [对控件进行类型安全的访问（不使用代码向导）](type-safe-access-to-controls-without-code-wizards.md)
##### [对控件进行类型安全的访问（使用代码向导）](type-safe-access-to-controls-with-code-wizards.md)
#### [将 Windows 消息映射到你的类](mapping-windows-messages-to-your-class.md)
#### [经常重写的成员函数](commonly-overridden-member-functions.md)
#### [经常添加的成员函数](commonly-added-member-functions.md)
#### [通用对话框类](common-dialog-classes.md)
#### [OLE 中的对话框](dialog-boxes-in-ole.md)
#### [演练：将 CTaskDialog 添加到应用程序](walkthrough-adding-a-ctaskdialog-to-an-application.md)
### [文档-视图体系结构](document-view-architecture.md)
#### [文档-视图示例列表](document-view-sample-list.md)
#### [文档-视图体系结构纵览](a-portrait-of-the-document-view-architecture.md)
#### [文档-视图体系结构的优点](advantages-of-the-document-view-architecture.md)
#### [MFC 应用程序向导创建的文档和视图类](document-and-view-classes-created-by-the-mfc-application-wizard.md)
#### [文档-视图体系结构的替换选项](alternatives-to-the-document-view-architecture.md)
#### [使用文档](using-documents.md)
##### [从 CDocument 派生文档类](deriving-a-document-class-from-cdocument.md)
##### [使用文档数据变量管理数据](managing-data-with-document-data-variables.md)
##### [针对文件进行数据序列化](serializing-data-to-and-from-files.md)
##### [跳过序列化机制](bypassing-the-serialization-mechanism.md)
##### [处理文档中的命令](handling-commands-in-the-document.md)
#### [使用视图](using-views.md)
##### [MFC 中可用的派生视图类](derived-view-classes-available-in-mfc.md)
##### [在视图中绘制](drawing-in-a-view.md)
##### [通过视图解释用户输入](interpreting-user-input-through-a-view.md)
##### [视图在打印中的作用](role-of-the-view-in-printing.md)
##### [滚动和缩放视图](scrolling-and-scaling-views.md)
#### [多文档类型、视图和框架窗口](multiple-document-types-views-and-frame-windows.md)
#### [初始化和清理文档和视图](initializing-and-cleaning-up-documents-and-views.md)
##### [初始化文档和视图](initializing-documents-and-views.md)
##### [清理文档和视图](cleaning-up-documents-and-views.md)
#### [向单个文档添加多个视图](adding-multiple-views-to-a-single-document.md)
### [窗体视图 (MFC)](form-views-mfc.md)
#### [在项目中插入窗体](inserting-a-form-into-a-project.md)
### [HTML 帮助：程序的上下文相关帮助](html-help-context-sensitive-help-for-your-programs.md)
### [MDI 选项卡式组](mdi-tabbed-groups.md)
### [菜单 (MFC)](menus-mfc.md)
#### [菜单示例列表](menu-sample-list.md)
#### [在程序执行过程中操作菜单](manipulating-menus-during-program-execution.md)
#### [如何：更新用户界面对象](how-to-update-user-interface-objects.md)
##### [何时调用更新处理程序](when-update-handlers-are-called.md)
##### [ON_UPDATE_COMMAND_UI 宏](on-update-command-ui-macro.md)
##### [CCmdUI 类](the-ccmdui-class.md)
### [OLE (MFC)](ole-mfc.md)
### [打印和打印预览](printing-and-print-preview.md)
#### [打印](printing.md)
##### [如何执行默认打印](how-default-printing-is-done.md)
##### [多页文档](multipage-documents.md)
##### [页眉和页脚](headers-and-footers.md)
##### [分配 GDI 资源](allocating-gdi-resources.md)
#### [打印预览体系结构](print-preview-architecture.md)
### [属性表 (MFC)](property-sheets-mfc.md)
#### [MFC 中的属性表和属性页](property-sheets-and-property-pages-in-mfc.md)
#### [在应用程序中使用属性表](using-property-sheets-in-your-application.md)
#### [将控件添加到属性表](adding-controls-to-a-property-sheet.md)
#### [交换数据](exchanging-data.md)
#### [创建无模式属性表](creating-a-modeless-property-sheet.md)
#### [处理应用按钮](handling-the-apply-button.md)
#### [属性表作为向导](property-sheets-as-wizards.md)
### [功能区设计器 (MFC)](ribbon-designer-mfc.md)
#### [如何：将现有 MFC 功能区转换为功能区资源](how-to-convert-an-existing-mfc-ribbon-to-a-ribbon-resource.md)
#### [如何：自定义应用程序按钮](how-to-customize-the-application-button.md)
#### [如何：自定义快速访问工具栏](how-to-customize-the-quick-access-toolbar.md)
#### [如何：添加功能区控件和事件处理程序](how-to-add-ribbon-controls-and-event-handlers.md)
#### [如何：从 MFC 应用程序中加载功能区资源](how-to-load-a-ribbon-resource-from-an-mfc-application.md)
#### [演练：使用 MFC 创建功能区应用程序](walkthrough-creating-a-ribbon-application-by-using-mfc.md)
#### [演练：更新 MFC 自由曲线应用程序（第 1 部分）](walkthrough-updating-the-mfc-scribble-application-part-1.md)
#### [演练：更新 MFC 自由曲线应用程序（第 2 部分）](walkthrough-updating-the-mfc-scribble-application-part-2.md)
### [状态栏](status-bars.md)
#### [MFC 中的状态栏实现](status-bar-implementation-in-mfc.md)
##### [更新状态栏窗格的文本](updating-the-text-of-a-status-bar-pane.md)
### [工具提示](tool-tips.md)
#### [Windows 中未从 CFrameWnd 派生的工具提示](tool-tips-in-windows-not-derived-from-cframewnd.md)
##### [启用工具提示](enabling-tool-tips.md)
##### [处理工具提示的 TTN_NEEDTEXT 通知](handling-ttn-needtext-notification-for-tool-tips.md)
##### [TOOLTIPTEXT 结构](tooltiptext-structure.md)
### [工具栏](toolbars.md)
#### [工具栏示例列表](toolbar-sample-list.md)
#### [MFC 工具栏实现](mfc-toolbar-implementation.md)
##### [工具栏基础知识](toolbar-fundamentals.md)
##### [停靠和浮动工具栏](docking-and-floating-toolbars.md)
##### [工具栏工具提示](toolbar-tool-tips.md)
##### [使用工具栏控件](working-with-the-toolbar-control.md)
##### [使用旧工具栏](using-your-old-toolbars.md)
### [可视化管理器](visualization-manager.md)
### [Windows](windows.md)
#### [窗口对象](window-objects.md)
##### [C++ 窗口对象与 HWND 之间的关系](relationship-between-a-cpp-window-object-and-an-hwnd.md)
##### [派生的窗口类](derived-window-classes.md)
##### [创建窗口](creating-windows.md)
###### [注册窗口类](registering-window-classes.md)
###### [常用窗口创建序列](general-window-creation-sequence.md)
##### [销毁窗口对象](destroying-window-objects.md)
###### [窗口析构序列](window-destruction-sequence.md)
###### [分配和取消分配窗口内存](allocating-and-deallocating-window-memory.md)
##### [将 CWnd 从其 HWND 中分离出来](detaching-a-cwnd-from-its-hwnd.md)
##### [使用窗口对象](working-with-window-objects.md)
##### [设备上下文](device-contexts.md)
##### [图形对象](graphic-objects.md)
###### [对象的单步和两步构建](one-stage-and-two-stage-construction-of-objects.md)
###### [将图形对象选入设备上下文](selecting-a-graphic-object-into-a-device-context.md)
#### [框架窗口](frame-windows.md)
##### [框架窗口类](frame-window-classes.md)
###### [应用程序向导创建的框架窗口类](frame-window-classes-created-by-the-application-wizard.md)
##### [框架窗口样式 (C++)](frame-window-styles-cpp.md)
###### [更改 MFC 创建的窗口的样式](changing-the-styles-of-a-window-created-by-mfc.md)
##### [框架窗口的作用](what-frame-windows-do.md)
##### [使用框架窗口](using-frame-windows.md)
###### [创建文档框架窗口](creating-document-frame-windows.md)
###### [何时初始化 CWnd 对象](when-to-initialize-cwnd-objects.md)
###### [销毁框架窗口](destroying-frame-windows.md)
###### [管理 MDI 子窗口](managing-mdi-child-windows.md)
###### [管理当前视图](managing-the-current-view.md)
###### [管理菜单、控件条和加速键](managing-menus-control-bars-and-accelerators.md)
###### [拖放框架窗口中的文件](dragging-and-dropping-files-in-a-frame-window.md)
###### [响应动态数据交换 (DDE)](responding-to-dynamic-data-exchange-dde.md)
###### [协调其他窗口操作](orchestrating-other-window-actions.md)
## [Windows 套接字](windows-sockets.md)
### [MFC 中的 Windows 套接字](windows-sockets-in-mfc.md)
#### [Windows 套接字：背景](windows-sockets-background.md)
#### [Windows 套接字：流套接字](windows-sockets-stream-sockets.md)
#### [Windows 套接字：数据报套接字](windows-sockets-datagram-sockets.md)
#### [Windows 套接字：结合使用套接字和存档](windows-sockets-using-sockets-with-archives.md)
#### [Windows 套接字：操作顺序](windows-sockets-sequence-of-operations.md)
#### [Windows 套接字：使用存档的套接字示例](windows-sockets-example-of-sockets-using-archives.md)
#### [Windows 套接字：使用存档的套接字的工作原理](windows-sockets-how-sockets-with-archives-work.md)
#### [Windows 套接字：使用类 CAsyncSocket](windows-sockets-using-class-casyncsocket.md)
#### [Windows 套接字：从套接字类派生](windows-sockets-deriving-from-socket-classes.md)
#### [Windows 套接字：套接字通知](windows-sockets-socket-notifications.md)
#### [Windows 套接字：阻止](windows-sockets-blocking.md)
#### [Windows 套接字：字节排序](windows-sockets-byte-ordering.md)
#### [Windows 套接字：转换字符串](windows-sockets-converting-strings.md)
#### [Windows 套接字：端口和套接字地址](windows-sockets-ports-and-socket-addresses.md)
## [Win32 Internet 扩展 (WinInet)](win32-internet-extensions-wininet.md)
### [WinInet 如何简化 Internet 客户端应用程序的创建](how-wininet-makes-it-easier-to-create-internet-client-applications.md)
### [MFC 如何简化 Internet 客户端应用程序的创建](how-mfc-makes-it-easier-to-create-internet-client-applications.md)
### [用于创建 Internet 客户端应用程序的 MFC 类](mfc-classes-for-creating-internet-client-applications.md)
### [Internet 客户端类的必备条件](prerequisites-for-internet-client-classes.md)
### [使用 MFC WinInet 类编写 Internet 客户端应用程序](writing-an-internet-client-application-using-mfc-wininet-classes.md)
### [典型 Internet 客户端应用程序中的步骤](steps-in-a-typical-internet-client-application.md)
### [典型 FTP 客户端应用程序中的步骤](steps-in-a-typical-ftp-client-application.md)
### [典型 FTP 客户端应用程序中的文件删除步骤](steps-in-a-typical-ftp-client-application-to-delete-a-file.md)
### [典型 Gopher 客户端应用程序中的步骤](steps-in-a-typical-gopher-client-application.md)
### [典型 HTTP 客户端应用程序中的步骤](steps-in-a-typical-http-client-application.md)
# [层次结构图](hierarchy-chart.md)
## [层次结构图类别](hierarchy-chart-categories.md)
# [MFC 自定义](customization-for-mfc.md)
## [键盘和鼠标自定义](keyboard-and-mouse-customization.md)
## [用户定义的工具](user-defined-tools.md)
## [自定义对安全有何影响](security-implications-of-customization.md)
# [MFC 技术说明](mfc-technical-notes.md)
## [按类别列出的技术说明](technical-notes-by-category.md)
## [按编号列出的技术说明](technical-notes-by-number.md)
## [TN001：窗口类注册](tn001-window-class-registration.md)
## [TN002：永久性对象数据格式](tn002-persistent-object-data-format.md)
## [TN003：将 Windows 句柄映射到对象](tn003-mapping-of-windows-handles-to-objects.md)
## [TN006：消息映射](tn006-message-maps.md)
## [TN011：将 MFC 作为 DLL 的一部分使用](tn011-using-mfc-as-part-of-a-dll.md)
## [TN014：自定义控件](tn014-custom-controls.md)
## [TN016：结合使用 C++ 多重继承和 MFC](tn016-using-cpp-multiple-inheritance-with-mfc.md)
## [TN017：销毁窗口对象](tn017-destroying-window-objects.md)
## [TN020：ID 命名和编号约定](tn020-id-naming-and-numbering-conventions.md)
## [TN021：命令和消息路由](tn021-command-and-message-routing.md)
## [TN022：标准命令实现](tn022-standard-commands-implementation.md)
## [TN023：标准 MFC 资源](tn023-standard-mfc-resources.md)
## [TN024：MFC 定义的消息和资源](tn024-mfc-defined-messages-and-resources.md)
## [TN025：文档、视图和框架创建](tn025-document-view-and-frame-creation.md)
## [TN026：DDX 和 DDV 例程](tn026-ddx-and-ddv-routines.md)
## [TN028：上下文相关帮助支持](tn028-context-sensitive-help-support.md)
## [TN029：拆分器窗口](tn029-splitter-windows.md)
## [TN030：自定义打印和打印预览](tn030-customizing-printing-and-print-preview.md)
## [TN031：控件条](tn031-control-bars.md)
## [TN032：MFC 异常机制](tn032-mfc-exception-mechanism.md)
## [TN033：MFC 的 DLL 版本](tn033-dll-version-of-mfc.md)
## [TN035：在 Visual C++ 中使用多个资源文件和头文件](tn035-using-multiple-resource-files-and-header-files-with-visual-cpp.md)
## [TN036：将 CFormView 与 AppWizard 和 ClassWizard 结合使用](tn036-using-cformview-with-appwizard-and-classwizard.md)
## [TN037：多线程 MFC 2.1 应用程序](tn037-multithreaded-mfc-2-1-applications.md)
## [TN038：MFC-OLE IUnknown 实现](tn038-mfc-ole-iunknown-implementation.md)
## [TN039：MFC-OLE 自动化实现](tn039-mfc-ole-automation-implementation.md)
## [TN040：MFC-OLE 就地调整大小和缩放](tn040-mfc-ole-in-place-resizing-and-zooming.md)
## [TN041：MFC-OLE1 迁移到 MFC-OLE 2](tn041-mfc-ole1-migration-to-mfc-ole-2.md)
## [TN042：ODBC 驱动程序开发人员建议](tn042-odbc-driver-developer-recommendations.md)
## [TN043：RFX 例程](tn043-rfx-routines.md)
## [TN044：针对 DBCS 的 MFC 支持](tn044-mfc-support-for-dbcs.md)
## [TN045：针对 Long Varchar-Varbinary 的 MFC-数据库支持](tn045-mfc-database-support-for-long-varchar-varbinary.md)
## [TN046：MFC 类的注释约定](tn046-commenting-conventions-for-the-mfc-classes.md)
## [TN047：放宽数据库事务要求](tn047-relaxing-database-transaction-requirements.md)
## [TN048：编写 MFC 数据库应用程序的 ODBC 安装程序和管理程序](tn048-writing-odbc-setup-and-administration-programs.md)
## [TN049：MFC-OLE MBCS 到 Unicode 转换层 (MFCANS32)](tn049-mfc-ole-mbcs-to-unicode-translation-layer-mfcans32.md)
## [TN050：MFC-OLE 常见对话框 (MFCUIx32)](tn050-mfc-ole-common-dialogs-mfcuix32.md)
## [TN051：现在和将来使用 CTL3D](tn051-using-ctl3d-now-and-in-the-future.md)
## [TN053：DAO 数据库类的自定义 DFX 例程](tn053-custom-dfx-routines-for-dao-database-classes.md)
## [TN054：使用 MFC DAO 类时直接调用 DAO](tn054-calling-dao-directly-while-using-mfc-dao-classes.md)
## [TN055：将 MFC ODBC 数据库类应用程序迁移到 MFC DAO 类](tn055-migrating-mfc-odbc-database-class-applications-to-mfc-dao-classes.md)
## [TN056：安装本地化的 MFC 组件](tn056-installation-of-localized-mfc-components.md)
## [TN057：MFC 组件本地化](tn057-localization-of-mfc-components.md)
## [TN058：MFC 模块状态实现](tn058-mfc-module-state-implementation.md)
## [TN059：使用 MFC MBCS-Unicode 转换宏](tn059-using-mfc-mbcs-unicode-conversion-macros.md)
## [TN060：新 Windows 公共控件](tn060-the-new-windows-common-controls.md)
## [TN061：ON_NOTIFY 和 WM_NOTIFY 消息](tn061-on-notify-and-wm-notify-messages.md)
## [TN062：Windows 控件的消息反射](tn062-message-reflection-for-windows-controls.md)
## [TN063：调试 Internet MFC 扩展 DLL](tn063-debugging-internet-extension-dlls.md)
## [TN064：ActiveX 控件中的单元模型线程](tn064-apartment-model-threading-in-activex-controls.md)
## [TN065：针对 OLE 自动化服务器的双重接口支持](tn065-dual-interface-support-for-ole-automation-servers.md)
## [TN066：从 MFC 3.x 到 4.0 的常见移植问题](tn066-common-mfc-3-x-to-4-0-porting-issues.md)
## [TN068：使用 Microsoft Access 7 ODBC 驱动程序执行事务](tn068-performing-transactions-with-the-microsoft-access-7-odbc-driver.md)
## [TN070：MFC 窗口类名](tn070-mfc-window-class-names.md)
## [TN071：MFC IOleCommandTarget 实现](tn071-mfc-iolecommandtarget-implementation.md)
# [类库概述](class-library-overview.md)
## [常规类设计理念](general-class-design-philosophy.md)
### [应用程序框架](application-framework.md)
### [与 C 语言 API 的关系](relationship-to-the-c-language-api.md)
## [根类：CObject](root-class-cobject.md)
## [MFC 应用程序体系结构类](mfc-application-architecture-classes.md)
### [应用程序和线程支持类](application-and-thread-support-classes.md)
### [命令传送类](command-routing-classes.md)
### [文档类](document-classes.md)
### [视图类（体系结构）](view-classes-architecture.md)
### [框架窗口类（体系结构）](frame-window-classes-architecture.md)
### [文档模板类](document-template-classes.md)
## [窗口、对话框和控件类](window-dialog-and-control-classes.md)
### [框架窗口类 (Windows)](frame-window-classes-windows.md)
### [视图类 (Windows)](view-classes-windows.md)
### [对话框类](dialog-box-classes.md)
### [控件类](control-classes.md)
### [控件条类](control-bar-classes.md)
## [绘制和打印类](drawing-and-printing-classes.md)
### [输出（设备上下文）类](output-device-context-classes.md)
### [绘图工具类](drawing-tool-classes.md)
## [简单数据类型类](simple-data-type-classes.md)
## [数组、列表和映射类](array-list-and-map-classes.md)
### [数组、列表和映射的模板类](template-classes-for-arrays-lists-and-maps.md)
### [随时可用的数组类](ready-to-use-array-classes.md)
### [随时可用的列表类](ready-to-use-list-classes.md)
### [随时可用的映射类](ready-to-use-map-classes.md)
## [文件和数据库类](file-and-database-classes.md)
### [文件 I-O 类](file-i-o-classes.md)
### [OLE DB 类](ole-db-classes.md)
### [DAO 类](dao-classes.md)
### [ODBC 类](odbc-classes.md)
## [Internet 和网络类](internet-and-networking-classes.md)
### [Windows 套接字类](windows-sockets-classes.md)
### [Win32 Internet 类](win32-internet-classes.md)
## [OLE 类](ole-classes.md)
### [OLE 容器类](ole-container-classes.md)
### [OLE 服务器类](ole-server-classes.md)
### [OLE 拖放和数据传输类](ole-drag-and-drop-and-data-transfer-classes.md)
### [OLE 通用对话框类](ole-common-dialog-classes.md)
### [OLE 自动化类](ole-automation-classes.md)
### [OLE 控件类](ole-control-classes.md)
### [活动文档类](active-document-classes.md)
### [OLE 相关类](ole-related-classes.md)
## [调试和异常类](debugging-and-exception-classes.md)
### [调试支持类](debugging-support-classes.md)
### [异常类](exception-classes.md)
# [演练 (MFC)](walkthroughs-mfc.md)
## [演练：使用新 MFC Shell 控件](walkthrough-using-the-new-mfc-shell-controls.md)
## [演练：将控件添加到工具栏](walkthrough-putting-controls-on-toolbars.md)
## [演练：向 MFC 项目添加 D2D 对象](walkthrough-adding-a-d2d-object-to-an-mfc-project.md)
## [演练：向 MFC 项目添加动画](walkthrough-adding-animation-to-an-mfc-project.md)
# [引用](reference/toc.md)