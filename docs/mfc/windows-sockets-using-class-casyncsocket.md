...
1.  构造[CAsyncSocket](../mfc/reference/casyncsocket-class.md)对象，并使用该对象创建基础**套接字**处理。  
    ...
    -   "端口": 短整数。  
         ...
         对于服务器套接字，必须指定的端口。 对于客户端套接字，你通常会接受此参数，这样就 Windows 套接字选择一个端口的默认值。  
  
    -   套接字类型： **SOCK_STREAM** （默认值） 或**SOCK_DGRAM**。  
  
    -   套接字的"地址"，如"ftp.microsoft.com"或"128.56.22.8"。  
  
         这是你在网络上的 Internet 协议 (IP) 地址。 你可能始终将依赖于此参数的默认值。  
...
4.  销毁`CAsyncSocket`对象。  
     ...
     如果你在堆栈上创建的套接字对象，包含函数超出范围时调用其析构函数。 如果在堆上创建的套接字对象，使用**新**运算符，你将负责使用**删除**运算符需销毁对象。  
  
     析构函数调用对象的[关闭](../mfc/reference/casyncsocket-class.md#close)之前销毁对象的成员函数。  
...
-   Unicode 和多字节字符集之间进行转换设置 (MBCS) 字符串。  
