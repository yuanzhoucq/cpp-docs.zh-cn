...
框架窗口使用[如何更新用户界面对象](how-to-update-user-interface-objects.md)中所述的 ON_UPDATE_COMMAND_UI 机制参与更新用户界面项。 空循环期间，工具栏上的按钮和其他控件栏会进行更新。 菜单栏上的下拉菜单中的菜单项正好在菜单下拉之前进行更新。
...
[CMDIFrameWnd](reference/cmdiframewnd-class.md)提供为 MDI 应用程序显示的 "窗口" 菜单上的标准命令的默认实现。 具体而言，实现“新建窗口”命令 (ID_WINDOW_NEW) 来创建新的框架窗口和当前文档的视图。 仅在需要高级自定义时重写这些实现。

同一文档类型的多个 MDI 子窗口共享菜单资源。 如果多个 MDI 子窗口是由同一个文档模板创建的，则它们可使用同一个菜单资源，并保存在 Windows 系统资源中。

## <a name="managing-the-status-bar"></a>管理状态栏

框架窗口还将状态栏置于其客户端区域内并管理状态栏的指示符。 框架窗口根据需要清除并更新状态栏中的消息区域，并在用户选择菜单项或工具栏按钮时显示提示字符串，如[如何在状态栏中显示命令信息](how-to-display-command-information-in-the-status-bar.md)中所述。
