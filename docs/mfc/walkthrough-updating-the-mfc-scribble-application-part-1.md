...
- 使用[资源编辑器](../windows/resource-editors.md)来创造自己的位图。 或使用资源编辑器从 Visual Studio 随附的可移植网络图形 (.png) 映像中组装位图, 并可从[Visual studio 图像库](https://docs.microsoft.com/visualstudio/designers/the-visual-studio-image-library)下载。
    ...
    但**功能区**用户界面需要某些位图支持透明图像。 透明位图使用32位像素, 其中24位指定颜色的红色、绿色和蓝色分量, 8 位定义指定颜色透明度的*alpha 通道*。 当前资源编辑器可以查看, 但不能修改包含32位像素的位图。 因此, 请使用外部图像编辑器而不是资源编辑器来操作透明位图。
...
1. 导入附加到**应用程序**按钮的 "文件" 菜单的位图。
   ...
   1. 导入 filesmall 文件, 其中包含11个 16x16 (16x176) 图像。 将位图的 ID 更改为`IDB_RIBBON_FILESMALL`。

   > [!NOTE]
   > 由于我们只需要前八个16x16 图像 (16x128), 因此可以选择从176到128裁剪此位图的右端宽度。

   1. 导入 filelarge, 其中包含九个 32x32 (32x288) 映像。 将位图的 ID 更改为`IDB_RIBBON_FILELARGE`。
...
1. 导入已调整大小的功能区面板的位图。 如果功能区太小而无法显示整个面板, 则在调整大小操作后使用这些位图或面板图标。
   ...
   1. 导入 panelicons 位图, 其中包含八个16x16 图像。 在**位图编辑器**的 "**属性**" 窗口中, 将位图的宽度调整为 64 (16x64)。 将位图的 ID 更改为`IDB_PANEL_ICONS`。
...
1. 以下修改创建用户单击**应用程序**按钮时显示的菜单。 单击 "**主要项**" 旁边的省略号 ( **...** ) 以打开 "**项编辑器**"。
   ...
   1. 在选中 "**项**类型"**按钮**的情况下, 单击 "**添加**" 以添加一个按钮。 将**标题**更改`&New`为, 将`ID_FILE_NEW`" **ID** " `0`更改为, 并`0`将图像更改为**大**到。

   1. 单击 "**添加**" 以添加一个按钮。 将**标题**更改`&Save`为, 将`ID_FILE_SAVE`" `2` **ID** " 更改为, 并将图像更改为大到。 `2`

   1. 单击 "**添加**" 以添加一个按钮。 将**标题**更改`Save &As`为, 将`ID_FILE_SAVE_AS`" `3` **ID** " 更改为, 并将图像更改为大到。 `3`

   1. 单击 "**添加**" 以添加一个按钮。 将**标题**更改`&Print`为, 将`ID_FILE_PRINT`" `4` **ID** " 更改为, 并将图像更改为大到。 `4`

   1. 将**项目**类型更改为 "**分隔符**", 然后单击 "**添加**"。

   1. 将**项**类型更改为**按钮**。 单击 "**添加**" 以添加第五个按钮。 将**标题**更改`&Close`为, 将`ID_FILE_CLOSE`" `5` **ID** " 更改为, 并将图像更改为大到。 `5`

1. 以下修改将在上一步中创建的 "**打印**" 按钮下创建一个子菜单。

   1. 单击 "**打印**" 按钮, 将 "**项**类型" 更改为 "**标签**", 然后单击 "**插入**"。 将**标题**更改`Preview and print the document`为。

   1. 单击 "**打印**" 按钮, 将 "**项**类型" 更改为 "**按钮**", 然后单击 "**插入**"。 将**标题**更改`&Print`为, 将`ID_FILE_PRINT`" `4` **ID** " 更改为, 并将图像更改为大到。 `4`

   1. 单击 "**打印**" 按钮, 然后单击 "**插入**" 以添加一个按钮。 将**标题**更改`&Quick Print`为, 将`ID_FILE_PRINT_DIRECT`" `7` **ID** " 更改为, 并将图像更改为大到。 `7`

   1. 单击 "**打印**" 按钮, 然后单击 "**插入**" 以添加其他按钮。 将**标题**更改`Print Pre&view`为, 将`ID_FILE_PRINT_PREVIEW`" `6` **ID** " 更改为, 并将图像更改为大到。 `6`
...
1. 以下修改创建显示在**应用程序**按钮菜单底部的 "退出" 按钮。
   ...
   1. 在选中 "**项**类型"**按钮**的情况下, 单击 "**添加**" 以添加一个按钮。 将**标题**更改`E&xit`为 `ID_APP_EXIT` ,`8`并将其更改为。
...
1. 在 "**编辑**" 面板中, 添加负责清除文档内容的按钮。 此按钮的消息 ID 已经在`IDR_SCRIBBTYPE`菜单资源中定义。 指定`Clear All`为按钮文本和修饰按钮的位图的索引。 打开 "**工具箱**", 然后将一个**按钮**拖到 "**编辑**" 面板。 单击该按钮, 然后将`ID_EDIT_CLEAR_ALL`**大图像索引** `Clear All` `0` 改为,并将"标题"更改为"索引到",将"索引"更改`0`为。
