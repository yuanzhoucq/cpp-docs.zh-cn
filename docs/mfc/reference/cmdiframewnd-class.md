...
-   MDI 框架窗口管理**MDICLIENT**窗口中，其进行重新定位控件条结合。 MDI 客户端窗口是直接 MDI 子框架窗口的父级。 **WS_HSCROLL**和**WS_VSCROLL**中指定的窗口样式`CMDIFrameWnd`不适用于 MDI 客户端窗口而不是主框架窗口，以便用户可以向上滚动 MDI 客户端 （如下所示 Windows 程序管理器区域，例如）。  
