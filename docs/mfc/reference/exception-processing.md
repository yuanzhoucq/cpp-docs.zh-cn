...
为了支持数据库异常，数据库类提供了两个异常类： `CDBException` 和 `CDaoException`，以及支持异常类型的全局函数：
...
如果*exception_class*为类 `CException`，则将捕获所有异常类型。 您可以使用[CObject：： IsKindOf](../../mfc/reference/cobject-class.md#iskindof)成员函数确定引发的特定异常。 捕获几种异常的更好方法是使用顺序**AND_CATCH**语句，每个语句都有不同的异常类型。
...
> [!NOTE]
>  **AND_CATCH**块定义为C++范围（用大括号分隔）。 如果在此范围中声明变量，请记住它们只能在该范围内访问。 这也适用于*exception_object_pointer_name*变量。
...
通常在 `AfxMessageBox` 向用户报告错误后立即调用此函数。
...
有关与 MFC DAO 类相关的异常的信息，请参阅本书中的类 `CDaoException` 和文章[异常：数据库异常](../../mfc/exceptions-database-exceptions.md)。
...
调用此函数可从你自己的代码中引发 `CDBException` 类型的异常。
...
*板*<br/>
一个指向 `CDatabase` 对象的指针，该对象表示与异常相关联的数据源连接。
...
框架从对 ODBC API 函数的调用接收 ODBC RETCODE 时调用 `AfxThrowDBException`，并将 RETCODE 解释为异常条件，而不是 expectable 错误。 例如，由于磁盘读取错误，数据访问操作可能会失败。
...
当出现严重错误（例如无法处理的未捕获的异常）时，MFC 成员函数在内部调用 `AfxAbort`。 如果遇到无法恢复的灾难性错误，可以在极少数情况下调用 `AfxAbort`。
