...
|[CMFCRibbonPanel::GetData](#getdata)|返回与面板关联的用户定义数据。|  
...
|[CMFCRibbonPanel::GetVisibleElements](#getvisibleelements)|获取可见的元素的数组。|  
|[CMFCRibbonPanel::HasElement](#haselement)||  
|[CMFCRibbonPanel::HitTest](#hittest)||  
|[CMFCRibbonPanel::HitTestEx](#hittestex)||  
|[CMFCRibbonPanel::Insert](#insert)|在给定位置插入一个功能区元素。|  
|[CMFCRibbonPanel::InsertSeparator](#insertseparator)|在给定位置插入分隔符。|  
|[CMFCRibbonPanel::IsCenterColumnVert](#iscentercolumnvert)|指定按列的是否所有面板元素应该都居中 （对齐） 垂直，。|  
...
|[CMFCRibbonPanel::Remove](#remove)|移除并根据需要删除位于指定索引处的元素。|  
|[CMFCRibbonPanel::RemoveAll](#removeall)|从功能区面板中移除所有元素。|  
|[CMFCRibbonPanel::Replace](#replace)|与另一个根据其各自的索引值替换一个元素。|  
|[CMFCRibbonPanel::ReplaceByID](#replacebyid)|一个元素替换另一个基于指定的命令 ID|  
|[CMFCRibbonPanel::SetCenterColumnVert](#setcentercolumnvert)|面板元素垂直对齐，按列进行排序。|  
|[CMFCRibbonPanel::SetData](#setdata)|将用户定义数据，功能区面板。|  
|[CMFCRibbonPanel::SetElementMenu](#setelementmenu)|将弹出菜单分配给具有给定的命令 ID 元素|  
|[CMFCRibbonPanel::SetElementRTC](#setelementrtc)|将添加到功能区面板提供的运行时类信息由指定的功能区元素。|  
|[CMFCRibbonPanel::SetElementRTCByID](#setelementrtcbyid)|将添加到功能区面板提供的运行时类信息由指定的功能区元素。|  
|[CMFCRibbonPanel::SetFocused](#setfocused)|设置焦点移到指定的功能区元素。|  
|[CMFCRibbonPanel::SetJustifyColumns](#setjustifycolumns)|启用或禁用列理由。|  
|[CMFCRibbonPanel::SetKeys](#setkeys)|设置显示功能区面板的键盘快捷键。|  
|[CMFCRibbonPanel::ShowPopup](#showpopup)||  
  
## <a name="remarks"></a>备注  
 功能区面板是相关的任务在功能区类别中创建的逻辑分组。 当功能区更改容量时，面板布局会自动调整以显示尽可能多的元素。  
  
 你可以获取功能区面板，通过调用包含在功能区类别[CMFCRibbonCategory::GetPanel](../../mfc/reference/cmfcribboncategory-class.md#getpanel)方法。  
  
## <a name="example"></a>示例  
 下面的示例演示如何配置`CMFCRibbonPanel`通过使用中的各种方法的对象`CMFCRibbonPanel`类。 该示例演示如何设置显示功能区面板的键盘快捷方式、 垂直对齐面板中的元素，按列，并启用列理由。 此代码片段属于[MS Office 2007 演示示例](../../visual-cpp-samples.md)。  
...
### <a name="remarks"></a>备注  
 调用此方法可添加到功能区面板的分隔符。 将已添加到以前的调用的功能区元素旁添加分隔符[cmfcribbonpanel:: Add](#add)。 若要在给定的位置插入分隔符，调用[CMFCRibbonPanel::InsertSeparator](#insertseparator)。  
...
### <a name="remarks"></a>备注  
 有关工具栏、 热图、 冷图像和已禁用的映像的详细信息，请参阅[CMFCToolBar 类](../../mfc/reference/cmfctoolbar-class.md)。  
...
### <a name="return-value"></a>返回值  
 由指定的命令 ID; 标识功能区元素否则为`NULL`如果任何功能区元素不标识与指定的命令 id。  
...
### <a name="remarks"></a>备注  
 如果功能区面板具有足够的空间来显示其功能区元素，则显示默认按钮。  
...
### <a name="return-value"></a>返回值  
 指向基功能区元素的有效指针位于位置`nIndex`在功能区面板中，或`NULL`如果没有指定索引处的元素。  
...
### <a name="remarks"></a>备注  
 此方法始终返回 `FALSE`。 重写此方法以检索功能区面板弹出菜单的首选的显示矩形。  
  
##  <a name="getpressed"></a>  CMFCRibbonPanel::GetPressed  
 如果用户当前按下它，检索指向功能区面板上的功能区元素的指针。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果功能区面板包含指定的功能区元素;否则为`FALSE`。  
...
### <a name="parameters"></a>参数  
 [in] `bHighlight`  
 `TRUE` 以突出显示功能区面板中;`FALSE`以 unhighlight 功能区面板。  
  
 [in] `point`  
 X 和 y 坐标的指针，相对于窗口的左上角。  
...
### <a name="parameters"></a>参数  
 ...
 `TRUE` 若要测试的功能区面板标题;否则为`FALSE`。  
  
### <a name="return-value"></a>返回值  
 如果指定的点位于它; 的功能区元素的指针否则为`NULL`。  
...
### <a name="return-value"></a>返回值  
 具有位于它; 的指定的点的功能区元素的从零开始的索引否则为-1。  
  
### <a name="remarks"></a>备注  
 进行测试的功能区面板中包含的功能区元素。  
  
##  <a name="insert"></a>  CMFCRibbonPanel::Insert  
 指定功能区元素的位置插入指定数组中的功能区元素包含在功能区面板。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果成功，则插入功能区元素否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 如果值`nIndex`为-1，或者如果`nIndex`等于数组中的功能区元素的数目，指定功能区元素添加到数组末尾。 如果值`nIndex`是超出范围，该方法将失败。  
...
### <a name="remarks"></a>备注  
 调用此方法以在指定的位置插入分隔符`nIndex`。 若要插入旁边的最新添加的功能区元素分隔符，调用[CMFCRibbonPanel::AddSeparator](#addseparator)。  
  
##  <a name="iscentercolumnvert"></a>  CMFCRibbonPanel::IsCenterColumnVert  
 指示是否在其显示矩形内居中的功能区元素的垂直位置。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果垂直定位的功能区元素为中心其显示矩形; 中否则为`FALSE`。  
  
##  <a name="iscollapsed"></a>  CMFCRibbonPanel::IsCollapsed  
 指示是否显示大小的功能区面板最小化沿水平方向。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果突出显示的功能区面板显示;，否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 当鼠标指针位于突出显示功能区面板的显示。  
  
##  <a name="isjustifycolumns"></a>  CMFCRibbonPanel::IsJustifyColumns  
 指示是否在功能区面板中的同一列中的功能区元素的显示尺寸设置为相同的宽度。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果在功能区面板中的同一列中的功能区元素的显示尺寸设置为相同的宽度;否则为`FALSE`。  
...
### <a name="remarks"></a>备注  
 此方法始终返回 `FALSE`。 重写此方法，以指示功能区面板是否主功能区面板。  
  
 当用户选择的应用程序按钮时，将显示主功能区面板。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果删除元素并将其删除 (如果`bDelete`是`TRUE`);`FALSE`如果不移除元素，或者如果没有任何功能区元素位于`nIndex`。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果原始的功能区元素已替换为已成功通过新的功能区元素;`FALSE`如果不替换功能区元素或如果没有指定索引处的元素。  
  
### <a name="remarks"></a>备注  
 若要通过命令 ID 替换功能区元素，调用[CMFCRibbonPanel::ReplaceByID](#replacebyid)。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果原始的功能区元素已替换为已成功通过新的功能区元素;`FALSE`未替换的功能区元素是否与指定的命令 ID 的元素不实际存在。  
  
### <a name="remarks"></a>备注  
 若要替换基于位置的功能区元素，调用[CMFCRibbonPanel::Replace](#replace)。  
  
##  <a name="setcentercolumnvert"></a>  CMFCRibbonPanel::SetCenterColumnVert  
 启用或禁用使其显示矩形中的功能区元素的垂直位置内居中。  
...
### <a name="parameters"></a>参数  
 [in] `bSet`  
 `TRUE` 若要使其显示矩形; 中的功能区元素的垂直位置居中`FALSE`若要禁用此功能。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `hMenu`  
 指定要添加到功能区面板 Windows 菜单的句柄。  
  
 [in] `bIsDefautCommand`  
 `TRUE` 若要指定是否在单击功能区元素应执行与功能区元素关联的命令。 在这种情况下，仅打开菜单，当用户单击功能区元素旁边的箭头。 `FALSE` 若要指定是否在单击功能区元素不应执行与功能区元素关联的命令。 在这种情况下，无论其中用户单击此元素将显示的弹出菜单。  
  
 [in] `bRightAlign`  
 `TRUE` 若要指定弹出菜单是右对齐;否则为`FALSE`。  
  
 [in] `uiMenuResID`  
 指定要将添加到功能区面板菜单上的资源 ID。  
  
### <a name="return-value"></a>返回值  
 `TRUE` 如果向功能区元素，则分配了菜单否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 调用此方法以将弹出菜单分配给功能区元素具有给定的命令 id。  
  
##  <a name="setelementrtc"></a>  CMFCRibbonPanel::SetElementRTC  
 将添加到功能区面板提供的运行时类信息由指定的功能区元素。  
...
### <a name="parameters"></a>参数  
 ...
 [in][out] `pRTC`  
 指向功能区元素添加到功能区面板的运行时类信息的指针。  
  
### <a name="return-value"></a>返回值  
 使用指定的运行时类信息创建功能区元素。  
  
### <a name="remarks"></a>备注  
 如果你想要将自定义元素 （例如，颜色按钮） 添加到功能区面板，你必须指定自定义元素的运行时类信息。 功能区存储此信息、 创建自定义的元素，并替换现有元素位于 （由标识） 指定的命令 id。 功能区然后返回一个指向新创建的元素。  
...
### <a name="parameters"></a>参数  
 ...
 [in][out] `pRTC`  
 指向功能区元素添加到功能区面板与关联的运行时类信息的指针。  
  
### <a name="return-value"></a>返回值  
 使用指定的运行时类信息创建功能区元素。  
  
### <a name="remarks"></a>备注  
 如果你想要将自定义元素 （例如，颜色按钮） 添加到功能区面板，你必须指定自定义元素的运行时类信息。 功能区存储此信息、 创建自定义的元素，并替换现有元素位于通过指定的命令 id。 然后会将指针返回新创建的元素。  
...
### <a name="parameters"></a>参数  
 [in] `bSet`  
 `TRUE` 调整列; 中的最大功能区元素的宽度的同一列中的功能区元素的宽度`FALSE`若要禁用此宽度调整。  
  
### <a name="remarks"></a>备注  
 在功能区面板中启用此功能时，同一列中的功能区元素的宽度调整到同一列中的最大功能区元素的宽度。  
...
### <a name="return-value"></a>返回值  
 如果此方法成功，则功能区面板的弹出菜单的指针否则为`NULL`。  
  
### <a name="remarks"></a>备注  
 折叠的功能区面板显示时，功能区面板的弹出菜单才可用。  
...
### <a name="return-value"></a>返回值  
 `TRUE` 如果父功能区具有 Windows 7 查找;否则为`FALSE`。  
