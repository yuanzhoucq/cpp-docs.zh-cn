---
title: CMFCRibbonPanel 类 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
|[CMFCRibbonPanel::FindByID](#findbyid)|返回由指定的命令 ID 来标识对应的元素|  
...
|[CMFCRibbonPanel::InsertSeparator](#insertseparator)|给定位置处插入分隔符。|  
|[CMFCRibbonPanel::IsCenterColumnVert](#iscentercolumnvert)|指定是否所有面板元素应该都居中 （对齐） 垂直列。|  
...
|[CMFCRibbonPanel::SetData](#setdata)|将用户定义数据与功能区面板。|  
...
|[CMFCRibbonPanel::SetKeys](#setkeys)|设置显示在功能区面板的键盘快捷键。|  
|[CMFCRibbonPanel::ShowPopup](#showpopup)||  
  
## <a name="remarks"></a>备注  
 功能区面板是创建功能区类别中的相关任务的逻辑分组。 为功能区更改的大小，面板布局自动调整以尽可能显示尽可能多的元素。  
  
 获取功能区面板中功能区类别包含通过调用[CMFCRibbonCategory::GetPanel](../../mfc/reference/cmfcribboncategory-class.md#getpanel)方法。  
...
### <a name="remarks"></a>备注  
 调用此方法以将分隔符添加到功能区面板。 将通过以前调用已添加的功能区元素旁边添加分隔符[cmfcribbonpanel:: Add](#add)。 若要在给定位置处插入一个分隔符，请调用[CMFCRibbonPanel::InsertSeparator](#insertseparator)。  
...
### <a name="return-value"></a>返回值  
 调用此方法以将工具栏添加到功能区面板。 工具栏将添加到先前的调用添加的功能区元素旁边[cmfcribbonpanel:: Add](#add)。  
  
### <a name="remarks"></a>备注  
 有关工具栏、 热映像、 冷映像和已禁用的映像的详细信息，请参阅[CMFCToolBar 类](../../mfc/reference/cmfctoolbar-class.md)。  
...
### <a name="return-value"></a>返回值  
 由指定的命令 ID; 标识功能区元素否则为 NULL，如果没有的功能区元素标识与指定的命令 id。  
...
### <a name="remarks"></a>备注  
 功能区面板具有足够的空间来显示其功能区元素时，将显示默认按钮。  
...
### <a name="return-value"></a>返回值  
 具有已下拉; 其弹出菜单的功能区元素的指针如果功能区元素不具有向下删除其弹出菜单，否则为 NULL。  
...
### <a name="return-value"></a>返回值  
 指向的基本功能区元素的有效指针位于位置*nIndex*在功能区面板或如果没有任何元素的指定索引处，则为 NULL。  
...
##  <a name="getindex"></a>  CMFCRibbonPanel::GetIndex  
 从功能区面板中包含的功能区元素的数组中检索指定功能区元素的从零开始的索引。  
...
### <a name="return-value"></a>返回值  
 指向用户当前按下它; 如果的功能区元素的指针否则为，为 NULL。  
...
### <a name="return-value"></a>返回值  
 如果功能区面板包含指定功能区元素，则为 TRUE否则为 FALSE。  
  
### <a name="remarks"></a>备注  
  
##  <a name="highlight"></a>  CMFCRibbonPanel::Highlight  
 设置为所选的功能区面板和指定点的功能区元素的突出显示颜色。  
...
### <a name="parameters"></a>参数  
 [in]*bHighlight*  
 为 TRUE，则突出显示功能区面板中;如果为 FALSE unhighlight 功能区面板。  
...
### <a name="parameters"></a>参数  
 ...
 测试功能区面板标题，则为 TRUE否则为 FALSE。  
  
### <a name="return-value"></a>返回值  
 如果指定的点位于它; 一个功能区元素的指针否则为，为 NULL。  
  
### <a name="remarks"></a>备注  
 测试包含在功能区面板添加的功能区元素。  
  
##  <a name="hittestex"></a>  CMFCRibbonPanel::HitTestEx  
 检索具有指定的点位于它的功能区元素的从零开始的索引。  
...
### <a name="return-value"></a>返回值  
 如果将功能区元素插入成功，则为 TRUE否则为 FALSE。  
  
### <a name="remarks"></a>备注  
 如果的值*nIndex*为-1，或者如果*nIndex*等于数组中的功能区元素的数目，则指定功能区元素添加到数组末尾。 如果的值*nIndex*是超出范围，该方法将失败。  
...
### <a name="return-value"></a>返回值  
 如果成功，则已插入分隔符，则返回 TRUE否则为 FALSE。  
  
### <a name="remarks"></a>备注  
 调用此方法以在指定的位置插入分隔符*nIndex*。 若要插入的分隔符旁边最近添加的功能区元素，请调用[CMFCRibbonPanel::AddSeparator](#addseparator)。  
  
##  <a name="iscentercolumnvert"></a>  CMFCRibbonPanel::IsCenterColumnVert  
 指示是否显示其矩形中居中的功能区元素的垂直位置。  
...
### <a name="return-value"></a>返回值  
 如果其显示矩形; 中居中的功能区元素的垂直位置，则返回 TRUE否则为 FALSE。  
  
##  <a name="iscollapsed"></a>  CMFCRibbonPanel::IsCollapsed  
 指示是否在功能区面板的显示大小最小化沿水平方向。  
...
### <a name="return-value"></a>返回值  
 如果显示大小的功能区面板已最小化沿水平方向; 则为 TRUE否则为 FALSE。  
...
### <a name="return-value"></a>返回值  
 如果突出显示功能区面板的显示; 则为 TRUE否则为 FALSE。  
...
### <a name="return-value"></a>返回值  
 如果功能区面板中的同一列中的功能区元素的显示尺寸设置为相同的宽度; 则为 TRUE否则为 FALSE。  
...
### <a name="remarks"></a>备注  
 此方法始终返回 FALSE。 重写此方法来指示功能区面板是主要的功能区面板。  
  
 当用户选择的应用程序按钮时显示主功能区面板。  
...
### <a name="parameters"></a>参数  
 [in]*nIndex*  
 指定从功能区面板中删除的元素的从零开始索引。  
  
 [in]*bDelete*  
 若要删除所移除; 的元素，则返回 TRUE否则为 FALSE。  
  
### <a name="return-value"></a>返回值  
 如果删除该元素并将其删除，则返回 TRUE (如果*bDelete*为 TRUE);如果未删除该元素或如果没有任何功能区元素位于属性为 FALSE *nIndex*。  
...
### <a name="return-value"></a>返回值  
 如果原始的功能区元素已替换为已成功将新的功能区元素; 则为 TRUE如果不替换功能区元素，或指定索引处没有任何元素，则为 FALSE。  
...
### <a name="return-value"></a>返回值  
 如果原始的功能区元素已替换为已成功将新的功能区元素; 则为 TRUE如果不替换功能区元素，或如果没有具有指定的命令的元素 FALSE ID 实际存在。  
...
### <a name="parameters"></a>参数  
 [in]*bSet*  
 为 TRUE，则中心及其显示矩形; 中的功能区元素的垂直位置如果为 FALSE 来禁用此功能。  
...
### <a name="parameters"></a>参数  
 [in]*uiCmdID*  
 指定在其中添加菜单的功能区元素的命令 ID。  
  
 [in]*hMenu*  
 指定要添加到功能区面板的 Windows 菜单的句柄。  
  
 [in]*bIsDefautCommand*  
 指定如果单击功能区元素时，应执行的功能区元素与关联的命令，则为 TRUE。 在这种情况下，当用户单击功能区元素旁边的箭头仅打开菜单。 为 FALSE，则指定是否单击功能区元素不应执行的功能区元素与关联的命令。 在这种情况下，弹出菜单会显示而不考虑当用户单击的元素上。  
  
 [in]*bRightAlign*  
 为 TRUE，则指定弹出菜单为右对齐;否则为 FALSE。  
  
 [in]*uiMenuResID*  
 指定要添加到功能区面板菜单中的资源 ID。  
  
### <a name="return-value"></a>返回值  
 如果菜单已分配给功能区元素，则为 TRUE否则为 FALSE。  
  
### <a name="remarks"></a>备注  
 调用此方法以将弹出菜单分配给功能区元素的具有给定的命令 id。  
...
### <a name="return-value"></a>返回值  
 通过使用指定的运行时类信息创建功能区元素。  
  
### <a name="remarks"></a>备注  
 如果你想要将自定义元素 （例如，颜色按钮） 添加到功能区面板，则必须指定自定义元素的运行时类信息。 在功能区存储此信息、 创建的自定义元素和替换的现有元素，位于 （按标识） 指定的命令 id。 在功能区然后返回一个指向新创建的元素。  
...
### <a name="parameters"></a>参数  
 ...
 [in][out]*pRTC*  
 指向添加到功能区面板的功能区元素与关联的运行时类信息的指针。  
  
### <a name="return-value"></a>返回值  
 通过使用指定的运行时类信息创建功能区元素。  
  
### <a name="remarks"></a>备注  
 如果你想要将自定义元素 （例如，颜色按钮） 添加到功能区面板，则必须指定自定义元素的运行时类信息。 在功能区存储此信息、 创建的自定义元素和替换现有元素位于由指定的命令 id。 它然后返回一个指向新创建的元素。  
...
### <a name="parameters"></a>参数  
 [in]*bSet*  
 若要调整为宽度的列; 中的最大功能区元素的同一列中的功能区元素的宽度，则返回 TRUE为 FALSE，则禁用此宽度调整。  
  
### <a name="remarks"></a>备注  
 在功能区面板中启用此功能后，同一列中的功能区元素的宽度调整为同一列中的最大功能区元素的宽度。  
...
### <a name="return-value"></a>返回值  
 如果此方法已成功，则在功能区面板的弹出菜单的指针否则为，为 NULL。  
  
### <a name="remarks"></a>备注  
 功能区面板的显示处于折叠状态时，功能区面板的弹出菜单才可用。  
