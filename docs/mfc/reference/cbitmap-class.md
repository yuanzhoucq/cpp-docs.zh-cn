...
|[CBitmap::CreateBitmapIndirect](#createbitmapindirect)|使用`BITMAP`结构中给定的宽度、高度和位模式 (如果已指定) 初始化对象。|
...
|[CBitmap::FromHandle](#fromhandle)|当给定 Windows `HBITMAP`位图的`CBitmap`句柄时, 返回指向对象的指针。|
|[CBitmap::GetBitmap](#getbitmap)|使用有关位图的信息填充结构。`BITMAP`|
...
有关使用图形对象 (如`CBitmap`) 的详细信息, 请参阅[图形对象](../../mfc/graphic-objects.md)。
...
由于颜色内存设备上下文可选择彩色或单色位图, 因此`CreateCompatibleBitmap`函数返回的位图的格式并不总是相同; 但是, nonmemory 设备上下文的兼容位图的格式始终在设备的格式。

完成通过`CreateCompatibleBitmap`函数创建的`CBitmap`对象后, 首先选择设备`CBitmap`上下文中的位图, 然后删除对象。
...
完成通过`CreateDiscardableBitmap`函数创建的`CBitmap`对象后, 首先选择设备`CBitmap`上下文中的位图, 然后删除对象。

##  <a name="fromhandle"></a>CBitmap:: FromHandle

当给定 Windows GDI 位图`CBitmap`的句柄时, 返回指向对象的指针。
...
如果成功, 则`CBitmap`为指向对象的指针; 否则为 NULL。

### <a name="remarks"></a>备注

如果对象尚未附加到句柄, 则会创建并附加`CBitmap`一个临时对象。 `CBitmap` 此临时`CBitmap`对象仅在下一次应用程序的事件循环中有空闲时间时才有效, 此时所有临时图形对象都会被删除。 指出这一点的另一种方法是: 只有在处理一条窗口消息的过程中, 临时对象才有效。
...
位图的宽度和高度 (以 0.1-毫米为单位)。 高度位于`cy` `CSize`对象的成员中, 并且宽度在`cx`成员中。 如果未使用`SetBitmapDimension`设置位图宽度和高度, 则返回值为0。
...
您可以使用[CGdiObject::D eleteobject](../../mfc/reference/cgdiobject-class.md#deleteobject)函数删除该`LoadBitmap` `CBitmap`函数加载的位图, 否则析构函数将删除该对象。
...
请注意, 必须在包括 WINDOWS 之前定义常量 OEMRESOURCE。H, 以便使用任何**OBM_** 常量。
...
先前的位图尺寸。 高度位于`CSize`对象的`cy`成员变量中, `cx`而 width 在成员变量中。
