...
## <a name="members"></a>成员  
 成员函数`CMapWordToOb`类似于类的成员函数[CMapStringToOb](../../mfc/reference/cmapstringtoob-class.md)。 由于此相似性，因此你可以使用 `CMapStringToOb` 参考文档获取成员函数细节。 无论您在何处`CString`或**const**指针，指向`char`作为函数参数或返回值，替换**WORD**。  
...
## <a name="remarks"></a>备注  
 ...
 如果您需要单独的转储**WORD** -  `CObject`元素，必须将转储上下文的深度设置为 1 或更高版本。  
  
 当`CMapWordToOb`对象被删除，或移除其元素时，`CObject`指针。 所引用的对象`CObject`指针不会被销毁。  
