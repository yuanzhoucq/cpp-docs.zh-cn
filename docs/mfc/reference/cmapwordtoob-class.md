...
## 成员  
 `CMapWordToOb` 的成员函数类似于选件类 [CMapStringToOb](../../mfc/reference/cmapstringtoob-class.md)的成员函数。  因此相似性，可以使用 `CMapStringToOb` 引用成员函数特定的文档。  无论在何处参见 `CString` 或 **const** 指向 `char` 作为函数参数或返回值，替代 **WORD**。  
...
## 备注  
 `CMapWordToOb` 合并 `IMPLEMENT_SERIAL` 宏支持序列化和转储其元素。  又序列化每个元素，如果将存储到存档，与重载中插入 \(**\<\<**\) 运算符或与 `Serialize` 成员函数。  
  
 如果需要转储单个 **WORD**\-`CObject` 元素，则必须将转储上下文的深度为1或更大。  
  
 当 `CMapWordToOb` 对象中移除时，或者，如果移除了其元素，移除 `CObject` 指针。  销毁 `CObject` 指针所引用的对象。  
  
 有关 `CMapWordToOb`的更多信息，请参见文章 [集合](../../mfc/collections.md)。  
