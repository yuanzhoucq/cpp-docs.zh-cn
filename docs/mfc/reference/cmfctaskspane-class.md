...
|[CMFCTasksPane::CreateMenu](#createmenu)|由框架创建的菜单调用**其他任务窗格**菜单按钮。|  
...
|[CMFCTasksPane::IsHistoryMenuButtonsEnabled](#ishistorymenubuttonsenabled)|指示是否**下一步**和**上一步**导航按钮具有下拉菜单。|  
...
5.  调用[cmfctaskspane:: Enablegroupcollapse](#enablegroupcollapse)以指定是否可以折叠项组。  
...
## <a name="example"></a>示例  
 下面的示例演示如何构造 `CMFCTasksPane` 对象和在 `CMFCTasksPane` 类中使用各种方法。 该示例演示如何启用任务组的折叠、 在上启用下拉菜单**下一步**和**上一步**导航按钮，启用而不是滚动条的滚动按钮，启用单词标签中文本的换行，设置任务窗格中，设置组标题的文本颜色以及设置水平和垂直边距的标题名称。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `nTaskIcon`  
 指定要显示任务旁边的图标。 框架将存储在映像的列表中的图标。 此参数是该列表的索引。  
  
 [in] `uiCommandID`  
 指定当用户单击任务时要执行该命令的命令 ID。 该任务被视为一个标签，如果`uiCommandID`为 0。  
...
### <a name="return-value"></a>返回值  
 窗口添加的组的从零开始索引或-1，如果通过指定的组`nGroup`不存在。  
...
### <a name="remarks"></a>备注  
 导航工具栏是框架显示任务窗格顶部的工具栏。 导航工具栏包含**回**，**转发**，和**主页**导航按钮和菜单按钮包含的可用页的列表。  
...
### <a name="remarks"></a>备注  
 当用户单击任务时，框架将调用此方法。 默认情况下，框架将检查与被单击的任务相关联的命令 ID，并且如果它不为零，将发送`WM_COMMAND`到任务窗格控件的所有者的消息。  
...
### <a name="remarks"></a>备注  
 此方法中删除单个组。 若要删除所有组，请调用[CMFCTasksPane::RemoveAllGroups](#removeallgroups)相反。  
...
### <a name="remarks"></a>备注  
 此方法更新任务窗格标题、 调整滚动条，将重新定位所有任务和重绘所有任务窗格控件。  
