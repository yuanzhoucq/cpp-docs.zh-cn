...
使用此运算符将`CFile`对象的句柄传递给需要`HANDLE`的 [ReadFileEx](/windows/win32/api/fileapi/nf-fileapi-readfileex) 和[getfiletime](/windows/win32/api/fileapi/nf-fileapi-getfiletime) 期间之类的函数。
...
*status*<br/>
包含新状态信息的缓冲区。 调用成员函数以 prefill 包含当前`CFileStatus`值的结构，然后根据需要进行更改。 `GetStatus` 如果值为0，则不更新相应的状态项。 有关`CFileStatus`结构的说明，请参阅[GetStatus](#getstatus)成员函数。
