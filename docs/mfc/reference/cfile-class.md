...
|[Cfile:: Getstatus](#getstatus)|检索打开的文件，或在静态版本的状态，检索指定的文件 （静态、 虚拟函数） 的状态。|  
|[CFile::LockRange](#lockrange)|锁定文件中的字节的范围。|  
|[CFile::Open](#open)|安全地使用错误测试的选项中打开一个文件。|  
|[CFile::Read](#read)|从当前的文件位置处的文件读取 （未缓冲） 数据。|  
|[CFile::Remove](#remove)|删除指定的文件 （静态函数）。|  
|[CFile::Rename](#rename)|重命名指定的文件 （静态函数）。|  
|[CFile::Seek](#seek)|将当前的文件指针定位。|  
|[CFile::SeekToBegin](#seektobegin)|将当前的文件指针置于文件的开头。|  
|[CFile::SeekToEnd](#seektoend)|将当前的文件指针置于文件末尾的定位。|  
|[CFile::SetFilePath](#setfilepath)|设置所选文件的完整文件路径。|  
|[CFile::SetLength](#setlength)|更改文件的长度。|  
|[CFile::SetStatus](#setstatus)|将指定的文件 （静态、 虚拟函数） 的状态设置。|  
|[CFile::UnlockRange](#unlockrange)|解除锁定文件中的字节的范围。|  
|[CFile::Write](#write)|写入到当前的文件位置的文件中的 （无缓冲） 数据。|  
...
## <a name="remarks"></a>备注  
 它直接提供未缓冲，二进制磁盘输入/输出服务，并间接支持文本文件和通过其派生类的内存文件。 `CFile`配合使用`CArchive`类，以支持 Microsoft 基础类对象的序列化。  
  
 此类，它的派生的类之间的层次关系允许您通过多态的所有文件对象上运行的程序`CFile`接口。 内存文件，例如，类似于磁盘文件。  
  
 使用`CFile`通用磁盘 I/O 及其派生类。 使用`ofstream`或其他 Microsoft iostream 类中的带格式的文本发送到磁盘文件。  
  
 通常情况下，自动在打开磁盘文件`CFile`构造和析构上已关闭。 静态成员函数允许您询问而无需打开该文件的文件的状态。  
...
### <a name="remarks"></a>备注  
 ...
 当处理异常、`CFile::Abort`区别`CFile::Close`在两个重要方面。 首先，**中止**函数不会引发异常有关失败原因失败，将忽略**中止**。 第二个，**中止**将不会**ASSERT**如果文件尚未打开或在以前已关闭。  
  
 如果您使用**新**分配`CFile`对象在堆中，则必须关闭文件后删除它。 **Abort** sets `m_hFile` to `CFile::hFileNull`.  
...
|`CFile::typeText`|通过对回车-换行对进行特殊处理，来设置文本模式（仅在派生类中使用）。|  
...
|`CFile::modeNoInherit`|阻止任何子进程使用文件句柄。|  
 ...
 默认构造函数将初始化成员，但不会将文件附加到 `CFile` 对象。 在使用此构造函数之后, 使用[CFile::Open](#open)方法打开文件并将其附加到`CFile`对象。  
...
### <a name="remarks"></a>备注  
 使用`Flush`并不保证进行刷新，`CArchive`缓冲区。 如果使用的存档，调用[CArchive::Flush](../../mfc/reference/carchive-class.md#flush)第一个。  
...
### <a name="remarks"></a>备注  
 例如，当您调用`GetFileName`生成一条消息给文件的相关用户`c:\windows\write\myfile.wri`，filename， `myfile.wri`，则返回。  
  
 若要返回整个文件的路径，包括名称、 调用[GetFilePath](#getfilepath)。 若要返回的文件的标题 ( `myfile`)，调用[GetFileTitle](#getfiletitle)。  
  
### <a name="example"></a>示例  
 此代码片段将打开系统。INI 文件的 WINDOWS 目录中。 如果找到，该示例将打印出的名称和路径和标题，如下面的输出所示︰  
  
 [!code-cpp[NVC_MFCFiles #&6;](../../atl-mfc-shared/reference/codesnippet/cpp/cfile-class_3.cpp)]  
  
##  <a name="a-namegetfilepatha--cfilegetfilepath"></a><a name="getfilepath"></a>CFile::GetFilePath  
 调用该成员函数以检索指定的文件的完整路径。  
...
### <a name="remarks"></a>备注  
 例如，当您调用`GetFilePath`生成一条消息给文件的相关用户`c:\windows\write\myfile.wri`，文件路径， `c:\windows\write\myfile.wri`，则返回。  
  
 返回只是该文件的名称 ( `myfile.wri`)，调用[GetFileName](#getfilename)。 若要返回的文件的标题 ( `myfile`)，调用[GetFileTitle](#getfiletitle)。  
...
### <a name="remarks"></a>备注  
 此方法调用[GetFileTitle](http://msdn.microsoft.com/library/windows/desktop/ms646924)检索文件的标题。 如果成功，则该方法返回系统将使用要向用户显示的文件名称的字符串。 否则，该方法调用[PathFindFileName](http://msdn.microsoft.com/library/windows/desktop/bb773589)以检索基础文件的文件名 （包括文件扩展名）。 因此，文件扩展名将始终将字符串中未包含返回的文件标题。 有关详细信息，请参阅[GetFileTitle](http://msdn.microsoft.com/library/windows/desktop/ms646924)和[PathFindFileName](http://msdn.microsoft.com/library/windows/desktop/bb773589)中[!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]。  
...
##  <a name="a-namegetstatusa--cfilegetstatus"></a><a name="getstatus"></a>Cfile:: Getstatus  
 此方法会检索状态信息与有关给定`CFile`对象实例或给定的文件路径。  
...
### <a name="parameters"></a>参数  
 `rStatus`  
 对用户提供的引用**CFileStatus**结构，它将接收的状态信息。 **CFileStatus**结构包含以下字段︰  
  
- **CTime m_ctime**创建该文件的日期和时间。  
  
- **CTime m_mtime**的日期和时间上次修改该文件。  
  
- **CTime m_atime**的日期和时间上一次访问该文件以进行读取。  
  
- **ULONGLONG m_size**以字节为单位，报告的 DIR 命令的文件的逻辑大小。  
...
- **char m_szFullName [_MAX_PATH]** Windows 字符集中的绝对文件名。  
 ...
 `lpszFileName`  
 在 Windows 字符集的字符串，它是设置所需的文件的路径。 路径可以是相对或绝对的或者它可以包含的网络路径名称。  
...
### <a name="remarks"></a>备注  
 非静态版本的**GetStatus**会检索打开的文件与相关联的状态信息给定`CFile`对象。  静态版本**GetStatus**从给定的文件路径获取文件状态，而无需实际打开该文件。 这是用于测试文件存在以及访问权限。  
  
 **M_attribute**的成员**CFileStatus**结构是指文件中的属性集。 `CFile`类提供了**属性**枚举类型，因此可以以符号方式指定的文件属性︰  
...
### <a name="remarks"></a>备注  
 锁定文件中的字节将阻止其他进程访问这些字节。 您可以锁定多个区域一个文件，但没有重叠区域允许。  
  
 如果解除锁定该区域，使用`UnlockRange`成员函数的字节范围必须与参数完全以前锁定的区域。 `LockRange`函数不会合并相邻区域; 如果有两种锁定的区域相邻，则必须单独解除锁定每个区域。  
  
> [!NOTE]
>  此函数不是可用于`CMemFile`的派生类。  
...
### <a name="remarks"></a>备注  
 `m_hFile`是类型的公共变量**UINT**。 它包含`CFile::hFileNull`（一个独立于操作系统的空文件指示符） 如果尚未分配句柄。  
  
 利用`m_hFile`建议不要，因为该成员的含义取决于派生的类。 `m_hFile`将成为公共成员，为方便起见，在支持非多态类的使用。  
...
### <a name="parameters"></a>参数  
 `lpszFileName`  
 一个字符串，是所需的文件的路径。 路径可以是相对、 绝对、 或网络名称 (UNC)。  
  
 `nOpenFlags`  
 一个**UINT** ，它定义的文件共享和访问模式。 它指定打开文件时要执行的操作。 您可以通过使用按位 OR 组合选项 ( **|** ) 运算符。 一个访问权限和一个共享选项是必需的;**modeCreate**和**modeNoInherit**模式都是可选的。 请参阅[CFile](#cfile)构造函数模式选项的列表。  
...
### <a name="remarks"></a>备注  
 两个函数形成了"安全"方法来打开一个文件，其中失败是正常、 预期的条件。  
  
 虽然`CFile`构造函数将在出现错误，引发的异常**打开**将返回**FALSE**错误条件。 **打开**仍可以初始化[CFileException](../../mfc/reference/cfileexception-class.md)对象，用于描述该错误，但是。 如果不提供`pError`参数，或者如果传递**NULL**为`pError`，**打开**将返回**FALSE**并且不会引发`CFileException`。 如果将指针传递到一个现有`CFileException`，和**打开**在遇到错误，该函数将它的信息填充描述该错误。 在既没有大小写将**打开**引发异常。  
...
|--------------|------------------------|------------------|----------------------------|  
|**NULL**|No|**则返回 TRUE**|无|  
|ptr 到`CFileException`|No|**则返回 TRUE**|未更改|  
|**NULL**|是|**FALSE**|无|  
...
##  <a name="a-nameoperatorhandlea--cfileoperator-handle"></a><a name="operator_handle"></a>CFile::operator 句柄  
 使用此运算符传递的句柄`CFile`对象传递给函数如[ReadFileEx](http://msdn.microsoft.com/library/windows/desktop/aa365468)和[GetFileTime](http://msdn.microsoft.com/library/windows/desktop/ms724320) ，期望`HANDLE`。  
...
### <a name="parameters"></a>参数  
 ...
 `nCount`  
 最大为从文件读取的字节数。 对于文本模式下的文件，通过对回车-换行对被视为单个字符。  
  
### <a name="return-value"></a>返回值  
 传输到缓冲区的字节数。 请注意，对于所有`CFile`类，则返回值可能小于`nCount`如果已到达文件末尾。  
...
### <a name="parameters"></a>参数  
 `lpszFileName`  
 一个字符串，是所需的文件的路径。 路径可以是相对路径或绝对的并且可以包含网络名称。  
 ...
 **删除**成员函数将引发异常，如果已连接的文件处于打开状态，或者不能删除该文件。 这相当于 DEL 命令。  
...
### <a name="parameters"></a>参数  
 `lOff`  
 要将文件指针移动的字节数。 值为正移动该文件; 末尾的文件指针负值将文件指针移向文件的开头。  
  
 `nFrom`  
 要从搜寻的位置。 请参阅备注部分中有关可能的值。  
  
### <a name="return-value"></a>返回值  
 如果此方法已成功，则将文件指针的位置否则，返回值是不确定和一个指向`CFileException`则会引发异常。  
...
|值|说明|  
|-----------|-----------------|  
|`CFile::begin`|查找从该文件的开头开始。|  
|`CFile::current`|查找从文件指针的当前位置。|  
|`CFile::end`|查找从该文件的末尾。|  
  
 当打开文件时，文件指针将位于 0，文件的开头。  
  
 您可以将文件指针设置为超出文件尾的某个位置。 如果这样做，并不会增加文件的大小，直到您写入到文件。  
  
 处理异常之后，此方法的异常处理程序必须删除异常对象。  
...
##  <a name="a-namesetfilepatha--cfilesetfilepath"></a><a name="setfilepath"></a>CFile::SetFilePath  
 调用此函数可指定该文件; 的路径例如，如果文件的路径不可用[CFile](../../mfc/reference/cfile-class.md)构造对象时，请调用`SetFilePath`提供它。  
...
### <a name="parameters"></a>参数  
 `dwNewLen`  
 所需的文件以字节为单位的长度。 此值可以是大于或小于该文件的当前长度。 将扩展该文件，或将其截断视情况而定。  
  
### <a name="remarks"></a>备注  
  
> [!NOTE]
>  与`CMemFile`，此函数可以引发`CMemoryException`对象。  
 ...
 *status*  
 包含新的状态信息的缓冲区。 调用**GetStatus**成员函数是预先填充**CFileStatus**结构与当前值，然后根据需要进行更改。 如果值为 0，则不会更新相应的状态项。 请参阅[GetStatus](#getstatus)成员函数的说明， **CFileStatus**结构。  
...
### <a name="remarks"></a>备注  
 ...
 请注意，如果您先打电话给`SetStatus`仅变更的属性的文件，以尝试和**m_mtime**文件状态结构中的成员为非零值，这些属性也可能受到影响 （更改的时间戳可能会产生负面影响的属性）。 如果您想要只更改文件的属性，首先设置**m_mtime**的文件状态结构的成员为零，然后再调到`SetStatus`。  
...
### <a name="parameters"></a>参数  
 ...
 `nCount`  
 要从缓冲区传输的字节数。 对于文本模式下的文件，通过对回车-换行对被视为单个字符。  
  
### <a name="remarks"></a>备注  
 **编写**对几个条件，包括磁盘已满条件的响应中引发异常。  
