...
|[COleControl：:D oClick](#doclick)|常用方法的实现 `DoClick` 。|
|[COleControl：:D oPropExchange](#dopropexchange)|序列化对象的属性 `COleControl` 。|
...
|[COleControl：： GetActivationPolicy](#getactivationpolicy)|更改支持接口的控件的默认激活行为 `IPointerInactive` 。|
...
|[COleControl：： GetFontTextMetrics](#getfonttextmetrics)|返回对象的指标 `CFontHolder` 。|
...
|[COleControl：： ResetStockProps](#resetstockprops)|`COleControl`将常用属性初始化为其默认值。|
...
|[COleControl：： WindowProc](#windowproc)|为对象提供 Windows 过程 `COleControl` 。|
...
可以将 OLE 控件插入 OLE 容器应用程序中，并使用双向系统事件激发并向容器公开方法和属性，与容器进行通信。 请注意，标准 OLE 容器只支持 OLE 控件的基本功能。 它们不支持 OLE 控件的扩展功能。 如果在控件中发生特定操作，将事件发送到容器，则会发生事件激发。 反过来，容器通过使用一组公开的方法和属性（类似于 c + + 类的成员函数和数据成员）与控件进行通信。 此方法允许开发人员控制控件的外观并在发生特定操作时通知容器。
...
在无窗口控件中，应始终使用 `COleControl` 成员函数而不是相应的 `CWnd` 成员函数或其相关的 Windows API 函数。
...
每次属性的值发生更改时都必须调用此方法，即使在未通过属性集方法进行更改的情况下也是如此。 请特别注意映射到成员变量的绑定属性。 任何时候都必须调用此类成员变量的更改 `BoundPropertyChanged` 。

## <a name="colecontrolboundpropertyrequestedit"></a><a name="boundpropertyrequestedit"></a>COleControl：： BoundPropertyRequestEdit

从接口请求权限 `IPropertyNotifySink` ，以更改控件提供的绑定属性值。
...
此函数 `COleControl` 作为常用方法（称为 DoClick）受支持。 有关详细信息，请参阅[ActiveX 控件：方法](../../mfc/mfc-activex-controls-methods.md)一文。
...
如果已使用控件向导创建 OLE 控件项目，则此函数的重写版本将通过调用基类函数序列化支持的常用属性 `COleControl` `COleControl::DoPropExchange` 。 将用户定义属性添加到 OLE 控件时，需要修改此函数以序列化新属性。 有关序列化的详细信息，请参阅[ActiveX 控件：序列化](../../mfc/mfc-activex-controls-serializing.md)一文。
...
调用此函数可正确处理非活动 OLE 控件的绘制。 仅当 OLE 控件子类控制 Windows 控件并应在控件的函数中调用此函数时，才应使用此函数 `OnDraw` 。
...
此函数通常由的默认实现调用 `COleControl::DoPropExchange` 。
...
通常，这将是由控件的重写调用的第一个函数 `COleControl::DoPropExchange` 。 加载时，此函数将读取永久性数据的版本号，并相应地设置[CPropExchange](../../mfc/reference/cpropexchange-class.md)对象的 version 特性。 保存时，此函数将写入永久性数据的版本号。
...
若要解决此问题，请手动更改控件的中的 SCODE 参数。ODL 文件 **`long`** 。 此外，使用 SCODE 参数的任何自定义事件、方法或属性也会导致相同的问题。
...
*PbParams*参数是以空格分隔的**VTS_** 列表。 其中一个或多个值（由空格（而不是逗号）分隔）指定函数的参数列表。 可能的值如下：
...
例如，事件的接收方可以修改*pnChar*，例如，将所有小写字符都转换为大写。 如果要检查修改后的字符，请重写 `OnKeyPressEvent` 。
...
更改支持接口的控件的默认激活行为 `IPointerInactive` 。
...
`IPointerInactive`启用接口时，容器会将 WM_SETCURSOR 和 WM_MOUSEMOVE 消息委托给它。 `COleControl`当调整鼠标坐标后，此接口的实现将通过控件的消息映射调度这些消息。

每当容器接收到 WM_SETCURSOR 或 WM_MOUSEMOVE 消息，并将鼠标指针放在支持的非活动对象上时 `IPointerInactive` ，它应调用 `GetActivationPolicy` 接口并从 POINTERINACTIVE 枚举返回标志。

您可以通过将相应条目添加到消息映射来处理这些消息，就像普通的窗口消息一样。 在处理程序中，避免在 `m_hWnd` 不事先检查其值是否为非 NULL 的情况下，使用成员变量（或使用它的任何成员函数）。

除设置鼠标光标和/或激发鼠标移动事件（例如提供特殊的视觉反馈）以外的任何对象都应返回 POINTERINACTIVE_ACTIVATEONENTRY 标志并仅在活动时才绘制反馈。 如果对象返回此标志，则容器应立即将其激活，然后将其转发到触发调用的同一消息 `GetActivationPolicy` 。
...
由通信的信息 `GetActivationPolicy` 不应由容器缓存。 相反，每次鼠标进入非活动对象时，都应调用此方法。

如果非活动对象在鼠标进入该对象时未请求就地激活，则只要鼠标指针停留在该对象上，它的容器就会通过调用[OnInactiveSetCursor](#oninactivesetcursor)将后续 WM_SETCURSOR 消息调度到此对象。

启用 `IPointerInactive` 接口通常意味着您希望控件始终能够处理鼠标消息。 若要在不支持接口的容器中获取此行为 `IPointerInactive` ，需要在可见时始终激活控件，这意味着控件应在其杂项标志中具有 OLEMISC_ACTIVATEWHENVISIBLE 标志。 但是，若要防止此标志在支持的容器中生效 `IPointerInactive` ，还可以指定 OLEMISC_IGNOREACTIVATEWHENVISIBLE 标志：
...
如果成功，则返回值指定当前背景色作为 OLE_COLOR 值。 可以通过调用将此值转换为 COLORREF 值 `TranslateColor` 。
...
如果控件已激活且无窗口，则返回 **`this`** 当前控件是否具有鼠标捕获（由控件的容器确定）; 如果没有捕获，则返回 NULL。

否则，将返回 `CWnd` 具有鼠标捕获的对象（与相同 `CWnd::GetCapture` ）。
...
返回的指针指向的实例 `IOleClientSite` 。 `IOleClientSite`由容器实现的接口是对象上下文的视图：它在文档中的定位位置，其中获取其存储、用户界面以及其他资源。
...
|`clipPaintDC`|如果未设置，则禁用对 `IntersectClipRect` 进行的调用 `COleControl` ，并获得较小的速度优势。 如果使用无窗口激活，标志将不起作用。|
|`pointerInactive`|如果已设置，则在控件处于非活动状态时，通过启用 `COleControl` 的 `IPointerInactive` 接口实现（默认情况下已禁用），提供鼠标交互。|
...
有关 `GetControlFlags` OLE 控件的其他优化的详细信息，请参阅[ActiveX 控件：优化](../../mfc/mfc-activex-controls-optimization.md)。
...
如果成功，则为容器客户端区域的显示设备上下文的指针 `CWnd` ; 否则返回值为 NULL。 显示设备上下文可以在后续的 GDI 函数中用于在容器的窗口的工作区中进行绘制。

### <a name="remarks"></a>备注

必须调用[ReleaseDC](#releasedc)成员函数，以便在绘制后释放上下文。 调用时 `GetDC` ，对象会将其要绘制的矩形传递到自己的客户端坐标中。 `GetDC`将其转换为容器工作区的坐标。 对象不应请求大于其自己的工作区矩形的所需绘制矩形，其大小可以使用[GetClientRect](#getclientrect)检索。 这可防止对象无意中绘制不应出现的位置。
...
此对象可通过其接口进行操作 `IDispatch` 。 你还可以使用 `QueryInterface` 获取对象提供的其他可用接口。 但是，该对象不是支持一组特定接口所必需的。 请注意，依赖容器扩展控件对象的特定功能可将控件的可移植性限制为其他任意容器。
...
如果控件已激活且无窗口，则返回 **`this`** 当前控件是否具有键盘焦点（由控件的容器确定）; 如果该控件没有焦点，则返回 NULL。

否则，将返回 `CWnd` 具有焦点的对象（与相同 `CWnd::GetFocus` ）。

### <a name="remarks"></a>备注

调用[SetFocus](#setfocus)时，激活的无窗口控件接收焦点。

## <a name="colecontrolgetfont"></a><a name="getfont"></a>COleControl：： Ivsfontandcolorstorage.getfont 错误
...
请注意，在完成后，调用方必须释放对象。 在控件的实现中，使用 `InternalGetFont` 来访问控件的常用字体对象。 有关在控件中使用字体的详细信息，请参阅[Activex 控件：在 Activex 控件中使用字体一](../../mfc/mfc-activex-controls-using-fonts.md)文。

## <a name="colecontrolgetfonttextmetrics"></a><a name="getfonttextmetrics"></a>COleControl：： GetFontTextMetrics

度量控件所拥有的任何对象的文本指标 `CFontHolder` 。
...
可以使用[COleControl：： SelectFontObject](#selectfontobject)函数选择此类字体。 `GetFontTextMetrics``TEXTMETRIC`如果成功，将通过*lptm*的有效指标信息初始化所指向的结构 `fontHolder` ; 如果不成功，则用零填充结构。 绘制控件时，应使用此函数，而不是[GetTextMetrics](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) ，因为控件（如任何嵌入的 OLE 对象）可能需要将其呈现到图元文件中。

`TEXTMETRIC`调用[SelectFontObject](#selectfontobject)函数时，将刷新默认字体的结构。 `GetFontTextMetrics`只应在选择 "常用字体" 属性后调用，以确保它提供的信息有效。
...
如果成功，则返回值指定当前前景色作为 OLE_COLOR 值。 可以通过调用将此值转换为[COLORREF](/windows/win32/gdi/colorref)值 `TranslateColor` 。
...
`GetStockTextMetrics`如果成功，函数会将 lptm 所指向的结构初始化为 `TEXTMETRIC` 有效的指标信息，如果不成功，则用零填充结构。 *lptm* 在绘制控件时使用此函数，而不是[GetTextMetrics](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) ，因为控件（如任何嵌入的 OLE 对象）可能需要将其呈现到图元文件中。

`TEXTMETRIC`调用函数时，将刷新默认字体的结构 `SelectStockFont` 。 仅在选择了常用字体后，才应调用此函数，以确保它提供的信息有效。
...
请注意，此函数的调用方必须对 `SysFreeString` 返回的字符串调用，以便释放资源。 在控件的实现中，使用 `InternalGetText` 来访问控件的 "常用文本" 或 "标题" 属性。

## <a name="colecontrolgetwindowlessdroptarget"></a><a name="getwindowlessdroptarget"></a>COleControl：： GetWindowlessDropTarget

`GetWindowlessDropTarget`如果希望将无窗口控件作为 OLE 拖放操作的目标，请重写。
...
指向对象接口的指针 `IDropTarget` 。 由于它没有窗口，因此无窗口对象无法注册 `IDropTarget` 接口。 但是，若要参与拖放，无窗口对象仍可以实现接口并将其返回 `GetWindowlessDropTarget` 。

### <a name="remarks"></a>备注

通常，这需要将该控件的窗口注册为放置目标。 但由于控件没有自己的窗口，容器将使用其自己的窗口作为放置目标。 控件只需提供接口的实现 `IDropTarget` ，容器可以在适当的时间委托调用。 例如：
...
## <a name="colecontrolinvalidatecontrol"></a><a name="invalidatecontrol"></a>COleControl：： Microsoft.office.core.iribbonui.invalidatecontrol
...
将使用 VBX 控件的窗体转换为使用 OLE 控件的窗体时，可能需要 OLE 控件的特殊加载代码。 例如，如果要加载 OLE 控件的实例，则可能会在中调用[PX_Font](persistence-of-ole-controls.md#px_font) `DoPropExchange` ：

[!code-cpp[NVC_MFCAxCtl#3](../../mfc/reference/codesnippet/cpp/colecontrol-class_4.cpp)]

但是，VBX 控件没有 Font 对象;每个字体属性都单独保存。 在这种情况下，你将使用 `IsConvertingVBX` 来区分这两种情况：

[!code-cpp[NVC_MFCAxCtl#4](../../mfc/reference/codesnippet/cpp/colecontrol-class_5.cpp)]

另一种情况是，如果您的 VBX 控件保存了专有的二进制数据（在其 VBM_SAVEPROPERTY 的消息处理程序中），并且您的 OLE 控件以不同的格式保存其二进制数据。 如果希望 OLE 控件与 VBX 控件向后兼容，则可以 `IsConvertingVBX` 通过区分 VBX 控件或 OLE 控件是否正在加载来使用函数来读取旧格式和新格式。
...
框架对的调用的 `IDispatch::Invoke` 实现 `IsInvokeAllowed` ，以确定是否可以调用给定函数（由标识 `dispid` ）。 OLE 控件的默认行为是仅当控件已初始化时才允许调用自动化方法;不过， `IsInvokeAllowed` 是一个虚函数，如有必要，可能会被重写（例如，将控件用作自动化服务器时）。
...
当容器调用控件的函数时，由框架调用 `IOleControl::Close` 。
...
当容器调用成员函数时由框架调用 `IOleObject::DoVerb` 。
...
当容器调用成员函数时由框架调用 `IOleObject::EnumVerbs` 。
...
默认实现调用 `COleControl::InvalidateControl` 。 如果控件是对 Windows 控件进行子类化，则默认实现还会向控件的窗口发送 WM_SETFONT 消息。
...
在容器调用后由框架调用 `IOleControl::FreezeEvents` 。
...
当容器调用成员函数时由框架调用 `IViewObject::GetColorSet` 。
...
*pExtentInfo*<br/>
指向 `DVEXTENTINFO` 指定大小调整数据的结构。 `DVEXTENTINFO`结构为：
...
重写此函数可返回对象的显示大小，该大小与结构中的建议大小和区模式最接近 `DVEXTENTINFO` 。 默认实现返回 FALSE，并且不对大小进行调整。
...
由框架调用，用于获取与重写之前返回的某个预定义字符串相对应的值 `COleControl::OnGetPredefinedStrings` 。
...
*dwCookie*<br/>
先前由的重写返回的 cookie 值 `COleControl::OnGetPredefinedStrings` 。

*lpvarOut*<br/>
指向结构的指针， `VARIANT` 通过该结构将返回属性值。
...
- 整个对象的 DVASPECT_CONTENT 边框。 对象原点的左上角，大小等于返回的范围 `GetViewExtent` *。*
...
*pRect*<br/>
指向[RECTL](/windows/win32/api/windef/ns-windef-rectl)结构，指定应在其中绘制对象的矩形。 此参数控制对象的定位和拉伸。
...
对象的大小转换为 `OnGetViewRect` 从特定位置（默认为显示的左上角）开始的矩形。 如果控件使用两遍绘图，并且它的透明和透明部分具有不同的尺寸，则重写此函数。
...
|VIEWSTATUS_DVASPECTTRANSPARENT|对象支持 DVASPECT_TRANSPARENT。 `IViewObjectEx`将绘图方位作为参数的所有方法都可以在此方面调用。|
...
*pRectBounds*<br/>
指向结构的指针，该 `RECT` 结构指定 OLE 控件客户端区域的边框。

*ptlLoc*<br/>
指向结构的指针，该 `POINT` 结构指定要检查是否命中的点。 在 OLE 工作区坐标中指定该点。
...
*prcLoc*<br/>
指向结构的指针 `RECT` ，该结构指定要检查是否存在命中的矩形（与对象矩形重叠），相对于对象的左上角。
...
指定的格式是以前使用[DelayRenderData](../../mfc/reference/coledatasource-class.md#delayrenderdata)或[DelayRenderFileData](../../mfc/reference/coledatasource-class.md#delayrenderfiledata)成员函数放置在控件对象中用于延迟呈现的格式。 此函数的默认实现将 `OnRenderFileData` 分别调用或 `OnRenderGlobalData` ，前提是所提供的存储介质是文件或内存。 如果请求的格式为 CF_METAFILEPICT 或持久属性集格式，则默认实现将呈现相应的数据并返回非零值。 否则，它将返回0并不执行任何操作。
...
有关详细信息，请参阅 `FORMATETC` `STGMEDIUM` Windows SDK 中的和结构。
...
重写此函数以按请求的格式和媒体提供数据。 根据您的数据，您可能希望改为重写此函数的其他版本之一。 如果要处理多个存储媒体，请重写 `OnRenderData` 。 如果你的数据在文件中或者大小可变，请重写 `OnRenderFileData` 。
...
重写此函数以按请求的格式和媒体提供数据。 根据您的数据，您可能需要替代此函数的其他版本之一。 如果要处理多个存储媒体，请重写 `OnRenderData` 。 如果你的数据在文件中或者大小可变，请重写 `OnRenderFileData` 。
...
默认实现将调用[DoPropExchange](#dopropexchange)，并传递一个 `CPropExchange` 对象，该对象会使属性设置为其默认值。

控件编写器可以在此可重写的中插入 OLE 控件的初始化代码。 如果未首先调用或，则在调用[IPersistStream：： load](/windows/win32/api/objidl/nf-objidl-ipersiststream-load)或[IPersistStorage：： Load](/windows/win32/api/objidl/nf-objidl-ipersiststorage-load)失败或[IPersistStreamInit：： InitNew](/windows/win32/api/ocidl/nf-ocidl-ipersiststreaminit-initnew)或[IPersistStorage：： InitNew](/windows/win32/api/objidl/nf-objidl-ipersiststorage-initnew)时，将调用此函数 `IPersistStream::Load` `IPersistStorage::Load` 。

## <a name="colecontrolonsetclientsite"></a><a name="onsetclientsite"></a>COleControl：： OnSetClientSite

当容器调用控件的函数时，由框架调用 `IOleControl::SetClientSite` 。
...
默认情况下，会 `OnSetClientSite` 检查是否加载数据路径属性，如果是，则调用 `DoDataPathPropExchange` 。
...
默认实现处理控件范围的大小调整。 如果该控件处于就地活动状态，则将对该容器进行调用 `OnPosRectChanged` 。
...
*lpRectClip*<br/>
指向结构的指针 `RECT` ，该结构指示要将控件剪裁到的矩形区域。
...
由于无窗口对象没有窗口，因此它们需要一个机制让容器向它们分派消息。 无窗口 OLE 对象通过 `OnWindowMessage` 接口上的方法 `IOleInPlaceObjectWindowless` （ [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject)的扩展支持）从其容器中获取消息。 `OnWindowMessage`不采用 `HWND` 参数。
...
- HTERROR 在屏幕背景上或在窗口之间的分隔线上（与 HTNOWHERE 相同，只不过 `DefWndProc` windows 函数发出系统提示音以指示错误）。
...
在显示任何模式对话框后调用此函数。 您必须调用此函数，以便容器可以启用所有顶级窗口 `PreModalDialog` 。 应将此函数与对的调用配对 `PreModalDialog` 。
...
基类支持将此函数 `COleControl` 作为常用方法，称为 Refresh。 这允许 OLE 控件的用户在特定时间重绘控件。 有关此方法的详细信息，请参阅文章[ActiveX 控件：方法](../../mfc/mfc-activex-controls-methods.md)。
...
您可以使用 `ResetStockProps` 和 `ResetVersion` 重写来改善控件的二进制初始化性能 `COleControl::OnResetState` 。 请参阅以下示例。 有关优化初始化的详细信息，请参阅[ActiveX 控件：优化](../../mfc/mfc-activex-controls-optimization.md)。
...
您可以使用 `ResetVersion` 和 `ResetStockProps` 重写来改善控件的二进制初始化性能 `COleControl::OnResetState` 。 请参阅[ResetStockProps](#resetstockprops)上的示例。 有关优化初始化的详细信息，请参阅[ActiveX 控件：优化](../../mfc/mfc-activex-controls-optimization.md)。
...
*lpClipRect*<br/>
指向指定要 `CRect` `RECT` 剪裁到的矩形的对象或结构。 仅滚动矩形内的像素。 即使在*lpRect*矩形中，它也不会受到影响。 如果*lpClipRect*为 NULL，则不会对滚动矩形执行任何剪切。
...
指向先前选定的对象的指针 `CFont` 。 完成后，应使用[CDC：： SelectObject](../../mfc/reference/cdc-class.md#selectobject)将此字体选择回设备上下文。
...
可以通过使用 `SerializeExtent` 、 `SerializeStockProps` 和 `SerializeVersion` 重写来改善控件的二进制持久性性能 `COleControl::Serialize` 。 请参阅以下示例。 有关优化初始化的详细信息，请参阅[ActiveX 控件：优化](../../mfc/mfc-activex-controls-optimization.md)。
...
可以通过使用 `SerializeStockProps` 、 `SerializeExtent` 和 `SerializeVersion` 重写来改善控件的二进制持久性性能 `COleControl::Serialize` 。 有关示例，请参阅[SerializeExtent](#serializeextent)中的代码。 有关优化初始化的详细信息，请参阅[ActiveX 控件：优化](../../mfc/mfc-activex-controls-optimization.md)。
...
可以通过使用 `SerializeVersion` 、 `SerializeExtent` 和 `SerializeStockProps` 重写来改善控件的二进制持久性性能 `COleControl::Serialize` 。 有关示例，请参阅[SerializeExtent](#serializeextent)中的代码。 有关优化初始化的详细信息，请参阅[ActiveX 控件：优化](../../mfc/mfc-activex-controls-optimization.md)。
...
然后，将重新创建并调用控件窗口 `OnBorderStyleChanged` 。
...
指向 `CWnd` 之前具有输入焦点的窗口对象的指针; 如果没有此类窗口，则为 NULL。

### <a name="remarks"></a>备注

如果控件已激活且无窗口，此函数会使控件的容器窗口代表控件上的输入焦点。 输入焦点会将键盘输入定向到容器的窗口，容器会将所有后续键盘消息调度到调用的 OLE 对象 `SetFocus` 。 之前有输入焦点的任何窗口都将丢失它。
...
在失败时调用此函数 `BoundPropertyRequestEdit` 。 此函数引发类型的异常， `COleDispScodeException` 以指示不允许设置操作。
...
*lpptfContainer*<br/>
指向一个结构的指针， `POINTF` 该结构包含容器的单元大小的坐标。
