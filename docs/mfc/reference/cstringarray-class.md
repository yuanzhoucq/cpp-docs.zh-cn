---
title: CStringArray 类 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
成员函数`CStringArray`类似于类的成员函数[CObArray](../../mfc/reference/cobarray-class.md)。 由于此相似性，因此你可以使用 `CObArray` 参考文档获取成员函数细节。 无论您在何处`CObject`指针作为返回值，替换[CString](../../atl-mfc-shared/using-cstring.md)对象 (不[CString](../../atl-mfc-shared/using-cstring.md)指针)。 无论你在何处看到作为函数参数的 `CObject` 指针，都请替换 `LPCTSTR`。
...
`CStringArray` 集成了 IMPLEMENT_SERIAL 宏来支持序列化和转储的它的元素。 如果将 `CString` 对象的数组存储到存档中（使用重载插入运算符或 `Serialize` 成员函数），则将依次序列化每个元素。
...
有关使用的详细信息`CStringArray`，请参阅文章[集合](../../mfc/collections.md)。
