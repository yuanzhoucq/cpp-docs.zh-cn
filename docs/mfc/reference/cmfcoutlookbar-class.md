...
2.  在处理主框架中的 WM_CREATE 消息时，调用[CMFCOutlookBar::Create](#create)方法来创建 Outlook 栏选项卡控件。  
...
3.  获取指向基础`CMFCOutlookBarTabCtrl`通过使用[CBaseTabbedPane::GetUnderlyingWindow](../../mfc/reference/cbasetabbedpane-class.md#getunderlyingwindow)。  
  
    ```cpp
    CMFCOutlookBarTabCtrl* pOutlookBar = (CMFCOutlookBarTabCtrl*) m_wndOutlookBar.GetUnderlyingWindow ();
    ```
    
4.  创建[CMFCOutlookBarPane 类](../../mfc/reference/cmfcoutlookbarpane-class.md)包含按钮的每个选项卡的对象。  
...
5.  调用[CMFCOutlookBarTabCtrl::AddTab](../../mfc/reference/cmfcbasetabctrl-class.md#addtab)添加每个新选项卡。设置*bDetachable*参数为 FALSE，以使非可拆分的页。 或者，使用[CMFCOutlookBarTabCtrl::AddControl](../../mfc/reference/cmfcoutlookbartabctrl-class.md#addcontrol)以添加可拆分的页面。  
  
    ```cpp
    pOutlookBar->AddTab (&m_wndOutlookPane, "General", (UINT) -1, TRUE);
    ```  

6.  若要添加`CWnd`的派生的控件 (例如， [CMFCShellTreeCtrl 类](../../mfc/reference/cmfcshelltreectrl-class.md)) 作为选项卡，创建控件并将调用[CMFCOutlookBarTabCtrl::AddTab](../../mfc/reference/cmfcbasetabctrl-class.md#addtab)将其添加到 Outlook 栏。  
