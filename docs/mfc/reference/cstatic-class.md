...
|名称|说明|
|----------|-----------------|
|[CStatic：： Create](#create)|创建 Windows 静态控件，并将其附加到 `CStatic` 对象上。|
...
如果在对话框中创建一个 `CStatic` 对象（通过对话框资源），则当用户关闭该对话框时，`CStatic` 对象将自动销毁。

如果在窗口中创建 `CStatic` 对象，则可能还需要销毁它。 在窗口中创建的 `CStatic` 对象将自动销毁。 如果使用**new**函数在堆上创建 `CStatic` 对象，则必须对该对象调用**delete** ，以便在完成该操作后将其销毁。
...
创建 Windows 静态控件，并将其附加到 `CStatic` 对象上。
...
*rect*<br/>
指定静态控件的位置和大小。 它可以是 `RECT` 结构，也可以是 `CRect` 对象。
...
通过两个步骤构造一个 `CStatic` 对象。 首先，`CStatic`调用构造函数，然后调用 `Create`，这将创建 Windows 静态控件，并将其附加到 `CStatic` 对象。
...
重写此函数以实现所有者描述的 `CStatic` 对象的绘制（该控件具有样式 SS_OWNERDRAW）。

##  <a name="getbitmap"></a>CStatic::GetBitmap

获取与 `CStatic`关联的位图的句柄，它以前设置了[SetBitmap](#setbitmap)。
...
获取与 `CStatic`关联的游标的句柄，它以前设置了[SetCursor](#setcursor)。
...
获取与 `CStatic`关联的增强型图元文件的句柄，它以前设置了[SetEnhMetafile](#setenhmetafile)。
...
- MFC 提供类 `CBitmap`，当必须使用位图图像执行更多操作时，可以使用它，而不只是调用 Win32 函数 `LoadBitmap`。 `CBitmap`，其中包含一种 GDI 对象，通常用于与 `CStatic`合作，后者是一个 `CWnd` 类，用于将图形对象显示为静态控件。

`CImage` 是一种 ATL/MFC 类，使你能够更轻松地使用与设备无关的位图（DIB）。 有关详细信息，请参阅[CImage 类](../../atl-mfc-shared/reference/cimage-class.md)。

- 典型用法是为 `CStatic::SetBitmap` 由 `CBitmap` 或 `CImage` 对象的 HBITMAP 运算符返回的 GDI 对象。 用于执行此操作的代码类似于以下行。

```
MyStaticControl.SetBitmap(HBITMAP(MyBitmap));
```

下面的示例在堆上创建两个 `CStatic` 对象。 然后，它使用 `CBitmap::LoadOEMBitmap` 和使用 `CImage::Load`的文件中的其他文件加载一个系统位图。
