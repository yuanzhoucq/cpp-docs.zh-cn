...
|[CMenu::Attach](#attach)|将 Windows 菜单句柄附加到`CMenu`对象。|
...
|[CMenu::CreateMenu](#createmenu)|创建一个空菜单, 并将其附加`CMenu`到对象。|
|[CMenu::CreatePopupMenu](#createpopupmenu)|创建一个空的弹出菜单, 并将其附加到`CMenu`对象。|
|[CMenu::DeleteMenu](#deletemenu)|从菜单中删除指定项。 如果菜单项具有关联的弹出菜单, 则会销毁弹出菜单的句柄, 并释放它所使用的内存。|
|[CMenu::DeleteTempMap](#deletetempmap)|删除`FromHandle`成员函数`CMenu`创建的任何临时对象。|
|[CMenu::DestroyMenu](#destroymenu)|销毁附加到`CMenu`对象的菜单, 并释放菜单占用的任何内存。|
|[CMenu::Detach](#detach)|从`CMenu`对象分离一个 Windows 菜单句柄并返回该句柄。|
|[CMenu::DrawItem](#drawitem)|当所有者描述的菜单的视觉方面发生变化时, 由框架调用。|
|[CMenu::EnableMenuItem](#enablemenuitem)|启用、禁用或暗显菜单项。|
|[CMenu:: FromHandle](#fromhandle)|返回一个指向给定 Windows `CMenu`菜单句柄的对象的指针。|
...
|[CMenu::GetSafeHmenu](#getsafehmenu)|返回此`m_hMenu` `CMenu`对象包装的。|
...
|[CMenu::LoadMenuIndirect](#loadmenuindirect)|从内存中的菜单模板加载菜单, 并将其附加到`CMenu`对象。|
...
将堆栈帧上的`CMenu`对象创建为本地对象,然后调用的成员函数以根据需要操作新菜单。`CMenu` 接下来, 调用[CWnd:: SetMenu](../../mfc/reference/cwnd-class.md#setmenu) , 将菜单设置为窗口, 后跟`CMenu`对象的[分离](#detach)成员函数的调用。 该`CWnd::SetMenu`成员函数将窗口的菜单设置为新菜单, 导致重绘窗口以反映菜单更改, 同时还将菜单的所有权传递给窗口。 对`Detach` `CMenu`从对象分离 HMENU 的调用, 以便当本地变量超出范围时, `CMenu`对象析构函数不会尝试销毁不再拥有的菜单。 `CMenu` 销毁窗口时, 菜单本身会自动销毁。
...
*nIDNewItem*<br/>
指定新菜单项的命令 ID, 或者, 如果*nFlags*设置为 MF_POPUP, 则为弹出菜单的菜单句`HMENU`柄 ()。 如果*nFlags*设置为 MF_SEPARATOR, 则忽略*nIDNewItem*参数 (不需要)。
...
|------------|-----------------------------------|
|MF_OWNERDRAW|包含应用程序提供的32位值, 应用程序可使用该值来维护与菜单项关联的附加数据。 当应用程序处理 WM_MEASUREITEM 和 WM_DRAWITEM 消息时, 此32位值可用于应用程序。 该值存储在与这些消息`itemData`一起提供的结构的成员中。|
...
应用程序可以通过在*nFlags*中设置值来指定菜单项的状态。 当*nIDNewItem*指定一个弹出菜单时, 它将成为它要追加到的菜单的一部分。 如果该菜单被销毁, 则追加的菜单也会被销毁。 追加的菜单应该与`CMenu`对象分离, 以避免冲突。 请注意, MF_STRING 和 MF_OWNERDRAW 对于的位图版本`AppendMenu`无效。

以下列表描述了可在*nFlags*中设置的标志:

- MF_CHECKED 充当 MF_UNCHECKED 的切换, 以将默认复选标记置于该项旁边。 当应用程序提供了检查标记位图 (请参阅[SetMenuItemBitmaps](#setmenuitembitmaps)成员函数) 时, 将显示 "选中标记" 位图。

- MF_UNCHECKED 充当 MF_CHECKED 的切换, 以删除项旁边的复选标记。 当应用程序提供了检查标记位图 (请参阅`SetMenuItemBitmaps`成员函数) 时, 将显示 "勾号 off" 位图。
...
- MF_OWNERDRAW 指定项是所有者描述项。 当首次显示菜单时, 拥有菜单的窗口将收到 WM_MEASUREITEM 消息, 该消息将检索菜单项的高度和宽度。 WM_DRAWITEM 消息是指每当所有者必须更新菜单项的视觉外观时发送的消息。 此选项对于顶级菜单项无效。
...
- MF_STRING、MF_OWNERDRAW、MF_SEPARATOR 和 bitmap 版本
...
- MF_BYCOMMAND 指定参数提供现有菜单项的命令 ID。 这是默认设置。

- MF_BYPOSITION 指定参数提供现有菜单项的位置。 第一项的位置为0。

- MF_CHECKED 充当 MF_UNCHECKED 的切换, 以将默认复选标记置于该项旁边。

- MF_UNCHECKED 充当 MF_CHECKED 的切换, 以删除项旁边的复选标记。

### <a name="return-value"></a>返回值

项以前的状态:MF_CHECKED 或 MF_UNCHECKED; 如果菜单项不存在, 则为0xFFFFFFFF。
...
在调用的 create 或 load 成员函数之一之前, 不会创建菜单。`CMenu:`
...
菜单最初为空。 可以使用`AppendMenu`或`InsertMenu`成员函数添加菜单项。 应用程序可以将弹出菜单添加到现有菜单或弹出菜单。 `TrackPopupMenu`成员函数可用于将此菜单显示为浮动菜单上的浮动菜单和跟踪选项。
...
如果菜单项具有关联的弹出菜单, `DeleteMenu`则会销毁弹出菜单的句柄, 并释放弹出菜单使用的内存。
...
由`CWinApp`空闲时间处理程序自动调用, 将删除[FromHandle](#fromhandle)成员函数创建的任何临时`CMenu`对象。
...
`DeleteTempMap``CMenu` 在`CMenu`删除对象之前, 分离附加到临时对象的 Windows 菜单对象。
...
菜单在销毁之前从`CMenu`对象分离。 在析构函数中自动调用 Windows `DestroyMenu`函数。 `CMenu`
...
`DRAWITEMSTRUCT`结构的成员定义要执行的绘图操作。 `itemAction` 重写此成员函数以实现所有者描述`CMenu`对象的绘制。 应用程序应还原在此成员函数终止之前为*lpDrawItemStruct*中提供的显示上下文选择的所有图形设备接口 (GDI) 对象。
...
*nEnable*<br/>
指定要执行的操作。 它可以是 MF_DISABLED、MF_ENABLED 或 MF_GRAYED 的组合, 具有 MF_BYCOMMAND 或 MF_BYPOSITION。 可以使用按位 "或" 运算符组合这些值。 这些值具有以下含义:
...
使用 MF_BYPOSITION 值需要应用程序使用正确`CMenu`的。 `CMenu`如果使用菜单栏的, 则顶级菜单项 (菜单栏中的项) 会受到影响。 若要按位置设置弹出菜单或嵌套`CMenu`的弹出菜单中项的状态, 应用程序必须指定弹出菜单的。

当应用程序指定 MF_BYCOMMAND 标志时, Windows 将检查从属于的`CMenu`所有弹出菜单项; 因此, 除非存在重复`CMenu`的菜单项, 否则使用菜单栏的就足够了。
...
返回一个指向`CMenu`对象的指针, 该指针指向具有菜单的 Windows 句柄的对象。
...
指向`CMenu`的指针, 它可以是临时的或永久的。

### <a name="remarks"></a>备注

如果对象尚未附加到 Windows 菜单对象, 则会创建并附加一个`CMenu`临时对象。 `CMenu`

此临时`CMenu`对象只有在下一次应用程序的事件循环中有空闲时间时才有效, 在这种情况下, 将删除所有临时对象。
...
`CMenu`如果具有一个, 则为当前与关联的上下文帮助 ID; 否则为零。
...
*uItem*<br/>
要获取其相关信息的菜单项的标识符或位置。 此参数的意义取决于的值`ByPos`。

*lpMenuItemInfo*<br/>
指向[MENUITEMINFO](/windows/win32/api/winuser/ns-winuser-menuiteminfow)的指针, 如 Windows SDK 中所述, 其中包含有关菜单的信息。

*fByPos*<br/>
值, 该值指定的`nIDItem`含义。 默认情况下`ByPos` , 为 FALSE, 表示 uItem 是菜单项标识符。 如果`ByPos`未设置为 FALSE, 则指示菜单项位置。
...
此成员函数实现 Win32 函数[GetMenuItemInfo](/windows/win32/api/winuser/nf-winuser-getmenuiteminfow)的行为, 如 Windows SDK 中所述。 请注意`GetMenuItemInfo`, 在的 MFC 实现中, 不使用菜单的句柄。
...
- MF_CHECKED 充当 MF_UNCHECKED 的切换, 以将默认复选标记置于该项旁边。 当应用程序提供了检查标记位图 (请参阅`SetMenuItemBitmaps`成员函数) 时, 将显示 "选中标记" 位图。

- MF_DISABLED 禁用菜单项, 使其不能被选中但不会使其变暗。

- MF_ENABLED 启用菜单项, 以便可以选择它, 并将其还原为灰显状态。 请注意, 此常量的值为 0;使用此值时, 应用程序不应对0进行测试。
...
- MF_UNCHECKED 充当 MF_CHECKED 的切换, 以删除项旁边的复选标记。 当应用程序提供了检查标记位图 (请参阅`SetMenuItemBitmaps`成员函数) 时, 将显示 "勾号 off" 位图。 请注意, 此常量的值为 0;使用此值时, 应用程序不应对0进行测试。
...
返回此`CMenu`对象包装的 HMENU 或 NULL`CMenu`指针。
...
指向一个`CMenu` `m_hMenu`对象的指针, 该对象的成员包含弹出菜单的句柄 (如果位于给定位置的弹出菜单中); 否则为 NULL。 `CMenu`如果对象不存在, 则创建一个临时对象。 不`CMenu`应存储返回的指针。
...
|nFlags|LpszNewItem 的解释|
|------------|-----------------------------------|
|MF_OWNERDRAW|包含应用程序提供的32位值, 应用程序可使用该值来维护与菜单项关联的附加数据。 此32位值可用于应用程序中`itemData` [WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem)和[WM_DRAWITEM](/windows/win32/Controls/wm-drawitem)消息提供的结构成员。 当菜单项最初显示或更改时, 将发送这些消息。|
...
*lpMenuItemInfo*<br/>
请参阅 Windows SDK中的`InsertMenuItem` lpmii 说明。

*fByPos*<br/>
请参阅 Windows SDK中的`InsertMenuItem` fByPosition 说明。
...
从应用程序的可执行文件加载菜单资源, 并将其附加`CMenu`到对象。
...
`mtOption`标志应包括弹出列表中最后一项的 MF_END, 以及主列表中最后一项的。 有关其他标志, 请参阅成员函数。`AppendMenu` 如果`mtId`在中`mtOption`指定 MF_POPUP, 则必须从 MENUITEMTEMPLATE 结构中省略此成员。

为 MENUITEMTEMPLATE 结构分配的空间必须足够`mtString`大, 以便将菜单项的名称包含为以 null 结尾的字符串。
...
|MF_STRING|包含指向以 null 结尾的字符串或的`CString`长指针。|
|MF_SEPARATOR|忽略*lpszNewItem*参数 (不需要)。|
...
每当窗口中的菜单发生更改 (无论是否显示窗口) 时, 应用程序应调用`CWnd::DrawMenuBar`。 若要更改现有菜单项的属性, 使用`CheckMenuItem`和`EnableMenuItem`成员函数的速度要快得多。
...
它不会销毁弹出菜单的句柄, 因此可以重复使用菜单。 在调用此函数之前, 应用程序可以调用`GetSubMenu`成员函数来检索弹出`CMenu`对象以便重复使用。
...
*lpMenuItemInfo*<br/>
请参阅 Windows SDK中的`SetMenuItemInfo` lpmii 说明。

*fByPos*<br/>
请参阅 Windows SDK中的`SetMenuItemInfo` fByPosition 说明。
...
*pWnd*<br/>
标识拥有弹出菜单的窗口。 即使指定了 TPM_NONOTIFY 标志, 此参数也不能为 NULL。 此窗口从菜单接收所有 WM_COMMAND 消息。 在 Windows 版本3.1 及更高版本中, 该窗口在返回之前`TrackPopupMenu`不会收到 WM_COMMAND 消息。 在 Windows 3.0 中, 窗口在返回前`TrackPopupMenu`接收 WM_COMMAND 消息。
