...
`DDX_CBIndex` 函数管理对话框、窗体视图或控件视图对象中的组合框控件与对话框、窗体视图或控件视图对象的**int**数据成员之间的**int**数据的传输。
...
`DDX_CBString` 函数管理对话框、窗体视图或控件视图对象中组合框控件的编辑控件与对话框、窗体视图或控件视图对象的 `CString` 数据成员之间的 `CString` 数据的传输。
...
`DDX_CBStringExact` 函数管理对话框、窗体视图或控件视图对象中组合框控件的编辑控件与对话框、窗体视图或控件视图对象的 `CString` 数据成员之间的 `CString` 数据的传输。
...
`DDX_Check` 函数管理对话框、窗体视图或控件视图对象中的复选框控件与对话框、窗体视图或控件视图对象的**int**数据成员之间的**int**数据的传输。
...
`DDX_Control` 函数对对话框、窗体视图或控件视图对象的*nIDC*所指定的控件进行子类控制。
...
调用 `DoDataExchange` 函数时，框架会提供*pDX*对象。 因此，只应在 `DoDataExchange`的重写中调用 `DDX_Control`。
...
*value*<br/>
在前两个版本中，是指对 `CTime` 或 `COleDateTime` 成员变量、对话框、窗体视图或控件视图对象的引用，该对象将与数据进行交换。 在第三个版本中，对 `CString` 数据成员控件视图对象的引用。

### <a name="remarks"></a>备注

调用 `DDX_DateTimeCtrl` 时，*值*将设置为日期和时间选择器控件的当前状态，或将控件设置为*值*，具体取决于交换的方向。

在上述第三个版本中，`DDX_DateTimeCtrl` 管理 `CString` 数据在控件视图对象的日期时间控件和[CString](../../atl-mfc-shared/reference/cstringt-class.md)数据成员之间的传输。 使用当前区域设置的日期和时间格式规则对字符串进行格式设置。
...
`DDX_ManagedControl` 调用[CWinFormsControl：： CreateManagedControl](cwinformscontrol-class.md#createmanagedcontrol)来创建一个与资源控件 ID 相匹配的控件。 使用 `DDX_ManagedControl` 从[CDialog：： OnInitDialog](cdialog-class.md#oninitdialog)中的资源 id 创建控件。 对于数据交换，无需将 DDX/DDV 函数用于 Windows 窗体控件。
...
调用 `DDX_IPAddress` 时，将从 IP 地址控件读取*值*，或将*值*写入控件，具体取决于交换的方向。
...
`DDX_LBIndex` 函数管理对话框、窗体视图或控件视图对象中的列表框控件与对话框、窗体视图或控件视图对象的**int**数据成员之间的**int**数据的传输。
...
`DDX_LBString` 函数管理对话框、窗体视图或控件视图对象中的列表框控件与对话框、窗体视图或控件视图对象的 `CString` 数据成员之间的 `CString` 数据的传输。
...
调用 `DDX_LBString` 以便将数据传输到列表框控件时，将选择控件中其开头匹配*值*的第一项。 （若要匹配整个项而不只是一个前缀，请使用[DDX_LBStringExact](#ddx_lbstringexact)。）如果没有匹配项，则不选择任何项。 匹配不区分大小写。

调用 `DDX_LBString` 从列表框控件传输数据时，会将*value*设置为当前列表框选择。 如果未选择任何项，则将*值*设置为长度为零的字符串。
...
`DDX_CBStringExact` 函数管理对话框、窗体视图或控件视图对象中的列表框控件的编辑控件与对话框、窗体视图或控件视图对象的 `CString` 数据成员之间的 `CString` 数据的传输。
...
调用 `DDX_LBStringExact` 将数据传输到列表框控件时，将选择控件中与*值*匹配的第一项。 （若要只匹配前缀而不是整个项，请使用[DDX_LBString](#ddx_lbstring)。）如果没有匹配项，则不选择任何项。 匹配不区分大小写。

调用 `DDX_CBStringExact` 从列表框控件传输数据时，会将*value*设置为当前列表框选择。 如果未选择任何项，则将*值*设置为长度为零的字符串。
...
*value*<br/>
对用于交换数据的对话框、窗体视图或控件视图对象的 `CTime` 或 `COleDateTime` 成员变量的引用。
...
`DDX_Radio` 函数管理对话框、窗体视图或控件视图对象中的单选控件组与对话框、窗体视图或控件视图对象的**int**数据成员之间的**int**数据的传输。 **Int**数据成员的值根据组中选定的单选按钮来确定。
...
例如，如果选中了组中的第一个单选按钮（带有 WS_GROUP 样式的按钮），则**int**成员的值为0，依此类推。
...
`DDX_Scroll` 函数管理对话框、窗体视图或控件视图对象中滚动条控件与对话框、窗体视图或控件视图对象的**int**数据成员之间的**int**数据的传输。
...
`DDX_Slider` 函数管理对话框或窗体视图中滑块控件与对话框或窗体视图对象的**int**数据成员之间的**int**数据的传输。
...
调用 `DDX_Slider` 时，*值*将设置为控件 thumb 的当前位置，或该值接收位置，具体取决于交换的方向。
...
`DDX_Text` 函数管理对话框、窗体视图或控件视图中的编辑控件与对话框、窗体视图或控件视图对象的[CString](../../atl-mfc-shared/reference/cstringt-class.md)数据成员之间的**int**、 **UINT**、 **long**、DWORD、`CString`、 **float**或**double**数据的传输。
...
*value*<br/>
对对话框、窗体视图或控件视图对象中的数据成员的引用。 *值*的数据类型取决于使用 `DDX_Text` 的哪些重载版本。
