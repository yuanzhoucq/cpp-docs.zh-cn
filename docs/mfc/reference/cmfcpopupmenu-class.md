...
|[CMFCPopupMenu::CheckArea](#checkarea)|确定弹出菜单相对点的位置。|  
...
|[CMFCPopupMenu::EnableMenuLogo](#enablemenulogo)|初始化一个弹出菜单上的徽标。|  
...
|[CMFCPopupMenu::GetForceMenuFocus](#getforcemenufocus)|指示是否将焦点返回到菜单栏中时将显示一个弹出菜单。|  
|[CMFCPopupMenu::GetForceShadow](#getforceshadow)||  
|[CMFCPopupMenu::GetHMenu](#gethmenu)|返回的附加的菜单资源的句柄。|  
|[CMFCPopupMenu::GetMenuBar](#getmenubar)|返回[CMFCPopupMenuBar](../../mfc/reference/cmfcpopupmenubar-class.md)嵌入弹出菜单。|  
|[CMFCPopupMenu::GetMenuItem](#getmenuitem)|返回指定索引处的菜单项的指针。|  
|[CMFCPopupMenu::GetMenuItemCount](#getmenuitemcount)|在弹出菜单中返回的项数。|  
|[CMFCPopupMenu::GetMessageWnd](#getmessagewnd)|返回指向以便框架路由的弹出菜单消息窗口的指针。|  
...
|[CMFCPopupMenu::GetParentToolBar](#getparenttoolbar)|返回指向在父级工具栏的指针。|  
...
|[CMFCPopupMenu::InsertItem](#insertitem)|将新项插入到弹出菜单中的指定位置。|  
|[CMFCPopupMenu::InsertSeparator](#insertseparator)|中的指定位置的弹出菜单中插入分隔符。|  
|[CMFCPopupMenu::IsAlwaysClose](#isalwaysclose)||  
|[CMFCPopupMenu::IsAlwaysShowEmptyToolsEntry](#isalwaysshowemptytoolsentry)||  
|[CMFCPopupMenu::IsCustomizePane](#iscustomizepane)|指示弹出菜单用作**QuickCustomizePane**。|  
|[CMFCPopupMenu::IsEscClose](#isescclose)||  
|[CMFCPopupMenu::IsIdle](#isidle)|指示是否当前空闲的弹出菜单。|  
...
|[CMFCPopupMenu::IsSendMenuSelectMsg](#issendmenuselectmsg)|指示当用户从弹出菜单中选择一个命令时，框架是否通知与父框架。|  
...
|[CMFCPopupMenu::RecalcLayout](#recalclayout)|当标准控件条切换为打开或关闭时或在框架窗口调整大小时，由框架调用。 (重写[CFrameWnd::RecalcLayout](../../mfc/reference/cframewnd-class.md#recalclayout)。)|  
|[CMFCPopupMenu::RemoveAllItems](#removeallitems)|清除的弹出菜单中的所有项。|  
...
|[CMFCPopupMenu::SetForceMenuFocus](#setforcemenufocus)|强制在输入的焦点返回到菜单栏中时将显示一个弹出菜单。|  
|[CMFCPopupMenu::SetForceShadow](#setforceshadow)|强制时弹出菜单显示的主框架外部绘制菜单阴影的框架。|  
...
|[CMFCPopupMenu::SetRightAlign](#setrightalign)|设置弹出菜单的菜单上对齐。|  
|[CMFCPopupMenu::SetSendMenuSelectMsg](#setsendmenuselectmsg)|设置一个标志，用于控制时，在用户选择命令后，弹出菜单是否通知其父框架。|  
|[CMFCPopupMenu::ShowAllCommands](#showallcommands)|强制弹出菜单显示的所有命令。|  
|[CMFCPopupMenu::TriggerResize](#triggerresize)||  
|[CMFCPopupMenu::UpdateAllShadows](#updateallshadows)|更新所有打开的弹出菜单的阴影。|  
|[CMFCPopupMenu::UpdateShadow](#updateshadow)|将更新弹出菜单的阴影。|  
...
### <a name="remarks"></a>备注  
 通常情况下，MFC 将自动创建弹出菜单。 如果您想要创建`CMFCPopupMenu`手动对象、 分配一个在堆上，然后调用[CMFCPopupMenu::Create](#create)。  
  
## <a name="example"></a>示例  
 下面的示例演示如何配置的弹出菜单对象。 该示例演示如何设置徽标和弹出菜单的声音、 设置动画速度和类型、 时弹出菜单显示在主框架外绘制菜单阴影、 设置最大宽度和设置的弹出菜单的右菜单对齐方式。 此代码段属于[的自定义页示例](../../visual-cpp-samples.md)。  
...
-   SHADOW_BOTTOM-`ptScreen`是段的弹出菜单的底部阴影效果。  
  
-   SHADOW_RIGHT-`ptScreen`是段的右阴影的弹出菜单。  
...
### <a name="remarks"></a>备注  
 此方法将其分配的资源`CMFCPopupMenu`。 若要创建的弹出菜单项，请调用[CMFCPopupMenu::Create](#create)。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `bOwnMessage`  
 一个布尔型参数，该值指示框架如何传送的菜单消息。 请参阅备注部分以了解更多信息。  
  
### <a name="return-value"></a>返回值  
 `TRUE`如果成功，则此方法否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 如果`bOwnMessage`是`TRUE`，框架会将传送到任何菜单消息`pWndParent`。 `pWndParent`不能`NULL`如果`bOwnMessage`是`TRUE.`如果`bOwnMessage`是`FALSE`，框架会将菜单消息传送到父弹出菜单。  
  
### <a name="example"></a>示例  
 下面的示例演示如何使用`Create`方法`CMFCPopuMenu`类。 此代码段属于[的自定义页示例](../../visual-cpp-samples.md)。  
...
### <a name="remarks"></a>备注  
 若要显示徽标，实现方法[CFrameWndEx::OnDrawMenuLogo](../../mfc/reference/cframewndex-class.md#ondrawmenulogo)在主框架窗口中。  
...
### <a name="remarks"></a>备注  
 如果启用声音，框架将调用[PlaySound](http://msdn.microsoft.com/library/windows/desktop/bb774426)方法时用户将打开一个弹出菜单，或选择菜单命令。 默认情况下，启用此功能。  
...
### <a name="return-value"></a>返回值  
 指向活动弹出菜单中，或者如果没有弹出菜单当前处于活动状态则为 NULL 指针。  
...
### <a name="return-value"></a>返回值  
 一个整数，指示以毫秒为单位，一个弹出菜单的动画完成所花费的时间。  
  
### <a name="remarks"></a>备注  
 动画速度是一个全局值。 使用[CMFCPopupMenu::SetAnimationSpeed](#setanimationspeed)更改弹出菜单的动画速度。  
...
### <a name="parameters"></a>参数  
 [in] `bNoSystem`  
 一个布尔型参数，该值指示是否此方法检查全局值。 如果您希望此方法以返回的此实例的动画样式 FALSE [CMFCPopupMenu 类](../../mfc/reference/cmfcpopupmenu-class.md)。  
  
### <a name="return-value"></a>返回值  
 一个枚举的值，描述的动画类型。  
  
### <a name="remarks"></a>备注  
 动画弹出菜单的样式是全局应用程序。 使用[CMFCPopupMenu::SetAnimationType](#setanimationtype)设置动画样式。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果当弹出菜单会显示出来; 时，将输入的焦点返回到菜单栏`FALSE`如果弹出菜单保持焦点。  
  
### <a name="remarks"></a>备注  
 默认情况下，您的应用程序不返回到菜单栏的焦点。 若要更改此设置，请使用[CMFCPopupMenu::SetForceMenuFocus](#setforcemenufocus)。  
...
##  <a name="getmenubar"></a>CMFCPopupMenu::GetMenuBar  
 返回[CMFCPopupMenuBar](../../mfc/reference/cmfcpopupmenubar-class.md)嵌入弹出菜单。  
...
### <a name="remarks"></a>备注  
 弹出菜单还拥有一个嵌入`CMFCPopupMenuBar`对象。 如果您使用不同的嵌入的类，必须重写此方法在派生类中。  
...
### <a name="remarks"></a>备注  
 表示菜单项[CMFCToolBarMenuButton 类](../../mfc/reference/cmfctoolbarmenubutton-class.md)。 当调用此方法时，它将指针返回到相应`CMFCToolBarMenuButton`。  
...
### <a name="return-value"></a>返回值  
 指向接收的弹出菜单消息; 窗口的指针`NULL`如果没有窗口。  
  
### <a name="remarks"></a>备注  
 当使用该方法[CMFCPopupMenu::Create](#create)若要创建的弹出菜单，您指定哪些窗口都将接收的菜单消息。  
...
### <a name="remarks"></a>备注  
 一个`CMFCPopupMenu`可以与菜单上的按钮相关联。 在此方案中，当用户选择父工具栏按钮时则显示弹出菜单。  
  
 如果弹出菜单的快捷菜单，它将具有无父级工具栏按钮。  
...
### <a name="return-value"></a>返回值  
 一个指向父`CMFCPopupMenu`对象;`NULL`如果没有父弹出菜单。  
  
### <a name="remarks"></a>备注  
 具有父对象的弹出菜单`CMFCPopupMenu`对象仅当它是一个子菜单。  
...
### <a name="return-value"></a>返回值  
 一个指向当前所选的菜单命令中;`NULL`如果未不选定任何项。  
  
### <a name="remarks"></a>备注  
 在弹出菜单的菜单命令由[CMFCToolBarMenuButton 类](../../mfc/reference/cmfctoolbarmenubutton-class.md)，或从派生的类`CMFCToolBarMenuButton`。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果弹出菜单可以隐藏很少使用的命令;否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 此方法指定是否可以隐藏弹出菜单只有很少使用的命令，如果启用该配置。 弹出菜单可以隐藏很少使用的命令，如果它有一个父按钮和父窗口派生自[CMFCMenuBar 类](../../mfc/reference/cmfcmenubar-class.md)。 使用[CMFCMenuBar::SetRecentlyUsedMenus](../../mfc/reference/cmfcmenubar-class.md#setrecentlyusedmenus)要启用此功能和[CMFCMenuBar::IsRecentlyUsedMenus](../../mfc/reference/cmfcmenubar-class.md#isrecentlyusedmenus)若要确定当前是否启用此功能。 您必须为父窗口调用这两种方法。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `iInsertAt`  
 新项的从零开始索引。 如果`iInsertAt`为-1，该项添加到菜单的末尾。  
  
### <a name="return-value"></a>返回值  
 位置插入项的从零开始的索引。 如果此方法失败，则为-1。  
  
### <a name="remarks"></a>备注  
 如果您提供的值无效，此方法将会失败`iInsertAt`，如整数大于当前在弹出菜单上的项的数目。  
...
### <a name="return-value"></a>返回值  
 分隔符插入的位置的从零开始的索引。 如果此方法失败，则为-1。  
  
### <a name="remarks"></a>备注  
 值为-1 表示`iInsertAt`意味着此方法将添加到弹出菜单的末尾的分隔符。  
...
### <a name="remarks"></a>备注  
 使用**QuickCustomizePane**以使用户能够直接自定义弹出菜单。 **QuickCustomizePane**是`CMFCPopupMenu`显示当用户单击工具栏按钮，以直接进行编辑。  
...
### <a name="remarks"></a>备注  
 默认情况下，一个弹出菜单时处于空闲模式显示动画已完成并且用户不滚动的弹出菜单。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果菜单为右对齐;`FALSE`如果菜单左对齐。  
  
### <a name="remarks"></a>备注  
 您可以使用[CMFCPopupMenu::SetRightAlign](#setrightalign)设置菜单对齐方式。 默认情况下，弹出菜单使用左对齐。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果框架通知与父框架;否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 该框架通过将其发送通知的父框架`WM_MENUSELECT`消息时使用选择的菜单命令。  
...
### <a name="remarks"></a>备注  
 此方法会自动对受移除项的任何分隔符进行排列。 有关框架如何重新排列分隔符的详细信息，请参阅[CMFCToolBar::RemoveButton](../../mfc/reference/cmfctoolbar-class.md#removebutton)。  
...
### <a name="remarks"></a>备注  
 动画速度的全局值，并影响应用程序中的所有弹出菜单。 此值指定了多长时间; 如果要完成的弹出菜单的动画。  
...
### <a name="remarks"></a>备注  
 请参阅[CMFCPopupMenu::GetAnimationType](#getanimationtype)有关的有效值列表`type`。  
...
### <a name="parameters"></a>参数  
 [in] `bValue`  
 `TRUE`如果你想要强制转换为时弹出菜单的菜单栏输入的焦点的框架将显示。 `FALSE`如果您想要保留焦点的弹出菜单。  
  
### <a name="remarks"></a>备注  
 此方法设置一个标志，是全局应用程序中的所有弹出菜单。 默认情况下，不启用此功能。  
  
##  <a name="setforceshadow"></a>CMFCPopupMenu::SetForceShadow  
 强制时弹出菜单显示的主框架外部绘制菜单阴影的框架。  
...
### <a name="parameters"></a>参数  
 [in] `bValue`  
 `TRUE`如果您想要绘制菜单阴影的框架`FALSE`否则为。  
  
### <a name="remarks"></a>备注  
 当调用此方法时，它在您的应用程序中设置全局标志。 此标志将影响所有应用程序中的弹出菜单。  
...
### <a name="remarks"></a>备注  
 如果某个菜单命令与关联的文本将不适合的最大宽度，则将被截断并不能容纳的部分替换为三个点。  
...
##  <a name="setsendmenuselectmsg"></a>CMFCPopupMenu::SetSendMenuSelectMsg  
 设置一个标志，用于控制时，在用户选择命令后，弹出菜单是否通知其父框架。  
...
### <a name="remarks"></a>备注  
 这是应用程序中的所有弹出菜单的全局选项。 如果启用此设置，将发送的弹出菜单`WM_MENUSELECT`当用户选择命令与父框架的消息。  
...
### <a name="remarks"></a>备注  
 弹出菜单显示在动画的控件或其他窗口都具有动态内容时，此方法很有用。  
...
### <a name="parameters"></a>参数  
 [in] `lprectScreen`  
 一个矩形，在屏幕坐标中，指定要更新的区域的边界。  
  
### <a name="remarks"></a>备注  
 当有阴影的弹出菜单与重叠动画的图像时，请调用此方法。  
