...
|[CListCtrl：： Create](#create)|创建一个列表控件，并将其附加到 `CListCtrl` 对象。|
...
除了标准列表样式以外，类还 `CListCtrl` 支持大量扩展样式，提供丰富的功能。 此功能的一些示例包括：
...
下面的代码示例定义了一个 `m_listCtrl` 用于访问当前列表视图控件的变量。 此变量将在下一个示例中使用。
...
如果成功，则返回 TRUE; 否则返回 FALSE。 如果结构的成员中的大小 `cbSize` `LVINSERTMARK` 不等于结构的实际大小，则返回 FALSE。
...
`GetNextSelectedItem`如果使用对的调用建立初始位置，则可以在向前迭代循环中使用 `GetFirstSelectedItemPosition` 。
...
您可以使用结构成员的 LVHT_ABOVE、LVHT_BELOW、LVHT_TOLEFT 和 LVHT_TORIGHT 值 `flag` 来确定是否滚动列表视图控件的内容。 例如，如果位置在工作区的上方和左侧，则可以合并其中两个标志。

您可以测试结构成员的 LVHT_ONITEM 值 `flag` ，以确定给定的位置是否在列表视图项之上。 此值是对结构成员的 LVHT_ONITEMICON、LVHT_ONITEMLABEL 和 LVHT_ONITEMSTATEICON 值的按位 "或" 运算 `flag` 。
...
如果成功，则返回 TRUE; 否则返回 FALSE。 如果结构的成员中的大小 `cbSize` `LVINSERTMARK` 不等于结构的实际大小，或者在当前视图中不应用插入点，则返回 FALSE。
