...
`DDX_FieldScroll`函数将同步记录视图中滚动条控件的滚动位置和 **`int`** 与记录视图关联的记录集的字段数据成员（或选择将其映射到的任何整数变量）的滚动位置。
...
`DDX_FieldSlider`函数同步记录视图中滑块控件的滚动块位置，以及与 **`int`** 记录视图关联的记录集的字段数据成员（或选择将其映射到的任何整数变量）。
