...
|[CMFCToolBar：： GetCustomizeButton](#getcustomizebutton)|检索指向与工具栏关联的 `CMFCCustomizeButton` 对象的指针。|
...
|[CMFCToolBar：： LoadToolBarEx](#loadtoolbarex)|使用 `CMFCToolBarInfo` 帮助器类从应用程序资源加载工具栏，以使应用程序能够使用较大的图像。|
...
重写此方法可在从 `CMFCToolbar`派生的类中提供自己的动态布局。
...
如果工具栏的父级不是 `CMFCReBar` 对象，则[CMFCToolBar：： AdjustLayout](#adjustlayout)方法会调用此方法以重新计算大小。
...
在从 `CMFCToolBar` 派生的对象中重写此方法，并在您不希望用户决定是否在图像下的工具栏按钮上显示文本标签时返回 FALSE。
...
使用[CMFCToolBar：： EnableTextLabels](#enabletextlabels)来指定是否显示文本。 默认值为 FALSE。 调用[CMFCToolBar：： AllowChangeTextLabels](#allowchangetextlabels)以指定用户是否可以在 "自定义" 对话框中更改此设置。
...
此方法使用 `CMFCToolBar::CalcLayout` 方法计算工具栏的大小。 如果*bStretch*为 TRUE，则它将传递*dwMode*参数的 LM_STRETCH 标志。 如果*bHorz*为 TRUE，则它将传递 LM_HORZ 标志。
...
在从 `CMFCToolBar` 派生的类中重写此方法，以提供您自己的高度计算。
...
默认实现返回 TRUE。 在从 `CMFCToolBar` 派生的类中重写此方法，以使用户无法关闭的工具栏对象。
...
您可以重写 `CanBeRestored` 方法，以自定义派生类中的此行为。
...
下面的示例演示如何使用 `CMFCToolBar` 类的 `CreateEx` 方法。 此代码片段是[IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
如果启用文本标签，则工具栏上的所有按钮都将放大，以提供用于在图像下显示标签的空间。 "自定义" 对话框的 "**工具栏**" 页上有 "**显示文本标签**" 复选框。 当用户选择工具栏并选中此选项时，框架将为所选工具栏调用 `EnableTextLabels`。 可以通过从[CMFCToolBar：： AllowChangeTextLabels](#allowchangetextlabels)返回 FALSE 来禁用派生自[CMFCToolBar](../../mfc/reference/cmfctoolbar-class.md)的对象的复选框。
...
指向包含给定的窗口句柄的 `CMFCToolBar` 对象的指针; 如果不存在相应的 `CMFCToolBar` 对象，则为 NULL。

### <a name="remarks"></a>备注

此共享方法检查应用程序中包含给定窗口句柄的 `CMFCToolBar` 对象的每个工具栏。
...
对[CObList 类](../../mfc/reference/coblist-class.md)对象的 const 引用，其中包含 `CMFCToolBar` 对象的集合。
...
如果工具栏按钮具有键盘焦点，则突出显示该按钮。 如果在此应用程序中对工具栏按钮进行热跟踪（有关详细信息，请参阅[CMFCToolBar：： GetHotBorder](#gethotborder)和[CMFCToolBar：： SetHotBorder](#sethotborder)），并且当没有工具栏按钮或菜单项具有键盘焦点时，鼠标指向它。
...
*lpRect*<br/>
弄一个指向 `CRect` 对象的指针，该对象接收图像边框的坐标。

### <a name="remarks"></a>备注

如果指定索引处的按钮不存在，则*lpRect*点设置为0的 `CRect` 对象。

### <a name="example"></a>示例

下面的示例演示如何使用 `CMFCToolBar` 类的 `GetItemRect` 方法。 此代码片段是[IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
一个 `CSize` 结构，它指定锁定的工具栏图像的大小，如果工具栏未锁定，则为空的 `CSize` 结构。

### <a name="remarks"></a>备注

锁定的图像是框架在用户无法自定义工具栏时使用的常规工具栏按钮图像的版本。

如果工具栏未锁定，则此方法返回宽度为零的 `CSize` 结构。 如果工具栏未锁定，则此方法还会在调试版本中生成断言失败。 有关锁定的工具栏的详细信息，请参阅[CMFCToolBar：： IsLocked](#islocked)。
...
指向位于所提供索引处的 `CMFCToolBarButton` 对象的指针; 如果不存在这样的对象，则为 NULL。

### <a name="remarks"></a>备注

当框架更新与工具栏按钮关联的工作区时，框架会调用此方法。 它调用了[CWnd：： InvalidateRect](../../mfc/reference/cwnd-class.md#invalidaterect)方法，其中包含提供的索引处存在的 `CMFCToolBarButton` 对象的客户端矩形。
...
当出现以下一种或多种情况时，`IsCommandRarelyUsed` 方法返回 FALSE：
...
下面的示例演示如何使用 `CMFCToolBar` 类中的 `LoadToolBar` 方法。 此代码片段是[IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
使用 `CMFCToolBarInfo` 帮助器类从应用程序资源加载工具栏，以使应用程序能够使用较大的图像。
...
默认实现不执行任何操作。 当工具栏具有在工具栏返回到其原始状态时必须被替换的虚按钮时，重写从 `CMFCToolBar` 派生的类中的 `OnReset`。
...
*strTTText*<br/>
弄对接收 tooltip 文本的 `CString` 对象的引用。
...
当要显示工具栏按钮的工具提示时，框架会调用此方法。 如果 `OnUserToolTip` 返回 TRUE，则框架将显示一个工具提示，其中包含*strTTText*中 `OnUserToolTip` 返回的文本。 否则，tooltip 包含按钮文本。
...
下面的示例演示如何使用 `CMFCToolBar` 类中的 `ReplaceButton` 方法。 此代码片段是[IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
此类使用 `CMFCCmdUsageCount` 类来跟踪命令的使用计数。 有关此类的详细信息，请参阅[CMFCCmdUsageCount 类](../../mfc/reference/cmfccmdusagecount-class.md)。
