...
ms.locfileid: "69504890"
---
# <a name="cmfctoolbar-class"></a>CMFCToolBar 类

类类似于[CToolBar 类](../../mfc/reference/ctoolbar-class.md), 但为用户界面功能提供额外支持。 `CMFCToolBar` 其中包括平面工具栏、有热图的工具栏、大图标、页导航按钮、锁定的工具栏、Rebar 控件、图像下的文本、背景图像和选项卡式工具栏。 `CMFCToolBar` 类还包含内置支持，用户可对工具栏和菜单、工具栏和菜单之间的拖放、组合框按钮、编辑框按钮、颜色选取器和汇总按钮进行自定义。
...
|[CMFCToolBar::HitTest](#hittest)|返回位于指定位置的工具栏按钮的索引。|
...
|[CMFCToolBar::IsAutoGrayInactiveImages](#isautograyinactiveimages)|指定是否启用自动生成非活动 (未突出显示) 按钮图像。|
...
|[CMFCToolBar::IsCommandRarelyUsed](#iscommandrarelyused)|确定是否很少使用命令 (请参阅[CMFCToolBar:: SetCommandUsageOptions](#setcommandusageoptions))。|
...
|[CMFCToolBar::LoadToolBarEx](#loadtoolbarex)|使用`CMFCToolBarInfo` helper 类从应用程序资源加载工具栏, 以使应用程序能够使用较大的图像。|
...
|[CMFCToolBar::DrawSeparator](#drawseparator)|在工具栏上重新绘制分隔符。|
...
|[CMFCToolBar::m_dblLargeImageRatio](#m_dbllargeimageratio)|指定大型图像的维度 (高度或宽度) 与常规图像维度之间的比率。|

## <a name="remarks"></a>备注

若要将`CMFCToolBar`对象合并到应用程序中, 请执行以下步骤:

1. 向主框架窗口添加对象。`CMFCToolBar`
...
若要插入特殊按钮, 如组合框或下拉工具栏, 请在父资源中保留虚拟按钮, 并使用[CMFCToolBar:: ReplaceButton](#replacebutton)替换运行时中的虚拟按钮。 有关详细信息，请参见[演练：将控件放置在](../walkthrough-putting-controls-on-toolbars.md)工具栏上。
...
重写此方法可在派生自`CMFCToolbar`的类中提供自己的动态布局。
...
如果工具栏的父`CMFCReBar`对象不是对象, [CMFCToolBar:: AdjustLayout](#adjustlayout)方法会调用此方法来重新计算大小。
...
在从派生的`CMFCToolBar`对象中重写此方法, 并在您不希望用户决定是否在图像下的工具栏按钮上显示文本标签时返回 FALSE。
...
此方法由框架调用, 以确定 "工具栏自定义" 页上的列表是否应包括从`CMFCToolBar`派生的特定对象。
...
如果*bEnable*为 TRUE, 则框架将使用*nGrayImagePercentage*从常规映像生成非活动映像。 否则, 必须通过使用[CMFCToolBar:: GetColdImages](#getcoldimages)方法提供一组非活动映像。 默认情况下, 此选项处于禁用状态。
...
此方法使用`CMFCToolBar::CalcLayout`方法计算工具栏的大小。 如果*bStretch*为 TRUE, 则它传递*DWMODE*参数的 LM_STRETCH 标志。 如果*bHorz*为 TRUE, 则它将传递 LM_HORZ 标志。
...
在从派生的`CMFCToolBar`类中重写此方法, 以提供您自己的高度计算。
...
框架调用此方法来确定用户是否可以关闭工具栏。 如果该方法返回 TRUE, 则该框架将在工具栏的 "系统" 菜单中启用 "SC_CLOSE" 命令, 用户可以使用 "自定义" 对话框中的工具栏列表中的复选框关闭工具栏。

默认实现返回 TRUE。 在从派生的`CMFCToolBar`类中重写此方法, 以使用户无法关闭的工具栏对象。
...
如果*iIndex*大于 0, 此方法将忽略工具栏上索引小于*iIndex*的任何按钮。
...
如果要提供特定样式, 请调用此方法, 而不是[CMFCToolBar:: Create](#create) 。 例如, 将*dwCtrlStyle*设置为 TBSTYLE_FLAT |TBSTYLE_TRANSPARENT 创建与 Internet Explorer 4 使用的工具栏类似的工具栏。

### <a name="example"></a>示例

下面的示例演示如何使用`CreateEx` `CMFCToolBar`类的方法。 此代码片段是[IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
此方法通过将`CBasePane::m_dwControlBarStyle`数据成员设置为 AFX_CBRS_FLOAT 来扩展基类实现[CBasePane:: EnableDocking](../../mfc/reference/cbasepane-class.md#enabledocking)。 此方法随后将*dwAlignment*传递到基类实现。
...
有关命令反射的详细信息, 请[参阅 TN062:Windows 控件](../../mfc/tn062-message-reflection-for-windows-controls.md)的消息反射。
...
如果启用文本标签, 则工具栏上的所有按钮都将放大, 以提供用于在图像下显示标签的空间。 "自定义" 对话框的 "**工具栏**" 页上有 "**显示文本标签**" 复选框。 当用户选择工具栏并选中此选项时, 框架将为所`EnableTextLabels`选工具栏调用。 可以通过从[CMFCToolBar:: AllowChangeTextLabels](#allowchangetextlabels)返回 FALSE 来禁用派生自[CMFCToolBar](../../mfc/reference/cmfctoolbar-class.md)的对象的复选框。
...
指向`CMFCToolBar`对象的指针, 该对象包含给定的窗口句柄, 如果不存在`CMFCToolBar`相应的对象, 则为 NULL。

### <a name="remarks"></a>备注

此共享方法检查应用程序`CMFCToolBar`中的每个工具栏, 其中包含给定的窗口句柄。
...
对包含`CMFCToolBar`对象集合的[CObList 类](../../mfc/reference/coblist-class.md)对象的常量引用。
...
如果指定索引处的按钮不存在, 则在该方法返回时, 框架会将*nID*和*nStyle*设置为 0, 并将*iImage*设置为-1。
...
指向与工具栏关联`CMFCCustomizeButton`的对象的指针。

### <a name="remarks"></a>备注

此方法检索显示在工具栏末尾的 "**自定义**" 按钮。 使用[CMFCToolBar:: EnableCustomizeButton](#enablecustomizebutton)方法向工具栏添加 "**自定义**" 按钮。

可以调用[CMFCToolBar:: IsExistCustomizeButton](#isexistcustomizebutton)方法来确定工具栏是否包含有效`CMFCCustomizeButton`的对象。
...
如果工具栏按钮具有键盘焦点, 则突出显示该按钮。 如果在此应用程序中对工具栏按钮进行热跟踪 (有关详细信息, 请参阅[CMFCToolBar:: GetHotBorder](#gethotborder)和[CMFCToolBar:: SetHotBorder](#sethotborder)), 并且鼠标在无工具栏按钮或菜单项具有键盘焦点。
...
`CMFCToolBar`当控件接收输入焦点时, 它将存储一个失去焦点的窗口的句柄, 以便以后可以还原。
...
如果`CRect`指定索引处的按钮不存在, 则将*lpRect*点设置为0的对象。

### <a name="example"></a>示例

下面的示例演示如何使用`GetItemRect` `CMFCToolBar`类的方法。 此代码片段是[IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
一个`CSize`结构, 它指定锁定的工具栏图像的大小, 如果`CSize`未锁定工具栏, 则为空结构。

### <a name="remarks"></a>备注

锁定的图像是框架在用户无法自定义工具栏时使用的常规工具栏按钮图像的版本。

如果工具栏未锁定`CSize` , 则此方法返回宽度为零的结构。 如果工具栏未锁定, 则此方法还会在调试版本中生成断言失败。 有关锁定的工具栏的详细信息, 请参阅[CMFCToolBar:: IsLocked](#islocked)。
...
如果此方法返回一个非零值, 则可以使用`CMFCToolBar::GetCommandTarget`方法检索指向父框架对象的指针。 有关使用此方法的示例, 请参阅 VisualStudioDemo 示例。
...
##  <a name="hittest"></a>CMFCToolBar:: System.windows.media.visualtreehelper.hittest
...
指向位于所提供`CMFCToolBarButton`索引处的对象的指针; 如果不存在这样的对象, 则为 NULL。

### <a name="remarks"></a>备注

当框架更新与工具栏按钮关联的工作区时, 框架会调用此方法。 它调用了[CWnd:: InvalidateRect](../../mfc/reference/cwnd-class.md#invalidaterect)方法, 其中包含提供的索引`CMFCToolBarButton`处存在的对象的客户端矩形。
...
在从`CMFCToolBar`派生的类中重写此方法, 如果您的栏可以显示带有额外边框大小的工具栏按钮, 则返回 TRUE, 否则返回 FALSE。 默认实现返回 TRUE。
...
当`IsCommandRarelyUsed`出现以下一种或多种情况时, 此方法将返回 FALSE:
...
当用户调用 "自定义" 对话框 ( [CMFCToolBarsCustomizeDialog 类](../../mfc/reference/cmfctoolbarscustomizedialog-class.md)) 时, 框架会更改模式。
...
此方法通过调用`CWnd::GetCurrentMessage`获取指向[MSG 结构](/windows/win32/api/winuser/ns-winuser-msg)的指针。 然后, 它会将该按钮的 HWND 与`MSG::lParam`和`MSG::hwnd`成员进行比较, 以确定该按钮是否是命令源。
...
下面的示例演示如何使用`LoadToolBar` `CMFCToolBar`类中的方法。 此代码片段是[IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
使用`CMFCToolBarInfo` helper 类从应用程序资源加载工具栏, 以使应用程序能够使用较大的图像。
...
如果未指定一组大图像, 框架将使用此数据成员。 例如, 如果只提供大小为16x16 的小图像集, 并希望大图像的大小为 24x24, 则将此数据成员设置为1.5。
...
默认实现不执行任何操作。 在`OnReset` 从`CMFCToolBar`派生的类中重写时, 如果工具栏具有在工具栏返回到其原始状态时必须被替换的虚按钮, 则为。
...
当要显示工具栏按钮的工具提示时, 框架会调用此方法。 如果`OnUserToolTip`返回 TRUE, 则框架将显示一个工具提示, 其中包含 strTTText `OnUserToolTip`中返回的文本。 否则, tooltip 包含按钮文本。

重`OnUserToolTip`写以自定义工具栏按钮的工具提示。 默认实现将调用[CMFCToolBar:: OnUserToolTip](#onusertooltip)以获取工具提示文本。
...
此方法通过调用[CWnd::P ostmessage](../../mfc/reference/cwnd-class.md#postmessage)并传递指定按钮的命令 ID 作为*wParam*参数, 将 WM_COMMAND 消息发送到拥有工具栏的窗口。

使用[ON_COMMAND](message-map-macros-mfc.md#on_command)宏可将 WM_COMMAND 消息映射到成员函数。
...
下面的示例演示如何使用`ReplaceButton` `CMFCToolBar`类中的方法。 此代码片段是[IE 演示示例](../../overview/visual-cpp-samples.md)的一部分。
...
如果*nStyle*为 TBBS_DISABLED, 则此方法将删除 TBBS_PRESSED 样式, 因为用户不能单击禁用的按钮。
...
如果*nMinUsagePercentage*等于或大于 100, 则为 FALSE; 否则为 FALSE。否则为 TRUE。
...
如果该方法成功, 则为非零值;否则, 如果指定`CMFCToolBarImages`的对象无效, 或者其图像大小与工具栏的默认图像大小不同, 则为0。

### <a name="remarks"></a>备注

框架使用用户定义的图像绘制用户自定义的工具栏按钮。 *PUserImages*指定的图像列表在应用程序中的所有工具栏之间共享。

如果指定`CMFCToolBarImages`的对象无效, 或者其图像大小与工具栏的默认图像大小不同, 则此方法会在调试版本中生成断言失败。

OutlookDemo、ToolTipDemo 和 VisualStudioDemo 示例使用此方法设置用户定义图像的全局集合。 它们加载名为 UserImages 的文件, 该文件位于应用程序的工作目录中。
