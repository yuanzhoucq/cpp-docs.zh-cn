...
## 成员  
 `CWordArray` 的成员函数类似于选件类 [CObArray](../../mfc/reference/cobarray-class.md)的成员函数。  因此相似性，可以使用 `CObArray` 引用成员函数特定的文档。  无论在何处参见 [CObject](../../mfc/reference/cobject-class.md) 指针作为函数参数或返回值，请替换 **WORD**。  
...
## 备注  
 `CWordArray` 合并 [IMPLEMENT\_SERIAL](../Topic/IMPLEMENT_SERIAL.md) 宏支持序列化和转储其元素。  如果运行存储到存档，与重载、运算符或与 [CObject::Serialize](../Topic/CObject::Serialize.md) 成员函数，反过来，序列化每个元素。  
  
> [!NOTE]
>  在使用数组之前，请使用 `SetSize` 建立它的大小并将其分配的内存。  如果不使用 `SetSize`，将元素添加到的数组使其最频繁分配和复制。  常见的重新分配和复制是低效的，并且可能产生内存碎片。  
  
 如果在数组需要各个元素转储，必须将转储上下文的深度为1或更大。  
  
 有关使用 `CWordArray`的更多信息，请参见文章 [集合](../../mfc/collections.md)。  
