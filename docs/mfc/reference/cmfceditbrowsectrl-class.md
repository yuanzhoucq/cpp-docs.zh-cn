...
1. 嵌入是`CMFCEditBrowseCtrl`对象或父窗口对象的派生的编辑浏览控件对象。

1. 如果您使用**类向导**若要创建一个对话框，添加一个编辑控件 ( `CEdit`) 到对话框窗体。 此外，添加一个变量来访问标头文件中的控件。 在标头文件中，将更改从变量的类型`CEdit`到`CMFCEditBrowseCtrl`。 编辑浏览控件将自动创建。 如果不使用**类向导**，添加`CMFCEditBrowseCtrl`变量标头文件，然后调用其`Create`方法。

1. 如果向对话框添加编辑浏览控件，请使用**ClassWizard**工具来设置数据交换。

1. 调用[EnableFolderBrowseButton](#enablefolderbrowsebutton)， [EnableFileBrowseButton](#enablefilebrowsebutton)，或[EnableBrowseButton](#enablebrowsebutton)方法来设置浏览模式，并显示浏览按钮。 调用[GetMode](#getmode)方法来获取当前浏览模式。

1. 若要提供自定义映像的浏览按钮，调用[SetBrowseButtonImage](#setbrowsebuttonimage)方法或重写[OnDrawBrowseButton](#ondrawbrowsebutton)方法。

1. 若要从编辑浏览控件中删除浏览按钮，调用[EnableBrowseButton](#enablebrowsebutton)方法替换*bEnable*参数设置为 FALSE。
