...
|“属性”|描述|
|----------|-----------------|
|[CDC：： AbortDoc](#abortdoc)|终止当前的打印作业，删除自上次调用成员函数以来应用程序写入到设备的所有内容 `StartDoc` 。|
...
|[CDC：： Arc](#arc)|绘制椭圆弧。|
|[CDC：： ArcTo](#arcto)|绘制椭圆弧。`Arc`除了更新当前位置外，此函数与类似。|
...
|[CDC：:D eleteDC](#deletedc)|删除与此对象相关联的 Windows 设备上下文 `CDC` 。|
|[CDC：:D eleteTempMap](#deletetempmap)|由 `CWinApp` 空闲时间处理程序调用以删除 `CDC` 由创建的任何临时对象 `FromHandle` 。 还会分离设备上下文。|
|[CDC：:D etach](#detach)|从此对象分离 Windows 设备上下文 `CDC` 。|
...
|[CDC：： EndDoc](#enddoc)|结束由成员函数启动的打印作业 `StartDoc` 。|
...
|[CDC：： FromHandle](#fromhandle)|`CDC`当给定设备上下文的句柄时，返回指向对象的指针。 如果 `CDC` 对象未附加到该句柄，则会创建并附加一个临时 `CDC` 对象。|
...
|[CDC：： GetCurrentBitmap](#getcurrentbitmap)|返回指向当前选定对象的指针 `CBitmap` 。|
|[CDC：： GetCurrentBrush](#getcurrentbrush)|返回指向当前选定对象的指针 `CBrush` 。|
|[CDC：： GetCurrentFont](#getcurrentfont)|返回指向当前选定对象的指针 `CFont` 。|
|[CDC：： GetCurrentPalette](#getcurrentpalette)|返回指向当前选定对象的指针 `CPalette` 。|
|[CDC：： GetCurrentPen](#getcurrentpen)|返回指向当前选定对象的指针 `CPen` 。|
...
|[CDC：:P layMetaFile](#playmetafile)|在给定设备上播放指定的元文件的内容。 的增强版本 `PlayMetaFile` 显示了存储在给定的增强格式图元文件中的图片。 可以任意次播放图元文件。|
...
|[CDC：： RestoreDC](#restoredc)|将设备上下文还原到以前保存的状态 `SaveDC` 。|
...
|[CDC：： SetPixelV](#setpixelv)|将指定坐标处的像素设置为指定颜色最近的近似值。 `SetPixelV`速度比更快， `SetPixel` 因为它不需要返回实际绘制的点的颜色值。|
...
|名称|描述|
|----------|-----------------|
|[CDC：： m_hAttribDC](#m_hattribdc)|此对象使用的特性-设备上下文 `CDC` 。|
|[CDC：： m_hDC](#m_hdc)|此对象使用的输出设备上下文 `CDC` 。|

## <a name="remarks"></a>备注

`CDC`对象提供用于处理设备上下文（如显示或打印机）的成员函数，以及用于处理与窗口的工作区关联的显示上下文的成员。

通过对象的成员函数执行所有绘制操作 `CDC` 。 类为设备上下文操作提供成员函数，使用绘图工具、类型安全图形设备接口（GDI）对象选择以及使用颜色和调色板。 它还提供了成员函数，用于获取和设置绘图属性、映射、使用视区、处理窗口区、转换坐标、使用区域、剪辑、绘制线条以及绘制简单形状、省略号和多边形。 还提供了成员函数用于绘制文本、使用字体、使用打印机转义、滚动和播放图元文件。

若要使用 `CDC` 对象，请对其进行构造，然后调用其成员函数，这些函数将使用设备上下文的并行 Windows 函数。

> [!NOTE]
> 在 Windows 95/98 下，所有屏幕坐标限制为16位。 因此， **`int`** 传递给 `CDC` 成员函数的必须在-32768 到32767的范围内。

对于特定用途，Microsoft 基础类库提供了多个派生自的类 `CDC` 。 `CPaintDC`封装对和的调用 `BeginPaint` `EndPaint` 。 `CClientDC`管理与窗口的工作区关联的显示上下文。 `CWindowDC`管理与整个窗口（包括其框架和控件）关联的显示上下文。 `CMetaFileDC`将设备上下文与图元文件相关联。
...
`CDC`包含两个设备上下文， [m_hDC](#m_hdc)和[m_hAttribDC](#m_hattribdc)，在创建对象时，请 `CDC` 引用同一设备。 `CDC`将所有的输出 GDI 调用定向到，对的 `m_hDC` 大多数特性调用 `m_hAttribDC` 。 （属性调用的示例为 `GetTextColor` ，而 `SetTextColor` 是输出调用。）

例如，框架使用这两个设备上下文来实现一个 `CMetaFileDC` 对象，该对象在从物理设备读取属性时将输出发送到图元文件。 在框架中采用类似的方式实现打印预览。 你还可以在特定于应用程序的代码中采用类似方式使用两个设备上下文。

有时可能需要 `m_hDC` 和设备上下文中的文本指标信息 `m_hAttribDC` 。 下面对函数提供了此功能：
...
如果应用程序遇到打印错误或已取消的打印操作，则不能尝试使用类的[EndDoc](#enddoc)或 `AbortDoc` 成员函数终止操作 `CDC` 。 GDI 在返回错误值之前会自动终止操作。

如果应用程序显示一个对话框，允许用户取消打印操作，则必须在 `AbortDoc` 销毁对话框之前调用它。

如果使用打印管理器来启动打印作业，则调用会 `AbortDoc` 擦除整个后台打印作业-打印机不接收任何内容。 如果打印管理器未用于启动打印作业，则在调用之前，可能已将数据发送到打印机 `AbortDoc` 。 在这种情况下，打印机驱动程序会重置打印机（如有可能）并关闭打印作业。
...
*ptEnd*<br/>
指定定义弧线结束点的点的 x 和 y 坐标（以逻辑单位表示）。 此点不必完全位于弧上。可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
*ptEnd*<br/>
指定定义弧线结束点的点的 x 和 y 坐标（以逻辑单位表示）。 此点不必完全位于弧上。可以 `POINT` 为此参数传递数据结构或 `CPoint` 对象。
...
`CDC::Arc`除了更新当前位置外，此函数与类似。 点（ *x1*， *y1*）和（ *x2*， *y2*）指定边框。 由给定边框形成的椭圆形定义弧线的曲线。圆弧从边框中心开始沿逆时针方向（默认弧线方向）向下（ *x3*， *y3*）相交。 该弧形结束时，从边框中心到（ *x4*， *y4*）的射线相交的位置。 如果起始点和终点相同，则绘制一个完整的椭圆。
...
*HDC*同时存储在 `m_hDC` 、输出设备上下文和 `m_hAttribDC` 属性设备上下文中。
...
打开路径方括号后，应用程序可以开始调用 GDI 绘图函数以定义位于该路径中的点。 应用程序可以通过调用成员函数来关闭开放路径方括号 `EndPath` 。 当应用程序调用时 `BeginPath` ，将丢弃以前的任何路径。
...
应用程序可以对齐字节边界上的窗口或客户端区域，以确保在 `BitBlt` 字节对齐的矩形上进行操作。 （在注册窗口类时设置 CS_BYTEALIGNWINDOW 或 CS_BYTEALIGNCLIENT 标志。）

`BitBlt`与字节对齐的矩形上的操作相比，其速度要快于 `BitBlt` 不是字节对齐的矩形上的操作。 如果要为自己的设备上下文指定类样式（如字节对齐），则必须注册窗口类，而不是依赖 Microsoft 基础类来执行此操作。 使用 global 函数[AfxRegisterWndClass](../../mfc/reference/application-information-and-management.md#afxregisterwndclass)。

GDI 转换*nWidth*和*nHeight*，一次使用目标设备上下文，一次使用源设备上下文。 如果生成的范围不匹配，则 GDI 将使用 Windows `StretchBlt` 函数根据需要压缩或拉伸源位图。

如果目标、源和模式位图不具有相同的颜色格式，则函数会 `BitBlt` 转换源和模式位图，使其与目标相匹配。 转换中将使用目标位图的前景色和背景色。

当 `BitBlt` 函数将单色位图转换为彩色颜色时，它会将白色位（1）设置为背景色，将黑色位（0）设置为前景色。 使用目标设备上下文的前景色和背景色。 若要将颜色转换为单色，请 `BitBlt` 将与背景色匹配的像素设置为白色，并将所有其他像素设置为黑色。 `BitBlt`使用颜色设备上下文的前景色和背景色从颜色转换为单色。
...
*ptStart*<br/>
指定定义弦的起点的点的 x 和 y 坐标（以逻辑单位表示）。 此点不必完全位于弦上。 可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
函数绘制的图形 `Chord` 最多可扩展到，但不包括右坐标和下坐标。 这意味着，图形的高度为*y2*  -  *y1* ，而图形的宽度是*x2*  -  *x1*。
...
函数通过绘制一条从当前位置到图形第一个点的直线（通常为对成员函数的最近一次调用指定的点）来关闭该图形 `MoveTo` ，并通过使用 "线条联接" 样式来连接这些直线。 如果使用 `LineTo` 成员函数（而不是）关闭某一图形 `CloseFigure` ，则使用 end cap 来创建角，而不是联接。 `CloseFigure`只有在设备上下文中有一个开放路径方括号时才应调用。

除非使用此函数显式关闭，否则路径中的图形处于打开状态。 （即使当前点和图形的起点相同也可以打开图形。）开始新图形后添加到路径的任何直线或曲线 `CloseFigure` 。
...
此函数只能用于为支持光栅操作的设备创建兼容的设备上下文。 有关设备上下文之间的位块传输的信息，请参阅[CDC：： BitBlt](#bitblt)成员函数。 若要确定设备上下文是否支持光栅操作，请参阅成员函数中的 RC_BITBLT 光栅功能 `CDC::GetDeviceCaps` 。
...
*lpszDriverName*<br/>
指向以 null 结尾的字符串，该字符串指定设备驱动程序的文件名（不带扩展名）（例如，"EPSON"）。 还可以 `CString` 为此参数传递对象。

*lpszDeviceName*<br/>
指向以 null 结尾的字符串，该字符串指定要支持的特定设备的名称（例如，"EPSON FX-80"）。 如果模块支持多个设备，则使用*lpszDeviceName*参数。 还可以 `CString` 为此参数传递对象。

*lpszOutput*<br/>
指向以 null 结尾的字符串，该字符串指定物理输出介质（文件或输出端口）的文件或设备名称。 还可以 `CString` 为此参数传递对象。

*lpInitData*<br/>
指向 `DEVMODE` 包含设备驱动程序的设备特定初始化数据的结构。 Windows `DocumentProperties` 函数检索在给定设备中填充的此结构。 如果设备驱动程序要使用用户通过 "控制面板" 指定的默认初始化（如果有），则*lpInitData*参数必须为 NULL。
...
*lpszDriverName*<br/>
指向以 null 结尾的字符串，该字符串指定设备驱动程序的文件名（不带扩展名）（例如，"EPSON"）。 可以 `CString` 为此参数传递对象。

*lpszDeviceName*<br/>
指向以 null 结尾的字符串，该字符串指定要支持的特定设备的名称（例如，"EPSON FX-80"）。 如果模块支持多个设备，则使用*lpszDeviceName*参数。 可以 `CString` 为此参数传递对象。

*lpszOutput*<br/>
指向以 null 结尾的字符串，该字符串指定物理输出介质（文件或端口）的文件或设备名称。 可以 `CString` 为此参数传递对象。
...
`DeleteDC`成员函数删除与 `m_hDC` 当前对象中的关联的 Windows 设备上下文 `CDC` 。 如果此 `CDC` 对象是给定设备的最后一个活动设备上下文，则会通知设备并释放设备使用的所有存储和系统资源。

`DeleteDC`如果对象已选择到设备上下文中，应用程序不应调用。 在删除对象之前，必须先从设备上下文中选择对象。
...
由 `CWinApp` 空闲时间处理程序自动调用，将 `DeleteTempMap` 删除 `CDC` 由创建的任何临时对象 `FromHandle` ，但不会销毁 `hDC` 与对象暂时关联的设备上下文句柄 `CDC` 。
...
调用此函数可 `m_hDC` 从对象分离（输出设备上下文） `CDC` ，并将和都设置 `m_hDC` `m_hAttribDC` 为 NULL。
...
在采样鼠标位置时以循环方式调用它，以便提供可视反馈。 调用时 `DrawDragRect` ，将清除上一个矩形，并绘制一个新矩形。 例如，当用户在屏幕上拖动矩形时， `DrawDragRect` 将擦除原始矩形，并在新位置中重绘新矩形。 默认情况下， `DrawDragRect` 通过使用半色调画笔来绘制矩形，以消除闪烁并创建平滑运动矩形的外观。

第一次调用时 `DrawDragRect` ， *lpRectLast*参数应为 NULL。
...
*nFlags*<br/>
用于指定要绘制的边框类型的标志。 `DrawEdge`有关参数值的表，请参阅中的 Windows SDK。 对于对角线，BF_RECT 标志指定由矩形参数限定的矢量的终点。
...
当应用程序调用时 `DrawEscape` ，由*NInputSize*和*lpszInputData*标识的数据将直接传递到指定的显示驱动程序。
...
由于这是一个布尔 XOR 函数，因此第二次使用同一矩形调用此函数将从显示中删除该矩形。 此函数绘制的矩形无法滚动。 若要滚动包含此函数绘制的矩形的区域，请首先调用 `DrawFocusRect` 以从显示中删除矩形，然后滚动区域，然后 `DrawFocusRect` 再次调用以在新位置绘制矩形。
...
*nState*<br/>
指定框架控件的初始状态。 可以是 Windows SDK 中的*uState*参数所描述的一个或多个值 `DrawFrameControl` 。 使用*nState*值 DFCS_ADJUSTRECT 调整边界矩形，使其不包含 "push" 按钮的周围边缘。
...
此代码在窗口右下角绘制大小控制手柄。 它适用于对话框的 `OnPaint` 处理程序，该处理程序没有样式，通常不包含可为其提供大小控制的其他控件（如状态栏）。

[!code-cpp[NVC_MFCDocView#34](../../mfc/codesnippet/cpp/cdc-class_6.cpp)]

## <a name="cdcdrawicon"></a><a name="drawicon"></a>CDC：:D rawIcon

在设备上绘制由当前对象表示的图标 `CDC` 。
...
图标资源之前必须使用函数 `CWinApp::LoadIcon` 、或进行加载 `CWinApp::LoadStandardIcon` `CWinApp::LoadOEMIcon` 。 `MM_TEXT`使用此函数之前，必须选择映射模式。
...
> [!NOTE]
> 某些*uFormat*标志组合可能会导致修改传递的字符串。 将 DT_MODIFYSTRING 与 DT_END_ELLIPSIS 或 DT_PATH_ELLIPSIS 一起使用可能会导致修改字符串，从而导致重写中的断言 `CString` 。 DT_CALCRECT、DT_EXTERNALLEADING、DT_INTERNAL、DT_NOCLIP 和 DT_NOPREFIX 的值不能与 DT_TABSTOP 值一起使用。
...
此成员函数使用设备上下文的选定字体、文本颜色和背景颜色来绘制文本。 除非使用 DT_NOCLIP 格式，否则将 `DrawText` 剪裁文本，使文本不会出现在给定矩形的外部。 除非给定 DT_SINGLELINE 格式，否则所有格式设置都将具有多个行。

如果所选字体对于指定的矩形太大，则 `DrawText` 成员函数不会尝试使用较小的字体。

如果指定了 DT_CALCRECT 标志，则将更新*lpRect*指定的矩形，以反映绘制文本所需的宽度和高度。

如果已设置 TA_UPDATECP 文本对齐标志（请参阅[CDC：： SetTextAlign](#settextalign)）， `DrawText` 则将显示从当前位置开始，而不是在给定矩形左侧显示的文本。 `DrawText`设置了 TA_UPDATECP 标志（即，DT_WORDBREAK 标志将不起作用）时，不会换行。
...
## <a name="cdcellipse"></a><a name="ellipse"></a>CDC：：椭圆形
...
此函数绘制的图形最多可扩展到，但不包括右坐标和下坐标。 这意味着，图形的高度为*y2*  -  *y1* ，而图形的宽度是*x2*  -  *x1*。
...
请注意，Microsoft Visual C++ 的新功能允许使用普通函数作为传递到的函数 `EnumObjects` 。 传递给的地址 `EnumObjects` 是指向导出的函数的指针，**导出**时使用的是 Pascal 调用约定。 在保护模式的应用程序中，不必使用 Windows MakeProcInstance 函数创建此函数，或在将其用于 FreeProcInstance Windows 函数后释放该函数。
...
通过从与对象关联的剪辑区域中排除窗口中的更新区域，防止在窗口的无效区域中进行绘制 `CDC` 。
...
- FLOODFILLBORDER 填充区域由*crColor*指定的颜色限定。 此样式与所执行的填充完全相同 `FloodFill` 。
...
此成员函数具有更大 `FloodFill` 的灵活性，因为你可以在*nFillType*中指定填充类型。
...
仅限内存设备上下文和支持光栅显示技术支持的设备 `ExtFloodFill` 。 有关详细信息，请参阅[GetDeviceCaps](#getdevicecaps)成员函数。
...
*lpDxWidths*<br/>
指向值的数组，这些值指示相邻字符单元的起源之间的距离。 例如， *lpDxWidths*[ *i*] 逻辑单元将分隔字符单元*i*和字符单元*i* + 1 的起源。 如果*lpDxWidths*为 NULL，则 `ExtTextOut` 将使用默认的字符间距。
...
如果*nOptions*为0， *lpRect*为 NULL，则函数将文本写入设备上下文，而不使用矩形区域。 默认情况下，函数不使用或更新当前位置。 如果应用程序在调用时需要更新当前位置 `ExtTextOut` ，则应用程序可以调用 `CDC` 成员函数[SetTextAlign](#settextalign) ，并将*nFlags*设置为 TA_UPDATECP。 设置此标志后，Windows 将在对的后续调用中忽略*x*和*y* ， `ExtTextOut` 而改用当前位置。 当应用程序使用 TA_UPDATECP 更新当前位置时， `ExtTextOut` 会将当前位置设置为上一行文本的末尾，或设置为*lpDxWidths*所指向的数组的最后一个元素所指定的位置（以较大者为准）。
...
需要使用[CBrush](../../mfc/reference/cbrush-class.md)成员函数[CreateHatchBrush](../../mfc/reference/cbrush-class.md#createhatchbrush)、 [CreatePatternBrush](../../mfc/reference/cbrush-class.md#createpatternbrush)和[CreateSolidBrush](../../mfc/reference/cbrush-class.md#createsolidbrush)创建画笔，或由 `GetStockObject` Windows 函数检索。

填充指定的矩形时，不 `FillRect` 包括矩形的右边缘和下边缘。 GDI 最多填充一个矩形，但不包括右边的列和下行，而与当前映射模式无关。 `FillRect`比较 `top` 指定矩形的、、 `bottom` `left` 和成员的值 `right` 。 如果 `bottom` 小于或等于 `top` ，或者 `right` 小于或等于 `left` ，则不绘制矩形。

`FillRect`类似于[CDC：： FillSolidRect](#fillsolidrect);不过，使用 `FillRect` 画笔，因此可用于使用纯色、抖动颜色、阴影画笔或图案填充矩形。 `FillSolidRect`仅使用纯色（由 COLORREF 参数指示）。 `FillRect`通常比更慢 `FillSolidRect` 。
...
必须使用 `CBrush` 成员函数 `CreateHatchBrush` 、、 `CreatePatternBrush` `CreateSolidBrush` 或来创建画笔 `GetStockObject` 。
...
*lpRect*<br/>
指定边框（以逻辑单位表示）。 可以为此参数传递指向[RECT](/windows/win32/api/windef/ns-windef-rect)数据结构或对象的指针 `CRect` 。
...
`FillSolidRect`非常类似于[CDC：： FillRect](#fillrect);但是， `FillSolidRect` 只使用纯色（由 COLORREF 参数指示），而使用 `FillRect` 画笔，因此可用于使用纯色、抖动颜色、阴影画笔或图案填充矩形。 `FillSolidRect`通常比更快 `FillRect` 。

> [!NOTE]
> 调用时 `FillSolidRect` ，以前使用[SetBkColor](#setbkcolor)设置的背景色设置为*clr*指示的颜色。
...
假定区域被*crColor*指定。 `FloodFill`函数从*x*和*y*指定的点处开始，并在所有方向上继续到颜色边界。
...
*lpRect*<br/>
指向[RECT](/windows/win32/api/windef/ns-windef-rect)结构或[CRect](../../atl-mfc-shared/reference/crect-class.md)对象，其中包含矩形的左上角和右下角的逻辑坐标。 还可以 `CRect` 为此参数传递对象。
...
绘制的边框与 `FrameRect` 成员函数用相同坐标绘制的边框处于同一位置 `Rectangle` （如果 `Rectangle` 使用的是1个逻辑单元宽度的笔）。 不填充矩形的内部 `FrameRect` 。
...
*pRgn*<br/>
指向 `CRgn` 标识要包含在边框中的区域的对象。 给定区域的坐标是在逻辑单元中指定的。
...
纵横比是设备像素宽度和高度形成的比率。 有关设备纵横比的信息用于创建、选择和显示字体。 Windows 提供了一个特殊筛选器，即纵横比筛选器，用于从所有可用字体中选择为特定纵横比设计的字体。 筛选器使用成员函数指定的纵横比 `SetMapperFlags` 。
...
*lpabc*<br/>
指向在函数返回时接收字符宽度的[ABC](/windows/win32/api/wingdi/ns-wingdi-abc)结构数组。 此数组必须至少包含与 `ABC` *NFirstChar*和*nLastChar*参数所指定范围内的字符数量相同的结构。
...
当 `GetCharABCWidths` 成员函数为字符检索负的 "A" 或 "C" 宽度时，该字符包括 underhangs 或 overhangs。

若要将 ABC 宽度转换为字体设计单元，应用程序应创建一个其高度（在 `lfHeight` [LOGFONT](/windows/win32/api/wingdi/ns-wingdi-logfontw)结构的成员中指定）等于 `ntmSizeEM` [NEWTEXTMETRIC](/windows/win32/api/wingdi/ns-wingdi-newtextmetricw)结构的成员中存储的值的字体。 （ `ntmSizeEM` 可以通过调用[EnumFontFamilies](/windows/win32/api/wingdi/nf-wingdi-enumfontfamiliesw) Windows 函数来检索成员的值。）
...
*lpabc*<br/>
指向接收字符宽度的[ABC](/windows/win32/api/wingdi/ns-wingdi-abc)结构的数组的指针。 此数组必须包含至少与 `ABC` *cgi*参数所指定的标志符号索引数量相同的结构。
...
使用输入设备上下文从当前字体中检索连续字符组中各个字符的宽度 `m_hAttribDC` 。
...
`CBitmap`如果成功，则为指向对象的指针; 否则为 NULL。
...
`CBrush`如果成功，则为指向对象的指针; 否则为 NULL。
...
`CPalette`如果成功，则为指向对象的指针; 否则为 NULL。
...
*dwOffset*<br/>
指定从表开头开始检索信息的偏移量。 如果此参数为0，则从*dwTable*参数所指定的表的开头检索信息。 如果此值大于或等于表的大小，则 `GetFontData` 返回0。
...
应用程序可以通过检查 OUTLINETEXTMETRIC 结构的成员来确定是否可以嵌入字体 `otmfsType` 。 [OUTLINETEXTMETRIC](/windows/win32/api/wingdi/ns-wingdi-outlinetextmetricw) 如果设置了的第1位 `otmfsType` ，则不允许嵌入该字体。 如果第1位为明文，则可以嵌入该字体。 如果设置了 bit 2，则嵌入为只读。
...
标志符号轮廓作为一系列轮廓返回。 每个轮廓由一个[TTPOLYGONHEADER](/windows/win32/api/wingdi/ns-wingdi-ttpolygonheader)结构定义，后跟所需的任意多个 `TTPOLYCURVE` 结构。 所有点都作为[POINTFX](/windows/win32/api/wingdi/ns-wingdi-pointfx)结构返回，表示绝对位置，而不是相对移动。 TTPOLYGONHEADER 结构成员指定的起始点 `pfxStart` 是轮廓[TTPOLYGONHEADER](/windows/win32/api/wingdi/ns-wingdi-ttpolygonheader)轮廓开始的点。 下面的[TTPOLYCURVE](/windows/win32/api/wingdi/ns-wingdi-ttpolycurve)结构可以是折线记录或样条记录。 折线记录是一系列的点;点之间绘制的线条描述字符的轮廓。 样条记录表示 TrueType 使用的二次曲线（即，二次 b 样条）。
...
如果成功，则为指向对象的指针 `CBrush` ; 否则为 NULL。
...
*lpkrnpair*<br/>
指向 `KERNINGPAIR` 当函数返回时接收字偶间距对的结构的数组。 此数组必须包含*nPairs*指定的最多结构。 如果此参数为 NULL，则该函数将返回该字体的字偶间距对的总数。
...
> [!NOTE]
> 如果调用[SetLayout](#setlayout)将 DC 更改为从右到左布局，则 `SetLayout` 会自动将映射模式更改为 MM_ISOTROPIC。 因此，对的任何后续调用 `GetMapMode` 都将返回 MM_ISOTROPIC。
...
[OUTLINETEXTMETRIC](/windows/win32/api/wingdi/ns-wingdi-outlinetextmetricw)结构包含与 TrueType 格式一起提供的大多数字体指标信息，包括[TEXTMETRIC](/windows/win32/api/wingdi/ns-wingdi-textmetricw)结构。 结构的最后四个成员 `OUTLINETEXTMETRIC` 是指向字符串的指针。 除了其他成员所需的空间外，应用程序还应为这些字符串分配空间。 由于对字符串大小没有系统规定的限制，因此分配内存的最简单方法是通过在首次调用函数时指定 NULL for *lpotm*来检索所需的大小 `GetOutlineTextMetrics` 。
...
当前剪辑区域不会偏移函数返回的宽度和高度 `GetOutputTabbedTextExtent` 。
...
当前剪辑区域不影响返回的宽度和高度 `GetOutputTextExtent` 。
...
*lpPoints*<br/>
指向[POINT](/windows/win32/api/windef/ns-windef-point) `CPoint` 放置直线终结点和曲线控制点的点数据结构或对象的数组。
...
如果*nCount*参数为非零值，则枚举的点数。 如果*nCount*为0，则为路径中的总点数（并不 `GetPath` 向缓冲区写入任何内容）。 如果*nCount*为非零值且小于路径中的点数，则返回值为-1。

### <a name="remarks"></a>备注

设备上下文必须包含关闭的路径。 路径的点以逻辑坐标形式返回。 点以设备坐标的形式存储在路径中，因此 `GetPath` 使用当前转换的逆点将点从设备坐标改为逻辑坐标。 可以在 `FlattenPath` 之前调用此成员函数 `GetPath` 以将路径中的所有曲线转换为行段。

### <a name="example"></a>示例

  请参阅[CDC：： BeginPath](#beginpath)的示例。

## <a name="cdcgetpixel"></a><a name="getpixel"></a>CDC：： Picture.getpixel
...
`SetPolyFillMode`有关多边形填充模式的说明，请参阅成员函数。
...
位图拉伸模式定义了如何从由成员函数拉伸或压缩的位图中删除信息 `StretchBlt` 。
...
当前剪辑区域不会偏移函数返回的宽度和高度 `GetTabbedTextExtent` 。
...
1. 向结果和的返回值应用按位 "与" 运算符 `GetTextAlign` 。
...
默认情况下， `GetTextExtent` 假定其检索了维度的文本沿水平线设置（即，行距为0）。 如果创建的字体指定非零行距，则必须显式转换文本角度以获取字符串的尺寸。

当前剪辑区域不影响返回的宽度和高度 `GetTextExtent` 。
...
有关详细信息，请参阅 `GradientFill` 中的 Windows SDK。
...
传递给的回调函数 `GrayString` 必须使用 **`__stdcall`** 调用约定，并且必须使用进行导出 **`__declspec`** 。

当框架处于预览模式时，对成员函数的调用 `GrayString` 会转换为 `TextOut` 调用，并且不会调用回调函数。
...
*lpRect*<br/>
指定矩形。 可以 `CRect` 向此参数传递对象或指向 `RECT` 结构的指针。
...
*lpRect*<br/>
指向 `RECT` 包含要反转的矩形的逻辑坐标的。 还可以 `CRect` 为此参数传递对象。

### <a name="remarks"></a>备注

反转是逻辑非运算，并翻转每个像素的位数。 在单色显示时，该函数使白色像素为黑色，黑色像素为白色。 在颜色显示时，反转取决于为显示生成颜色的方式。 `InvertRect`用相同矩形调用两次会将显示还原为其以前的颜色。
...
*情况*<br/>
指定线条的端点。 可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
默认情况下， `m_hDC` 与 `m_hAttribDC` 包装的其他设备上下文相等 `CDC` 。 通常， `CDC` 创建输出的 GDI 调用会转向 `m_hDC` 设备上下文。 可以初始化 `m_hDC` 并 `m_hAttribDC` 指向不同的设备。 有关如何使用这两个设备上下文的详细信息，请参阅[CDC](../../mfc/reference/cdc-class.md)类说明。
...
*dwRop*<br/>
指定前台和后台三元光栅操作代码，该函数使用该代码来控制源数据和目标数据的组合。 后台光栅操作代码存储在此值高位字的高位字节内;前台光栅操作代码存储在此值高位字的低字节内;此值的低位字将被忽略，并且应为零。 宏 MAKEROP4 创建了前台和后台光栅操作代码的组合。 请参阅 "备注" 部分，了解此函数上下文中的前景和背景讨论。 `BitBlt`有关通用光栅操作代码的列表，请参阅成员函数。
...
如果源、模式和目标位图的颜色格式不同，则此函数将转换模式或源格式，或同时转换两者，以匹配目标格式。 如果掩码位图不是单色位图，则会出现错误。 当记录增强型图元文件时，如果源设备上下文标识增强型图元文件设备上下文，则会发生错误（并且函数返回0）。 并非所有设备都支持 `MaskBlt` 。 应用程序应调用 `GetDeviceCaps` 来确定设备是否支持此功能。 如果未提供掩码位图，此函数的行为与 `BitBlt` 使用前台光栅操作代码完全相同。 掩码位图中的像素偏移量映射到源设备上下文位图中的点（0，0）。 这适用于掩码位图包含一组掩码的情况;通过调整发送到的像素偏移量和矩形大小，应用程序可以轻松地将任何一个应用程序应用到 blitting 任务 `MaskBlt` 。
...
*情况*<br/>
指定新位置。 可以 `POINT` `CPoint` 为此参数传递结构或对象。

### <a name="return-value"></a>返回值

作为对象的上一个位置的 x 和 y 坐标 `CPoint` 。
...
并非所有设备上下文都支持 `PatBlt` 函数。 若要确定设备上下文是否支持 `PatBlt` ，请 `GetDeviceCaps` 使用 RASTERCAPS 索引调用成员函数并检查 RC_BITBLT 标志的返回值。
...
*lpRect*<br/>
指定边框。 可以 `CRect` 向此参数传递对象或指向 `RECT` 结构的指针。
...
*ptEnd*<br/>
指定弧的终结点。此点不必完全位于弧上。可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
此函数绘制的图形延伸到，但不包括右坐标和下坐标。 这意味着，图形的高度为*y2*  -  *y1* ，而图形的宽度是*x2*  -  *x1*。 边框的宽度和高度必须大于2个单位且小于32767个单位。
...
*lpBounds*<br/>
指向一个 `RECT` 结构或 `CRect` 对象，该对象包含用于显示图片的边框的坐标。 在逻辑单元中指定坐标。
...
的第二个版本 `PlayMetaFile` 显示存储在给定的增强格式图元文件中的图片。 当应用程序调用第二个版本的时 `PlayMetaFile` ，Windows 将使用增强型图元文件头中的图片框架将图片映射到*lpBounds*参数指向的矩形。 （在调用之前，可以通过在输出设备中设置世界变换来剪切或旋转此图片 `PlayMetaFile` 。）沿矩形边缘的点包括在图片中。 在播放增强型图元文件之前，可以通过在输出设备中定义剪辑区域来裁剪增强型图元文件。

如果增强型图元文件包含可选调色板，则在调用的第二个版本之前，应用程序可以通过在输出设备上设置调色板来实现一致的颜色 `PlayMetaFile` 。 若要检索可选调色板，请使用 `GetEnhMetaFilePaletteEntries` Windows 函数。 增强型图元文件可以嵌入到新创建的增强型图元文件中，方法是调用的第二个版本 `PlayMetaFile` ，并将源增强型图元文件播放到新增强型图元文件的设备上下文

此函数保留输出设备上下文的状态。 此函数将删除在增强型图元文件中创建但未删除的任何对象。 若要停止此函数，应用程序可以 `CancelDC` 从其他线程调用 Windows 函数以终止操作。 在这种情况下，该函数将返回零。
...
目标坐标将根据目标设备上下文进行转换；源坐标将根据源设备上下文进行转换。 如果源转换具有旋转或切变，则返回错误。 如果目标矩形和源矩形没有相同的颜色格式，则 `PlgBlt` 转换源矩形以匹配目标矩形。 并非所有设备都支持 `PlgBlt` 。 有关详细信息，请参阅成员函数中 RC_BITBLT 光栅功能的说明 `CDC::GetDeviceCaps` 。
...
当前位置既不由函数使用，也不会更新 `PolyBezier` 。 图形未填充。 此函数使用当前笔绘制线条。
...
- PT_CLOSEFIGURE 指定在完成此点的 PT_LINETO 或 PT_BEZIERTO 类型后，图形自动关闭。 将从该点到最近的 PT_MOVETO 或点绘制线条 `MoveTo` 。
...
此函数可用于绘制非连续数字，以代替对 `CDC::MoveTo` 、 `CDC::LineTo` 和成员函数的连续调用 `CDC::PolyBezierTo` 。 使用当前笔绘制线条和样条，而不填充图。 如果通过调用成员函数启动了一个活动路径 `CDC::BeginPath` ，则会 `PolyDraw` 将添加到该路径。 *LpPoints*数组和*lpTypes*中包含的点指示每个点是 `CDC::MoveTo` 、 `CDC::LineTo` 还是操作的一部分 `CDC::BezierTo` 。 还可以关闭数字。 此函数更新当前位置。
...
*lpPoints*<br/>
指向指定多边形顶点的点的数组。 数组中的每个点为 `POINT` 结构或 `CPoint` 对象。
...
当前多边形填充模式可以通过使用 `GetPolyFillMode` 和成员函数来检索或设置 `SetPolyFillMode` 。
...
*lpPoints*<br/>
指向 `POINT` `CPoint` 定义多边形顶点的结构或对象的数组。
...
对函数的调用中指定的每个多边形都 `PolyPolygon` 必须关闭。 与成员函数创建的多边形不同 `Polygon` ，由创建的多边形 `PolyPolygon` 不会自动关闭。
...
*情况*<br/>
指定要在逻辑坐标中检查的点。 可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
当某个窗口具有输入焦点和调用时 `RealizePalette` ，Windows 将确保该窗口将显示所有请求的颜色，最多可在屏幕上同时提供最大数目。 Windows 还会通过将其与可用颜色进行匹配来显示在窗口的调色板中找不到的颜色。
...
*lpRect*<br/>
指定逻辑单元中的矩形。 可以 `CRect` 向此参数传递对象或指向 `RECT` 结构的指针。
...
*lpRect*<br/>
指向 `RECT` 结构或 `CRect` 包含指定矩形的逻辑坐标的对象。
...
调用此成员函数以将设置 `m_hAttribDC` 为 NULL。
...
当输出设备上下文附加到对象时，无法调用此成员函数 `CDC` 。 使用 `Detach` 成员函数分离输出设备上下文。

## <a name="cdcresetdc"></a><a name="resetdc"></a>CDC：： ResetDC

调用此成员函数以更新由对象包装的设备上下文 `CDC` 。
...
设备上下文从 Windows 结构中指定的信息更新 `DEVMODE` 。 此成员函数仅重置属性设备上下文。

当窗口处理消息时，应用程序通常会使用 `ResetDC` 成员函数 `WM_DEVMODECHANGE` 。 在打印文档时，还可以使用此成员函数更改纸张方向或纸张箱。
...
*nSavedDC*<br/>
指定要还原的设备上下文。 它可以是上一个函数调用返回的值 `SaveDC` 。 如果*nSavedDC*为-1，则将还原最近保存的设备上下文。
...
`RestoreDC`通过从以前对成员函数的调用创建的堆栈中弹出状态信息来还原设备上下文 `SaveDC` 。

堆栈可包含多个设备上下文的状态信息。 如果*nSavedDC*指定的上下文不在堆栈顶部，则会 `RestoreDC` 删除*nSavedDC*指定的设备上下文与堆栈顶部之间的所有状态信息。 删除的信息将丢失。
...
*lpRect*<br/>
指定逻辑单元中的边框。 可以 `CRect` 向此参数传递对象或指向 `RECT` 结构的指针。

*情况*<br/>
*点*的 x 坐标指定用来绘制圆角的椭圆的宽度（以逻辑单位表示）。 *Point*的 y 坐标指定用来绘制圆角的椭圆的高度（以逻辑单位表示）。 可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
此函数绘制的图形延伸到，但不包括右坐标和下坐标。 这意味着，图形的高度为*y2*  -  *y1* ，而图形的宽度是*x2*  -  *x1*。 边框的高度和宽度必须大于2个单位且小于32767个单位。
...
一个整数，用于标识保存的设备上下文。 如果发生错误，则为0。 此返回值可用于通过调用还原设备上下文 `RestoreDC` 。

### <a name="remarks"></a>备注

稍后可以使用还原已保存的设备上下文 `RestoreDC` 。
...
*lpRectScroll*<br/>
指向 `RECT` `CRect` 包含滚动矩形坐标的结构或对象。

*lpRectClip*<br/>
指向 `RECT` `CRect` 包含剪辑矩形坐标的结构或对象。 如果此矩形小于*lpRectScroll*所指向的原始矩形，则只会在较小的矩形中滚动。

*pRgnUpdate*<br/>
标识滚动过程所发现的区域。 `ScrollDC`函数定义此区域; 它不一定是矩形。

*lpRectUpdate*<br/>
指向 `RECT` 结构或 `CRect` 对象，该结构或对象接收限定滚动更新区域的矩形的坐标。 这是需要重新绘制的最大矩形区域。 当函数返回时，结构或对象中的值将处于工作区坐标中，而不考虑给定设备上下文的映射模式。
...
如果*lpRectUpdate*为 NULL，则 Windows 不会计算更新矩形。 如果*pRgnUpdate*和*LPRECTUPDATE*都为 NULL，则 Windows 不会计算更新区域。 如果*pRgnUpdate*不为 NULL，则 Windows 将假定其包含滚动过程（由成员函数定义）所发现的区域的有效指针 `ScrollDC` 。 如果需要，可以将*lpRectUpdate*中返回的更新区域传递给 `CWnd::InvalidateRgn` 。

`ScrollWindow` `CWnd` 当需要滚动窗口的整个工作区时，应用程序应使用类的成员函数。 否则，应使用 `ScrollDC` 。
...
- RGN_COPY 新的剪辑区域是*pRgn*标识的区域的副本。 此功能与的第一个版本是相同的 `SelectClipRgn` 。 如果由*pRgn*标识的区域为 NULL，则新的剪辑区域将成为默认剪辑区域（NULL 区域）。
...
函数假定给定区域的坐标是在设备单位中指定的。 某些打印机设备支持比图形输出更高的分辨率文本输出，以保留表达文本指标所需的精度。 这些设备以较高的分辨率（即文本单位）报告设备单位。 然后，这些设备将调整图形的坐标，使多个报告的设备单位仅映射到1个图形单元。 应始终 `SelectClipRgn` 使用文本单位调用函数。
...
指向要替换的对象的指针。 这是一个指向派生自的类的对象的指针 `CGdiObject` ，如 `CPen` ，取决于所使用的函数的版本。 如果出现错误，则返回值为 NULL。 此函数可能返回指向临时对象的指针。 此临时对象只在处理一条 Windows 消息期间有效。 有关详细信息，请参阅 `CGdiObject::FromHandle`。

采用 region 参数的成员函数的版本执行与成员函数相同的任务 `SelectClipRgn` 。 其返回值可以是下列任意值：
...
应用程序一次只能选择一个位图到内存设备上下文中，而只选择一个内存设备上下文。 位图的格式必须是单色或与设备上下文兼容;如果不是，则 `SelectObject` 返回一个错误。

对于 Windows 3.1 和更高版本， `SelectObject` 无论是否在图元文件中使用，函数都将返回相同的值。 在以前版本的 Windows 中，在 `SelectObject` 图元文件中使用时返回一个非零值，如果失败，则返回0。
...
指向对象的指针， `CPalette` 该对象标识由*pPalette*指定的调色板替换的逻辑调色板。 如果出现错误，则为 NULL。
...
- SYSTEM_FONT 系统字体。 默认情况下，Windows 使用系统字体来绘制菜单、对话框控件和其他文本。 不过，最好不要依靠 SYSTEM_FONT 获取对话框和窗口使用的字体。 相反，请将 `SystemParametersInfo` 函数与 SPI_GETNONCLIENTMETRICS 参数一起使用来检索当前字体。 `SystemParametersInfo`考虑当前主题并为标题、菜单和消息对话框提供字体信息。
...
一个指针，指向在 `CGdiObject` 函数成功时被替换的对象。 指向的实际对象是[CPen](../../mfc/reference/cpen-class.md)、 [CBrush](../../mfc/reference/cbrush-class.md)或[CFont](../../mfc/reference/cfont-class.md)对象。 如果调用失败，则返回值为 NULL。
...
指定函数的结果 `SetAbortProc` 。 下面的某些值比其他值更有可能。
...
请注意，Microsoft Visual C++ 的功能可以简化传递到的回调函数的创建 `SetAbortProc` 。 传递给 `EnumObjects` 成员函数的地址是一个指针，指向使用 `__declspec(dllexport)` 和（调用约定）导出的函数 **`__stdcall`** 。
...
*lpRectBounds*<br/>
指向 `RECT` `CRect` 用于设置边框的结构或对象。 在逻辑坐标中给出了矩形尺寸。 此参数可以为 NULL。
...
*情况*<br/>
指定新原点的 x 坐标和 y 坐标。 每个值都必须在0-7 范围内。 可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
画笔原点的默认坐标为（0，0）。 若要更改画笔的原点，请调用 `UnrealizeObject` 对象的函数 `CBrush` ，调用 `SetBrushOrg` ，然后调用 `SelectObject` 成员函数以在设备上下文中选择画笔。
...
颜色调整值用于调整 `CDC::StretchBlt` 设置半色调模式时对成员函数的调用的源位图的输入颜色。
...
通常情况下，不会调用 `SetLayout` 窗口。 而是通过设置[扩展的窗口样式](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles)（如 WS_EX_RTLREADING）来控制窗口中的从右到左布局。 设备上下文（如打印机或图元文件）不会继承此布局。 为从右到左布局设置设备上下文的唯一方法是调用 `SetLayout` 。

如果调用**SetLayout （LAYOUT_RTL** ），则 `SetLayout` 会将映射模式自动更改为 MM_ISOTROPIC。 因此，对[GetMapMode](#getmapmode)的后续调用将返回 MM_ISOTROPIC 而不是 MM_TEXT。

在某些情况下，例如，对于许多位图，可能需要保留从左到右的布局。 在这些情况下，通过调用 `BitBlt` 或来呈现图像 `StretchBlt` ，然后将*dwLayout*的位图控件标志设置为 LAYOUT_BITMAPORIENTATIONPRESERVED。
...
- MM_ISOTROPIC 逻辑单元转换为具有相同缩放轴的任意单位;即，沿 x 轴的1个单位等于沿 y 轴的1个单位。 使用 `SetWindowExt` 和 `SetViewportExt` 成员函数可指定所需的单位和轴的方向。 GDI 根据需要进行调整，以确保 x 和 y 单位保持相同的大小。
...
> [!NOTE]
> 如果调用[SetLayout](#setlayout)将 DC （设备上下文）更改为从右到左布局，则 `SetLayout` 会自动将映射模式更改为 MM_ISOTROPIC。
...
仅使用光栅字体的应用程序可以使用 `SetMapperFlags` 函数来确保字体映射器所选的字体在指定的设备上有吸引力且可读。 使用可缩放（TrueType）字体的应用程序通常不使用 `SetMapperFlags` 。
...
仅当尚未将设备上下文附加到对象时，才能调用此成员函数 `CDC` 。 此成员函数将设置， `m_hDC` 但不会将设备上下文附加到 `CDC` 对象。
...
*情况*<br/>
指定要设置的点的逻辑 x 和 y 坐标。 可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
不是所有的设备都支持 `SetPixel` 函数。 若要确定设备是否支持 `SetPixel` ，请 `GetDeviceCaps` 使用 RASTERCAPS 索引调用成员函数并检查 RC_BITBLT 标志的返回值。
...
点必须位于剪辑区域和设备图面的可见部分中。 并非所有设备都支持成员函数。 有关详细信息，请参阅成员函数中的 RC_BITBLT 功能 `CDC::GetDeviceCaps` 。 `SetPixelV`速度比更快， `SetPixel` 因为它不需要返回实际绘制的点的颜色值。
...
当多边形填充模式为备用模式时，系统将在每个扫描行上填充奇数个和偶数个多边形边之间的区域。 也就是说，系统在第三方和第四方之间填充区域，依此类推。 此模式为默认模式。
...
- TA_UPDATECP 更新每次调用文本输出函数后的当前 x 位置。 新位置位于文本边界矩形的右边。 设置此标志时，将忽略在对成员函数的调用中指定的坐标 `TextOut` 。
...
在 `TextOut` `ExtTextOut` 显示或设备上定位文本字符串时，和成员函数使用这些标志。 标志指定特定点和限定文本的矩形之间的关系。 此点的坐标作为参数传递给 `TextOut` 成员函数。 限定文本的矩形由文本字符串中的相邻字符单元格构成。
...
*nCharExtra*<br/>
指定要添加到每个字符的额外空间量（以逻辑单位为单位）。 如果当前映射模式不为 `MM_TEXT` ，则将转换*nCharExtra*并将其舍入到最接近的像素。
...
*nBreakExtra*<br/>
指定要添加到文本行的总额外空间（以逻辑单位为单位）。 如果当前映射模式不为 `MM_TEXT` ，则此参数指定的值将转换为当前映射模式并舍入到最近的设备单元。
...
`SetTextJustification`调用成员函数后，对文本输出函数（如）的调用会在 `TextOut` 指定的分行符数之间平均分配指定的额外空间。 换行符通常是空格字符（ASCII 32），但也可以通过字体定义为其他字符。

成员函数 `GetTextExtent` 通常与一起使用 `SetTextJustification` 。 `GetTextExtent`计算在对齐前给定行的宽度。 应用程序可以通过在*nBreakExtra* `GetTextExtent` 对齐后从字符串的宽度减去返回的值，来确定要在 nBreakExtra 参数中指定的空间量。
...
对齐每个行后，必须清除此错误词，以防将其合并到下一行。 可以通过 `SetTextJustification` 在*nBreakExtra*设置为0的情况调用来清除字词。
...
作为[CSize](../../atl-mfc-shared/reference/csize-class.md)对象的视区的以前的范围。 出现错误时，返回的对象的 x 坐标和 y 坐标都 `CSize` 设置为0。

### <a name="remarks"></a>备注

视区和设备上下文窗口一起定义了 GDI 映射在逻辑坐标系中的点如何指向实际设备的坐标系统中的点。 换言之，它们定义了如何将逻辑坐标转换为设备坐标。

如果设置了以下映射模式，则将 `SetWindowExt` 忽略对和 `SetViewportExt` 的调用：
...
设置 MM_ISOTROPIC 模式时，应用程序必须在 `SetWindowExt` 调用之前调用成员函数 `SetViewportExt` 。
...
*情况*<br/>
指定视区的原点。 值必须在设备坐标系统范围内。 可以 `POINT` `CPoint` 为此参数传递结构或对象。

### <a name="return-value"></a>返回值

作为对象的视区的上一页（以设备坐标表示） `CPoint` 。

### <a name="remarks"></a>备注

视区和设备上下文窗口一起定义了 GDI 映射在逻辑坐标系中的点如何指向实际设备的坐标系统中的点。 换言之，它们定义了如何将逻辑坐标转换为设备坐标。

视区原点标记设备坐标系统中的点，GDI 将窗口源（由成员函数指定的逻辑坐标系统中的点）映射到该系统 `SetWindowOrg` 。 GDI 按照将窗口原点映射到视区原点所需的相同过程来映射所有其他点。 例如，位于窗口原点附近点附近的所有点都将位于视区原点附近的点。 同样，通过窗口原点的行中的所有点都将位于穿过视区原点的行中。
...
窗口的以前的范围（以逻辑单位为单位），表示为 `CSize` 对象。 如果发生错误，则返回的对象的 x 坐标和 y 坐标都 `CSize` 设置为0。

### <a name="remarks"></a>备注

该窗口与设备上下文视区一起定义了如何将逻辑坐标系统中的点映射到设备坐标系统中的点。

如果设置了以下映射模式， `SetWindowExt` 则将忽略对和 `SetViewportExt` 函数的调用：
...
*情况*<br/>
指定窗口的新原点的逻辑坐标。 可以 `POINT` `CPoint` 为此参数传递结构或对象。
...
窗口源在逻辑坐标系中标记 GDI 将视区的位置（由函数指定的设备坐标系统中的一个点） `SetWindowOrg` 。 GDI 按照将窗口原点映射到视区原点所需的相同过程来映射所有其他点。 例如，位于窗口原点附近点附近的所有点都将位于视区原点附近的点。 同样，通过窗口原点的行中的所有点都将位于穿过视区原点的行中。
...
通知设备驱动程序，新的打印作业正在启动，并且所有后续 `StartPage` 和 `EndPage` 调用都应在同一作业下进行后台处理，直到 `EndDoc` 发生调用为止。
...
系统在 `ResetDC` 对和的调用之间禁用成员 `StartPage` 函数 `EndPage` 。
...
`StretchBlt`函数将位图从*pSrcDC*指定的源设备移动到目标设备，该目标设备由正在调用其成员函数的设备上下文对象表示。 *XSrc*、 *ySrc*、 *nSrcWidth*和*nSrcHeight*参数定义了源矩形的左上角和尺寸。 *X*、 *y*、 *nWidth*和*nHeight*参数指定了目标矩形的左上角和尺寸。 *DwRop*指定的光栅操作定义了源位图和目标设备上已经存在的位的组合方式。
...
不是所有的设备都支持 `StretchBlt` 函数。 若要确定设备是否支持 `StretchBlt` ，请 `GetDeviceCaps` 使用 RASTERCAPS 索引调用成员函数并检查 RC_STRETCHBLT 标志的返回值。
...
作为对象的字符串（以逻辑单位表示）的维度 `CSize` 。

### <a name="remarks"></a>备注

文本以当前所选的字体书写。 如果*nTabPositions*为0， *lpnTabStopPositions*为 NULL，则选项卡将扩展为平均字符宽度的8倍。

如果*nTabPositions*为1，则制表位由*lpnTabStopPositions*数组中第一个值指定的距离隔开。 如果*lpnTabStopPositions*数组包含多个值，则将为数组中的每个值设置一个制表位，直至*nTabPositions*指定的数字为止。 *NTabOrigin*参数允许应用程序在 `TabbedTextOut` 单个行中多次调用函数。 如果应用程序每次将*nTabOrigin*设置为相同的值，则该应用程序多次调用该函数，则该函数将展开与*nTabOrigin*指定的位置相关的所有选项卡。

默认情况下，函数不使用或更新当前位置。 如果应用程序在调用函数时需要更新当前位置，则应用程序可以调用[SetTextAlign](#settextalign)成员函数，并将*nFlags*设置为 TA_UPDATECP。 设置此标志后，Windows 将忽略对的后续调用的*x*和*y*参数 `TabbedTextOut` ，而使用当前位置。
...
如果应用程序在调用时需要更新当前位置 `TextOut` ，则应用程序可以调用 `SetTextAlign` 成员函数，并将*nFlags*设置为 TA_UPDATECP。 设置此标志后，Windows 将忽略对的后续调用的*x*和*y*参数 `TextOut` ，而使用当前位置。
...
当系统调色板发生更改时，具有已实现逻辑调色板的非活动窗口可能会 `UpdateColors` 作为重绘其工作区的替代方法。
...
仅当当前笔是由第二个版本的成员函数创建的几何笔时，此函数才成功; 如果使用的是的第一个版本创建了笔，则此函数将 `CreatePen` `CreatePen` 具有大于1的宽度（以设备单位为单位）。 设备上下文必须包含关闭的路径。 路径中的任何 Bzier 曲线都转换为逼近成一条直线的直线序列。 因此，在调用后，路径中不会保留任何 Bzier 曲线 `WidenPath` 。
