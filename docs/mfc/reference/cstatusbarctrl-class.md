...
|[CStatusBarCtrl::DrawItem](#drawitem)|当所有者描述的状态栏控件的可视方位更改时调用。|
...
|[CStatusBarCtrl::SetSimple](#setsimple)|指定状态栏控件是显示简单文本还是显示通过先前对的调用`SetParts`设置的所有控件部件。|
...
此控件 (因而`CStatusBarCtrl`类) 仅适用于在 windows 95/98 和 windows NT 版本3.51 及更高版本下运行的程序。
...
创建一个状态栏控件, 并将其附加到`CStatusBarCtrl`对象。
...
*pParentWnd*<br/>
指定状态栏控件的父窗口 (通常为`CDialog`)。 它不能为 NULL。
...
可以通过`CStatusBarCtrl`两个步骤构造。 首先, 调用构造函数, 然后调用`Create`, 它将创建状态栏控件, 并将其附加`CStatusBarCtrl`到对象。

状态窗口的默认位置沿父窗口的底部, 但您可以指定 CCS_TOP 样式, 使其显示在父窗口的工作区的顶部。 可以指定 SBARS_SIZEGRIP 样式, 以在状态窗口的右端包含大小调整手柄。 不建议将 CCS_TOP 和 SBARS_SIZEGRIP 样式组合在一起, 因为即使系统在状态窗口中绘制, 生成的大小调整手柄也不起作用。

若要创建具有扩展窗口样式的状态栏, 请调用[CStatusBarCtrl:: CreateEx](#createex)而`Create`不是。
...
创建一个控件 (子窗口) 并将`CStatusBarCtrl`其与对象关联。
...
默认情况下, 此成员函数不执行任何操作。 重写此成员函数以实现所有者描述`CStatusBarCtrl`对象的绘制。
...
此方法发送[SB_GETICON](/windows/win32/Controls/sb-geticon)消息, 如 Windows SDK 中所述。
...
下面的代码示例定义了一个用于`m_statusBar`访问当前状态栏控件的变量。 此变量将在下一个示例中使用。
...
- SBT_POPOUT 绘制文本时, 将显示一个边框, 使其显示在状态栏上方。

- SBT_OWNERDRAW 如果文本具有 SBT_OWNERDRAW 绘制类型, 则*pType*接收此消息, 并返回与文本关联的32位值, 而不是长度和操作类型。
...
*lpszText*<br/>
用于指定要设置的文本且以 Null 结尾的字符串的地址。 如果*n*为 SBT_OWNERDRAW, 则*lpszText*表示32位数据。
