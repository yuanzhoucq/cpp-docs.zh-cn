...
*pstrAgent*  
指向用于标识应用程序或调用 Internet 函数 （例如，"Microsoft Internet 浏览器"） 的实体的名称的字符串的指针。 如果*pstrAgent*是**NULL** （默认值） 时，框架会调用全局函数[AfxGetAppName](application-information-and-management.md#afxgetappname)，这将返回包含应用程序的以 null 结尾的字符串名称。 某些协议使用此字符串来标识应用程序到服务器。
...
- `INTERNET_OPEN_TYPE_PRECONFIG` 在注册表中使用预配置的设置进行连接。 此访问类型被设置为默认值。 若要连接通过 TIS 代理，设置*dwAccessType*为此值; 你然后注册表正确设置。
...
`CInternetSession` 第一个 Internet 函数称为应用程序。 它将初始化内部数据结构，并为将来的调用，从应用程序做好准备。
...
- `ERROR_NO_MORE_ITEMS` 没有为指定的 URL 的任何 cookie，所有其父级。
...
*dwflags*  
任意组合`INTERNET_FLAG_*`标志。 请参阅中的表**备注**部分[chttpconnection::](../../mfc/reference/chttpconnection-class.md#openrequest)有关的说明*dwFlags*值。
...
*pstrURL*  
指向要开始读取的 url 的名称的指针。 仅 Url 开头文件:，ftp:，gopher:，或 http： 支持。 断言如果*pstrURL*是**NULL**。
...
- `INTERNET_FLAG_TRANSFER_BINARY` 将文件传输为二进制文件。
...
指针的`OpenURL`取决于返回*pstrURL*的服务类型。 下表说明了可能的指针`OpenURL`可以返回。
