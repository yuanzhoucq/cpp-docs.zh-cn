...
- PS_JOIN_MITER 联接是斜时设置的当前限制内[SetMiterLimit](http://msdn.microsoft.com/library/windows/desktop/dd145076)函数。 如果联接超出此限制，它被凸凹。  
 ...
 *pLogBrush*  
 指向[LOGBRUSH](http://msdn.microsoft.com/library/windows/desktop/dd145035)结构。 如果*nPenStyle*是 PS_COSMETIC，`lbColor`的成员`LOGBRUSH`结构指定的钢笔颜色并*lbStyle*隶属`LOGBRUSH`结构必须设置为 BS_纯色。 如果 nPenStyle PS_GEOMETRIC，所有成员必须都用于指定触笔的画笔属性。  
