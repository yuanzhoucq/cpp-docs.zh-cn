...
有关这些运行时对象服务和序列化的详细信息, 请参阅[CObject 类:访问运行时类信息](../../mfc/accessing-run-time-class-information.md)。
...
|[DECLARE_OLECTLTYPE](#declare_olectltype)|声明控件`GetUserTypeNameID`类`GetMiscStatus`的和成员函数。|
|[DECLARE_PROPPAGEIDS](#declare_proppageids)|声明 OLE 控件提供了一个属性页列表以显示其属性。|
|[IMPLEMENT_OLECREATE](#implement_olecreate)|支持由 OLE 系统创建对象。|
|[IMPLEMENT_OLECTLTYPE](#implement_olectltype)|实现控件`GetUserTypeNameID`类`GetMiscStatus`的和成员函数。|
...
在从`CObject`派生类时, 添加了访问对象的类的运行时信息的能力。
...
如果使用 DECLARE_ 动态和 IMPLEMENT_DYNAMIC 宏 (如所述), 则可在运行时使用 RUNTIME_CLASS 宏`CObject::IsKindOf`和函数来确定对象的类。

如果类声明中包含 DECLARE_DYNAMIC, 则 IMPLEMENT_DYNAMIC 必须包含在类实现中。
...
如果类声明中包含 DECLARE_DYNCREATE, 则 IMPLEMENT_DYNCREATE 必须包含在类实现中。
...
声明控件`GetUserTypeNameID`类`GetMiscStatus`的和成员函数。
...
`GetUserTypeNameID`和`GetMiscStatus`是中声明的`COleControl`纯虚函数。 由于这些函数是纯虚函数, 因此必须在控件类中进行重写。 除了 DECLARE_OLECTLTYPE, 还必须将 IMPLEMENT_OLECTLTYPE 宏添加到控件类声明。
...
在类声明的末尾使用宏。`DECLARE_PROPPAGEIDS` 然后, 在定义类的成员函数的 .cpp 文件中, 使用`BEGIN_PROPPAGEIDS`宏、控件的每个属性页的宏条目`END_PROPPAGEIDS`和宏声明属性页列表的末尾。
...
如果类声明中包含 DECLARE_SERIAL, 则 IMPLEMENT_SERIAL 必须包含在类实现中。

DECLARE_SERIAL 宏包括 DECLARE_DYNAMIC 和 DECLARE_DYNCREATE 的所有功能。

可以使用 AFX_API 宏为使用 DECLARE_SERIAL 和 IMPLEMENT_SERIAL 宏`CArchive`的类自动导出提取运算符。 用下面的代码将类声明 (位于 .h 文件中) 括起来:
...
生成动态C++ `CObject`派生的类所需的代码, 该代码具有对类名称和层次结构内位置的运行时访问。
...
当与 DECLARE_DYNCREATE `CObject`宏一起使用时, 允许在运行时动态创建派生类的对象。
...
此框架使用此功能来动态创建新对象, 例如, 在序列化期间从磁盘读取对象时。 在类实现文件中添加 IMPLEMENT_DYNCREATE 宏。 有关详细信息, 请参阅[CObject 类主题](../../mfc/using-cobject.md)。
...
> [!NOTE]
>  如果使用 IMPLEMENT_OLECREATE_FLAGS, 可以使用*nFlags*参数指定对象支持的线程模型。 如果只想支持单一 treading 模型, 请使用 IMPLEMENT_OLECREATE。
...
实现控件`GetUserTypeNameID`类`GetMiscStatus`的和成员函数。
...
除了 IMPLEMENT_OLECTLTYPE, 还必须将 DECLARE_OLECTLTYPE 宏添加到控件类声明。
...
> [!NOTE]
>  ActiveX ControlWizard 使用的默认设置如下:OLEMISC_ACTIVATEWHENVISIBLE、OLEMISC_SETCLIENTSITEFIRST、OLEMISC_INSIDEOUT、OLEMISC_CANTLINKINSIDE 和 OLEMISC_RECOMPOSEONRESIZE。
...
RUNTIME_CLASS 返回指向*class_name*指定的类的[CRuntimeClass](../../mfc/reference/cruntimeclass-structure.md)结构的指针。 只有`CObject`通过 DECLARE_DYNAMIC、DECLARE_DYNCREATE 或 DECLARE_SERIAL 声明的派生类才能返回指向结构的`CRuntimeClass`指针。
...
允许通过 OLE `CCmdTarget`自动化创建派生类的对象。
...
如果类声明中包含 DECLARE_OLECREATE, 则 IMPLEMENT_OLECREATE 必须包含在类实现中。 使用 DECLARE_OLECREATE 的类声明还必须使用 DECLARE_DYNCREATE 或 DECLARE_SERIAL。
...
> [!NOTE]
>  如果使用的是 IMPLEMENT_OLECREATE, 则默认情况下, 仅支持单线程模型。 如果使用 IMPLEMENT_OLECREATE_FLAGS, 可以使用*nFlags*参数指定对象支持的线程模型。
