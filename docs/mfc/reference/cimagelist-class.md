...
|[CImageList：： Copy](#copy)|复制对象中的图像 `CImageList` 。|
|[CImageList：： Create](#create)|初始化图像列表，并将其附加到 `CImageList` 对象。|
|[CImageList：:D eleteImageList](#deleteimagelist)|删除图像列表。|
|[CImageList：:D eleteTempMap](#deletetempmap)|由[CWinApp](../../mfc/reference/cwinapp-class.md)空闲时间处理程序调用，以删除 `CImageList` 由创建的任何临时对象 `FromHandle` 。|
|[CImageList：:D etach](#detach)|从对象分离图像列表对象 `CImageList` 并返回图像列表的句柄。|
|[CImageList：:D ragEnter](#dragenter)|在拖动操作过程中锁定更新，并在指定位置显示拖动图像。|
|[CImageList：:D ragLeave](#dragleave)|解锁窗口并隐藏拖动图像，以便可以更新窗口。|
|[CImageList：:D ragMove](#dragmove)|在拖放操作过程中移动正在拖动的图像。|
|[CImageList：:D ragShowNolock](#dragshownolock)|在拖动操作期间显示或隐藏拖动图像，而不锁定窗口。|
|[CImageList：:D raw](#draw)|绘制拖放操作过程中正在拖动的图像。|
|[CImageList：:D rawEx](#drawex)|在指定的设备上下文中绘制图像列表项。 函数使用指定的绘制样式，并将图像与指定的颜色混合。|
|[CImageList：:D rawIndirect](#drawindirect)|从图像列表绘制图像。|
|[CImageList：： EndDrag](#enddrag)|结束拖动操作。|
|[CImageList：： ExtractIcon](#extracticon)|基于图像列表中的图像和掩码创建图标。|
|[CImageList：： FromHandle](#fromhandle)|`CImageList`当给定图像列表的句柄时，返回指向对象的指针。 如果 `CImageList` 对象未附加到该句柄，则会创建并附加一个临时 `CImageList` 对象。|
|[CImageList：： FromHandlePermanent](#fromhandlepermanent)|`CImageList`当给定图像列表的句柄时，返回指向对象的指针。 如果 `CImageList` 对象未附加到句柄，则返回 NULL。|
...
|名称|描述|
|----------|-----------------|
|[CImageList：： operator HIMAGELIST](#operator_himagelist)|返回附加到的 HIMAGELIST `CImageList` 。|
...
有关使用的详细信息 `CImageList` ，请参阅[控件](../../mfc/controls-mfc.md)和[使用 CImageList](../../mfc/using-cimagelist.md)。
...
此函数创建用于拖动的临时图像列表。 图像将指定的图像及其掩码与当前光标组合在一起。 为了响应后续 WM_MOUSEMOVE 消息，您可以使用成员函数移动拖动图像 `DragMove` 。 若要结束拖动操作，可以使用 `EndDrag` 成员函数。
...
|值|含义|
|-----------|-------------|
|ILC_COLOR|如果未指定任何其他 ILC_COLOR * 标志，则使用默认行为。 通常情况下，默认值为 ILC_COLOR4;但对于较旧的显示驱动程序，默认值为 ILC_COLORDDB。|
|ILC_COLOR4|使用4位（16色）与设备无关的位图（DIB）部分作为图像列表的位图。|
...
可以通过 `CImageList` 两个步骤构造。 首先，调用构造函数，然后调用 `Create` ，它将创建图像列表，并将其附加到 `CImageList` 对象。
...
由 `CWinApp` 空闲时间处理程序自动调用， `DeleteTempMap` 删除 `CImageList` 由[FromHandle](#fromhandle)创建的所有临时对象，但不会销毁 `hImageList` 暂时与对象关联的任何句柄（） `ImageList` 。
...
|值|含义|
|-----------|-------------|
|ILD_BLEND25，ILD_FOCUS|绘制图像，将25% 与系统突出显示颜色混合。 如果图像列表不包含掩码，此值将不起作用。|
|ILD_BLEND50、ILD_SELECTED ILD_BLEND|绘制图像，将50% 与系统突出显示颜色混合。 如果图像列表不包含掩码，此值将不起作用。|
|ILD_MASK|绘制掩码。|
...
*sz*<br/>
相对于图像的左上角绘制的图像部分的大小。 请参阅 Windows SDK [ImageList_DrawEx](/windows/win32/api/commctrl/nf-commctrl-imagelist_drawex)中的*dx*和*dy* 。

*clrBk*<br/>
图像的背景色。 请参阅 Windows SDK [ImageList_DrawEx](/windows/win32/api/commctrl/nf-commctrl-imagelist_drawex)中的*rgbBk* 。

*clrFg*<br/>
图像的前景色。 请参阅 Windows SDK [ImageList_DrawEx](/windows/win32/api/commctrl/nf-commctrl-imagelist_drawex)中的*rgbFg* 。

*nStyle*<br/>
指定绘制样式的标志。 请参阅 Windows SDK [ImageList_DrawEx](/windows/win32/api/commctrl/nf-commctrl-imagelist_drawex)中的*fStyle* 。
...
*五*<br/>
一个[点](/windows/win32/api/windef/ns-windef-point)结构，其中包含将在其中绘制图像的 x 坐标和 y 坐标。

*sz*<br/>
一个[大小](/windows/win32/api/windef/ns-windef-size)结构，指示要绘制的图像的大小。

*ptOrigin*<br/>
一个[点](/windows/win32/api/windef/ns-windef-point)结构，其中包含指定绘制操作的左上角的 x 坐标和 y 坐标（相对于图像本身）。 不绘制 x 坐标左侧和上方 y 坐标上方的图像的像素。
...
*rgbBack*<br/>
默认情况下，图像背景色 CLR_DEFAULT。 此参数可以是应用程序定义的 RGB 值，也可以是以下值之一：
...
*rgbFore*<br/>
图像前景色，默认情况下 CLR_DEFAULT。 此参数可以是应用程序定义的 RGB 值，也可以是以下值之一：
...
与 ILS_APLHA 一起使用时，此成员保存 alpha 通道的值。 此值可以是从0到255的值，0是完全透明的，255完全不透明。
...
覆盖图像是在主图像上绘制的图像，该图像在此成员函数中由*n*参数指定。 使用带有[INDEXTOOVERLAYMASK](/windows/win32/api/commctrl/nf-commctrl-indextooverlaymask)宏指定的覆盖掩码的从 1[开始的索引](#draw)，绘制覆盖掩码。
...
`CImageList`当给定图像列表的句柄时，返回指向对象的指针。
...
如果成功，则为指向对象的指针 `CImageList` ; 否则为 NULL。

### <a name="remarks"></a>备注

如果 `CImageList` 尚未附加到句柄，则 `CImageList` 会创建并附加一个临时对象。 此临时 `CImageList` 对象仅在下一次应用程序的事件循环中有空闲时间之前有效，此时将删除所有临时对象。
...
如果 `CImageList` 对象未附加到句柄，则返回 NULL。
...
*lpPoint*<br/>
接收当前拖动位置的[点](/windows/win32/api/windef/ns-windef-point)结构的地址。
...
如果成功，则为对象表示的图像列表的句柄 `CImageList` ; 否则为 NULL。
...
*pArchive*<br/>
指向对象的指针，将 `CArchive` 从该对象中读取图像列表。
...
如果成功，则返回的版本 **`int`** 返回映像的从零开始的索引; 否则为-1。
...
*回车*<br/>
要设置的背景色。 它可以 CLR_NONE。 在这种情况下，将使用掩码以透明方式绘制图像。

### <a name="return-value"></a>返回值

如果成功，则为上一种背景色;否则 CLR_NONE。
...
由于拖动函数在拖动操作期间使用新图像，因此应在调用后使用 Windows [ShowCursor](/windows/win32/api/winuser/nf-winuser-showcursor)函数隐藏实际的鼠标光标 `CImageList::SetDragCursorImage` 。 否则，系统在拖动操作期间可能看起来具有两个鼠标光标。

## <a name="cimagelistsetimagecount"></a><a name="setimagecount"></a>CImageList：： SetImageCount

调用此成员函数可重置对象中的图像数量 `CImageList` 。
...
*pArchive*<br/>
一个指针，指向要 `CArchive` 在其中存储图像列表的对象。
