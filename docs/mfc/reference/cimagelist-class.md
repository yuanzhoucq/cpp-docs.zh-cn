...
|[CImageList：： Create](#create)|初始化图像列表，并将其附加到 `CImageList` 的对象。|
|[CImageList：:D eleteImageList](#deleteimagelist)|删除图像列表。|
|[CImageList：:D eleteTempMap](#deletetempmap)|由[CWinApp](../../mfc/reference/cwinapp-class.md)空闲时间处理程序调用以删除 `FromHandle`创建的任何临时 `CImageList` 对象。|
|[CImageList：:D etach](#detach)|从 `CImageList` 对象分离图像列表对象并返回图像列表的句柄。|
...
|[CImageList：： FromHandle](#fromhandle)|当给定图像列表的句柄时，返回指向 `CImageList` 对象的指针。 如果 `CImageList` 对象未附加到该句柄，则会创建并附加一个临时 `CImageList` 对象。|
|[CImageList：： FromHandlePermanent](#fromhandlepermanent)|当给定图像列表的句柄时，返回指向 `CImageList` 对象的指针。 如果 `CImageList` 对象未附加到句柄，则返回 NULL。|
...
|名称|说明|
|----------|-----------------|
|[CImageList：： operator HIMAGELIST](#operator_himagelist)|返回附加到 `CImageList`的 HIMAGELIST。|
...
此控件（因此 `CImageList` 类）仅适用于在 Windows 95/98 和 Windows NT 版本3.51 及更高版本下运行的程序。
...
此函数创建用于拖动的临时图像列表。 图像将指定的图像及其掩码与当前光标组合在一起。 为了响应后续 WM_MOUSEMOVE 消息，您可以使用 `DragMove` 成员函数移动拖动图像。 若要结束拖动操作，可以使用 `EndDrag` 成员函数。
...
*nFlags*<br/>
指定要创建的图像列表的类型。 此参数可以是下列值的组合，但它只能包含 `ILC_COLOR` 值之一。
...
可以通过两个步骤构造 `CImageList`。 首先，调用构造函数，然后调用 `Create`，这将创建图像列表，并将其附加到 `CImageList` 对象。
...
由 `CWinApp` 空闲时间处理程序自动调用，`DeleteTempMap` 删除由[FromHandle](#fromhandle)创建的所有临时 `CImageList` 对象，但不会销毁暂时与 `ImageList` 对象关联的任何句柄（`hImageList`）。
...
当给定图像列表的句柄时，返回指向 `CImageList` 对象的指针。
...
如果 `CImageList` 尚未附加到句柄，则会创建一个临时的 `CImageList` 对象并将其附加。 此临时 `CImageList` 对象仅在下一次应用程序的事件循环中有空闲时间之前有效，在这段时间内删除所有临时对象。
...
由于拖动函数在拖动操作过程中使用新图像，因此在调用 `CImageList::SetDragCursorImage`后，您应该使用 Windows [ShowCursor](/windows/win32/api/winuser/nf-winuser-showcursor)函数隐藏实际的鼠标光标。 否则，系统在拖动操作期间可能看起来具有两个鼠标光标。

##  <a name="setimagecount"></a>CImageList：： SetImageCount

调用此成员函数可重置 `CImageList` 对象中的映像数。
