...
类和消息映射机制`WndProc`隐藏函数。 `CWnd` 传入的 Windows 通知消息会通过消息映射自动路由**到正确的**<em>消息</em> `CWnd`成员函数。 重写**On**<em>消息</em>成员函数以便处理派生类中的成员特定消息。
...
*nCheck*<br/>
指定要执行的操作。 如果*n*为非零值`CheckDlgButton` ，则成员函数将在该按钮旁边放置一个复选标记; 如果为0，则删除复选标记。 对于三状态按钮，如果*n*为2，则按钮状态为 "不确定"。
...
*nIDListBox*<br/>
指定列表框的标识符。 如果*nIDListBox*为0， `DlgDirList`则假定不存在列表框，也不会尝试填充一个列表框。

*nIDStaticPath*<br/>
指定用于显示当前驱动器和目录的静态文本控件的标识符。 如果*nIDStaticPath*为0， `DlgDirList`则假定不存在此类文本控件。
...
*nIDComboBox*<br/>
指定对话框中组合框的标识符。 如果*nIDComboBox*为0， `DlgDirListComboBox`则假定不存在任何组合框，且不会尝试填充组合框。

*nIDStaticPath*<br/>
指定用于显示当前驱动器和目录的静态文本控件的标识符。 如果*nIDStaticPath*为0， `DlgDirListComboBox`则假定不存在此类文本控件。
...
如果`CWnd`是用 CS_OWNDC 样式创建的，并且映射模式不是 MM_TEXT，则`GetUpdateRect`成员函数将以逻辑坐标为矩形。 否则， `GetUpdateRect`将在工作区坐标中提供矩形。 如果没有更新区域， `GetUpdateRect`则将该矩形设置为空（将所有坐标设置为0）。
...
如果*为 TRUE，则在*所有应用程序都从处理此调用返回后，Windows 可能会终止。 因此，让应用程序在中`OnEndSession`执行终止所需的所有任务。
...
如果重`OnEraseBkgnd`写处理消息并清除背景，则该重写应返回非零值以响应 WM_ERASEBKGND; 这表示不需要进一步擦除。 如果返回0，则窗口将保持标记为需要擦除。 （通常，这意味着`fErase` `PAINTSTRUCT`结构的成员将为 TRUE。）
...
WHEEL_DELTA 设置为120，以允许更精细的轮式轮，例如无凹槽的自由旋转滚轮。 更精细的轮轮按旋转发送更多的消息，但每条消息的增量值更小。 若要使用此类轮，请在达到 WHEEL_DELTA 之前添加传入的*zDelta*值（以便为给定的增量旋转获得相同的响应），或滚动部分行以响应更频繁的消息。 还可以选择滚动粒度并累积增量，直到达到 WHEEL_DELTA。
...
如果调用`RedrawWindow`成员函数并设置了 RDW_INTERNALPAINT 标志，则窗口可能会收到内部绘制消息。 在这种情况下，该窗口可能没有更新区域。 应用程序应调用[GetUpdateRect](#getupdaterect)成员函数来确定窗口是否有更新区域。 如果`GetUpdateRect`返回0，则应用程序不应调用[BeginPaint](#beginpaint)和[EndPaint](#endpaint)成员函数。
...
如果任何应用程序返回0，则不会结束 Windows 会话。 如果一个应用`OnQueryEndSession`程序返回0，Windows 将停止调用，并为任何已返回非零值的应用程序发送参数值为 FALSE 的[WM_ENDSESSION](#onendsession)消息。
...
- SW_SCROLLCHILDREN 滚动通过*lpRectScroll*中指定的像素*数与所*指向的矩形相交的所有子*窗口。* Windows 会向交叉*lpRectScroll*的所有子窗口发送[WM_MOVE](/windows/win32/winmsg/wm-move)消息，即使它们不移动也是如此。 当滚动子窗口并且光标矩形与滚动矩形相交时，插入符号会重新定位。
...
`SetScrollRange`如果对[SetScrollPos](#setscrollpos)成员函数的调用立即调用，则`SetScrollPos`成员函数中的*bRedraw*参数应为0，以防滚动条绘制两次。
