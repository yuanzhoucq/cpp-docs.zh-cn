...
### <a name="return-value"></a>返回值  
 窗口的样式。 有关在 MFC 中使用的窗口样式的详细信息，请参阅[窗口样式](styles-used-by-mfc.md#window-styles)。  
...
### <a name="remarks"></a>备注  
 ...
 `LockWindowUpdate`成员函数不会使给定的窗口不可见，并不会清除[WS_VISIBLE](styles-used-by-mfc.md#window-styles)样式位。  
