---
title: "CPaneFrameWnd 类 |Microsoft 文档"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|`CPaneFrameWnd::PreTranslateMessage`|类使用[CWinApp](../../mfc/reference/cwinapp-class.md)将窗口消息转换之前调度到[TranslateMessage](http://msdn.microsoft.com/library/windows/desktop/ms644955)和[DispatchMessage](http://msdn.microsoft.com/library/windows/desktop/ms644934) Windows 函数。|  
...
## <a name="remarks"></a>备注  
 ...
 微型框架窗口根据包含的窗格样式在标题上显示按钮。 如果可以关闭窗格中 ( [CBasePane::CanBeClosed](../../mfc/reference/cbasepane-class.md#canbeclosed))，它将显示关闭按钮。 如果窗格具有 `AFX_CBRS_AUTO_ROLLUP` 样式，它将显示指针。  
  
 如果从 `CPaneFrameWnd` 派生类，则必须告诉框架如何创建类。 可以通过重写创建该类[CPane::CreateDefaultMiniframe](../../mfc/reference/cpane-class.md#createdefaultminiframe)，或者设置`CPane::m_pMiniFrameRTC`成员，使其指向您的类的运行时类信息。  
...
### <a name="parameters"></a>参数  
 ...
 [out] `bDrawTab`  
 如果`TRUE`，绘制一个选项卡。 如果`FALSE`，不绘制一个选项卡。  
...
### <a name="return-value"></a>返回值  
 非零，如果最小化框架可停靠到`pDockingBar`; 否则为 0。  
...
##  <a name="create"></a>CPaneFrameWnd::Create  
 创建一个袖珍框架窗口，并将其附加到[CPaneFrameWnd](../../mfc/reference/cpaneframewnd-class.md)对象。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `dwStyle`  
 指定的窗口样式。 有关详细信息，请参阅[窗口样式](../../mfc/reference/window-styles.md)。  
...
### <a name="remarks"></a>备注  
 在两个步骤中创建了袖珍框架窗口。 首先，框架会创建`CPaneFrameWnd`对象。 其次，它调用`Create`创建 Windows 袖珍框架窗口，并将其附加到`CPaneFrameWnd`对象。  
...
### <a name="parameters"></a>参数  
 [in] `dwStyleEx`  
 指定扩展的窗口样式。 有关详细信息，请参阅[扩展窗口样式](../../mfc/reference/extended-window-styles.md)  
...
### <a name="parameters"></a>参数  
 [out] `bWasDocked`  
 `TRUE`如果已停靠窗格中;，否则为`FALSE`。  
  
### <a name="return-value"></a>返回值  
 如果该操作成功，`CDockablePane`窗格中，已插接为; 否则为`NULL`。  
...
### <a name="return-value"></a>返回值  
 具有指定的控件 ID; 窗格中否则为`NULL`，如果没有窗格具有指定的控件 id。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `nSensitivity`  
 增加此大小的微型框架窗口的搜索区域。 如果给的定点落在增加的区域，微型框架窗口将满足搜索条件。  
  
 [in] `pFrameToExclude`  
 指定要从搜索中排除的微型框架窗口。  
  
 [in] `bFloatMultiOnly`  
 如果`TRUE`，仅搜索已的微型框架窗口`CBRS_FLOAT_MULTI`样式。 如果`FALSE`，搜索所有的微型框架窗口。  
  
### <a name="return-value"></a>返回值  
 指向包含的微型框架窗口的指针`pt`; 否则为`NULL`。  
...
### <a name="remarks"></a>备注  
 调用此方法来确定最小化框架窗口的高度。 默认情况下，高度设置为`SM_CYSMCAPTION`。 有关详细信息，请参阅[GetSystemMetrics 函数](http://msdn.microsoft.com/library/windows/desktop/ms724385)。  
...
### <a name="parameters"></a>参数  
 [out] `rectCaption`  
 包含的大小和位置的微型框架窗口标题，在屏幕坐标。  
  
### <a name="remarks"></a>备注  
 若要计算的微型框架窗口标题的边框的框架调用此方法。  
...
### <a name="return-value"></a>返回值  
 微型框架窗口中的第一个窗格或`NULL`如果微型框架窗口包含无法窗格。  
...
### <a name="return-value"></a>返回值  
 最小化框架中包含窗格中或`NULL`如果微型框架窗口包含无法窗格。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `bDetectCaption`  
 如果`TRUE`，检查点的针对标题。 如果`FALSE`，忽略的标题。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果必须向; 下滚微型框架窗口否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 要确定是否应该向下滚微型框架窗口的框架调用此方法。 如果它包含至少一个窗格，它汇总/下滚功能的微型框架窗口启用`AFX_CBRS_AUTO_ROLLUP`标志。 在创建窗格时，设置此标志。 有关详细信息，请参阅[cbasepane:: Createex](../../mfc/reference/cbasepane-class.md#createex)。  
  
 默认情况下，框架都将检查是否将鼠标指针位于的微型框架窗口边框以确定窗口是否具有向下滚。 您可以重写此行为在派生类中。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果必须汇总微型框架窗口;否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 要确定是否应汇总的微型框架窗口的框架调用此方法。 如果它包含至少一个窗格，它汇总/下滚功能的微型框架窗口启用`AFX_CBRS_AUTO_ROLLUP`标志。 在创建窗格时，设置此标志。 有关详细信息，请参阅[cbasepane:: Createex](../../mfc/reference/cbasepane-class.md#createex)。  
  
 默认情况下，框架都将检查是否将鼠标指针位于的微型框架窗口边框以确定窗口是否具有要进行汇总。 您可以重写此行为在派生类中。  
...
### <a name="remarks"></a>备注  
 此静态成员设置为`TRUE`注册具有的微型框架窗口类`CS_SAVEBITS`样式。 这可能有助于减少闪烁当用户拖动微型框架窗口。  
...
### <a name="remarks"></a>备注  
 由框架来确定是否应回滚的微型框架窗口，向上或向下调用此方法。  
  
 默认情况下，框架将调用[CPaneFrameWnd::IsRollUp](#isrollup)和[CPaneFrameWnd::IsRollDown](#isrolldown)只拉伸或还原微型框架窗口。 您可以重写此方法在派生类来使用不同的可视化效果。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `bShow`  
 `TRUE`如果当前显示窗格中;`FALSE`如果要隐藏窗格。  
  
### <a name="remarks"></a>备注  
 显示或隐藏微型框架窗口的窗格中时由框架调用。 默认实现不执行任何操作。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `bCheckVisibility`  
 `TRUE`若要指定应返回仅可见的窗格的;否则为`FALSE`。  
  
### <a name="return-value"></a>返回值  
 用户单击了，窗格中或`NULL`没有窗格在该位置不存在。  
...
### <a name="remarks"></a>备注  
 此方法通过调用更新所有的微型框架窗口[CWnd::RedrawWindow](../../mfc/reference/cwnd-class.md#redrawwindow)为每个窗口。  
...
### <a name="parameters"></a>参数  
 ...
 [in] `bDestroy`  
 指定到微型框架窗口的操作。 如果`bDestroy`是`TRUE`，此方法将立即销毁微型框架窗口。 如果它是`FALSE`，此方法在特定的延迟后销毁微型框架窗口。  
  
 [in] `bNoDelayedDestroy`  
 如果`TRUE`、 延迟析构被禁用。 如果`FALSE`、 延迟启用析构。  
  
### <a name="remarks"></a>备注  
 立即或在特定的延迟后，框架可以销毁微型框架窗口。 如果您想要延迟的最小化框架窗口析构，传递`FALSE`中`bNoDelayedDestroy`参数。 框架处理时发生延迟的析构`AFX_WM_CHECKEMPTYMINIFRAME`消息。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果未停靠状态; 微型框架窗口`FALSE`如果它停靠。  
  
##  <a name="sizetocontent"></a>CPaneFrameWnd::SizeToContent  
 调整的微型框架窗口的大小，以便它等效于包含的窗格中。  
