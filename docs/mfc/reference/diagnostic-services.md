...
调用此函数可在执行 MFC 应用程序的调试版本时，导致中断（在调用 `AfxDebugBreak`的位置）。
...
`AfxDebugBreak` 在 MFC 应用程序的发布版本中不起作用，因此应将其删除。 此函数只应在 MFC 应用程序中使用。 使用 Win32 API 版本 `DebugBreak`，导致非 MFC 应用程序中断。
...
*Pobject*参数应为指向对象的指针，并且可以是**常量**。 指向的对象和类必须支持 `CObject` 的运行时类信息。 例如，为了确保 `pDocument` 是指向 `CMyDoc` 类的对象或它的任何派生对象的指针，可以编写代码：
...
ASSERT_VALID 调用作为参数传递的对象的 `AssertValid` 成员函数。
...
你可以在程序中的任何位置使用 DEBUG_NEW，你通常会使用**NEW**运算符来分配堆存储。
...
> [!NOTE]
>  在以前版本的 MFC （4.1 及更早版本）中，需要在调用 IMPLEMENT_DYNCREATE 或 IMPLEMENT_SERIAL 宏的所有语句之后放置 `#define` 语句。 现在已没有必要。
...
DEBUG_ONLY 宏等效于 `#ifdef _DEBUG` 和 `#endif`的周围*表达式*。
...
`afxDump` 是预定义的[CDumpContext](../../mfc/reference/cdumpcontext-class.md)对象，它允许你将 `CDumpContext` 信息发送到调试器输出窗口或调试终端。 通常，提供 `afxDump` 作为 `CObject::Dump`的参数。

在 Windows NT 和 Windows 的所有版本下，调试应用程序时，会将 `afxDump` 输出发送到C++ Visual 的 "输出"-"调试" 窗口。

此变量仅在 MFC 的调试版本中定义。 有关 `afxDump`的详细信息，请参阅[调试 MFC 应用程序](/visualstudio/debugger/mfc-debugging-techniques)。
...
程序代码不应调用 `AfxDump`，而应改为调用适当对象的 `Dump` 成员函数。
...
- `delayFreeMemDF` 释放内存的延迟时间。 当程序释放内存块时，分配器不会将该内存返回到基础操作系统。 这会对程序施加最大的内存压力。

- 每次分配或释放内存时 `AfxCheckMemory` `checkAlwaysMemDF` 调用。 这会大大降低内存分配和释放的速度。
...
将检查当前在堆上分配的所有内存块，包括**新**分配的内存块，而不是通过直接调用基础内存分配器分配的内存块，如**malloc**函数或 `GlobalAlloc` Windows 函数。 如果发现任何块已损坏，则会向调试器输出输出一条消息。

如果包括行

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

在程序模块中，对的后续调用 `AfxCheckMemory` 显示分配内存的文件名和行号。

> [!NOTE]
>  如果模块包含可序列化类的一个或多个实现，则必须将 `#define` 行放在上一次 IMPLEMENT_SERIAL 宏调用之后。
...
下面的示例反映了从 MFC 对话框应用程序中的按钮处理程序调用 `AfxDumpStack` 而生成的单个行：
...
|`+ 181 bytes`|从函数原型的地址（在本例中为 `void AfxDumpStack(unsigned long)`）到返回地址（在本例中为 `00427D55`）的偏移量（以字节为单位）。|

`AfxDumpStack` 在 MFC 库的 debug 和 nondebug 版本中可用;但是，即使可执行文件在共享 DLL 中使用 MFC，此函数也始终以静态方式链接。 在共享库实现中，在 MFCS42 中找到该函数。LIB 库（及其变体）。
...
有关 `AfxEnableMemoryTracking`的详细信息，请参阅[调试 MFC 应用程序](/visualstudio/debugger/mfc-debugging-techniques)。
...
*bObject*<br/>
如果分配针对 `CObject`派生的对象，则为 TRUE;否则为 FALSE。
...
为应用程序的内存空间中的所有可序列化 `CObject`派生类调用指定的迭代函数。
...
可序列化 `CObject`派生类是使用 DECLARE_SERIAL 宏派生的类。 每次调用时，传递给*pContext*中 `AfxDoForAllClasses` 的指针都会传递给指定的迭代函数。
...
为从已分配**新**的 `CObject` 派生的所有对象执行指定的迭代函数。
...
不枚举 Stack、global 或 embedded 对象。 每次调用时，传递给*pContext*中 `AfxDoForAllObjects` 的指针都会传递给指定的迭代函数。
