...
|[CBasePane：:D ockPane](#dockpane)|将一个窗格停靠到另一个窗格或框架窗口。|
|[CBasePane：:D ockPaneUsingRTTI](#dockpaneusingrtti)|使用运行时类型信息停靠窗格。|
|[CBasePane：:D ockToFrameWindow](#docktoframewindow)|将可停靠的窗格停靠到帧。|
|[CBasePane：:D oesAllowDynInsertBefore](#doesallowdyninsertbefore)|确定是否可以在此窗格和父框架之间动态插入另一个窗格。|
...
|`CBasePane::get_accParent`|由框架调用以检索对象的父对象的 `IDispatch` 接口。 （重写[CWnd：： get_accParent](../../mfc/reference/cwnd-class.md#get_accparent)。）|
...
|[CBasePane：： GetEnabledAlignment](#getenabledalignment)|返回应用于窗格的 CBRS_ALIGN_ 样式。|
...
|[CBasePane：:P aneFromPoint](#panefrompoint)|返回包含给定点的窗格。|
...
|名称|说明|
|----------|-----------------|
|[CBasePane：:D oPaint](#dopaint)|填充窗格的背景。|

## <a name="remarks"></a>备注

如果要创建一个窗格类，该类支持 MFC 中可用的扩展停靠功能，则必须从 `CBasePane` 或从[CPane 类](../../mfc/reference/cpane-class.md)派生。

## <a name="customization-tips"></a>自定义提示

以下自定义提示与 `CBasePane Class` 以及从其继承的任何类相关：
...
- 如果你想要创建一个静态窗格，该窗格将无法浮动，并阻止任何其他窗格（类似于 OutlookDemo 示例中的 Outlook 栏），请将其创建为非浮动的，并重写[CBasePane：:D oesallowdyninsertbefore](#doesallowdyninsertbefore)返回 FALSE。 如果创建的窗格没有 AFX_CBRS_FLOAT 样式，则默认实现将返回 FALSE。
...
此方法检查 AFX_CBRS_RESIZE 标志，默认情况下，它在 `CBasePane::OnCreate`中指定。 如果未指定此标志，则停靠管理器将在内部将窗格标记为可移动，而不是将其停靠。
...
如果窗格可以转换为选项卡式文档，则为 TRUE;否则为 FALSE。 `CBasePane::CanBeTabbedDocument` 总是返回 FALSE。
...
创建类 `lpszClassName`的窗口。 如果指定 WS_CAPTION，则此方法将清除 WS_CAPTION 样式位并将 `CBasePane::m_bHasCaption` 设置为 TRUE，因为库不支持带有标题的窗格。
...
`EnableDocking` 设置 `CBasePane::m_dwEnabledAlignment` 的内部标志，并且框架停靠时，框架将检查此标志。
...
`CBasePane` 不实现此方法。 如果*pvarChildren*为 NULL，则此方法返回 E_INVALIDARG。 否则，此方法将返回 DISP_E_MEMBERNOTFOUND。
...
如果拖动窗格在屏幕上通过拖动矩形指示 DT_STANDARD，则为。 DT_IMMEDIATE 是否拖动窗格的内容。

### <a name="remarks"></a>备注

框架调用此方法来确定窗格的当前停靠模式。

如果未定义 `CBasePane::m_dockMode` （DT_UNDEFINED），则将从全局停靠模式（`AFX_GLOBAL_DATA::m_dockModeGlobal`）获取停靠模式。

通过设置*m_dockMode*或覆盖 `GetDockingMode` 可以控制每个窗格的停靠模式。
...
如果面板停靠，则为指向 `CDockingPanesRow` 的指针; 否则为 NULL。
...
在创建时由[CBasePane：： SetPaneStyle](#setpanestyle)方法设置的控件条样式（包括 CBRS_ 样式）的组合。
...
使用此函数可检索指向父选项卡式窗口的指针。 有时调用 `GetParent`是不够的，因为窗格可能位于停靠包装器（ [CDockablePaneAdapter 类](../../mfc/reference/cdockablepaneadapter-class.md)）内或窗格适配器（ [CDockablePaneAdapter 类](../../mfc/reference/cdockablepaneadapter-class.md)）内。 通过使用 `GetParentTabWnd` 您可以在这些情况下检索有效指针（假定父对象是选项卡式窗口）。
...
只有可停靠的窗格可以在多窗格框架窗口中浮动。 因此，`CBasePane::IsInFloatingMultiPaneFrameWnd` 总是返回 FALSE。
...
如果设置 WS_VISIBLE，则为 TRUE;否则为 FALSE。
...
包含给定点的 `CBasePane`派生对象，如果未找到任何窗格，则为 NULL。
...
框架在将窗格的状态保存到注册表时调用此方法。 重写派生类中的 `SaveState` 以存储附加信息。
...
此方法可用于设置 afxres.h 中定义的任何 CBRS_ 样式。 由于窗格样式和窗格对齐方式存储在一起，因此，通过将其与当前对齐方式组合来设置新样式，如下所示。
...
如果*pWndInsertAfter*为 NULL，则此方法将调用[CWnd：： SetWindowPos](../../mfc/reference/cwnd-class.md#setwindowpos)。 如果*pWndInsertAfter*为非 NULL，此方法将调用 `DeferWindowPos`。
...
此方法显示或隐藏窗格。 使用此方法（而不是 `ShowWindow`），因为此方法会向相关的停靠管理器通知窗格可见性中的更改。
