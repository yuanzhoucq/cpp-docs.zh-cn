...
ms.locfileid: "68682350"
---
# <a name="cmfclistctrl-class"></a>CMFCListCtrl 类

类通过支持[CMFCHeaderCtrl 类](../../mfc/reference/cmfcheaderctrl-class.md)的高级标头控件功能, 扩展了[CListCtrl 类](../../mfc/reference/clistctrl-class.md)的功能。 `CMFCListCtrl`
...
`CMFCListCtrl`为[CListCtrl 类](../../mfc/reference/clistctrl-class.md)提供两个增强功能。 首先, 它指示列排序是可用选项, 方法是在标头上自动绘制排序箭头。 其次, 它支持同时对多个列进行数据排序。
...
一个整数, 指示两个项的相对位置。 负值指示第一项应在第二个项之前, 正值指示第一项应在第二项之后, 0 表示两项等效。
...
的`OnGetCellBkColor`默认实现不使用提供的输入参数, 而只是调用`GetBkColor`。 因此, 默认情况下, 整个列表控件将具有相同的背景色。 您可以在`OnGetCellBkColor`派生类中重写, 以使用单独的背景色标记各个单元。
...
默认情况下, 此方法`GetTextColor`将调用, 而不考虑输入参数。 整个 list 控件将具有相同的文本颜色。 可以在派生`OnGetCellTextColor`类中重写, 以使用单独的文本颜色标记各个单元。
