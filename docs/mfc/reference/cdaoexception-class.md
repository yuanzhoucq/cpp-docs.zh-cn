...
可以访问[CATCH](../../mfc/reference/exception-processing.md#catch)表达式范围内的异常对象。 还可以通过`CDaoException` [AfxThrowDaoException](../../mfc/reference/exception-processing.md#afxthrowdaoexception) global 函数从自己的代码中引发对象。

在 MFC 中，所有 DAO 错误都表示为类型`CDaoException`为的异常。 捕获此类型的异常时，可以使用`CDaoException`成员函数从数据库引擎错误集合中存储的任何 DAO 错误对象中检索信息。 发生每个错误时，将一个或多个错误对象放置在错误集合中。 （通常，该集合仅包含一个错误对象; 如果使用的是 ODBC 数据源，则更有可能获得多个错误对象。）当另一个 DAO 操作生成错误时，将清除错误集合，并将新的错误对象放置在错误集合中。 不生成错误的 DAO 操作不会影响 "错误" 集合。

有关 DAO 错误代码，请参阅文件 DAOERR。高. 有关相关信息，请参阅 DAO 帮助中的 "可捕获的数据访问错误" 主题。

有关常规处理或`CDaoException`对象的详细信息，请参阅文章[异常处理（MFC）](../../mfc/exception-handling-in-mfc.md)和[异常：数据库异常](../../mfc/exceptions-database-exceptions.md)。 第二篇文章包含的示例代码演示了 DAO 中的异常处理。
...
1. 检查[m_pErrorInfo](#m_perrorinfo)数据成员中`GetErrorInfo`返回的当前 [CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md) 结构。 其成员提供有关 DAO 错误的信息。
...
`GetErrorInfo`将信息存储在异常对象的`m_pErrorInfo`数据成员中。 有关所返回信息的简要说明，请参阅[m_pErrorInfo](#m_perrorinfo)。 如果捕获 MFC 引发的类型`CDaoException`的异常，则该`m_pErrorInfo`成员将已填充。 如果选择直接调用 DAO，则必须自行`GetErrorInfo` `m_pErrorInfo`调用异常对象的成员函数。 有关更详细的说明，请参阅[CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md)结构。
...
- NO_AFX_DAO_ERROR 最新的操作未导致 MFC 扩展错误。 但是，此操作可能会产生 DAO 或 OLE 中的其他错误，因此应检查[m_pErrorInfo](#m_perrorinfo) ，可能[m_scode](#m_scode)。
...
- AFX_DAO_ERROR_OBJECT_NOT_OPEN 您尝试基于未处于打开状态的 querydef 或 tabledef 对象打开记录集。
...
|CDaoErrorInfo 成员|Information|含义|
|--------------------------|-----------------|-------------|
