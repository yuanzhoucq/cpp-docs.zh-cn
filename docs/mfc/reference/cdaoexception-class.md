---
title: CDaoException 类 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-mfc
...
2.  调用异常对象的[GetErrorCount](#geterrorcount)成员函数来确定数据库引擎错误集合中的错误对象数。 (通常只有一个，除非您使用的 ODBC 数据源。)  
  
3.  调用异常对象的[GetErrorInfo](#geterrorinfo)成员函数按集合，通过异常对象中索引一次检索一个特定的错误对象。 将异常对象视为一个 DAO 错误对象的代理。  
  
4.  检查当前[CDaoErrorInfo](../../mfc/reference/cdaoerrorinfo-structure.md)结构的`GetErrorInfo`中返回[m_pErrorInfo](#m_perrorinfo)数据成员。 其成员提供 DAO 错误有关的信息。  
  
5.  对于 ODBC 数据源，请重复步骤 3 和 4，根据需要对多个错误对象。  
  
6.  如果构造在堆上的异常对象，将其与删除**删除**运算符时完成。  
