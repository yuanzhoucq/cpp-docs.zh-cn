...
|[CMFCToolBarImages::Clear](#clear)|释放系统资源是分配给此对象。|  
|[CMFCToolBarImages::ConvertTo32Bits](#convertto32bits)|将它添加下划线到 32 bpp 图像的位图。|  
...
|[CMFCToolBarImages::DeleteImage](#deleteimage)|删除映像都具有的指定的索引，从工具栏图像的工具栏图像本集包含用户定义的映像。|  
|[CMFCToolBarImages::Draw](#draw)|绘制单个工具栏图像 （按钮）。|  
|[CMFCToolBarImages::DrawEx](#drawex)||  
|[CMFCToolBarImages::EnableRTL](#enablertl)||  
|[CMFCToolBarImages::EndDrawImage](#enddrawimage)|绘制的工具栏图像之后，请释放系统资源。|  
|[CMFCToolBarImages::ExtractIcon](#extracticon)|返回具有指定的映像索引，从工具栏图像的图标。|  
|[CMFCToolBarImages::FillDitheredRect](#fillditheredrect)|通过使用一个画笔，它具有工具栏背景色填充的矩形。|  
|[CMFCToolBarImages::GetAlwaysLight](#getalwayslight)||  
|[CMFCToolBarImages::GetBitsPerPixel](#getbitsperpixel)|返回带下划线的图像的当前的分辨率。|  
|[CMFCToolBarImages::GetCount](#getcount)|返回在工具栏上的映像数量。|  
|[CMFCToolBarImages::GetDisabledImageAlpha](#getdisabledimagealpha)|返回用于已禁用的映像的 alpha 通道值。|  
|[CMFCToolBarImages::GetFadedImageAlpha](#getfadedimagealpha)||  
|[CMFCToolBarImages::GetImageSize](#getimagesize)|检索存储在内存 （源大小），工具栏图像的大小或在屏幕 （目标大小） 绘制工具栏图像的大小。|  
...
|[CMFCToolBarImages::GetResourceOffset](#getresourceoffset)|指定的资源 id。 返回的图像索引|  
|[CMFCToolBarImages::GetScale](#getscale)|返回当前的缩放比率的带下划线的映像。|  
|[CMFCToolBarImages::GetTransparentColor](#gettransparentcolor)||  
|[CMFCToolBarImages::GrayImages](#grayimages)|用灰色工具栏图像，使其看已禁用。|  
|[CMFCToolBarImages::Is32BitTransparencySupported](#is32bittransparencysupported)|确定操作系统是否支持 32 位 alpha 混合。|  
|[CMFCToolBarImages::IsPreMultiplyAutoCheck](#ispremultiplyautocheck)||  
|[CMFCToolBarImages::IsRTL](#isrtl)|确定是否启用了从右到左 (RTL) 支持。|  
|[CMFCToolBarImages::IsReadOnly](#isreadonly)|确定工具栏图像是否是只读的。|  
|[CMFCToolBarImages::IsScaled](#isscaled)|告知带下划线的映像轻松缩放。|  
|[CMFCToolBarImages::IsUserImagesList](#isuserimageslist)|确定此组的工具栏图像是否包含用户定义的映像。|  
|[CMFCToolBarImages::IsValid](#isvalid)|确定此组的工具栏图像是否包含有效的工具栏图像。|  
...
|[CMFCToolBarImages::MirrorBitmap](#mirrorbitmap)|水平会镜像位图。|  
...
|[CMFCToolBarImages::Save](#save)|如果此组的工具栏图像包含用户定义的映像，将工具栏图像存储在文件中。|  
|[CMFCToolBarImages::SetAlwaysLight](#setalwayslight)||  
|[CMFCToolBarImages::SetDisabledImageAlpha](#setdisabledimagealpha)|设置用于已禁用的映像的 alpha 通道值。|  
...
|[CMFCToolBarImages::SetTransparentColor](#settransparentcolor)|设置工具栏图像的透明色。|  
|[CMFCToolBarImages::SmoothResize](#smoothresize)|平滑地调整带下划线的映像。|  
|[CMFCToolBarImages::UpdateImage](#updateimage)|更新位图中的用户定义的工具栏图像。|  
...
## <a name="remarks"></a>备注  
 由管理的工具栏图像的位图，完整`CMFCToolbarImages`包含，它具有固定大小的一个或多个小型工具栏图像 （按钮）。  
  
## <a name="example"></a>示例  
 下面的示例演示如何配置`CMFCToolBarImages`通过使用中的各种方法的对象`CMFCToolBarImages`类。 该示例演示如何设置工具栏图像的大小，加载一个映像，以及设置图像的透明色。 此代码片段属于[Visual Studio 演示示例](../../visual-cpp-samples.md)。  
...
### <a name="return-value"></a>返回值  
 如果此方法成功，则已添加的工具栏图像的从零开始索引否则为-1。  
...
### <a name="return-value"></a>返回值  
 工具栏数映像`CMFCToolBarImages`对象维护后新位图已成功; 添加为-1 如果操作失败。  
...
### <a name="remarks"></a>备注  
 构造`CMFCToolBarImages`对象，初始化其呈现引擎并设置为其默认值 16 x 15 像素的图像大小。 使用[CMFCToolBarImages::SetImageSize](#setimagesize)之前添加映像更改的图像的大小。  
...
##  <a name="createfromimagelist"></a>CMFCToolBarImages::CreateFromImageList  
 初始化从工具栏图像[CImageList 类](../../mfc/reference/cimagelist-class.md)对象。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果映像已成功，则删除`FALSE`的图像索引是无效的如果`CMFCToolbarImages`对象是临时的`CMFCToolbarImages`对象不包含用户定义的映像，或如果某一其他出现错误。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果成功，则显示了指定的映像`FALSE`如果的图像索引无效或发生了一些其他错误。  
...
### <a name="remarks"></a>备注  
 使用此方法使用的系统颜色 COLOR_BTNFACE 和 COLOR_BTNHIGHLIGHT 的平均值的颜色填充矩形。 如果系统使用的 256 或更少的颜色，将填充的矩形与这两种颜色为抖色样式。  
...
### <a name="remarks"></a>备注  
 你可以调用[CMFCToolBarImages::SetDisabledImageAlpha](#setdisabledimagealpha)若要更改的 alpha 通道值。  
...
##  <a name="getimagesize"></a>CMFCToolBarImages::GetImageSize  
 检索存储在内存 （源大小），工具栏图像的大小或在屏幕 （目标大小） 绘制工具栏图像的大小。  
...
### <a name="return-value"></a>返回值  
 A`SIZE`结构，以像素为单位指定映像的大小。  
  
### <a name="remarks"></a>备注  
 源映像的大小是存储在图像的大小[CMFCToolbarImages](../../mfc/reference/cmfctoolbarimages-class.md)对象。 你可以调用[CMFCToolBarImages::SetImageSize](#setimagesize)设置源的大小。 默认值为 16 x 15 像素。  
...
### <a name="remarks"></a>备注  
 工具栏图像存储在名为的单个位图中的行*映像以及*。 若要查找映像也的工具栏图像，乘以的图像的索引的工具栏图像的宽度 (请参阅[CMFCToolBarImages::GetImageSize](#getimagesize)) 很好地获取内部映像图像的水平偏移量。  
...
### <a name="return-value"></a>返回值  
 如果此方法成功，则一个图像索引如果指定的资源 ID 的映像不存在，则为-1。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果支持 32 位 alpha 混合，则;否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 此静态方法用于在运行时确定操作系统是否支持 32 位 alpha 混合。 在支持此功能[!INCLUDE[Win2kFamily](../../c-runtime-library/includes/win2kfamily_md.md)]及更高版本。  
...
### <a name="remarks"></a>备注  
 `CMFCToolbarImages`对象是只读的从只读文件，加载工具栏图像的位图时或当位图已复制在使用`CMFCToolBarImages::CopyTemp`方法。  
...
### <a name="remarks"></a>备注  
 应用程序本地化为读取从右到左，如阿拉伯语、 希伯来语、 波斯语或乌尔都语语言时，使用 RTL 支持。  
  
##  <a name="isuserimageslist"></a>CMFCToolBarImages::IsUserImagesList  
 指定此集的工具栏图像是否包含用户定义的映像。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果`CMFCToolBarImages`对象是否有效; 否则为`FALSE`。  
  
### <a name="remarks"></a>备注  
 `CMFCToolBarImages`与工具栏图像的位图其句柄时，可能不是有效对象`NULL`。  
...
### <a name="remarks"></a>备注  
 将此成员变量设置为`FALSE`若要启用真彩色 alpha 混合的工具栏图像。  
...
### <a name="return-value"></a>返回值  
 `TRUE`如果绘制工具栏图像所需的资源已分配了成功，否则`FALSE`。  
  
### <a name="remarks"></a>备注  
 调用此方法后，你可以调用[CMFCToolBarImages::Draw](#draw)任意次数。 完成绘制后，必须调用[CMFCToolBarImages::EndDrawImage](#enddrawimage)以释放分配的资源`PrepareDrawImage`。  
  
##  <a name="save"></a>CMFCToolBarImages::Save  
 如果此组的工具栏图像包含用户定义的映像，将工具栏图像存储在文件中。  
...
### <a name="remarks"></a>备注  
 使用此方法设置已禁用的映像的自定义 alpha 值。 默认值为 127，这将导致产生是半透明的禁用的按钮图像。 如果设置值为 0，已禁用的图像将是完全透明的。 如果你设置的值为 255，已禁用的映像将完全不透明。  
...
### <a name="remarks"></a>备注  
 默认情况下，工具栏图像的大小为 16 x 15 像素。 如果你想要使用的不同大小的工具栏图像，请调用此方法。  
...
### <a name="return-value"></a>返回值  
 一个整数值，表示带下划线的映像，以每像素 (bpp) 位数的当前分辨率。  
