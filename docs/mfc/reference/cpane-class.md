...
`CPane` 类是[CControlBar 类](../../mfc/reference/ccontrolbar-class.md)的增强功能。 如果要升级现有 MFC 项目，请将出现的所有 `CControlBar` 替换为 `CPane`。
...
|[CPane：： Create](#create)|创建一个控件栏并将其附加到 `CPane` 对象上。|
|[CPane：： CreateDefaultMiniframe](#createdefaultminiframe)|为浮动窗格创建微型框架窗口。|
|[CPane：： CreateEx](#createex)|创建一个控件栏并将其附加到 `CPane` 对象上。|
...
通常，不会直接实例化 `CPane` 的对象。 如果需要具有插接功能的窗格，请从[CDockablePane](../../mfc/reference/cdockablepane-class.md)派生对象。 如果需要 toolbar 功能，请从[CMFCToolBar](../../mfc/reference/cmfctoolbar-class.md)派生对象。
...
此方法创建一个 Windows 窗格，并将其附加到 `CPane` 对象上。
...
一个[CRect](../../atl-mfc-shared/reference/crect-class.md)对象，该对象包含窗格每一侧的当前宽度（以像素为单位）。 例如，`CRect` 对象的 `left` 成员的值是左边框的宽度。
...
一个 `CDockingPanesRow`*，指向窗格停靠的停靠行，如果窗格未停靠，则为 NULL。
...
`CPane` 类包含一个 `CMFCDragFrameImpl` 对象 `m_dragFrameImpl`，该对象负责绘制当用户在标准停靠模式下移动窗格时显示的矩形。 当用户移动窗格时，作用点用于相对于当前鼠标位置绘制矩形。
...
当窗格停靠或浮动时，窗格标题将显示在标题区域中。 如果窗格是选项卡式组的一部分，则标题将显示在选项卡区域中。 如果该窗格处于自动隐藏模式，则标题将显示在 `CMFCAutoHideButton`上。
...
只有可停靠的窗格可以在多窗格框架窗口中浮动。 因此，`CPane::IsInFloatingMultiPaneFrameWnd` 总是返回 FALSE。
...
如果你的应用程序中的一个或多个停靠窗格重写 `GetMinSize`，或者如果你的应用程序调用 `SetMinSize`，则可能需要将此静态成员设置为 TRUE，以便使框架能够一致地处理窗格的大小。
...
一个 `CSize` 对象，它包含新矩形和旧矩形（旧矩形- *rectNew*）之间的宽度和高度差异。
...
默认情况下，通过调用 `CDockSite::RemovePane`在停靠窗格中取消注册窗格。
...
框架在将窗格状态保存到注册表时调用此方法。 重写派生类中的 `SaveState` 以存储附加信息。
...
与窗格关联的自动隐藏按钮的外观基于两个因素。 如果窗格处于活动状态，并且 `static BOOL CMFCAutoHideButton::m_bOverlappingTabs` 为 TRUE，则框架会将 "自动隐藏" 按钮显示为图标和标签。 对于非活动窗格，框架只显示 "自动隐藏" 图标。
...
如果某个窗格处于浮动窗格，则将其放在[CPaneFrameWnd](../../mfc/reference/cpaneframewnd-class.md) （袖珍框架）窗口上。 在调用[CPane：： CreateDefaultMiniframe](#createdefaultminiframe)时，可以提供将使用的自定义 `CPaneFrameWnd`派生类。
