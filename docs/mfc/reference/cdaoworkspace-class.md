...
2.  调用对象的`Create`成员函数来创建基础 DAO 工作区。 必须指定一个工作区名称。  
  
3.  另外也可以调用[追加](#append)如果你想要添加到数据库引擎的工作区集合工作区。 您可以使用工作区不会将附加它。  
...
2.  调用`SetDefaultPassword`并且，如果你愿意， [SetDefaultUser](#setdefaultuser)。  
  
3.  调用`Create`对于此工作区对象或后续条件，而不在指定的密码。  
...
2.  调用`SetDefaultUser`并且，如果你愿意， [SetDefaultPassword](#setdefaultpassword)。  
  
3.  调用`Create`对于此工作区对象或后续条件，而不指定用户名。  
