...
|[CPropertySheet::PressButton](#pressbutton)|模拟在属性表中选择的指定按钮。|
...
|名称|描述|
|----------|-----------------|
|[CPropertySheet::m_psh](#m_psh)|Windows [PROPSHEETHEADER](/windows/desktop/api/prsht/ns-prsht-propsheetheadera_v2)结构。 提供对基本属性表参数的访问。|
...
`CPropertySheet`为 Windows 98 和 Windows NT 2000 中引入的扩展[PROPSHEETHEADER](/windows/desktop/api/prsht/ns-prsht-propsheetheadera_v2)结构提供支持。 结构包含其他标志和支持使用 "水印" 背景位图的成员。

若要在属性表对象中自动显示这些新图像, 请在调用[CPropertySheet:: 构造](#construct)或[CPropertySheet:: CPropertySheet](#cpropertysheet)时传递位图和调色板图像的有效值。

即使不`CPropertySheet` `CDialog`是派生自 [CDialog](../../mfc/reference/cdialog-class.md), 管理对象也像管理对象。`CPropertySheet` 例如, 创建属性表需要两部分构造: 调用构造函数, 然后为模式属性表调用[DoModal](#domodal)或为无模式属性表[创建](#create)。 `CPropertySheet`具有两种类型的构造函数:[CPropertySheet:: 构造](#construct)和[CPropertySheet:: CPropertySheet](#cpropertysheet)。
...
将禁止`Window Styles`任何其他用户, 不应启用它们。

交换`CPropertySheet`对象和外部对象之间的数据与与`CDialog`对象交换数据类似。 重要的区别在于, 属性表的设置通常是`CPropertyPage`对象的成员变量, 而不`CPropertySheet`是对象本身的成员变量。
...
`AddPage`将[CPropertyPage](../../mfc/reference/cpropertypage-class.md#cpropertypage)对象添加到`CPropertySheet`对象的页列表中, 但不会实际为该页创建窗口。 框架推迟在用户选择该页之前为该页创建窗口。

使用`AddPage`添加属性页时`CPropertySheet` , `CPropertyPage`是的父级。 若要从属性页访问属性表, 请调用[CWnd:: GetParent](../../mfc/reference/cwnd-class.md#getparent)。

在创建要调用`AddPage`的属性表窗口之前, 无需等待。 通常, 你将在`AddPage`调用[DoModal](#domodal)或[Create](#create)前调用。

如果在显示`AddPage` "属性" 页后调用, 则选项卡行将反映新添加的页面。
...
如果尚未调用某个类构造函数, 则调用此成员函数。 例如, 在声明`Construct`或分配`CPropertySheet`对象数组时调用。 对于数组, 必须调用`Construct`数组中的每个成员。

若要显示属性表, 请调用[DoModal](#domodal)或[Create](#create)。 第一个参数中包含的字符串将放在属性表的标题栏中。

如果使用上面列出的第三个或`Construct`第四个原型, 并为*hbmWatermark*、 *hpalWatermark*和/或*hbmHeader*参数传递有效值, 则可以自动显示水印和/或标题图像。

### <a name="example"></a>示例

下面的示例演示了在什么情况下应`Construct`调用。
...
如果使用上面的第三个或`CPropertySheet`第四个原型, 并为*hbmWatermark*、 *hpalWatermark*和/或*hbmHeader*参数传递有效值, 则可以自动显示水印和/或标头图像。
...
`Create`可在构造函数中调用, 也可以在调用构造函数后调用。

默认样式 (通过传递-1 表示为*dwStyle*) 实际上是 WS_SYSMENU&#124;WS_POPUP&#124;WS_CAPTION&#124;DS_MODALFRAME&#124;DS_CONTEXTHELP&#124;WS_VISIBLE。 默认的扩展窗口样式 (用0作为*dwExStyle*传递) 实际上是 WS_EX_DLGMODALFRAME 的。

成员`Create`函数将在创建属性表后立即返回。 若要销毁属性表, 请调用[CWnd::D estroywindow](../../mfc/reference/cwnd-class.md#destroywindow)。

使用对`Create`的调用显示的无模式属性表没有 "确定"、"取消"、"立即应用" 和 "帮助" 按钮作为模式属性表。 用户必须创建所需的按钮。
...
如果函数成功, 则为 IDOK 或 IDCANCEL;否则为0或-1。 如果属性表已建立为向导 (请参阅[SetWizardMode](#setwizardmode)), `DoModal`则返回 ID_WIZFINISH 或 IDCANCEL。

### <a name="remarks"></a>备注

返回值对应于关闭属性表的控件的 ID。 此函数返回后, 对应于属性表和所有页面的窗口将被销毁。 对象本身将仍然存在。 通常, 在返回 IDOK 后`DoModal` , 将从[CPropertyPage](../../mfc/reference/cpropertypage-class.md)对象中检索数据。
...
> [!NOTE]
>  如果正在使用异步异常处理模型进行编译, 则必须由操作系统处理此异常。 有关异常处理模型的详细信息, 请参阅[/EH (异常处理模型)](../../build/reference/eh-exception-handling-model.md)。 在这种情况下, 不要使用`CPropertySheet::DoModal` C++ try-catch 块包装对的调用, 其中 catch 处理所有异常`catch (...)`, 例如。 此块将处理适用于操作系统的异常, 并导致不可预知的行为。 但是, 您可以安全地C++将异常处理用于特定异常类型或结构化异常处理, 在这种情况下, 将访问冲突异常传递到操作系统。
...
默认情况下, 如果属性表具有的选项卡数多于在属性表宽度的单个行中容纳的选项卡, 则选项卡将在多个行中堆叠。 若要使用滚动选项卡而不是堆栈`EnableStackedTabs`选项卡, 请在调用[DoModal](#domodal)或[Create](#create)之前, 调用*bStacked*设置为 FALSE。

创建模式或`EnableStackedTabs`无模式属性表时, 必须调用。 若要将此样式并入`CPropertySheet`派生类中, 请为 WM_CREATE 写入消息处理程序。 在[CWnd:: OnCreate](../../mfc/reference/cwnd-class.md#oncreate)的重写版本中, `EnableStackedTabs( FALSE )`调用基类实现前调用。
...
获取属性表窗口的活动页的索引号, 然后使用返回的索引号作为的参数`GetPage`。
...
例如, 你将使用`GetPageIndex`来获取页面索引, 以便使用[SetActivePage](#setactivepage)或[GetPage](#getpage)。
...
一个结构, 其成员存储[PROPSHEETHEADER](/windows/desktop/api/prsht/ns-prsht-propsheetheadera_v2)的特性。

### <a name="remarks"></a>备注

使用此结构可以在构造后, 初始化属性表的外观, 但在使用[DoModal](#domodal)成员函数显示它之前。 例如, 将的*dwSize*成员`m_psh`设置为要使用的属性表的大小。
...
[GetDialogBaseUnits](/windows/desktop/api/winuser/nf-winuser-getdialogbaseunits) Windows 函数返回系统字体的大小信息, 但如果在资源定义文件中使用 DS_SETFONT 样式, 则可以为每个属性表指定不同的字体。 Windows SDK 中描述的[MapDialogRect](/windows/desktop/api/winuser/nf-winuser-mapdialogrect) Windows 函数为此对话框使用适当的字体。

成员函数将 lpRect 中的对话框单位替换为屏幕单位 (像素), 以便可以使用该矩形来创建对话框或将控件放置在一个框中。  `MapDialogRect`
...
调用此成员函数以响应 WM_INITDIALOG 消息。 此消息将在[Create](#create)或[DoModal](#domodal)调用过程中发送到属性表, 这会在显示属性表之前立即发生。
...
有关 Windows SDK Pressbutton 消息的详细信息, 请参阅[PSM_PRESSBUTTON](/windows/desktop/Controls/psm-pressbutton) 。

对的调用`PressButton`不会将[PSN_APPLY](/windows/desktop/Controls/psn-apply)通知从属性页发送到框架。 若要发送此通知, 请调用[CPropertyPage:: OnOK](../../mfc/reference/cpropertypage-class.md#onok)。
...
例如, 如果某`SetActivePage`一页上的用户操作应导致另一个页面成为活动页面, 则使用。
...
*nStyle*<br/>
指定属性表标题的样式。 样式必须指定为0或 PSH_PROPTITLE。 如果样式设置为 PSH_PROPTITLE, 则在指定为标题的文本后会出现 "属性" 一词。 例如, 调用`SetTitle`("simple", PSH_PROPTITLE) 将生成属性表标题 "简单属性"。
...
- PSWIZB_DISABLEDFINISH "禁用完成" 按钮

### <a name="remarks"></a>备注

仅`SetWizardButtons`在对话框处于打开状态后调用; 在调用`SetWizardButtons` [DoModal](#domodal)之前, 无法调用。 通常, 应从`SetWizardButtons` [CPropertyPage:: OnSetActive](../../mfc/reference/cpropertypage-class.md#onsetactive)调用。
...
有三个向导属性页: `CStylePage`、 `CColorPage`和`CShapePage`。 `CPropertySheet`  下面的代码段演示如何启用和禁用向导属性页上的 "**后退**" 和 "**下一步**" 按钮。
...
在`SetWizardMode`调用[DoModal](#domodal)前调用。 调用`SetWizardMode`后, `DoModal`将返回 ID_WIZFINISH (如果用户以 "完成" 按钮关闭) 或 IDCANCEL。
