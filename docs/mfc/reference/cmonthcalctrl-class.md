...
|名称|说明|
|----------|-----------------|
|[CMonthCalCtrl：： Create](#create)|创建月历控件，并将其附加到 `CMonthCalCtrl` 对象上。|
...
|[CMonthCalCtrl：： System.windows.media.visualtreehelper.hittest](#hittest)|确定月历控件的哪个部分位于屏幕上的给定点。|
...
您可以在创建月历控件时，通过将各种样式应用于该对象来对其进行自定义。 Windows SDK 中的月历[控件样式](/windows/win32/Controls/month-calendar-control-styles)中描述了这些样式。
...
创建月历控件，并将其附加到 `CMonthCalCtrl` 对象上。
...
1. 调用[CMonthCalCtrl](../../mfc/reference/cmonthcalctrl-class.md)构造 `CMonthCalCtrl` 的对象。

1. 调用此成员函数，该函数创建月历控件并将其附加到 `CMonthCalCtrl` 的对象。

调用 `Create`时，将初始化公共控件。 你调用 `Create` 的版本决定了它的大小：
...
此方法发送 Windows SDK 中描述的[MCM_GETCALENDARBORDER](/windows/win32/Controls/mcm-getcalendarborder)消息。
...
此方法发送 Windows SDK 中描述的[MCM_GETCALENDARCOUNT](/windows/win32/Controls/mcm-getcalendarcount)消息。
...
|参数|说明|
|---------------|-----------------|
|*pmcGridInfo*|弄指向[MCGRIDINFO](/windows/win32/api/commctrl/ns-commctrl-mcgridinfo)结构的指针，该结构接收当前月历控件的相关信息。 调用方负责分配和初始化此结构。|
...
此方法发送 Windows SDK 中描述的[MCM_GETCALENDARGRIDINFO](/windows/win32/Controls/mcm-getcalendargridinfo)消息。

### <a name="example"></a>示例

下面的代码示例定义了用于以编程方式访问月历控件的变量 `m_monthCalCtrl`。 此变量将在下一个示例中使用。
...
此方法发送 Windows SDK 中描述的[MCM_GETCALID](/windows/win32/Controls/mcm-getcalid)消息。
...
一个[COLORREF](/windows/win32/gdi/colorref)值，指定与月历控件的部分关联的颜色（如果成功）。 否则，此成员函数将返回-1。
...
此方法发送 Windows SDK 中描述的[MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview)消息。
...
*pDateTime*<br/>
指向[SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime)结构的指针，该结构将接收当前选定的日期信息。 此参数必须是有效的地址，并且不能为 NULL。
...
> [!NOTE]
>  如果设置了样式 MCS_MULTISELECT，则此成员函数将失败。

在 MFC 的 `GetCurSel`实现中，可以指定 `COleDateTime` 使用情况、`CTime` 用法或 `SYSTEMTIME` 结构用法。
...
此成员函数实现 Win32 消息[MCM_GETFIRSTDAYOFWEEK](/windows/win32/Controls/mcm-getfirstdayofweek)的行为，如 Windows SDK 中所述。 一周中的几天以整数表示，如下所示。
...
此成员函数实现 Win32 消息[MCM_GETMAXSELCOUNT](/windows/win32/Controls/mcm-getmaxselcount)的行为，如 Windows SDK 中所述。 将此成员函数用于具有 MCS_MULTISELECT 样式集的控件。
...
此方法发送 Windows SDK 中描述的[MCM_GETMAXTODAYWIDTH](/windows/win32/Controls/mcm-getmaxtodaywidth)消息。
...
如果成功，此成员函数将返回非零值，`lpRect` 将接收适用的边界信息。 如果不成功，则成员函数返回0。
...
*refMaxRange*<br/>
对 `COleDateTime` 或包含允许的最大日期的 `CTime` 对象的引用。

*pMinRange*<br/>
指向[SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime)结构的指针，该结构包含位于范围下限的日期。

*pMaxRange*<br/>
指向 `SYSTEMTIME` 结构的指针，该结构包含范围的最大端的日期。
...
此成员函数实现 Win32 消息[MCM_GETMONTHRANGE](/windows/win32/Controls/mcm-getmonthrange)的行为，如 Windows SDK 中所述。 在 MFC 的 `GetMonthRange`实现中，可以指定 `COleDateTime` 使用情况、`CTime` 使用情况或 `SYSTEMTIME` 结构用法。
...
*pMaxRange*<br/>
指向 `COleDateTime` 对象、`CTime` 对象或[SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime)结构的指针，该对象包含范围最高端的日期。
...
此成员函数实现 Win32 消息[MCM_GETRANGE](/windows/win32/Controls/mcm-getrange)的行为，如 Windows SDK 中所述。 在 MFC 的 `GetRange`实现中，可以指定 `COleDateTime` 使用情况、`CTime` 用法或 `SYSTEMTIME` 结构用法。
...
此成员函数实现 Win32 消息[MCM_GETSELRANGE](/windows/win32/Controls/mcm-getselrange)的行为，如 Windows SDK 中所述。 如果应用于不使用 MCS_MULTISELECT 样式的月历控件，`GetSelRange` 将会失败。

在 MFC 的 `GetSelRange`实现中，可以指定 `COleDateTime` 使用情况、`CTime` 使用情况或 `SYSTEMTIME` 结构用法。
...
*pDateTime*<br/>
指向将接收日期信息的[SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime)结构的指针。 此参数必须是有效的地址，并且不能为 NULL。
...
此成员函数实现 Win32 消息[MCM_GETTODAY](/windows/win32/Controls/mcm-gettoday)的行为，如 Windows SDK 中所述。 在 MFC 的 `GetToday`实现中，可以指定 `COleDateTime` 使用情况、`CTime` 用法或 `SYSTEMTIME` 结构用法。
...
`HitTest` 使用 `MCHITTESTINFO` 结构，该结构包含有关命中测试的信息。
...
此方法发送 Windows SDK 中描述的[MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview)消息。 如果该消息返回 MCMV_CENTURY，则此方法返回 TRUE。
...
此方法发送 Windows SDK 中描述的[MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview)消息。 如果该消息返回 MCMV_DECADE，则此方法返回 TRUE。
...
此方法发送 Windows SDK 中描述的[MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview)消息。 如果该消息返回 MCMV_MONTH，则此方法返回 TRUE。
...
此方法发送 Windows SDK 中描述的[MCM_GETCURRENTVIEW](/windows/win32/Controls/mcm-getcurrentview)消息。 如果该消息返回 MCMV_YEAR，则此方法返回 TRUE。
...
如果此方法成功，则边框宽度设置为*cxyBorder*参数。 否则，边框宽度将重置为当前[主题](/windows/win32/Controls/visual-styles-overview)指定的默认值，如果未使用主题，则为零。

此方法发送 Windows SDK 中描述的[MCM_SETCALENDARBORDER](/windows/win32/Controls/mcm-setcalendarborder)消息。
...
|参数|说明|
|---------------|-----------------|
|*calid*|中[日历标识符](/windows/win32/Intl/calendar-identifiers)常量之一。|
...
日历标识符指定特定于区域的日历，如公历（本地化）、日语或回历。 如果计算机上安装了包含日历的区域设置，请使用 `SetCalID` 方法来显示由*calid*参数指定的日历。
...
下面的代码示例设置月历控件以显示日语高层时代日历。 仅当计算机上安装了该日历时，`SetCalID` 方法才会成功。
...
|参数|说明|
|---------------|-----------------|
|*dwNewView*|中指定 "每月"、"年"、"年" 或 "世纪" 视图的下列值之一。<br /><br /> MCMV_MONTH：月度视图<br /><br /> MCMV_YEAR：年度视图<br /><br /> MCMV_DECADE：十年视图<br /><br /> MCMV_CENTURY：世纪视图|
...
此方法发送 Windows SDK 中描述的[MCM_SETCURRENTVIEW](/windows/win32/Controls/mcm-setcurrentview)消息。
...
*pDateTime*<br/>
指向[SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime)结构的指针，该结构包含要设置为当前选择的日期。
...
此成员函数实现 Win32 消息[MCM_SETCURSEL](/windows/win32/Controls/mcm-setcursel)的行为，如 Windows SDK 中所述。 在 MFC 的 `SetCurSel`实现中，可以指定 `COleDateTime` 使用情况、`CTime` 用法或 `SYSTEMTIME` 结构用法。
...
*pStates*<br/>
指向值的[MONTHDAYSTATE](/windows/win32/Controls/monthdaystate)数组的指针，这些值定义月历控件在其显示中将如何绘制。 MONTHDAYSTATE 数据类型是一个位域，其中每个位（1到31）表示一个月中某一天的状态。 如果某个位处于打开状态，则以粗体显示对应的那一天，否则将不会突出显示。
...
此成员函数实现 Win32 消息[MCM_SETFIRSTDAYOFWEEK](/windows/win32/Controls/mcm-setfirstdayofweek)的行为，如 Windows SDK 中所述。
...
*pMaxRange*<br/>
指向 `COleDateTime` 对象、`CTime` 对象或 `SYSTEMTIME` 结构的指针，该对象包含范围最高端的日期。
...
此成员函数实现 Win32 消息[MCM_SETRANGE](/windows/win32/Controls/mcm-setrange)的行为，如 Windows SDK 中所述。 在 MFC 的 `SetRange`实现中，可以指定 `COleDateTime` 使用情况、`CTime` 使用情况或 `SYSTEMTIME` 结构用法。
...
此成员函数实现 Win32 消息[MCM_SETSELRANGE](/windows/win32/Controls/mcm-setselrange)的行为，如 Windows SDK 中所述。 在 MFC 的 `SetSelRange`实现中，可以指定 `COleDateTime` 使用情况、`CTime` 使用情况或 `SYSTEMTIME` 结构用法。
...
*pDateTime*<br/>
在第二个版本中，是指向包含当前日期信息的[CTime](../../atl-mfc-shared/reference/ctime-class.md)对象的指针。 在第三个版本中，指向包含当前日期信息的[SYSTEMTIME](/windows/win32/api/minwinbase/ns-minwinbase-systemtime)结构的指针。
...
若要成功调用 `SizeMinReq` 将显示一个月的日历的全月日历控件。
