...
### <a name="remarks"></a>备注  
 为了将自定义命令关联的视图，你必须先注册视图通过使用[CMouseManager::AddView](#addview)。 `AddView`方法需要的视图标识符作为输入参数。 一旦你注册一个视图，就可以调用`CMouseManager::SetCommandForDblClk`相同视图标识符输入参数提供给`AddView`。 此后，当用户双击鼠标中已注册的视图，应用程序将执行命令由*uiCmd。* 若要支持的自定义鼠标行为，还需要自定义注册到鼠标管理器的视图。 有关自定义鼠标行为的详细信息，请参阅[键盘和鼠标自定义](../keyboard-and-mouse-customization.md)。  
