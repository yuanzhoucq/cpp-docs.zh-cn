...
如果你尚未在基于 ODBC 的类中实现批量行提取，或者使用基于 DAO 的类（已过时），则 ClassWizard 将 `DoFieldExchange` `CRecordset` `CDaoRecordset` 通过为记录集中的每个字段数据成员调用 RFX 函数（对于 ODBC 类）或 DFX 函数（对于 DAO 类）来重写的成员函数。
...
有关在和函数中出现 RFX 和批量 RFX 函数的示例 `DoFieldExchange` `DoBulkFieldExchange` ，请参阅[RFX_Text](#rfx_text)和 [RFX_Text_Bulk] #rfx_text_bulk）。 DFX 函数与 RFX 函数非常类似。
...
在对象的字段数据成员 `CRecordset` 和 ODBC 类型的数据源上的记录列之间传输字节数组 SQL_BINARY、SQL_VARBINARY 或 SQL_LONGVARBINARY。
...
*pFX*<br/>
指向[CFieldExchange](cfieldexchange-class.md)类的对象的指针。 此对象包含用于定义每次函数时的上下文的信息。 有关对象可以指定的操作的详细信息 `CFieldExchange` ，请参阅[记录字段交换： RFX 的工作方式](../../data/odbc/record-field-exchange-how-rfx-works.md)。
...
这些类型的数据源中的数据映射到记录集中的类型或从类型映射到 `CByteArray` 该记录集。
...
在对象的字段数据成员 `CRecordset` 和 ODBC 类型 SQL_BIT 的数据源中记录的列之间传输布尔数据。
...
在对象的字段数据成员 `CRecordset` 和 ODBC 类型的数据源上的记录列之间传输单个字节 SQL_TINYINT。
...
在 `CTime` 对象的字段数据成员 `CRecordset` 和 ODBC 类型的数据源上的记录列之间传输或 TIMESTAMP_STRUCT 数据 SQL_DATE、SQL_TIME 或 SQL_TIMESTAMP。
...
函数的第二个版本使用对结构的引用 `TIMESTAMP_STRUCT` 。 在调用之前，必须自行设置此结构。 在此版本中，不支持对话框数据交换（DDX）支持或代码向导支持。 函数的第三个版本的工作方式类似于第一个版本，只不过它采用对[COleDateTime](../../atl-mfc-shared/reference/coledatetime-class.md)对象的引用。

### <a name="remarks"></a>备注

`CTime`该函数的版本施加一些中间处理的开销，并具有一定范围内的限制。 如果发现其中一个因素太有限，请使用第二个版本的函数。 但请注意，缺少代码向导和 DDX 支持以及自行设置结构的要求。
...
在对象的字段数据成员和 ODBC 类型 SQL_DOUBLE 的数据源上的记录列之间传输**双精度浮点**数据 `CRecordset` 。
...
*value*<br/>
存储在指定的数据成员中的值（要传输的值）。 对于从记录集到数据源的传输， **`double`** 会从指定的数据成员获取类型为的值。 对于从数据源得到记录集的传输，将在指定的数据成员中存储值。
...
在对象的字段数据成员 `CRecordset` 和 ODBC 类型 SQL_SMALLINT 的数据源中记录的列之间传输整数数据。
...
*value*<br/>
存储在指定的数据成员中的值（要传输的值）。 对于从记录集到数据源的传输， **`int`** 会从指定的数据成员获取类型为的值。 对于从数据源得到记录集的传输，将在指定的数据成员中存储值。
...
在对象的字段数据成员 `CRecordset` 和 ODBC 类型 SQL_INTEGER 的数据源中记录的列之间传输长整数数据。
...
*value*<br/>
存储在指定的数据成员中的值（要传输的值）。 对于从记录集到数据源的传输， **`long`** 会从指定的数据成员获取类型为的值。 对于从数据源得到记录集的传输，将在指定的数据成员中存储值。
...
使用类[CLongBinary](clongbinary-class.md)在对象的字段数据成员 `CRecordset` 和 ODBC 类型 SQL_LONGVARBINARY 或 SQL_LONGVARCHAR 的数据源上的记录列之间传输二进制大型对象（BLOB）数据。
...
*value*<br/>
存储在指定的数据成员中的值（要传输的值）。 对于从记录集到数据源的传输， `CLongBinary` 会从指定的数据成员获取类型为的值。 对于从数据源得到记录集的传输，将在指定的数据成员中存储值。
...
在对象的字段数据成员 `CRecordset` 和 ODBC 类型的数据源上的记录列之间传输浮点数据 SQL_REAL。
...
*value*<br/>
存储在指定的数据成员中的值（要传输的值）。 对于从记录集到数据源的传输， **`float`** 会从指定的数据成员获取类型为的值。 对于从数据源得到记录集的传输，将在指定的数据成员中存储值。
...
在 `CString` `CRecordset` ODBC 类型为 SQL_LONGVARCHAR、SQL_CHAR、SQL_VARCHAR、SQL_DECIMAL 或 SQL_NUMERIC 的数据源的对象的字段数据成员和记录的列之间传输数据。
...
*pFX*<br/>
指向类的对象的指针 `CFieldExchange` 。 此对象包含用于定义每次函数时的上下文的信息。 有关对象可以指定的操作的详细信息 `CFieldExchange` ，请参阅[记录字段交换： RFX 的工作方式](../../data/odbc/record-field-exchange-how-rfx-works.md)。

*szName*<br/>
数据列的名称。

*value*<br/>
存储在指定的数据成员中的值（要传输的值）。 对于从记录集到数据源的传输， `CString` 会从指定的数据成员获取类型为的值。 对于从数据源得到记录集的传输，将在指定的数据成员中存储值。
...
此示例演示多个对 `RFX_Text` 的调用。 另请注意两个对的调用 `CFieldExchange::SetFieldType` 。 对于参数，必须将调用写入 `SetFieldType` 及其 RFX 调用。 输出列调用及其关联 RFX 调用通常由代码向导编写。
...
将 ODBC 数据源的列中的多行数据传输到派生对象中的相应数组 `CRecordset` 。
...
*prgLengths*<br/>
指向长整数数组的指针。 此数组将存储*prgByteVals*所指向的数组中每个值的长度（以字节为单位）。 请注意，如果对应数据项包含 Null 值，则将存储 SQL_NULL_DATA 的值。 有关更多详细信息，请参阅 `SQLBindCol` *Odbc SDK 程序员参考*中的 odbc API 函数。
...
*prgLengths*<br/>
指向长整数数组的指针。 此数组将存储*prgBoolVals*所指向的数组中每个值的长度（以字节为单位）。 请注意，如果对应数据项包含 Null 值，则将存储 SQL_NULL_DATA 的值。 有关更多详细信息，请参阅 `SQLBindCol` *Odbc SDK 程序员参考*中的 odbc API 函数。
...
将 ODBC 数据源的列中的多行传输到派生对象中的相应数组 `CRecordset` 。
...
将从 ODBC 数据源的列 TIMESTAMP_STRUCT 数据的多行传输到派生对象中的相应数组 `CRecordset` 。
...
*prgLengths*<br/>
指向长整数数组的指针。 此数组将存储*prgTSVals*所指向的数组中每个值的长度（以字节为单位）。 请注意，如果对应数据项包含 Null 值，则将存储 SQL_NULL_DATA 的值。 有关更多详细信息，请参阅 `SQLBindCol` *Odbc SDK 程序员参考*中的 odbc API 函数。
...
将来自 ODBC 数据源的列的多个双精度浮点数据行传输到派生对象中的相应数组 `CRecordset` 。
...
*prgDblVals*<br/>
指向值的数组的指针 **`double`** 。 此数组将存储要从数据源传输到记录集的数据。

*prgLengths*<br/>
指向长整数数组的指针。 此数组将存储*prgDblVals*所指向的数组中每个值的长度（以字节为单位）。 请注意，如果对应数据项包含 Null 值，则将存储 SQL_NULL_DATA 的值。 有关更多详细信息，请参阅 `SQLBindCol` *Odbc SDK 程序员参考*中的 odbc API 函数。

### <a name="remarks"></a>备注

数据源列必须具有 ODBC 类型的 SQL_DOUBLE。 记录集必须定义指向的类型指针的字段数据成员 **`double`** 。
...
将来自 ODBC 数据源的列的多个长整数数据行传输到派生对象中的相应数组 `CRecordset` 。
...
*prgLengths*<br/>
指向长整数数组的指针。 此数组将存储*prgLongVals*所指向的数组中每个值的长度（以字节为单位）。 请注意，如果对应数据项包含 Null 值，则将存储 SQL_NULL_DATA 的值。 有关更多详细信息，请参阅 `SQLBindCol` *Odbc SDK 程序员参考*中的 odbc API 函数。

### <a name="remarks"></a>备注

数据源列必须具有 ODBC 类型的 SQL_INTEGER。 记录集必须定义指向的类型指针的字段数据成员 **`long`** 。
...
将 ODBC 数据源的列中的多行浮点数据传输到派生对象中的相应数组 `CRecordset` 。
...
*prgFltVals*<br/>
指向值的数组的指针 **`float`** 。 此数组将存储要从数据源传输到记录集的数据。

*prgLengths*<br/>
指向长整数数组的指针。 此数组将存储*prgFltVals*所指向的数组中每个值的长度（以字节为单位）。 请注意，如果对应数据项包含 Null 值，则将存储 SQL_NULL_DATA 的值。 有关更多详细信息，请参阅 `SQLBindCol` *Odbc SDK 程序员参考*中的 odbc API 函数。

### <a name="remarks"></a>备注

数据源列必须具有 ODBC 类型的 SQL_REAL。 记录集必须定义指向的类型指针的字段数据成员 **`float`** 。
...
将多行字符数据从 ODBC 数据源的列传输到派生对象中的相应数组 `CRecordset` 。
...
*prgLengths*<br/>
指向长整数数组的指针。 此数组将存储*prgStrVals*所指向的数组中每个值的长度（以字节为单位）。 此长度不包括 null 终止字符。 请注意，如果对应数据项包含 Null 值，则将存储 SQL_NULL_DATA 的值。 有关更多详细信息，请参阅 `SQLBindCol` *Odbc SDK 程序员参考*中的 odbc API 函数。
...
你必须在重写中手动编写调用 `DoBulkFieldExchange` 。 此示例演示对 `RFX_Text_Bulk` 数据传输的调用，以及对的调用 `RFX_Long_Bulk` 。 这些调用前面有一个对[CFieldExchange：： SetFieldType](cfieldexchange-class.md#setfieldtype)的调用。 请注意，对于参数，必须调用 RFX 函数，而不是 Bulk RFX 函数。
...
数据在 DAO 的类型 DAO_I4 之间进行映射，并在 **`long`** 记录集中键入。
...
`DFX_LongBinary`提供以与 MFC ODBC 类兼容。 `DFX_LongBinary` 函数在 [CDaoRecordset](cdaorecordset-class.md) 对象的字段数据成员和数据源上的记录列之间使用`CLongBinary`类传输二进制大型对象（BLOB）数据。   数据在 DAO 的类型 DAO_BYTES 之间进行映射，并在记录集中[CLongBinary](clongbinary-class.md)类型。
...
*value*<br/>
存储在指定的数据成员中的值（要传输的值）。 对于从记录集到数据源的传输， **`short`** 会从指定的数据成员获取类型为的值。 对于从数据源得到记录集的传输，将在指定的数据成员中存储值。
...
数据在 DAO 的类型 DAO_I2 之间进行映射，并在 **`short`** 记录集中键入。
...
数据在 DAO 的类型 DAO_R4 之间进行映射，并在 **`float`** 记录集中键入。
...
此示例演示多个对 `DFX_Text` 的调用。 另请注意两个对[CDaoFieldExchange：： SetFieldType](cdaofieldexchange-class.md#setfieldtype)的调用。 您必须编写对的第一次调用 `SetFieldType` 及其**DFX**调用。 第二次调用及其关联的**DFX**调用通常由生成类的代码向导编写。
