...
指定一个列表，其中包含表示 RGB 颜色值的32位整数，后跟指向 OLE 字体对象的 `IFontDisp` 接口的指针。
...
> [!NOTE]
> 已为所有变体类型定义了其他变体常量（VTS_FONT 和 VTS_PICTURE 除外），它们提供指向变量数据常量的指针。 这些常量是使用 `VTS_Pconstantname` 约定命名的。 例如，VTS_PCOLOR 是指向 VTS_COLOR 常量的指针。
