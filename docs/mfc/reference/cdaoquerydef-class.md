...
|名称|说明|
|----------|-----------------|
|[CDaoQueryDef::CDaoQueryDef](#cdaoquerydef)|构造 `CDaoQueryDef` 对象。 接下来，根据需要调用 `Open` 或 `Create`。|
...
|[CDaoQueryDef：： Close](#close)|关闭 querydef 对象。 完成后C++销毁对象。|
|[CDaoQueryDef：： Create](#create)|创建基础 DAO querydef 对象。 使用 querydef 作为临时查询，或调用 `Append` 将其保存到数据库中。|
...
|[CDaoQueryDef：： m_pDatabase](#m_pdatabase)|指向与 querydef 关联的 `CDaoDatabase` 对象的指针。 Querydef 可能保存在数据库中。|
...
1. 在所有情况下，首先构造一个 `CDaoQueryDef` 对象，并提供指向查询所属的[CDaoDatabase](../../mfc/reference/cdaodatabase-class.md)对象的指针。

1. 然后根据需要执行以下操作：

   - 若要使用现有的已保存查询，请调用 querydef 对象的[Open](#open)成员函数，同时提供已保存查询的名称。

   - 若要创建新保存的查询，请调用 querydef 对象的[create](#create)成员函数，同时提供查询的名称。 然后调用[追加](#append)，通过将查询追加到数据库的 QueryDefs 集合来保存查询。 `Create` 将 querydef 置于打开状态，因此在调用之后 `Create` 不调用 `Open`。
...
- 调用对象的 `Execute` 成员函数直接执行操作查询或 SQL 传递查询
...
若要使用 querydef 对象创建 `CDaoRecordset` 对象，通常需要创建或打开 querydef，如上所述。 然后，在调用[CDaoRecordset：： Open](../../mfc/reference/cdaorecordset-class.md#open)时构造记录集对象，并将指针传递到 querydef 对象。 传递的 querydef 必须处于打开状态。 有关详细信息，请参阅类[CDaoRecordset](../../mfc/reference/cdaorecordset-class.md)。

不能使用 querydef 创建记录集（querydef 的最常见用途），除非它处于打开状态。 通过调用 `Open` 或 `Create`，将 querydef 置于打开状态。
...
- 如果对象表示临时 querydef （而不是保存在数据库中），请调用 `Create`，并传递一个空字符串作为查询的名称。 调用 `Create`后，通过直接设置其特性来初始化 querydef。 不要调用 `Append`。
...
完成 querydef 对象后，调用其[Close](#close)成员函数。 如果有指向 querydef 的指针，请使用**delete**运算符销毁C++对象。
...
关闭 querydef 会释放基础 DAO 对象，但不会销毁保存的 DAO querydef 对象或 `CDaoQueryDef` C++的对象。 这不同于[CDaoDatabase：:D eletequerydef](../../mfc/reference/cdaodatabase-class.md#deletequerydef)，它将在 DAO （如果不是临时 querydef）中从数据库的 QueryDefs 集合中删除 querydef。
...
如果未在*lpszSQL*中提供 SQL 语句，则不能使用 `Execute` 运行查询，但可以使用它来创建记录集。 在这种情况下，MFC 使用记录集的默认 SQL 语句。
...
对于返回记录的查询（例如 select 查询），`Execute` 不起作用。 `Execute` 通常用于大容量操作查询，如**UPDATE**、 **INSERT**或**SELECT INTO**，或用于数据定义语言（DDL）操作。

> [!TIP]
>  使用 ODBC 数据源的首选方法是将表附加到 Microsoft Jet （。MDB）数据库。 有关详细信息，请参阅 DAO 帮助中的 "用 DAO 访问外部数据库" 主题。

调用 querydef 对象的[GetRecordsAffected](#getrecordsaffected)成员函数，以确定受最近 `Execute` 调用影响的记录数。 例如，`GetRecordsAffected` 返回有关执行操作查询时删除、更新或插入的记录数的信息。 当级联更新或删除有效时，返回的计数不会反映相关表中的更改。

如果同时包括 `dbInconsistent` 和 `dbConsistent`，或者不包含这两个项，则结果为默认值 `dbInconsistent`。

`Execute` 不返回记录集。 对选择记录的查询使用 `Execute` 会导致 MFC 引发[CDaoException](../../mfc/reference/cdaoexception-class.md)类型的异常。
...
此函数仅与 ODBC 数据源和某些 ISAM 驱动程序一起使用。 它不用于 Microsoft Jet （。MDB）数据库;在这种情况下，`GetConnect` 返回空字符串。 有关详细信息，请参阅[SetConnect](#setconnect)。
...
`GetFieldCount` 有助于遍历 querydef 中的所有字段。 为此，请将 `GetFieldCount` 与[issomapper.getfieldinfo](#getfieldinfo)结合使用。
...
`GetParameterCount` 有助于遍历 querydef 中的所有参数。 为此，请将 `GetParameterCount` 与[GetParameterInfo](#getparameterinfo)结合使用。
...
- `dbQSPTBulk` 用于 `dbQSQLPassThrough`，用于指定不返回记录的查询。
...
调用此成员函数以确定当前是否打开了 `CDaoQueryDef` 对象。
...
如果 `CDaoQueryDef` 对象当前已打开，则为非零值;否则为0。
...
在对 ODBC 数据源执行表示 SQL 传递查询的 querydef 之前，请使用 `SetConnect` 设置连接字符串，并调用[SetReturnsRecords](#setreturnsrecords)以指定查询是否返回记录。
...
Querydef 名称是用户定义的唯一名称。 可以在将 querydef 对象追加到 QueryDefs 集合之前调用 `SetName`。
...
此成员函数可让你覆盖在连接的数据源上的后续操作之前的默认秒数 "超时"。 由于网络访问问题、过多的查询处理时间等，操作可能会超时。 如果要更改查询超时值，请在使用此 querydef 执行查询之前调用 `SetODBCTimeout`。 （由于 ODBC 重用连接，因此相同连接上的所有客户端的超时值都是相同的。）
...
指定要设置为 `COleVariant` 对象的值。 有关在 `COleVariant` 对象中设置所需值和类型的信息，请参阅[COleVariant](../../mfc/reference/colevariant-class.md)类。
...
在这种情况下，必须创建 querydef 并使用其他 `CDaoQueryDef` 成员函数设置其属性。 有关外部数据库的说明，请参阅[SetConnect](#setconnect)。
...
`SetSQL` 的典型用法是设置要在 SQL 传递查询中使用的 querydef 对象。 （有关目标 DBMS 上的 SQL 传递查询的语法，请参阅 DBMS 的文档。）
