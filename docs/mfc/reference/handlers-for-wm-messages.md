---
title: WM_ 消息的处理程序
ms.date: 11/04/2016
f1_keywords:
- WM_
helpviewer_keywords:
- WM_ messages [MFC]
ms.assetid: cad81690-90bf-4f77-943f-a435e7563bdd
ms.openlocfilehash: ea95958ccc7180ffa6472ea2f288dda85223cea5
ms.sourcegitcommit: 6052185696adca270bc9bdbec45a626dd89cdcdd
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 10/31/2018
ms.locfileid: "50605044"
---
# <a name="handlers-for-wm-messages"></a>WM_ 消息的处理程序

以下主题对应的映射条目。

|主题|映射项|
|-----------|-----------------|
|[A-C](../../mfc/reference/wm-message-handlers-a-c.md)|通过 ON_WM_CTLCOLOR ON_WM_ACTIVATE|
|[D-E](../../mfc/reference/wm-message-handlers-d-e.md)|通过 ON_WM_ERASEBKGND ON_WM_DEADCHAR|
|[F-K](../../mfc/reference/wm-message-handlers-f-k.md)|通过 ON_WM_KILLFOCUS ON_WM_FONTCHANGE|
|[L-M](../../mfc/reference/wm-message-handlers-l-m.md)|通过 ON_WM_MOVING ON_WM_LBUTTONDBLCLK|
|[N-O](../../mfc/reference/wm-message-handlers-n-o.md)|通过 ON_WM_NCRBUTTONUP ON_WM_NCACTIVATE|
|[P-R](../../mfc/reference/wm-messages-p-r.md)|通过 ON_WM_RENDERFORMAT ON_WM_PAINT|
|[S](../../mfc/reference/wm-messages-s.md)|通过 ON_WM_SYSKEYUP ON_WM_SETCURSOR|
|[T-Z](../../mfc/reference/wm-messages-t-z.md)|通过 ON_WM_WININICHANGE ON_WM_TIMECHANGE|

## <a name="see-also"></a>请参阅

[消息映射](../../mfc/reference/message-maps-mfc.md)

