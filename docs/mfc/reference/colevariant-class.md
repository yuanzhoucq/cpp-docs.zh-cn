...
此数据类型用于 OLE 自动化。 具体而言， [DISPPARAMS](/windows/win32/api/oaidl/ns-oaidl-dispparams)结构包含指向变体结构的数组的指针。 结构用于将参数传递到[IDispatch：： Invoke。](/windows/win32/api/oaidl/nf-oaidl-idispatch-invoke) `DISPPARAMS`
