...
### <a name="remarks"></a>备注  
 每个调用此方法[CToolTipCtrl 类](../../mfc/reference/ctooltipctrl-class.md)创建[CTooltipManager::CreateToolTip](#createtooltip)。 父控件应调用此方法从其`OnDestroy`处理程序。 这需要从 framework 正确删除工具提示。 此方法会设置*pToolTip*为之前它将返回 NULL。  
...
### <a name="remarks"></a>备注  
 此方法设置的运行时类和初始参数， [CToolTipManager](../../mfc/reference/ctooltipmanager-class.md)创建工具提示时使用。 当控件调用[CTooltipManager::CreateToolTip](#createtooltip)和工具提示中将类型传递，它是一种类型由*nTypes*，工具提示管理器创建工具提示控件的实例指定的运行时类*pRTC*并将传递所指定的参数*pParams*到新的工具提示。  
