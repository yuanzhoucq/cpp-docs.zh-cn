...
|[CBrush::FromHandle](#fromhandle)|当给定 Windows `HBRUSH`对象的`CBrush`句柄时，返回指向对象的指针。|
...
若要使用`CBrush`对象，请`CBrush`构造对象并将其传递给`CDC`需要画笔的任何成员函数。

画笔可以为纯色、阴影线或图案。

有关的详细信息`CBrush`，请参阅[图形对象](../../mfc/graphic-objects.md)。
...
- HS_BDIAGONAL 向下影线（从左到右），45度

- HS_CROSS 水平和垂直交叉影线

- HS_DIAGCROSS 45 度的交叉影线
...
*pBitmap*<br/>
指向一个`CBitmap`对象，该对象指定画笔用于绘制的位图。

### <a name="remarks"></a>备注

`CBrush`具有四个重载构造函数。不带参数的构造函数将构造`CBrush`一个未初始化的对象，必须先对其进行初始化，然后才能使用该对象。

如果使用不带参数的构造函数，则必须用[CreateSolidBrush](#createsolidbrush)、 `CBrush` [CreateHatchBrush](#createhatchbrush)、 [CreateBrushIndirect](#createbrushindirect)、 [CreatePatternBrush](#createpatternbrush)或[初始化生成的对象CreateDIBPatternBrush](#createdibpatternbrush)。 如果使用采用参数的构造函数之一，则无需进行进一步的初始化。 如果遇到错误，带参数的构造函数可能会引发异常，而不带参数的构造函数将始终会成功。
...
*nUsage*<br/>
指定 [BITMAPINFO](/windows/win32/api/wingdi/ns-wingdi-bitmapinfo) 数据结构（" 打包的 DIB" 的一部分）的`bmiColors[]`字段是否在当前已实现的逻辑调色板中包含显式的 RGB 值或索引。 参数必须是下列值之一：

- DIB_PAL_COLORS 颜色表包含16位索引数组。

- DIB_RGB_COLORS 颜色表包含原义文本值。
...
- 在第一个版本中，若要获取 DIB 的句柄，请调用`GlobalAlloc` Windows 函数以分配全局内存块，然后使用打包的 DIB 填充内存。

- 在第二个版本中，无需调用`GlobalAlloc`为打包的 DIB 分配内存。

打包的 DIB 包含一个`BITMAPINFO`数据结构，后面紧跟定义位图像素的字节数组。 用作填充图案的位图的像素应为8像素。 如果位图更大，则 Windows 只使用对应于位图左上角的前8行和8列像素的位创建填充模式。
...
- [CreateDIBPatternBrush](/windows/win32/api/wingdi/nf-wingdi-createdibpatternbrush)（提供此功能只是为了与版本低于3.0 的 Windows 版本编写的应用程序兼容; 请`CreateDIBPatternBrushPt`使用函数。）
...
当应用程序使用创建`CreateSolidBrush`的画笔完成后，应从设备上下文中选择画笔。
...
当应用程序使用创建`CreateSysColorBrush`的画笔完成后，应从设备上下文中选择画笔。
...
给定 Windows [HBRUSH](#operator_hbrush)对象的`CBrush`句柄时，返回指向对象的指针。
...
如果对象尚未附加到句柄，则会创建并附加`CBrush`一个临时对象。 `CBrush` 此临时`CBrush`对象仅在下一次应用程序的事件循环中有空闲时间之前有效。 此时，将删除所有临时图形对象。 换言之，只有在处理一条窗口消息的过程中，临时对象才有效。
