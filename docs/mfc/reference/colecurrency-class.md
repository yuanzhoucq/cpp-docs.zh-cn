...
|[operator +=, -=](#operator_plus_minus_eq)|将添加并减去`COleCurrency`从此值`COleCurrency`对象。|
|[operator */](#operator_star)|刻度`COleCurrency`的整数值的值。|
|[operator *=, /=](#operator_star_div_eq)|缩放此`COleCurrency`的整数值的值。|
|[operator <<](#operator_stream)|输出`COleCurrency`值设为`CArchive`或`CDumpContext`。|
|[operator >>](#operator_stream)|输入`COleCurrency`对象从`CArchive`。|
...
*nUnits*， *nFractionalUnits*指示要复制到新的单位和小数部分 （在 1/万为单位） 的值的`COleCurrency`对象。
...
- COleCurrency (`cySrc`) 构造`COleCurrency`对象从[货币](/windows/desktop/api/wtypes/ns-wtypes-tagcy)值。
...
基础[货币](/windows/desktop/api/wtypes/ns-wtypes-tagcy)此结构`COleCurrency`对象。
...
有关详细信息，请参阅[货币](/windows/desktop/api/wtypes/ns-wtypes-tagcy)Windows SDK 中的条目。
...
*nUnits*， *nFractionalUnits*指示的单位和小数部分 （在 1/万为单位） 的值复制到此`COleCurrency`对象。
