...
下面的示例演示如何使用中的各种方法`CMFCToolBarFontSizeComboBox`类，以配置`CMFCToolBarFontSizeComboBox`对象。 该示例演示了如何从文本框中检索的字体大小，以缇为单位、 字体大小组合框填充所有有效大小的给定字体，并以缇为单位指定字体大小。 此代码片段属于 [Word Pad 示例](../../overview/visual-cpp-samples.md)。
