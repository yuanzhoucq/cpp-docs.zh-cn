...
|[CComboBoxEx::DeleteItem](#deleteitem)|删除`ComboBoxEx`控件中的项。|
...
|[CComboBoxEx::GetImageList](#getimagelist)|检索指向分配`ComboBoxEx`给控件的图像列表的指针。|
...
|[CComboBoxEx::SetExtendedStyle](#setextendedstyle)|设置控件内的`ComboBoxEx`扩展样式。|
|[CComboBoxEx::SetImageList](#setimagelist)|设置`ComboBoxEx`控件的图像列表。|
|[CComboBoxEx::SetItem](#setitem)|设置`ComboBoxEx`控件中项的特性。|
...
在标准组合框中, 组合框的所有者负责通过创建组合框作为所有者描述控件来绘制图像。 使用`CComboBoxEx`时, 不需要设置绘制样式 CBS_OWNERDRAWFIXED 和 CBS_HASSTRINGS, 因为它们是隐含的。 否则, 你必须编写代码来执行绘制操作。 一个`CComboBoxEx`控件对于每个项最多支持三个图像: 一个用于选定状态, 一个用于未选定状态, 另一个用于覆盖图像。

## <a name="styles"></a>样式

`CComboBoxEx`支持样式 CBS_SIMPLE、CBS_DROPDOWN、CBS_DROPDOWNLIST 和 WS_CHILD。 在您创建窗口时传递的所有其他样式将被控件忽略。 创建窗口后, 可以通过调用`CComboBoxEx`成员函数[SetExtendedStyle](#setextendedstyle)来提供其他组合框样式。 利用这些样式, 您可以:
...
*pParentWnd*<br/>
指向[CWnd](../../mfc/reference/cwnd-class.md)对象的指针, 该对象是组合框 (通常为`CDialog`) 的父窗口。 它不能为 NULL。
...
1. 调用此成员函数, 该函数创建扩展的 Windows 组合框并将其附加`CComboBoxEx`到对象。
...
调用此函数可创建扩展组合框控件 (子窗口) 并将其与`CComboBoxEx`对象关联。
...
`CreateEx`使用由*dwExStyle*指定的扩展 Windows 样式创建控件。 必须使用[SetExtendedStyle](#setextendedstyle)设置特定于扩展组合框控件的扩展样式。 例如, 使用`CreateEx`将此类样式设置为 WS_EX_CONTEXTHELP, 但使用`SetExtendedStyle`将此类样式设置为 CBES_EX_CASESENSITIVE。 有关详细信息, 请参阅主题中的[ComboBoxEx 控件扩展样式](/windows/win32/Controls/comboboxex-control-extended-styles)Windows SDK 中所述的样式。
...
此成员函数实现消息[CBEM_DELETEITEM](/windows/win32/Controls/cbem-deleteitem)的功能, 如 Windows SDK 中所述。 调用 Deleteitem.php 时, 将向父窗口发送包含 CBEN_DELETEITEM 通知的[WM_NOTIFY](/windows/win32/controls/wm-notify)消息。

##  <a name="getcomboboxctrl"></a>  CComboBoxEx::GetComboBoxCtrl

调用此成员函数以获取指向`CComboBoxEx`对象内组合框控件的指针。
...
控件由封装的父窗口组成。 `CComboBox` `CComboBoxEx`

返回值指向的对象是临时对象,在下一个空闲处理时间内被销毁。`CComboBox`
...
当使用 CBS_DROPDOWN 样式创建控件时,该控件使用编辑框。`CComboBoxEx`

返回值指向的对象是临时对象,在下一个空闲处理时间内被销毁。`CEdit`
...
调用此成员函数以获取指向`CComboBoxEx`控件所使用的图像列表的指针。
...
返回值指向的对象是临时对象,在下一个空闲处理时间内被销毁。`CImageList`
...
调用`InsertItem`时, 将向父窗口发送包含[CBEN_INSERTITEM](/windows/win32/Controls/cben-insertitem)通知的[WM_NOTIFY](/windows/win32/controls/wm-notify)消息。
...
*pImageList*<br/>
指向`CImageList`对象的指针, 该对象包含用于`CComboBoxEx`控件的图像。
...
此成员函数实现消息[CBEM_SETIMAGELIST](/windows/win32/Controls/cbem-setimagelist)的功能, 如 Windows SDK 中所述。 如果更改默认编辑控件的高度, 请在调用`SetImageList`时调用 Win32 函数[SetWindowPos](/windows/win32/api/winuser/nf-winuser-setwindowpos)以调整控件大小, 否则它将不会正确显示。

返回值指向的对象是临时对象,在下一个空闲处理时间内被销毁。`CImageList`

##  <a name="setitem"></a>CComboBoxEx:: SetItem

设置`ComboBoxEx`控件中项的特性。
