...
- `dbLongBinary`Long Binary （OLE 对象）;你可能希望使用 MFC 类[CByteArray](../../mfc/reference/cbytearray-class.md) ，而不是`CLongBinary`类`CByteArray` ，因为更丰富且更易于使用。
...
|`dbLongBinary`|0|Long Binary （OLE 对象;[CByteArray](../../mfc/reference/cbytearray-class.md);使用而不`CLongBinary`是）|
...
- `dbDescending`字段按降序（Z-a 或 100-0）排序（仅适用于索引对象的字段集合中的字段对象; 在 MFC 中，索引对象本身包含在 tabledef 对象中）。 如果省略此常量，则字段将按升序（A-z 或 0-100）排序（默认值）。
...
对主要、次要和以上的引用指示类[CDaoTableDef](../../mfc/reference/cdaotabledef-class.md#getfieldinfo)、 [CDaoQueryDef](../../mfc/reference/cdaoquerydef-class.md#getfieldinfo)和[CDaoRecordset](../../mfc/reference/cdaorecordset-class.md#getfieldinfo)类中的`GetFieldInfo`成员函数返回信息的方式。

字段对象不由 MFC 类表示。 相反，以下类的 DAO 对象基础 MFC 对象包含字段对象的集合：[CDaoTableDef](../../mfc/reference/cdaotabledef-class.md)、 [CDaoRecordset](../../mfc/reference/cdaorecordset-class.md)和[CDaoQueryDef](../../mfc/reference/cdaoquerydef-class.md)。 这些类提供成员函数来访问某些字段信息项，或者可以通过`CDaoFieldInfo` `GetFieldInfo`调用包含对象的成员函数，一次通过对象访问所有这些项。

除了使用来检查对象属性之外，还可以使用`CDaoFieldInfo`构造用于在 tabledef 中创建新字段的输入参数。 更简单的选项可用于此任务，但如果你想要更精细地控制，则可以使用采用`CDaoFieldInfo`参数的[CDaoTableDef：： CreateField](../../mfc/reference/cdaotabledef-class.md#createfield)版本。

`GetFieldInfo`成员函数（包含字段的类）检索到的信息存储`CDaoFieldInfo`在结构中。 调用其字段集合中存储字段对象的包含对象的成员函数。`GetFieldInfo` `CDaoFieldInfo`还定义了`Dump`调试版本中的成员函数。 可以使用`Dump`转储`CDaoFieldInfo`对象的内容。
