...
## <a name="remarks"></a>备注  
 `CTaskDialog`类取代了标准 Windows 消息框，并具有其他功能，例如用于从用户收集信息的新控件。 此类是在 MFC 库[!INCLUDE[vs_dev10_long](../../build/includes/vs_dev10_long_md.md)]。 `CTaskDialog`从 Windows Vista 开始提供。 早期版本的 Windows 无法显示`CTaskDialog`对象。 使用`CTaskDialog::IsSupported`以在运行时确定当前用户是否可以显示任务对话框。 中仍受支持的标准 Windows 消息框[!INCLUDE[vs_dev10_long](../../build/includes/vs_dev10_long_md.md)]。  
...
### <a name="return-value"></a>返回值  
 如果已启用该控件，则返回 FALSE 如果还没有，则为 TRUE。  
  
### <a name="remarks"></a>备注  
 你可以使用此方法来确定同时命令按钮控件的可用性以及常见的按钮`CTaskDialog`类 *。  
...
### <a name="remarks"></a>备注  
 使用此函数可确定在运行时，如果计算机运行你的应用程序支持`CTaskDialog`类。 如果计算机不支持`CTaskDialog`，应提供的信息传达给用户的另一种方法。 如果它尝试使用你的应用程序将崩溃`CTaskDialog`不支持的计算机上`CTaskDialog`类。  
...
### <a name="remarks"></a>备注  
 此方法调用[ShellExecute](http://msdn.microsoft.com/library/windows/desktop/bb762153)之前它将返回，则为 S_OK。  
...
### <a name="remarks"></a>备注  
 此方法用于更改命令按钮控件启用还是已添加到后需要进行提升`CTaskDialog`类。  
...
### <a name="remarks"></a>备注  
 不能后显示窗口中调用此方法的此实例`CTaskDialog`创建类。 如果这样做，则此方法将引发异常。  
...
### <a name="remarks"></a>备注  
 内容`CTaskDialog`类是向对话框中的主要部分中的用户显示的文本。  
...
### <a name="remarks"></a>备注  
 扩展区域`CTaskDialog`类使你能够向用户提供的其他信息。 扩展区域中的主要部分是`CTaskDialog`位于正下方的标题和内容的字符串。  
...
### <a name="remarks"></a>备注  
 ...
 此方法将引发的异常[确保](diagnostic-services.md#ensure)宏如果`CTaskDialog`显示或输入的参数为 NULL。  
  
 A`CTaskDialog`只能接受`HICON`或`LPCWSTR`为页脚图标。 这通过构造函数中设置选项 TDF_USE_HICON_FOOTER 配置或[CTaskDialog::SetOptions](#setoptions)。 默认情况下，`CTaskDialog`配置为使用`LPCWSTR`作为页脚图标的输入类型。 如果您尝试设置使用的不适合的类型的图标，则此方法会生成异常。  
...
### <a name="remarks"></a>备注  
 ...
 A`CTaskDialog`只能接受`HICON`或`LPCWSTR`作为主图标。 你可以通过设置 TDF_USE_HICON_MAIN 选项构造函数中的或在配置这[CTaskDialog::SetOptions](#setoptions)方法。 默认情况下，`CTaskDialog`配置为使用`LPCWSTR`作为主图标的输入类型。 如果您尝试设置使用的不适合的类型的图标，则此方法会生成异常。  
...
### <a name="remarks"></a>备注  
 主要指令`CTaskDialog`类是向大加粗字体中的用户显示的文本。 它位于标题栏下方的对话框中。  
...
### <a name="parameters"></a>参数  
 [in]*bEnabled*  
 为 true 以启用选框栏中;FALSE 将禁用选框栏和将其从删除`CTaskDialog`。  
  
 [in]*nMarqueeSpeed*  
 一个整数，指示选框栏的速度。  
  
### <a name="remarks"></a>备注  
 选框栏将显示主文本的下方`CTaskDialog`类。  
  
 使用*nMarqueeSpeed*若要设置该字幕条; 速度较大的值指示较慢的速度。 值为 0， *nMarqueeSpeed*这将使 Windows 默认速度将转移选框栏。  
...
### <a name="remarks"></a>备注  
 ...
 若要显示进度栏，必须启用 TDF_SHOW_PROGRESS_BAR 选项和 TDF_SHOW_MARQUEE_PROGRESS_BAR 必须不启用。 此方法会自动设置 TDF_SHOW_PROGRESS_BAR 并清除 TDF_SHOW_MARQUEE_PROGRESS_BAR。 使用[CTaskDialog::SetOptions](#setoptions)可以手动更改的此实例的选项[CTaskDialog 类](../../mfc/reference/ctaskdialog-class.md)。  
...
### <a name="parameters"></a>参数  
 ...
 [in]*bEnabled*  
 若要启用单选按钮，则为 TRUE如果为 FALSE，则若要禁用的单选按钮。  
...
### <a name="parameters"></a>参数  
 [in]*bChecked*  
 如果为验证复选框选择时`CTaskDialog`会显示出来;为 false，则验证复选框未选中时`CTaskDialog`显示。  
...
### <a name="remarks"></a>备注  
 此方法将引发的异常[确保](diagnostic-services.md#ensure)宏如果此实例的`CTaskDialog`类已显示。  
...
### <a name="remarks"></a>备注  
 此静态方法可用于创建的实例`CTaskDialog`而无需显式创建的类`CTaskDialog`在代码中的对象。 因为没有任何`CTaskDialog`对象，不能调用的任何其他方法`CTaskDialog`如果你使用此方法来显示`CTaskDialog`给用户。  
