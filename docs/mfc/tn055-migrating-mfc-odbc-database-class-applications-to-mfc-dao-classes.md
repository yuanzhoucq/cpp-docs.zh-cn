...
- 用于指定记录集开放类型和记录集开放选项等的常量和宏已更改。
   ...
   对于 DAO 类，DAO 在标头文件 (DBDAOINT.H) 中提供了对这些选项的定义。 因此，记录集类型是 `CRecordset` 的枚举成员，但对于 DAO，此类型是一个常量。 例如，你将使用`snapshot`时指定的一种`CRecordset`ODBC 中但`DB_OPEN_SNAPSHOT`时指定的一种`CDaoRecordset`。

- 默认记录集类型`CRecordset`是`snapshot`时的默认记录集类型`CDaoRecordset`是`dynaset`（请参阅下面有关 ODBC 类快照的其他问题的说明）。
...
- 异常类已更改。 `CDBExceptions` ODBC 类中引发和`CDaoExceptions`在 DAO 类中。

- `RFX_Date` 使用`CTime`和`TIMESTAMP_STRUCT`对象，而`DFX_Date`使用`COleDateTime`。 `COleDateTime`几乎等同于`CTime`，但依赖于使用 8 字节 OLE`DATE`而不是一个 4 字节`time_t`以便它可以存放多范围较大的数据。
