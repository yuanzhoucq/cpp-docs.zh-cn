...
- 始终按需基于环境 LocaleID 属性的当前值加载资源。 MFC ActiveX 控件示例[本地化](../overview/visual-cpp-samples.md)使用此策略。
...
- 使用 `OnAmbientChanged` notification 函数为容器的区域设置动态加载适当的资源。

    > [!NOTE]
    >  这对控件有效，但运行时 DLL 将不能在环境 LocaleID 属性更改时动态更新其自己的资源。 此外，ActiveX 控件的运行时 DLL 将使用线程区域设置确定其资源的区域设置。

本文其余部分介绍两个本地化策略。 第一种策略对[控件的可编程性接口](#_core_localizing_your_control.92.s_programmability_interface)（属性、方法和事件的名称）进行了本地化。 第二个策略使用容器的环境 LocaleID 属性来[本地化控件的用户界面](#_core_localizing_the_control.92.s_user_interface)。 有关控件本地化的演示，请参阅 MFC ActiveX 控件示例[本地化](../overview/visual-cpp-samples.md)。
...
若要确保控件的客户可以找到本地化的类型库，请在 Windows 系统注册表的 TypeLib 部分下注册区域设置特定的 .TLB 文件。 为此，提供了[AfxOleRegisterTypeLib](reference/registering-ole-controls.md#afxoleregistertypelib)函数的第三个参数（通常为可选）。 以下示例为 ActiveX 控件注册一个法语类型库：
...
请注意，可以在 switch 语句的每个 case 中检查子语言 ID，以提供更专用的本地化。 有关此函数的演示，请参阅 `GetResourceHandle` MFC ActiveX 控件示例[本地化](../overview/visual-cpp-samples.md)中的函数。

当控件第一次将自身加载到容器中时，它可以调用[COleControl：： AmbientLocaleID](reference/colecontrol-class.md#ambientlocaleid)来检索区域设置 ID。 控件之后可以将返回的区域设置 ID 值传递到 `GetLocalizedResourceHandle` 函数，这将加载合适的资源库。 控件应将生成的句柄（如果有）传递给[AfxSetResourceHandle](reference/application-information-and-management.md#afxsetresourcehandle)：

[!code-cpp[NVC_MFC_AxLoc#4](codesnippet/cpp/mfc-activex-controls-localizing-an-activex-control_4.cpp)]

将上面的代码示例放入控件的成员函数，例如[COleControl：： OnSetClientSite](reference/colecontrol-class.md#onsetclientsite)的重写。 此外， *m_hResDLL*应是控件类的成员变量。

您可以使用类似的逻辑本地化控件的属性页。 若要本地化属性页，请将类似于以下示例的代码添加到属性页的实现文件（在[COlePropertyPage：： OnSetPageSite](reference/colepropertypage-class.md#onsetpagesite)的重写中）：
