...
- CBRS_TOP （默认值） 的 pin 顶部控件条。

- CBRS_BOTTOM Pin 底部将控件条。

- CBRS_NOALIGN 不会重新定位控件条父级一起调整大小。
...
- `CDockBar` 通用停靠区域适用于其他`CControlBar`派生的对象。 此类中可用的特定成员函数和变量可能在将来的版本中发生更改。

所有控件条对象/窗口都将是某个父框架窗口的子窗口。 它们通常将作为同级添加到框架的工作区（例如，MDI 客户端或视图）。 控件条的子窗口 ID 很重要。 控件条的默认布局仅适用于控件条起到 AFX_IDW_CONTROLBAR_LAST AFX_IDW_CONTROLBAR_FIRST 范围中的 Id。 请注意，即使具有 256 个控件条 ID，其中前 32 个控件条 ID 也是特定的，因为它们由打印预览体系结构直接支持。
...
控件条的布局算法非常简单。 框架窗口将 WM_SIZEPARENT 一条消息发送到控件条范围内的所有子级。 除此消息之外，还将传递指向父级矩形工作区的指针。 此信息发送给处于 Z 顺序的子级。 控件条子级使用此信息来确定自己的位置并缩减父级工作区的范围。 常规工作区剩下的最后一个矩形（小于控件条）将用于确定主要工作区窗口的位置（通常是 MDI 客户端、视图或拆分窗口）。

有关更多详细信息，请参阅 `CWnd::RepositionBars` 和 `CFrameWnd::RecalcLayout` 。

MFC 私有 Windows 消息，包括 WM_SIZEPARENT，记录在[技术说明 24](../mfc/tn024-mfc-defined-messages-and-resources.md)。
...
通常更新指示器的方法是通过 ON_UPDATE_COMMAND_UI 机制。 空闲时，状态栏将调用具有指示器窗格字符串 ID 的 ON_UPDATE_COMMAND_UI 处理程序。
...
工具栏是具有一行位图按钮（可能包含分隔符）的控件条。 支持两种样式的按钮：按键和复选框按钮。 可以使用复选框按钮和 ON_UPDATE_COMMAND_UI 构建单选按钮组功能。
...
工具栏按钮，始终将更新的方法是通过 ON_UPDATE_COMMAND_UI 机制。 空闲时，工具栏将调用该按钮的命令 id 的 ON_UPDATE_COMMAND_UI 处理程序。 ON_UPDATE_COMMAND_UI 不调用分隔符，但对按键和复选框按钮调用。
...
自定义 Api 将允许您更改指定的工具栏按钮的状态，最好是应更改这些状态的工具栏按钮表示的命令的 ON_UPDATE_COMMAND_UI 处理程序中。 请记住，空闲处理将更改与 ON_UPDATE_COMMAND_UI 处理程序，工具栏按钮的状态，以便在下一步后，对这些状态通过 SetButtonStyle 所做的任何更改可能会丢失空闲。

工具栏按钮将发送 WM_COMMAND 消息通常由一个 ON_COMMAND 处理程序提供 ON_UPDATE_COMMAND_UI 处理程序的同一个类中处理和等常规按钮或菜单项。
...
`CDialogBar` 的用法类似于 `CFormView`。 您必须定义对话栏的对话模框并删除 WS_CHILD 之外的所有样式。 请注意，对话框必须是不可见的。

`CDialogBar` 的控件通知将发送给控件条的父级（如工具栏按钮）。

## <a name="ccmdui-support-for-cdialogbar"></a>CDialogBar 的 CCmdUI 支持

对话栏按钮应该通过 ON_UPDATE_COMMAND_UI 处理程序机制进行更新。 在空闲时，对话栏将调用 ON_UPDATE_COMMAND_UI 处理程序与具有 ID 的所有按钮的命令 ID > = 0x8000 (即中的命令 Id 的范围)。
