...
当标签编辑已取消或已完成时, 树控件将发送[TVN_ENDLABELEDIT](/windows/win32/Controls/tvn-endlabeledit)通知消息。 *LParam*参数是[NMTVDISPINFO](/windows/win32/api/commctrl/ns-commctrl-tvdispinfow)结构的地址。 **项**成员是标识项并包含已编辑的文本的[TVITEM](/windows/win32/api/commctrl/ns-commctrl-tvitemw)结构。 您负责根据需要更新项的标签（可能在验证编辑过的字符串之后）。 如果取消编辑, `TV_ITEM`则的 pszText 成员为0。
