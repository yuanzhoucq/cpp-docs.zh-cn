 ...
 **消息反射是什么**  
 ...
 Windows 控件经常到其父窗口发送通知消息。 例如，许多控件发送控制颜色通知消息 (`WM_CTLCOLOR`或其变化版本之一) 到其父以允许父提供用于绘制控件的背景画笔。  
 ...
 在 MFC 4.0 中的旧机制仍然正常工作，父窗口可以处理通知消息。 此外，但是，MFC 4.0 促进重用通过提供一种称为"消息反射"功能允许子控件的窗口或父窗口中，或两者中要处理这些通知消息。 在控件背景的颜色示例中，您现在可以编写通过处理反射设置其自己的背景颜色的控件类`WM_CTLCOLOR`消息 — 所有而不依赖于父级。 (请注意，由于消息反射通过 MFC 实现，不由 Windows，父窗口类必须派生自`CWnd`有关消息反射来工作。)  
  
 通过为某些消息，如所有者描述的列表框的消息提供虚函数的较旧版本的 MFC 未类似于消息反射 (`WM_DRAWITEM`，依次类推)。 新的消息反射机制是通用和一致。  
...
-   有关宏**WM_COMMAND**通知是**ON_CONTROL_REFLECT**。  
  
-   有关宏**WM_NOTIFY**反射是**ON_NOTIFY_REFLECT**。  
  
-   有关宏`ON_UPDATE_COMMAND_UI`反射是**ON_UPDATE_COMMAND_UI_REFLECT**。  
...
5.  使用类向导，将添加的处理程序反映`WM_CTLCOLOR`消息到您`CYellowEdit`类。 请注意，你可以处理的消息列表中的消息名称的前面等号指示该消息将反映。 这中所述[反映的消息定义消息处理程序](../mfc/reference/defining-a-message-handler-for-a-reflected-message.md)。  
