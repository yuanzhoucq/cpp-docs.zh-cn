...
ms.locfileid: "87215915"
---
# <a name="virtual-list-controls"></a>虚拟列表控件

虚拟列表控件是具有 LVS_OWNERDATA 样式的列表视图控件。 此样式使控件能够支持到**DWORD**值的项计数（默认项计数仅扩展到 **`int`** ）。 但是，此样式提供的最大优势在于，在任何时候都只能在内存中创建一个数据项子集。 这允许虚拟列表视图控件将其自身用于大数据库信息，在这些数据库中，访问数据的特定方法已就位。
...
在 LVN_GETDISPINFO 通知消息的处理程序中，必须检查正在请求的信息类型。 可能的值包括：
