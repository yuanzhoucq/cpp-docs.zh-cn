...
本部分中的主题介绍如何使用 `CString` 进行编程。  有关 `CString` 类的参考文档，请参阅 [CStringT](../atl-mfc-shared/reference/cstringt-class.md) 的文档。  
  
 若要使用 `CString`，请包含 `atlstr.h` 标头。  
  
 `CString`、`CStringA` 和 `CStringW` 类是称为 [CStringT](../atl-mfc-shared/reference/cstringt-class.md) 的类模板的专用化，此类模板基于它们所支持的字符数据类型。  
