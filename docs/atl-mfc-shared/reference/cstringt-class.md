...
- **StrTraitATL< wchar_t** &#124; **`char`** &#124; **TCHAR、ChTraitsCRT< wchar_t** &#124; **`char`** &#124; **TCHAR > >**
   ...
   类需要 CRT 支持并在指定的模块中搜索资源字符串 `m_hInstResource` （应用程序的模块类的成员）。

- **StrTraitATL< wchar_t** &#124; **`char`** &#124; **TCHAR、ChTraitsOS< wchar_t** &#124; **`char`** &#124; **TCHAR > >**

   类不要求 CRT 支持并在指定的模块中搜索资源字符串 `m_hInstResource` （应用程序的模块类的成员）。
...
`CString`、 `CStringA` 和 `CStringW` 将从 MFC dll （MFC90.DLL）导出，而不是从用户 dll 中导出。 这样做是为了防止 `CStringT` 定义多次。
...
由于 `CStringT` 使用模板参数定义受支持的字符类型（ [wchar_t](../../c-runtime-library/standard-types.md)或[char](../../c-runtime-library/standard-types.md)），因此方法参数类型可能会很复杂。 若要简化此问题，请在整个类中定义和使用一组预定义类型 `CStringT` 。 下表列出了各种类型：

|名称|描述|
|----------|-----------------|
|`XCHAR`|**`wchar_t`** **`char`** 与对象具有相同字符类型的单个字符（或） `CStringT` 。|
|`YCHAR`|单个字符（ **`wchar_t`** 或 **`char`** ），其相对字符类型为 `CStringT` 对象。|
|`PXSTR`|指向 **`wchar_t`** **`char`** 与对象具有相同字符类型的字符串（或）的指针 `CStringT` 。|
|`PYSTR`|一个指针，指向字符串（ **`wchar_t`** 或），其 **`char`** 相对字符类型为 `CStringT` 对象。|
|`PCXSTR`|指向 **`const`** **`wchar_t`** **`char`** 与对象具有相同字符类型的字符串（或）的指针 `CStringT` 。|
|`PCYSTR`|一个指针，指向 **`const`** 字符串（ **`wchar_t`** 或），其 **`char`** 相对字符类型为 `CStringT` 对象。|

> [!NOTE]
> 以前使用的未记录方法 `CString` （例如）的代码 `AssignCopy` 必须替换为使用以下记录的方法的代码 `CStringT` （如 `GetBuffer` 或 `ReleaseBuffer` ）。 这些方法是从继承的 `CSimpleStringT` 。
...
此函数在中设置并追加一系列字符和值 `CStringT` 。 每个可选参数（如果有）将根据*pszFormat*中相应的格式规范进行转换并追加，或由*nFormatID*标识的字符串资源。
...
- `CStringT`（ `const unsigned char*` `psz` ）：使你可以从指向的 `CStringT` 指针构造 **`unsigned char`** 。
...
此函数在中设置和存储一系列字符和值 `CStringT` 。 每个可选参数（如果有）根据*pszFormat*中相应的格式规范进行转换和输出，或由*nFormatID*标识的字符串资源进行输出。
...
从此对象中提取长度为*nCount*个字符的子字符串 `CStringT` ，从位置*iFirst* （从零开始）开始。
...
从此对象中提取最后一个（即右边） *nCount*字符 `CStringT` ，并返回提取的子字符串的副本。
