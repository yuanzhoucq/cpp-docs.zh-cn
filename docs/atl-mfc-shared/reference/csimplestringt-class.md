...
|名称|描述|
|----------|-----------------|
|[CSimpleStringT::CSimpleStringT](#ctor)|以多种方式构造 `CSimpleStringT` 的对象。|
...
|[CSimpleStringT::GetBuffer](#getbuffer)|返回指向 `CSimpleStringT`中的字符的指针。|
|[CSimpleStringT::GetBufferSetLength](#getbuffersetlength)|返回指向 `CSimpleStringT`中的字符的指针，并将截断为指定长度。|
...
|[CSimpleStringT::ReleaseBuffer](#releasebuffer)|释放 `GetBuffer`返回的缓冲区的控制。|
|[CSimpleStringT::ReleaseBufferSetLength](#releasebuffersetlength)|释放 `GetBuffer`返回的缓冲区的控制。|
...
|[CSimpleStringT::operator\[\]](#operator_at)|返回指定位置处的字符— `GetAt`的运算符替换。|
|[CSimpleStringT::operator +=](#operator_add_eq)|将新字符串连接到现有字符串的末尾。|
|[CSimpleStringT::operator =](#operator_eq)|将新值分配给 `CSimpleStringT` 的对象。|
...
调用此方法可将现有 `CSimpleStringT` 对象追加到另一个 `CSimpleStringT` 对象中。
...
将一个或多个字符复制到 `CSimpleStringT` 的对象。
...
调用此方法可将字符从*pchSrc*复制到*pchDest*字符串。 与 `CopyChars`不同，`CopyCharsOverlapped` 提供一种安全的方法，可从可能重叠的字符缓冲区进行复制。

### <a name="example"></a>示例

请参阅[CSimpleStringT：： CopyChars](#copychars)的示例或 `CSimpleStringT::SetString` 的源代码（位于 atlsimpstr 中）。
...
*pszSrc*<br/>
要复制到此 `CSimpleStringT` 对象的以 null 值结束的字符串。

*nLength*<br/>
`pch`中的字符数计数。

*pStringMgr*<br/>
指向 `CSimpleStringT` 对象的内存管理器的指针。 有关 `CSimpleStringT``IAtlStringMgr` 和内存管理的详细信息，请参阅[内存管理和 CStringT](../memory-management-with-cstringt.md)。

### <a name="remarks"></a>备注

构造一个新的 `CSimpleStringT` 对象。 由于构造函数将输入数据复制到新分配的存储中，因此可能会导致内存异常。

### <a name="example"></a>示例

下面的示例演示如何使用 ATL **typedef** `CSimpleString``CSimpleStringT::CSimpleStringT`。 `CSimpleString` 是类模板 `CSimpleStringT`的常用专用化。
...
*iChar*<br/>
`CSimpleStringT` 对象中的字符从零开始的索引。 *IChar*参数必须大于或等于0且小于[GetLength](#getlength)返回的值。 否则，`GetAt` 将生成异常。
...
调用此方法以返回由*iChar*指定的一个字符。 重载的下标（ **[]** ）运算符是 `GetAt`的方便别名。 空终止符无需使用 `GetAt`生成异常即可进行寻址。 但是，它不会被 `GetLength`计数，并且返回的值为0。
...
如果*nMinBufferLength*大于当前缓冲区的长度，`GetBuffer` 会销毁当前缓冲区，并将其替换为所请求大小的缓冲区，并将对象引用计数重置为零。 如果以前在此缓冲区上调用了[LockBuffer](#lockbuffer) ，则会丢失缓冲区锁。

### <a name="return-value"></a>返回值

指向对象的（以 null 终止的）字符缓冲区的 `PXSTR` 指针。

### <a name="remarks"></a>备注

调用此方法以返回 `CSimpleStringT` 对象的缓冲区内容。 返回的 `PXSTR` 不是一个常数，因此允许直接修改 `CSimpleStringT` 内容。

如果使用 `GetBuffer` 返回的指针来更改字符串内容，则必须先调用[ReleaseBuffer](#releasebuffer) ，然后才能使用任何其他 `CSimpleStringT` 成员方法。

对 `ReleaseBuffer` 的调用后 `GetBuffer` 返回的地址可能无效，因为其他 `CSimpleStringT` 操作可能导致 `CSimpleStringT` 缓冲区重新分配。 如果不更改 `CSimpleStringT`的长度，则不会重新分配缓冲区。

销毁 `CSimpleStringT` 对象时，会自动释放缓冲区内存。

如果自行跟踪字符串长度，则不应追加终止 null 字符。 但是，在用 `ReleaseBuffer`释放缓冲区时，必须指定最终字符串长度。 如果追加了终止 null 字符，则应该为长度传递-1 （默认值）。 然后 `ReleaseBuffer` 确定缓冲区长度。
...
*nLength*<br/>
`CSimpleStringT` 字符缓冲区的准确大小（以字符为限）。
...
调用此方法可检索指定长度的 `CSimpleStringT` 对象的内部缓冲区。 返回的 `PXSTR` 指针不是**常量**，因此允许直接修改 `CSimpleStringT` 内容。

如果使用[GetBufferSetLength](#getbuffersetlength)返回的指针更改字符串内容，请调用 `ReleaseBuffer` 以更新 `CsimpleStringT` 的内部状态，然后再使用任何其他 `CSimpleStringT` 方法。

对 `ReleaseBuffer` 的调用后 `GetBufferSetLength` 返回的地址可能无效，因为其他 `CSimpleStringT` 操作可能导致 `CSimpleStringT` 缓冲区重新分配。 如果未更改 `CSimpleStringT`的长度，则不会重新分配缓冲区。

销毁 `CSimpleStringT` 对象时，会自动释放缓冲区内存。

如果自行跟踪字符串长度，请不要追加终止 null 字符。 使用 `ReleaseBuffer`释放缓冲区时，必须指定最终的字符串长度。 如果在调用 `ReleaseBuffer`时追加一个终止 null 字符，则 `ReleaseBuffer`长度的传递-1 （默认值），`ReleaseBuffer` 将对缓冲区执行 `strlen` 以确定其长度。
...
对于多字节字符集（MBCS），`GetLength` 对每个8位字符进行计数;也就是说，一个多字节字符中的前导和结尾字节被计为两个字节。 有关调用此函数的示例，请参阅[FreeExtra](#freeextra) 。
...
调用此方法可检索 `CSimpleStringT` 对象所使用的内存管理器。 有关内存管理器和字符串对象的详细信息，请参阅[内存管理和 CStringT](../memory-management-with-cstringt.md)。
...
> [!NOTE]
>  返回 `PCXSTR` 指针为**const** ，不允许直接修改 `CSimpleStringT` 内容。
...
如果 `CSimpleStringT` 对象的长度为0，则返回 TRUE;否则为 FALSE。
...
调用此方法可锁定 `CSimpleStringT` 对象的缓冲区。 通过调用 `LockBuffer`，可创建字符串的副本，引用计数使用-1。 当引用计数值为-1 时，缓冲区中的字符串被视为处于 "锁定" 状态。 处于锁定状态时，将通过两种方式保护字符串：
...
> [!NOTE]
>  如果对锁定的缓冲区调用[GetBuffer](#getbuffer) ，并将 `GetBuffer` 参数 `nMinBufferLength` 设置为大于当前缓冲区的长度，则将丢失缓冲区锁。 此类对 `GetBuffer` 的调用会销毁当前缓冲区，并将其替换为所请求大小的缓冲区，并将引用计数重置为零。
...
> [!NOTE]
>  可以使用下标（ **[]** ）运算符来获取 `CSimpleStringT`中的字符的值，但不能使用它来更改 `CSimpleStringT`中的字符的值。
...
运算符接受其他 `CSimpleStringT` 对象或字符。 请注意，只要使用此串联运算符，就可能会发生内存异常，因为可以为添加到此 `CSimpleStringT` 对象中的字符分配新存储。
...
不复制任何字符;仅返回指针。 请小心处理此运算符。 如果在获取了字符指针之后更改 `CString` 对象，则可能会导致重新分配使指针失效的内存。
...
调用此方法可为 `CSimpleStringT` 对象分配特定缓冲区大小。

`CSimpleStringT` 如果无法为字符缓冲区分配空间，则会生成 STATUS_NO_MEMORY 异常。 默认情况下，内存分配由 WIN32 API 函数执行 `HeapAlloc` 或 `HeapReAlloc`。
...
*iChar*<br/>
`CSimpleStringT` 对象中的字符从零开始的索引。 *IChar*参数必须大于或等于0且小于[GetLength](#getlength)返回的值。
...
将字符串复制到 `CSimpleStringT` 对象。 `SetString` 覆盖缓冲区中较旧的字符串数据。

的两个版本都 `SetString` 检查*pszSrc*是否为 null 指针，如果为，则引发 E_INVALIDARG 错误。

`SetString` 的单参数版本需要*pszSrc*指向以 null 结尾的字符串。

`SetString` 的双参数版本也需要*pszSrc*为以 null 结尾的字符串。 它使用*nLength*作为字符串长度，除非首先遇到空终止符。

`SetString` 的双参数版本还会检查*pszSrc*是否指向 `CSimpleStringT`中当前缓冲区的位置。 在这种特殊情况下，`SetString` 使用内存复制函数，该函数在将字符串数据复制回其缓冲区时不会覆盖字符串数据。
...
`CSimpleStringT` 析构函数自动调用 `UnlockBuffer`，以确保调用析构函数时不会锁定缓冲区。 有关此方法的示例，请参阅[LockBuffer](#lockbuffer)。
