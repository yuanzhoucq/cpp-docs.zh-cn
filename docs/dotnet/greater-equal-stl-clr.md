...
模板类描述一个函数，它在调用时，只有第一参数不小于第二大时才返回 true。  根据其参数类型使用它来指定一个函数对象。  
...
## 备注  
 模板类描述带有两个参数的仿函数。  它定义成员运算符 `operator()`，以便在对象作为函数调用时，在第一参数大于或等于第二个时，它返回 true。  
