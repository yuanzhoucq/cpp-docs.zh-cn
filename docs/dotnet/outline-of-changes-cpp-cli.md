...
ms.locfileid: "46415117"
---
# <a name="outline-of-changes-ccli"></a>更改概要 (C++/CLI)

此图显示了您所做的更改的一些示例从托管扩展语言中的 Visual c + + 的 c + +。 请按照随附的详细信息每个项的链接。

## <a name="no-double-underscore-keywords"></a>任何双下划线关键字

已删除双下划线的所有关键字的前面，有一个例外。 因此，`__value`变得`value`，并`__interface`变得`interface`，依次类推。 若要防止名称冲突的关键字和用户代码中的标识符之间，关键字被视为上下文。

请参阅[语言关键字 (C + + CLI)](../dotnet/language-keywords-cpp-cli.md)有关详细信息。
...
CLR 数组已经过重新设计。 它是类似于 stl`vector`模板集合，但映射到基础`System::Array`类-也就是说，它不是模板实现。
...
### <a name="shorthand-initialization-of-local-clr-array"></a>本地 CLR 数组的速记初始化
...
- 内置算术类型和对象类层次结构之间的重载解析托管扩展和 Visual c + + 中相反。 其副作用是，CLR 枚举将不再隐式转换为算术类型。

- 在新语法中，CLR 枚举可以保持自己的作用域，不是托管扩展中的用例。 以前，枚举器是枚举的可见; 包含作用域内现在，枚举器枚举的作用域内封装。

请参阅[CLR 枚举类型](../dotnet/clr-enum-type.md)有关详细信息。

## <a name="removal-of-box-keyword"></a>删除的 __box 关键字
...
请参阅[跟踪句柄装箱值](../dotnet/a-tracking-handle-to-a-boxed-value.md)有关详细信息。
