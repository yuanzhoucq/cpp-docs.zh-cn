---
title: 使用 for each 循环访问 c + + 标准库集合 |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
下面的代码示例使用`for each`来循环访问[\<映射 >](../standard-library/map.md)。
...
下面的代码示例使用常量引用 (`const&`) 的 c + + 标准库容器的迭代变量。 可以使用的引用 (`&`) 上可以声明为任何的类型集合的迭代变量作为*T*`&`。
