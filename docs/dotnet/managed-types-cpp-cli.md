---
title: 将托管类型 (C + + CLI) |Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-cli
...
2.  首先，隔离和重现或其他错误情况的异常。 由于过时的异常发生的问题，应可重现。  
  
3.  检查任何引用的程序集在应用程序中的时间戳。  
  
4.  如果任何引用的程序集的时间戳晚于应用程序的最后一个编译的时间戳，您的应用程序已过期。 如果发生这种情况，重新编译应用程序与最新程序集，并进行所需的任何代码更改。  
  
5.  重新运行该应用程序中，执行重现该问题，并验证该异常不会发生的步骤。  
