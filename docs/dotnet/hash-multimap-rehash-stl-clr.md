...
## 备注  
 成员函数重新生成哈希表，确保该`() <=` [hash\_multimap::load\_factor](../dotnet/hash-multimap-load-factor-stl-clr.md)[hash\_multimap::max\_load\_factor](../dotnet/hash-multimap-max-load-factor-stl-clr.md)。  否则，此哈希表的尺寸增大仅根据需要在插入后。\(为固定大小决不自动会降低。\)使用哈希来调整表的大小。  
