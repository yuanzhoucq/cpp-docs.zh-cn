...
ms.locfileid: "34704758"
---
# <a name="how-to-create-verifiable-c-projects-ccli"></a>如何： 创建可验证的 c + + 项目 (C + + /cli CLI)

Visual c + + 应用程序向导不会创建可验证项目。

> [!IMPORTANT]
> 不建议使用 visual Studio 2015 和 Visual Studio 2017 不支持 **/clr: pure**和 **/clr: safe**创建可验证项目。 如果你需要可验证代码，我们建议您将转换为 C# 代码。

但是，如果你使用的较旧版本的支持的 Visual c + + 编译器工具集 **/clr: pure**和 **/clr: safe**，项目可以转换为可验证。 本主题介绍如何设置项目属性和修改项目源代码文件，以转换你的 Visual c + + 项目，以便生成可验证的应用程序。
...
1. 上**常规**下页上**配置属性**节点，设置**公共语言运行时支持**属性**安全 MSIL 公共语言运行时支持 (/: safe)**。

1. 上**高级**下页上**链接器**节点，设置**CLR 映像类型**属性**强制安全 IL 映像 (/ CLRIMAGETYPE:SAFE)**。
...
1. 上**高级**下页上**链接器**节点，输入`Main`作为**入口点**属性值。
