...
请注意，容器仅确保其键具有等效排序的元素（以及到相同整数值的哈希）在存储桶中相邻。 与模板类[hash_multiset （STL/CLR）](../dotnet/hash-multiset-stl-clr.md)不同，模板类的对象 `hash_set` 可确保所有元素的键都是唯一的。 （没有两个键具有等效的顺序。）
...
第一个成员函数删除由*where*指向的受控序列的元素，并返回一个迭代器，该迭代器指定删除的元素之外剩余的第一个元素; 如果此类元素不存在，则返回[hash_set：： end （STL/CLR）](../dotnet/hash-set-end-stl-clr.md) `()` 。 使用它可以删除单个元素。
...
用序列 [ `right.begin()` ， `right.end()` ）、默认的排序谓词和默认哈希函数初始化受控序列。 用于指定初始受控序列，该序列是由 hash_set 对象*权限*控制的序列的副本，具有默认的排序谓词和哈希函数。
...
成员函数确定受控序列中的第一个元素 `X` ，该元素将哈希处理为与*键*相同的存储桶并具有与*键*等效的排序。 如果此类元素不存在，它将返回[hash_set：： end （STL/CLR）](../dotnet/hash-set-end-stl-clr.md) `()` ; 否则返回指定的迭代器 `X` 。 用于查找当前在受控序列中与指定键匹配的一系列元素的开头。
...
成员函数确定受控序列中的最后一个元素 `X` ，该元素将哈希处理为与*键*相同的存储桶并具有与*键*等效的排序。 如果此类元素不存在，或者如果 `X` 是受控序列中的最后一个元素，则它将返回[hash_set：： END （STL/CLR）](../dotnet/hash-set-end-stl-clr.md) `()` ; 否则返回指定第一个元素的迭代器 `X` 。 使用它可以查找受控序列中当前与指定键匹配的元素序列的末尾。
