...
|[hash_set::difference_type (STL/CLR)](#difference_type)|两个元素间的 （可能是带符号） 距离的类型。|  
|[hash_set::generic_container (STL/CLR)](#generic_container)|容器的泛型接口的类型。|  
|[hash_set::generic_iterator (STL/CLR)](#generic_iterator)|容器的泛型接口的迭代器类型。|  
...
|[hash_set::hasher (STL/CLR)](#hasher)|用于密钥的哈希委托。|  
|[hash_set::iterator (STL/CLR)](#iterator)|受控序列的迭代器的类型。|  
|[hash_set::key_compare (STL/CLR)](#key_compare)|两个键的排序的委托。|  
...
|[hash_set::size_type (STL/CLR)](#size_type)|两个元素间的 （非负号） 距离的类型。|  
...
|[hash_set::count (STL/CLR)](#count)|对与指定的键匹配的元素计数。|  
...
|[hash_set::key_comp (STL/CLR)](#key_comp)|将复制为两个键排序的委托。|  
|[hash_set::load_factor (STL/CLR)](#load_factor)|对每个存储桶的平均元素数进行计数。|  
|[hash_set::lower_bound (STL/CLR)](#lower_bound)|查找与指定的键匹配的范围开始处。|  
|[hash_set::make_value (STL/CLR)](#make_value)|构造值对象。|  
...
|[hash_set::to_array (STL/CLR)](#to_array)|受控的序列复制到新数组。|  
...
### <a name="remarks"></a>备注  
 成员函数可有效地调用[hash_set:: erase (STL/CLR)](../dotnet/hash-set-erase-stl-clr.md) `(` [hash_set:: begin (STL/CLR)](../dotnet/hash-set-begin-stl-clr.md) `(),` [hash_set:: end (STL/CLR)](../dotnet/hash-set-end-stl-clr.md)`())`. 你可以使用它来确保受控的序列为空。  
...
### <a name="remarks"></a>备注  
 第一个成员函数删除指向的受控序列的元素`where`，并返回指定已移除的元素之外保留的第一个元素的迭代器或[hash_set:: end (STL/CLR)](../dotnet/hash-set-end-stl-clr.md)`()`如果此类元素不存在。 用于删除单个元素。  
...
### <a name="remarks"></a>备注  
 成员函数将返回`(float)` [hash_set:: size (STL/CLR)](../dotnet/hash-set-size-stl-clr.md) `() /` [hash_set::bucket_count (STL/CLR)](../dotnet/hash-set-bucket-count-stl-clr.md)`()`。 你可以使用它来确定的平均存储桶大小。  
...
### <a name="remarks"></a>备注  
 成员函数将返回受控序列的长度。 用于确定当前受控序列中的元素的数目。 如果你关注的只是序列是否具有非零大小，请参阅[hash_set:: empty (STL/CLR)](../dotnet/hash-set-empty-stl-clr.md)`()`。  
...
### <a name="remarks"></a>备注  
 成员函数将确定最后一个元素`X`作为相同的存储桶进行哈希处理到受控序列中`key`和具有等效顺序到`key`。 如果此类元素不存在，或者`X`为受控序列中的最后一个元素，它将返回[hash_set:: end (STL/CLR)](../dotnet/hash-set-end-stl-clr.md)`()`; 否则它将返回指定以外的第一个元素的迭代器`X`. 用于当前与指定的键匹配的控制序列中查找的元素序列的末尾。  
