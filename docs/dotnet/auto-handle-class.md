...
|[auto_handle::swap](#swap)|将对象与另一个对象交换 `auto_handle` 。|  
...
|[auto_handle::operator auto_handle](#operator-auto-handle)|与兼容的类型之间的类型转换运算符 `auto_handle` 。|  
|[auto_handle::operator bool](#operator-bool)|用于 `auto_handle` 条件表达式中的运算符。|
|[auto_handle：： operator！](#operator-logical-not)|用于 `auto_handle` 条件表达式中的运算符。|  
...
当前 `auto_handle` 强制转换为的 `auto_handle<_other_type>` 。
