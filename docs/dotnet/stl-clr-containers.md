...
STL/CLR 库包含类似于 c + + 标准库中的容器，但它在 .NET Framework 的托管环境中运行。 它不会与实际的 c + + 标准库保持同步，并为传统支持维护。

本文档提供 STL/CLR 中的容器的概述，例如容器元素的需求、可以插入到容器中的元素类型以及容器中的元素的所有权问题。 在适当的情况下，将介绍本机 c + + 标准库和 STL/CLR 之间的差异。
