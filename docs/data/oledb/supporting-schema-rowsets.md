...
-   **C** *短名称* **SessionTRSchemaRowset**处理的表信息的请求 (`DBSCHEMA_TABLES`架构行集)。  
  
-   **C** *短名称* **SessionColSchemaRowset**处理的列信息的请求 (`DBSCHEMA_COLUMNS`架构行集)。 向导提供了这些类，它返回一个 DOS 提供程序的架构信息的示例实现。  
  
-   **C** *短名称* **SessionPTSchemaRowset**处理有关提供程序类型的架构信息的请求 (`DBSCHEMA_PROVIDER_TYPES`架构行集)。 向导提供的默认实现返回`S_OK`。  
  
 可以自定义这些类来处理适合您的提供程序的架构信息：  
  
-   在中**C***短名称***SessionTRSchemaRowset**，必须填写目录、 表和描述字段 (`trData.m_szType`， `trData.m_szTable`，和`trData.m_szDesc`)。 该向导生成的示例使用只有一个行 （表）。 其他提供程序可能会返回多个表。  
...
### <a name="example"></a>示例  
 此代码摘自[UpdatePV](http://msdn.microsoft.com/c8bed873-223c-4a7d-af55-f90138c6f38f)示例。 UpdatePv 支持三个所需的架构行集： `DBSCHEMA_TABLES`， `DBSCHEMA_COLUMNS`，和`DBSCHEMA_PROVIDER_TYPES`。 为了举例说明如何在您的提供程序中实现架构支持，本主题逐步讲解如何完成实现`DBSCHEMA_TABLE`行集。  
...
```  
 ...
 有关支持架构行集的提供程序的示例，请参阅[UpdatePV](http://msdn.microsoft.com/c8bed873-223c-4a7d-af55-f90138c6f38f)示例。  
