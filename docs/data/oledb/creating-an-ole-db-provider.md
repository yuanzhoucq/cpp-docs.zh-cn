...
1. 修改中的代码`Execute`中的方法[CMyProviderRowset(MyProviderRS.h)](cmyproviderrowset-myproviderrs-h.md)。 有关示例，请参阅[字符串读入 OLE DB 提供程序](../../data/oledb/reading-strings-into-the-ole-db-provider.md)。  
  
1. 该属性映射中编辑[MyProviderDS.h](cmyprovidersource-myproviderds-h.md)， [MyProviderSess.h](cmyprovidersession-myprovidersess-h.md)，并[MyProviderRS.h](cmyproviderrowset-myproviderrs-h.md)。 向导将创建包含一个提供程序可能会实现的所有属性的属性映射。 完成属性映射，并删除或注释掉您的提供程序不需要支持的属性。  
  
1. 更新 PROVIDER_COLUMN_MAP，可以在中找到[CMyProviderRowset(MyProviderRS.h)](cmyproviderrowset-myproviderrs-h.md)。 有关示例，请参阅[存储字符串中的 OLE DB 提供程序](../../data/oledb/storing-strings-in-the-ole-db-provider.md)。  
