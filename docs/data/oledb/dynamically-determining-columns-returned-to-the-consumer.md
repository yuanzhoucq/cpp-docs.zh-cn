...
若要处理`IColumnsInfo::GetColumnsInfo`调用中，删除 PROVIDER_COLUMN_MAP，定义了函数`GetColumnInfo`，从`CAgentMan`用户记录中 CustomRS.h 并将其替换为你自己的定义为`GetColumnInfo`函数：
...
接下来，实现`GetColumnInfo`函数中 CustomRS.cpp，如下面的代码中所示。
