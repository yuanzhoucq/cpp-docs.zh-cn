...
该向导还会生成三个会话类： `CCustomSessionColSchema`， `CCustomSessionPTSchema`，和`CCustomSessionTRSchema`。 这些会话用于架构行集。 架构行集允许的提供程序没有使用者无需执行查询或提取数据的情况下向使用者返回的元数据。 提取元数据可以是比发现提供程序功能更快。  
