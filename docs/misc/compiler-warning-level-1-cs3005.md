...
仅大小写不同的标识符“identifier”不符合 CLS  
 ...
 [公共](../Topic/public%20\(C%23%20Reference\).md)、[受保护](../Topic/protected%20\(C%23%20Reference\).md) 或 `protected` `internal` 标识符仅一个或多个字母与另一个 `public`、`protected` 或 `protected` `internal` 标识符不同，不符合公共语言规范 \(CLS\)。 有关 CLS 遵从性的详细信息，请参见[编写符合 CLS 的代码](http://msdn.microsoft.com/zh-cn/4c705105-69a2-4e5e-b24e-0633bc32c7f3)和[语言独立性和与语言无关的组件](../Topic/Language%20Independence%20and%20Language-Independent%20Components.md)。  
