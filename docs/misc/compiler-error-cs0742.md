...
1.  向查询中添加一个 [select 子句](../Topic/select%20clause%20\(C%23%20Reference\).md)或 [group 子句](../Topic/group%20clause%20\(C%23%20Reference\).md)。  
...
```  
 ...
 如果 `group` 子句使用 [into](../Topic/into%20\(C%23%20Reference\).md) 关键字将分组结果存储到临时标识符中，则它不能用作查询中的最后一个子句。 仍需要使用 `select` 或另一个 `group` 子句。  
