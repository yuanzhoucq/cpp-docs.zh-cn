...
赋值语句或声明中的表达式将 <xref:System.Void> 用作变量、过程参数、函数返回或类型实参的类型。  
 ...
 可使用 [GetType 运算符](../Topic/GetType%20Operator%20\(Visual%20Basic\).md) 运算符来测试引用变量，以查看其运行时类型是否为 <xref:System.Void>，但不可在任何其他上下文中使用 <xref:System.Void>。  
...
## 请参阅  
 <xref:System.Void>   
 [GetType 运算符](../Topic/GetType%20Operator%20\(Visual%20Basic\).md)