...
无法给静态只读字段“name”的字段赋值（静态构造函数或变量初始值设定项中除外）  
 ...
 [readonly](../Topic/readonly%20\(C%23%20Reference\).md) 变量必须与构造函数具有相同的[静态](../Topic/static%20\(C%23%20Reference\).md)用法，在此构造函数中你要对其进行初始化。 有关详细信息，请参阅[静态构造函数](../Topic/Static%20Constructors%20\(C%23%20Programming%20Guide\).md)。  
