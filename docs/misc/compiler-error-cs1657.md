...
无法将“parameter”作为 ref 或 out 参数进行传递，原因是“reason”  
 ...
 如果在某个变量是只读变量的上下文将该变量作为 [ref](../Topic/ref%20\(C%23%20Reference\).md) 或 [out](../Topic/out%20\(C%23%20Reference\).md) 参数进行传递，则会发生此错误。 只读上下文包括 [foreach](../Topic/foreach,%20in%20\(C%23%20Reference\).md) 迭代变量、[using](../Topic/using%20Statement%20\(C%23%20Reference\).md) 变量和 `fixed` 变量。 要解决此错误，请勿调用在 `using` 块、`foreach` 语句和 `fixed` 语句中采用 `foreach`、`using` 或 `fixed` 变量作为 `ref` 或 `out` 参数的函数。  
