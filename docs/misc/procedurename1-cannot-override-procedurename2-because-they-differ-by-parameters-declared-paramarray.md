...
派生类中的过程重写基类中同名的过程，但参数列表不同。  
 ...
 要重写继承类中的过程，重写过程必须与其参数列表、访问级别和返回类型（如果有）匹配。 特别是，它必须与所有 [Optional](../Topic/Optional%20\(Visual%20Basic\).md) 或 [ParamArray](../Topic/ParamArray%20\(Visual%20Basic\).md) 声明匹配。  
...
-   如果希望参数列表与基类版本不同，则不能重写它。 请考虑改为重载。 有关更多信息，请参见[过程重载](../Topic/Procedure%20Overloading%20\(Visual%20Basic\).md)。  
