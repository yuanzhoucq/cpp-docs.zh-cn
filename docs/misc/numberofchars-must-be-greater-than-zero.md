---
title: "NumberOfChars 必须大于 0 | Microsoft Docs"
ms.custom: ""
ms.date: "11/16/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "vbrTextFieldParser_NumberOfCharsMustBePositive"
ms.assetid: 3eea4bbf-cd49-4d19-adfb-0e2adf087065
caps.latest.revision: 7
caps.handback.revision: 7
author: "stevehoag"
ms.author: "shoag"
manager: "wpickett"
---
# NumberOfChars 必须大于 0
使用 `TextFieldParser` 对象的 `PeekChars` 方法时，必须提供一个大于 `0` 的 `NumberOfChars` 值。  
  
### 更正此错误  
  
-   将 `NumberOfChars` 更改为大于 `0` 的值。  
  
## 请参阅  
 [如何：读取具有多种格式的文本文件](../Topic/How%20to:%20Read%20From%20Text%20Files%20with%20Multiple%20Formats%20in%20Visual%20Basic.md)   
 [My.Computer.FileSystem.OpenTextFieldParser 方法](http://msdn.microsoft.com/zh-cn/e5869f85-c078-485f-8323-8dc716494546)   
 [TextFieldParser.PeekChars 方法](http://msdn.microsoft.com/zh-cn/4a180d26-d46d-4cc1-9af7-d23abe27c89b)   
 [使用 TextFieldParser 对象分析文本文件](../Topic/Parsing%20Text%20Files%20with%20the%20TextFieldParser%20Object%20\(Visual%20Basic\).md)   
 [TextFieldParser 对象](../Topic/TextFieldParser%20Object.md)