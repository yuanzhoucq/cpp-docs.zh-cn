...
必须在 fixed 或者 using 语句声明中提供初始值设定项  
 ...
 必须声明并初始化在 [fixed 语句](../Topic/fixed%20Statement%20\(C%23%20Reference\).md)中的变量。 有关详细信息，请参阅[不安全代码和指针](../Topic/Unsafe%20Code%20and%20Pointers%20\(C%23%20Programming%20Guide\).md)。  
...
```  
 ...
 以下的示例也生成 CS0210，因为 [using 语句](../Topic/using%20Statement%20\(C%23%20Reference\).md) 没有初始值设定项。  
