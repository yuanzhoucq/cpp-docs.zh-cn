...
接口中的事件不能具有 add 或 remove 访问器  
 ...
 不能在[接口](../Topic/interface%20\(C%23%20Reference\).md)中定义某一事件的访问器函数。 有关详细信息，请参阅 [事件](../Topic/Events%20\(C%23%20Programming%20Guide\).md) 和 [接口](../Topic/Interfaces%20\(C%23%20Programming%20Guide\).md)。  
