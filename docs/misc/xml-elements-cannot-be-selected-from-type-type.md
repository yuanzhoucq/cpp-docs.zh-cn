...
针对不属于类型 <xref:System.Xml.Linq.XElement>、<xref:System.Xml.Linq.XDocument> 或 `IEnumerable(Of XElement)` 的对象引用了一个 XML 子元素。 有关详细信息，请参阅[XML 子轴属性](../Topic/XML%20Child%20Axis%20Property%20\(Visual%20Basic\).md)。  
...
## 请参阅  
 [XML 子轴属性](../Topic/XML%20Child%20Axis%20Property%20\(Visual%20Basic\).md)   
