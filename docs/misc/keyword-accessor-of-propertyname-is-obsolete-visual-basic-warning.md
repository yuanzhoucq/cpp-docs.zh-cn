...
语句尝试读取或写入属性，为此属性，其相应的过程已标记为 <xref:System.ObsoleteAttribute> 特性，指令会将其视为警告。  
 ...
 默认情况下，此消息是一个警告，因为 <xref:System.ObsoleteAttribute> 的 <xref:System.ObsoleteAttribute.IsError%2A> 属性为 `False`。 有关隐藏警告或将警告视为错误的详细信息，请参见 [在 Visual Basic 中配置警告](../Topic/Configuring%20Warnings%20in%20Visual%20Basic.md)。  
