...
泛型类型标记为 `<CLSCompliant(True)>`但其类型形参之一的约束指定一个标记为 `<CLSCompliant(False)>` 的类型，没有标记或不合格的类型，因为它是不符合要求的类型。  
 ...
 以下 [!INCLUDE[vbprvb](../Token/vbprvb_md.md)] 数据类型不符合 CLS：  
  
-   [SByte 数据类型](../Topic/SByte%20Data%20Type%20\(Visual%20Basic\).md)  
  
-   [UInteger 数据类型](../Topic/UInteger%20Data%20Type.md)  
  
-   [Ulong 数据类型](../Topic/ULong%20Data%20Type%20\(Visual%20Basic\).md)  
  
-   [Ushort 数据类型](../Topic/UShort%20Data%20Type%20\(Visual%20Basic\).md)  
