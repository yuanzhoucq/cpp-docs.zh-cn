...
无法对属性或索引器“name”赋值 \-\- 它是只读的  
 ...
 [readonly](../Topic/readonly%20\(C%23%20Reference\).md) 字段只能采用构造函数或声明中的一个赋值。 有关详细信息，请参阅[构造函数](../Topic/Constructors%20\(C%23%20Programming%20Guide\).md)。  
  
 如果 `readonly` 字段为 [static](../Topic/static%20\(C%23%20Reference\).md) 并且构造函数没有被标记为 `static`，也会导致 CS0191。  
