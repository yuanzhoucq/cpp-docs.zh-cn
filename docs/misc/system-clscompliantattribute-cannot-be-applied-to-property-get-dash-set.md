...
属性定义将 <xref:System.CLSCompliantAttribute> 特性应用于其 `Get` 或 `Set` 语句。  
 ...
 对于符合 [语言独立性和与语言无关的组件](../Topic/Language%20Independence%20and%20Language-Independent%20Components.md) \(CLS\) 的属性，整个属性必须标记为 `<CLSCompliant(True)>`。 你必须将 <xref:System.CLSCompliantAttribute> 应用到 [Property 语句](../Topic/Property%20Statement.md) 中，而不是应用到 `Get` 或 `Set` 语句。  
