...
“member”不能是外部的，也无法声明主体  
 ...
 当类成员标记为[外部](../Topic/extern%20\(C%23%20Reference\).md)，这意味着该成员的定义位于另一个文件中。 因此，标记为 **外部**的类成员不能在类中定义。 删除 `extern` 关键字或删除该定义。 有关详细信息，请参阅[方法](../Topic/Methods%20\(C%23%20Programming%20Guide\).md)。  
