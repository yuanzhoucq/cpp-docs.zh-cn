...
---
# “Handles”在运算符声明上无效
[Operator 语句](../Topic/Operator%20Statement.md) 指定 [Handles](../Topic/Handles%20Clause%20\(Visual%20Basic\).md) 关键字。  
 ...
 `Operator` 过程需要 `Public` 和 `Shared` 两个关键字，而转换运算符需要 `Widening` 或 `Narrowing` 关键字。 有关更多信息，请参见[运算符过程](../Topic/Operator%20Procedures%20\(Visual%20Basic\).md)。  
