...
添加的模块必须用 CLSCompliant 特性标记才能与程序集匹配  
 ...
 已将使用 [\/target: module](../Topic/-target:module%20\(C%23%20Compiler%20Options\).md) 编译器选项编译的模块添加到使用 [\/addmodule](../Topic/-addmodule%20\(C%23%20Compiler%20Options\).md) 的编译中。 但是，该模块对公共语言规范 \(CLS\) 的遵从性与当前编译的 CLS 状态不一致。  
