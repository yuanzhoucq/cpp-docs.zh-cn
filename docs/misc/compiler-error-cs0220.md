...
在 checked 模式下，运算在编译时溢出  
 ...
 [已检查](../Topic/checked%20\(C%23%20Reference\).md)（这是默认操作）检测到一个操作，结果导致数据丢失。 更正赋值的输入或使用 [未检查](../Topic/unchecked%20\(C%23%20Reference\).md)来解决此错误。 有关更多信息，请参见[Checked 和 Unchecked](../Topic/Checked%20and%20Unchecked%20\(C%23%20Reference\).md)。  
