...
ms.locfileid: "87214264"
---
# <a name="interior_ptr-ccli"></a>interior_ptr (C++/CLI)

内部指针** 声明指向引用类型内部，但不指向对象本身的指针。 内部指针可以指向引用句柄、值类型、装箱类型句柄、托管类型的成员或托管数组的元素。
...
**interior_ptr**具有到的隐式转换 **`bool`** ，这允许其在条件语句中使用。
...
- [如何：声明和使用内部指针和托管数组（c + +/CLI）](how-to-declare-and-use-interior-pointers-and-managed-arrays-cpp-cli.md)
