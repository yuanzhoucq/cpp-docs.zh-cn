...
还可以根据[泛型类型参数的约束 (C++/CLI)](constraints-on-generic-type-parameters-cpp-cli.md)，对可用于类型参数的类型声明泛型类。 在下面的示例中，任何用于 `ItemType` 的类型都必须实现 `IItem` 接口。 例如，尝试使用未实现 `IItem` 的 int 会产生编译时错误，因为类型参数不符合约束条件。
...
下面的示例声明包含一个字段 `myField` 的泛型结构 `MyGenStruct`，并将不同类型（int、double、`String^`）的值分配给此字段。
