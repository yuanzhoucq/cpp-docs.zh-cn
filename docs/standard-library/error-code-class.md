...
`error_code`如果`error_code`对象与右传递的对象不相等，则**为 true** ; 否则为 false。
...
仅当[值](#value)不等于零时，运算符才会返回一个可转换为**true**的值。 返回类型只能转换为**bool**，而不`void *`能转换为或其他已知的标量类型。
