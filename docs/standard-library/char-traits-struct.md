---
title: "char_traits 结构 | Microsoft 文档"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
### <a name="parameters"></a>参数  
 ...
 `dest_size`  
 `dest` 的大小。 如果 `char_type` 是 `char`，则此大小以字节为单位。 如果 `char_type` 是 `wchar_t`，则此大小以字为单位。  
...
### <a name="parameters"></a>参数  
 ...
 `dest_size`  
 `dest` 的大小。 如果 `char_type` 是 `char`，则以字节为单位。 如果 `char_type` 是 `wchar_t`，则以字为单位。  
