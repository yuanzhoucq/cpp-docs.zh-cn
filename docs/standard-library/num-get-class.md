...
`InputIterator` 数值获取函数从中读取其输入的迭代器的类型。
...
`_Iosbase` [Ios_base](../standard-library/ios-base-class.md)转换使用其标志。

`_State` 到哪些 failbit 状态 (请参阅[ios_base:: iostate](../standard-library/ios-base-class.md#iostate)) 添加在失败时。
