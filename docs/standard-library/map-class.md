...
若要访问元素的键值，请使用 `cIter`**第一个** -> ，它等效于（\* `cIter`）。 **first**。
...
`const_reverse_iterator` 类型无法修改元素的值，它用于反向循环访问 map。
...
如果 **在排序顺序中先于且不等于**，则该函数会返回 `left`true`right`。
...
有关如何声明和使用 [ 的示例，请参阅 ](#value_type)value_type`mapped_type` 的示例。
...
*right*\
要复制到 [ 中的 ](../standard-library/map-class.md)map`map`。
