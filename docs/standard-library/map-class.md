...
|[迭代器](#iterator)|可读取或修改映射中任何元素的双向迭代器的 typedef。|
...
|[指针](#pointer)|指向映射中的**const**元素的指针的 typedef。|
...
若要访问元素的映射基准值，请使用 `cIter` -> **second**，其等同于 (\* `cIter`)。 **second** 相同。
...
映射所定义的 `const_reverse_iterator` 是[value_type](#value_type)`pair<const Key, Type>` 类型的元素的元素，其第一个成员是元素的键，第二个成员是元素所持有的映射基准。
...
映射所定义的迭代器指向作为[value_type](#value_type)的对象的元素，它的类型为 `pair<const Key, Type>`，其第一个成员是元素的键，第二个成员是元素保存的映射基准。
...
*Comp* \
用于对映射中元素排序的类型 `const Traits` 的比较函数，默认为 `hash_compare`。
...
映射所定义的 `reverse_iterator` 是[value_type](#value_type)`pair<const Key, Type>` 类型的元素的元素，其第一个成员是元素的键，第二个成员是元素所持有的映射基准。
...
对于 map *m*，如果两个元素*e1*（*版 k1*， *d1*）和*e2*（*k2*， *d2*）都是类型 `value_type` 的对象，其中*版 k1*和*版 k1*是其 0 类型的键， *d1*和*d2*是3 类型的数据，则 4 等效于 5。 存储对象会定义成员函数
