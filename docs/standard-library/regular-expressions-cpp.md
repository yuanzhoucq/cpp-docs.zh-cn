...
ms.locfileid: "81368517"
---
# <a name="regular-expressions-c"></a>正则表达式 (C++)

C++标准库支持多个正则表达式语法。 本主题讨论使用正则表达式时可用的语法变体。

## <a name="regular-expression-grammar"></a><a name="regexgrammar"></a>正则表达式语法

要使用的正则表达式语法通过使用其中一个`std::regex_constants::syntax_option_type`枚举值来指定。 这些正则表达式语法在 std：：regex_constants 中定义：

- `ECMAScript`：这是最接近 JavaScript 和 .NET 语言使用的语法。
- `basic`：POSIX 基本正则表达式或 BRE。
- `extended`：POSIX 扩展正则表达式或 ERE。
- `awk`：这是`extended`，但它具有非打印字符的其他转义。
- `grep`：这是`basic`，但它也允许换行符分隔。
- `egrep`：这是`extended`，但它也允许换行符分隔。

默认情况下，如果未指定语法，`ECMAScript`则假定为。 只能指定一个语法。

除了语法之外，还可以应用几个标志：

- `icase`：匹配时忽略大小写。
- `nosubs`：忽略标记的匹配项（即括号中的表达式）;不存储替换。
- `optimize`：使匹配更快，并可能牺牲更大的施工时间。
- `collate`：使用区域设置敏感的排序规则序列（例如，窗体的范围"[a-z]）。"

零个或多个标志可以与语法组合，以指定正则表达式引擎行为。 如果仅指定标志，`ECMAScript`则假定为语法。
...
- “[`expr`]”形式的*括号表达式*，可匹配目标序列中已包含在表达式 `expr` 定义集内的字符或排序规则元素；或采用“[^`expr`]”形式，可匹配目标序列中未包含在表达式 `expr` 定义集内的字符或排序规则元素。
  ...
  - 窗体`ch1`-`ch2`"" 的*字符范围*。 将闭值域 [`ch1`, `ch2`] 所表示的字符添加到 `expr` 定义的集合。
...
“（*子表达式*）”形式的*捕获组*，或者采用 `basic` 和  中的“\\（*子表达式`grep`* \\）”形式，可匹配目标序列中与分隔符之间的模式相符的字符序列。

- 窗体\\`k`""的*标识转义*，与目标序列中的字符`k`匹配。
...
在`ECMAScript``basic`中`grep`，和 ， 元素也可以是窗体""\\`dd`*的反向引用*，`dd`其中表示与目标序列中与 N 捕获*组*匹配的字符序列匹配的十进制值 N。 例如，“(a)\1”可匹配目标序列“aa”，因为第一个（而且是唯一一个）捕获组可匹配初始序列“a”，然后 \1 可匹配最终序列“a”。

在 `ECMAScript` 中，元素可以是下列项之一：

- 窗体"（？：*子表达式*）"*的非捕获组*。 可匹配目标序列中与分隔符之间的模式相符的字符序列。
...
- 窗体"""\a"、"\b"、"\f"、"\n"、"\r"、"\t"\\\\或"\v"*的文件格式转义*。 它们分别匹配目标序列中的反斜杠、警报符、退格符、换页符、换行符、回车符、水平制表符和垂直制表符。

- 形式""\\`ooo`的*八进制转义序列*。 可匹配目标序列中由一个、两个或三个八进制数字 `ooo` 的值表示的字符。

### <a name="repetition"></a>重复

除*肯定断言*、*否定断言*或*定位点*之外的任何元素都可以后接一个重复计数。 最常见的重复计数采用“{`min`,`max`}”形式，或者在 `basic` 和 `grep` 中采用“\\{`min`,`max`\\}”形式。 后跟这种重复计数形式的元素可至少匹配 `min` 次连续出现的与该元素相符的序列，最多不超过 `max` 次。 例如，"a"{2,3}匹配目标序列"aa"和目标序列"aaa"，但不匹配目标序列"a"或目标序列"aaa"。
...
- {2}a" 匹配目标序列"aa"，但不匹配目标序列"a"或目标序列"aa"。

- a"{2,}匹配目标序列"aa"，目标序列"aaa"等，但与目标序列"a"不匹配。

- "匹配\*目标序列"，"目标序列"a"，目标序列"a"，目标序列"aa"，等等。
...
- "a？ 匹配目标序列""和目标序列"a"，但不匹配目标序列"aa"。

- “a+”可匹配目标序列“a”、目标序列“aa”，以此类推，但不匹配目标序列“”。

在`ECMAScript`中，所有形式的重复计数可以跟后面跟字符"？"，它指定*一个非贪婪的重复*。

### <a name="concatenation"></a>串联

无论是否后跟*重复计数*，正则表达式都可以串联成较长的正则表达式。 由此产生的表达式可匹配由分别与各个元素相符的序列串联而成的目标序列。 例如，"a{2,3}b"匹配目标序列"aab"和目标序列"aaab"，但与目标序列"ab"或目标序列"aaab"不匹配。
...
## <a name="grammar-summary"></a><a name="grammarsummary"></a>语法摘要
...
## <a name="semantic-details"></a><a name="semanticdetails"></a>语义详细信息
...
- \*a" 匹配目标序列"aaa"，但与目标序列"a"\*不匹配。

- a" 与目标序列"aaa"不匹配，但与目标序列"a"\*匹配。\\\*
...
|`extended`, `egrep`|['''，""，""，""，"'"，"'"，""，"$"，"\"，"？"，"&#124;"|\\\*|
|`awk`|`extended` 加 { '"', '/' }|
|`ECMAScript`|除可能构成标识符的字符以外的所有字符。 通常，这包括字母、数字、"$"和\_单码转义序列。 有关详细信息，请参阅 ECMAScript 语言规范。|
...
否定断言可匹配除其内容外的任意项。 它不占用目标序列中的任何字符。 例如，"（！aa）（a）"\*匹配目标序列"a"，并将捕获组 1 与子序列"a"关联。 它不匹配目标序列“aa”或目标序列“aaa”。
...
非捕获组用于将其内容标记为正则表达式语法中的一个单元，但不会对目标文本设置标签。 例如，"（a）（？？：b）（c）"\*将目标文本"abbc"匹配，并将捕获组 1 与子序列"a"关联，捕获组 2 与子序列"c"。

### <a name="non-greedy-repetition"></a>非贪婪重复

非贪婪重复将占用与模式匹配的目标序列的最短子序列。 贪婪重复将占用最长子序列。 例如，"（a+）（a\*b）"与目标序列"aaab"匹配。 当使用非贪婪重复时，它会将捕获组 1 与目标序列开头的子序列“a”相关联，将捕获组 2 与目标序列末尾的子序列“aab”相关联。 当使用贪婪匹配时，它会将捕获组 1 与子序列“aaa”相关联，将捕获组 2 与子序列“b”相关联。
...
- '\*' 在所有情况下都有特殊的含义，除非它是正则表达式中的第一个字符或正则表达式中初始"*"之后的第一个字符，或者它是捕获组中捕获组的第一个字符或初始"*"之后的第一个字符。
...
- "[aaa）（a）]"\*匹配目标序列"aaa"，并将捕获组 1 与子序列"aaaa"关联。

- "（aa）（a\*）"匹配目标序列"aaa"，并将捕获组 1 与目标序列开头的子序列"aa"关联，捕获组 2 与目标序列末尾的子序列"aa"。
...
## <a name="matching-and-searching"></a><a name="matchingandsearching"></a>匹配和搜索
...
## <a name="format-flags"></a><a name="formatflags"></a>格式标志
...
|"$$"||"$"|
||"&"\\|"&"|
|"$"（\`美元符号后跟回报价）*与正则表达式匹配的子序列前面的字符序列 （`[match.prefix().first, match.prefix().second)`）|
|“$'”（美元符号后跟正撇号）||与正则表达式相匹配的子序列之后的字符序列 (`[match.suffix().first, match.suffix().second)`)|
|“$n”|“\n”|与位置`n`捕获组的匹配的字符序列，其中`n`的数字介于 0 和 9 之间`[match[n].first, match[n].second)`（）|
||"\\\n"|“\n”|
|“$nn”||与位置`nn`捕获组匹配的字符序列，其中`nn`的数字介于 10 和 99 之间`[match[nn].first, match[nn].second)`（）|
