...
要使用的正则表达式语法是通过使用其中一个`std::regex_constants::syntax_option_type`枚举值指定的。 以下正则表达式语法在 std:: regex_constants 中定义:

- `ECMAScript`：这与 JavaScript 和 .NET 语言所使用的语法最接近。
- `basic`：POSIX 基本正则表达式或 BRE。
- `extended`：POSIX 扩展正则表达式或 ERE。
- `awk`：这是`extended`, 但对于非打印字符, 它还有其他转义符。
- `grep`：这是`basic`, 但它也允许用换行符 ("\n") 来分隔分支结构。
- `egrep`：这是`extended`, 但它还允许换行符分隔分支结构。

默认情况下, 如果未指定语法, `ECMAScript`则采用。 只能指定一种语法。

除语法外, 还可以应用多个标志:
- `icase`：匹配时忽略大小写。
- `nosubs`：忽略标记的匹配项 (即括号中的表达式);不存储任何替换。
- `optimize`：以更好的构造时间为代价, 使匹配速度更快。
- `collate`：使用区分区域设置的排序规则序列 (例如, "[a-z]" 形式的范围)。

可以将零个或多个标志与语法组合在一起, 以指定正则表达式引擎的行为。 如果仅指定标志, `ECMAScript`则将假定为语法。
...
除*肯定断言*、*否定断言*或*定位点*之外的任何元素都可以后接一个重复计数。 最常见的重复计数采用“{`min`,`max`}”形式，或者在 `basic` 和 `grep` 中采用“\\{`min`,`max`\\}”形式。 后跟这种重复计数形式的元素可至少匹配 `min` 次连续出现的与该元素相符的序列，最多不超过 `max` 次。 例如, "a{2,3}" 匹配目标序列 "aa" 和目标序列 "aaa", 但不匹配目标序列 "a" 或目标序列 "aaaa"。
...
- "a{2}" 匹配目标序列 "aa", 但不匹配目标序列 "a" 或目标序列 "aaa"。

- "a{2,}" 匹配目标序列 "aa"、目标序列 "aaa", 以此类推, 但不匹配目标序列 "a"。

- "a\*" 匹配目标序列 ""、目标序列 "a"、目标序列 "aa", 以此类推。
...
在`ECMAScript`中, 所有形式的重复计数都可以后跟字符 "？", 这会指定*非贪婪重复*。

### <a name="concatenation"></a>串联

无论是否后跟*重复计数*，正则表达式都可以串联成较长的正则表达式。 由此产生的表达式可匹配由分别与各个元素相符的序列串联而成的目标序列。 例如, "a{2,3}b" 匹配目标序列 "aab" 和目标序列 "aaab", 但不匹配目标序列 "ab" 或目标序列 "aaaab"。
...
- "a\*" 匹配目标序列 "aaa", 但不匹配目标序列 "a\*"。

- "a\\\*" 不匹配目标序列 "aaa", 但匹配目标序列 "a\*"。
...
否定断言可匹配除其内容外的任意项。 它不占用目标序列中的任何字符。 例如, "(! aa) (a\*)" 匹配目标序列 "a", 并将捕获组1与子序列 "a" 相关联。 它不匹配目标序列“aa”或目标序列“aaa”。
...
非捕获组用于将其内容标记为正则表达式语法中的一个单元，但不会对目标文本设置标签。 例如, "(a) (？: b)\*(c)" 匹配目标文本 "abbc", 并将捕获组1与子序列 "a" 相关联, 并将捕获组2与子序列 "c" 相关联。

### <a name="non-greedy-repetition"></a>非贪婪重复

非贪婪重复将占用与模式匹配的目标序列的最短子序列。 贪婪重复将占用最长子序列。 例如, "(a +) (a\*b)" 匹配目标序列 "aaab"。 当使用非贪婪重复时，它会将捕获组 1 与目标序列开头的子序列“a”相关联，将捕获组 2 与目标序列末尾的子序列“aab”相关联。 当使用贪婪匹配时，它会将捕获组 1 与子序列“aaa”相关联，将捕获组 2 与子序列“b”相关联。
...
- "\*" 在所有情况下均具有特殊含义, 只不过它是正则表达式中的第一个字符, 或在正则表达式中第一个 "^" 之后的第一个字符, 或者是捕获组的第一个字符或跟随捕获组中的初始 "^"。
...
- "(= aa) (a\*)" 匹配目标序列 "aaaa", 并将捕获组1与子序列 "aaaa" 相关联。

- "(aa) (a\*)" 匹配目标序列 "aaaa", 并将捕获组1与目标序列开头的子序列 "aa" 相关联, 并将捕获组2与目标序列末尾的子序列 "aa" 相关联。
...
|“$n”|“\n”|在位置`n`与捕获组相匹配的字符序列, 其中`n`是一个介于0到9之间的`[match[n].first, match[n].second)`数字 ()|
||"\\\n"|“\n”|
|“$nn”||在位置`nn`与捕获组相匹配的字符序列, 其中`nn`是一个介于10和 99 (`[match[nn].first, match[nn].second)`) 之间的数字|
