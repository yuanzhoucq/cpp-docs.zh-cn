...
描述一个输入迭代器对象。 它从输入流中提取 `Type` 类的对象，并通过其存储的、指向 `pointer``basic_istream`< , `CharType`> 的 `Traits` 类型对象访问此输入流。
...
第一个构造函数通过空指针初始化输入流指针，并创建流末尾迭代器。 第二个构造函数使用 *& _Istr*初始化输入流指针，然后尝试提取并存储 `Type`类型的对象。
...
类型是 `basic_istream`\< **CharType**、**特征**> 的同义词。

### <a name="example"></a>示例

有关如何声明和使用 [ 的示例，请参阅 ](#istream_iterator)istream_iterator`istream_type`。
...
第一个成员运算符返回对从输入流提取的 `Type` 类型的递增对象的引用，第二个成员函数返回对象的副本。
