...
|||
|-|-|
|[unordered_set：：操作员*](#op_eq)|复制哈希表。|

## <a name="remarks"></a>备注

对象通过调用两个存储对象（类型[unordered_set：：key_equal](#key_equal)的比较函数对象和类型[unordered_set：：哈希器](#hasher)的哈希函数对象）来命令它控制的顺序。 通过调用成员函数[unordered_set：：key_eq](#key_eq)`()`访问第一个存储的对象。通过调用成员函数[unordered_set：hash_function](#hash)`()`访问第二个存储的对象。 具体而言，对于所有 `X` 类型的值 `Y` 和 `Key`，`key_eq()(X, Y)` 调用将仅在两个参数值拥有等效顺序时返回 true；`hash_function()(keyval)` 调用将生成 `size_t` 类型的值的分布。 与类模板[unordered_multiset类](../standard-library/unordered-multiset-class.md)不同，类型`unordered_set`对象可确保`key_eq()(X, Y)`受控序列的任何两个元素始终为 false。 （键是唯一的。）

此对象还存储最大加载因子，用于指定每个存储桶的元素的最大所需平均数量。 如果插入元素会导致[unordered_set：：load_factor](#load_factor)`()`超过最大负载系数，则容器会增加存储桶的数量并根据需要重建哈希表。

受控序列中元素的实际顺序取决于哈希函数、比较函数、插入顺序、最大加载因子和存储桶的当前数量。 通常无法预测受控序列中的元素顺序。 但是，可以始终确保具有等效顺序的任何元素子集在受控序列中相邻。

对象通过[unordered_set：：allocator_type](#allocator_type)类型的存储分配器对象为其控制的顺序分配和释放存储。 此类分配器对象必须具有与类型`allocator`对象相同的外部接口。 请注意，分配容器对象时不会复制存储的分配器对象。
...
成员函数调用[unordered_set：擦除](#erase)`(`[unordered_set：开始](#begin)`(),`[unordered_set：结束](#end)`())`。
...
## <a name="count"></a><a name="count"></a>计数
...
成员函数返回按[unordered_set：：equal_range](#equal_range)`(keyval)`分隔的范围中的元素数。
...
如果`pair`已进行插入，则**其 bool**组件返回 true;`unordered_set`如果已包含其键在排序中具有等效值的元素，并且其迭代器组件返回插入新元素或元素已位于的地址的地址，则其 bool 组件返回 true。
...
有关代码示例，请参阅[设置：emplace](../standard-library/set-class.md#emplace)。
...
## <a name="hasher"></a><a name="hasher"></a>哈瑟
...
*瓦尔蒂*\
模板参数，用于指定unordered_set可用于构造[value_type](../standard-library/map-class.md#value_type)元素的参数类型，并将完全转发*Val*作为参数。
...
*IList*\
要[从中](../standard-library/initializer-list.md)复制元素initializer_list。

### <a name="return-value"></a>返回值

单元素成员函数 （1） 和 （2） 返回一[对](../standard-library/pair-structure.md)，如果进行了插入，其**bool**分量为 true;如果unordered_set已包含其键在排序中具有等效值的元素，则为 false。 如果**bool**组件为 true，则返回值对的迭代器组件指向新插入的元素;如果**bool**组件为 false，则指向现有元素。
...
范围成员函数 （5） 将元素值序列插入到一个unordered_set中，该unordered_set对应于范围`[First, Last)`中的迭代器寻址的每个元素;因此，不会插入*Last。* 容器成员函数 `end()` 是指容器中最后一个元素之后的位置，例如，`s.insert(v.begin(), v.end());` 语句尝试将 `v` 的所有元素插入到 `s` 中。 只插入在该范围中具有唯一值的元素；忽略副本。 若要观察拒绝了哪些元素，请使用单个元素版本的 `insert`。

初始化程序列表成员函数 （6） 使用[initializer_list](../standard-library/initializer-list.md)将元素复制到unordered_set。
...
有关代码示例，请参阅[设置：插入](../standard-library/set-class.md#insert)。

## <a name="iterator"></a><a name="iterator"></a>迭 代

提供可读取unordered_set中元素的恒定[转发迭代器](../standard-library/forward-iterator-tag-struct.md)的类型。
...
有关如何声明和使用**迭代器**的示例，请参阅[示例。](../standard-library/set-class.md#begin)
...
成员函数返回`(float)`[unordered_set：大小](#size)`() / (float)`[unordered_set：bucket_count，](#bucket_count)`()`即每个存储桶的平均元素数。
...
*对*\
正在复制到 的`unordered_set`[unordered_set。](../standard-library/unordered-set-class.md)

### <a name="remarks"></a>备注

在上用 中的任何现有元素后`unordered_set`，`operator=`复制 或将*右侧*的内容移动到 中`unordered_set`。
...
## <a name="rehash"></a><a name="rehash"></a>重复
...
成员函数交换*和*之间的`*this`受控序列。 如果[unordered_set：：get_allocator](#get_allocator)`() == right.get_allocator()`，它在恒定时间内这样做，则仅由于复制类型`Tr`存储的特性对象而引发异常，并且不会使指定两个受控序列中元素的引用、指针或迭代器无效。 否则，它所执行的元素分配和构造函数调用数量会与两个受控序列中的元素数量成正比。
...
第一个构造函数指定由*右*控制的序列的副本。 第二个构造函数指定空的受控序列。 第三个构造函数*通过向右移动*来指定序列的副本 第四个到第八个构造函数使用initializer_list指定要复制的元素。 第九个构造函数插入元素值 `[first, last)` 的序列。

所有构造函数还初始化若干存储的值。 对于复制构造函数，值从*右*获取。 否则：

最小存储桶数是参数*bucket_count（* 如果存在）;否则，它是此处描述为实现定义的值`N0`的默认值。
