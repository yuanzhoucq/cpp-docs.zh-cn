...
|-|-|  
|类型定义|描述|  
|[unordered_set::allocator_type](#unordered_set__allocator_type)|用于管理存储的分配器的类型。|  
|[unordered_set::const_iterator](#unordered_set__const_iterator)|受控序列的常量迭代器的类型。|  
|[unordered_set::const_local_iterator](#unordered_set__const_local_iterator)|受控序列的常量存储桶迭代器的类型。|  
|[unordered_set::const_pointer](#unordered_set__const_pointer)|元素的常量指针的类型。|  
|[unordered_set::const_reference](#unordered_set__const_reference)|元素的常量引用的类型。|  
|[unordered_set::difference_type](#unordered_set__difference_type)|两个元素间的带符号距离的类型。|  
|[unordered_set::hasher](#unordered_set__hasher)|哈希函数的类型。|  
|[unordered_set::iterator](#unordered_set__iterator)|受控序列的迭代器的类型。|  
|[unordered_set::key_equal](#unordered_set__key_equal)|比较函数的类型。|  
|[unordered_set::key_type](#unordered_set__key_type)|排序键的类型。|  
|[unordered_set::local_iterator](#unordered_set__local_iterator)|受控序列的存储桶迭代器的类型。|  
|[unordered_set::pointer](#unordered_set__pointer)|指向元素的指针的类型。|  
|[unordered_set::reference](#unordered_set__reference)|元素的引用的类型。|  
|[unordered_set::size_type](#unordered_set__size_type)|两个元素间的无符号距离的类型。|  
|[unordered_set::value_type](#unordered_set__value_type)|元素的类型。|  
  
|||  
|-|-|  
|成员函数|描述|  
|[unordered_set::begin](#unordered_set__begin)|指定受控序列的开头。|  
|[unordered_set::bucket](#unordered_set__bucket)|获取键值的存储桶编号。|  
|[unordered_set::bucket_count](#unordered_set__bucket_count)|获取存储桶数。|  
|[unordered_set::bucket_size](#unordered_set__bucket_size)|获取存储桶的大小。|  
|[unordered_set::cbegin](#unordered_set__cbegin)|指定受控序列的开头。|  
|[unordered_set::cend](#unordered_set__cend)|指定受控序列的末尾。|  
|[unordered_set::clear](#unordered_set__clear)|删除所有元素。|  
|[unordered_set::count](#unordered_set__count)|查找与指定键匹配的元素数。|  
|[unordered_set::emplace](#unordered_set__emplace)|添加就地构造的元素。|  
|[unordered_set::emplace_hint](#unordered_set__emplace_hint)|添加就地构造的元素，附带提示。|  
|[unordered_set::empty](#unordered_set__empty)|测试元素是否存在。|  
|[unordered_set::end](#unordered_set__end)|指定受控序列的末尾。|  
|[unordered_set::equal_range](#unordered_set__equal_range)|查找与指定键匹配的范围。|  
|[unordered_set::erase](#unordered_set__erase)|移除指定位置处的元素。|  
|[unordered_set::find](#unordered_set__find)|查找与指定键匹配的元素。|  
|[unordered_set::get_allocator](#unordered_set__get_allocator)|获取存储的分配器对象。|  
|[unordered_set::hash_function](#unordered_set__hash_function)|获取存储的哈希函数对象。|  
|[unordered_set::insert](#unordered_set__insert)|添加元素。|  
|[unordered_set::key_eq](#unordered_set__key_eq)|获取存储的比较函数对象。|  
|[unordered_set::load_factor](#unordered_set__load_factor)|对每个存储桶的平均元素数进行计数。|  
|[unordered_set::max_bucket_count](#unordered_set__max_bucket_count)|获取最大的存储桶数。|  
|[unordered_set::max_load_factor](#unordered_set__max_load_factor)|获取或设置每个存储桶的最多元素数。|  
|[unordered_set::max_size](#unordered_set__max_size)|获取受控序列的最大大小。|  
|[unordered_set::rehash](#unordered_set__rehash)|重新生成哈希表。|  
|[unordered_set::size](#unordered_set__size)|对元素数进行计数。|  
|[unordered_set::swap](#unordered_set__swap)|交换两个容器的内容。|  
|[unordered_set::unordered_set](#unordered_set__unordered_set)|构造容器对象。|  
  
|||  
|-|-|  
|运算符|描述|  
|[unordered_set::operator=](#unordered_set__operator_eq)|复制哈希表。|  
  
## <a name="remarks"></a>备注  
 对象通过调用两个存储对象，即一个 [unordered_set::key_equal](#unordered_set__key_equal) 类型的比较函数对象和一个 [unordered_set::hasher](#unordered_set__hasher) 类型的哈希函数对象，对它控制的序列进行排序。 可以通过调用成员函数 [unordered_set::key_eq](#unordered_set__key_eq)`()` 访问第一个存储对象；通过调用成员函数 [unordered_set::hash_function](#unordered_set__hash_function)`()` 访问第二个存储对象。 具体而言，对于所有 `Key` 类型的值 `X` 和 `Y`，`key_eq()(X, Y)` 调用将仅在两个参数值拥有等效顺序时返回 true；`hash_function()(keyval)` 调用将生成 `size_t` 类型的值的分布。 与模板类 [unordered_multiset 类](../standard-library/unordered-multiset-class.md)不同，模板类 `unordered_set` 的对象可确保 `key_eq()(X, Y)` 对于受控序列的任意两个元素始终为 false。 （键是唯一的。）  
  
 此对象还存储最大加载因子，用于指定每个存储桶的元素的最大所需平均数量。 如果插入元素导致 [unordered_set::load_factor](#unordered_set__load_factor)`()` 超出最大加载因子，容器将增加存储桶的数量并根据需要重新生成哈希表。  
  
 受控序列中元素的实际顺序取决于哈希函数、比较函数、插入顺序、最大加载因子和存储桶的当前数量。 通常无法预测受控序列中的元素顺序。 但是，可以始终确保具有等效顺序的任何元素子集在受控序列中相邻。  
  
 对象通过 [unordered_set::allocator_type](#unordered_set__allocator_type) 类型的存储分配器对象为其控制的序列分配并释放存储。 此分配器对象必须与 `allocator` 模板类的对象的外部接口相同。 请注意，分配容器对象时不会复制存储的分配器对象。  
...
### <a name="remarks"></a>备注  
 前两个编号函数返回向前迭代器，指向序列的第一个元素（或紧邻空序列后的位置）。 最后两个成员函数返回一个向前迭代器，它指向存储桶 `nbucket` 的第一个元素（或刚超出空存储桶末尾）。  
...
##  <a name="a-nameunorderedsetcbegina--unorderedsetcbegin"></a><a name="unordered_set__cbegin"></a>  unordered_set::cbegin  
 返回一个 `const` 迭代器，此迭代器用于发现范围中的第一个元素。  
...
### <a name="return-value"></a>返回值  
 `const` 向前访问迭代器，它指向范围的第一个元素，或刚超出空范围末尾（对于空范围为 `cbegin() == cend()`）。  
  
### <a name="remarks"></a>备注  
 如果使用 `cbegin` 的返回值，则不能修改范围中的元素。  
  
 可以使用此成员函数替代 `begin()` 成员函数，保证返回值为 `const_iterator`。 它一般与 [auto](../cpp/auto-cpp.md) 类型推导关键字联合使用，如下例所示。 在该示例中，将 `Container` 视为支持 `begin()` 和 `cbegin()` 的任何类型的可修改（非 `const`）的容器。  
...
### <a name="return-value"></a>返回值  
 指向刚超出范围末尾的 `const`向前访问迭代器。  
  
### <a name="remarks"></a>备注  
 `cend` 用于测试迭代器是否超过了其范围的末尾。  
  
 可以使用此成员函数替代 `end()` 成员函数，保证返回值为 `const_iterator`。 它一般与 [auto](../cpp/auto-cpp.md) 类型推导关键字联合使用，如下例所示。 在该示例中，将 `Container` 视为支持 `end()` 和 `cend()` 的任何类型的可修改（非 `const`）的容器。  
...
### <a name="remarks"></a>备注  
 此成员函数调用 [unordered_set::erase](#unordered_set__erase)`(` [unordered_set::begin](#unordered_set__begin)`(),` [unordered_set::end](#unordered_set__end)`())`。  
...
### <a name="remarks"></a>备注  
 该成员函数返回由 [unordered_set::equal_range](#unordered_set__equal_range)`(keyval)` 分隔的范围内的元素数量。  
...
### <a name="remarks"></a>备注  
 ...
 有关代码示例，请参阅 [set::emplace](../standard-library/set-class.md#set__emplace)。  
...
### <a name="remarks"></a>备注  
 前两个成员函数返回一个向前迭代器，它指向刚超出序列末尾的位置。 最后两个成员函数返回一个向前迭代器，它指向刚超出存储桶 `nbucket` 的末尾。  
...
### <a name="remarks"></a>备注  
 成员函数返回 [unordered_set::equal_range](#unordered_set__equal_range)`(keyval).first`。  
...
|`Where`|开始搜索正确插入点的位置。|  
|`ValTy`|一种模板参数，它指定 unordered_set 可用于构造 [value_type](../standard-library/map-class.md#map__value_type) 元素的参数类型，并将 `Val` 作为参数完美转发。|  
|`First`|要复制的第一个元素的位置。|  
...
### <a name="return-value"></a>返回值  
 单个元素成员函数 (1) 和 (2) 将返回 [pair](../standard-library/pair-structure.md)，如果完成插入，则其 `bool` 组件为 true；如果 unordered_set 已经包含一个其键在排序中具有等效值的元素，则为 false。 如果 `bool` 组件为 true，则返回值对的迭代器组件将指向新插入的元素；如果 `bool` 组件为 false，则指向现有元素。  
...
### <a name="remarks"></a>备注  
 ...
 若要访问单个元素成员函数返回的 `pair``pr` 的迭代器组件，请使用 `pr.first`；若要在返回的对中取消引用迭代器，请使用 `*pr.first`，这将向你提供一个元素。 若要访问 `bool` 组件，请使用 `pr.second`。 有关示例，请参阅本文后面的示例代码。  
  
 容器的 [value_type](../standard-library/map-class.md#map__value_type) 是属于该容器的 typedef；对于集，`unordered_set<V>::value_type` 是类型 `const V`。  
  
 范围成员函数 (5) 将元素值序列插入到 unordered_set 中，它对应于迭代器在范围 `[First, Last)` 中所处理的每个元素；因此，不会插入 `Last`。 容器成员函数 `end()` 是指容器中最后一个元素之后的位置，例如，`s.insert(v.begin(), v.end());` 语句尝试将 `v` 的所有元素插入到 `s` 中。 只插入在该范围中具有唯一值的元素；忽略副本。 若要观察拒绝了哪些元素，请使用单个元素版本的 `insert`。  
  
 初始化表达式列表成员函数 (6) 使用 [initializer_list](../standard-library/initializer-list.md) 将元素复制到 unordered_set 中。  
  
 有关就地构造的元素的插入（即不会执行复制或移动操作），请参阅 [set::emplace](../standard-library/set-class.md#set__emplace) 和 [set::emplace_hint](../standard-library/set-class.md#set__emplace_hint)。  
  
 有关代码示例，请参阅 [set::insert](../standard-library/set-class.md#set__insert)。  
...
### <a name="example"></a>示例  
  有关如何声明和使用**迭代器**的示例，请参阅 [begin](../standard-library/set-class.md#set__begin) 的示例。  
...
### <a name="remarks"></a>备注  
 成员函数返回每个存储桶的平均元素数 `(float)`[unordered_set::size](#unordered_set__size)`() / (float)`[unordered_set::bucket_count](#unordered_set__bucket_count)`()`。  
...
### <a name="remarks"></a>备注  
 第一个成员函数将返回存储的最大加载因子。 第二个成员函数将用 `factor` 替换存储的最大加载因子。  
...
|-|-|  
|参数|描述|  
|` right`|要复制到 `unordered_set` 中的 [unordered_set](../standard-library/unordered-set-class.md)。|  
...
### <a name="remarks"></a>备注  
 成员函数交换 `*this` 和 `right` 之间的受控序列。 如果为 [unordered_set::get_allocator](#unordered_set__get_allocator)`() == right.get_allocator()`，则它在固定时间内执行此操作，它仅在对类型 `Tr` 的存储特征对象进行复制时引发异常，并且不使任何引用、指针或指定两个受控序列中的元素的迭代器失效。 否则，它所执行的元素分配和构造函数调用数量会与两个受控序列中的元素数量成正比。  
