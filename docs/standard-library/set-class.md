...
`Traits` 提供可比较两个元素值作为排序键以确定其在集中的相对顺序的函数对象的类型。 此参数是可选自变量，默认值为二元谓词 **less***\<Key>*。
...
`Allocator` 表示存储的分配器对象封装有关集的分配和解除分配内存的详细信息的类型。 此参数是可选的而默认值是 **分配器 * * *\<密钥 >。*
