...
成员存储具有非规范化值的浮点类型 `denorm_present`，实际上是可变数量的指数位。
...
如果该类型具有类模板中定义的显式专用化，**则为 true** ; 否则为。如果不是，则**为 false** 。
...
返回该类型的最负有限值（通常为整数类型和浮点类型的 `-max()` `min()`）。 如果 `is_bounded` 为**true**，则返回值是有意义的。
...
如果该类型可以在舍入之前检测小值，**则为 true** ;如果不能，则**为 false** 。
