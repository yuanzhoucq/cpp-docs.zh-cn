...
所有名称以外的其他**运算符 delete**并**运算符 new** c + + 库标头中定义`std`命名空间，或嵌套在命名空间中`std`命名空间。 例如，将名称 `cin` 引用为 `std::cin`。 但是请注意，该宏名称不受命名空间限定，因此你始终需要写入不带命名空间限定符的 `__STD_COMPLEX`。

在某些转换环境，包括 c + + 库标头可能的外部名称提升中声明`std`至全局命名空间，命名空间包含单个**使用**声明每个名称。 否则，标头不能将任何库名称引入当前命名空间中。
