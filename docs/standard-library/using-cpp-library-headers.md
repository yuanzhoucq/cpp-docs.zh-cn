...
所有名称以外的其他**运算符 delete**并**运算符 new**中C++库标头中定义`std`命名空间，或嵌套在命名空间中`std`命名空间。 例如，将名称 `cin` 引用为 `std::cin`。 但是请注意，该宏名称不受命名空间限定，因此你始终需要写入不带命名空间限定符的 `__STD_COMPLEX`。

在某些转换环境，包括C++库标头中声明的外部名称可能会提升`std`至全局命名空间，命名空间包含单个**使用**声明每个名称。 否则，标头不能将任何库名称引入当前命名空间中。

C++标准要求 C 标准标头声明命名空间中的所有外部名称`std`，然后将它们提升至全局命名空间包含单个**使用**声明每个名称。 但在某些转换环境中，C 标准标头不包含命名空间声明，所有名称都直接在全局命名空间中进行声明。 因此，用于处理命名空间的可移植性最高的方法将遵循以下两种规则：
