...
`Allocator` 表示存储的分配器对象封装有关 hash_set 的分配和解除分配内存的详细信息的类型。 此参数是可选的而默认值是 **分配器 * * *\<密钥 >。*
...
`key` 要从 hash_set 要搜索的元素的排序键进行比较的自变量键。
...
`_Where` 要从 hash_set 中删除的元素的位置。

`first` 从 hash_set 中删除的第一个元素的位置。

`last` 从 hash_set 中删除最后一个元素以外的位置。

`key` 若要从 hash_set 中删除的元素的键。
...
`key` 要从 hash_set 要搜索的元素的排序键匹配的自变量键。
...
`right` 自变量 hash_set 提供与目标 hash_set 交换的元素。
