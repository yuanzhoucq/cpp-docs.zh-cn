...
*特征*\
类型包括两个函数对象，其中一个是可以将两个元素值作为排序键进行比较以确定其相对顺序，另一个是类型的无符号整数的哈希函数 `size_t` 。 此自变量是可选自变量，默认值为 `hash_compare<Key, less<Key> >`。
...
Hash_set 通过调用 value_compare 类型的存储哈希对象，对其控制的序列进行排序 `Traits` 。 [value_compare](#value_compare) 此存储对象可通过调用成员函数 [key_comp](#key_comp) 进行访问。 此类函数对象的行为必须与类*hash_compare<键的对象相同，> 更少 \<Key> 。* 具体而言，对于类型为 Key 的所有值 `key` ，调用特征（ `key` ）会生成类型 size_t 的值的分布。

通常，元素仅需小于比较元素即可建立此顺序；因此，给定任意两个元素，可以确定这两个元素等效（即两者均不小于对方）或其中一个小于另一个。 这会导致在非等效元素之间进行排序。 在技术性更强的说明中，比较函数是一个二元谓词，在标准数学的意义上引发严格弱排序。 二元谓词*f*（ *x*， *y*）是包含两个参数对象（x 和 y）和一个返回值（true 或 false）的函数对象。 如果二元谓词具有非自反性、反对称性和传递性且等效可传递，对 hash_set 进行的排序将为严格弱排序，其中两个对象 *x* 和 *y* 定义为在 *f*( *x*, *y*) 和 *f*( *y*, *x*) 均为 false 时等效。 如果键之间的更强相等条件取代了等效性，则排序将为总排序（即所有元素彼此排序），并且匹配的键将难以彼此辨别。
...
|[const_iterator](#const_iterator)|一种类型，它提供可读取中的元素的双向迭代器 **`const`** `hash_set` 。|
|[const_pointer](#const_pointer)|一种类型，它提供指向中的元素的指针 **`const`** `hash_set` 。|
...
如果将的返回值 `begin` 分配给，则 `const_iterator` 不能修改 hash_set 对象中的元素。 如果将的返回值 `begin` 分配给，则 `iterator` 可以修改 hash_set 对象中的元素。
...
一种类型，它提供可读取 hash_set 中的元素的双向迭代器 **`const`** 。
...
一种类型，它提供指向 **`const`** hash_set 中的元素的指针。
...
在大多数情况下，应该使用[const_iterator](#const_iterator)来访问 hash_set 对象中的元素 **`const`** 。
...
一种类型，该类型提供对 **`const`** 存储在 hash_set 中的元素的引用，以便读取和执行 **`const`** 操作。
...
一种类型，它提供可读取 hash_set 中的任何元素的双向迭代器 **`const`** 。
...
|参数|描述|
|-|-|
|*初始值*|要插入 [hash_set](../standard-library/hash-set-class.md) 的元素的值，除非 `hash_set` 已包含该元素（更宽泛地说，是其键经等效排序的元素）。|

### <a name="return-value"></a>返回值

`emplace`成员函数返回一个对，其 **`bool`** 组件在 **`true`** 插入时返回， **`false`** 如果 `hash_set` 已包含一个其键在排序中具有等效值的元素，并且其迭代器组件返回新元素的插入位置或已定位的元素的位置，则为。
...
**`true`** 如果 hash_set 为空，则为; 否则为。**`false`** 如果 hash_set 不是空的。
...
如果将的返回值 `find` 分配给 `const_iterator` ，则不能修改 hash_set 的对象。 如果将的返回值 `find` 分配给 `iterator` ，则可以修改 hash_set 的对象。
...
第一个构造函数创建一个空的起始 `hash_set`，第二个构造函数指定用于建立元素顺序的比较函数 ( `Comp`) 的类型，第三个构造函数显式指定要使用的分配器类型 ( `Al`)。 关键字关键字 **`explicit`** 取消了某些类型的自动类型转换。
...
|*时间*|要从 `hash_set` 中复制的最后一个元素以外的位置。|
|*IList*|从中复制元素的 initializer_list。|

### <a name="return-value"></a>返回值

第一个 `insert` 成员函数返回一个对 **`bool`** ，其组件在 **`true`** 插入时返回， **`false`** 如果 `hash_set` 已包含其键在排序中具有等效值的元素，并且其迭代器组件返回新元素的插入位置或已定位的元素。
...
一个 `iterator` 或 `const_iterator` ，用于寻址 hash_set 中其键等于或大于参数键的元素的位置，或用于发现该键 hash_set 中最后一个元素之后的位置的位置（如果找不到该键的匹配项）。
...
|参数|描述|
|-|-|
|*然后*|要复制到 `hash_set` 中的 [hash_set](../standard-library/hash-set-class.md)。|
