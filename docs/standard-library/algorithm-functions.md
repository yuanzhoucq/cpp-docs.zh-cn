...
`first` 发现向前迭代器范围内的第一个元素的位置搜索。

`last` 发现向前迭代器一个元素的位置的最后一个元素的范围中搜索。

`comp` 给定要搜索的范围内相邻元素的值满足的条件的二元谓词。
...
`first` 一个输入迭代器，指示在何处可以开始检查的条件。 该迭代器将标记元素范围的起始位置。

`last` 输入迭代器，该值指示要检查的条件的元素范围的末尾。

`comp` 要测试的条件。 这是用户定义的谓词函数对象，定义被检查元素要满足的条件。 谓词采用一个参数并返回 `true` 或 `false`。
...
`first` 一个输入迭代器，指示在何处可以开始检查满足条件的元素范围。

`last` 输入迭代器，该值指示要检查的条件的元素范围的末尾。

`comp` 要测试的条件。 这是由用户定义的谓词函数对象提供的。 谓词定义所测试的元素应满足的条件。 谓词采用一个参数并返回 `true` 或 `false`。
...
`value` 值所需元素的值匹配或与二元谓词指定的元素值必须满足的条件。

`comp` 用户定义谓词函数对象，用于定义在其中一个元素小于另一个的理解。 二元谓词采用两个参数，并且在满足时返回 `true`，未满足时返回 `false`。
...
`first` 寻址源范围中第一个元素的位置的输入迭代器。

`last` 发现是一个源范围中的最后一个元素的位置的输入迭代器。

*destBeg*发现目标范围内第一个元素的位置的输出迭代器。
...
`first` 寻址源范围中第一个元素的位置的双向迭代器。

`last` 寻址一个源范围中的最后一个元素的位置的双向迭代器。

`destEnd` 寻址的目标范围内一个元素的最后一个元素的位置的双向迭代器。
...
`first` 输入迭代器指示开始的范围中要检查的条件。

`last` 输入迭代器，该值指示范围的末尾。

`dest` 输出迭代器，该值指示复制元素的目标。

`_Pred` 对其测试的范围中每个元素的条件。 由用户定义的谓词函数对象提供这一条件。 谓词采用一个参数并返回 `true` 或 `false`。
...
一次为每个`N`范围内`[0, last - first)`，严格增加值的`N`从最低值开始 如果`dest`和`first`指定存储区域，`dest`不得位于范围`[ first, last )`。
...
`first` 输入迭代器，指示从中复制元素的位置。

`count` 有符号或无符号整数指定的类型元素的数目来复制。

`dest` 指示在何处可以将元素复制到一个输出迭代器。
...
`first` 输入迭代器寻址的范围中第一个元素的位置要遍历。

`last` 输入迭代器寻址一个元素的位置的最后一个元素的范围中要遍历。
...
`first` 发现输入迭代器范围内的第一个元素的位置要搜索。

`last` 输入迭代器寻址一个元素的位置的最后一个元素的范围中要搜索。

`_Pred` 定义要计数的元素是否满足的条件的用户定义谓词函数对象。 谓词采用一个参数并返回 **true** 或 **false**。
...
`First1` 输入迭代器确定第一个范围中的第一个元素的位置以进行测试。

`Last1` 输入迭代器寻址第一个范围中的最后一个元素过去一个元素的位置以进行测试。

`First2` 输入发现的迭代器在第二个范围中第一个元素的位置进行测试。

`First2` 发现输入迭代器在第二个范围中的最后一个元素位置以进行测试。
...
`comp` 定义对一个元素小于另一个的理解的用户定义谓词函数对象。
...
`first` 发现向前迭代器范围内的第一个元素的位置进行遍历。

`last` 向前迭代器寻址一个元素的位置的最后一个元素的范围中要遍历。

`val` 要分配给范围中元素的值 [ `first`， `last`)。
...
`First` 发现范围内的第一个元素的位置的输出迭代器的值分配给`Val`。

`Count` 有符号或无符号整数指定的类型的元素数可以被指定值。

`Val` 要分配给范围中元素的值 [ `First`，*第一个 + 计数*)。
...
`first` 发现输入迭代器范围内的第一个元素的位置要搜索指定的值。

`last` 输入迭代器寻址的范围中的最后一个元素的一个元素的位置要搜索指定的值。
...
`First1` 发现向前迭代器范围内的第一个元素的位置搜索。

`Last1` 发现向前迭代器之后的位置之一的最后一个元素的范围中搜索。

`First2` 发现向前迭代器范围内的第一个元素的位置搜索。

`Last2` 发现向前迭代器之后的位置之一的最后一个元素的范围中搜索。
...
`last1` 发现向前迭代器一个元素的位置的最后一个元素的范围中搜索。

`first2` 发现向前迭代器范围内的第一个元素的位置匹配。

`last2` 向前迭代器寻址一个元素的位置的最后一个元素的范围中要匹配。
...
`pred` 用户定义谓词函数对象或[lambda 表达式](../cpp/lambda-expressions-in-cpp.md)，它定义要搜索元素满足的条件。 谓词采用单个参数并返回 `true`（满足条件）或 `false`（不满足条件）。 `pred` 的签名必须是有效的 `bool pred(const T& arg);`，其中，`T` 是 `InputIterator` 在取消引用时可隐式转换为的类型。 显示 `const` 关键字的目的仅为说明函数对象或 lambda 不应修改参数。
...
`pred` 用户定义谓词函数对象或[lambda 表达式](../cpp/lambda-expressions-in-cpp.md)，它定义要搜索的元素不满足的条件。 谓词采用单个参数并返回 `true`（满足条件）或 `false`（不满足条件）。 `pred` 的签名必须是有效的 `bool pred(const T& arg);`，其中，`T` 是 `InputIterator` 在取消引用时可隐式转换为的类型。 显示 `const` 关键字的目的仅为说明函数对象或 lambda 不应修改参数。
...
`first` 寻址要处理的范围内第一个元素的位置的输入迭代器。

`last` 发现一个元素的位置的最后一个元素的范围中的输入迭代器操作。

`_Func` 应用于每个元素的范围中的用户定义函数对象。
...
`first` 寻址的值为要分配的范围中第一个元素的位置的向前迭代器。

`last` 寻址的值为要分配的范围中的最后一个元素的一个元素的位置向前迭代器。

`_Gen` 函数对象调用不带任何参数，用于生成要分配给每个范围中元素的值。
...
`First` 寻址的值为要分配的范围中第一个元素的位置的输出迭代器。

`Count` 有符号或无符号整数指定的类型的元素数为由生成器函数分配一个值。

`Gen` 函数对象调用不带任何参数，用于生成要分配给每个范围中元素的值。
...
`first1` 发现两个排序的源范围的第一个要测试的第二个的所有元素是否都包含在第一个中的第一个元素的位置的输入迭代器。

`last1` 输入迭代器寻址超出一位置的最后一个元素中第一个的两个排序的源范围的第二个的所有元素是否都包含在第一个测试。

`first2` 在第二个两个发现的第一个元素的位置的输入迭代器连续已排序源范围的第二个的所有元素是否都包含在第一个测试。

`last2` 中的两个连续已排序的源范围的第二个要测试的第二个的所有元素是否都包含在第一个发现的最后一个元素之后的一个位置的输入迭代器。

`comp` 用户定义谓词函数对象，用于定义在其中一个元素小于另一个的理解。 二元谓词采用两个参数并在条件满足时返回 **true** ，不满足时返回 **false**。
...
`first` 发现的两个连续的第一个中的第一个元素的位置的双向迭代器已排序范围要合并且排序为一个范围。

`middle` 在两个连续的第二个发现的第一个元素的位置的双向迭代器已排序范围要合并且排序为一个范围。

`last` 在两个连续的第二个发现最后一个元素之后的一个元素的位置的双向迭代器已排序范围要合并且排序为一个范围。

`comp` 定义意义上说，在其中一个元素大于另一个的用户定义谓词函数对象。 二元谓词采用两个参数，并且应在第一个元素小于第二个元素时返回 **true** ；否则返回 **false** 。
...
`first` 随机访问迭代器，指示要检查堆的范围的开始。

`last` 一个随机访问迭代器，该值指示范围的末尾。

`comp` 要测试的顺序元素的条件。 二元谓词采用单个参数并返回`true`或`false`。
...
`begin` 指定要检查堆的范围的第一个元素的随机访问迭代器。

`end` 指定要检查堆的范围的结束一个随机访问迭代器。

`compare` 一个二元谓词，指定严格弱排序条件定义堆。 未指定 `compare` 时，默认谓词为 `std::less<>`。
...
第一个模板函数返回最后一个迭代器`next`中`[ begin , end ]`其中`[ begin , next)`是排序通过函数对象的一个堆`std::less<>`。 如果距离`end - begin < 2`，该函数将返回`end`。
...
`first` 一个输入迭代器，指示一系列开始若要检查的条件的位置。

`last` 输入迭代器，该值指示范围的末尾。

`comp` 要测试的条件。 由用户定义的谓词函数对象提供，用于定义被搜索元素要满足的条件。 谓词采用一个参数并返回 `true` 或 `false`。
...
`First1` 引用范围的第一个元素的向前迭代器。

`Last1` 引用一个范围的最后一个元素之后的前向迭代器。

`First2` 引用第二个范围，用于比较的第一个元素的向前迭代器。

`Last2` 指的是最后一个元素之后的第二个范围，用于比较向前迭代器。

`Pred` 一个谓词，针对等效性测试并返回`bool`。
...
`first` 一个向前迭代器，该值指示要检查的范围的开始处。

`last` 指示范围的末尾向前迭代器。

`comp` 要进行测试以确定两个元素间的顺序的条件。 谓词采用一个参数并返回 `true` 或 `false`。 此设置执行与 `operator<` 相同的任务。

### <a name="remarks"></a>备注

第一个模板函数返回[is_sorted_until](http://msdn.microsoft.com/en-us/bbad99d0-deaa-4fe6-ae58-eb5b3e4dded0)`( first, last ) == last`。 `operator<`函数执行顺序比较。
...
`first` 指示要检查的范围开始的位置向前迭代器。

`last` 指示范围的末尾向前迭代器。

`comp` 要进行测试以确定两个元素间的顺序的条件。 谓词采用一个参数并返回 `true` 或 `false`。
...
`first1` 输入迭代器确定第一个范围中的第一个元素的位置进行比较。

`last1` 输入迭代器寻址第一个范围中的最后一个元素的一个元素的位置进行比较。

`first2` 输入迭代器确定在第二个范围中的第一个元素的位置进行比较。

`last2` 输入迭代器寻址第二个范围中的最后一个元素的一个元素的位置进行比较。
...
`value` 的第一个位置或可能的第一个位置进行搜索时排序的范围中的值。
...
`first` 寻址要将其转换为堆的范围内第一个元素的位置的随机访问迭代器。

`last` 寻址要将其转换为堆的范围内的最后一个元素的一个元素的位置的随机访问迭代器。
...
`first` 发现向前迭代器范围内的第一个元素的位置在其中搜索最大的元素。

`last` 一个向前迭代器寻址的范围中的最后一个元素的一个元素的位置要搜索的最大元素。
...
`first1` 寻址要合并且排序为一个范围的两个排序的源范围的第一个中的第一个元素的位置的输入迭代器。

`last1` 发现超出一位置的最后一个元素中第一个要合并且排序为一个范围的两个排序的源范围的输入迭代器。

`first2` 在第二个要合并且排序为一个范围的两个连续已排序的源范围中发现第一个元素的位置的输入迭代器。

`last2` 在第二个要合并且排序为一个范围的两个连续已排序的源范围中最后一个元素之后发现一个元素的位置的输入迭代器。

`result` 发现两个源范围要合并为一个排序范围的目标范围内第一个元素的位置的输出迭代器。
...
`first` 发现向前迭代器范围内的第一个元素的位置在其中搜索最小元素。

`last` 一个向前迭代器寻址的范围中的最后一个元素的一个元素的位置要搜索的最小元素。
...
如果序列为非空，函数最多执行`3 * (last - first - 1) / 2`比较。
...
第一个模板函数返回`pair<const Type&, const Type&>( right , left )`如果`right`是小于`left`。 否则，它将返回 `pair<const Type&, const Type&>( left , right )`。
...
`Last2` 发现输入迭代器在第二个范围中的最后一个元素位置以进行测试。

`Comp` 用户定义谓词函数对象，将每个范围中的当前元素进行比较并确定它们是否等效。 满足条件时，它将返回 **true**，不满足时将返回 **false**。
...
`first` 输入迭代器，指示要移动的元素范围的开始的位置。

`last` 输入迭代器，该值指示要移动的元素范围的末尾。
...
`first` 指示要移动元素的范围的开头的迭代器。

`last` 指示要移动元素的范围的末尾的迭代器。 此元素未移动。
...
`first` 用于的双向迭代器指向范围中的第一个元素的位置排列。

`last` 用于的双向迭代器指向一个位置的最后一个元素的范围中要排列。

`comp` 定义在排序中连续元素要满足的比较条件的用户定义谓词函数对象。 二元谓词采用两个参数并在条件满足时返回 **true** ，不满足时返回 **false**。
...
`first` 一个随机访问迭代器寻址的范围中第一个元素的位置进行分区。

*_Nth*寻址元素的位置的随机访问迭代器进行正确排序的分区的边界上。

`last` 一个随机访问迭代器寻址的范围中的最后一个元素的一个元素的位置进行分区。
...
`first` 输入迭代器，该值指示从何处着手要检查的一系列针对的条件的元素。

`last` 输入迭代器，该值指示的元素范围的末尾。

`comp` 要测试的条件。 由定义条件的用户定义的谓词函数对象提供。 谓词采用一个参数并返回 `true` 或 `false`。
...
`first` 一个随机访问迭代器寻址的范围中第一个元素的位置以进行排序。

`sortEnd` 一个随机访问迭代器寻址子范围中的最后一个元素的一个元素的位置以进行排序。

`last` 一个随机访问迭代器寻址一个元素的位置的最后一个元素的范围中要部分排序。
...
`first1` 寻址源范围中第一个元素的位置的输入迭代器。

`last1` 发现一个元素的位置的最后一个元素的源范围中的输入迭代器。

`first2` 寻址的已排序的目标范围内第一个元素的位置的随机访问迭代器。

`last2` 寻址一个元素的位置的最后一个元素的已排序的目标范围内的随机访问迭代器。

`comp` 用户定义的谓词函数对象，用于定义要执行两个元素是否满足的条件视为等效。 二元谓词采用两个参数，并且在满足时返回 `true`，未满足时返回 `false`。
...
`first` 一个双向迭代器寻址的范围中第一个元素的位置进行分区。

`last` 一个双向迭代器寻址的范围中的最后一个元素的一个元素的位置进行分区。

`comp` 定义元素将以进行分类时应满足的条件的用户定义谓词函数对象。 谓词采用一个参数并返回 **true** 或 **false**。
...
`first` 一个输入迭代器，指示要检查的条件的范围的开头。

`last` 输入迭代器，该值指示范围的末尾。

`dest1` 使用测试的输出迭代器用于将返回 true，该条件的元素复制`_Pred`。

`dest2` 使用测试的输出迭代器用于将返回 false，该条件的元素复制`_Pred`。

`_Pred` 要测试的条件。 由定义要测试条件的用户定义谓词函数对象提供。 谓词采用一个参数并返回 `true` 或 `false`。
...
`first` A `ForwardIterator` ，该值指示要检查的条件的范围的开始。

`last` A `ForwardIterator` ，该值指示范围的末尾。
...
`first` 寻址堆中的第一个元素的位置的随机访问迭代器。

`last` 寻址一个元素的位置的最后一个元素的堆中的随机访问迭代器。
...
`comp` 定义在排序中连续元素要满足的比较条件的用户定义谓词函数对象。 二元谓词采用两个参数，并且在满足时返回 `true`，未满足时返回 `false`。
...
Std::random_shuffle() 函数已弃用，替换为[std:: shuffle](../standard-library/algorithm-functions.md#shuffle)。 有关代码示例和详细信息，请参阅[\<随机 >](../standard-library/random.md)和 Stackoverflow 文章[为什么是 std:: random_shuffle 弃用方法在 C + + 14？](http://go.microsoft.com/fwlink/p/?linkid=397954)。
...
`first` 寻址要从中删除元素范围内的第一个元素的位置的向前迭代器。

`last` 寻址要从中删除元素的范围中的最后一个元素的一个元素的位置向前迭代器。
...
`first` 寻址要从中删除元素范围内的第一个元素的位置的输入迭代器。

`last` 要从中删除元素的范围中的最后一个元素发现一个元素的位置的输入迭代器。

`result` 寻址要向其删除元素的目标范围内第一个元素的位置的输出迭代器。
...
`first` 前向迭代器，指向要从中删除元素范围内的第一个元素的位置。

`last` 前向迭代器，指向要从中删除元素的范围中的最后一个元素的一个元素的位置。
...
`first` 向前迭代器指向将被替换元素的范围中的第一个元素的位置。

`last` 向前迭代器指向将被替换元素的范围中的最后一个元素的一个元素的位置。

`_OldVal` 正在替换的元素的旧值。

`_NewVal` 正在分配给具有旧值的元素的新值。
...
`first` 输入迭代器指向将被替换元素的范围中的第一个元素的位置。

`last` 输入迭代器指向一个位置的最后一个元素中将被替换元素的范围。

`result` 输出迭代器指向要将序列已更改的元素复制到的目标范围内的第一个元素。
...
`result` 输出迭代器指向第一个元素的元素复制到的目标范围内的位置。

`_Pred` 必须满足的一元谓词是元素的值将被替换。

`val` 正在分配给其旧值满足谓词的元素的新值。
...
`last` 迭代器指向将被替换元素的范围中的最后一个元素的一个元素的位置。
...
`first` 双向迭代器，指向元素进行置换在此范围内的第一个元素的位置。

`last` 指向一个位置的最后一个元素的元素进行置换在此范围中的双向迭代器。
...
`first` 双向迭代器指向在其中元素进行置换的源范围中的第一个元素的位置。

`last` 指向一个位置的最后一个元素范围内的元素进行置换的源范围中的双向迭代器。
...
`first` 一个向前迭代器寻址的范围中第一个元素的位置要轮换。

`middle` 定义在其元素将要与范围的第一个部分中交换的范围的第二部分中的第一个元素的位置的范围内的边界一个向前迭代器。

`Last` 一个向前迭代器寻址一个元素的位置的最后一个元素的范围中要轮换。
...
_`Last`的向前迭代器寻址一个元素的位置的最后一个元素的范围中要轮换。

`result` 发现目标范围内第一个元素的位置的输出迭代器。
...
`first1` 发现两个排序的源范围的第一个要相并且排序为一个范围表示两个源范围的差异中的第一个元素的位置的输入迭代器。

`last1` 输入迭代器寻址超出一位置的最后一个元素中第一个的两个排序的源范围要相并且排序为一个范围表示两个源范围的差异。

`first2` 在两个连续的第二个中发现第一个元素的位置的输入迭代器已排序源范围相并且排序为一个范围表示两个源范围的差异。

`last2` 寻址过去一个元素的位置的输入迭代器的最后一个元素中第二个两个连续已排序源范围要相并且排序为一个范围表示两个源范围的差异。

`result` 发现两个源范围要将相并为一个排序范围表示两个源范围的差异的目标范围内第一个元素的位置的输出迭代器。
...
`first1` 发现的第一个元素中的两个排序的源范围的第一个要相并且排序为一个范围表示两个源范围的交集的位置的输入迭代器。

`last1` 输入迭代器寻址超出一位置的最后一个元素中第一个的两个排序的源范围要相并且排序为一个范围表示两个源范围的交集。

`first2` 在两个连续的第二个中发现第一个元素的位置的输入迭代器已排序源范围相并且排序为一个范围表示两个源范围的交集。

`last2` 寻址过去一个元素的位置的输入迭代器的最后一个元素中第二个两个连续已排序源范围相并且排序为一个范围表示两个源范围的交集。

**_** *结果*发现其中的两个源范围的目标范围内第一个元素的位置的输出迭代器要将相并为一个排序范围表示两个源的交集范围。
...
`first1` 发现两个排序的源范围的第一个要相并且排序为一个范围表示两个源范围的对称差异中的第一个元素的位置的输入迭代器。

`last1` 输入迭代器寻址超出一位置的最后一个元素中第一个的两个排序的源范围要相并且排序为一个范围表示两个源范围的对称差异。

`first2` 在两个连续的第二个中发现第一个元素的位置的输入迭代器已排序源范围相并且排序为一个范围表示两个源范围的对称差异。

`last2` 寻址过去一个元素的位置的输入迭代器的最后一个元素中第二个两个连续已排序源范围相并且排序为一个范围表示两个源范围的对称差异。

**_** *结果*发现其中的两个源范围的目标范围内第一个元素的位置的输出迭代器要为一个排序范围表示的对称差异的两个相源范围。
...
`first1` 发现的第一个元素中的两个排序的源范围的第一个要相并且排序为一个范围表示两个源范围的并集的位置的输入迭代器。

`last1` 输入迭代器寻址超出一位置的最后一个元素中第一个的两个排序的源范围要相并且排序为一个范围表示两个源范围的并集。

`first2` 在两个连续的第二个中发现第一个元素的位置的输入迭代器已排序源范围相并且排序为一个范围表示两个源范围的并集。

`last2` 寻址过去一个元素的位置的输入迭代器的最后一个元素中第二个两个连续已排序源范围相并且排序为一个范围表示两个源范围的并集。

**_** *结果*发现其中的两个源范围的目标范围内第一个元素的位置的输出迭代器要将相并为一个排序范围表示两个源范围的并集。
...
`first` 要的迭代器范围中的第一个元素重新排序 （含） 之间。 必须满足 `RandomAccessIterator` 和 `ValueSwappable` 的要求。

`last` 指向的迭代器范围中的最后一个元素为重新排序、 排他。 必须满足 `RandomAccessIterator` 和 `ValueSwappable` 的要求。

`gen` 随机数生成器，`shuffle()`函数将用于该操作。 必须满足 `UniformRandomNumberGenerator` 的要求。
...
`last` 一个随机访问迭代器寻址的范围中的最后一个元素的一个元素的位置以进行排序。

`comp` 定义在排序中连续元素要满足的比较条件的用户定义谓词函数对象。 该二元谓词采用两个参数，并且，如果两个参数按顺序排序，将返回 `true`，否则将返回 `false`。 该比较器函数必须对序列中的元素对进行严格弱排序。 有关详细信息，请参阅[算法](../standard-library/algorithms.md)。
...
`first` 寻址目标堆中的第一个元素的位置的随机访问迭代器。

`last` 寻址目标堆中的最后一个元素的一个元素的位置的随机访问迭代器。
...
`_Pred` 定义元素将以进行分类时应满足的条件的用户定义谓词函数对象。 谓词采用一个参数并返回 **true** 或 **false**。
...
`first` 双向迭代器寻址的范围中第一个元素的位置来进行排序。

`last` 双向迭代器寻址的范围中的最后一个元素的一个元素的位置来进行排序。
...
运行时间复杂性`stable_sort`取决于可用，内存量 （给定足够的内存） 的最佳大小写，但*O*( *N*日志*N*) 和最差的情况下是*O*( *N* (日志*N* ) 2)，其中*N* =  *姓氏的名字。* 通常情况下，**sort** 算法比 `stable_sort` 快得多。
...
`left` 为第一个重写时，能够交换其内容的第一个对象。 对于第二次重写，为交换其内容的第一个对象数组。

`right` 为第一个重写时，第二个对象具有交换其内容。 对于第二次重写，为交换其内容的第二个对象数组。
...
`first1` 前向迭代器，指向其元素将要进行交换的第一个范围的第一个位置。

`last1` 指向一个其元素将要进行交换的第一个范围的最后一个位置之后一个向前迭代器。

`first2` 前向迭代器，指向其元素将要进行交换的第二个范围的第一个位置。
...
`first1` 寻址要运算的第一个源范围中第一个元素的位置的输入迭代器。

`last1` 发现第一个源范围中的最后一个元素的一个元素的位置的输入迭代器操作。

`first2` 寻址要运算的第二个源范围中第一个元素的位置的输入迭代器。

`result` 发现目标范围内第一个元素的位置的输出迭代器。

`_Func` 用户定义的一元函数对象应用于第一个源范围中每个元素的算法的第一个版本中使用或用户定义 (UD) 二元函数对象按向前顺序成对，应用该算法的第二个版本中使用到两个源范围。
...
`first` 寻址要扫描的移除重复项的范围内第一个元素的位置的向前迭代器。

`last` 寻址一个元素的位置的最后一个元素要扫描的移除重复项的范围内的前向迭代器。
...
`first` 一个向前迭代器寻址源范围中第一个元素的位置要复制。

`last` 一个向前迭代器寻址源范围中的最后一个元素的一个元素的位置要复制。

`result` 发现接收连续副本的目标范围内第一个元素的位置的输出迭代器中删除。
...
`first` 要搜索的范围内第一个元素的位置。

`last` 一个元素的位置的最后一个元素要搜索的范围内。

`value` 排序需要会导致超出发现返回的迭代器的元素的值的范围中的值。
