...
|操作员|说明|
|-|-|
|[操作员](#op_star)|取消引用运算符，用于实现 \* `i`  =  `x` 前端插入的输出迭代器表达式。|
|[operator + +](#op_add_add)|将 `front_insert_iterator` 递增到下一个可用来存储值的位置。|
|[operator =](#op_eq)|赋值运算符，用于实现 \* `i`  =  `x` 前端插入的输出迭代器表达式。|
...
第一个成员运算符计算 `container.push_front( val)` ，然后返回 **`*this`** 。
