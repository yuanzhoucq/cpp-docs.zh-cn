...
其类型是类模板的专用化的参数 `match_results` 或 `sub_match` 通过调用 `str` 成员函数进行转换;

类型为类模板的专用化的参数 `basic_string` 保持不变;

通过将参数值传递给构造函数，将参数值传递给构造函数，以便 `basic_string` 的类模板。
