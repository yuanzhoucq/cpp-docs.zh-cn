...
[Vector \<bool> 引用类](../standard-library/vector-bool-class.md#reference_class)是一个嵌套类，其对象可提供对向量对象中的元素（单个位）的引用 \<bool> 。
...
|||
|-|-|
|[向量](#vector)|构造一个向量，它具有特定大小、具有特定值的元素、具有特定 `allocator`，或将其构造成某个其它向量的副本。|
...
|[器](#iterator)|一个类型，它提供可读取或修改向量中任何元素的随机访问迭代器。|
|[变为](#pointer)|一个类型，提供指向向量中元素的指针。|
...
|[返回](#back)|返回对向量中最后一个元素的引用。|
|[准备](#begin)|对该向量中第一个元素返回随机访问迭代器。|
|[功能](#capacity)|返回在不分配更多的存储的情况下向量可以包含的元素数。|
...
对自变量中的下标元素的引用。 如果*position*大于矢量的大小， `at` 则将引发异常。
...
如果将的返回值 `begin` 分配给[vector：： const_iterator](#const_iterator)，则 `vector` 无法修改对象。 如果将 `begin` 的返回值分配给 [vector::iterator](#iterator)，则无法修改 `vector` 对象。
...
如果返回值为 `crend` （适当递减），则 `vector` 不能修改对象。
...
*置于*\
[向量](../standard-library/vector-class.md)中插入第一个元素的位置。
...
任何插入操作都可能会消耗大量资源，请参阅[vector 类](../standard-library/vector-class.md)，了解有关性能的讨论 `vector` 。
...
向量的超过末尾迭代器。 如果向量为空，则为 `vector::end() == vector::begin()` 。
...
如果将的返回值 `front` 分配给 `const_reference` ，则无法修改矢量对象。 如果将 `front` 的返回值分配给 **reference**，则可以修改矢量对象。
...
作为前置条件，*第一个*和*最后一个*不得迭代器进入向量，或该行为未定义。 任何插入操作都可能会消耗大量资源，请参阅[vector 类](../standard-library/vector-class.md)，了解有关性能的讨论 `vector` 。
...
如果容器的大小小于请求的大小， *new_size* `resize` 会将元素添加到向量，直到达到请求的大小。 如果容器的大小大于请求的大小，则 `resize` 会删除离容器末尾最近的元素，直到其达到*new_size*大小。 如果容器的当前大小与请求的大小相同，则不执行任何操作。
...
第三个构造函数指定类的默认值的元素的指定数目（*计数*）的重复 `Type` 。
