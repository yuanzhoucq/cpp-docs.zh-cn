...
第一个构造函数将构造序列末迭代器。 第二个构造函数将首先初始化存储的值 `begin` 、存储的*first*值 `end` 和*最后一个*值，并将存储的值 `pregex` 与 `&re` `flags` *f*一起存储。 然后，它调用 `regex_search(begin, end, match, *pregex, flags)`。 如果搜索失败，则构造函数会将对象设置为序列末迭代器。
