...
类模板描述可用作区域设置 facet 的对象，用于控制类型序列 `CharType` 到货币值的转换。
...
如果 **iosbase**. [标志](../standard-library/ios-base-class.md#flags)  & [showbase](../standard-library/ios-functions.md#showbase)的值为非零 **。** `curr_symbol` 必须在等于 **money_base::symbol** 的元素在此格式模式中出现的位置匹配。 否则，如果 **money_base::symbol** 出现在此格式模式末尾，或如果没有符号字符串的元素要进行匹配，则不会匹配货币符号。 否则，将选择性地匹配货币符号。
