...
*性状*\
包含两个函数对象的类型，一个是类*Traits，* 能够将两个元素值作为排序键来确定其相对顺序;另一种是哈希函数，它是一个未排序谓词，将元素的键值映射到类型的无`size_t`符号整数。 此自变量是可选自变量，默认值为 `hash_compare<Key, less<Key>>`。
...
hash_multimap 通过调用类型 [value_compare](../standard-library/value-compare-class.md) 的已存储哈希 `Traits` 对象，对其控制的序列进行排序。 此存储对象可通过调用成员函数 [key_comp](../standard-library/hash-map-class.md#key_comp) 进行访问。 这样的函数对象必须与类[hash_compare](../standard-library/hash-compare-class.md)`<Key, less<Key>>`的对象一样。 具体而言，针对类型为 `Key` 的所有值 `Key`，调用 `Traits (Key)` 将对类型为 `size_t` 的值进行分布。

通常，元素仅需小于比较元素即可建立此顺序；因此，给定任意两个元素，可以确定这两个元素等效（即两者均不小于对方）或其中一个小于另一个。 这会导致在非等效元素之间进行排序。 在技术性更强的说明中，比较函数是一个二元谓词，在标准数学的意义上引发严格弱排序。 二进制谓词f（x，y）是一个函数对象，具有两`x`个`y`参数对象和**true**或**false**的返回值。 如果二进制谓词是不可反反射的、反对称的和传递性的，并且当 f（x、y 和 f（y，x） 都是`x`false`y`时，则对 hash_multimap施加的顺序是一个严格的弱排序，如果等价性是传递的，则两个对象被定义为等效对象。 **false** 如果键之间的更强相等条件取代了等效性，则排序将为总排序（即所有元素彼此排序），并且匹配的键将难以彼此辨别。
...
|[const_pointer](#const_pointer)|提供指向`hash_multimap`中**const**元素的指针的类型。|
|[const_reference](#const_reference)|一种类型，用于读取和执行**const**操作，提供`hash_multimap`对存储在 中的**const**const 元素的引用。|
|[const_reverse_iterator](#const_reverse_iterator)|提供可读取 中的任何**const**元素的双向迭代器的类型`hash_multimap`。|
|[difference_type](#difference_type)|一种有符号整数类型，此类型可用于表示 `hash_multimap` 中迭代器指向的元素间范围内的元素数量。|
|[迭 代](#iterator)|一种类型，它提供可读取或修改 `hash_multimap` 中任何元素的双向迭代器。|
...
|[指针 (pointer)](#pointer)|一种类型，它提供指向 `hash_multimap` 中的某个元素的指针。|
|[参考](#reference)|一种类型，此类型提供对存储在 `hash_multimap` 中的元素的引用。|
...
|[空](#empty)|测试 `hash_multimap` 是否为空。|
|[结束](#end)|返回一个迭代器，此迭代器用于发现 `hash_multimap` 中最后一个元素之后的位置。|
...
|[交换](#swap)|交换两个 `hash_multimap` 的元素。|
...
## <a name="hash_multimapbegin"></a><a name="begin"></a>hash_multimap：开始
...
如果将 返回`begin`值分配给 ，`const_iterator`则无法修改对象hash_multimap中的元素。 如果将 返回`begin`值分配给 ，`iterator`则可以修改hash_multimap对象中的元素。
...
## <a name="hash_multimapcend"></a><a name="cend"></a>hash_multimap：克恩德
...
## <a name="hash_multimapclear"></a><a name="clear"></a>hash_multimap：：清除
...
由`const_iterator`hash_multimap定义的对象指向[value_type](#value_type)的对象，该对象的类型`pair<const Key, Type>`为 。 键值通过第一个成员对可用，已映射元素的值通过第二个成员对可用。

要取消引用`const_iterator``cIter`指向hash_multimap元素，请使用`->`运算符。
...
由`const_reverse_iterator`hash_multimap定义的对象指向[value_type](#value_type)的对象，这些对象的类型`pair<const Key, Type>`为，其第一个成员是元素的键，其第二个成员是元素持有的映射基准。

要取消引用`const_reverse_iterator``crIter`指向hash_multimap元素，请使用`->`运算符。

要访问元素的键的值，请使用`crIter->first`等效于`(*crIter).first`。 要访问元素的映射基准值，请使用`crIter->second`，这等效于`(*crIter).second`。

### <a name="example"></a>示例

有关如何声明和使用 `const_reverse_iterator` 的示例，请参阅 [rend](#rend) 的示例。

## <a name="hash_multimapcount"></a><a name="count"></a>hash_multimap：计数
...
## <a name="hash_multimapcrbegin"></a><a name="crbegin"></a>hash_multimap：克·克贝京
...
## <a name="hash_multimapcrend"></a><a name="crend"></a>hash_multimap：克伦德
...
## <a name="hash_multimapdifference_type"></a><a name="difference_type"></a>hash_multimap：:d）类型
...
## <a name="hash_multimapemplace"></a><a name="emplace"></a>hash_multimap：：地点
...
|参数|说明|
|-|-|
|*瓦尔*|此值用于移动构造要插入到 [hash_multimap](../standard-library/hash-multimap-class.md) 的元素。|
...
|参数|说明|
|-|-|
|*瓦尔*|要插入 [hash_multimap](../standard-library/hash-multimap-class.md) 的用于移动构造元素的值，除非此 `hash_multimap` 已包含该元素（更宽泛地说，是其键经等效排序的元素）。|
...
## <a name="hash_multimapempty"></a><a name="empty"></a>hash_multimap：：空
...
## <a name="hash_multimapend"></a><a name="end"></a>hash_multimap：结束
...
## <a name="hash_multimaperase"></a><a name="erase"></a>hash_multimap：擦除
...
## <a name="hash_multimapfind"></a><a name="find"></a>hash_multimap：查找
...
成员函数返回一个迭代器，用于解决hash_multimap的元素，该元素的排序键是`equivalent`二进制谓词下的参数键，该参数基于不太可比性的关系引发排序。

如果将 `find` 的返回值赋给 `const_iterator`，则无法修改 hash_multimap 对象。 如果将 返回`find`值分配给 ，`iterator`则可以修改hash_multimap对象。
...
|参数|说明|
|-|-|
|*铝*|要用于此 hash_multimap 对象的存储分配器类，默认为 `Allocator`。|
|*康普*|用于对 map 中元素排序的类型 `const Traits` 的比较函数，默认为 `Traits`。|
|*对*|所构造集要作为其副本的映射。|
...
前三个构造函数指定一个空的初始hash_multimap;第二种指定用于建立元素顺序的比较函数类型 （*Comp*），第三个指定要使用的分配器类型 （`_Al`） 。 关键字 `explicit` 取消了某些种类的自动类型转换。
...
## <a name="hash_multimapinsert"></a><a name="insert"></a>hash_multimap：插入
...
|*哪里*|有关从哪里开始搜索正确插入点的位置的提示。|
|*第一*|要从映射中复制的第一个元素的位置。|
|*最后*|要从映射中复制的最后一个元素以外的位置。|
...
如果插入点紧随`insert`*何处*之后，则插入可以发生在 提示版本的摊销常量时间中，而不是对数时间。

## <a name="hash_multimapiterator"></a><a name="iterator"></a>hash_multimap：迭代器
...
由`iterator`hash_multimap定义的对象指向[value_type](#value_type)的对象，这些对象的类型`pair`\<为**const Key，类型**>，其第一个成员是元素的键，其第二个成员是元素持有的映射基准。

要取消引用指向hash_multimap元素的**迭代器**`Iter`，请使用`->`运算符。

若要访问元素的键值，请使用 `Iter` -> **first**，其等同于 (\* `Iter`)。 **第一**个 。 若要访问元素的映射值，请使用 `Iter` -> **second**，其作用与 (\* `Iter`). **第一**个 。
...
有关*密钥*的详细信息，请参阅[hash_multimap 类](../standard-library/hash-multimap-class.md)主题的备注部分。
...
如果将 `lower_bound` 的返回值赋给 `const_iterator`，则无法修改 hash_multimap 对象。 如果将 返回`lower_bound`值分配给 ，`iterator`则可以修改hash_multimap对象。
...
有关*类型*的详细信息，请参阅[hash_multimap类](../standard-library/hash-multimap-class.md)主题。
...
## <a name="hash_multimapoperator"></a><a name="op_eq"></a>hash_multimap：：操作员*
...
|参数|说明|
|-|-|
|*对*|正在复制到 `hash_multimap` 的 [hash_multimap](../standard-library/hash-multimap-class.md)。|

### <a name="remarks"></a>备注

在上用 中的任何现有元素后`hash_multimap`，`operator=`复制 或将*右侧*的内容移动到 中`hash_multimap`。
...
## <a name="hash_multimappointer"></a><a name="pointer"></a>hash_multimap：:p奥米特
...
## <a name="hash_multimapreference"></a><a name="reference"></a>hash_multimap：参考
...
## <a name="hash_multimaprend"></a><a name="rend"></a>hash_multimap：end
...
## <a name="hash_multimapsize"></a><a name="size"></a>hash_multimap：大小
...
## <a name="hash_multimapswap"></a><a name="swap"></a>hash_multimap：交换
...
如果将 `upper_bound` 的返回值赋给 `const_iterator`，则无法修改 hash_multimap 对象。 如果将 返回`upper_bound`值分配给 ，`iterator`则可以修改hash_multimap对象。
...
对于hash_multimap*米*，如果两个元素 e1 *（k1，* *d1）* 和*e2**（k2，* *d2）* 是[类型value_type](#value_type)的对象，*其中 k1*和*k2*是其类型[key_type](#key_type)键， d1 和`m.key_comp()(k1, k2)` *d2* [是它们mapped_type](#mapped_type)类型的数据， 则`m.value_comp()(e1, e2)`等效于 。 *e1* *d2* 存储对象会定义成员函数
...
`value_type`由于关联容器的键\<不能使用非恒定迭代器或引用\<更改关联容器的键，因此mapped_type>，因此声明为对[key_type，mapped_type>，](#mapped_type)而不是对key_type。 [key_type](#key_type)
