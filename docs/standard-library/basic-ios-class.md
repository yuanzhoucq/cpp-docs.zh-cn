...
`state` （可选）你想要清除所有标志后设置标志。 默认为 `goodbit`。

`reraise` （可选）指定是否应重新引发异常。 默认为 `false`（将不重新引发异常）。
