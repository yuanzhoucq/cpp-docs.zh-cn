...
## <a name="remarks"></a>备注  
 ...
 类 basic_stringbuf< `Elem`, `Tr`, `Alloc`> 的对象将来自其构造函数的 `ios_base::`[openmode](../standard-library/ios-base-class.md#ios_base__openmode) 自变量的副本作为其 `stringbuf` 模式 **mode** 进行存储：  
...
|||  
|-|-|  
|[basic_stringbuf](#basic_stringbuf__basic_stringbuf)|构造 `basic_stringbuf` 类型的对象。|  
  
### <a name="typedefs"></a>Typedef  
  
|||  
|-|-|  
|[allocator_type](#basic_stringbuf__allocator_type)|该类型是模板参数 `Alloc`的同义词。|  
|[char_type](#basic_stringbuf__char_type)|将类型名与 `Elem` 模板参数关联。|  
|[int_type](#basic_stringbuf__int_type)|使 `basic_filebuf` 范围中的此类型等效于 `Tr` 范围中具有相同名称的类型。|  
|[off_type](#basic_stringbuf__off_type)|使 `basic_filebuf` 范围中的此类型等效于 `Tr` 范围中具有相同名称的类型。|  
|[pos_type](#basic_stringbuf__pos_type)|使 `basic_filebuf` 范围中的此类型等效于 `Tr` 范围中具有相同名称的类型。|  
|[traits_type](#basic_stringbuf__traits_type)|将类型名与 `Tr` 模板参数关联。|  
  
### <a name="member-functions"></a>成员函数  
  
|||  
|-|-|  
|[overflow](#basic_stringbuf__overflow)|将新字符插入到已满缓冲区时可以调用的受保护虚函数。|  
|[pbackfail](#basic_stringbuf__pbackfail)|受保护虚拟成员函数尝试将元素放回到输入缓冲区中，随后使它成为当前元素（由下一个指针指向）。|  
|[seekoff](#basic_stringbuf__seekoff)|受保护虚拟成员函数尝试更改受控制流的当前位置。|  
|[seekpos](#basic_stringbuf__seekpos)|受保护虚拟成员函数尝试更改受控制流的当前位置。|  
|[str](#basic_stringbuf__str)|设置或获取字符串缓冲区中的文本，而无需更改写入位置。|  
|swap||  
|[underflow](#basic_stringbuf__underflow)|从输入流中提取当前元素的受保护虚拟成员函数。|  
...
### <a name="parameters"></a>参数  
 `_Mode`  
 [ios_base::openmode](../standard-library/ios-base-class.md#ios_base__openmode) 中的枚举之一。  
...
### <a name="return-value"></a>返回值  
 如果该函数不成功，它将返回 **traits_type::eof**。 否则，它将返回 **traits_type::**[not_eof](../standard-library/char-traits-struct.md#char_traits__not_eof)(_ *Meta*)。  
  
### <a name="remarks"></a>备注  
 如果 _ *Meta* 不等于 **traits_type::**[eof](../standard-library/char-traits-struct.md#char_traits__eof)，则受保护虚拟成员函数尝试将元素 **traits_type::**[to_char_type](../standard-library/char-traits-struct.md#char_traits__to_char_type)(\_ *Meta*) 插入输出缓冲区。 它可以用多种方法执行此操作：  
...
### <a name="remarks"></a>备注  
 如果 `_Meta` 等于 **traits_type::**[eof](../standard-library/char-traits-struct.md#char_traits__eof)，要推送回的元素在当前元素之前实际上已是流中的一个元素了。 否则，则由 **byte** = **traits_type::**[to_char_type](../standard-library/char-traits-struct.md#char_traits__to_char_type)(_ *Meta*) 替换该元素。 该函数可以用多种方法放回元素：  
...
### <a name="parameters"></a>参数  
 `_Off`  
 要搜寻的相对于 `_Way` 的位置。 有关详细信息，请参阅 [basic_stringbuf::off_type](#basic_stringbuf__off_type)。  
  
 `_Way`  
 偏移操作的起点。 请参阅 [ios_base::seekdir](../standard-library/ios-base-class.md#ios_base__seekdir)，查看可能的值。  
  
 `_Mode`  
 指定指针位置的模式。 默认值允许修改读取和写入位置。 有关详细信息，请参阅 [ios_base::openmode](../standard-library/ios-base-class.md#ios_base__openmode)。  
...
-   如果 **mode & ios_base::out** 是非零值且存在输出缓冲区，则序列是整个输出缓冲区（以 `pbase` 开头的 [epptr](../standard-library/basic-streambuf-class.md#basic_streambuf__epptr) - [pbase](../standard-library/basic-streambuf-class.md#basic_streambuf__pbase) 元素）。  
  
-   如果 **mode & ios_base::in** 是非零值且存在输入缓冲区，则序列是整个输入缓冲区（以 `eback` 开头的 [egptr](../standard-library/basic-streambuf-class.md#basic_streambuf__egptr) - [eback](../standard-library/basic-streambuf-class.md#basic_streambuf__eback) 元素）。  
...
### <a name="return-value"></a>返回值  
 如果该函数不成功，它将返回 **traits_type::**[eof](../standard-library/char-traits-struct.md#char_traits__eof)。 否则，它返回输入流中的已转换的当前元素。  
  
### <a name="remarks"></a>备注  
 受保护的虚拟成员函数尝试从输入缓冲区提取当前元素 **byte**，提出当前流位置，并返回元素作为 **traits_type::**[to_int_type](../standard-library/char-traits-struct.md#char_traits__to_int_type)( **byte**)。 它可以一种方式进行此操作：如果读取位置可用，它采用 **byte** 作为存储在读取位置中的元素，并提出输入缓冲区的下一个指针。  
