---
title: "divides 结构 | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
#### 参数  
 `Type`, `Type1`, `Type2`  
 支持`operator/` 使用指定或者推导类型的操作数的类型。  
  
 `Left`  
 除法运算的左侧操作。  未指定的模版需要类型`Type`的左值引用参数。  专有模版确实完美地继承了推断类型`Type1`的左值和右值引用参数。  
  
 `Right`  
 除法运算的右侧操作。  未指定的模版需要类型`Type`的左值引用参数。  专有模版确实完美地继承了推断类型`Type2`的左值和右值引用参数。  
...
**向量 v2 \= \( 2 4 6 8 10 12 \)**  
**智能元素求商果是：\( 0 1.75 2.33333 2.625 2.8 2.91667 \)**   
## 要求  
 **标头：** \<起作用的\>  
