...
- `try_lock` 方法尝试在不阻止的情况下获取互斥体的所有权。 **`bool`** **`true`** 如果方法获取所有权，则其返回类型可转换为，否则为 **`false`** 。
...
|[once_flag 结构](../standard-library/once-flag-structure.md)|表示一个 **`struct`** ，它与模板函数结合使用， `call_once` 以确保只调用一次初始化代码，即使存在多个执行线程。|
|[try_to_lock_t 结构](../standard-library/try-to-lock-t-structure.md)|表示一个 **`struct`** ，它定义 `try_to_lock` 对象并用于选择的重载构造函数之一 `unique_lock` 。|
