...
类模板封装一个或多个分配器的嵌套。 每个这样的类都具有一个类型为 `outer_allocator_type` 的最外层分配器，该类型也即 `Outer`，是 `scoped_allocator_adaptor` 对象的公共基类。 `Outer` 可用于分配容器要使用的内存。 通过调用 `outer_allocator` 可获取对此分配器基对象的引用。
