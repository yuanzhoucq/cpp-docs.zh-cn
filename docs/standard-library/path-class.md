...
|[Iterator](#iterator)|指定`path`组件的`myname`双向常量迭代器。|
...
|[relative_path](#relative_path)|返回的`myname`相对路径组件。 |
...
|[root_path](#root_path)|返回的`myname`根路径组件。|
