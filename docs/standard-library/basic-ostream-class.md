...
第一个构造函数通过调用[init](../standard-library/basic-ios-class.md#init)（）初始化基类 `strbuf` 。 第二个构造函数通过调用[basic_ios：： move](../standard-library/basic-ios-class.md#move)初始化基类 `(right)` 。
...
每个将*val*转换为数值字段，并通过调用**use_facet<num_put \<Elem, OutIt> **（）将其插入 `getloc` 。 **put**（**OutIt**（ `rdbuf` ）， ** \* this**， `getloc` ， **val**）。 此处， **OutIt**定义为**ostreambuf_iterator \<Elem, Tr> **。 函数返回** \* this**。
...
每个将*val*转换为数值字段，并通过调用**use_facet<num_put \<Elem, OutIt> **（ `getloc` ）**（****OutIt**（ `rdbuf` ）， ** \* this**， `getloc` ， **val**）插入它。 此处， **OutIt**定义为**ostreambuf_iterator \<Elem, Tr> **。 函数返回** \* this**。
...
如果[fail](../standard-library/basic-ios-class.md#fail)为 **`false`** ，则第一个成员函数为某些临时对象调用**newpos =** [rdbuf](../standard-library/basic-ios-class.md#rdbuf) **->** [pubseekpos](../standard-library/basic-streambuf-class.md#pubseekpos)（*_Pos*） `pos_type` `newpos` 。 如果 `fail` 为 false，则第二个函数调用**newpos = >rdbuf** [pubseekoff](../standard-library/basic-streambuf-class.md#pubseekoff)（*_Off，_Way*）。 在任一情况下，if （ `off_type` ）**newpos = =** （ `off_type` ）（-1）（定位操作失败），该函数将调用**istr。**[setstate](../standard-library/basic-ios-class.md#setstate)（**failbit**）。 这两个函数都返回** \* 此**。
...
类卫士 {public：显式卫士（basic_ostream \<Elem, Tr>& _Ostr）; operator bool （） const; ~ sentry （）;};
...
如果[fail](../standard-library/basic-ios-class.md#fail) **`false`** ，则成员函数返回[rdbuf](../standard-library/basic-ios-class.md#rdbuf) **->** [pubseekoff](../standard-library/basic-streambuf-class.md#pubseekoff)（0， `cur` ， **in**）。 否则，将返回 `pos_type`(-1)。
