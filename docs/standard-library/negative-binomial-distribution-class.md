---
title: negative_binomial_distribution 类
ms.date: 11/04/2016
f1_keywords:
...
类模板描述生成用户指定积分类型的值的分布，或者如果没有提供，则类型**int，** 根据负双分分布离散概率函数进行分布。 下表链接到有关各个成员的文章。
...
属性成员`k()`并`p()`分别返回当前存储的分布参数值*k*和*p。*
...
有关负二项分布离散概率函数的详细信息，请参阅 Wolfram MathWorld 文章[负二项分布](https://go.microsoft.com/fwlink/p/?linkid=400516)。
...
结构param_type = 类型`<`negative_binomial_distributionresult_type>distribution_type;param_type（result_type k = 1，双 p = 0.5）;result_type k（） const;双 p（） const;
