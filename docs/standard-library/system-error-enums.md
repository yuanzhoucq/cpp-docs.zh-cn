...
|||  
|-|-|  
|[errc 枚举](#errc_enumeration)|[io_errc 枚举](#io_errc_enumeration)|  
  
##  <a name="a-nameerrcenumerationa--errc-enumeration"></a><a name="errc_enumeration"></a>  errc 枚举  
...
##  <a name="a-nameioerrcenumerationa--ioerrc-enumeration"></a><a name="io_errc_enumeration"></a>  io_errc 枚举  
 为 \<iostream> 中的错误条件提供符号名称。 可用于创建 [error_condition](../standard-library/error-condition-class.md) 对象，其用于与由 [ios_base::failure](../standard-library/ios-base-class.md#ios_base__failure)`code()` 函数返回的值进行比较。  
  
class io_errc { stream = 1 };  
  
### <a name="remarks"></a>备注  
 [std::make_error_code()](../standard-library/system-error-functions.md#make_error_code) 和 [std::make_error_condition()](../standard-library/system-error-functions.md#make_error_condition) 都是为此枚举重载的。  
  
 `ios_base::failure` 可以包含 `error_condition` 之外的错误代码类别。  
