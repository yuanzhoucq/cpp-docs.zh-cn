...
ms.workload:
- cplusplus
ms.openlocfilehash: bc3c1a2f2f1695e92d8ff6d0583ea74b500422ef
...
`Type``T`，`U`支持二进制文件的类型`operator-`接受的操作数的指定或推断类型。

`Left` 运算的左的操作数。 未专用化的模板采用 `Type` 类型的左值引用参数。 专用化的模板可完美转移推断类型 `T` 的左值和右值引用参数。

`Right` 操作的右操作数。 未专用化的模板采用 `Type` 类型的左值引用参数。 专用化的模板可完美转移推断类型 `U` 的左值和右值引用参数。
