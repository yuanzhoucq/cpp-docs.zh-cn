...
- **输出**。 一个输出迭代器`X`可以向前循环访问序列通过使用 + + 运算符，可以通过使用一次，编写元素和\*运算符。

- **输入**。 一个输入迭代器`X`可以向前循环访问序列通过使用 + + 运算符，并通过使用可以读取元素任意次数\*运算符。 可以使用 ++ 和 != 运算符比较输入迭代器。 递增输入迭代器的任何副本之后，没有其他任何副本可以安全地进行比较、取消引用或递增。

- **向前**。 一个向前迭代器`X`可以向前循环访问序列使用 + + 运算符和可以读取的任何元素或通过使用写入非常量元素任意次数\*运算符。 可以使用 -> 运算符访问元素成员，并使用 == 和 != 运算符比较向前迭代器。 可以创建向前迭代器的多个副本，其中每个副本都可以独立地取消引用和递增。 未使用对任何容器的引用进行初始化的向前迭代器称为 null 向前迭代器。 Null 向前迭代器的比较结果始终是相等的。

- 双向。 双向迭代器 `X` 可以替代向前迭代器。 您可以但是，还递减双向迭代器，在-`X`， `X`-，或 (`V` = \*`X`-)。 可以采用与向前迭代器相同的方式访问元素成员和比较双向迭代器。
