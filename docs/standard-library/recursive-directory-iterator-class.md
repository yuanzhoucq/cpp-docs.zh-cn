...
ms.locfileid: "72686807"
---
# <a name="recursive_directory_iterator-class"></a>recursive_directory_iterator 类

描述一个输入迭代器，该迭代器通过目录中的文件名进行排序，可能以递归方式降序到子目录。 对于迭代器 `X`，表达式 `*X` 的计算结果为类 `directory_entry` 的对象，该对象包装文件名和任何有关其状态的已知信息。
...
1. `stack<pair<directory_iterator, path>>` 的类型为的对象，在此处 `mystack` 用于处于阐释的目的，该对象表示要进行排序的目录的嵌套

1. `myentry` 此处 `directory_entry` 名为的对象，它表示目录序列中的当前文件名

1. 在此处 `no_push` 类型为**bool**的对象，它记录是否禁用了递归下降到子目录中

1. `directory_options` 类型的对象（称为 `myoptions`），用于记录在构造时建立的选项

@No__t_0 类型的默认构造对象在 `mystack.top().first` 有一个序列结尾迭代器，并表示序列末迭代器。 例如，给定目录 `abc` 项 `def` （目录）、`def/ghi` 和 `jkl`，代码如下所示：
...
将调用 `path("abc/def/ghi")` 和 `path("abc/jkl")` 参数的访问。 可以通过以下两种方式，通过目录子树限定排序：

1. 仅当使用其值 `directory_options::follow_directory_symlink` 的 `directory_options` 参数构造 `recursive_directory_iterator` 时，才会扫描目录符号链接。
...
|成员函数|描述|
|-|-|
|[长度](#depth)|返回 `mystack.size() - 1`，因此 `pval` 深度为零。|
|[disable_recursion_pending](#disable_recursion_pending)|在 `no_push` 中存储**true** 。|
|[版本号](#increment)|按顺序前进到下一个文件名。|
...
|[operator==](#op_eq)|仅当 `*this` 和*权限*都是序列结尾迭代器，或者两者都不是序列的结束迭代器时，才返回**true** 。|
...
该函数尝试推进到嵌套序列中的下一个文件名。 如果成功，它会将该文件名存储在 `myentry`;否则，它将生成序列末迭代器。
...
仅当 `*this` 和*权限*都是序列结尾迭代器，或者两者都不是序列的结束迭代器时，才返回**true** 。
...
第一个构造函数将生成序列末迭代器。 第二个和第三个构造函数在 `myoptions` `no_push` 和 `directory_options::none` 中存储**false** ，然后尝试将*pval*作为目录打开和读取。 如果成功，则它们将初始化 `mystack` 和 `myentry` 以指定嵌套序列中的第一个非目录文件名;否则，它们将生成序列末迭代器。

第四个和第五个构造函数的行为与第二个和第三个构造函数的行为相同，不同之处在于它们*首先将 `myoptions`* 默认构造函数的行为符合预期。
