...
为数值处理提供的 c + + 标准库数值算法具有自己的标头文件 [\<numeric>](numeric.md) ，并且函数对象和适配器是在 [\<functional>](functional.md) 返回布尔值的标头函数对象中定义的。 默认二元谓词是比较 `operator<`。 通常，排序的元素需小于比较元素，因此，给定任意两个元素，可以确定它们是等效的（即两者均不小于对方）或其中一个小于另一个。 这将导致在非等效元素之间进行排序。
...
|[=](algorithm-functions.md#equal)|逐个元素比较两个范围是否相等或是否在二元谓词指定的意义上等效。|
|[equal_range](algorithm-functions.md#equal_range)|在排序的范围中查找符合以下条件的位置对：第一个位置小于或等效于指定元素的位置，第二个位置大于此元素位置，等效意义或用于在序列中建立位置的排序可通过二元谓词指定。|
|[适合](algorithm-functions.md#fill)|将相同的新值分配给指定范围中的每个元素。|
|[fill_n](algorithm-functions.md#fill_n)|将新值分配给以特定元素开始的范围中的指定数量的元素。|
|[find](algorithm-functions.md#find)|在范围中找到具有指定值的元素的第一个匹配项位置。|
|[find_end](algorithm-functions.md#find_end)|在范围中查找与指定序列相同的最后一个序列，或在二元谓词指定的意义上等效的最后一个序列。|
|[find_first_of](algorithm-functions.md#find_first_of)|在目标范围中搜索若干值中任意值的第一个匹配项，或搜索在二元谓词指定的意义上等效于指定元素集的若干元素中任意元素的第一个匹配项。|
|[find_if](algorithm-functions.md#find_if)|在范围中找到满足指定条件的元素的第一个匹配项位置。|
|[find_if_not](algorithm-functions.md#find_if_not)|返回指示的范围中不满足条件的第一个元素。|
|[for_each](algorithm-functions.md#for_each)|将指定的函数对象按向前顺序应用于范围中的每个元素并返回此函数对象。|
|[for_each_n](algorithm-functions.md#for_each_n)||
|[带来](algorithm-functions.md#generate)|将函数对象生成的值分配给范围中的每个元素。|
...
|[make_heap](algorithm-functions.md#make_heap)|将指定范围中的元素转换到第一个元素是最大元素的堆中，其中排序条件可通过二元谓词指定。|
|[max](algorithm-functions.md#max)|比较两个对象并返回较大对象，其中排序条件可通过二元谓词指定。|
|[max_element](algorithm-functions.md#max_element)|在指定范围中查找最大元素的第一个匹配项，其中排序条件可通过二元谓词指定。|
|[merge](algorithm-functions.md#merge)|将两个排序的源范围中的所有元素合并为一个排序的目标范围，其中排序条件可通过二元谓词指定。|
|[min](algorithm-functions.md#min)|比较两个对象并返回较小对象，其中排序条件可通过二元谓词指定。|
|[min_element](algorithm-functions.md#min_element)|在指定范围中查找最小元素的第一个匹配项，其中排序条件可通过二元谓词指定。|
|[minmax](algorithm-functions.md#minmax)|比较两个输入参数，并按最小到最大的顺序将它们作为参数对返回。|
|[minmax_element](algorithm-functions.md#minmax_element)|在一次调用中执行 [min_element](algorithm-functions.md#min_element) 和 [max_element](algorithm-functions.md#max_element) 执行的操作。|
|[匹配](algorithm-functions.md#mismatch)|逐个元素比较两个范围是否相等或是否在二元谓词指定的意义上等效，并找到出现不同的第一个位置。|
...
|[replace](algorithm-functions.md#replace)|检查范围中的每个元素，并替换与指定值匹配的元素。|
...
|[范例](algorithm-functions.md#sample)||
|[search](algorithm-functions.md#search)|在目标范围中搜索其元素与给定序列中的元素相等或在二元谓词指定的意义上等效于给定序列中的元素的序列的第一个匹配项。|
|[search_n](algorithm-functions.md#search_n)|在范围中搜索具有特定值或按二元谓词的指定与此值相关的指定数量的元素。|
|[set_difference](algorithm-functions.md#set_difference)|将属于一个排序的源范围、但不属于另一排序的源范围的所有元素相并到一个排序的目标范围，其中排序条件可通过二元谓词指定。|
|[set_intersection](algorithm-functions.md#set_intersection)|将属于两个排序的源范围的所有元素相并为一个排序的目标范围，其中排序条件可通过二元谓词指定。|
|[set_symmetric_difference](algorithm-functions.md#set_symmetric_difference)|将属于一个而不是两个排序的源范围的所有元素相并为一个排序的目标范围，其中排序条件可通过二元谓词指定。|
|[set_union](algorithm-functions.md#set_union)|将至少属于两个排序的源范围之一的所有元素相并为一个排序的目标范围，其中排序条件可通过二元谓词指定。|
|[sort](algorithm-functions.md#sort)|将指定范围中的元素按非降序顺序排列，或根据二元谓词指定的排序条件排列。|
|[随机选择](algorithm-functions.md#shuffle)|使用随机数生成器重新排列给定范围中的元素。|
