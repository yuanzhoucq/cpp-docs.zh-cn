...
*比较*提供函数对象可比较两个元素值作为排序键以确定其相对顺序，在多重集合中的类型。 默认值是二元谓词 **less**\<Key>。
...
`Allocator` 表示存储的分配器对象封装有关多重集的分配和解除分配内存的详细信息的类型。 默认值为 **分配器 * * *\<密钥 >。*
...
[\<设置 > 成员](http://msdn.microsoft.com/en-us/0c2d57c0-173f-4204-b579-c5f06aad8b95)<br/>
