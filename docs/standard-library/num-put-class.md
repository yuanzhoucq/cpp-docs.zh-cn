...
`OutputIterator` 数值放置的函数写入其输出的迭代器的类型。
...
`_Iosbase` 指定的流，其中包含用于强调的输出和格式化的输出的标志 numpunct facet 的区域设置。
...
`dest` 寻址插入的字符串的第一个元素的迭代器。

`_Iosbase` 指定包含用于强调的输出和格式化的输出的标志 numpunct facet 的区域设置的流。
